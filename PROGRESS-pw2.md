...
- polaris `./nova.sh test src --strict-effects`: канон **37/0/18**, ветка pw2 даёт
  **КРАСНО: 17/20/18** (baseline main на ТОМ ЖЕ nova-polaris — чисто 37/0/18).
  Тот же класс, что №295 (мега-CU регрессия), но СИЛЬНЕЕ: весь `src`-пакет —
  ОДИН CU (~40 файлов), 20 CC-FAIL «passing nova_int to nova_str», байт-в-байт
  сигнатура `a_q3_println_debug_record`. Один файл (`src/auth.nv`) standalone на
  pw2 — чисто. Т.е. регрессия НЕ узкий 1-файловый мега-CU-артефакт, а порог по
  объёму/составу merged-CU (std 90+ файлов держится; polaris ~40 и conformance
  636 — ловят). **ЭТО ЛОМАЕТ ЗАЯВЛЕННЫЙ ГЕЙТ ОКНА, не только мега-CU интегратора
  — доложено владельцу/интегратору, push НЕ должен идти до решения по №295.**
