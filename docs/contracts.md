...
- [Verification attributes](#verification-attributes)
  - [`#verify`](#verify) — strict, compile error if unprovable
  - [`#unverified`](#unverified)
  - [`#pure`](#pure)
  - [`#must_verify` (removed)](#must_verify-removed--use-verify)
  - [`#trusted`](#trusted)
...
**Strict SMT verification.** The compiler encodes the function body
and all contracts as an SMT query. If the solver cannot prove the
contracts — unknown, timeout, or counterexample — compilation
**fails** with an error (no runtime fallback).
