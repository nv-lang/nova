...
- **Doc-comments публичного std (`///`, `//!`) — английский**: это пользовательский артефакт
  будущего релиза.
...
**Проверка (планируется):** `W_REDUNDANT_OF` — `Type.of(...)`, когда литерал `[...]`
дал бы ТОТ ЖЕ тип (избыточный `.of`). НЕ срабатывает, когда `.of` фиксирует тип,
который литерал не даёт (`Vec[u32].of(1,2,3)` легально). Связано с [[feedback-ctor-new-not-of]].
