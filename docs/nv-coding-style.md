...
- **`with_*` никогда не мутирует** — всегда возвращает НОВОЕ значение:
  `with_capacity(n)` (конструктор с параметром), `with_extension(e)`/`with_params(ps)`
  (копия приёмника с заменённым полем). Мутирующий вариант так не называть
  (мутаторы — `set_*` либо метод-свойство, см. ниже).
