...
- [`embed_str("path")` — один файл как `str`](#embed_strpath--один-файл-как-str)
...
## `embed_str("path")` — один файл как `str`
...
(фикстура `greeting.txt` содержит ровно `Hello, Nova!`, без завершающего
переноса строки; адаптировано из `spec_tests/conformance/d412e_embed_str.nv`.)

- `embed_str` — компайл-тайм интринсик-сестра `embed`: то же чтение ОДНОГО
  файла, но результат — `str`, а не `[]u8`. Контракт аргумента идентичен:
  только строковый литерал, путь относительно `.nv`-файла вызова, граница —
  package-root вызывающего; каждая диагностика, которую `embed` может дать
  на том же входе, переиспользуется дословно: файл не найден →
  `E_EMBED_NOT_FOUND`, путь ведёт на директорию → `E_EMBED_IS_A_DIR`
  («используй `embed_dir(...)`»), выход за package-root →
  `E_EMBED_OUTSIDE_PROJECT`, `\` в литерале → `E_EMBED_PATH_BACKSLASH`.
- Единственная дополнительная проверка, которой нет у `embed`: байты файла
  обязаны быть валидным UTF-8 (инвариант `str`). Невалидное содержимое —
  ошибка компиляции `E_EMBED_NOT_UTF8` с offset'ом ПЕРВОГО невалидного байта.
- Материализация идёт через ту же инфраструктуру интернированного
  `str`-литерала, что и любой рукописный `"…"` — ноль новых
  codegen-примитивов, та же нулевая копия из `.rodata`, что у `embed`.
- Имя `embed_str` зарезервировано так же, как `embed`/`embed_dir` — своей
  функцией или переменной его не затенить.
