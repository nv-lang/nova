...
> `p[i] = v`, `p ± i`, `p - q`, `p < q` (все сравнения порядка). С окна
> p-ptr (2026-08-05) формы записи отвергает и `nova check` — раньше они
> падали только на стадии сборки.
...
> **Вызовы методов через указатель** (`p.method()`) входят в модель D216 §5,
> но в bootstrap-компиляторе сейчас ненадёжны (вызов через указатель может
> тихо вернуть неверное значение) — дефект передан в работу и отслеживается.
> До исправления сначала читайте значение: `p.read().method()`.
...
авто-промоутит stack-значения в кучу. `&x` всегда даёт `*T` с
read-only-pointee; записываемый pointee — явный опт-ин кастом:
...
- `E_POINTER_OP_USE_METHOD` — выведенный операторный доступ (`*p`, `*p = v`,
  `p[i]`, `p[i] = v`, `p ± i`, `p - q`, сравнения порядка); используйте
  intrinsic-методы (`.read()` / `.write()` / `.read_at()` / `.write_at()` /
  `.offset()` / `.dist()`)
