...
> **Почему `*mut uninit`, а не просто `*uninit`.** Сегодня голый `*uninit T`
> тоже разрешает запись (запись в неинициализированную память — это
> инициализация, а не мутация), поэтому обе формы компилируются.
> `*mut uninit T` — форма, переживающая исправление, отслеживаемое как
> `[M-174.5-pointer-ops-methods]`: после него голый `*uninit T` станет
> только для чтения.
