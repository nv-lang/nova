...
mono-ресивер), и аналогично `-> FilterIter[Self,U]` / `-> FilterMapIter[Self,U,V]`.
Это опускает повторяющийся тип ресивера в позиции повторного вложения;
семантика идентична полному написанию типа ресивера. Поддержка компилятора для
...
оно эмитится как файл-скоупный статический синглтон вместо env-бокса +
