...
## Когда контракты проверяются: во время компиляции и во время исполнения

У контракта две линии защиты, и они независимы.

**Во время исполнения — всегда.** `requires`, `ensures` и `invariant`
проверяются при работе программы. Это происходит без всяких флагов и отключить
это нельзя: нарушенный контракт остановит программу с диагностикой.

**Во время компиляции — по запросу.** Компилятор умеет ДОКАЗЫВАТЬ контракт
через SMT-решатель. Доказанный контракт даёт единственную выгоду: его
рантайм-проверку можно **убрать** из готовой программы — она заведомо истинна,
проверять нечего.
...
Доказательство — дорогая работа, и платить за неё при каждой сборке не нужно:
на обычном коде без контрактов оно не находит ничего, а время занимает.
Включайте его там, где оно даёт результат — в релизных сборках, где важна
скорость готовой программы, и когда хотите убедиться, что контракт действительно
доказуем, а не просто «пока не падал».

### Что это значит для безопасности

**Ничего.** Флаг влияет на скорость компиляции и на скорость готовой программы,
но не на её корректность:

- **без `--verify`** — ни один контракт не доказан, значит **все** проверки
  остаются в программе и выполняются при работе;
- **с `--verify`** — доказанные проверки убраны как заведомо истинные,
  недоказанные остались.

То есть отсутствие флага делает программу медленнее, но не менее безопасной.
Обратное неверно: флаг не может «пропустить» нарушение — то, что не доказано,
проверяется при исполнении.
## Содержание
