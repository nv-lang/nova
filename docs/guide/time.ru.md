...
**Wire остаётся int** (см. «Почему typed-wire не отгружен» ниже) — весь
user-facing surface, тем не менее, **полностью typed** и **полностью мокабелен**,
включая `Monotonic` (Plan 175).
...
## Почему typed effect-wire не отгружен (архитектурная находка)
...
| clock injection / mock | monkey-patch/`synctest`-bubble | нет std (crates) | `@sinonjs/fake-timers` (monkey-patch) | `Clock`-DI виральна, молча падает на real-clock если забыли пробросить | DI виральна | **нет вообще** | `Clock`-протокол, `TestClock`, но виральна (`<C: Clock>` через все сигнатуры) | **handler лексически ограничен областью видимости, ambient, не вирусит сигнатуры** (D11/D61) |
