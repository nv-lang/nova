...
> гейтится и по OS, и по slow — `_slow` снимается ДО OS-проверки). Полные наборы
> коммитятся как `*_conformance_slow.nv`. Отложен (`[M-156-slow-subtree-dir]`) лишь
> каталог-вариант `slow/` + сентинел `_slow.toml` для медленных folder-module —
> добавится аддитивно, когда появится первый такой тест.
