...
Используй пороги из [D298](../spec/decisions/09-tooling.md#d298--test-suite-time-budget):
- `total_ms ≥ 3000` (compile + run) → переименовать в `_slow.nv`
- `run_ms ≥ 2000` (IO/sleep/stress) → переименовать в `_slow.nv`
- `compile_ms ≥ 500` (крупные mono-инстанции) → переименовать в `_slow.nv`

Если тест медленен только из-за большого N — создай **fast-variant** (см. D298 §Fast-variant).

Измерить: `nova test --results-file timings.json`, затем проанализировать JSON.
Профиль текущего suite: `docs/research/12-test-suite-profile-2026.md`.
...
Тесты stdlib идут в **`nova_tests/<тема>/`** — точно так же как тесты языковых фич. Это единственный верифицированный путь.

```
nova_tests/
├── str/          ← тесты str-методов (char_at, split, conversions…)
├── plan91/       ← тесты stdlib API (sort, text methods…)
├── plan103_*/    ← тесты sync-примитивов (Mutex, Channel…)
└── plan91_12/    ← тесты net (TcpListener, UdpSocket…)
```

Тест в `nova_tests/` использует stdlib как пользователь — импортирует типы, вызывает методы, проверяет контракт снаружи. Это и есть правильный подход.

**Inline тесты в `std/**/*.nv`** (`test "..."` блоки прямо в файлах реализации) существуют в некоторых файлах (`std/collections/hashmap.nv`, `std/time/duration.nv` и др.), но механизм их запуска (`nova test --include-stdlib`) **никогда не верифицировался на практике**. Не добавляй inline тесты в std-файлы — пиши тесты в `nova_tests/`.
