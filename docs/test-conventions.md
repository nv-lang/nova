...
**Compile time не является критерием** — на Windows/clang floor составляет ~15-35 с на любой тест (nova_rt.h + GC headers). `compile_ms` одинаков у `basics/literals` и у тяжёлого concurrency-теста.

Критерий — **характер теста**, не абсолютные миллисекунды:

| Признак | Действие |
|---|---|
| `run_ms` значительно выше медианы (~1.4 с) — intentional sleep/stress | → `_slow.nv` |
| Имя содержит `stress`, `bench`, `perf` | → `_slow.nv` |
| Тест таймаутит при 60 с лимите | → `_slow.nv` |
| Тест медленен только из-за большого N, но N можно уменьшить | → создай fast-variant (малый N) + оригинал → `_slow.nv` |
| Тест медленен только из-за compile time — функциональный тест | → оставить в дефолте |

Измерить: `nova test --results-file timings.json`, затем проанализировать JSON.
Профиль текущего suite: [docs/research/12-test-suite-profile-2026.md](research/12-test-suite-profile-2026.md).
...
Запуск: `nova test --include-stdlib` — stdlib-файлы проходят через тот же pipeline что и `nova_tests/` (folder_module, slow-lane, EXPECT-маркеры).
