...
### Два места для тестов

Nova поддерживает два места для тестов:

| Место | Когда использовать | Как запускать |
|---|---|---|
| `nova_tests/` | Тесты языковых фич, планов, интеграционные | `nova test` |
| `std/**/*.nv` (inline) | Тесты самого stdlib-модуля, рядом с кодом | `nova test --include-stdlib` (флаг реализован, но практически не использовался — уточни статус перед применением) |

**Inline тесты в std** — `test "..."` блоки прямо в файле реализации (`std/collections/hashmap.nv`, `std/time/duration.nv` и др.). Используются когда тест неотделим от реализации и проверяет внутренние инварианты модуля. Module path файла остаётся `module collections.hashmap` — тест живёт в том же namespace.

Если тест проверяет поведение stdlib **снаружи** (как пользователь) — он идёт в `nova_tests/`, а не inline.
