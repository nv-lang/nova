...
- 🚫 **Никакого `git stash`.** `.git` общая для всех worktree → stash/refs/reflog глобальны → коллизия и
  ...
  **commit + reset** в своей ветке. *(Внимание: `compiler-conventions.md` в разделе про верификацию ещё
  упоминает `git stash` — это устаревший текст, правило выше его перекрывает.)*
