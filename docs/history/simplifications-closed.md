...
- **Что упрощено:** добавление @clone() в HashMap (которая вызывает
  `key.hash()` где K — generic с Hash bound) даёт CC-FAIL: erased
...
- Built-in primitive vtables (nova_int, nova_bool, nova_byte, nova_f64,
  nova_str) — Hash + Compare where applicable.
...
- **Scope-decision C для 0.1** (2026-05-27) — Plan 91 + Plan 83.12 + Plan 103.x
  co-planned под общим shipping gate. Раньше Plan 91 декларировал
  «net/sync — Plan 18, 0.2+», Plan 83.12 — «backend без TCP hypothetical»;
  противоречие не было разрешено. C отражает фактический статус: 103.x уже
  в main (sync едет в 0.1 без правок), 83.12 — последний крупный модуль.
  Альтернатива A («91 как написан, net в 0.2») отвергнута — слабый
  backend-claim. Альтернатива B («91 расширяется, поглощает net») отвергнута
