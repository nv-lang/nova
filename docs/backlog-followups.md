...
- **[M-vec-elem-type-mismatch-silent]** Compiler accepted passing a `Vec[int]` argument to a parameter typed `Vec[u32]` (and `int` to `u32`) with NO type error — the element-type mismatch surfaced only as a runtime OOB (see fixed `[M-152.5-collation-conformance-u32-overflow]`). The checker should reject `Vec[int]`↔`Vec[u32]` (and scalar `int`↔`u32`) at call boundaries, or require an explicit cast. Priority: P1 (silent memory-reinterpretation class of bug). Likely in arg-coercion: `Vec[T]` element type not compared, or numeric widening treated as assignable both directions.
