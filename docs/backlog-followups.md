...
- **[M-172.1-U1-lib-import-needs-U4]** Plan 172.1 U.1.3b/U.1.6 (gate, discovered 2026-06-20 via impl-разведка). Полное удаление `load_builtins` (хардкоженный `include_str!`-список 10 std-файлов) для **библиотечных** пакетов (`std.runtime.sync`/`std.net`/`std.ffi.cstr`/`std.runtime.raw_mem`) ЗАВИСИТ от **U.4** (один inference). Корень: корпус импортит `sync` в ~0 тестов — он всегда шёл registry-only через `load_builtins` (только `extern`-сигнатуры), поэтому **тела** Nova-body lib-функций (`Once.try_start` и др.) НИКОГДА не кодогенерились. При инлайне (через `import`) вскрывается codegen-re-derive: `infer_expr_c_type` выводит тип возврата метода сам и врёт (`self.try_start_won()` декларирован `-> bool`, выводится `nova_int` → `if condition must be bool`). Чинить патчем в codegen ЗАПРЕЩЕНО (§0/§1 — «правило в codegen = долг к переносу в чекер»; «сокращать фрагментацию, не углублять»). Корректный фикс — U.4 (typed IR: чекер резолвит тип метода, codegen читает). Минимальный-prelude + char/str.from часть (U.1.3a) от U.4 НЕ зависит и проверена (no-import surface PASS). Priority: P1 (архитектурный, soundness). Эксперимент-доказательство откатан; реализация — в порядке U.2→U.3→U.4, затем U.1.3b/U.1.6.
