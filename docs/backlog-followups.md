...
- **[M-152.5-collation-conformance-u32-overflow]** `nova_tests/plan152_5/collation_conformance.nv` (CollationTest_SHIFTED order check) RUN-FAIL: `array: index 12884901890 out of bounds for length 4` (= 3·2³²+2, a u32→int overflow in the conformance order loop). **Pre-existing** on HEAD (verified independent of Plan 91.18 Collator namespace change). Rolls the whole `plan152_5` folder-module to FAIL. Priority: P1 (masks the module's green state). Likely u32 arithmetic in the conformance pair-index decode wrapping into a huge index.
