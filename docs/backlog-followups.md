...
- **[M-180-serde-attributes]** ✅ CLOSED — infra (Plan 180 Ф.6, D382, 2026-07-06): `serde_attrs: Vec<SerdeArg>` on `TypeDecl`/`SumVariant`/`RecordField`; `parse_serde_attr` at all three positions; unknown key → E_SERDE_BAD_ATTRIBUTE (not silent). Only `tag`/`content`/`untagged` consumed; field-customization keys → `[M-180-serde-field-attributes]`.
...
- **[M-104.10-highlight-lexical-occurrences]** Ф.15 `documentHighlight` (`document_highlight.rs`) резолвит scope символа семантически (тот же Ф.7-резолвер `resolve_highlight_scope`), но within-scope скан вхождений — текстовый (word-boundary + skip строк/комментариев + `.`-member-исключение для локалей), НЕ per-occurrence resolve каждого кандидата до decl-`Span` (зеркалит остаток rename `[M-104.10-rename-full-resolve]`). Остаток (bounded, degrade-gracefully): (a) record/named-arg field-label одинакового написания (`Point { x: … }` при highlight локали `x`) → подсвечен как read; (b) `..rest` array-pattern slice-bind (нет AST-span) → read вместо write. Scope-корректность точная; это read/write-классификация на текстовых совпадениях. Follow-up: полный per-occurrence resolve при выкатке общего name-resolution pass. Файл: `nova-lsp/src/document_highlight.rs`. Priority: P3 (IDE-качество, не корректность).
- **[M-104.10-expr-types-coverage]** `ModuleEnv.expr_types` (opt-in per-expression type map, `check_module_with_expr_types`) реализована на РЕАЛЬНОЙ инференции чекера (`infer_expr_type` + семантический канал `resolved_types_buf`), покрывает весь plan-required набор. Остаток НЕ записываемого (bounded, degrade-gracefully): (a) generic instance method-chain returns (`v.map(f).filter(g)` mid-chain) — Call-арм `infer_expr_type` расцеплён с общей instance-return-инференцией (172.1); (b) non-primitive `TupleLit` — buf аннотирует лишь all-primitive кортежи, у `infer_expr_type` нет `TupleLit`-арма; (c) generic-instance `RecordLit` / element-типы с несвязанным type-param — gated by design; (d) [Ф.6 наблюдение] range-литерал local-binding (`ro r = 0..=5`) — тип `r` не пропагируется (пишутся только int-операнды `0`/`5`), member-hover `r.start` на таком локале → None (через `Range`-параметр / user-record local — работает). Follow-up: полный независимый expr-type walker (Ф.2b) при реальной IDE-потребности; расширить buf/`infer_expr_type` на (a)/(b)/(d) точечно. Файл: `compiler-codegen/src/types/mod.rs` (`record_expr_type_ide`). Priority: P3 (IDE-качество, не корректность компилятора).

## Plan 104.10 Ф.16 — foldingRange plain-comment remainder (2026-07-04)

- **[M-104.10-folding-plain-comments]** Ф.16 `foldingRange` (`folding_range.rs`) сворачивает регионы из AST-span'ов (fn/type-тела, вложенные `{}`, import-группы, multi-line doc-comment'ы). Единственный пробел (bounded): run'ы обычных `//`-line-комментариев НЕ сворачиваются — лексер их отбрасывает (`skip_line_comment`), они не доходят до AST; сворачиваются только doc-comment'ы (`///`/`//!`, склеенные лексером в `DocBlock` со span'ом). Nova не имеет block-комментариев (`/* */`). Follow-up: token-stream side-channel для плоских comment-run'ов при реальной потребности. Файл: `nova-lsp/src/folding_range.rs`. Priority: P3 (IDE-качество, не корректность).

## Plan 104.10 Ф.11 — organizeImports name-scan remainder (2026-07-04)

- **[M-104.10-organize-imports-namescan]** Ф.11 `source.organizeImports` (`organize_imports.rs`) удаляет unused-импорты / прунит selective-item'ы / сортирует; re-export (`export import`) сохраняется verbatim (D29/D288). «Used»-детект — **текстовый** whole-word name-scan (план допускает «expr_types/name-scan»), НЕ type-aware. Bounded-остаток: консервативный false-**keep** — если введённое имя совпадает с идентификатором, встречающимся в теле по иной причине (одноимённое поле/метод/локаль, напр. `.Foo()` / field `Foo`), импорт `.{Foo}` сочтётся used и сохранится, даже если реально не используется. Направление безопасное — used-импорт НИКОГДА ошибочно не удаляется; изредка сохраняется реально-unused. Follow-up: точный анализ через `expr_types`/символьную резолюцию при реальной потребности. Файл: `nova-lsp/src/organize_imports.rs`. Priority: P3 (IDE-качество, не корректность).

## Plan 104.10 Ф.9 — inlay-hint config granularity remainder (2026-07-04)

- **[M-104.10-inlay-config-granularity]** Ф.9 inlay hints (`inlay_hints.rs`) даёт type-hints (`ro x = expr` → `: T` из Ф.2 `expr_types`) и parameter-name-hints (`foo(1,2)` → `a:`/`b:` из резолвнутого callee), оба toggle'аются через `InlayHintConfig{type_hints,parameter_hints}` (default on) из `initializationOptions`/`didChangeConfiguration` (`nova.inlayHints.{typeHints,parameterHints,enable}`). Остаток (bounded, degrade-gracefully): (a) гранулярность — только два headline-toggle + master `enable`; тонкие rust-analyzer-ручки (hints лишь для литеральных аргументов, hide-single-param, closure-return hints, макс. длина метки, chaining-hints) НЕ экспонированы; (b) type-hints для **call-return** типов (`ro x = add(1,2)`) отсутствуют — IDE resolve-путь не гоняет `number_exprs`, поэтому ExprId-keyed семантический канал не приджойнен (пере-использует `[M-104.10-expr-types-coverage]`, НЕ дефект Ф.9); литеральные/ident-биндинги (синтаксическая инференция) работают. Никогда неверный hint — только отсутствие. Follow-up: расширить конфиг-схему + Ф.2b full expr-walker при реальной потребности. Файл: `nova-lsp/src/inlay_hints.rs`. Priority: P3 (IDE-качество, не корректность).

## Plan 104.10 Ф.10 — full semantic tokens scope-approximation (2026-07-04)

- **[M-104.10-semantic-tokens-scope]** Ф.10 full semantic tokens (`semantic_tokens.rs`) расширяет узкий Plan 123.5.2-producer (был только cached-`@field`, legend `[PROPERTY]`) до **полного pass'а**: legend расширен до 13 token-types (namespace/type/typeParameter/parameter/variable/property/enumMember/function/method/keyword/comment/string/number) + 3 modifiers (declaration/readonly/**cached**); каждый лексем токенизируется. Архитектура: полнота из **лексера** (все keyword/литералы/doc-comment/идентификаторы, работает даже при type-error `env=None`), точность из **AST** (override-map: декларации fn/type/param/field/variant/generic/const + declaration-модификатор, per-fn scope-walk параметр-uses → `parameter`, typeref-позиции → `type`/`typeParameter`) + emission-time name-set/лексер-контекст fallback (member перед `(` → `method`, иначе `property`; bare перед `(` → `function`/`type`/`enumMember`; и т.д.). cached-`@field` подсветка сохранена **как частный случай модификатора** (все `@field` reads → `property`, cache-eligible дополнительно несут `cached`|`readonly`). Delta (`semantic_tokens_delta.rs`) не тронут — producer отдаёт полный вектор, edit-script прежний. Остаток (bounded, degrade-gracefully, НИКОГДА не «нет токена» — лексер всегда даёт ≥`variable`/`keyword`): (a) **scope-approximation** — параметр-use матчится по имени параметра enclosing-fn БЕЗ отслеживания nested-shadowing (closure-param / `let`, переиспользующий имя параметра, всё ещё репортится как `parameter`); (b) идентификаторы внутри interpolated-string `${…}` покрыты окружающим `string`-токеном (лексер не сплитит интерполяцию); (c) внутренности сигнатур effect/protocol-методов не walk'аются для `declaration`-модификатора; (d) первый сегмент runtime-пути в expression-позиции (`std.collections.X`) вне import-строки классифицируется как `variable`, не `namespace` (без модульного set'а). cached-детект гоняет field-cache pipeline на свежем parse src (как Plan 123.5.2), чтобы `cached`-модификатор остался byte-for-byte-совместим. Follow-up: полноценная scope/name-resolution (rust-analyzer-стиль) + сплит интерполяции при реальной потребности. Файл: `nova-lsp/src/semantic_tokens.rs`. Priority: P3 (IDE-качество, не корректность).

## Plan 104.10 Ф.20 — run-test lens filter substring remainder (2026-07-04)

- **[M-104.10-runtest-filter-substring]** Ф.20 codeLens (`code_lens.rs`) даёт три навигационных линзы: run-test (`▶ Run test` над `test "…"` → серверный `nova.runTest` → спавн `nova test <file> --filter <name> --quiet`), references (`N references` над `fn`/`type` из Ф.12 `ReferencesIndex`, счётчик совпадает с `textDocument/references`) и implementations (`N implementations` над `protocol` из Ф.19 AST-скана). Остаток (bounded, degrade-safe): run-test передаёт `--filter <name>` — **подстрочный** матч по display-name, не точный. Тест, чьё имя является подстрокой имени другого теста в том же файле, при клике запустит и «надмножество» тестов. CLI имеет `--filter-from <file>` (точный display-name матч, по строке на тест), но требует генерации temp-файла и знания полного display-name (может включать path-префикс). Follow-up: exact-name флаг в `nova test` ИЛИ генерация temp filter-файла из линзы. Деградация безопасна — запуск надмножества, не пропуск (тест всегда прогонится). Файлы: `nova-lsp/src/code_lens.rs` (`run_test_lens`), `nova-lsp/src/server.rs` (`run_nova_test`). Priority: P3 (IDE-удобство, не корректность).

## Plan 174 — Language & FFI features (umbrella; 2026-07-06 заход 174.2/174.1/174.5)
...
- **[M-174.1-parse-engine-structural]** (NEW, OPEN) Живой остаток 174.1: `str.@parse_uint` + `parse_prim.nv` (generic вариант-B try_from/parse per D310) + удаление codegen-хардкода (routing в Nova-движок → typed ParseIntError/ParseFloatError/ParseBoolError/ParseCharError вместо flat-string) + `T.parse(s,radix)`-поверхность + float-канон (§4.1). Gate: координация 172.1-hardcode-зона (`emit_c.rs` parse `:~29234`/`:~29290`/`:~40390`) × 177 Ф.2b. Priority: P2.
...
## Plan 172.5 — In-out `ref`-параметры (core landed 2026-07-06)
...
## Plan 179 Ф.2 — brotli decode C-FFI (landed 2026-07-06)

- **[M-179-brotli-vendor-lib]** ✅ ЗАКРЫТ (2026-07-06). google/brotli v1.2.0 **декодер** (commit `27cc9fe9`, MIT) собран однократно из исходников (MSVC x64 `/MT /O2`, 12 TU: `common/*` + `dec/*`) и вендорен **headers+lib без исходников** (стиль libuv): `compiler-codegen/nova_rt/brotli/include/brotli/*.h` + `compiler-codegen/nova_rt/brotli/lib/libbrotlidec.lib` (tracked, `!`-негейт `*.lib` в `compiler-codegen/.gitignore`) + build-cache `target/brotli-cache/libbrotlidec.lib` (+version.txt). `brotli_decode(data, max_output)` = `std/encoding/compress/{ffi.nv,brotli.nv}` поверх шима `nova_rt/brotli_shim.{h,c}`; http `Content-Encoding: br` распаковывается (`[M-178-autodecompress-br]` закрыт), `Accept-Encoding: gzip, deflate, br`.
...
## Plan 183 Ф.2 — доп. компиляторные находки, промотированы из журнала плана в Ф.5 (2026-07-06)

Три дефекта ниже упоминались только как проза в «Заход 2» `docs/plans/183-net-rework.md`
(митигированы в коде net2-теста, не блокируют план) — Ф.5 даёт им отдельные маркеры для
видимости владельцу вне контекста плана 183.
