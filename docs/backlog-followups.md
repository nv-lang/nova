...
- **[M-scalar-nonliteral-narrowing-not-enforced]** Per spec D54 + D227 (`03-syntax.md:2342`: «неявное сужение … запрещённое для не-литералов, как `as`-cast narrowing D54»), narrowing a **non-literal** wider int into a narrower int must require explicit `as`. Currently the compiler enforces this for **literals** (`ro b u8 = 260` → `E_LIT_OUT_OF_RANGE`) and for **array-element** narrowing (`E_ARRAY_ELEM_NARROW`, Plan 154.1), but NOT for scalar **binding** or **function-argument** boundaries: `ro c int = 30; ro a u8 = c` and `take_u8(c)` both compile silently (verified 2026-06-19) — runtime truncation without `as`, a spec violation. **Large blast radius:** std pushes `int` values into `Vec[u8]`/`[u16]`/`[u32]` WITHOUT `as` pervasively (~375 `push` sites — buffers/codecs/crypto/unicode), so strict enforcement needs either a std migration (add `as`) or an explicit spec exception for narrowing where the value provably fits / for fn-arg coercion. Decision + likely a dedicated plan required before fixing. Priority: P1 (soundness/spec-conformance), but gated on the std-migration scoping decision. NOT the same bug as `[M-generic-arg-type-mismatch-silent]` (that is whole-container pointer reinterpretation; this is scalar value truncation).
