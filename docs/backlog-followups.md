...
- **[M-D282-ffi-abi-type-list]** D282 (extern "C" fn, `08-runtime.md:8255`) rule 2 **C-ABI-тип-лист занижен/неверен** vs реального использования (`std/net/ffi.nv`). Сейчас: «C-нативные: `int`, `u8`-`u64`, `bool`, `*T`, `*()`, `CStr`; Option/Result/**str** — ABI mismatch». По факту: (a) **туплы** (анон/именованные) из C-ABI-типов используются — `extern "C" fn socket_addr_parse(s str) -> (int, CSocketAddr)` (ffi.nv:32) — это C-struct by-value, C-ABI-совместимо; в списке нет. (b) **`str`** (`{ptr,len}` value-record, D139) — C-ABI-совместим (POD-struct) и используется (`socket_addr_parse(s str)`), но D282 ошибочно числит как mismatch. (c) **value-records / C-структуры** (`CSocketAddr`/`CTcpStream`/`CUdpSocket`/`CTcpListener`) используются, не в списке. (d) скаляры минимальны — нет `f64`/`f32`/`i8`-`i64`/`char`. (e) `Option[*T]` via NPO — C-совместим. **Фикс (sign-off 2026-06-22):** переписать D282 rule 2: C-ABI-совместимо = скаляры (`int`/`i*`/`u*`/`f*`/`bool`/`char`) + raw-ptr (`*T`/`*()`/`CStr`) + `Option[*T]` (NPO) + **value-records и туплы (анон/именованные) рекурсивно из C-ABI-типов** (by-value C-struct) — в **параметрах И возврате**. Исключения: GC-типы (`Vec`/heap-record), closures-with-env, generic tagged unions (`Option[non-ptr]`/`Result`/sum). Смежно: ABI-тег у `*fn`-типа (`*extern "C" fn` для C-callback). Priority: P2 (spec догоняет реальность; `08-runtime.md` не править в одиночку — координация 91.12/172/138.5).
