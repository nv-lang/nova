...
**Структура `docs/`** (карта — [`docs/README.md`](../../README.md)): `docs/guide/` — пользовательские
гайды (публикуются на nv-lang.org, синк сайта тянет ТОЛЬКО отсюда — `www/site/scripts/sync-decisions.mjs`);
`docs/dev/` — конвенции разработки, процесс, промпты для агентов (этот файл — один из них); на сайт
не попадает никогда. `docs/plans/` — планы, отдельно, статус пофайлово (см. §2 ниже). Кладя новый
документ в `docs/`, сразу решай — гайд это для пользователя языка или внутреннее для контрибьютора.
...
- **Plan 274 — `novac` (бренд — **Carina**, релиз-цель Nova Carina 1.0), компилятор Nova на Nova** (зонтик, проектный):
...
### Сборка вне главной репы — БЕЗ КОПИРОВАНИЯ

**Worktree с 2026-08-14 не требует НИЧЕГО** (реестр №650): тулчейн сам находит
vcpkg главного дерева через собственный `.git`-файл worktree и сам материализует
libuv-сабмодуль, печатая строку о каждой подмене. Env-блок ниже для worktree
НЕ нужен; если свежее дерево не собирает `examples/basics/hello.nv` — это
регресс №650, а не повод возвращать ритуал.

**Пакетные репы (nova-http/tls/polaris/compress) — переменными.** У них нет
`.git`-файла worktree и нет сабмодуля — автоподмена недоступна по построению.
**НИЧЕГО НЕ КОПИРОВАТЬ.** Блок ниже копируется целиком (замени `<main>` на путь
к репе `nova`):
