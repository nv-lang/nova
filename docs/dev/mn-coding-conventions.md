...
**НОРМА.** `nova_throw_cancel` / cancel-unwind исполняется ТОЛЬКО когда есть
активный fail-frame (`_nova_fail_top != NULL`). Код, доставляющий cancel
(`nova_sched_wake` → resume → throw), обязан гарантировать этот инвариант
установкой §10. `abort("cancel-throw outside any supervised scope")` — не
«обработка ошибки», а сигнал нарушенного порядка unwind: чинить нужно доставку,
а не глушить abort.

**ПОЧЕМУ.** `effects.h` (~431-466): если `_nova_fail_top == NULL` в момент
cancel-throw — `abort()`. Это структурный маркер гонки cancel-wake vs fail-frame
restore (boehm-eager-cost, spawnctx-волна). Симптом-обманка: выглядит как
runtime-hang/OOM, а на деле — SIGABRT из-за перепутанного порядка.

**ПЛОХО**
```c
void deliver_cancel(mco_coro* co) {
    resume(co);
    nova_throw_cancel("scope cancelled");   /* если fail_top==NULL → abort() */
}
```
...
**ПРОВЕРКА.** grep stderr на `cancel-throw outside any supervised scope`; ноль
