...
## ГДЕ ЖИВУТ ТЕСТЫ — ПРОСТАЯ СХЕМА (решение владельца 2026-08-08)

Требование владельца дословно: «нужно минимальное количество инвариантов по
системе тестирования, сейчас она очень сложная: это мы запускаем, это не
запускаем, это мы помним, это мы не помним». Аудит нашёл **девять** мест
хранения тестов. Оставляем **два живых**, всё остальное перестаёт быть местом
для тестов.

### Два живых места — и всё

| Куда | Что кладём | Кто гоняет |
|---|---|---|
| `spec_tests/conformance/` | язык, диагностика (`neg/`), рантайм и конкурентность (`standalone/`), регресс-фикстуры закрытых И открытых дефектов | авторитетный гейт |
| `std/src/<модуль>/` | `*_test.nv` пиром рядом с модулем; негатив-компиляции — `neg/` | `nova test std` |

**Правило одной строкой: тест лежит там, где его гоняют. Больше нигде тестов нет.**

### Что перестало быть местом для тестов

- **`nova_tests` → `nova_tests.old`** (885 файлов) — архив, не гоняется ничем,
  кандидат на удаление после ревизии «не осталось ли ценного». Имя само отвечает
  на вопрос «это запускается?» — раньше это приходилось помнить.
  В `nova_tests/` остались ТОЛЬКО два несущих для CI входа, и это не тесты в
  нашем смысле: `contracts/**` (294, вход `contracts-z3.yml`) и `doc/fixtures/**`
  (28, вход `nova-doc.yml`). Пути в воркфлоу поэтому не менялись.
- **`docs/plans/repro` и `docs/plans/repros`** — распускаются. Три пробы, на
  которые ссылались НОРМАТИВНЫЕ документы, перенесены в
  `spec_tests/conformance/standalone/`; C-контроль переехал в
  `scripts/tools/libuv-control-experiment.c`. Остальные — разобрать пофайлово:
  дефект закрыт → фикстура в conformance; дефект открыт → фикстура с
  зафиксированным ожиданием; ни то ни другое → удалить (история живёт в отчёте
  окна и коммите).
- **`spec_tests/fixtures/known_red`** — удалён: обход раннера не видел его
  НИКОГДА (`is_fixture_dir`), это был мёртвый груз.

### Правило для проб (уточнено аудитом — важная поправка)

Прежняя формулировка «коммитящаяся проба обязана исполняться раннером» неполна.
**Нельзя требовать, чтобы проба была зелёной**: проба ОТКРЫТОГО дефекта обязана
краснеть, иначе она не проба. Правильно так:

> У коммитящейся пробы обязано быть **зафиксированное ожидание**. Дефект закрыт →
> зелёная фикстура в conformance. Дефект открыт → фикстура с инвертирующим
> маркером в лейне, который дефолт-гейт не гоняет, а `--full` гоняет — тогда её
> краснота зафиксирована как ОЖИДАЕМАЯ, а **позеленение** (дефект тихо
> починился либо проба протухла) становится видимым провалом. Ожидания нет —
> не коммитится.

### Нормативные документы

Нормативный документ (`docs/dev/**`, `spec/**`) может ссылаться **только на
исполняемый гейтом артефакт** либо на кейс-стади в `docs/dev/cases/`. Ссылка на
неисполняемую пробу — дефект документа: она гниёт молча (реестр №454).
## Тест или bench?
...
## Многофайловая фикстура обязана называть ПОРЯДОК файлов, а не только форму

**Правило (2026-09-18, выведено из разбора №761 интегратором и окном Карины).**
Если фикстура состоит из нескольких файлов ОДНОГО модуля, её описание обязано
назвать, какой файл сортируется раньше и почему это важно для предмета. Клетка,
не называющая порядок, зелена или красна **от того, как назвали файлы**, а не от
того, что чинилось.

**Почему это не педантизм.** Пиры модуля обрабатываются ОТСОРТИРОВАННЫМИ
(`nova-cli/src/main.rs`, `files.sort()`), то есть порядок задаётся ИМЕНАМИ, а имена
в фикстуре выбирают не думая — `a.nv`, `b.nv`, `use.nv`. Значит фактор,
управляющий поведением компилятора, оказывается выставлен случайно и невидимо.

**Замер, из которого правило родилось.** Дефект №761 флипается местом объявления
типа: в первом по сортировке файле модуля сборка чистая, в седьмом — три ошибки C.
При этом ЧЕТЫРЕ минимальные пробы интегратора (потребитель раньше объявления; два
потребителя в разных файлах; те же формы через методы; newtype в арме суммы) дали
PASS — в том числе та, где порядок выставлен в «плохую» сторону. То есть
порядок — фактор НЕОБХОДИМЫЙ, но не достаточный, и обе стороны этого выяснились
только потому, что порядок назвали ЯВНО и стали им управлять.

**Как писать.** В шапке фикстуры — строка вида «`a_early.nv` сортируется раньше
`b_late.nv`; объявление во ВТОРОМ, потребитель в ПЕРВОМ». Обратный порядок —
ОТДЕЛЬНАЯ клетка контролем: она обязана быть зелёной и до, и после починки, иначе
фикстура судит сборку целиком, а не предмет.

**Чего это правило не требует:** переименовывать существующие многофайловые
фикстуры. Оно применяется к новым и к тем, что правятся; старые называют порядок
тогда, когда до них дойдёт своя работа.
