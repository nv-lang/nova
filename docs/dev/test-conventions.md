...
Семь правил, выведенных из фактического профиля багов 221.x (семьи per-position,
...
6. **Матрица ОСЕЙ ИСПОЛНЕНИЯ для рантайм-механизмов** (добавлено 2026-08-06,
   решение владельца; выведено из №398/№390/№224). Правило 2 описывает
   синтаксические позиции — этого хватает для конструкций языка и НЕ хватает
   для механизмов рантайма, у которых поведение зависит не от того, где
   конструкция написана, а от того, **как исполняется**. Для таких механизмов
   фикстура обязана крыть произведение осей, а не один сценарий:

   - **размещение работы:** прямо в теле области ↔ через `spawn` ↔ вложенная
     область;
   - **способ управления:** `cancel:` ↔ `timeout:` ↔ `deadline:` ↔ комбинация;
   - **вид блокирующей операции:** сон ↔ сетевая ↔ канальная ↔ `join`;
   - **момент события:** до входа ↔ во время парковки ↔ после завершения.

   **Урок, породивший правило:** все десять сценариев
   `std/src/concurrency/supervised_deadline_test.nv` кладут работу в `spawn`.
   Прямая операция тела под `cancel:` не покрыта ни одним тестом — и механизм
   девять дней числился починенным (D442, №224 «закрыт»), пока проба владельца
   не показала обратное. Матрица позиций тут не помогла бы: синтаксис
   одинаковый, различается путь доставки отмены.

7. **Негатив на контракт, а не только на энфорс** (добавлено 2026-08-06).
   Правило 5 требует neg-фикстуру на новый КОД ошибки. Этого мало: контракт
   модуля описывает и то, чего происходить НЕ должно, а такие утверждения у
   нас почти не пишутся — на 121 тест-файл в `std/src` приходится 11 негативов
   (8%; для сравнения, в conformance 547 на 1148, 32%). Поэтому: тест
   публичного контракта модуля обязан утверждать не только «делает, что
   обещано», но и **«не делает того, чего не обещано»** — операция не
   срабатывает там, где не должна; повторный вызов идемпотентен; чужой
   объект не затрагивается; вызов после завершения — no-op, а не паника.
   Негатив контракта живёт рядом с позитивом (тот же `*_test.nv`), это НЕ
   compile-error-фикстура и в `neg/` он не переезжает.

---

### Проба «подсунь заведомо негодное» — ОБЯЗАТЕЛЬНЫЙ шаг приёмки · 2026-08-06

Зелёная фикстура не доказывает ничего, пока не показано, что она умеет краснеть.
Поэтому приёмка ЛЮБОГО фикса (не только silent-wrong-value, см. пиннинг ниже)
включает шаг: **сломай собственный фикс и убедись, что новые фикстуры красные.**
Для энфорса — подсунь заведомо негодный вход и проверь, что он отвергнут по
ТОЧНОМУ коду; для рантайм-фикса — убери доставку/проверку и убедись, что тест
падает; для диагностики — проверь, что она НЕ ложнит на законном коде рядом.

Это ручной mutation-test в миниатюре: у нас нет инструмента mutation score, и
эта проба — его единственный доступный заменитель. Отчёт окна обязан содержать
вердикт пробы дословно; «фикстуры зелёные» без неё приёмкой не считается.

**Почему поднято из рекомендации в обязательный шаг:** за 2026-08-05/06 девять
раз подряд оказывалось, что механизм считался закрытым, а закрыт был один
названный случай — №304/335/336/338/347/349/353/381/394, плюс №398 (D442
обещал покрытие, которого нет) и №224 (ложное закрытие, державшееся девять дней
при живом P1-маркере в backlog).
