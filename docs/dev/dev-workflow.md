...
| **окно** | как сделать взятую работу | пишет код и тесты в СВОЁМ worktree, гоняет ТОЧЕЧНУЮ верификацию, коммитит по именам файлов | не пушит в `main`, не тегает, не гоняет полный гейт и мега-CU, не запускает фон (см. раздел ниже), не берёт номер сам — пишет `№TBD` |
