<!-- SPDX-License-Identifier: CC-BY-4.0 -->
# Как устроена разработка Nova и как подхватить работу

> **Кому:** новому агенту или человеку, который должен быстро понять, **что это за проект**
> и **как продолжать над ним работать**. Это не дубль [README.md](../../README.md) (язык) и не
> [AGENTS.md](../../AGENTS.md) (build/test для агентов) — это **связующий документ о процессе**:
> источники истины, план-ориентированная разработка, модель worktree, жёсткие правила, dev-логи.
> Подробности всегда в специализированных доках — здесь ссылки, не копии.


## ЗАПРЕТ ФОНА В ОКНАХ (закреплено 2026-08-08 — запрет в тексте не сработал)

> **УТОЧНЕНИЕ 2026-08-29 (по вопросу владельца: «возможно, файл устарел и не
> соответствует `/delegate`»).** Он был прав: ниже речь о ФОНОВОЙ КОМАНДЕ
> ОБОЛОЧКИ, а не о фоновом АГЕНТЕ, и без этой оговорки текст читался как запрет
> на всё разом — тогда как `/delegate` приезжает в то же окно и требует агентов
> мониторить, то есть предполагает их запущенными.
>
> **Фоновая КОМАНДА (`nohup`, `run_in_background`, откреплённый процесс) —
> запрещена окну, и она же ненадёжна у всех:** такой запуск умирает молча, и
> потому даже интегратор гоняет долгое СИНХРОННО с явным таймаутом, дробя на
> части. Исключение у интегратора — только полный гейт через
> `scripts/tools/gate-bg.sh`, где уведомление приходит ему самому.
>
> **Фоновый АГЕНТ — разрешён с 2026-08-26 тому, кто делегирует**, с проверкой
> каждые пять минут и работой между проверками. Дом этого правила —
> [`/delegate`](../../.claude/commands/delegate.md); здесь ссылка, а не копия.
>
> Общее у обоих — причина: ход, закончившийся ОЖИДАНИЕМ фона, мёртв. Запрещено
> не «фоновое» как таковое, а ожидание того, чего сам же и ждёшь.

**Проблема.** Окно, закончившее ход фразой «жду уведомления фоновой задачи»,
МЕРТВО: уведомления приходят интегратору, а не окну, и разбудить его некому.
За одну сессию так погибли ТРИ окна — при том, что запрет «не заканчивай ход
ожиданием уведомления» стоял в брифе КАЖДОГО прямым текстом.

**Вывод.** Запрещать надо не поведение, а ПРИЁМ, который к нему ведёт. Пока у
окна есть возможность запустить фон, оно рано или поздно на неё сядет.

**НОРМА для брифов окон — формулировать так:**

> Тебе ЗАПРЕЩЕНО запускать что-либо в фоне: никаких `nohup`, никакого
> `run_in_background`, никаких откреплённых процессов. Только синхронный запуск
> с явным таймаутом в той же команде:
> `timeout 900 <команда> > /tmp/лог 2>&1; echo "код=$?"`
> Не укладывается в один вызов — дроби на части, которые укладываются.
> Совсем не укладывается — доложи текстом «не укладывается, нужен интегратор»;
> это законный исход, а запуск фона — нет.

Плюс отдельной строкой: **полный гейт окну гонять нельзя** — его гоняет
интегратор (`scripts/tools/gate-bg.sh`); окну только точечная верификация.

Интегратор — исключение: он гейт запускает откреплённо и получает уведомление
сам, поэтому у него фон законен и обязателен (иначе залипает в ожидании, см.
`scripts/tools/gate-bg.sh`).
## 0. Кто что решает — таблица на один экран

Роли собирались из пяти файлов, и потому «кто решает» выяснялось по памяти.
**Это единственный дом таблицы;** остальные документы ссылаются сюда, а не
повторяют — две копии разойдутся на первой правке.

| роль | РЕШАЕТ | ДЕЛАЕТ | НЕ ДЕЛАЕТ |
|---|---|---|---|
| **владелец** | что входит в релиз, изменения языка, приоритеты, спорные развилки | ставит теги в `nova`, задаёт нормы | не пишет код по мелочи — за этим окна |
| **интегратор** | что вливать в `main`, когда пушить, номера записей реестра и D-блоков | сливает ветки, гоняет полный гейт и мега-CU, пушит на три зеркала, тегает пакетные репы | не решает за владельца изменения языка; не берёт номер D-блока сам, если владелец назначил координатора |
| **координатор номеров** | выдаёт номера D-блоков и записей реестра, когда окон много | ведёт очередь номеров | не судит содержание — только уникальность |
| **окно** | как сделать взятую работу | пишет код и тесты в СВОЁМ worktree, гоняет ТОЧЕЧНУЮ верификацию, коммитит по именам файлов | не пушит в `main`, не тегает, не гоняет полный гейт и мега-CU, не запускает фоновых КОМАНД оболочки (агентов — тем более: их пускает тот, кто делегирует), не берёт номер сам — пишет `№TBD` |
| **агент окна** | ничего | читает, перечисляет, сверяет по образцу из брифа | не спавнит суб-агентов, не заходит в главное дерево, не решает «баг или норма», не выносит приёмку |

**Читается это так:** право убывает сверху вниз, а обязанность доказывать —
снизу вверх. Агент доказывает окну, окно — интегратору строкой вердикта,
интегратор — владельцу зелёным CI. Ни один уровень не принимает свои же слова
как доказательство.

**Фон — не исключение из таблицы, а её следствие:** окну он запрещён, потому что
уведомление о фоновой задаче приходит НЕ ему (раздел «ЗАПРЕТ ФОНА В ОКНАХ» ниже),
а интегратору он законен и обязателен, потому что уведомление приходит именно
ему. Это одно правило, а не два разных.

## TL;DR за 60 секунд

- **Nova** — системный язык с алгебраическими эффектами, структурной конкурентностью и опц. контрактами;
  компилятор на Rust, кодоген в C, рантайм на C (Boehm GC). См. [README.md](../../README.md).
- **Три источника истины:** `spec/decisions/` (D-блоки = *почему*/семантика, нормативны) →
  `docs/plans/` (*что*/роадмап) → код (*как*/текущее поведение). **Spec-first:** семантику решаем в D-блоке
  **до** кода. При расхождении спека↔код — код = текущее поведение, спека = намерение; **не доверять одному
  слепо, проверять**.
- **Работа = планы.** Всё ведётся нумерованными самодостаточными планами в `docs/plans/`. Запуск: «**выполни
  план NNN**». Индекс — [docs/plans/README.md](../plans/README.md).
- **Worktree на план.** Главный репо `d:/Sources/nv-lang/nova` — точка интеграции; каждый активный план
  живёт в соседнем worktree `../nova-pNN`. Сейчас их ~11.
- **Жёсткие правила:** никакого `git stash`; `git add` только по именам файлов; `git commit -s` (DCO),
  без `Co-Authored-By`; пересобрать `nova-cli` после правок `.rs`; тесты только через C-codegen.
- **После большой задачи:** обновить `docs/project-creation.txt`, `docs/dev/simplifications.md` и discussion-log
  в **отдельном** репо `nova-private`. В `simplifications.md` — только ДЕЙСТВУЮЩЕЕ упрощение (rationale + условие
  снятия); закрытое сразу переносится в [docs/history/simplifications-closed.md](../history/simplifications-closed.md),
  диагнозы/хроники фиксов туда не пишутся вовсе (см. шапку файла).

---

## 1. Что это за проект

Nova — системный ЯП «для эпохи ИИ»: побочные эффекты видны в сигнатуре (`Db Net Fail`), что делает
ревью локальным, а тесты — без моков (подмена хендлера через `with`). Ставка: «LLM пишет, человек ревьюит».

Сейчас это **bootstrap-компилятор**, не дизайн-документ: парсер + чек + кодоген в C + нативный рантайм.
Один пайплайн (`nova build`/`nova test`) — **интерпретатора нет** (`nova run` не поддержан намеренно).

Глубже: [README.md](../../README.md) (обзор + примеры), [spec/overview.ru.md](../../spec/overview.ru.md),
[spec/effects.ru.md](../../spec/effects.ru.md), [examples/getting_started.nv](../../examples/getting_started.nv).

## 2. Карта репозитория (что важно для разработки)

| Путь | Что это |
|---|---|
| `compiler-codegen/` | Rust-крейт компилятора: `parser`, `types`, `codegen/emit_c.rs`, `test_runner.rs`, диагностика. Библиотека `nova_codegen` + бинарь `nova-codegen` (внутренний). |
| `compiler-codegen/nova_rt/` | C-рантайм: эффекты (`effects.h`), файберы, GC (`alloc_boehm.c`), libuv-планировщик. |
| `nova-cli/` | Пользовательский CLI `nova` (`build`/`run`/`test`/`check`/`doc`). **path-зависит** от `compiler-codegen` → правка `.rs` в кодогене авто-пересобирает `nova`. |
| `nova-lsp/` | LSP-сервер (`nova-lsp`). **Не трогать без явной задачи.** |
| `std/` | Стандартная библиотека на Nova (`prelude/`, коллекции, sync, net…). |
| `nova_tests/` | Тест-фикстуры `.nv` (folder-модули по темам/планам, `neg/` для compile-error). |
| `spec/` | Спецификация языка. `spec/decisions/` = **D-блоки** (нормативны). |
| `docs/` | Гайды разработчика + `plans/` + конвенции + dev-логи (`project-creation.txt`, `simplifications.md`). |
| `examples/` | Примеры на Nova. |
| `editors/` | Подсветка синтаксиса (VSCode/Vim/Emacs/Sublime). |

**Соседние рабочие копии (вне дерева репо):**
- `../nova-pNN` — worktree активных планов (одна `.git` с главным репо — см. §5).
- `../www` — **отдельный** репозиторий сайта nv-lang.org.
- `nova-private` — **отдельный** репозиторий: discussion-log, приватные заметки. Не в main-репо.

Сборочная модель: два независимых Cargo-крейта (`compiler-codegen`, `nova-cli` + `nova-lsp`) и
Nova-workspace `nova.toml` (members: `std`, `examples`, `nova_tests`). Подробнее: [docs/guide/nova-cli.md](../guide/nova-cli.md).

## РАБОТА НЕ НАЧИНАЕТСЯ БЕЗ КРИТЕРИЕВ ПРИЁМКИ — это первая работа, а не помеха ей

**Дом правила здесь** (перенесено 2026-09-16 из слэш-команды `/next` при её удалении:
правило общее для всех окон, а команда была ролевой и правило в ней жило вторым домом).

**Берёшь шаг — прочти его критерии приёмки и назови ДОСЛОВНО:** что запускается, что
обязано быть зелёным, какая строка вердикта считается доказательством, чем доказывается
КРАСНОТА (проба «подсунь заведомо негодное»). Критерий вида «сделать X» — не критерий.

**Критериев нет или они не проверяемы машиной — это ПЕРВАЯ РАБОТА:** исследуй задачу,
сформулируй их и впиши в план тем же слиянием, ДО первой строки кода. Без них «готово»
решается ощущением, а такое не выдерживает первого прогона и переделывается целиком.
Критерий, который машиной не проверить, так и помечается: «приёмка глазами, потому
что …» — это законно, но обязано быть НАЗВАНО, а не подразумеваться.

**Вместе с критериями пишутся ДВА РАЗДЕЛА, и это не украшение:** «**Не делаем
(границы)**» и «**Открытые вопросы**» — у каждого адресат, умолчание и срок. Граница
объясняет читателю, что отсутствие чего-то — РЕШЕНИЕ, а не забывчивость: без неё
следующее окно доделывает «очевидное», которого никто не заказывал. Вопрос с умолчанием
не даёт работе встать из-за молчания адресата; вопрос БЕЗ разумного умолчания —
блокирующий, и такой допустим ОДИН на волну. Оба можно объявить один раз на весь план —
волны их наследуют.

**Держится стражем** `scripts/guards/check-plan-wave-boundaries.py`, и он судит только
волны, датированные после своего заведения: страж, покрасневший на всей истории планов,
был бы отключён первым же окном.

## 3. Три источника истины (главная ментальная модель)

| Слой | Где | Отвечает на | Статус |
|---|---|---|---|
| **Решения (D-блоки)** | `spec/decisions/*.md` | *Почему* так устроено, какая семантика | **Нормативны.** Меняются записью нового/амендмента, не задним числом. |
| **Планы** | `docs/plans/*.md` (+ README) | *Что* делаем, в каком порядке, критерии приёмки | Рабочий роадмап, статусы CLOSED/IN-PROGRESS/retracted. |
| **Код** | `compiler-codegen/`, `std/` | *Как* сейчас реально себя ведёт компилятор | Истина о **текущем** поведении. |

Правила:
- **Spec-first:** изменение синтаксиса/семантики начинается с D-блока в `spec/decisions/`, потом код
  (`compiler-conventions.md` §5; пример — D315 «ResolvedType — единый носитель типа»).
- **Никогда не выдумывать синтаксис** по аналогии с другими языками — сверяться с `spec/decisions/` и
  `examples/`. Перед предложением идеи — проверить `spec/decisions/history/rejected.md` (может, уже отклонено).
- **Спека ↔ код расходятся** — это бывает (спека местами устаревает). Код = текущее поведение, спека =
  намерение. **Проверять оба, не верить одному слепо**; расхождение фиксировать (маркер/план), а не молча.

Структура D-блоков: 10 тематических файлов (`01-philosophy` … `10-overloading`) + `README.md` (индекс) +
`history/{rejected,evolution}.md`. Нумерация сквозная (D1…D315+); амендменты — `D216 V2/V3`; отозванные —
помечены `RETRACTED`. Свежий свободный номер уточнять по индексу (на момент написания: `D315` занят 172.1,
`D313` свободен).

## 4. Как организована работа: план-ориентированная разработка

- **План = самодостаточный файл** `docs/plans/NNN-<slug>.md` со всем контекстом для исполнения. Запуск
  одной фразой: «**выполни план NNN**». Внутри: фазы (`Ф.0`/`Ф.1`/…), критерии приёмки, источники, тесты.
- **Под-планы** наследуют номер: `169.1.2` — под-план `169.1`. Крупный план дробится на под-планы.
- **Индекс** всех планов — [docs/plans/README.md](../plans/README.md) (большой файл, читать
  страницами через offset/limit). **Статус** — только пофайлово: строка `**Статус:**` внутри
  самого `docs/plans/NNN-*.md`; сводный обзор — сгенерированный [docs/plans/STATUS.md](../plans/STATUS.md)
  (регенерация: `bash scripts/tools/gen-plan-status.sh`). Ручная сводная таблица статусов — запрещена
  (см. [conventions-governance.md](conventions-governance.md)).
- **Корень `docs/plans/` — только планы** (`NNN[.x]-<slug>.md`) + `README.md`/`STATUS.md`/
  `backlog-followups.md`. Рабочие файлы агентов (чекпоинты/progress/notes/карты/verification
  живых волн) — в [docs/plans/wip/](../plans/wip/), не в корне. При закрытии волны (план ЗАКРЫТ/влит
  в main) её чекпоинт из `wip/` **удаляется** (история — в git, ссылки на удалённый файл
  заменяются текстом «чекпоинт волны удалён при закрытии, см. git-историю»); если волна ещё
  активна, чекпоинт остаётся в `wip/` до закрытия (схема 2026-07-17).
- **В ПАКЕТНЫХ репах (nova-http/tls/compress/bignum/polaris) рабочие файлы
  окон НЕ КОММИТЯТСЯ ВООБЩЕ** (решение владельца 2026-08-04): своей зоны
  `wip/` там нет, класть некуда, а корень — публичное лицо пакета. Журнал
  окна ведётся на диске и остаётся некоммиченным; выводы уезжают в реестр
  [221.1](../plans/221.1-bug-sweep.md) и планы репы `nova`. Обеспечено
  физически: в `.gitignore` каждого пакета стоят `PROGRESS*.md`, `REPORT*.md`,
  `BRIEF*.md`. Повод: шесть журналов пролежали в корне `nova-bignum` на
  публичном виду, ещё три — в `nova-tls`/`nova-polaris` (№313).
- **Уборка `wip/` — часть ПРИЁМКИ, а не добрая воля** (№313): закрывая план,
  проверь, не остался ли его чекпоинт в `wip/`, и удали. Правило «удаляется
  при закрытии» существовало и раньше, но без энфорса — в `wip/` накопилось
  128 файлов, включая чекпоинты давно закрытых волн. Машинная проверка
  (файл в `wip/` при закрытом плане — красный) — задел плана 231.2.
- **Обязательный сквозной критерий** многих планов: «**без упрощений, как для прода**» — фаза не
  закрывается заглушкой/TODO; фазирование — это порядок, не урезание объёма.
- **Followup-маркеры `[M-…]`** — отложенная работа:
  - привязанные к плану → секция *Followups* того плана;
  - «плавающие» → [docs/plans/backlog-followups.md](../plans/backlog-followups.md) (**только живые/открытые**).
    Запись в [simplifications.md](simplifications.md) — **не всегда**: тот файл держит
    действующие осознанные УПРОЩЕНИЯ (с обоснованием и условием снятия), а диагнозы
    дефектов и хроники фиксов его шапка запрещает прямо. Условие — в
    [AGENTS.md](../../AGENTS.md#followup-markers-m), здесь не повторяется (правка
    2026-09-04: тут стояло безусловное «+ запись», и окно, честно ему следуя,
    попадало в запрещённый файлом случай).
  - Жизненный цикл маркера описан в [AGENTS.md](../../AGENTS.md#followup-markers-m).

## 5. Рабочий цикл (как подхватить и вести работу)

1. **Прочитать план** целиком (`docs/plans/NNN-*.md`) — он самодостаточен.
2. **Worktree на план.** Главный репо — точка интеграции; для плана — свой worktree:
   ```sh
   git worktree add -b plan-NN-<slug> ../nova-pNN main
   ```
   Соглашение имён: **`nova-pNN`** (не `nova-planNN`). На изолированной задаче создавать свой worktree
   сразу, а не переключать ветки в чужой рабочей копии. `git worktree list` — что сейчас занято.
3. **Менять код** spec-first (D-блок при изменении семантики).
4. **Собрать** (после любой правки `.rs`):
   ```sh
   cargo build --release --manifest-path nova-cli/Cargo.toml      # → nova-cli/target/release/nova(.exe)
   ```
   На worktree перед сборкой **обновить mtime** изменённого `.rs` (иначе cargo может не увидеть правку):
   PowerShell `(Get-Item path.rs).LastWriteTime = (Get-Date)`.
5. **Тестировать только через C-codegen** (не интерпретатор):
   ```sh
   nova-cli/target/release/nova test nova_tests/<папка>     # таргетно по ходу
   nova-cli/target/release/nova test --filter <substr>
   # одиночный дебаг с артефактами:
   compiler-codegen/target/debug/nova-codegen test-build FILE.nv --toolchain clang --keep-artifacts
   ```
   Per-fix — таргетная фикстура; полный `nova test` — в конце фазы. Полный регресс ~60-90 мин →
   **дробить на батчи < 10 мин** (потолок таймаута Bash/PS — 10 мин). Детали флагов и EXPECT-маркеров:
   [docs/dev/test-conventions.md](test-conventions.md).
6. **Коммит по задаче** (одна задача → один коммит; несколько → несколько коммитов):
   ```sh
   git add <конкретные файлы>           # НИКОГДА -A / .
   git diff --cached --stat             # проверить индекс (чужие pre-staged правки!)
   git commit -s -m "fix(...): ..."     # -s = DCO Signed-off-by (CI требует); без Co-Authored-By
   ```
7. **Обновить dev-логи** (после большой задачи — см. §7).
8. **Синк в main** после фазы — двунаправленно: pull main → ветка (FF) **и** merge ветка → main (FF).
   FF в общий main может попасть в чужую checked-out ветку → проверять HEAD, двигать ref аккуратно
   (`git branch -f main` при необходимости).

## 6. Жёсткие операционные правила (нарушение = поломка у других агентов)

- 🚫 **Никакого `git stash`.** `.git` общая для всех worktree → stash/refs/reflog глобальны → коллизия и
  потеря чужих изменений. Для baseline-сравнения — **temp-worktree** (`git worktree add`) **или**
  **commit + reset** в своей ветке **или** **patch + checkout** (`git diff > p.patch; git checkout --
  <files>; …; git apply p.patch`). (`compiler-conventions.md` §7.5 синхронизирован с этим правилом.)
- 🎯 **`git add` только по именам файлов**, никогда `-A`/`.` — рядом работают другие агенты в параллельных
  worktree. Не включать в `git add` уже удалённые/перемещённые пути (иначе вся команда падает на bad
  pathspec и коммитятся только pre-staged чужие правки).
- 🔍 **Перед каждым commit — `git diff --cached --stat`**: в индексе могут лежать чужие pre-staged изменения.
- ✍️ **`git commit -s`** (DCO обязателен, CI гейтит). **Без `Co-Authored-By: Claude`** и подобных AI-trailer.
  Руками трейлер **не добавлять** — repo-хук срезает его автоматически при коммите (отдельно следить/проверять не нужно).
- 🔁 **Пересобрать `nova-cli` после любой правки `.rs`** (включая `compiler-codegen` — путь-зависимость).
  GC env (`NOVA_GC_LIB_DIR`/`NOVA_GC_INCLUDE_DIR`) для worktree-сборок — из главного репо.
- 🧪 **Тесты только C-codegen** (`nova test` / `test-build`); интерпретатор не используем.
- 📐 **Не выдумывать синтаксис** — `spec/decisions/` + `examples/`.
- 🤝 **Конвенции нормативны.** Любое изменение конвенции-дока **или** отклонение в коде — только по
  согласованию с владельцем + маркер `[M-*]` + запись в `backlog-followups.md`. Без самоправок и молчаливых
  отклонений (`docs/dev/conventions-governance.md`).
- 🧷 **Окно обязано прогнать СВОИ фикстуры и вписать вердикт дословно.** Не «фикстуры написаны», а
  строка прогона. Гейт-список брифа без этого пункта неполон. Прецедент 2026-08-04 (окно p-lang): пачка
  сдана с вердиктами cargo / std / polaris / ratchet / lint, но собственные фикстуры не запускались ни
  разу — одна не компилировалась вовсе, вторая падала 2/4. Поймал мега-CU у интегратора, №309 откачен.
- 🌐 **Диагностику «CI сломался» начинать СНАРУЖИ — со статус-страницы GitHub**
  (`https://www.githubstatus.com/api/v2/summary.json`, одна секунда). Прецедент
  2026-08-06/07: прогоны перестали запускаться по push, и было перебрано полдюжины
  ВНУТРЕННИХ гипотез — настройки организации, состояние воркфлоу, skip-маркеры,
  `check-suites`, права, биллинг, версия «GitHub гасит автотриггеры за красноту»
  (все неверны). Причиной оказался **глобальный сбой GitHub Actions**: Major Outage
  с 15:22 UTC, воздействие Critical, формулировка инцидента дословно совпадала с
  симптомом — «многие события push и pull request не запускают новые workflow runs»,
  плюс вторичное воздействие на Pages (деплой сайта висел в очереди без раннера).
  Признаки, при которых сначала смотрим наружу: прогонов нет ВООБЩЕ (ноль
  `check-suites` на коммите), ручной `workflow_dispatch` при этом проходит, и то же
  самое видно в ДРУГИХ репах семьи. Порядок: внешний статус → потом своё. Полный
  протокол — реестр 221.1 №395.
- 🧨 **Проба «подсунь заведомо негодное» — обязательный пункт КАЖДОГО брифа и приёмки**
  (2026-08-06, решение владельца; норма — `test-conventions.md`). Окно обязано сломать
  собственный фикс и показать, что новые фикстуры КРАСНЕЮТ: для энфорса — подсунуть
  заведомо негодный вход и проверить отказ по ТОЧНОМУ коду; для рантайм-фикса — убрать
  доставку/проверку; для диагностики — убедиться, что она НЕ ложнит на законном коде
  рядом. Вердикт пробы — дословно в отчёт. **«Фикстуры зелёные» без вердикта пробы
  приёмкой не считается.** Это наш единственный заменитель mutation-тестирования:
  инструмента для него нет (№400).
- 🧭 **Рантайм-механизм принимается по МАТРИЦЕ ОСЕЙ ИСПОЛНЕНИЯ, а не по одному сценарию**
  (`test-conventions.md` правило 6): размещение работы (прямо в теле области / через
  `spawn` / вложенная область) × способ управления (`cancel:` / `timeout:` / `deadline:` /
  комбинация) × вид блокирующей операции (сон / сетевая / канальная / `join`) × момент
  события (до входа / во время парковки / после завершения). Клетку, которую физически
  не покрыть, пишем в отчёт с причиной — молча пропускать запрещено. Урок №398: механизм
  девять дней числился починенным, потому что все десять тестов клали работу в `spawn`.
- ➖ **Позитив И негатив на каждый пункт приёмки** (`test-conventions.md` правило 7):
  не только «делает, что обещано», но и «не делает того, чего не обещано» — не срабатывает
  там, где не должно; повторный вызов идемпотентен; чужой объект не затронут; вызов после
  завершения — no-op, а не паника.
- 🔕 **Фоновые агенты** — **ПРАВИЛО СМЕНИЛОСЬ 2026-08-26, и дом его —
  [`/delegate`](../../.claude/commands/delegate.md), не этот файл.** Здесь стояло
  «спрашивать подтверждение перед запуском»; владелец это снял, и теперь агентов на
  дешёвых моделях запускают АВТОМАТИЧЕСКИ там, где это оправдано, а не спрашивают
  каждый раз. Что осталось в силе: фон ловит серверный rate-limit и падает МОЛЧА,
  поэтому чекпоинт в файл, идемпотентные шаги, фильтрация пустых результатов — и
  ПРОВЕРКА КАЖДЫЕ ПЯТЬ МИНУТ (цифра владельца): упавший фон не присылает ничего,
  и ждать его сигнала значит не узнать о смерти вовсе. Между проверками — работать
  над независимым, а не ждать.

## 7. Куда писать после большой задачи

| Файл | Репо | Что |
|---|---|---|
| [docs/project-creation.txt](../project-creation.txt) | main | Строка-итог по задаче (хронология создания проекта). |
| [docs/dev/simplifications.md](simplifications.md) | main | История маркеров/упрощений (append-only). |
| `discussion-log.md` | **nova-private** (отдельный репо) | Развёрнутый лог обсуждений/решений. |

Стиль: **сообщения коммитов — по-английски** (решение владельца 2026-08-09;
репозиторий публичный и зеркалится на три площадки, историю читают снаружи).
Внутренние dev-логи — по-русски с английскими техническими терминами;
публичные доки (`README.md`, `AGENTS.md`, `CONTRIBUTING.md`) — по-английски.
Историю не переписываем: коммиты до 2026-08-09 остаются по-русски.

## 8. Где сейчас идёт работа / как сориентироваться

- Текущие статусы — **только** пофайлово (`**Статус:**` в `docs/plans/NNN-*.md`) + сгенерированный
  [docs/plans/STATUS.md](../plans/STATUS.md); приоритеты — [docs/plans/README.md](../plans/README.md) (снапшот-хайлайты),
  [docs/dev/simplifications.md](simplifications.md), `nova-private/`. Внешним заметкам про статус не доверять.
- Активные направления видно по worktree (`git worktree list`) и по индексу планов. На момент написания
  крупные узлы: унификация системы ошибок/cleanup (план 173 + 174/175/176), единый type-engine 172.x
  (D315 ResolvedType), консолидация тестов 169.1.x.
- Открытые «плавающие» долги — [docs/plans/backlog-followups.md](../plans/backlog-followups.md).

## 9. Частые грабли

| Грабля | Как правильно |
|---|---|
| Правишь `.rs` в worktree, cargo «не видит» изменений | Обнови mtime файла перед `cargo build` (PowerShell `LastWriteTime`). |
| `git add` падает на bad pathspec | Не включай удалённые/перемещённые пути; добавляй только существующие. |
| Тест классифицируется «не туда» | Классификация — по **маркеру** `EXPECT_*` (первые ~30 строк), не по папке/суффиксу. |
| Net/concurrency-тесты падают вне главного репо | libuv/GC берут `repo_root = current_dir`; гонять из соответствующего worktree, env из main. |
| Спека противоречит коду | Код = текущее поведение; спека = намерение. Проверить оба, расхождение зафиксировать. |
| Полный `nova test` убивается по таймауту | Дробить на батчи < 10 мин (потолок таймаута инструмента). |
| Проверяешь, что правило языка ДЕЙСТВИТЕЛЬНО работает | Мало пробы «рабочая форма против ломающейся» — она показывает лишь, что механизм различает два ожидаемых случая. Обязательна третья проба: **подсунь заведомо негодное** (тип без нужных методов в бáунд, ro-источник там, где требуется mut, снятую форму записи). Прецедент 2026-08-06: бáунд `[S AsSlice[T]]` пропускал ЛЮБОЙ тип — `append(NoSlice{})` проходил `nova check` молча, падая лишь на этапе C (№381); канон std этого не ловил, потому что в корпусе никто так не пишет. |
| Планируешь работу по статусам реестра багов | **Реестр отстаёт от кода — сверяй пробой перед запуском окна.** Статус пишется руками и после слияния фикса обновляется не всегда. Прецеденты 2026-08: №34 висел в списке блокеров тега 9 дней после закрытия и увёл планирование; у четырёх номеров (№283/№304/№309/№315) фикс был в коде, а строка красная. Правило: номер, взятый в работу, сначала воспроизводится пробой на свежем бинаре; не воспроизводится — закрывается, а не чинится. |
| Считаешь открытые баги по эмодзи или по словам в описании | Считай по явному признаку закрытия в **последних** ячейках строки. Описание дефекта содержит слова «открыт», «критический» и эмодзи всегда — счёт по ним даёт разные числа на каждом проходе (у интегратора вышло 27 → 36 → 22 на одном и том же реестре). |

## 10. Карта документов (куда смотреть за деталями)

| Тема | Документ |
|---|---|
| Онбординг агента (build/test/правила) | [AGENTS.md](../../AGENTS.md) |
| Обзор языка + примеры | [README.md](../../README.md), [spec/overview.ru.md](../../spec/overview.ru.md) |
| Вклад, DCO, лицензии | [CONTRIBUTING.md](../../CONTRIBUTING.md) |
| Решения/семантика | [spec/decisions/README.md](../../spec/decisions/README.md) |
| Все планы (индекс/навигация) | [docs/plans/README.md](../plans/README.md) |
| Статусы планов (сгенерировано, пофайловый источник) | [docs/plans/STATUS.md](../plans/STATUS.md) |
| Тесты (маркеры, folder-модули, флаги) | [docs/dev/test-conventions.md](test-conventions.md) |
| Правила разработки компилятора | [docs/dev/compiler-conventions.md](compiler-conventions.md) |
| Конвенция кодирования M:N-рантайма (проактив) / отладка гонок (реактив) | [docs/dev/mn-coding-conventions.md](mn-coding-conventions.md) / [docs/dev/debugging-races.md](debugging-races.md) |
| Управление конвенциями (мета) | [docs/dev/conventions-governance.md](conventions-governance.md) |
| CLI-справка | [docs/guide/nova-cli.md](../guide/nova-cli.md) |
| Открытые долги `[M-*]` | [docs/plans/backlog-followups.md](../plans/backlog-followups.md) |
| Модель ошибок/cleanup | [docs/dev/idioms/error-and-cleanup-model.md](idioms/error-and-cleanup-model.md) |

## Спека пишется ДО реализации, а не вместе с кодом

Правило владельца, подтверждённое 2026-08-09 («кажется мы договаривались писать
спеку и D до реализации»).

Формулировка `CLAUDE.md` — «язык-меняющее слияние не пушится без D-амендмента в
спеке в том же слиянии» — задаёт **крайний срок**, а не «одновременно». Она не
запрещает написать решение раньше; наоборот, порядок «сперва решение, потом
код» — норма:

* окно реализует **по решению**, а не приносит решение задним числом;
* форма обсуждается на тексте, где правка стоит минуты, а не на коде, где она
  стоит волны;
* если решение не пишется — значит оно не додумано, и это видно ДО того, как
  потрачена работа.

Открытый, но ещё не решённый вопрос живёт в `spec/open-questions.ru.md`; как только
владелец выбрал форму — он становится D-блоком, **не дожидаясь** реализации.
Прецедент: [D449](../../spec/decisions/06-concurrency.md) (`supervised(on_timeout:)`)
записан до кода; интегратор сперва заявил обратное («станет D-блоком вместе с
кодом») и был поправлен.
