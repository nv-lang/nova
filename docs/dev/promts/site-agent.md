...
## ПЕРВООЧЕРЕДНАЯ ЗАДАЧА (владелец 2026-08-02: исполнить этим агентом, не ждать)
**Актуализация `/install/`** (en: `site/src/partials/install-index.html` + head-партиал;
ru-зеркало обязательно). Три конкретные правки:
- (а) Шаг «Clone and build»: `git clone https://github.com/nv-lang/nova` →
  `git clone --recursive https://github.com/nv-lang/nova` (libuv — git submodule;
  без него `nova build` у чистого пользователя не соберёт рантайм). Рядом строкой:
  уже склонировал без флага — `git submodule update --init`.
- (б) Шаг «Run the test suite (optional)»: `nova test nova_tests` — устаревший корпус;
  заменить на конформанс-smoke `nova-cli/target/release/nova test spec_tests/conformance`
  с честной пометкой о длительности (полный прогон — десятки минут), либо короткий
  вариант: собрать и запустить hello из следующего раздела как проверку тулчейна.
  Финальную формулировку предложи в отчёте — интегратор утвердит на приёмке.
- (в) Пройди страницу целиком на релиз-нейтральность: никаких утверждений о релизах/
  тегах/бинарных артефактах nova до v0.1; source-build — единственный путь установки
  сейчас, так и подавать.
- (г) ПРОВЕРЬ ФАКТОМ перед сдачей: чистый клон во временную папку (вне существующих
  реп) → шаги страницы дословно → `hello.nv` собирается и печатает. Если чистая
  сборка падает (кандидат: GC-библиотека не в гите — vcpkg-вопрос) — это НАХОДКА
  класса A-V7: НЕ чинить самому, зафиксировать шаги падения в отчёте интегратору,
  а на странице до фикса не обещать того, что не работает.

## Известные точки актуализации (снято 2026-08-02; проверь git прежде чем делать)
1. *(перенесено в «Первоочередную задачу» выше)*
