...
* **ЗАМЕР ОКНА (2026-09-04, вечер), уточняющий охотничью оговорку «стража без инъекции входа не пробовать»: инъекция есть у ВСЕХ 83 стражей `check-novac-*`** — корень приходит аргументом (`a = sys.argv` + `a[1]` у python, `${1:-...}` у shell), ноль исключений. Значит мета-страж применим ко всей семье, а тринадцать стражей, названных охотником пропущенными, отсеиваются другим свойством — им нужен БИНАРЬ, git или прогон корпуса, и на пустом корне они честно скажут «судить нечего». Это меняет объём будущей волны: не «часть семьи», а вся.
* **И урок делегирования, купленный тем же замером:** инвентарь этой оси был заказан агенту (haiku) со списком образцов, написанным ПО ПАМЯТИ (`sys.argv[1]`, `ROOT="$1"`, …). Живая форма — `a = sys.argv` — в список не попала, агент выполнил бриф буквально и вернул «56 из 84 без инъекции». Ошибка не агента, а брифа: **образец для механического исполнителя берётся из дерева грепом, а не из головы**, и выдача агента сверяется вызывающим до использования (правило `/delegate`, здесь оно и сработало).
