#!/bin/sh
# Probe with-bytes-handler: SIBLING: `with Random = "ab".bytes()` (an applied value type): novac accepts; the oracle builds and runs it.
# Run: sh cmd.sh from THIS directory (it cds to the repo root itself).
. ../env.sh || exit 1
cd "$ROOT" || exit 1
P="$HUNT/with-bytes-handler/probe.nv"
