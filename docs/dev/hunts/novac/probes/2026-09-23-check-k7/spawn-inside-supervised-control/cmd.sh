#!/bin/sh
# Probe spawn-inside-supervised-control: CONTROL: the same spawn inside `supervised`; both compilers accept, oracle runs.
# Run: sh cmd.sh from THIS directory (it cds to the repo root itself).
. ../env.sh || exit 1
cd "$ROOT" || exit 1
P="$HUNT/spawn-inside-supervised-control/probe.nv"
