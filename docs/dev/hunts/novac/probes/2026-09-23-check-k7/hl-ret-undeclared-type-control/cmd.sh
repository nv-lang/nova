#!/bin/sh
# Probe hl-ret-undeclared-type-control: CONTROL: the same undeclared `Nope` in the op's RETURN annotation; both compilers refuse.
# Run: sh cmd.sh from THIS directory (it cds to the repo root itself).
. ../env.sh || exit 1
cd "$ROOT" || exit 1
P="$HUNT/hl-ret-undeclared-type-control/probe.nv"
