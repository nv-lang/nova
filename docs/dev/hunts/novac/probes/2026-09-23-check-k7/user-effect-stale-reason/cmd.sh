#!/bin/sh
# Probe user-effect-stale-reason: a user-declared effect used by `with`, a handler literal and an op call; oracle builds and prints 42.
# Run: sh cmd.sh from THIS directory (it cds to the repo root itself).
. ../env.sh || exit 1
cd "$ROOT" || exit 1
P="$HUNT/user-effect-stale-reason/probe.nv"
