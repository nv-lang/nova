#!/bin/sh
# Probe 1 -- same_dir on two paths that name ONE directory with the separators
# spelled differently. Function-level: the code under test is copied verbatim
# from novac/src/pipeline/pipeline.nv (lines 110-439).
#
# NOVA = full path to the oracle binary, e.g.
#   $NOVA (см. env.sh рядом: путь выводится от корня репозитория)
