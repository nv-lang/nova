...
## ВАЖНО ДЛЯ ВОСПРОИЗВЕДЕНИЯ: f06 работает ТОЛЬКО ВНЕ ЭТОГО ДЕРЕВА

Замерено при приёмке 2026-08-30, и это не оговорка, а свойство находки.
`f06-prelude-absent-silent` про ОТСУТСТВИЕ std — а внутри репозитория `std/`
находится и без переменной окружения. Числа:

- из каталога пробы В ДЕРЕВЕ: `env -u NOVA_STD_PATH nova check hello.nv` → **PASS: 1**;
- та же пара файлов, скопированная ЗА пределы репозитория: **FAIL: 1**,
  `undefined identifier \`println\``.

Значит воспроизводить f06 надо так: скопировать `hello.nv` и `nova.toml` в
каталог вне дерева Nova и запустить там. Находка от этого не слабеет — она
именно про внешнего человека, у которого репозитория нет вовсе.

**Урок общего свойства, стоит записать: барьер «пробы в дереве» (278 Ф.5) имеет
слепое пятно — проба про ОТСУТСТВИЕ окружения не может жить внутри этого
окружения.** Файл-проба такую находку хранит, но не доказывает; доказательство —
названная здесь команда с выходом за дерево.

Тем же замером выяснилось, что оба манифеста проб требуют поля `license`
(`check-license-hygiene`, №556): добавлено с пометкой прямо в файлах — строка
не меняет того, что пробы меряют, и это проверено повторным запуском.
