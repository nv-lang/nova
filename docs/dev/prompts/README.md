# Промпты для агентов

Этот каталог хранит **рабочие промпты** для повторяемых операций над
проектом nova-lang. Промпты — отдельные от планов: план описывает
**что и зачем**, промпт — **как именно прогнать конкретную операцию**.

## Когда использовать

Промпт пишется когда:
- Операция повторяется (регенерация, sweep, миграция).
- Логика операции стабильна, может выполняться много раз.
- Хочется чтобы любой агент / разработчик мог прогнать её одинаково.

## Что внутри промпта

- Цель в одном абзаце.
- Точные входы (какие файлы / реестры читать) и выходы (что писать).
- Шаги: numbered list, без двусмысленностей.
- Acceptance: как проверить что прогон успешен.
- Ограничения: что **не** делать (например, не править вручную).

## Список

| Промпт | Что делает |
|---|---|
| [../read-project.md](../read-project.md) | **НЕ промпт и больше не здесь.** Ориентация по проекту — её читают, а не исполняют, поэтому лежит рядом с `dev-workflow.md`. Шаг 2 онбординга после `AGENTS.md` |
| [read-toolchain.md](read-toolchain.md) | Только инфраструктура: nova CLI, test runner, структура репо. Входит в read-project. |
| [regen-runtime.md](regen-runtime.md) | Регенерирует `std/src/runtime/*.nv` из `runtime_registry.rs` (Plan 13 Ф.3). |
| ~~update-plans-readme.md~~ | **СНЯТ 2026-08-12.** Предписывал вести рукописный индекс планов со статусами в `docs/plans/README.md` — практику, запрещённую ещё 2026-07-21 (рукописная сводка расходится молча). Источник правды статуса — строка `**Статус:**` в самом плане; единственная сводка — автогенерируемый [../../plans/STATUS.md](../../plans/STATUS.md). Держит `check-no-handwritten-plan-index.sh`, реестр №612 |
| [update-spec.md](update-spec.md) | Аудит и обновление `spec/decisions/*.md` (D-blocks) + `spec/<topic>.md` — drift detection vs implementation, новые D-blocks для закрытых планов. |
| [window-292-queue-snapshot.md](window-292-queue-snapshot.md) | Бриф окну на Ш.2 плана 292: скрипт `scripts/tools/queue-snapshot.py`, выводящий очередь работ в `target/queue.json`, который читает Stop-хук. Самодостаточен: переписка, из которой он вырос, не нужна |
| [opencode-brief-template.md](opencode-brief-template.md) | Шаблон задания механическому исполнителю: команды целиком, точный путь, явный список запретов, образец строки результата |
| [window-brief-template.md](window-brief-template.md) | Шаблон брифа окну на sonnet/opus: запреты, выживание под сторожем (молчание убивает), настройка worktree, приёмка, образец строки результата. Появился после того, как три окна подряд были сняты за молчание — виноват был бриф, а не окна. |
| [integrator-handoff.md](integrator-handoff.md) | Восстановление сессии главного интегратора: роль, инварианты, текущее состояние и блокеры |
| [window-guards-handoff.md](window-guards-handoff.md) | Восстановление сессии окна трека **стражей и гейта** (реестр №1065, №1113): дерево, остаток с держателем у каждого пункта, что уже решено замером и решать заново не надо, четыре ловушки смены |
| [window-283-handoff.md](window-283-handoff.md) | Восстановление сессии окна 283 (разбор строки в float): граница задачи по слову владельца, состояние ветки, очередь, шесть ловушек сессии и что НЕ его |
