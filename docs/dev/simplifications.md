...
[2026-09-21 registry #1264 (novac check×K4) — одиночный `?` (Try-оператор, D85), ветка `measure/1186-check-vs-test`] **Где:** `novac/src/check/literal_rules.nv` (`TRY_MSG`), парсер/лексер рядом (`lex/lex.nv`, `tree/tree.nv`, `parse/expr.nv`). **Что упрощено:** `a?` теперь ПАРСИТСЯ (свой `TokenKind.Question`/`NodeKind.Try`) и отказывается ИМЕНОВАННО в чекере вместо анонимного parser-fallback — но САМА СЕМАНТИКА (early-return propagation на `Option`/`Result`, D85's правило про тип возврата объемлющей функции, диагностика `E_TRY_IN_FAIL_FN`) НЕ реализована вообще. **Почему:** полная семантика требует контроля потока (взаимодействие с `check/tail_rules.nv`'s "does body end with a value"), эмиссии early-return и отдельной проверки типа возврата функции — объём отдельной волны, а не диагностической правки; задача этого коммита — честно назвать причину отказа (легальная-но-нереализованная форма), не реализовать форму. **Как чинить:** реализовать `?` целиком по D85 (`spec/decisions/04-effects.md:4652-4931`): судить тип возврата объемлющей функции, развернуть в `match {Ok/Some => v, Err/None => return ...}` на этапе эмиссии, снять `TRY_MSG`. **Приоритет:** L (не блокирует тег — форма редка в самом novac, 5 известных носителей self-build, все не self-critical пути).

[2026-08-10 Plan 265 Ф.3 / №553 — `Random.real_random()`, worktree `nova-p553`, ветка `p553-secure-random`] **Где:** `std/src/testing/handlers/core_test.nv` (главная фикстура непредсказуемости). **Что упрощено:** фикстура «два вызова без `with Random = …` не совпадают» тестирует `Random.u64()` напрямую, а НЕ `Uuid.v4()`, хотя приёмка №553 называет буквально `Uuid.v4()`. **Почему:** ЛЮБОЙ compile unit, импортирующий `std.identifiers.uuid` (даже без обращения к `NIL`/`MAX`), падает CC-FAIL — pre-existing, НЕ связанный с №553 баг кодогена констант-указателей (`[M-uuid-const-pointer-aggregate-init]`, `docs/plans/backlog-followups.md`), подтверждённый на неизменённом `main`. `Random.u64()` — тот же контракт на сыром опе эффекта, изолированный от этого стороннего блокера. **Как чинить:** когда `[M-uuid-const-pointer-aggregate-init]` закрыт — добавить дублирующую `Uuid.v4()`-фикстуру рядом (`std/src/identifiers/uuid_test.nv`, естественный дом), снять эту заметку. **Приоритет:** L (Random.u64()-фикстура полностью покрывает свойство №553; Uuid.v4()-версия — желательное дополнение, не пробел в приёмке).
...
- **Гейты:** `cargo build`/`--release` чисто, без новых warning на новых
  ...
  упёрся в ПРЕДСУЩЕСТВУЮЩУЮ (не от этого окна) проблему: `TlsStream.
  read_bytes` не резолвится при свежей пересборке, хотя метод РЕАЛЬНО
  существует в `nova-tls/src/stream.nv:293`; гипотеза про lock-файл
  ПРОВЕРЕНА И ОТКЛОНЕНА (`git diff examples/nova.lock.toml` — пустой,
  только CRLF); на чистом main маскируется `build cache hit — reusing
  generated C` — точный механизм не расследован (вне периметра S1a) —
  вне периметра
...
## 2026-08-23 — бюджет яруса гейта novac: одна одноразовая цена вычтена, вторая названа

Бюджет судит ПРОВЕРКИ, а не условия, при которых есть что проверять. Утром из
времени яруса вычли сборку novac: ярус loop шёл 15с на свежем бинаре и 26с сразу
после слияния, пересобравшего оракула, и второй прогон покраснел на ЗДОРОВОМ
дереве. Вычитание записано в гейте и говорит цену вслух в вердикте; проверено в
обе стороны (`touch` исходника → «сборка novac 5с не в счёт», судится 14с при
стене 19с).

Вечером той же перепроверкой нашлась ВТОРАЯ одноразовая цена того же класса:
прогон сразу после `cargo build` оракула дал ярус push 414с при пределе 400 и
302с на следующем прогоне — ключ кэша PCH включает штамп оракула, поэтому новый
бинарь заставляет собрать заголовки заново внутри дифференциального стража. Она
НЕ вычтена: заведён плавающий маркер `[M-novac-gate-pch-not-subtracted]`
(P2 OPEN, строка в `backlog-followups.md`) с названным механизмом — смоук
печатает секунды сборки PCH, гейт их вычитает тем же приёмом.

Почему маркер, а не «поднять базу»: работа не выросла, а поднятая база спрятала
бы находку, ради которой этот предел и заведён (утром он поймал страж, подорожавший
с 13с до 38с).

## 2026-08-24 — маркер `[M-novac-gate-pch-not-subtracted]` переехал из плавающих в план 274

Заведён вчера как плавающий и попал в `backlog-followups.md`; страж
`bug-number-sync` на CI сразу потребовал ему № в реестре 221.1 — по правилу
владельца №217 каждый плавающий маркер нумерован. Номер был бы неправдой: 221.1
— реестр дефектов ПОСТАВЛЯЕМОГО компилятора, а маркер про вычитание сборки PCH из
бюджета яруса `gate-novac.sh`, то есть про инструмент плана 274.

Дом определён AGENTS.md и без исключений: план-связанный маркер живёт в своём
плане, плавающий — в OPEN-виде. Строка снята из `backlog-followups.md`, описание
осталось в плане 274 вместе с названным механизмом (смоук печатает секунды PCH,
гейт их вычитает).

Полезное здесь не перенос, а то, ЧТО его потребовало: маршрут маркера проверяется
стражем, и ошибка маршрутизации нашлась за сутки, а не к релизу.

## 2026-08-27 — витрина: снят обход `[M-flagship-spawn-capture-value-struct-ptr-mismatch]`

Аудит A-V8 (план 221), интегратор. Обход заставлял `fetch_one` брать
четыре скаляра вместо записи `Source`, а `fetch_guarded` — разбирать `Source`
на локали перед `spawn`. Причина (CC-FAIL на захвате value-записи во вложенный
spawn-контекст) числилась починенной в бэклоге — и это проверено ЗАПУСКОМ, а не
принято на веру: регресс-фикстура того же имени дала 80 прогонов из 80 без
падений (реестр №470), а после снятия обхода витрина собирается под
`--strict-effects` и даёт `PASS: 10 FAIL: 0 SKIP: 1` — тот же счёт, что и до.
Строка убрана из OPEN-вида `backlog-followups.md` по жизненному циклу маркеров.

## 2026-08-27 — витрина: снят обход `[M-187-monotonic-plus-operator-mono]`

Аудит A-V8 (план 221), интегратор. В `live.nv` стояло `t0.plus(budget)` вместо
`t0 + budget`, потому что операторная форма давала CC-FAIL при двух и более местах
вызова в одном CU (столкновение mono-кэша обёртки оператора). Проверено
ЗАПУСКОМ дважды: отдельная проба с двумя `t0 + budget` в одном CU собирается и
исполняется; сама витрина с обоими местами, переведёнными на оператор, собирается
под `--strict-effects` и даёт `PASS: 10 FAIL: 0 SKIP: 1`. Оба комментария обхода убраны,
маркер из `live.nv` исчез полностью.

- **2026-08-29 — `[M-bitwise-operator-method-naming]` СНЯТ решением владельца: имена ОСТАЮТСЯ `@bitand`/`@bitor`/`@bitxor`/`@bitnot`.** Вопрос стоял с 2026-07-27 («@bitand и @and — сравни, возможно назвали неверно»); владелец подтвердил префиксную семью — короткие `@and`/`@or` читались бы как логические, а Rust-эталон префиксует ровно по этой причине. Обе рабочие части маркера были сделаны раньше (D46-амендмент; план 234 ✅ 2026-08-27), оставалось только имя. Остаток по `@rem` на named-tuple живёт в своём маркере `[M-named-tuple-operator-overload-incomplete]` и этим снятием не задет.
