...
## 2026-09-04 — `[M-novac-lex-escape-byte-step]` заведён (окно 274)

Байтовый шаг через экранирование в лексере novac: подозрение без носителя, потому что отказ по неизвестному экранированию приходит ПОЗЖЕ лексера и прикрывает класс. Наводка пришла от соседнего окна (его №853/№832 — тот же класс в оракуле, там носитель был). Проба вышла зелёной НЕ О ТОМ — отказ дал белый список экранирований, а не граница символа; это и записано в маркер, чтобы следующий читатель не принял зелёную пробу за отсутствие класса. Реестром не заводится: строка без воспроизведения завышает расстояние до тега.
