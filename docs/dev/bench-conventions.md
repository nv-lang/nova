...
Bench нужен если план **явно затрагивает производительность**. Признаки:

- В плане есть слова: «производительность», «throughput», «latency», «оптимизация», «ускорение», «bench», «perf», «regression gate».
- План меняет алгоритм или структуру данных, где скорость — часть контракта (коллекции, планировщик, GC, codegen-оптимизации).
- В плане есть явное задание: «добавить бенчмарк», «замерить», «сравнить baseline».

Bench **не нужен** если:

- План добавляет/меняет семантику языка (типы, эффекты, синтаксис) — покрывается `nova test`.
- План исправляет баг — достаточно regression-теста в `nova_tests/`.
- Plan добавляет тест для самого теста (тест-инфраструктура, test-conventions).

Если не уверен — **не пиши bench**. Бенчмарк без baseline бесполезен; неправильный бенч хуже отсутствия.
