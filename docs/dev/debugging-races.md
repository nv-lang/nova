...
```bash
# Canonical pattern (see scripts/tools/stress_bisect.sh):
bash scripts/tools/stress_bisect.sh nova_tests/<plan>/<repro>.nv 30
...
| **Stochastic intermittent FAIL** | Bisect via `scripts/tools/stress_bisect.sh` + `git bisect run` | reused | `scripts/` |
...
## §1a. Референс раннего Go — ДИАГНОСТИЧЕСКИЙ приём, а не архитектурный совет

> Правило владельца 2026-08-07: «Go как референс для M:N уже давал положительные фиксы,
> нужно использовать». В `mn-coding-conventions.md` раздел 0 — про ПРОЕКТИРОВАНИЕ («сверься
> до того, как писать»). Здесь — про РАССЛЕДОВАНИЕ: симптом уже есть, и референс подсказывает,
> ГДЕ искать.

**Приём:** взять симптом, найти класс задачи, который он затрагивает, и спросить — как этот
класс решён у раннего Go (рантайм на C, те же условия: M:N, сборщик, переключение стеков).
Если у них соответствующий инвариант выражен ЯВНО, а у нас выводится КОСВЕННО — это первый
подозреваемый, и проверять его дешевле, чем ловить симптом.

**Таблица «симптом → что сверять с референсом»:**

| Симптом | Класс | Что у референса выражено явно | Наш способ |
|---|---|---|---|
| Разные случайные падения, порча стека, «собралось живое» | границы стека для сборщика | реестр границ на каждую горутину | для главного фибра — проба адреса + чтение карты памяти процесса |
| Падение при завершении процесса, живые фибры на выходе | порядок гашения относительно закрытия цикла событий | явный порядок | у нас гарантия сильнее (осушение сирот), но порядок был неверным — №418 |
| Отмена «прилетела не туда», срабатывание вне области | доставка сигнала отмены | сигнал в канал, безвреден без адресата | исключение; опоздавшая отмена валила процесс — №431 |
| Состояние фибра порвалось между проверкой и действием | смена статуса фибра | атомарная смена статуса | у нас уже по этому образцу |

**Живой пример, ради которого раздел и написан (2026-08-07).** Симптом №427: остаточная
нестабильность в несколько процентов с РАЗНЫМИ сигнатурами — неопознанные `SIGSEGV`, порча
стека, отмена не там. Прямая охота дорогая: для частоты 3-5% нужно 92-154 полных прогона
(окно p427 сделало 21 и не поймало ничего). Сверка с референсом дала подозреваемого за
минуты: границы стека главного фибра у нас вычисляются косвенно, у Go — хранятся явно. А
профиль «разные случайные сигнатуры» — типичный для «сборщик изредка не видит часть стека»,
и ровно этот механизм был вторым корнем №418. Итог: №430 поднят из К3 в К1 и проверяется
ПЕРВЫМ, до дорогой длинной серии.

**Как этим пользоваться, коротко:**
1. Опиши симптом одной фразой и отнеси к классу из таблицы (или добавь строку).
2. Спроси: у референса этот инвариант ЯВНЫЙ или выводится?
3. Если у нас косвенно — сначала сделай явным и повтори прогоны. Часто это и есть корень.
4. Дискриминатор для класса «сборщик не видит стек»: `GC_DONT_GC=1` — чистая серия под ним
   при грязной без него подтверждает класс (приём сработал в p418).

**Чего НЕ делать:** копировать код (лицензия) и переносить решения там, где наши гарантии
СИЛЬНЕЕ. Пример: Go не гарантирует осушение сирот при выходе, мы гарантируем — это не
пробел у нас, а более сильный контракт, и «сделать как у Go» здесь означало бы регресс.
...
git bisect run scripts/tools/stress_bisect.sh nova_tests/<plan>/<repro>.nv 30
...
### 2.5 «Симптом в подсистеме X — корень в подсистеме Y». СНАЧАЛА ПРОВЕРЬ ПОСЫЛКУ

**Урок №390, 2026-08-08. Цена ошибки — сутки работы и ПЯТЬ ложных гипотез.**

Симптом: второй `TcpStream.read()` после частичных данных и закрытия пира виснет
навсегда. Естественная формулировка — «сокет закрыт, а завершение чтения не
приходит». Из неё выросли и умерли пять гипотез подряд: баг libuv → наша пляска
`uv_read_stop`/`uv_read_start` → отложенность перезапуска → привязка к потоку →
межворкерная привязка цикла.

**Все пять были мимо, потому что неверна была САМА ПОСЫЛКА.** Сокет никто не
закрывал. Настоящий корень — в КОМПИЛЯТОРЕ: escape-анализ не распознавал
`mut x = T.new(...)` (конструктор без аннотации типа) как источник value-record,
значение не промоутилось в кучу, `&x` оставался указателем на СТЕК, живущий после
возврата из функции. Дальше: refcount `TcpStream` — мусор → гейт
`@rc.fetch_sub(1) == 1` не проходил → `Net.close_stream` не вызывался → сервер
физически не закрывал сокет → FIN не уходил вовсе. libuv была права: доставлять
было нечего.

#### Приём, который надо было применить ПЕРВЫМ

**Проверь посылку инструментированием, а не рассуждением.** Формулировка «X
произошло, а Y не пришло» содержит ДВА утверждения. Прежде чем искать, почему нет
Y, докажи, что было X.

Решающая проба заняла минуты: `fprintf` на входе в `net_tcp_close`. Вывод показал
вызов ТОЛЬКО для клиента — серверный `close()` не звался ни разу. С этого момента
вся сетевая ветка расследования отпала.

#### Порядок, экономящий сутки

1. **Разложи симптом на утверждения и докажи КАЖДОЕ.** «Пир закрылся» — отдельное
   утверждение, а не фон. Инструментируй точку, где оно должно стать истинным.
2. **Контрольный эксперимент на чистой библиотеке.** Тот же сценарий на нашей
   vendored-зависимости БЕЗ нашего рантайма — разделяет «библиотека» и «мы» за
   один прогон. Образец: `scripts/tools/libuv-control-experiment.c` (три
   режима: немедленный перезапуск, непрерывное чтение, отложенный перезапуск —
   точная модель парковки файбера). Все три дали корректный `UV_EOF` — библиотека
   оправдана окончательно.
3. **Сведи к репро БЕЗ подсистемы симптома.** Если корень не в ней, он обязан
   воспроизводиться без неё. Здесь: `spec_tests/conformance/standalone/p390_scratch_escape_rc.nv` —
   три строки, ни одного сокета, `fetch_sub` возвращает `2748661030792` вместо `1`.
   **Несетевой репро сетевого бага — доказательство, что подсистема невиновна.**
4. **Обходной эксперимент как проба гипотезы.** Снять подозреваемый гейт (здесь —
   условие `== 1` в `@close()`) и посмотреть, исчезнет ли симптом. Исчез → корень
   в том, что этот гейт вычисляет, а не в том, что за ним.

#### Признаки, что ты ищешь не там

- Гипотезы кончаются, а симптом держится: **ПЯТЬ подряд закрытых гипотез в одной
  подсистеме — сигнал сменить подсистему, а не придумать шестую.**
- Контрольный эксперимент на «виновной» библиотеке проходит успешно.
- Симптом не воспроизводится ничем, кроме полного сценария (значит ты не нашёл
  минимальную причину, а описываешь следствие).
- Обвиняемая библиотека широко используется (libuv несёт Node.js): критический
  баг в ней был бы замечен не тобой. Это не доказательство её невиновности, но
  сильный довод сначала проверить СВОЙ слой.

#### Смежное правило про замеры

Диагностические цифры снимать ТОЛЬКО через `scripts/tools/measure.sh` — он
отказывается мерить при идущем гейте, живых сборках и загрузке ЦП выше порога, и
сам считает разброс. Причина: за те же сутки ТРИ вывода о производительности были
сделаны по цифрам, снятым под нагрузкой, и все три опровергнуты чистым повтором
(реестр 221.1 №449, №437, №429).
...
### 3.3 Stress harness `scripts/tools/stress_bisect.sh`

**Usage:**
```bash
scripts/tools/stress_bisect.sh nova_tests/<plan>/<repro>.nv [stress_n=15]
```
...
`timeout`, or `timeout 300 bash scripts/tools/stress_bisect.sh ...`) so a hang is
...
**Run:**
```bash
git bisect run scripts/tools/stress_bisect.sh nova_tests/<plan>/<repro>.nv 66
```
