...
SMT-солвер. Модель — **enforce-with-elision** (D24 / Plan 140), а *не*
debug-only assert'ы: **доказанный** контракт элидируется (нулевая цена в
...
fn at(buf []int, i int) -> int
    requires 0 <= i && i < buf.len     // ✓ настоящая проверка границ
=> buf[i]
```
...
границ молча превращается в no-op). Nova отвергает цепочку (и bool/unit-операнды
`<` `<=` `>` `>=`, `E_RELATIONAL_OPERAND_NOT_ORDERED`) на этапе parse/check; пишите
через `&&` (Plan 150 / D248).

### Self-access (`@field`, `@len()`) в контрактах метода

Контракт **метода** может ссылаться на состояние receiver'а: читать поле через
`@field` или встроенный size-аксессор `@len()` / `@cap()` / `@byte_len()` /
`@is_empty()` (call-форма взаимозаменяема с полем: `@len()` ≡ `@len`):

```nova
fn Vec[T] @index(i int) -> T
    requires 0 <= i && i < @len     // ✓ ссылается на длину receiver'а
{
    unsafe { @data[i] }
}
```

SMT-солвер моделирует receiver как сущность `_self`; каждый `@field` становится
uninterpreted `_field_<name>(_self)`, поэтому `@len` в `requires` и `@len` в
`ensures` — один и тот же терм (согласованное рассуждение). Разрешено только
**чтение** — контракт это выражение, записать поле в нём негде.

Когда такой контракт недоказан и срабатывает в runtime, сообщение о нарушении
...
- **доказуемо** in-bounds доступ компилируется **без runtime-проверки** (zero-cost);
- **недоказанный** доступ сохраняет проверку и аварийно падает на OOB (в debug *и*
  release) — без тихого UB.

Верификатор доказывает доступ in-bounds, когда индекс ограничен, например:

```nova
for i in 0 .. v.len() {
    sum = sum + v[i]          // доказано: i ∈ [0, v.len()) → проверка элидируется
    v[i] = v[i] * 2           // write-back тоже элидируется (in-place сохраняет длину)
}
ro s = v[0 .. v.len()]        // slice v[a..b]: доказано 0<=a && a<=b && b<=v.len()

fn at(v Vec[int], i int) -> int
    requires 0 <= i && i < v.len()
=> v[i]                       // cross-fn: граница берётся из `requires`
```

Элизия требует SMT-бэкенда (`NOVA_SMT_BACKEND=z3`); без него проверяется каждый
доступ (safe degrade). Также нужна **инвариантность длины** вектора в области
видимости — length-changing вызов (`push`/`pop`/…) на том же векторе сохраняет
проверку (soundness). Для доступа, доказанного только через `requires`, проверка
сохраняется под `--contracts=off` / `#unchecked` (там `requires` уже не enforced).
`@get`/`@first`/`@last` возвращают `Option` и дают `None` на OOB — у них нет
контракта границ.
...
контракты как SMT-запрос и спрашивает солвер. Доказанные контракты
элидируются (zero-cost, и в debug, и в release). Недоказанные —
проверяются в runtime И в debug, И в release (enforce-with-elision;
для `#verify` недоказуемость это compile-error, см. ниже).
...
Отказ от SMT-*верификации* (не от enforcement). Контракты **недоказаны**,
поэтому проверяются в runtime И в debug, И в release (enforce-with-elision —
ничего не элидируется). Используйте для контрактов, которые солвер не может
обработать (нелинейная арифметика, строки и т.д.). Чтобы снять и runtime-
проверку — `#unchecked` / `--contracts=off`.
...
Доказано → элидируется (zero-cost, и в debug, и в release); недоказано →
runtime-проверка остаётся в debug И в release (enforce-with-elision).
...
**Доказуемо-безопасные overflow-чеки элидируются.** Каждый `int` `+`/`-`/`*`
...
Нужен `NOVA_SMT_BACKEND=z3`; без него проверяется всё. `*` нелинеен — Z3 может оставить чек.
