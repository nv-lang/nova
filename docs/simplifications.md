...
### [M-52-type-inference-no-annotation] ✅ ЗАКРЫТО (Plan 55.5; см. closure 2026-05-22)
...
### [M-52-spread-not-supported] ✅ ЗАКРЫТО (Plan 55 + 56; см. closure 2026-05-22)
...
### [M-52-multi-instance-hashmap-collision] ✅ ЗАКРЫТО (Plan 55.6; см. closure 2026-05-22)
...
## Plan 52.x — закрытие mono-pass блокеров (2026-05-22)

Аудит «блокеров» Plan 52.x в автономной сессии: все они уже сняты
Plan 55/56/59, документация устарела и вводила в заблуждение.

### [M-52-spread-not-supported] / [M-52-type-inference-no-annotation] / [M-52-multi-instance-hashmap-collision] — ✅ ЗАКРЫТЫ
- **Что было:** маркеры описывали mono-pass corruption / name-collision
  как блокеры spread `[...m,k:v]`, `HashMap.@clone()`, `@merge_from()`,
  `let m = [k:v]` без аннотации.
- **Чем снято:** Plan 55.4 (`c4ce5b61a66` — save/restore
  current_fn_return_ty + protocol-method whitelist), Plan 55.5
  (`a951986eba8` — mono name-collision), Plan 56 (`6657743c1b1` —
  @clone codegen unlock), Plan 59 Ф.7.2 (idiomatic refactor).
  Map-spread инфраструктура — Plan 55 (`6c1fe6465f8`).
- **Проверка:** все формы протестированы и работают —
  `nova_tests/map_literals/positive_clone_merge.nv` (@clone /
  @merge_from / HashMap.from) + `positive_spread.nv` (5 форм spread).
  Раньше покрытия не было: `plan55/f4_hashmap_clone_method.nv` вопреки
  имени `.clone()` НЕ вызывает (это regression-guard «HashMap не
  ломается», не тест @clone).
- **Доделка all-spread:** `[...a, ...b]` без аннотации
  мис-классифицировался как array-литерал — у парсера нет `k:` для
  disambiguation и нет типов. Фикс в `MapLitAnnotator` (`types/mod.rs`):
  аннотатор трекает типы let-биндингов и параметров текущего item'а;
  для all-spread `ArrayLit` без expected-типа выводит map-тип из
  spread-источников (`infer_map_type_from_spreads`) → конверсия в
...
## Флаки-тесты (2026-05-22)

### [M-flaky-mn-actual-workload] ✅ ЗАКРЫТО (Plan 92, 2026-05-22)
- **Где:** `nova_tests/concurrency/mn_runtime_actual_workload.nv` —
  тест M:N work-stealing runtime.
- **Что было:** тест периодически падал в полном `nova test` под
  CPU-contention, проходил изолированно.
- **Диагностика (Plan 92 Ф.0):** воспроизведено под устойчивой
  нагрузкой — 66/100 FAIL, **все** на `assert(on_w0 < 16)`;
  `assert(total_ran == 16)` не падал 0/352. Корень — **Case A,
  harness-flakiness**: `on_w0 < 16` проверял, что work-stealing
  раскидал 16 fibers по >1 worker'у — но это оппортунистичная
  эвристика планировщика, не контракт: под starvation worker-потоков
  ОС один worker законно выполняет всё. M:N-рантайм корректен (вся
  работа выполнена — `total_ran` держится всегда), soundness-дыры нет.
- **Как починено:** тест переписан — контракт (`total_ran == 16`,
  все fibers отработали на worker-пуле) проверяется детерминированно
  каждую попытку; распределение — статистически (32 независимых
  batch'а, pass если распределение наблюдалось ≥1 раз;
  `P(false-fail) ≈ 0.66^32 ≈ 2.6e-6` даже под экстремальной
  нагрузкой; сломанный round-robin → 0 распределений → детерминированный
  fail). Standalone под 24-burner нагрузкой → PASS.
- **Политика флаки-тестов** задокументирована — `docs/test-conventions.md`
...
### [M-90-no-bce] Plan 90 — bounds-check без elimination

- **Где** — `nova_str_byte_at` (`nova_rt.h`), `nova_array_copy_from`/
  `copy_within`/`fill` (`array.h`).
- **Что упрощено** — каждый `byte_at` / bulk-вызов выполняет bounds-check,
  даже когда вызывающий уже гарантировал диапазон (напр. `while i <
  byte_len`). Лишняя ветка не устраняется.
- **Почему** — корректность и безопасность приоритетны (язык без сырых
  указателей, D6); современный C-компилятор часто элиминирует проверку
  сам; это модель Go (BCE на стороне компилятора, не `unsafe`).
- **Как чинить** — bounds-check elimination в codegen (Plan 90 Ф.0.2,
...
> на 4 sub-plan'а 99.1/99.2/99.3/99.4. Foundation (99.1):
> `resolve_method_level_subst` helper extracted из user-generic
> dispatch + integration в Option/Result DeclaredBody с расширенным
> mono_name `_<T>_<U>...`, `register_novaopt_decl(U)` lazy-emit,
> `infer_method_level_return_for_sum` для `&self` inference.
> Contextual ctors (99.2): bare `Some(v)`/`None`/`Ok(v)`/`Err(e)`
> используют `current_fn_return_ty`. Atomic migration (99.3): 6
> commits per-method (`Option.map[U]`/`unwrap_or_else`/`ok_or[E]`,
> `Result.map[U]`/`map_err[F]`/`unwrap_or_else`). Tests (99.4): 8
> фикстур plan99/ (6 positive + 2 negative). **14/14 Option/Result-
> методов на Nova** — только `unwrap` C-routed (Plan 61 lineage).
> Паритет Rust `Option::<T>::map<U, F: FnOnce(T)->U>`.
