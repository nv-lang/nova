...
**Plan 40 R6 pin list для NovaAfterState** — был добавлен в audit R6 как
...
fallback на nova_alloc остаётся до закрытия Plan 43. Это conditional
...
который Linux получает. Когда Plan 43 закроется, Windows получит то же
преимущество.
