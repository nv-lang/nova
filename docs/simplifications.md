...
- **Impact:** users specifying `Duration.from_nanos(500_000)` (500 ?s)
...
- ?? Plan 118.1 Ф.3/Ф.4 + Plan 118.3 + Plan 118 Ф.7 — DEFERRED с rationale
...
- **`[M-83.10.4-iso-cancel-startup-race]` ЗАКРЫТ структурно Ф.2** (gopark) — production-кода НЕ
  потребовалось. Timer-backed park (Time.sleep) вето?ит на cancel перед arming + driver async-close
...
- **РЕЗОЛВ `[M-153.5-flatten-nested-receiver]` — вложенные generic-ресиверы произвольной глубины + flatten
  ...
  []T` под D239) в `std/collections/vec/restructure.nv`: pre-size `out = with_capacity(? inner.len())` +
...
- **Гейты:** `nova test std/concurrency/rate_limiter_test.nv` 1/0;
  ...
  `nova test --positive --compile-error spec_tests/conformance` 95/0 (?=0,
...
- **Гейты:** conformance один CU 111/0 + 7 SKIP (?0); err173 folder-CU +
  ...
  идентично, ?=0); cargo build --release чист.
...
- **Гейты:** conformance один CU 111/0 + 7 SKIP (?0); err173_2 CU PASS
  (все supervisor-тесты + 3 новых); err173 + any_is + plan110 PASS;
  runtime_panics CU PASS (precedence в т.ч.); std/src/concurrency ?=0
