...
### [⚠️ codegen-only ЗАКР 2026-05-09; interp Ф.6-bis pending] Plan 14 Ф.6: D69 variadic + spread

- **Что:** declaration `fn f(...items []T)` + call-site spread
  `f(...arr)` + mixed `f(a, ...arr, b)` работают **через C-codegen
  pipeline (production)**. Покрывает D69 спецификацию.
- **Где:** `compiler-codegen/src/{ast/mod.rs, parser/mod.rs,
...
- **Use-site эффект:**
    ...
    // Вызовы (через codegen → C → exe):
    join("a", "b", "c")          // → []str = ["a","b","c"]
    join(...arr)                  // → []str = arr
    join(...prefix, "tail")       // mixed
    ```
- **⚠️ Verification gap (обнаружено 2026-05-10):** изначально считалось
  D69 закрытым по результату `run_tests.ps1` (PASS 91/91, codegen
  pipeline). Но `nova-codegen test variadic.nv` (interp-mode) даёт
  **7/7 FAIL** — `eval_call` не collect'ит variadic args в Value::Array.
- **Не покрывается:**
    + **Interp-mode collection** для variadic-fn (Ф.6-bis, ~30 строк
      в `interp/mod.rs::eval_call`).
    + Spread в interp (`nova-codegen run` отвергает компиле-error'ом
      — partial shim был добавлен).
...
- **Std refactor:** `std/path/path.nv` `Path.join(parts []str)` →
  `Path.join(...parts []str)` — теперь принимает variadic (через codegen).
- **План:** docs/plans/14-stdlib-codegen-gaps.md Ф.6 ⚠️ codegen ЗАКРЫТ,
  Ф.6-bis (interp) — pending.
- **Verification practice TODO:** добавить в CI / pre-commit hook'е
  параллельный прогон `nova-codegen test` для всех `nova_tests/**.nv`
  (test-runner сейчас гоняет только codegen pipeline).
