...
### [M-52-from-fields-canonical] `#from_fields` honored только для `HashMap`
- **Где:** `compiler-codegen/src/types/mod.rs` (MapLitCtx + MapLitAnnotator)
- **Что упрощено:** Маркер `#from_fields` распознаётся по **имени типа**
  (последний сегмент path), не по полному canonical identity
  `collections.hashmap.HashMap`. Bootstrap honored локальные user-типы
  с `#from_fields` (parser принимает их), но D55 map-coercion
  `{field: v}` сейчас полагается на shadowing-by-name.
- **Почему:** Canonical-identity lookup требует cross-file resolver state
  на момент type-check; в bootstrap достаточно префикса "HashMap".
- **Как чинить:** Сравнивать полный path `["std","collections","hashmap","HashMap"]`
  через resolver canonical-identity API.
- **Приоритет:** L (расширяемость на `OrderedMap`/`BTreeMap` — future).

### [M-52-frompairs-protocol] `FromPairs[K, V]` для `[k:v]` не реализован
- **Где:** `compiler-codegen/src/desugar.rs::build_map_block`
- **Что упрощено:** Десугаринг `[k:v]` всегда вызывает
  `HashMap.with_capacity`, хардкод. Альтернативные map-типы через
  протокол `FromPairs[K, V]` не работают.
- **Почему:** Протокол требует выбора реализации по expected type —
  отдельный механизм. Bootstrap нужен только `HashMap`.
- **Как чинить:** Ввести `protocol FromPairs[K, V]` в prelude.
- **Приоритет:** L.
