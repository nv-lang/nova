...
### [M-flaky-mn-actual-workload] флаки M:N-тест в полном прогоне
- **Где:** `nova_tests/concurrency/mn_runtime_actual_workload.nv` —
  тест M:N work-stealing runtime.
- **Что:** тест периодически падает в полном `nova test` (16
  параллельных job'ов — высокая CPU-contention), но стабильно проходит
  изолированно. Наблюдение при закрытии Plan 89: 1 FAIL в полном
  прогоне → 3/3 PASS при изолированном перезапуске.
- **Почему проблема:** флаки-тест роняет «зелёный» CI и **маскирует
  настоящие регрессии** — при `1 FAIL` нельзя без ручного разбора
  отличить «флаки» от реального бага.
- **Не диагностировано:** корень не установлен — это либо
  timing-assumption в самом тесте (harness-flakiness), либо настоящая
  гонка в M:N-runtime (тогда soundness-дефект). Различить — задача
  Ф.0 плана.
- **План исправления:** `docs/plans/92-flaky-mn-test-stabilization.md`
  (Plan 92, proposed) — Ф.0 диагностика + decision point, Ф.1
  стабилизация, Ф.2 аудит прочих concurrency-тестов, Ф.3 docs.
- **Приоритет:** M (CI-надёжность; настоящая регрессия в M:N не
  замаскирована — наблюдаемые отказы loud).
