...
## Статус: работа сделана, коммиты в процессе (после сбоя окна)

Все три задачи (№616 `-> consume T`, №611 постфикс параметра, №615
R2-split) реализованы и проверены точечно. На момент этого чекпоинта
правки лежат в рабочем дереве НЕЗАКОММИЧЕННЫМИ — коммичу по частям в
порядке брифа: спека → компилятор → корпус.
