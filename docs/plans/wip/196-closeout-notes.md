...
## 7. П5 — терминал-фиксы по зондам wip/ (196-probes-notes.md): ❌ НЕ ЗАКРЫТЫ (честные архитектурные остатки)

**Источник:** `docs/plans/wip/196-probes-notes.md` — единственный файл в
`wip/196*` семье, содержащий И «терминал» И «зонд» с КОНКРЕТНЫМИ red-фикстурами
(остальные грep-хиты — общее упоминание слова «терминал»/«зонд» в других
контекстах, не эта серия). 5 находок: `B10m_ident_empty_fallback` (phase-1c
pre-scan write-once баг) + 4 «терминала» (`B11al_panic_method_p67`,
`B12q_panic_path_p67`, `B12r_panic_path_no_method_seg`, `B12s_panic_path_no_parts`)
— ВСЕ 5 подтверждены живыми (red-фикстуры в `spec_tests/fixtures/known_red/`),
рецепты записаны, но НЕ применялись (вне мандата probes-волны).

**Попытка (эта волна): `B11al_panic_method_p67`** — рецепт выглядел самым
узким/конкретным («чекер обязан отклонять метод на `*T` вне
`is_raw_pointer_intrinsic_method`-аллоулиста как `E_UNKNOWN_METHOD`»).
Реализовано (`check_instance_overload`, `types/mod.rs:10923`): добавлен
`TypeRef::Pointer` арм — если `!is_raw_pointer_intrinsic_method(method_name)`
→ `E_UNKNOWN_METHOD`, симметрично существующим FixedArray/primitive армам
рядом. **Собралось чисто, НО не сработало** — прогон `spec_tests/fixtures/
known_red/p196_b11al_terminal_probe.nv` (debug-print добавлен временно)
показал: `check_instance_overload`'s `BoundCtx::infer_arg_ty(obj, scope)`
возвращает `None` для `Ident("p")` ДО того, как код вообще доходит до моего
нового арма — `p`'s тип (`ro p = arr.ptr()`) НИКОГДА не попадает в `scope:
HashMap<String,TypeRef>`, потому что `walk_stmt`'s `Stmt::Let`-ветка
(`types/mod.rs:19606-19621`) сама вызывает `Self::infer_arg_ty(&d.value, scope)`
на RHS — и `infer_arg_ty`'s `ExprKind::Call`-арм (`~21225-21248`) знает ТОЛЬКО
`Type.new()` (Atomic-семья), ничего про `.ptr()`/FixedArray-аксессоры. Т.е.
рецепт из зонда (добавить проверку в `check_instance_overload`) НЕДОСТАТОЧЕН
САМ ПО СЕБЕ — нужно ЕЩЁ научить `infer_arg_ty` (ОБЩИЙ, широко используемый
best-effort bound-checker helper — потребитель НЕ только `check_instance_overload`,
но и CAS-ordering/protocol-satisfaction/другие bound-проверки) резолвить
`.ptr()`/`.len()`-аксессоры. Это РАСШИРЯЕТ блast-радиус за пределы одной
функции — правка теперь трогает ОБЩИЙ инфраструктурный путь, потребовала бы
полной кросс-корпусной re-верификации ВСЕХ потребителей `infer_arg_ty`
(не только raw-pointer класса), что выходит за рамки «узкого точечного
фикса» и ближе к «отдельной архитектуре». **Откачено полностью** (`git
checkout` на `types/mod.rs`, 0 диф от П4-коммита) — половинный/нерабочий
патч не оставлен в дереве (zero-tolerance §4а).

**Вердикт по всем 5 (честный список остатков, НЕ закрыты):**
1. `B10m_ident_empty_fallback` — нужна body-инференция expr-body callee
   (Q1-продюсер) ИЛИ topological/fixpoint pre-seed pass — новая
   инференс-возможность, не точечный патч.
2. `B11al_panic_method_p67` — рецепт зонда неполон (см. попытку выше);
   реальный фикс требует РАСШИРЕНИЯ `infer_arg_ty` (общий helper, широкий
   blast-радиус) — отдельная волна с полной кросс-корпусной проверкой.
3. `B12q_panic_path_p67` — общий Path-static-call validator; риск
   false-positive регрессии без исчерпывающего энумерирования ВСЕХ уже
   легитимных 2-сегментных Path-форм (Channel.new/ChanReader/try_from/
   effect_schemas/var_types fn_ret_*/sum-variant ctor — минимум 6 разных
   каналов, пропуск любого = регрессия на реальном коде).
4. `B12r_panic_path_no_method_seg` — сцеплен с ОТКРЫТЫМ backlog-пунктом
   `[M-d289-module-qualified-path-method-collision-cu]` (main-лог,
   30fdd2b9f, найден приёмкой D431 буквально накануне этой волны) — парсер
   жадно глотает `.Ident`-цепочки для module-qualified путей тоже;
   трогать без глубокого пересмотра рискованно.
5. `B12s_panic_path_no_parts` — Channel 2 для callee через произвольное
   выражение (Index/Ternary/…) — структурно возможно (тот же путь, что уже
   работает для HOF-параметров), но не проверено этой волной (время).

**Гейт:** попытка №2 полностью откачена (`git checkout`, 0 diff), 0
поведенческих изменений от П4-коммита — повторный полный conformance-гейт не
требуется (код идентичен уже-зелёному П4).
...
- П5 (терминал-фиксы по зондам wip/): ❌ НЕ ЗАКРЫТЫ — все 5 (B10m + 4
  терминала) честно задокументированы как остатки, требующие отдельной
  архитектуры (попытка на B11al показала рецепт зонда неполон, откачено).
- П6 (реестр 196-one-truth-closeout.md): ✅ ЗАКРЫТО — итоговая секция «Итог
  финальной closeout-волны 2026-07-21» добавлена в конец реестра, Статус-строка
  обновлена (план ОСТАЁТСЯ IN PROGRESS, честно — НЕ заявлено закрытие).

## 8. Финальное подтверждение гейтов (HEAD этой волны, все 6 пунктов применены)

- `cargo build --release` (compiler-codegen + nova-cli) — 0 errors (оба).
- `nova test spec_tests/conformance --jobs 12` (СИНХРОННО, foreground,
  `NOVA_OFFLINE=1`) — **PASS: 126  FAIL: 0  SKIP: 16**. ЗЕЛЁНЫЙ (идентично П1
  гейту — П2-П4 добавили только doc-комментарии/env-gated зонд, 0 поведенческих
  изменений default-пути).
- `nova build --strict-effects --mode release examples/flagship/aggregator/
  src/main.nv` (`NOVA_OFFLINE=1`) — **built (24.73s)**, 0 ошибок.

## Итог (для отчёта владельцу)

**Снесено физически: 0 движков** (обе попытки — B11q/B11r и B11al-чекер-фикс —
наткнулись на живые corpus-хиты на ПЕРВОМ же repro, откачены полностью).
**Доставлено:** 1 продюсер-gap закрыт (П1, closure_if_ctor_peek), 1 новый
точный зонд (`NOVA_B5_MEANINGFUL_TRACE`). **3 честных отрицательных вердикта**
(B11q/B11r, resolve_result_option_ret, rt_slots_from_call fallback) — каждый с
конкретным классом+числом, НЕ голословно. **5 терминал-остатков**
(196-probes-notes.md) — задокументированы как требующие отдельной архитектуры,
одна попытка (B11al) показала рецепт зонда неполон. **Реестр актуализирован**,
статус ЧЕСТНО остаётся IN PROGRESS (план НЕ закрыт — заявлять закрытие при 8
живых honest-остатках было бы `feedback-no-done-claims-on-documents`
нарушением).

Коммиты (ветка `p196-closeout`, база main `58804953d`): `d46610dae` (baseline+
worktree), `0d4ee870d` (П1 impl), `418afb69f` (П1 гейты), `428529179` (П2
вердикт), `6805f2643` (П3 вердикт), `9d4209be1` (П4 зонд+вердикт), `34e9a9e41`
(П5 попытка+остатки), + финальный коммит реестра. В main НЕ мёржено, push НЕ
делался (по заданию).
