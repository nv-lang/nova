...
## Приёмка интегратора, раунд 2 (2026-08-06) — мега-CU 679/2, третий регресс: тот же прогон, другая форма

Интегратор перепрогнал мега-CU после фиксов regressions 1/2: **PASS 679
FAIL 2**. `neg_str_from_retracted` (№TBD-F, пред-существующий) закрылся сам
(видимо, порядко-зависимый мега-CU-артефакт — не трогал, не разбирал заново).
Остался ОДИН регресс, и он от фикса регресса 2 (`emit_detach.rs`).

### Регресс 3 — `detach_mut_capture_outlives_frame.nv` RUN-FAIL

```
FAIL: detach mut-capture survives caller frame death (heap-box, not &stack) — assert failed: n.load() == 1
```

**Сценарий — ПРОТИВОПОЛОЖЕН m240**: там box должен жить ЧЕРЕЗ итерации цикла
(одна ячейка на несколько detach-исполнений). Здесь `dmc_kick(mut n
AtomicInt)` — `n` это ПАРАМЕТР (Plan 184 §Р10 указатель в CALLER'а frame,
который тут ещё жив — умирает только `dmc_kick`'s СОБСТВЕННЫЙ кадр перед
тем, как orphan проснётся). Мутация обязана дойти до CALLER'а хранилища.

**Дифференциальная проверка (по требованию брифа) — собрал ОБА сценария в
одну пробу** (`m240`-цикл + `dmc`-переживание кадра + третий сценарий: ОБА
фактора вместе — `mut`-параметр захвачен ВНУТРИ while-цикла, кадр умирает):
после фикса регресса 2 (box-once-guard) `m240`-часть проходила, `dmc`-часть
— нет. Значит box-once-guard САМ ПО СЕБЕ корректен (не откатывал) — проблема
в ДРУГОМ месте, которое раскрылось только СЕЙЧАС для параметр-формы захвата.

**Корень (глубже, чем регресс 2, но в том же файле)**: `emit_detach`'s
capture-анализ решает by-value/by-ref ЧЕРЕЗ `self.var_mutable.contains(name)`
— а `var_mutable` заполняется ТОЛЬКО `Stmt::Let`-обработкой (`let mut x =
...`), НИКОГДА для ПАРАМЕТРОВ функции. Captured `mut`-ПАРАМЕТР (`n`) всегда
классифицировался как read-only (`by_value=true`) — тело `detach{}` мутировало
ОДНОРАЗОВЫЙ BY-VALUE СНИМОК в ctx-структуре (`ctx->n = (*n);` — снимок значения,
не адрес), никогда не касаясь реального storage вызывающего. Невидимо ДО этой
волны: у старого pointer-newtype `AtomicInt` «значение» снимка САМО было
указателем (`Nova_AtomicInt*`) — копирование указателя не расходилось с
shared-объектом. Для value-inside — расходится.

**Первая попытка фикса (обнаружена и ОТКАЧЕНА ДО коммита)**: расширить
`var_mutable` напрямую, чтобы включало и mut-параметры. Прогнав тест —
поймал НОВУЮ проблему: `var_mutable` управляет и ДРУГИМИ, несвязанными
решениями (например, capture у handler-литералов, `~emit_c.rs:12395` — там
captured PARAM СОЗНАТЕЛЬНО берётся BY VALUE, чтобы не породить dangling
stack-pointer для escaping-литерала, который может пережить свою функцию).
Расширение `var_mutable` сломало бы ЭТОТ инвариант для ВСЕХ потребителей
сразу. Откачено.

**Итоговый фикс — два взаимодополняющих изменения**:

1. Новое поле `mut_param_names: HashSet<String>` (`emit_c.rs`) — узкий
   реестр, население ИДЕНТИЧНО `ref_params` (тот же fn-entry loop, тот же
   save/restore паттерн вокруг тела функции), но читается ТОЛЬКО
   `emit_detach`'s собственным capture-анализом — никакой другой потребитель
   `var_mutable` его не видит.
2. В `emit_detach.rs`: (а) `is_mut = var_mutable.contains(name) ||
   mut_param_names.contains(name)` — параметр теперь корректно классифицируется
   как мутабельная захватка; (б) НОВАЯ ветка `source_is_already_ptr` — когда
   источник УЖЕ указатель (P10 `ref_params`-параметр ИЛИ внешняя
   fiber-капча, `is_outer_cap && !outer_by_value`), бокс НЕ аллоцируется
   вовсе — указатель пробрасывается напрямую в ctx-поле (без разыменования,
   без копирования значения). Storage остаётся тем, чем был — принадлежит
   ТОМУ, ЧЕЙ это кадр (вызывающему, либо внешнему уже-established боксу) —
   ровно то, что `#share`-типы обязаны обеспечивать. Для ГЕНУИННОГО ЛОКАЛА
   (объявленного прямо в текущей функции — m240-случай) ветка НЕ активируется
   (там `ref_params`/`is_outer_cap` оба false) — box-once-guard из регресса 2
   остаётся единственным путём, не тронут.

### Верификация (синхронно)

Комбинированная проба (`m240`-цикл + `dmc`-переживание кадра + оба фактора
вместе) — все 3 теста PASS. Оба реальных фикстура standalone: `PASS: 1 FAIL:
0` каждый (`m240_detach_box_while_loop_read_after.nv`,
`detach_mut_capture_outlives_frame.nv` — последняя проверена с временно
добавленным `import std.time.duration`, отсутствующим только из-за
standalone-изоляции от peer-файлов той же `spec_tests.conformance`; сам
файл в репозитории не менялся). `d172_realtime_blocking_attrs.nv` (регресс 1)
— по-прежнему `PASS: 1 FAIL: 0`.
...
`lines` 64349 → 64387 (+38, только новое поле `mut_param_names` + save/restore
в `emit_c.rs`; сам anti-copy-фикс и capture-классификация — в
`emit_detach.rs`, вне метрики). Интегратор просил `lines<=64350` (свою базу
уже свёл до этого числа) — фикс регресса 3 требует БОЛЬШЕ; обоснование
дописано в `scripts/guards/arch-ratchet.baseline`. **База снова НЕ
финализирована** — сведение 64350-vs-моя-64387 на интеграторе при мёрже, как
и в прошлый раз.

### Файлы (дополнение)

- `compiler-codegen/src/codegen/emit_c.rs` — новое поле `mut_param_names` +
  population (зеркало `ref_params`) + save/restore на входе/выходе `emit_fn`.
- `compiler-codegen/src/codegen/emit_c/emit_detach.rs` — `is_mut` теперь
  учитывает `mut_param_names`; новая ветка `source_is_already_ptr` (указатель
  пробрасывается напрямую, без бокса, для `ref_params`/внешних fiber-капч).
- `scripts/guards/arch-ratchet.baseline` — новая запись (64349→64387).
