...
- Решение: не трогать в этом окне — риск/объём непропорционален выигрышу
  сверх уже закрытой типовой дыры. Существующий `[M-59.1-channel-new-
  cleanup]` остаётся ОТКРЫТЫМ (не заведён новый маркер — тот же дефект,
  уточнён этим design-note); реестры (`221.1-bug-sweep.md`/
  `backlog-followups.md`) НЕ правились этим окном — по конвенции файла
  «Статусы обновляет интегратор при вливании волн» (221.1-bug-sweep.md
  строка 11); интегратору передаётся текст этого файла для переноса в
  реестр при приёмке.
...
- Фаза 5 — гейты, ВСЕ ЗЕЛЁНЫЕ (точные числа — см. финальный отчёт):
  `cargo build --release` (nova-cli + nova-codegen) чист; `nova lint` —
  4 файла, 0 находок; arch-ratchet OK (lines=64505<=64505 path B,
  infer=348<=348 неизменно); `nova check std/src` — **147/26/60**,
  байт-в-байт канон; polaris `./nova.sh test src --strict-effects`
  (прогнан напрямую нужным биноvarником nova-chan против
  nova-polaris/src) — **37/0/18**, байт-в-байт канон; все 6 neg-фикстур
  (3 новых + 3 существующих) вместе в одном CU — 6/6 PASS; все 5
  pos-фикстур (новая + 4 существующих, включая РЕГРЕССИРОВАВШУЮ на Фазе
  1b `channel_elem_type_word_safe.nv`) вместе в одном CU — 5/5 PASS;
  `pos_channel_send_consume_share.nv` — PASS и одиночно, и в 5-файловом
  CU (парная прогонка брифа). ✅ ГОТОВО.
