...
## 8. Детач+panic → снос дублирующей ветки `resolve_mono_type_args_ch` (шаг 4, финал)

**Детач:** заменил хвост `resolve_mono_type_args_ch` (после неудачи `compose_mono_type_args_ch`) —
per-name loop + `Ok(lowered)` на его конце — на `panic!` в момент, когда loop дошёл бы до конца
(«unreachable-by-construction direct-hit»). Пересобрал release-бинарь, прогнал ВЕСЬ заданный корпус
заново (std/{collections,time,encoding}, standalone/88, aggregator `--strict-effects`) —
**0 паник на всех трёх**, PASS 30/0/16skip · PASS 68/0 · built OK, идентично прогонам §2/§5/§6.

**Снос:** заменил panic-детач на постоянный `Ok(legacy_pairs)` (без перестроения `lowered`), с
сохранением byte-parity гарантии (та же величина, что и была бы возвращена, доказано by construction
§2 + сохранена как safety-net независимо от корректности доказательства — эта ветка ВСЕГДА возвращала
`Ok(legacy_pairs)` на любом из трёх своих выходов, включая гипотетический «success»). Добавлен отдельный
trace-лейбл `fallback=post-compose-miss` вместо тихого поглощения — если будущий продюсер когда-нибудь
СДЕЛАЕТ эту точку достижимой, перепись увидит её отдельной меткой, а не потеряет сигнал.

**Пересобрано и перепрогнано ПОСЛЕ сноса** (не только после detach) — std 30/0/16skip, standalone 68/0,
aggregator built OK (31.64s) — идентично detach-прогону, регрессий нет.

**Гейт-1 (строки emit_c.rs строго убыли):** `git show d9af43662:.../emit_c.rs | wc -l` = **57307** →
рабочая копия = **57299** (−8 строк). `git diff --stat`: 20 insertions(+), 28 deletions(-).

---

## Итог волны-B

- **Легаси-движки Q10 (`resolve_mono_type_args` 19840, `resolve_method_level_subst` 21133) НЕ снесены** —
  подтверждён живой производственный fallback-трафик на ВСЁМ заданном корпусе (см. §7); снос требует
  Zone CH продюсера в `types/mod.rs` (СТОП-пункт задания, вне зоны/полномочий волны-B).
- **Перевод call-сайтов на `_ch`** — уже был завершён ДО этой волны (проверено, не моя правка).
- **Census «три hand-duplicated engines»** — третий (инлайн instance-method dispatch) давно
  консолидирован в `resolve_instance_call_subst` (W1-i.A); доккомент об этом устарел (не правил —
  вне моей зоны, помечаю для будущего чтения).
- **Единственная фактическая правка:** снятие доказанно-мёртвой дублирующей ветки в
  `resolve_mono_type_args_ch` (детач+panic → 0 паник на полном корпусе → снос) — emit_c.rs строки
  строго убыли (57307→57299, гейт-1 выполнен), byte-parity подтверждён (std/standalone/aggregator
  все PASS/OK идентично до и после).
