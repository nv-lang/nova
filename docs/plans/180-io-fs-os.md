...
type SeekFrom | Start(u64) | End(int) | Current(int)   // Start=u64 (абсолют ≥0, Rust-стиль); End/Current=signed int (относит., ±). Альт. Ф.0: Go-uniform `int`(i64)+negative→InvalidInput (эргономичнее: int-литералы без `as u64`)
