...
**Статус:** ✅ CLOSED 2026-07-12 — ВСЕ 4 фазы СЛИТЫ в main (Ф.1 retract D26 `c4f446477`, Ф.2 `to_cstr`
`1e42ca656`, Ф.3 str-exact-len `5b5f71f22`, Ф.4 FFI `8e8986e8c`). Ф.3 (emit_c.rs/аллокатор) выполнена
независимо на ветке `str-nul-199` — оговорка «после 196» ниже неактуальна (199 ушёл раньше, конфликта
с 196-агентом нет: 196 правит `infer_call_ret_c`, 199 тронул string-эмиссию — разные регионы). Приоритет
был P2 (язык-чистка). ⚠ Проверить в CI-green: не связан ли `std/encoding/compress/brotli` RUN-FAIL со снятым
NUL-инвариантом.
