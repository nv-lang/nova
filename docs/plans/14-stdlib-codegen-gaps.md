...
## Ф.2 — `non-constant expression in const declaration`

**Что:** [D33](../../spec/decisions/03-syntax.md#d33) разрешает любые
expression в `const`. Сейчас `emit_c.rs:821` принимает только
...
**Файлы:** `compiler-codegen/src/codegen/emit_c.rs` (const-emit path,
~120 строк включая lazy-init shim).

**Тесты:**
- `nova_tests/syntax/const_complex.nv` — record/array/call в const,
  spread-default паттерн (D52).
...
## Ф.4 — fn-поле в record + вызов через member-access

**Что:** [D35](../../spec/decisions/03-syntax.md#d35).

```nova
type Op { name str, f fn(int) -> int }

let op = Op { name: "inc", f: (x) => x + 1 }
op.f(5)                      // ❌ codegen эмитит direct-call вместо closure-call
```

В `emit_c.rs:3756 ExprKind::Member` (когда объект — record и поле fn-typed)
+ `ExprKind::Call` с `func = Member` — нужно проверить, что type поля —
fn-type и заэмитить через `NOVA_CLOS_CALL_*`.

**Затронутые std-файлы:** некоторые callback-tables (rate_limiter
strategy, потенциально crypto handlers). Не блокирует много, но это
основа для plug-in API.

**Файлы:** `compiler-codegen/src/codegen/emit_c.rs` Member-routing
(~40 строк).

**Тесты:**
- Раскомментить пункт 3 BOOTSTRAP-GAP в `fn_first_class.nv` —
  `op.f(x)` / `(@f)(x)` через record fn-field.
...
| **Ф.2** const non-trivial | 🔥 высокий | ~120 строк, 1-2 дня | нет |
| **Ф.3** free-fn-as-value | ✅ ЗАКРЫТ | ~95 строк, 5 тестов | — |
| **Ф.4** fn-в-record | средне-высокий | ~40 строк, 0.5 дня | нет |
