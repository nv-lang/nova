...
**Статус:** ✅ **ЗАКРЫТ** (2026-05-12). Ф.1 ✅, Ф.2 ✅, Ф.3 ✅, Ф.4 ✅,
Ф.6 ✅, Ф.7 ✅ (6 из 7 фаз закрыты в declared scope).

- **Ф.5 (cross-file resolve)** вынесена в отдельный [Plan 35](35-cross-file-resolve.md) — низкий ROI, заслуживает самостоятельного scope'а.
- **Накопленные блокеры std/** (Ф.7-bis, hashmap protocol-dispatch, tuple-types и т.д.) — формулируются отдельными планами по приоритету.
- **Текущий регресс stdlib type-check** (50/50 → 33/44 на 2026-05-12) — закрывается [Plan 34](34-stdlib-typecheck-and-compile-fix.md).
...
## Ф.5 — Cross-file resolution для compile-mode → [Plan 35](35-cross-file-resolve.md)

> ⛔ **ВЫНЕСЕНА В ОТДЕЛЬНЫЙ ПЛАН** (2026-05-12). См.
> [Plan 35](35-cross-file-resolve.md). Раздел оставлен для контекста.
...
**Что осталось за рамками этого плана (2026-05-12 closure):**
- Ф.5 (cross-file resolve) → [Plan 35](35-cross-file-resolve.md). Cheap-вариант
  («known limitation, multi-file через interp») остаётся в силе как
  default; full реализация — Plan 35.
- «Накопленные блокеры» std/ (см. таблицу выше) — каждый формулируется
  в отдельный план при необходимости. Type-check регресс — [Plan 34](34-stdlib-typecheck-and-compile-fix.md).
