...
**Статус:** 📋 PROPOSED 2026-07-10 (архитектура ПОДТВЕРЖДЕНА владельцем). **Приоритет:** P2.
...
- **Ф.1** .c-шим к C-TLS + .nv-фасад std/tls (сохранить публичную поверхность + тесты).
- **Ф.2** удалить Rust tls_shim + `[ffi.staticlib]`-cargo + legacy detect_tls; доки на `[ffi]`.
- **Ф.3** (после [03.1]) вынести nova-tls в отдельную репу; монорепо — внешний dep.
