...
## Открытый под-вопрос (нужен sign-off): раскладка пакета

Мир: манифест+README в корне, код в **`src/`** (Rust/npm/Python/Zig). Nova сейчас —
плоско (Go-стиль, `src/` убран 2026-05-22). Владелец не хочет исходники в корне.
- **Опция src/** (мировой стандарт): вернуть `src/`; чистый корень; ради согласованности
  и `std` перевести на `src/` (отдельный мех-план).
- **Опция плоская:** оставить, но раскладывать по подпапкам-доменам (как std).
Рекомендация — `src/`. **РЕШИТЬ до сборки nova-tls.**
