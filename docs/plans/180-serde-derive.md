...
| Q22 | Timestamp/Duration/Monotonic (std/time, Plan 175) — Ред.2 | **`Timestamp` ↔ RFC3339-строка** (D342-mapping); `Duration` — секунды/ISO-8601, финализировать с 175-owner в Ф.0; **`Monotonic` НЕсериализуем** → `E_SERDE_FIELD_NOT_SERIALIZABLE` + neg-тест (175 §7 требует verify отсутствия derive-пути) | Канон peers: Go `time.Time`=RFC3339, Rust chrono-serde=RFC3339; антипример — Swift `.deferredToDate` (Double секунд от 2001, непереносимо) |
...
- **Timestamp (Q22):** RFC3339 round-trip (когда 175 приземлил Timestamp); `Option[Option[T]]`-ambiguity зафиксирована тестом (Some(None)==None на wire — документированная потеря).
...
- **Plan 175 (std/time)** — Q22: `Timestamp` wire-format (RFC3339) согласовать с 175-owner; **`Monotonic` НЕсериализуем** (E_SERDE_FIELD_NOT_SERIALIZABLE + neg) — 175 §7 требует verify отсутствия derive-пути.
