...
| **unknown-field-policy** | **= IGNORE by-default (forward-compat, serde-паритет) + opt-in `deny_unknown_fields`** | ignore default; `deny_unknown_fields` opt-in = | ignore default | **DENY default** (`ignoreUnknownKeys=false`) | **ignore молча** (footgun) | ignore default | configurable | **DENY default** (`ignore_unknown_fields=false`) |
...
| Q5 | unknown-field policy: deny vs ignore | **IGNORE by-default (forward-compat), `#serde(deny_unknown_fields)` → opt-in strict (`DeError{UnknownField}`).** | serde default = ignore (API-evolution). Лучше Go (молча ignore, нет opt-in deny). Considered-alternative: kotlinx и Zig — DENY-default (exception на unknown) — отвергнут ради forward-compat; deny остаётся opt-in. |
