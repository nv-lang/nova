...
- 🔴 **`format_num` float shortest-round-trip — RED (реальный блокер критического пути).** Корень: `compiler-codegen/nova_rt/conv.h:142` — `nova_f64_to_str` = `snprintf("%g")` = **6 значащих цифр**. Проба: `str.from(0.1)=="0.1"` ✅, но `str.from(3.141592653589793)` ❌ (→ `"3.14159"`), `str.from(1234567.89)` ❌ (→ `"1.23457e+06"`). **⇒ `json.nv format_num` (json.nv:693 `str.from(n)`) ЛОССИ для произвольных f64 → `decode(encode(v))==v` на float-полях ЛОМАЕТСЯ.** Это НЕ только serde — **существующее json float-кодирование уже лосси** для >6-знач-цифр. Гейт §8.0 запрещает forward этого на критпути. **Prereq Ф.4:** shortest-round-trip f64-форматтер (Ryū/Grisu/`%.17g`-min) в runtime ИЛИ `format_num` — но blast-radius огромен (меняет вывод КАЖДОГО float→str в языке; масса тестов ждут `%g`-форму) → отдельный под-заход, НЕ drive-by. Followup `[M-180-f64-shortest-roundtrip]`.
