...
> **Уровень:** Top-level.
> **Создан:** 2026-06-26. **Статус:** `✅ ВЫПОЛНЕН (record-path, honest-complete) — 2026-07-05`. **Completeness-аудит close-out (2026-07-05):** adversarial-аудит нашёл 6 реальных дыр полноты record-path — ВСЕ починены (не отложены): `Option[value-record]` + `HashMap[str,value-record]` mono-ordering (value-record-field NovaOpt/pointer hoist в `value_record_defs_buf`); `Option[Option[int]]` (рекурсивный inline null-check + typed `None as Option[T]`) / `Option[Vec[str]]` (`.serialize` receiver-invariant return-fallback); narrow-скаляры i8..i64/u8..u32/uint/f32 (synthesizer-inline ser/deser + range-guard; char/i128/u128 → typed `E_AUTO_DERIVE_FIELD_LACKS_PROTOCOL`, НЕ ICE); `[]u8`→base64 ([M-180-bytes-base64] CLOSED); encode-side lossy-int guard (`SerLossyInteger`, симметрично `is_exact_int`). Плюс латентный `?`-lowering no-op (глотал serialize-Err в `json_encode`) и early-prototype gap для structural `nova_opt_eq`. Гейты: conformance 38/0; extended round-trip (`autoderive_ext.nv`) PASS; neg typed-error PASS; zero-regression vs parent 64675407 (46-file sample, 0 delta). Container-nested narrow-scalar → typed-error (честный scope-out [M-180-container-narrow-scalar]). **Ф.1 (протоколы/ошибки/data-model) + Ф.2-record (компилятор auto-derive `#impl(Serialize+Deserialize)`) + Ф.4 (JSON-backend + typed `json_decode[T]`/`json_encode`) — ПРИЗЕМЛЕНЫ.** Ф.2-sum — externally-tagged ✅ ПРИЗЕМЛЕНО (2026-07-06, D345): `#impl(Serialize+Deserialize)` на sum синтезирует externally-tagged wire (unit→`"V"`; single→`{"V":x}`; tuple→`{"V":[a,b]}`; record→`{"V":{f}}`); gate `[M-126-sum-*-rich]` закрыт Ф.1; runtime `is_str`+`UnknownVariant`; verify `nova_tests/serde/sum_autoderive.nv` 8 блоков; conformance 53/0. Ф.5 (internal/adjacent/untagged) — GATED на `#serde`-attribute-инфре ([M-180-serde-tagging-modes]/[M-180-serde-attributes]). Ф.3 (атрибуты `#serde`) — followup [M-180-serde-attributes] (record-DTO round-trip'ится на каноничных именах). **Реализация потребовала 11 компилятор-фиксов** (сверх 2, ожидавшихся §0): value-record-receiver+generic-method; static-method Path+Member+typevar return-infer; turbofish free-fn + Result/Option return-resolve; primitive-instance-generic-method mono; bound-satisfaction `#impl(builtin)` short-circuit; `type_refs_equiv_modulo_self` рекурсия; serde-inject-перед-check. **Гейты:** #impl round-trip (flat/nested/Vec/Option/float) PASS; typed DeError (Syntax/MissingField/UnexpectedType/LossyInteger) PASS; neg non-serializable→compile-error PASS; conformance single-CU PASS; регрессия-сэмпл clean (все FAIL — pre-existing str.len/namespace/folder-conflict). **🔓 Разблокирует Plan 178 typed `.json[T]` для record-DTO.** (Ред. 2 — 2026-07-03: аудит 7-языков; M-161 CLOSED; Q22 Timestamp.)
> **Продолжение:** [180.1 — паритет с Rust serde + где лучше оригинала](180.1-serde-parity-and-beyond.md)
> (📋 PROPOSED 2026-07-22): field-attributes (rename/skip/default/alias/flatten — ГЛАВНАЯ дыра
> паритета), узкие скаляры в контейнерах, char/i128, untagged, `Json.decode[T]`-API, `#serde(with)`;
> плюс «лучше оригинала»: strict-by-default для неизвестных полей, path-rich ошибки из коробки,
> встроенный schema-gen, compile-time валидация wire-контракта.
...
> компилятор-гейтов** (🔴 `[M-126-sum-*-rich]` OPEN; `[M-161-parametric-return]` ✅ CLOSED 2026-06-16, но остаётся typevar-receiver-ограничение D355 — re-verify Ф.0; #serde-attr AST — нет `attrs`-поля;
...
> рискованный план по объёму скрытой компиляторной работы. Ред. 2 пройдена 2026-07-03.
...
> **D-блоки (NEW):** D340–D346 (data-model+протоколы / record-auto-derive-контракт / data-model↔synth-mapping / атрибуты+валидация / JSON-backend / enum-tagging / numeric+depth-soundness). **D-карта (Ред.2 2026-07-03):** committed high-water 3xx = **D355** (D354/D355 уже в спеке; прежний якорь «D332 (178)» устарел — 178 renumber'ится в **D357–D362** и D340–D346 не задевает); D333–D339 = резерв Plan 179 (+буфер). Старт **D340** (свободны, grep=0 — verify Ф.0).
...
> - 🟡 **`[M-161-parametric-return]` — ✅ CLOSED 2026-06-16** (776447ab/9065c637/3ba2dacc; spec **D355**, 02-types.md). **НО** Q13-решение (compiler-mono) может уцелеть по ДРУГОЙ причине: D355-blanket требует **typevar-ресивер**, а `fn[T Serialize] Option[T] @serialize` — конкретный generic-ресивер, вне blanket-механики → **re-verify blanket-`.nv` в Ф.0**; до подтверждения container-conformance = компиляторные monomorphic special-cases (Ф.2).
> - 🟡 **Attribute-инфра** — `#serde(k=v)` на полях/вариантах **не существует** (AST `RecordField`/`SumVariant` без `attrs`; парсер знает только hardcoded type-attrs + field-level `#visible_to`). Ф.3 расщеплён на **Ф.3a (parser+AST+validation)** → **Ф.3b (synth-consumption)**. Field-attr-прецедент = `#visible_to` (parser/mod.rs:4333 — снимок 2026-07-03, искать по `visible_to`) — расширяем, не с нуля.
> **DEPS:** Plan 177 (D325, Result-everywhere — ✅ landed); `std/encoding/json` (✅ production, RFC 8259); `std/encoding/base64` (для `bytes`↔base64); auto-derive-машина `#impl(P)` (D109 amend + D230, ✅ landed для record/tuple — [auto-derive-guide.md](../guide/auto-derive-guide.md)).
> **Координация:** 🔓 **этот план разблокирует Plan 178 (std/http)** typed `.json[T]()`/`.json(v T)` (178 Q20 HARD-GATE, [178 Q20/§3.5/§3.client](178-std-http.md)) — для **record-DTO** (Ф.4, record-only Ф.2). Dynamic `.json() -> JsonValue` в 178 приземляется СЕЙЧАС над существующим json (не ждёт serde).
...
## 0. Ф.0-VERIFY — эмпирические находки (2026-07-04)

> Прогон: parent-бинарь nova-p180 (main `d97c0dbe`, debug), `nova test` C-codegen-путь.
> Все пробы удалены (net-zero на коде). Гейты проверены ФАКТОМ, не предположением (§7.7).

**Разблокировано (язык-уровень — Ф.1 shape РАБОТАЕТ):**
- ✅ **Q12 generic-bound protocol-методы компилируются и исполняются.** Проба: `type Ser protocol { mut @put_int(v int) }` + `type Serial protocol { @serial[S Ser](s mut S) -> () }` + ручной `fn Point @serial[S Ser](s mut S)` → round-trip PASS. Механизм — **D119 (method-level type params) + D72 (bounds)**; method-level `S` инферится из арг-типа. **Serialize/Serializer-форма плана валидна as-is.**
- ✅ **Static protocol-метод с generic-bound + `Result[Self,E]` работает.** Проба: `type Deserial protocol { .deserial[D Deser](d mut D) -> Result[Self, DeErr] }` + `fn Point.deserial[D Deser](d mut D)` через `?` → PASS. **Deserialize-форма плана (`.deserialize[D Deserializer]`) валидна.**
- ✅ **`consume protocol` (type-модификатор §3.2 SeqSerializer) ПАРСИТСЯ и type-check'ается** (`type SeqSer consume protocol { mut @element(..), consume @end() }`).
- ✅ **Мьютуально-рекурсивные protocol-bounds** (`Serial` ссылается на `Ser`, `Serializer.serialize_some[T Serialize]`) компилируются.

**Q13/D355 re-verify — ОБНОВЛЕНИЕ ПЛАНА (ожидалось «НЕТ» → факт «ДА»):**
- ✅ **Generic-receiver container-impl РАБОТАЕТ как plain `.nv`.** Проба: `fn Option[T Serial] @serial[S Ser](s mut S)` (match Some/None) — PASS; прецедент уже в проде: `fn Option[T Debug] @debug` / `fn Result[T Debug, E Debug] @debug` в `std/prelude/protocols.nv:660,681`. **⇒ container-conformance (`Option[T]`/`[]T`/`HashMap[str,V]`/nested) выразим `.nv`-impl'ами, НЕ обязательно compiler-side mono.** Это СНИЖАЕТ compiler-объём Q13/D355 против пессимизма плана. (D355-blanket typevar-ресивер здесь и не нужен — конкретный generic-ресивер `Option[T]` — это обычный D119/D72-impl, а не blanket.) **Правка для Ф.1/Ф.2:** container-impl'ы в `.nv` (как Debug), синтезатору остаются только record-struct и (гейт) sum.

**Гейты — ПОДТВЕРЖДЁННО РЕАЛЬНЫЕ:**
- 🔴 **`[M-126-sum-equal-rich]`/`-clone-rich`/`-hash-rich` — РЕАЛЬНО OPEN** (verified): `compiler-codegen/src/protocols/auto_derive.rs:554-845` — sum-arm'ы = заглушки (equal=identity-fallback, hash=literal 0, clone=self-placeholder, compare/display/debug=placeholder). Rich per-variant synth НЕ существует. **Ф.2-sum/Ф.5 действительно гейтнуты.** (Уже в backlog:64 — verified, не дубль.) НЕ блокирует Ф.1/Ф.2-record/Ф.4-record-DTO (sum не нужен).
- ✅ **`format_num` float shortest-round-trip — RESOLVED (keystone-фикс 2026-07-04, `[M-180-f64-shortest-roundtrip]` CLOSED).** Был RED: `nova_f64_to_str` = `snprintf("%g")` = 6 знач.цифр → лосси (`str.from(3.141592653589793)`→"3.14159", `str.from(1234567.89)`→"1.23457e+06"), `decode(encode(v))==v` на float-полях ломался. **Фикс (без упрощений):** shortest-round-trip форматтер — минимальная десятичная `s`, т.ч. `strtod(s)==v` бит-в-бит. Единая точка `nova_rt.h::nova_f64_shortest`/`nova_f32_shortest` (§0/§3): default `%g` первым (zero test-churn на ≤6-знач-значениях), эскалация 7..17 (f64)/7..9 (f32, `strtof`) до первого точного round-trip. Funnel всех float→str: `str.from`/`@display`/`@debug`/`${x}`/`StringBuilder.append`/`println(float)`. Сопутств.: `str.from(f32)`-dispatch (emit_c.rs, был truncate-to-int); f32 `@display`/append переведены с `str.from(@ as f64)` (widening-tail) на f32-precise `str.from(@)`. **Гейты пройдены:** conformance 38/0; round-trip pos-тест (nova_tests/plan91_13/json_roundtrip_float_shortest.nv); float JSON `decode(encode(v))==v` PASS. **⇒ критпуть Ф.4 (JSON-backend) по float-round-trip РАЗБЛОКИРОВАН** (§8.0-гейт удовлетворён).

**Codegen-блокеры для specced `value`-типов (реальные, узкие):**
- ⚠️ **`Result[T, <value-record>]` как return-тип PROTOCOL-метода → missing-mono-struct CC-FAIL** (`unknown type name NovaRes_..._NovaValue_DeErr`). A/B: `DeErr value` → CC-FAIL; `DeErr {…}` (heap) → PASS (всё прочее идентично, protocol полностью impl'ится/зовётся). Free/generic-fn Result[_, value-record] — OK (не protocol-контекст). **⇒ `SerError`/`DeError` специфицированы `value {…}` (§3.7), но каждый Serializer/Deserializer-метод возвращает `Result[_, SerError/DeError]` (protocol-метод) → упрётся в баг.** Правка: **`SerError`/`DeError` = heap-record** (default; корректнее — ошибки не скаляры D215) ИЛИ фикс mono-enrollment. Followup `[M-180-valuerecord-err-protocol-method-mono]`.
- ⚠️ **`value`-record RECEIVER + method-level-generic метод → CC-FAIL** (receiver передан by-value туда, где mono ждёт `*T`: `passing 'NovaValue_Point' to incompatible type 'NovaValue_Point *'`). Heap-record receiver → OK. **⇒ scalar/value-record DTO с synth'ированным `@serialize[S Serializer]` (Ф.2) упрётся.** Правка/фикс до Ф.2 value-record-покрытия. Followup `[M-180-valuerecord-receiver-generic-method]`.

**Аудиты (подтверждают Ред.2):**
- ✅ **`Json.parse` depth-bound — НЕТ** (`depth` только в pretty-printer json.nv:778-817; рекурсивный parse без лимита) → §6 prereq реальный.
- ✅ **Raw-numeric-token — НЕТ** (лексер сразу f64: json.nv:218 `NumTok(f64)`, :411 `read_number()->f64`) → Q15 на f64, arbitrary-precision §11.
- ✅ **AST `RecordField`/`SumVariant` — БЕЗ `attrs`** (`ast/mod.rs:1135,1189`; RecordField имеет `visible_to`, но не serde-attrs) → Ф.3a реально добавляет AST-поле; field-attr-прецедент `#visible_to` (parser/mod.rs:4333) — расширяем.
- ✅ **D340–D346 свободны** (grep=0 в `spec/decisions/`), committed high-water 3xx = D355 → **D-карта verified, старт D340**.

**Вывод Ф.0.** Ф.1-shape (протоколы/generic-bound/static/consume/container-`.nv`) язык-уровнем **РАЗБЛОКИРОВАН**. Но ПОЛНЫЙ Ф.1–Ф.4 без упрощений упирается в: (1) f64-formatter RED (float-round-trip критпуть), (2) 2 codegen-бага для specced `value` SerError/DeError, (3) объём auto-derive-synth (generic-param-метод + static + companion `UserVisitor`-ТИП — синтез ТИПА, не только метода — новое, не reuse), (4) полная 8-протокольная сеть + JSON-backend `.nv`. Это плана-масштаб «Волна 2/3» (README §Очередность), НЕ один заход без упрощений. **Данный заход = Ф.0-VERIFY (net-zero на коде; статус+находки+backlog).** Реализация Ф.1+ начинается с правок выше (heap-errors или mono-фикс + f64-formatter-под-заход).
...
**🔴 Прямой HARD-GATE — этот план разблокирует Plan 178.** [178:9-10](178-std-http.md) явно гейтит **typed `.json[T]()`/`.json(v T)`** на «**NEW serde-sub-plan** (auto-derive `Serialize`/`Deserialize`; owner 2026-06-26)»; dynamic `.json() -> JsonValue` Plan 178 приземляет сам над существующим `std/encoding/json`, но typed-форма — то, ради чего HTTP-клиент эргономичен (`client.get(url).send()?.json[User]()` вместо ручного дерева) — **не может приземлиться без Ф.4 этого плана**. То есть Plan 180 — на критическом пути std/http (как `std/encoding/compress` для auto-decompress, [178 Q20](178-std-http.md)). **Важно (честно):** typed `.json[User]` для HTTP нужен только для **record-DTO** (request/response — это структуры, не sum-типы с payload), а record-auto-derive приземляется в Ф.2 «сейчас». Sum-auto-derive + enum-tagging (Ф.2-sum/Ф.5) **НЕ на критическом пути 178** — гейт открывается record-only.
...
- **🏆 AUTO-DERIVE через ЕДИНЫЙ `#impl`-mechanism — headline-win. Компилятор синтезирует `Serialize`/`Deserialize` той же memberwise-рекурсивной машиной, что уже авто-выводит `==`/`clone`/`hash`/`compare`/`display`** ([auto-derive-guide.md](../guide/auto-derive-guide.md): D109 amend + D230, memberwise рекурсивно по полям). Serde становится **седьмым/восьмым членом** этого семейства — никакого `#[derive(Serialize)]`-обвеса как у Rust (там это отдельный proc-macro-плагин), никакого отдельного conformance-ритуала (Swift с SE-0295/5.5 синтезирует Codable вкл. enum-с-payload, но ТОЛЬКО external-tagged; не-external — ручной `init(from:)`), никакой runtime-рефлексии по тегам (Go/Java). **Решение по UX (§3.0 Q1, честно):** Nova использует **тот же `#impl(Serialize + Deserialize)` opt-in**, что и `#impl(Equal+Hash+Clone)` — НЕ «магический default-on» (это сломало бы консистентность семейства auto-derive, где `==`/`clone` тоже требуют `#impl`). **Differentiator не в «нулевой аннотации», а в «нулевой церемонии тела»:** одна и та же `#impl`-аннотация даёт и `==`, и serde **бесплатно по структуре**, тогда как Rust требует ОТДЕЛЬНЫЙ `#[derive(Serialize, Deserialize)]` + crate, Swift — conformance + ручной код для enum, Go — медленную рефлексию. «Структура типа — её serde-контракт» через уже-существующий, уже-знакомый пользователю механизм.
...
- **🏆 format-agnostic data-model (как serde, но lean для Nova).** Протокол `Serialize`/`Deserialize` говорит на **format-agnostic data-model** (`bool/int/uint/float/str/bytes/option/seq/map/struct/enum/unit`), а не на JSON напрямую → **один и тот же derived-тип** едет в JSON **сейчас** и TOML/binary **позже** (§11) **без перекомпиляции типа**. Go `encoding/json` зашит в JSON; Swift `Codable` агностичен (✅ паритет), но требует conformance-ритуал (enum-synth external-only, SE-0295) и известно нескор (container-boxing/dynamic dispatch JSONDecoder — 2023 rewrite улучшил, всё равно не mono-zero-cost); Nova = агностичность serde + единый `#impl`-механизм + mono-zero-cost **одновременно**.
...
- **🟡 enum-tagging full matrix — differentiator «позже» (честно).** Externally-tagged по умолчанию (`{"Variant": payload}`), `#serde(tag="type")` internal, adjacent, untagged — на **родных** Nova sum-типах с structural-eq (Plan 172.1). Swift с SE-0295 (5.5) синтезирует Codable и для enum-with-associated-values, но ТОЛЬКО external-tagged (не-external — ручной `init(from:)`, конфигурации нет); Go вообще не имеет sum-типов. **НО:** sum-auto-derive синтез **на main НЕ существует** (`[M-126-sum-*-rich]` OPEN) → это **гейт/под-план 180.2**, НЕ «бесплатный reuse». Заявляем differentiator честно как **планируемый**, гейтнутый на sum-synth-инфру.

## 2. Эталон (cross-lang serde/derive)

**🏆** = Nova **строго лучше** лучшего peer'а; **=** = паритет; **🟡** = таргет-differentiator, гейтнут (sum-synth/later).

| Фича | **Nova-target** | Rust serde | Swift Codable | Kotlin kotlinx | Go encoding/json | Java Jackson/Gson | TS zod/io-ts | Zig std.json |
|---|---|---|---|---|---|---|---|---|
| **derive-mechanism** | **🏆 единый `#impl(Serialize+Deserialize)` (та же машина, что `==`/`clone`/`hash`; нет отд. плагина/conformance)** | `#[derive(Serialize, Deserialize)]` (отд. proc-macro crate) | compiler-synth (`: Codable` conformance; enum-synth external-only SE-0295) | compiler-plugin `@Serializable` | **runtime reflection** + struct-теги (no derive) | **runtime reflection** + аннотации | runtime schema-объект (ручной `z.object`) | **comptime `@typeInfo` — НОЛЬ аннотаций** (но JSON-only + implicit-footgun) |
| **runtime-cost** | **🏆 zero (monomorphized compile-time, как `@equal`)** | zero (monomorphized) = | low (synth; JSONDecoder container-boxing — известная боль) | low (plugin-generated) | **высокий (reflection per-call)** | **высокий (reflection)** | средний (interpret schema) | zero (comptime) = |
| **type-safety** | **🏆 статич.: опечатка поля/тега = COMPILE-ERROR; mismatch = typed `DeError`+path** | статич. = | статич. = | статич. = | **stringly теги — опечатка КОМПИЛИТСЯ, silent-loss** | **аннотации-строки, опечатка молча** | статич.-ish (schema отдельно от типа — drift) | статич., НО implicit: priv/resource-поля едут МОЛЧА (Nova opt-in звучнее); ошибки byte-offset, без typed path |
| **format-agnostic** | **🏆 data-model agnostic (JSON сейчас, TOML/binary позже без перекомпиляции типа) + единый `#impl`** | agnostic (эталон) = | agnostic (conformance; enum external-only) | agnostic (pluggable) = | **JSON-only** | формат-плагины (но reflection) | **JSON-only** | **JSON-only** (std.json) |
| **record auto-derive** | **🏆 `#impl` memberwise (flat/nested/generic/recursive) СЕЙЧАС** | `#[derive]` = | synth (struct ✅) = | plugin = | reflection | reflection | ручной schema | любой struct БЕЗ аннотаций (comptime) |
| **enum/sum auto-derive + tagging** | **🟡 external default + internal/adjacent/untagged — гейт на sum-synth (`[M-126-sum-*-rich]`), 180.2** | все 4 режима (эталон) | synth external-only (SE-0295); не-external = ручной `init(from:)` | `@JsonClassDiscriminator` хорошо = (но default INTERNAL) | **нет sum-типов** | `@JsonTypeInfo` (verbose) | union-схемы (ручные) | ручные `jsonStringify`/`jsonParse`-хуки |
| **unknown-field-policy** | **🏆 REVERSED 2026-07-22 (Plan 180.1 Ф.7, D436): DENY by-default** (`DeError{UnknownField}`) + opt-out `#serde(allow_unknown)` — supersedes the IGNORE-by-default row below | ignore default; `deny_unknown_fields` opt-in = | ignore default | **DENY default** (`ignoreUnknownKeys=false`) | **ignore молча** (footgun) | ignore default | configurable | **DENY default** (`ignore_unknown_fields=false`) |
| **numeric fidelity** | **= JSON→f64-граница 2^53 явно проверена (`OutOfRange`/`LossyInteger`), не silent** | serde_json `Number` arbitrary-prec (строго лучше) | Codable f64-граница (=) | (=) | `json.Number` string-preserving (лучше) | BigInteger/BigDecimal (лучше) | number-граница JS (=) | **лучше: parse из сырого токена — lossless u64>2^53** |
...
| **attributes** | **= `#serde(rename/rename_all/skip/default/flatten/tag/content/untagged/deny_unknown_fields)`** | полный набор (эталон) = | `CodingKeys` (нет rename_all/flatten) | `@SerialName/@Transient/…` = | теги (узко) | богатые (reflection) = | `.transform/.default/.passthrough` = | НЕТ attr-набора (хуки) |
| **zero-copy / borrow** | **🟡 позже (§11): borrow `&str`/`&[]u8`** (сейчас owned) | `#[serde(borrow)]` (эталон) | нет (owned) | нет (owned) | нет (копии) | нет | нет | слайсы из входного буфера (alloc-опции) |

**Взять:** serde — **data-model + Serializer/Deserializer-разделение + enum-tagging-matrix + attribute-набор + recursion_limit** (золотой стандарт); Swift Codable — **compiler-synth-дух** (container-абстракция Keyed/Unkeyed/SingleValue, `CodingKeys`); Kotlin — **plugin-synth + format-pluggability**; zod — **path-rich validation-ошибки**. **Избегать:** Go/Java **runtime-рефлексию** + JSON-привязку Go + silent tag-typo + silent-unknown-drop; Swift не-external enum-tagging (ручной `init(from:)`), `Dictionary[Int:V]`→JSON-МАССИВ-footgun и `.deferredToDate`; Zig implicit-serialize (priv/resource-поля едут молча); zod **schema-отдельно-от-типа**. **Доказательство ≥ best-peer построчно:** 🏆 на **derive-mechanism / runtime-cost / type-safety / format-agnostic / record-auto-derive**; **=** на unknown-policy / attributes / depth / numeric (паритет — numeric честно НЕ лучше serde arbitrary-precision, помечено `=` не 🏆); **🟡** на enum-tagging / zero-copy (таргет, гейтнут). **Headline:** единый `#impl`-механизм даёт serde той же машиной, что `==`/`clone`/`hash`. **Квалификация (Zig):** у Zig std.json церемония ещё ниже (НОЛЬ аннотаций — comptime-рефлексия на любом struct), но ценой JSON-only, implicit-footgun (priv/resource-поля едут молча — Nova opt-in `#impl` звучнее), отсутствия attr-набора и typed-path-ошибок (byte-offset). Nova = осознанный opt-in + agnostic-модель + typed-path + attrs — комбинации нет ни у кого.

## 3. Архитектура

**Принцип (auto-derive-precedent, PURE).** serde в Nova — **format-agnostic протокол + value-логика**, БЕЗ I/O-эффекта (чистый кодек над значениями/байтами; net-триада здесь не нужна — нет сокета, нет release-обязательства; module-conventions явно: «PURE codec/serde need NO effect»). Headline-механизм: **компилятор синтезирует тела `@serialize`/`.deserialize` для `record` (Ф.2 «сейчас») и `sum` (Ф.2-sum, гейт 180.2)** через `#impl(Serialize + Deserialize)` — той же машиной, что уже синтезирует `@equal`/`@clone`/`@hash` ([auto-derive-guide.md](../guide/auto-derive-guide.md), `E_AUTO_DERIVE_*`). Атрибуты `#serde(...)` лишь **кастомизируют** (rename/skip/flatten/default/rename_all/tag) или **opt-out**. Всё fallible → `Result[T, SerError|DeError]` (R1/D325).

> **🔑 Это КОМПИЛЯТОРНАЯ работа.** План описывает контракт + форму эмитируемого кода для имплементора (как `#impl(Clone)` synthesize'ит memberwise-копию). Сам план — НЕ код; Ф.2 — расширение auto-derive-движка (**`compiler-codegen/src/protocols/auto_derive.rs`**: `synthesize_equal`/`hash`/`clone`/`compare`/`display`/`debug` + AST→AST pass `inject_synthesized_methods`; вызовы из main.rs/test_runner.rs — **НЕ emit_c.rs**; в emit_c.rs живёт только structural-eq 172.1: `emit_field_eq`/`register_container_eq_mono` = Q13-зеркало) на два новых протокола. **Честный объём (Ред.2):** машина single-method и инжектит только МЕТОДЫ — для serde нужны generic-param `[S Serializer]`, static `.deserialize` и синтез companion-ТИПА (`UserVisitor`); inject-pass типы не создаёт → Ф.2 НЕ чистый reuse (registration `is_builtin_protocol`/`builtin_protocol_method` — тривиальна; method+type-synthesis — новое). **Честная граница:** record-synth переиспользует существующий memberwise-проход (✅ landed для record/tuple); **sum-synth НЕ существует** — `[M-126-sum-equal-rich]`/`-clone-rich`/`-hash-rich` OPEN; Plan 172.1 добавил sum-**equality** на codegen-emit-уровне (ДРУГОЙ механизм — структурный per-type `emit_field_eq`, НЕ синтезированный метод в method_table). Поэтому sum-serde — **новая synth-инфра**, гейт.
...
### 3.0. Закрытые решения (Q1–Q22 — РЕШЕНЫ; Q22 добавлен Ред.2)

| # | Вопрос | РЕШЕНИЕ | Обоснование (peer) |
|---|---|---|---|
| Q1 | auto-derive **by-default** vs `#impl`-opt-in | **`#impl(Serialize + Deserialize)` opt-in — ТА ЖЕ аннотация, что `#impl(Equal+Hash+Clone)`.** НЕ «магический default-on». `#serde(...)` кастомизирует/opt-out'ит. | **Консистентность с семейством auto-derive** (`==`/`clone`/`hash` тоже `#impl`-opt-in, [auto-derive-guide.md:6-8](../guide/auto-derive-guide.md)). Default-on сломал бы единообразие и навязал бы serde release-resource-типам. Differentiator = «единый механизм + нулевое тело», НЕ «нулевая аннотация». Rust требует ОТДЕЛЬНЫЙ `#[derive]`-плагин — у Nova одна `#impl`-семья. |
| Q2 | data-model | **Lean serde-набор (12 кейсов): `bool/int/uint/float/str/bytes/option/unit/seq/map/struct/enum`.** Data-model `Int` = **i64-wide** (Nova `int` = `intptr_t`, address-sized — Plan 133: serialize = widen до i64; deserialize в `int` = range-check под target-ширину, 32-bit: |v|≥2³¹ → `OutOfRange`); `uint` = u64-wide аналогично; `float`=f64, `char`→str-1, кортежи→`seq`, newtype→прозрачен. | serde-модель — золотой стандарт, но 29 методов избыточны (один integer/float-тип). 12 visit-методов vs serde 29. |
...
| Q4 | enum-tagging default | **Externally-tagged (`{"VariantName": payload}`) — default.** `#serde(tag="t")` internal, `#serde(tag,content)` adjacent, `#serde(untagged)` untagged. Unit-вариант → bare-строка `"VariantName"`. **ГЕЙТ Ф.2-sum/180.2.** | serde-канон; Swift SE-0295 тоже external-default. Контрапункт: kotlinx sealed-default = INTERNAL (`classDiscriminator="type"`) — отвергнут: external не конфликтует с полями payload и не требует struct-only. |
| Q5 | unknown-field policy: deny vs ignore | ~~IGNORE by-default (forward-compat), `#serde(deny_unknown_fields)` → opt-in strict.~~ **REVERSED 2026-07-22 (владелец, Plan 180.1 Ф.7, D436): DENY by-default** (`DeError{UnknownField}`), `#serde(allow_unknown)` → opt-out ignore. `deny_unknown_fields` retained as an accepted no-op (already-default). | serde default = ignore (API-evolution). Лучше Go (молча ignore, нет opt-in deny). Considered-alternative: kotlinx и Zig — DENY-default (exception на unknown) — **этот альтернативный путь и был выбран владельцем при реверсе** (no-magic/AI-first: тихая опечатка в конфиге хуже, чем громкая ошибка при декодировании). |
...
| Q13 | generic-conformance контейнеров (`Option[T]`/`[]T`/`HashMap`) | **Compiler-side monomorphic special-cases в синтезаторе, НЕ blanket-`.nv`-impl.** Синтезатор эмитит `@serialize`/`.deserialize` для container-типа per element-mono. | **`[M-161]` ✅ CLOSED (D355, 2026-06-16)**, но D355-blanket требует **typevar-ресивер** — `fn[T Serialize] Option[T] @serialize` (конкретный generic-ресивер) вне blanket-механики → re-verify Ф.0; до подтверждения — mono, зеркало `emit_field_eq`/`register_container_eq_mono` (Plan 172.1 `f56cd7b7`/`bd56022e`). |
...
| Q21 | per-format / направление имён | **Конвенции per-format → на BACKEND** (`json.encode_with(v, .{rename_all: CamelCase})` / `decode_with`); тип хранит каноничные имена, поля БЕЗ атрибутов. **Направления** (ser-имя ≠ de-имя; приём алиасов) → `#serde(rename(ser="x", de="y"))` + `#serde(alias=["a","b"])`. **Произвольно РАЗНЫЕ имена per-format** (не конвенция) → **НЕ в auto-derive** (ломает narrow-waist) → wrapper-тип на формат ИЛИ ручной `#impl`-opt-out. | serde: `rename(serialize/deserialize)` есть; alias-прецедент — kotlinx `@JsonNames`; per-format-произвол serde тоже толкает в отдельные типы (узкая талия = одно каноническое описание). Backend-конвенция (camelCase JSON vs snake_case TOML) покрывает ~90% без атрибутов на полях. |
| Q22 | Timestamp/Duration/Monotonic (std/time, Plan 175) — Ред.2 | **`Timestamp` ↔ RFC3339-строка** (D342-mapping); **`Duration` ↔ ISO-8601-строка** (✅ решено 2026-07-03 по исследованию peers: emit strict `PT…S`, дробные секунды ≤9 знаков = lossless-nanos, отрицательная — ведущий `-`; parse строгий ISO-8601; числовые формы сек/мс — followup `#serde`-attr); **`Monotonic` НЕсериализуем** → `E_SERDE_FIELD_NOT_SERIALIZABLE` + neg-тест (175 §7 требует verify отсутствия derive-пути) | Timestamp-канон: Go `time.Time`=RFC3339, chrono-serde=RFC3339; антипример — Swift `.deferredToDate`. Duration-peers РАСКОЛОТЫ (kotlinx = ISO-строка default; Jackson = число default / ISO-канон при отключении; Go = int64-nanos, критикуем; serde = `{secs,nanos}`; protobuf-JSON = `"3.5s"`) → выбор ISO-строки: симметрия с Timestamp-строкой + самоописуемость единиц (голое `300` — сек или мс? — классический класс багов, анти-AI-first) + lossless (f64-секунды теряют nanos) |
...
/// Data-model Int=i64-wide (Nova int=intptr_t → widen/range-check, Q2), Uint=u64-wide;
/// float=f64, char→str-1, кортежи→seq, newtype прозрачен.
#unstable(since = "0.1")
...
export type Serializer protocol {
    ...
    // ── enum (tagging Ф.5/180.2; default external) — 4 variant-метода (serde-модель):
    //    layering-инвариант: format-agnostic слой НЕ знает про JsonValue (Ред.2-фикс).
    mut @serialize_unit_variant(enum_name str, idx int, variant str) -> Result[(), SerError]
    mut @serialize_newtype_variant[T Serialize](enum_name str, idx int, variant str, v T) -> Result[(), SerError]
    mut @serialize_seq_variant(enum_name str, idx int, variant str, len int) -> Result[SeqSerializer, SerError]
    mut @serialize_struct_variant(enum_name str, idx int, variant str, len int) -> Result[StructSerializer, SerError]
}
...
// Типа VariantPayload НЕТ (Ред.2-фикс layering-leak): payload идёт через 4 variant-метода выше —
// иначе format-agnostic протокол ссылался бы на JsonValue, противореча «Serialize НЕ знает про JSON».
```
...
**Скаляры/контейнеры — conformant (Q13, compiler-side, НЕ blanket `.nv`).** `bool/int/u64/f64/str/[]u8` получают `Serialize`/`Deserialize`-impl **синтезом в компиляторе** (как built-in routines в `@equal`-synth). `Option[T]`, `[]T` (→seq), `HashMap[str,V]` (→map) — синтезатор эмитит **monomorphic per-element** impl (зеркало `register_container_eq_mono`, Plan 172.1), потому что D355-blanket (закрывший `[M-161]` 2026-06-16) требует **typevar-ресивер** — конкретный generic-ресивер `Option[T]` вне его механики. **Ф.0 re-verify**: если покрыто — часть выражается `.nv`; иначе всё compiler-side.

### 3.4. AUTO-DERIVE — компиляторный synth-контракт (Ф.2 record; Ф.2-sum гейт)

**Главное.** Компилятор synthesize'ит тела `@serialize`/`.deserialize` для типа с `#impl(Serialize + Deserialize)` **структурно**, ровно как `#impl(Clone)` даёт memberwise `@clone` ([auto-derive-guide.md](../guide/auto-derive-guide.md)). **Триггер — `#impl`-аннотация (Q1)**, как у всего семейства auto-derive (НЕ default-on).
...
**🟡 Attribute-инфра НЕ существует** (Ф.3 расщеплён): AST `RecordField`/`SumVariant` **не имеют `attrs`-поля**; парсер знает hardcoded type-attrs (`#stable`/`#deprecated`/…) + **field-level `#visible_to`** (parser/mod.rs:4333 — снимок 2026-07-03; единственный field-attr-прецедент). Поэтому:
...
| `#serde(alias=["x","y"])` | поле | deser ДОПОЛНИТЕЛЬНО принимает эти имена (ser не меняется, Q21; kotlinx `@JsonNames`-прецедент) |
...
`JsonSerializer`/`JsonDeserializer` слоятся над существующим `JsonValue`/`Json.parse` (Q11) — НЕ переписывают парсер. Это **точка, которую потребляет Plan 178** для **record-DTO** ([178 Q20/§3.5/§3.client](178-std-http.md)).
...
**Контракт для Plan 178:** `Body.@json[T](self) -> Result[T, HttpError]` ([178 §3.5 `Body.@json`](178-std-http.md)) = `json.decode_bytes[T](body_bytes)`, маппя `DeError → HttpError{Protocol(de.to_str())}` (source-chaining). `RequestBuilder.@json[T](v T)` ([178 §3.client `RequestBuilder.@json`](178-std-http.md)) = `json.encode[T](v)` + `Content-Type: application/json`. **Dynamic `.json() -> JsonValue`** Plan 178 уже имеет напрямую через `encoding/json`; **typed `.json[T]` для record-DTO гейтится Ф.2-record + Ф.4** — это разблокировка [178 Q20](178-std-http.md). **sum-DTO** в HTTP-payload (редко) → ждёт Ф.2-sum/180.2 (честный sub-gate).

**Numeric-фиделити (Q15):** `JsonValue.Num`=f64 (json.nv:94). `json.decode[T]` в `int`/`uint`-поле проверяет «f64 — точное целое в `[-2^53, 2^53]`»: не-целое ИЛИ `|v|≥2^53` → `DeError{LossyInteger}`; вне i64/u64 → `OutOfRange`; negative→uint → `OutOfRange`. **Никакого silent-lossy.** u64>2^53 — известная JSON-граница (документируется; binary-backend позже без потерь). `bytes`-поле ↔ base64 (Q9, `std/encoding/base64`). **`Json.parse` raw-token — аудит выполнен (Ред.2 2026-07-03): НЕТ** (лексер сразу f64: `NumTok(f64)`, `read_number`) → `LossyInteger`-проверка работает на f64 (отклоняет lossy честно); arbitrary-precision — §11 (Zig-паритет lossless-int достижим только с raw-token, честно помечено).

**Streaming — честный scope-out:** `json.decode` = полная DOM-материализация (`Json.parse`→`JsonValue`→`T`) — ≈2× память на больших payload; incremental/streaming-deser (serde `from_reader`, Go `Decoder`, Jackson streaming) — followup `[M-180-streaming]` (§11).

**Map-детерминизм (D344):** `json.encode` **сортирует ключи** `HashMap[str,_]` (Go json сортирует; serde_json Map=BTreeMap — тоже отсортирован) → детерминированный вывод (golden-тесты, diff-able); «детерминизм по построению» для HashMap без сортировки НЕ выполняется.
...
- **Ф.0 — GATE (без кода). «сейчас».** Закрыть §3.0 (Q1–Q22 — все решены; Q22 финализирован 2026-07-03); написать **D340–D346 spec-first** (§5); **verify D-карты** (committed high-water = D355; 178 → D357–D362; старт **D340**, grep=0). **Verify на main (КРИТИЧНО — закрывает critic-gaps):** (1) **record-auto-derive-машина** `#impl(Equal/Clone/Hash)` — memberwise-synth живёт в **`compiler-codegen/src/protocols/auto_derive.rs`** (synthesize_*/inject_synthesized_methods; НЕ emit_c.rs), это **тот же seam**, в который Ф.2-record встраивает `Serialize`/`Deserialize` (регистрация: `is_builtin_protocol`/`builtin_protocol_method`); честный объём: inject-pass инжектит только методы — synthesis companion-типа `UserVisitor` = новое; (2) **🔴 sum-auto-derive `[M-126-sum-equal-rich]`/`-clone-rich`/`-hash-rich` — OPEN** (verified: identity-fallback/placeholder-заглушки в auto_derive.rs; followup-маркеры только в auto-derive-guide.md — **зарегистрировать в backlog-followups OPEN-view при Ф.0**) → Ф.2-sum = ГЕЙТ, НЕ reuse; Plan 172.1 sum-eq = ДРУГОЙ механизм; (3) **`[M-161-parametric-return]` — ✅ CLOSED 2026-06-16 (D355)** → re-verify: D355-blanket требует typevar-ресивер — покрывает ли конкретный generic-ресивер `fn[T Serialize] Option[T] @serialize`? Ожидаемо НЕТ → container-conformance = compiler-side mono (зеркало `register_container_eq_mono`); если ДА — часть выразима blanket-`.nv`; (4) **🟡 attribute-инфра** — verify AST `RecordField`/`SumVariant` без `attrs` (verified: ast/mod.rs RecordField/SumVariant), field-attr-прецедент `#visible_to` (parser:4333 снимок) → Ф.3a-объём; (5) **`std/encoding/json`** `JsonValue`/`Json.parse`/`@into()`/strict-`DuplicateKey` стабильны; **аудит depth-bound — выполнен (Ред.2): в `Json.parse` НЕТ** (`depth` только в pretty-printer; рекурсивный parse без лимита) → **обязательный prereq** §6 (depth-guard в парсер / bounded-driver), не «возможный»; **аудит raw-numeric-token — выполнен: НЕТ** (лексер сразу f64) → Q15 на f64, arbitrary-precision §11; (6) **verify `format_num` shortest-round-trip** (`0.1`/`1e-300`/subnormal/`-0.0`) — иначе `decode(encode(v))==v` ломается на float-полях. **GATE.** DEP: Plan 177 (✅), `std/encoding/json` (✅), auto-derive-машина record (✅).
...
- **Ф.6 — АТРИБУТЫ AST + режимы тегирования (internal+adjacent). ✅ ПРИЗЕМЛЕНО; untagged 🔴 GATED (2026-07-06, D382; владелец 2026-07-06).** Две части. **Часть 1 — `#serde(...)`-инфра (закрывает разгейтивание Ф.5):** AST-поле `serde_attrs: Vec<SerdeArg>` на `TypeDecl`/`SumVariant`/`RecordField` (`SerdeArg = Tag(str)|Content(str)|Untagged`); парсер `parse_serde_attr` (grammar `#serde(key[="v"], …)`) в трёх позициях (type — через `parse_type_attrs`; field — рядом с `#visible_to`; variant — leading-marker в `parse_one_sum_variant`); политика неизвестного атрибута = **ошибка `E_SERDE_BAD_ATTRIBUTE`** (прецедент `#impl`/`#from_fields`, не silent — бьёт Go/Jackson tag-typo). Field-customization-ключи (rename/skip/flatten/default/alias/deny_unknown_fields) — грамматика общая, потребление = отдельный followup `[M-180-serde-field-attributes]` **(ЗАКРЫТО для record-типов 2026-07-22, Plan 180.1 Ф.1/Ф.10, D435 — см. [180.1](180.1-serde-parity-and-beyond.md); остался узкий `[M-180-serde-flatten]`)**. spec: **D382 (атрибуты декларации)**. **Часть 2 — режимы тегирования (Ф.5):** `serde_tagging_mode(td)` выводит `SerdeTagging {External|Internal{tag}|Adjacent{tag,content}|Untagged}` из type-level `serde_attrs` + статическая валидация (E_SERDE_TAGGING_CONFLICT / _CONTENT_WITHOUT_TAG / _TAGGING_ON_NON_SUM / _INTERNAL_TAG_NON_STRUCT / _UNTAGGED_GATED). **Internal + adjacent — ПРИЗЕМЛЕНЫ** (синтез ser/deser по режиму поверх существующих `Serializer`/`Deserializer`-примитивов, без новых backend-методов). **Untagged — GATED (`[M-180-untagged-codegen-mono]`):** синтез корректен (try-each по value-семантике курсора, C-код валиден), НО компиляция untagged-derive тела пертурбирует codegen `std/encoding/json` в том же CU (mono-collection-ordering → `Json.parse` мис-тегирует число как bool) → `#serde(untagged)` reject at compile-time (`E_SERDE_UNTAGGED_GATED`) до codegen-hardening-префикса. Компилятор-баг, НЕ serde-логика. **Компилятор-фикс (без упрощений, разблокировал internal+adjacent):** match/if result-type reconciliation — `Ok(x)`/`Err(e)`-арм по отдельности даёт stub-сторону (`NovaRes_nova_int_*` OK / `_nova_str` ERR) → курсор-Result мис-лейаут → decode ложный `UnexpectedType`; чинится сборкой конкретного OK (из `Ok`-арма) + ERR (из `Err`-арма) через `novares_ok_err`-split уже-посчитанных типов арм/веток (side-effect-free), в `emit_match`/`emit_if_expr` + их `infer_expr_c_type`-зеркалах; + предпочтение concrete-OK-Result над erased. Order-independent; genuine `Result[int,E]` без изменений; **zero-regression ~50 dirs байт-в-байт vs parent**. pos: `std/encoding/serde/tagging_test.nv` (peer, internal+adjacent round-trip + wire-contract decode). neg: `std/encoding/serde_neg/{untagged,internal_tuple,conflict,on_record,bad_attr}.nv` (прогон `nova test std/encoding/serde_neg --compile-error` = 5/0). Гейт: conformance 54/0; serde autoderive/sum_autoderive без регрессий. **[M-180-serde-attributes] (infra) CLOSED; [M-180-serde-tagging-modes] CLOSED для internal+adjacent; NEW `[M-180-untagged-codegen-mono]` + `[M-180-serde-field-attributes]`.** DEP: Ф.2-sum (✅), Ф.4 (✅).

---

## 5. Spec / D / Q / docs

**D-номера (Ред.2 2026-07-03):** committed high-water 3xx = **D355** (D354/D355 в спеке; 178 renumber'ится в D357–D362, D340–D346 не задевает); D333–D339 = резерв Plan 179 (+буфер). serde старт **D340** (свободны, grep=0 — verify Ф.0).
...
- **NEW D341** — **record-auto-derive-контракт (компилерный синтез).** `#impl(Serialize+Deserialize)` opt-in (Q1, симметрия с `Equal/Clone/Hash`); memberwise-reuse `@clone`-машины; контракт emitted-shape; `Ti: Serialize` иначе **E_SERDE_FIELD_NOT_SERIALIZABLE** (поле названо, НЕ silent-drop); container-conformance compiler-side mono (Q13, `[M-161]`-обход); cycle-detect compile-time. **Headline-differentiator** (единый `#impl` vs Rust отд.`#[derive]`/Go reflection; Zig-квалификация §2). Cross-ref **D186** (`#impl`-грамматика, 02-types.md) + D109/D230. Вирусность честно: все referenced-типы обязаны Serialize (как kotlinx `@Serializable`-вирус) — E_SERDE_FIELD_NOT_SERIALIZABLE делает её явной.
- **NEW D342** — **data-model ↔ синтез mapping.** record→`Struct`, `Vec`→`Seq`, `HashMap[str,_]`→`Map`, `Option`→`Option` (**None → `null` default**; absent — opt-in `skip_serializing_if_none`), скаляры→прим (**`int` = i64-wide: widen на ser / range-check под target-ширину на deser — Q2**), `[]u8`→`Bytes`. Inverse для deser. **numeric-fidelity** (Q15): int/uint точное-целое-check. **`Option[Option[T]]`-ambiguity документируется** (Some(None) и None оба → null; round-trip не идентичен — serde-паритет, тест фиксирует). **Timestamp/Duration/Monotonic (Q22):** `Timestamp` ↔ RFC3339-строка; `Duration` ↔ ISO-8601-строка (`PT…S`, lossless-nanos дробью ≤9 знаков, знак спереди); `Monotonic` НЕсериализуем (+neg; координация Plan 175).
...
- **NEW D344** — **JSON-backend над `std/encoding/json`.** `json.encode[T]`/`decode[T]`/`encode_pretty`/`to_value`/`from_value`/`decode_bytes`/`decode_with`; data-model↔`JsonValue`-mapping; **path-проброс DeError**; **depth-guard** (Q14 default 128 → `DepthLimitExceeded`); **numeric-fidelity** (Q15 `LossyInteger`/`OutOfRange`); **bytes**↔base64 (Q9); `ParseJsonError`→`DeError{Syntax,source}` chain; strict-`DuplicateKey` reuse (Q18); **map-детерминизм: encode сортирует ключи** (Go/serde_json-BTreeMap прецедент — golden-тесты). **Контракт для Plan 178 `.json[T]` (record-DTO).**
...
- **Q-closures:** **Q (effect?)** — RESOLVED: **PURE** (§9). **Q (auto vs opt-in)** — RESOLVED §3.0 Q1: **`#impl`-opt-in** (симметрия семейства, НЕ default-on). **Q (Serializer shape)** — RESOLVED Q3/Q12: generic-bound driven-pull. **Q (None-кодирование)** — RESOLVED (Ред.2-фикс): **None → `null` по умолчанию** (serde-канон — Rust default именно null; консистентно с §3.2 `serialize_none`→null); **absent — через opt-in `#serde(skip_serializing_if_none)`**; deser принимает и null, и absent для `Option`-поля (Q7). Прежняя формулировка «absent default» была фактической ошибкой и конфликтовала с §3.2/§3.5. **Q (unknown-field default)** — RESOLVED Q5: ignore (opt-in `deny`). **Q (sum-synth)** — RESOLVED Ф.2-sum: ГЕЙТ `[M-126-sum-*-rich]`/180.2. **Q (container blanket)** — RESOLVED Q13: compiler-side mono (`[M-161]`-обход). **Q (depth/numeric)** — RESOLVED Q14/Q15/D346.
- **docs/* (новые):** **`docs/serde.md`** (data-model + протоколы + record-auto-derive-контракт + атрибуты-таблица + tagging + **cross-lang §2 — 7-языковая таблица вкл. Zig** + §1a + JSON-backend + soundness-инварианты); `docs/dev/idioms/serde-json.md` (рецепты `json.encode`/`decode`/`rename_all`/`flatten`). Amend auto-derive-главу (рядом с `==`/`clone`/`hash`, отметить `#impl(Serialize)` как члена семейства + честную sum-гейт-ноту).
...
- **ОБЯЗАТЕЛЬНЫЙ prereq в `std/encoding/json`** (аудит Ред.2 выполнен): `Json.parse` **НЕ имеет** depth-bound (Q14 stack-DoS на парс-стороне — `depth` только в pretty-printer) → добавить depth-guard в парсер ИЛИ обернуть `Json.parse` в bounded-driver — работа этого плана (Ф.4 prereq), координируется с std/encoding owner. Raw-numeric-token тоже НЕТ (лексер сразу f64) → arbitrary-precision откладывается в §11 (f64-`LossyInteger` честно отклоняет).
...
`nova_tests/serde/` (pos = folder-module `module nova_tests.serde`, peer-файлы с описательными именами); `nova_tests/serde/neg/` — **ТОЛЬКО** `EXPECT_COMPILE_ERROR` (`module neg.<name>`, маркер `// EXPECT_COMPILE_ERROR <substr>` **без двоеточия**, один маркер/файл); **runtime-Err-проверки (UnknownField/MissingField/LossyInteger/DepthLimitExceeded/…) = ПОЗИТИВНЫЕ test-блоки с assert на Err-kind** (НЕ neg/ — маркера EXPECT_RUNTIME не существует); `rt/` для PURE-serde почти пуст (deep-nested-НЕ-crash — как `_slow`). Inline unit-test-блоки в `std/encoding/serde/*.nv` — канон. `*_slow.nv` — D277/D298 (opt-in `--include-slow`, коммитятся). **Mock-тест:** serde PURE (без effect) → mock-триада **НЕ требуется** (§9). Round-trip property как json.nv:948 (`decode(encode(v)) == v`); **map-golden-тесты валидны** (encode сортирует ключи, D344).

**spec_tests/conformance — ОБЯЗАТЕЛЬНОЕ D-покрытие (Ред.2):** `spec_tests/conformance/d340_serde_datamodel.nv` · `d341_record_autoderive.nv` · `d342_datamodel_mapping.nv` · `d343_serde_attrs.nv` · `d344_json_backend.nv` · `d345_sum_tagging.nv` (gated Ф.2-sum) · `d346_soundness.nv` + compile-error-сторона в `spec_tests/conformance/neg/` (E_SERDE_*-семейство); **amend ⇒ обновлять СУЩЕСТВУЮЩИЙ d-файл в том же изменении** (Ф.3b амендит D341/D343 → правка d341_*/d343_*; Ф.5 амендит D345 → d345_*); serde как новый член `#impl`-семьи = **amend D109/D230 ⇒ обновить d109_*/d230_*-файлы**; прогон `nova test spec_tests` отдельной командой.
...
- **option/seq/map:** `Option[T]` None↔null (default; absent при `skip_serializing_if_none`; deser принимает оба); пустой `[]`/`{}`; `HashMap[str,T]` (encode отсортирован — golden); вложенные `Vec[Vec[int]]`.
- **numeric-fidelity (Q15):** small int round-trip exact; 2^53 граница; negative int round-trip.
- **float round-trip:** f64 shortest-round-trip (`0.1`, `1e-300`, subnormal, `-0.0`) — `decode(encode(v))==v` на float-полях (гейт: Ф.0-verify `format_num`).
- **Timestamp/Duration (Q22):** RFC3339 / ISO-8601-`PT…S` round-trip (когда 175 приземлил типы; отрицательный Duration; наносекунды lossless); `Option[Option[T]]`-ambiguity зафиксирована тестом (Some(None)==None на wire — документированная потеря).
...
**neg (`neg/`, EXPECT_*):**
- **non-serializable → `EXPECT_COMPILE_ERROR`:** record с must-consume/resource-полем (E_SERDE_FIELD_NOT_SERIALIZABLE, **поле названо**); raw-pointer-поле; **`Monotonic`-поле (Q22)**; `HashMap[int,_]` → E_SERDE_NONSTRING_MAP_KEY (Q16); **silent field-drop НЕ происходит** (pos-control: все поля в выводе).
...
5. **Container-conformance — честно compiler-side** (Q13; D355-blanket typevar-receiver-ограничение), НЕ blanket-`.nv`-фикция. Если Ф.0 покажет, что часть выразима как `.nv` — задокументировать; иначе всё в синтезаторе.
...
7. **Конвенции:** D325 (Result-everywhere, R5 `Fail[E]`-запрет neg-тест); PURE (без effect — обосновано); test-conventions Ред.2 (pos folder-module / neg `neg/` compile-only / Err-проверки позитивные); nv-sourcing (serde-логика+JSON-backend в `.nv`, reuse существующего json `.nv`; auto-derive-synth в компиляторе — необходимо).
8. **Гейт корректности (Ред.2):** `spec_tests/conformance` зелёный (d340–d346 + amended d109/d230) + pos/neg-фикстуры фазы + **nova_tests baseline-delta = 0** (baseline = parent-коммит через temp-worktree/commit+reset, ТОТ ЖЕ бинарь; nova_tests сам по себе НЕ гейт; флака ≠ регрессия).
9. **Docs + маркеры:** `docs/serde.md` с 7-языковой таблицей (вкл. Zig); followup-маркеры §11 зарегистрированы в OPEN-view беклога; Q22-решение приземлено (Timestamp-mapping + Monotonic-neg).

**Per-phase:**
- **Ф.0:** §3.0 (Q1–Q22) закрыт; D340–D346 spec-first; record-auto-derive-seam локализован (**auto_derive.rs**); **sum-synth-OPEN verified + M-126-маркеры зарегистрированы в backlog; `[M-161]` CLOSED — blanket-re-verify (D355 typevar-receiver); attribute-no-attrs → Ф.3a-объём**; depth/raw-token-аудит выполнен (оба НЕТ → prereq §6 / §11); `format_num` float-verify; Q22 ✅ финализирован (Timestamp=RFC3339, Duration=ISO-8601, Monotonic=запрет); D-карта verified (старт D340).
...
- **Ф.4:** `json.encode`/`decode`/`encode_pretty`/`to_value`/`from_value`/`decode_bytes`/`decode_with` round-trip; e2e-bridge; deser-error с path; **depth-guard + numeric-fidelity neg зелёные**; **🔓 контракт `json.decode[T]`/`encode[T]` готов для RECORD-DTO → Plan 178 Ф.json РАЗГЕЙЧЕН (record)** (явный sign-off в README/178 §HARD-GATE; sum-DTO остаётся sub-gate на Ф.2-sum).
...
**🔑 COMPILER auto-derive — работа для ИМПЛЕМЕНТОРА (не код в плане).** Ф.2 (record)/Ф.3b/Ф.2-sum/Ф.5 — синтез в **компиляторе** (**`compiler-codegen/src/protocols/auto_derive.rs`** — synthesize_*/inject_synthesized_methods; в emit_c.rs только structural-eq-зеркало Q13). План **специфицирует контракт + emitted-shape** (D341/D342/D343/D345), это **НЕ сам код**. **Честная граница:** record-synth = reuse существующей memberwise-машины (✅ landed); sum-synth = НОВАЯ инфра (`[M-126-sum-*-rich]` OPEN) → гейт/180.2; container-conformance = compiler-side mono (D355 typevar-receiver-ограничение; `[M-161]` CLOSED) НЕ blanket-`.nv`; attribute-storage = НОВЫЙ AST/parser-объём (Ф.3a). Ф.3a-парсер расширяет `#visible_to`-механику (parser/mod.rs:4333 — снимок).

**Координировать:**
- **🔓 Plan 178 (std/http)** — serde **ГЕЙТ для 178 Ф.json (record-DTO)**: typed `.json[T]()`/`.json(v T)` жёстко гейтятся на `json.decode[T]`/`encode[T]` ([178 Q20/§3.5/§3.client](178-std-http.md), Q20). **Ф.4-релиз = sign-off** в README + 178 §HARD-GATE для **record-DTO**. **dynamic `.json()->JsonValue`** в 178 приземляется СЕЙЧАС над существующим json (не ждёт serde). **sum-DTO** в HTTP (редко) → sub-gate на Ф.2-sum/180.2 (явно отметить в 178, что record-DTO достаточно для MVP).
...
- **`[M-161-parametric-return]` — ✅ CLOSED 2026-06-16 (D355)**; НО D355-blanket требует typevar-ресивер → container-conformance пока compiler-mono (Q13); Ф.0 re-verify — если конкретные generic-ресиверы покрыты, часть container-impl переносится в `.nv` (followup).
- **Plan 175 (std/time)** — Q22 ✅ (2026-07-03): `Timestamp`=RFC3339-строка, `Duration`=ISO-8601-строка (`PT…S`); **`Monotonic` НЕсериализуем** (E_SERDE_FIELD_NOT_SERIALIZABLE + neg) — 175 §7 требует verify отсутствия derive-пути.
...
- **НЕ `git stash`** — репо-глобален, worktree делят `.git` → collision/потеря (feedback-worktree-shared-stash). Baseline для diff/regress = **temp-worktree** (`git worktree add ../nova-180-base <parent>`) или **commit+reset**, **ТОТ ЖЕ бинарь**, не stash.
...
- **`nova test` требует ЯВНЫЙ путь**; батч-канон: `nova test nova_tests/<dirs> --results-file rN.json` батчами <10 мин + хвост `--rerun-failed`; **ОТДЕЛЬНО** `nova test spec_tests` и `nova test std`; per-fix verify = targeted fixture, full только в конце фазы. **Гейт корректности (Ред.2):** spec_tests d340–d346 + pos/neg-фикстуры + **baseline-delta=0** (baseline = parent, ТОТ ЖЕ бинарь, temp-worktree/commit+reset; nova_tests сам по себе НЕ гейт; флака ≠ регрессия).
- **rebuild `nova-cli`** после правок `.nv` (include_str!) И после изменений auto-derive-синтеза в `compiler-codegen`; **mtime-touch `.rs` перед `cargo build`** (план правит compiler-codegen — stale-build риск максимальный).
- **Изолированный worktree `nova-p180`** сразу (feedback-isolated-worktree / worktree-naming); cd-префикс в каждой Bash-команде (cwd дрейфует, feedback_worktree_cwd_clarity); env `NOVA_GC_LIB_DIR`/`INCLUDE_DIR` на main repo, libuv-submodule копировать и удалить его `.git` (project-worktree-nova-test-setup).
- **DCO `git commit -s`** (CI-гейт); после фазы — **bidirectional sync с main**; не выдумывать синтаксис — `spec/decisions/` + `examples/`.
...
- **`[M-180-streaming]`** — streaming/incremental deser (serde `from_reader`, Go `Decoder`, Jackson streaming): `json.decode` сейчас = полная DOM-материализация (≈2× память на больших payload — честная нота §3.8). «потом».
- **`[M-180-serde-other-variant]`** — `#serde(other)` catch-all-вариант (serde-прецедент) для forward-compat по неизвестному enum-варианту (сейчас `DeError{UnknownVariant}`); гейт Ф.2-sum. «потом».
