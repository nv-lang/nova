...
- **206.1** — [div/mod/neg trap](206.1-div-neg-trap.md) ✅ РЕАЛИЗОВАН 2026-07-16 (D426, P1): always-on guard для `/`/`%`/унарного `-` (все `Ints`) — division by zero/MIN÷-1/negation overflow трапят вместо SIGFPE/UB; `.nv`-бланкеты `checked_div/_rem`/`wrapping_div/_rem`/`checked_neg`/`wrapping_neg`; см. [206.1-progress.md](206.1-progress.md) — точечно верифицирован, полный conformance-гейт у оркестратора.
