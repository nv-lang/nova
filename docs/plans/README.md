...
| 13 | [13-runtime-stdlib-and-autogen.md](13-runtime-stdlib-and-autogen.md) | Runtime stdlib (str/math) + auto-gen std/runtime/*.nv (read-only projection реестра компилятора) | MVP ✅ + Ф.8 ✅ (декомпозиция builtins.nv); осталось Ф.4 + Ф.9 (API polish: `Self` chaining, `@plus` для str+SB, char_len→len, []char, read_char/read_str, отмена auto-derive try_read_*) |
