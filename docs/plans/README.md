...
| 36 | [36-stdlib-examples-coverage.md](36-stdlib-examples-coverage.md) | `nova check --recursive <dir>` для automated type-check всего std/ и examples/. Закрывает gap в test infrastructure: сейчас stdlib coverage делается ad-hoc PowerShell-loop'ом, examples не проверяются вообще. Прецеденты — `cargo check`, `go vet ./...`. | план, не начат |
