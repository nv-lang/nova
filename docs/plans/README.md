...
| 36 | [36-cli-production-hardening.md](36-cli-production-hardening.md) | Production hardening `nova check` / `nova test` — **R1-R30 + AD1-AD12** после **4-way audit** (cargo + go/CI + nova-specific + implementability/ergonomics/architecture = **160 gaps closed**). **MVP** (Ф.0 + Ф.1) = fix existing `cmd_check` correctness bug (не вызывает infer_effects/lint_module) + **`nova check <path>` + `nova test <path>` polymorphic в одной фазе** (общий фундамент walk/parallel/exit codes/manifest). Exit codes 0/1/2/3/101, parallel = num_cpus с deterministic output (AD4 per-file streaming, resolves R12 vs R19 contradiction), GC backend + ALLOC_REQUIRES awareness, panic hook для guaranteed exit=101. Architecture: new crate `nova-frontend` (sibling к codegen + cli), `Diagnostic` extension (severity/code/spec_link/notes), `DiagRenderer` trait, `trait CheckCache`, unified `ManifestResolver` (закрывает 4 nested nova.toml). **Sub-plans 36.A-E** (post-MVP): outputs (JSON/SARIF/JUnit), caching (blake3), CI (pre-commit/GHA), ergonomics (--explain/completions), workspace. `--tests-dir` clean break (без deprecation). | план, не начат |
