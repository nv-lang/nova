...
| 33.1 | [33.1-contracts-core.md](33.1-contracts-core.md) | **Core:** `requires`/`ensures`/`old`/`result` + `ContractResult[R,E]` + Z3 SMT + debug runtime fallback + zero-cost release CI gate. Только straight-line код. Decision point: после 33.1 решаем стоит ли 33.2. ~2 недели. | 🟡 **в работе** (2026-05-12): runtime-stack закрыт (Ф.1/Ф.2/Ф.4 + block-bodies + early-return + 10 R4-примеров, 194/194 PASS); SMT Z3 backend (Ф.3) отложен — требует libz3 setup |
