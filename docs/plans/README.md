...
- **206** — [арифметическая политика: 5 исходов из 1 overflow-примитива](206-arithmetic-overflow-policy.md) ✅ ЗАКРЫТ 2026-07-15 (D423): trap-дефолт sized-int + `@overflowing_*` интринсик (Ф.1/Ф.1b) + `.nv`-бланкеты checked/saturating/wrapping на `Ints` (Ф.2) + Duration/Timestamp D317-миграция (Ф.3); conformance 470/0; в main. `unchecked_*` — отдельный отложенный item (не план 206.1).
- **206.1** — [div/mod/neg trap](206.1-div-neg-trap.md) ✅ РЕАЛИЗОВАН 2026-07-16 (D427, P1): always-on guard для `/`/`%`/унарного `-` (все `Ints`) — division by zero/MIN÷-1/negation overflow трапят вместо SIGFPE/UB; `.nv`-бланкеты `checked_div/_rem`/`wrapping_div/_rem`/`checked_neg`/`wrapping_neg`; см. [206.1-progress.md](206.1-progress.md) — точечно верифицирован, полный conformance-гейт у оркестратора.
