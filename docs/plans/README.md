...
- **206** — [арифметическая политика: 5 исходов из 1 overflow-примитива](206-arithmetic-overflow-policy.md) 🔨 в работе 2026-07-15: Ф.0 (`type Ints` + D423 спек-амендмент, D310 full-union exemption) + Ф.1 (`@overflowing_add/_sub/_mul` интринсик) + Ф.1b (sized-int trap-дефолт, решение A — `nova_<T>_checked_*` эффектс.h, 8/8 pin-тестов PASS) ЗАВЕРШЕНЫ и точечно верифицированы (ветка `plan206-overflow`, прогресс в [206-progress.md](206-progress.md)); Ф.2 (`.nv`-обёртки checked/saturating/wrapping) + Ф.3 (Duration-миграция) — следующая волна; полный conformance-гейт не гонялся (точечная верификация only).
