...
| 36 | [36-polymorphic-cli-paths.md](36-polymorphic-cli-paths.md) | Polymorphic positional path в `nova check` / `nova test` (file-or-dir, recursive по умолчанию для dir). Closes gap: сейчас stdlib coverage через ad-hoc PowerShell loop, examples не покрыты. `--tests-dir` deprecated. Прецеденты — `cargo check <path>`, `go vet ./...`. | план, не начат |
