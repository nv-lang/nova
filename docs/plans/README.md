...
| 35 | [35-cross-file-resolve.md](35-cross-file-resolve.md) | Cross-file resolve: **Ф.1** — type-check name resolution (wildcard `import X.Y.*` + bare-name visibility, обнаружено при работе над Plan 34); **Ф.2** — codegen module-resolver для compile-mode (`nova build foo.nv` с `import std.X.Y`, вынесено из Plan 14 Ф.5). Низкий ROI / высокая стоимость. | план, не начат, низкий приоритет |
