...
| 34 | [34-stdlib-typecheck-and-compile-fix.md](34-stdlib-typecheck-and-compile-fix.md) | stdlib type-check + compile fix. **Type-check часть** (Ф.1-3): регресс 33/44 → 45/45 + `std.testing.handlers` (`seeded`/`fixed_ms`) + 4 parser-фикса (json/regex/property). **Compile часть** (Ф.4-7, в работе): после nova test sweep'а 4/52 PASS — расширен scope, refactor `int as char` через `char.try_from?`, strict-bool fixes, for-in nova_int, skip runtime/ в test. Cross-file codegen и generic specialization вынесены в Plan 35 и накопленные блокеры. | активный, Ф.1-3 ✅, Ф.4-7 в работе |
