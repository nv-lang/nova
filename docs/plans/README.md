...
| 36 | [36-polymorphic-cli-paths.md](36-polymorphic-cli-paths.md) | Polymorphic positional path в `nova check` / `nova test` (file-or-dir, recursive по умолчанию для dir). 14 production-grade requirements (R1-R14) после cargo/go reality check: exit code triplet 0/1/2, implicit excludes (target/node_modules/_*/.* /std/runtime), symlink canonicalize + dedup, project-root boundary, NDJSON output с byte-ranges, content-hash caching, parallelism = num_cpus, `--keep-going` aggregation, deprecate `--tests-dir`. Closes gap: stdlib coverage через ad-hoc PowerShell loop, examples не покрыты. | план, не начат |
