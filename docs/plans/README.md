...
| 34 | [34-stdlib-typecheck-and-compile-fix.md](34-stdlib-typecheck-and-compile-fix.md) | stdlib type-check + compile fix. **Ф.1-3** type-check: 33/44 → 45/45 + `std.testing.handlers` + 4 parser-фикса. **Ф.5.1** `--skip` флаг (агент). **Ф.5.2** `int as char` refactor через `char.try_from` в 5 файлах (codegen-гейт разблокирован). **Ф.5.3/5.4** оказались D72/generic-specialization блокерами уровня spec/codegen — вынесены в отдельные планы. **Финал:** 4/41 test PASS — закрыт declared scope; оставшиеся FAIL'ы требуют Plan 35, Plan 37 и новых планов. | ✅ **ЗАКРЫТ 2026-05-12** |
| 35 | [35-cross-file-resolve.md](35-cross-file-resolve.md) | **v2 после 3-way audit (65 gaps).** Cross-file resolve для multi-file stdlib через codegen. **MVP** = Ф.0 (FileId в Span — shared с Plan 36.A) + Ф.1 (R1-R21: filesystem loader + in-memory cache + topological walk с cycle detection + signature/body 2-pass + schema merge + effect/capability propagation + ALLOC_REQUIRES transitive + mangling + deterministic output + DCE + visibility enforcement). **Sub-plans 35.A-E**: wildcard imports + `export use` + prelude (35.A), on-disk cache + incremental (35.B), cross-file generic bounds (35.C), stable mangling v0-style (35.D), cfg + internal/ + editions (35.E). **Real blockers**: `std/encoding/hex.nv`, `std/crypto/*`, `std/collections/{lru,deque,...}` — все валятся на cross-file. | план, не начат, **высокий приоритет** (blocker для multi-file stdlib через codegen) |
