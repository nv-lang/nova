...
# Plan 116 — std/tls: TLS-слой поверх `TcpStream` (mbedTLS C-шим; HTTPS-энейблер)

> **Создан 2026-05-31** (историческое имя файла — «std-tls-effect»; сам эффект
>   ретрактирован актуализацией, см. ниже).
> **Статус:** ✅ ЗАКРЫТ 2026-07-11 — **https-prod-ready ЯДРО ГОТОВО:** mbedTLS-бэкенд
>   (T40, Rust удалён) + полная TLS-поверхность (std/tls 29/29) + teardown (T39) +
>   M-178 codegen-разгейт → **`std/http/transport/real` e2e PASS на реальном mbedTLS.**
>   Критерии #1-4 ✅. Хвост #5 (вынос nova-tls) + #6 (examples/доки) **вынесены в
>   [Plan 193](193-nova-tls-repo.md)** (packaging, не функционал). (Ф.0 закрыта 2026-07-10.)
