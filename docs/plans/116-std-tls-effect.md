...
> **Статус:** 🟢 ЯДРО ГОТОВО 2026-07-11 — **https-prod-ready подтверждён:**
>   mbedTLS-бэкенд (T40, Rust удалён) + полная TLS-поверхность (std/tls 29/29) +
>   teardown (T39) + M-178 codegen-разгейт → **`std/http/transport/real` e2e
>   PASS на реальном mbedTLS.** Осталось для полного закрытия 116: #5 вынос в
>   репу `nova-tls` (Ф.3, разблокирован 03.1) + #6 examples/доки. (Ф.0 закрыта
>   2026-07-10.)
