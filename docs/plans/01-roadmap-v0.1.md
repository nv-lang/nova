...
```
nova-lang/compiler-codegen/             (Rust crate, name = "nova-codegen")
├── Cargo.toml
├── src/
│   ├── lexer/                          токенизация
...
│   ├── interp/                         treewalk interpreter (для `run`/`test`)
│   ├── codegen/
│   │   └── emit_c.rs                   C-codegen (для `compile`)
│   ├── diag.rs                         структурированные ошибки
│   ├── lib.rs
│   └── main.rs                         CLI: nova run / check / test / compile
├── nova_rt/                            C-runtime для скомпилированных бинарей
│   ├── alloc.{h,c}                     ref-counting allocator + Boehm GC option
│   ├── effects.{h,c}                   handler-стек, D61
│   ├── fibers.{h,c}                    minicoro stackful coroutines
...
запускаются через CLI компилятора.

**Эволюция:** изначально было два crate'а — `compiler-bootstrap`
(treewalk-only) и `compiler-codegen` (форк bootstrap'а с добавлением
C-codegen'а). К 2026-05-08 bootstrap был **удалён** — codegen
поглотил его функциональность, поддерживать дубликат стало
overhead'ом без выгоды (см. эволюцию в [history/evolution.md]).
...
3. **Структура репо.** Monorepo: всё живёт в `nova-lang/`. Текущая
   ...
   - `compiler-codegen/` — Rust-компилятор: парсер, type-checker,
     treewalk-interp, C-backend codegen
   - `compiler/` — будущий self-hosted компилятор на Nova
     (появится после того как codegen сможет запустить парсер
     написанный на Nova)
   - `nova_tests/` (top-level) — conformance-тесты языка
   - `std/` — стандартная библиотека Nova
