...
[decisions.md](../../decisions.md), синтаксис — в
[syntax.md](../../syntax.md). Здесь — план **реализации**.

## Bootstrap-стратегия

| Этап | Язык | Что делает |
|---|---|---|
| **v0.1–v1.0** | Rust | Bootstrap компилятор (см. [Q17](../../open-questions.md)) |
| **v2.0+** | Nova | Self-hosting — переписать компилятор на Nova |

Выбор Rust обоснован в [Q17](../../open-questions.md#q17-bootstrap-язык-компилятора-—-rust):
...
- Структурная типизация ([D15](../../decisions.md))
- Inference типов локальных переменных
- Generic-параметры через мономорфизацию
- **Effect inference** для private ([D28](../../decisions.md))
- **Effect checking** для public — обязательны в сигнатуре
- Exhaustiveness check для match ([D17](../../decisions.md))
- Проверка `?` требует `Throws[E]` в сигнатуре ([D25](../../decisions.md))
- Проверка `with X = ... { ... }` совместим с эффектом

**Не делается в v0.1:**
- SMT-проверка контрактов ([D24](../../decisions.md)) — только
  runtime-check в debug
- Effect polymorphism ([Q6](../../open-questions.md))
- Concurrent GC ([D6](../../decisions.md)) — для интерпретатора простой
  mark-and-sweep, production-уровневый concurrent collector с паузами
  <1ms — в v0.4+ вместе с LLVM
- `region { ... }` блок и эффект `Realtime` ([D6](../../decisions.md)) —
  v0.5
...
- Fiber'ы через **stackful coroutines** для `Async`/`Par`. Конкретная
  реализация — открытый вопрос ([Q3](../../open-questions.md#q3-реализация-fiber-stack-для-async)),
  склонность — `corosensei` для быстрого старта
- `panic` = смерть текущего fiber'а ([D13](../../decisions.md)),
...
- Native codegen (LLVM)
- JIT
- Time-travel debugging ([R8](../../revolutionary.md))
- Hot reload
- Production concurrent GC — для интерпретатора достаточно простого
  mark-and-sweep ([D6](../../decisions.md))
...
Из списка [D26](../../decisions.md) в v0.1 реализуется:

- **`Io`** — `stdout.write`, `stderr.write`, `print`, `println`
- **`Throws[E]`** — `throw`/`?` через handler ([D25](../../decisions.md))
...
- Формат [R5.3](../../revolutionary.md): место → причина →
...
| Production concurrent GC | v0.4 | После LLVM, целевые паузы <1ms ([D6](../../decisions.md)) |
| Region/Realtime эффект ([D6](../../decisions.md)) | v0.5 | После основного GC |
| SMT для контрактов ([D24](../../decisions.md)) | v0.6 | Z3-интеграция большая работа |
| Effect polymorphism ([Q6](../../open-questions.md)) | v0.5 | Сложная теория |
| `comptime`/macros ([Q7](../../open-questions.md)) | v1.0+ | Не критично для итераций |
| JIT | v1.5+ | Не приоритет |
| Time-travel debugging ([R8](../../revolutionary.md)) | v0.8 | Нужна запись handler-вызовов |
| Distributed handlers ([R12](../../revolutionary.md)) | v1.0+ | Поверх готовой системы эффектов |
| LSP-сервер | v0.5 | После стабилизации API |
| Package manager | v0.6 | Сначала monorepo-style разработка |
| Hot reload | v1.0+ | После JIT |
| Schema evolution ([Q13](../../open-questions.md)) | v0.9 | После того как stdlib устаканится |
| Bitflags / enum с числами ([Q15](../../open-questions.md)/[Q16](../../open-questions.md)) | v0.5 | Сначала базовый язык |
| Erlang-style per-fiber heap | v2.0+ | v1.0 — Go-style shared heap ([Q12](../../open-questions.md)) |

### Архитектура bootstrap-компилятора

```
nova-compiler/                          (Rust workspace)
├── Cargo.toml
├── crates/
│   ├── nova-lexer/                     токенизация
│   ├── nova-parser/                    AST builder (chumsky или ручной)
│   ├── nova-ast/                       типы AST
│   ├── nova-types/                     type checker + effect inference
│   ├── nova-resolve/                   импорты, разрешение имён
│   ├── nova-interp/                    treewalk interpreter
│   ├── nova-stdlib/                    встроенные функции и handler'ы
│   ├── nova-diag/                      структурированные ошибки
│   └── nova-cli/                       command-line entry point
├── tests/
│   ├── parser/                         golden-тесты парсера
│   ├── types/                          tests на type-check
│   ├── interp/                         тестовые программы
│   └── audit_nv/                       упрощённый audit.nv
└── examples/
    ├── hello.nv
    ├── effects.nv
    └── audit_simplified.nv
...
- Lexer всех токенов (актуальные ключевые слова из
  [D22](../../decisions.md), [D27](../../decisions.md),
  [D29](../../decisions.md))
- Parser деклараций
...
- Resolve имён: модули, импорты ([D29](../../decisions.md)), шейдинг,
...
- `spawn`, `parallel for`, `supervised` ([Q12](../../open-questions.md)
  v1.0-план)
- `panic` = exit fiber'а ([D13](../../decisions.md))
...
- Reference: ссылка на [decisions.md](../../decisions.md)/[syntax.md](../../syntax.md)
...
- Все примеры из [syntax.md](../../syntax.md),
  [effects.md](../../effects.md) парсятся и type-check'аются
- Ошибки структурированные, формат [R5.3](../../revolutionary.md)
...
concurrent GC ([D6](../../decisions.md)) с паузами <1ms (ZGC/Shenandoah/
...
([D6](../../decisions.md)). Effect polymorphism
([Q6](../../open-questions.md)), LSP-сервер. Закрывается
[Q15](../../open-questions.md)/[Q16](../../open-questions.md) (enum с
...
([D24](../../decisions.md)). Package manager.

**v0.8** — Time-travel debugging ([R8](../../revolutionary.md)). Запись
handler-вызовов и replay.

**v0.9** — Schema evolution stdlib-паттерн
([Q13](../../open-questions.md)). Финализация концепции.

**v1.0** — стабилизация, документация, готовность к публичному релизу.
После v1.0 — стабильность синтаксиса ([R5.4](../../revolutionary.md),
[D10](../../decisions.md)).
...
2. **Fiber-runtime в bootstrap.** `corosensei` (cross-platform
   ...
   [Q3](../../open-questions.md#q3-реализация-fiber-stack-для-async).
3. **Структура репо.** Monorepo (компилятор внутри `nova-lang/`,
   например `nova-lang/compiler/`) vs отдельный репозиторий
   `nova-compiler/`. Сейчас `nova-lang/` — дом дизайн-документов;
   решение принимается при создании первого crate'а.
