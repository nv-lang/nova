...
### Recovery plan для Plan 114.4.1 (assoc const + const fn)

См. оригинальный план body (Ф.2 sub-phases Ф.2.1-Ф.2.6 для assoc const;
Ф.3 sub-phases Ф.3.1-Ф.3.9 для const fn).

Recommended order:
1. **Plan 114.4.1 Ф.A** — assoc const D200 (record + sum + generic
   T-independent + T-dependent per-mono). ~½ dev-day.
2. **Plan 114.4.2 Ф.B** — const fn D199 (comptime evaluator + body checker
   + memoization). ~1 dev-day.

Each ships independently per safety hatch.
