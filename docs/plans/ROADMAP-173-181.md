...
| 173    | B | 🔨 | **Ф.1 ЗАКРЫТА** (#1/#2/#4/#3/#7). **Ф.2 defer-kernel В РАБОТЕ:** Ф.2.0 D314-spec + де-риск-карта закрыты (c625808e; 🔴 D194-премиса ложна→parity+followup, nova_scope_exit policy, rename-collision, interrupt→Failure). Реализация: ✅A0→✅R1(43f9ee5b)→✅R2(ffb76506)→✅B1 parser/AST defer(o)(e0d95a31; +доукомплектовал #3 e3fce6f3)→✅B2 codegen+checker outcome-материализация(23f512d8; 4 exit-path Success/Failure/Panic/Cancel/interrupt, резолвер биндит `o`, плейн byte-identical)→🟡B3 consume-desugar ЧАСТИЧНО(outcome-примитив унифицирован; full run-site merge ЗАБЛОКИРОВАН — compose-семантика consume(panic-dominance/pairwise) vs user-defer(chain) genuinely РАЗНЫЕ, +checker consume-body≠defer-body; монолит=корректный consume-flavored-defer-entry; followups [M-173-b3-runsite-unify]/[M-173-consume-interrupt-cleanup])→✅D194 disasm-parity+spec-sync(спека к факту: элизия НЕ реализована→followup [M-173-d194-perf-elision]; parity подтверждён mutexguard≡baseline)→C nova_scope_exit→E-hub. **Ф.2-scope-решение владельцу: принять sound-subset(монолит) или инвестировать в risky full-merge** |
| 174    | B | ⏳ | после 173 |
| 176    | B | ⏳ | после 173 |
| 175    | B | ⏳ | параллельно |
