...
| 173    | B | 🔨 | **Ф.1 ЗАКРЫТА** (#1/#2/#4/#3/#7). **Ф.2 defer-kernel В РАБОТЕ:** Ф.2.0 D314-spec + де-риск-карта закрыты (c625808e; 🔴 D194-премиса ложна→parity+followup, nova_scope_exit policy, rename-collision, interrupt→Failure). Реализация: ✅A0→✅R1(43f9ee5b)→✅R2(ffb76506)→✅B1 parser/AST defer(o)(e0d95a31; +доукомплектовал #3 e3fce6f3)→✅B2 codegen+checker outcome-материализация(23f512d8; 4 exit-path Success/Failure/Panic/Cancel/interrupt, резолвер биндит `o`, плейн byte-identical)→🟡B3 consume-desugar ЧАСТИЧНО(outcome-примитив унифицирован; full run-site merge ЗАБЛОКИРОВАН — compose-семантика consume(panic-dominance/pairwise) vs user-defer(chain) genuinely РАЗНЫЕ, +checker consume-body≠defer-body; монолит=корректный consume-flavored-defer-entry)→✅D194 disasm-parity+spec-sync(элизия НЕ реализована→followup)→✅E doc-sweep+hub(0636a9ed; R1/R2 renames в 16 live-доках, hub переписан под факт)→🔴C nova_scope_exit ЗАБЛОКИРОВАН(terminal-сайты несогласованы per-kind→followup [M-173-c-transport-normalize]). **Ф.2 SAFE-SCOPE ЗАКРЫТ; structural-finale(merge/transport/interrupt)→documented followups; D314 language-goal ДОСТИГНУТ, 174/176 разблокированы (им нужно ПОВЕДЕНИЕ, оно работает)** |
| 174    | B | ⏳ | после 173 |
| 176    | B | ⏳ | после 173 |
| 175    | B | ⏳ | параллельно |
