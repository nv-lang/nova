...
| 173    | B | 🔨 | **Ф.1 ЗАКРЫТА** (#1/#2/#4/#3/#7). **Ф.2 defer-kernel В РАБОТЕ:** Ф.2.0 D314-spec + де-риск-карта закрыты (c625808e; 🔴 D194-премиса ложна→parity+followup, nova_scope_exit policy, rename-collision, interrupt→Failure). Реализация: ✅A0→✅R1(43f9ee5b)→✅R2(ffb76506)→✅B1 parser/AST defer(o)(e0d95a31; +доукомплектовал #3 e3fce6f3)→✅B2 codegen+checker outcome-материализация(23f512d8; 4 exit-path Success/Failure/Panic/Cancel/interrupt, резолвер биндит `o`, плейн byte-identical)→✅B3-merge consume ФИЗИЧЕСКИ СЛИТ в defer-kernel(монолит удалён 285→~101стр; **§4a panic-dominance разблокировал** — cleanup-PANIC доминирует в defer-kernel ТОЖЕ→десугар behavior-preserving; consume_policy на DeferEntry + enter_consume_defer_scope + 4 run-site branch; cancel-shield/timeout/RT/exactly-once re-home'нуты; попутно fix interrupt→cleanup(Failure) [M-173-consume-interrupt-cleanup ЗАКРЫТ] + early-return/break-skip монолита)→✅D194 disasm-parity+spec-sync(элизия НЕ реализована→followup)→✅E doc-sweep+hub(0636a9ed; R1/R2 renames в 16 live-доках, hub переписан под факт)→✅C nova_scope_exit ЗАКРЫТ(единый terminal-transport helper в effects.h; §5-согласование свело per-kind к канону PANIC/CANCEL→rethrow_with_suppressed СОХРАНЯЯ chain+reason; SITE A with-Fail CATCH через `caught`-флаг устранил per-kind dispatch, defer+consume FAIL→TRANSPARENT; compose/outcome остаются в codegen; grep-guard пройден; conformance 38/38 + весь defer/consume/with-Fail корпус zero-regression vs baseline-бинарь). **Ф.2 ПОЛНОСТЬЮ ЗАКРЫТА (structural-finale достигнут); D314 language-goal + physical-unification + единая точка политики ДОСТИГНУТЫ, 174/176 разблокированы** |
| 174    | B | ⏳ | после 173 |
| 176    | B | ⏳ | после 173 |
| 175    | B | ⏳ | параллельно |
