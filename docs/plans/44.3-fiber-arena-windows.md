...
> **Статус:** 🟡 **НЕ РЕАЛИЗОВАН** (Windows на calloc-fallback, fixed
> 56 KB; arena — Linux/macOS-only). 4 неудачные попытки 2026-05-13/14.
>
> ⚠️ **Документированный root cause ОПРОВЕРГНУТ перепроверкой
> 2026-05-21.** Шапка ранее утверждала «minicoro `MCO_USE_ASM` не
...
> Context). Либо 44.3 использовал старый minicoro без Windows-asm-
> backend'а, либо диагноз был ошибочным. **Реальная причина 4 провалов
> неизвестна** — нужна re-diagnosis.
>
> **➡ Re-diagnosis + production-реализация — [Plan 82](82-windows-fiber-tib-unblock.md)**
> (Ф.0 = заново установить настоящий блокер против текущего кода).
> Plan 44.3 остаётся историческим документом «4 попытки 2026-05-13/14
> и что они показали»; Plan 82 — re-diagnosis + production-реализация.
>
