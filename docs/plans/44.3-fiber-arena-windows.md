...
> **Статус:** 🔒 **FUNDAMENTALLY BLOCKED.** 4 неудачные попытки
> (2026-05-13/14). Root cause: Windows kernel NT_TIB tracking — minicoro
> `MCO_USE_ASM` бэкенд переключает только RSP, не обновляет
> `NT_TIB.StackBase/StackLimit`; fiber на `VirtualAlloc`-стеке → SEH
> unwind / stack-overflow detection читают диапазон OS-стека → hang.
> Windows остаётся на calloc-fallback (fixed 56 KB, single-threaded
> cooperative; arena Linux/macOS-only). Подробности — секция
> «Неудачные попытки» ниже. Шапка-метка обновлена 2026-05-21 (аудит
> план-статусов: была устаревшая «2 попытки / открытый»).
> **Цель (нереализуема в ТЕКУЩЕМ подходе):** догнать Linux/macOS arena
> на Windows — lazy commit, growable stacks, single GC root.
>
> **➡ Разблокировка исследуется в [Plan 82](82-windows-fiber-tib-unblock.md)**
> — обход TIB-блокера через альтернативные avenue (Windows native
> fibers `MCO_USE_FIBERS` / ручной TIB-fixup / scoped-down fallback).
> Plan 44.3 остаётся историческим документом «почему лобовой подход
> (`MCO_USE_ASM` + custom arena) мёртв»; Plan 82 — активная попытка
> решить задачу другим путём.
>
> **Prerequisite:** Plan 44.2 закрыт (Linux/macOS arena landed).
> Plan 44.3 — symmetric Windows реализация, унифицирует platform pathways.
>
