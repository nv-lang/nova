...
- **Ф.2 (мост) — мягкая зависимость от окна p364** (`[M-linear-capture-gate-blind-to-consume-let]`,
  221.1 №364): её форма (`match`-арм + `spawn consume conn`) канонична и ПРОВЕРЕНА СБОРКОЙ
  2026-08-06, поэтому p364 требует ПЕРЕПРОВЕРКИ, а не переписывания. Делать Ф.2 до p364
  допустимо — тогда перепроверить сразу после его вливания (Ф.0-г).
