...
**Поэтому V1 делается на СУММЕ** (`type Upstream enum Socks5 { … }` + `@dial`,
матчащийся по варианту и делегирующий в файл протокола). Это не выбор дизайна, а
обход живого дефекта, и так это и записано в коде примера — чтобы читатель
флагмана не принял обход за рекомендацию.

Цена суммы честная: добавление протокола трогает ДВА общих места (объявление
суммы и `match` в `@dial`), тогда как у протокола-интерфейса общее место одно —
фабрика. Выигрыш суммы — исчерпывающая проверка вариантов компилятором, но здесь
он мал: точка диспетчеризации ровно одна.

**Обязательный follow-up:** после закрытия дефекта протокол-значения пример
переводится на протокол-интерфейс. Это не «можно будет» — это то, ради чего
владелец задавал структуру, и сумма стоит здесь только пока форма недоступна.
