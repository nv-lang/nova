...
- **Ф.2** — мост (`examples/net/socks5_http_bridge/main.nv`): подключить `socks`
  ...
  **НЕ начата, и ДО ТЕГА не начинается.** Прежняя формулировка «ждёт
  `[M-73.1-destructure]`/`[M-consume-param-spawn-defer-active]`» **УСТАРЕЛА**:
  оба (№378/№379) закрыты и влиты, форма relay из §3.2 собирается до бинаря
  (проверено интегратором на пакете `7ed38407d`). Живой блокер РОВНО ОДИН —
  **№390**, и он не снимается организационно:
  - решение владельца/интегратора — №390 до тега НЕ чиним (корень в libuv на
    Windows, патч задел бы все TCP-чтения);
  - записанная митигация «обязательный `supervised(timeout:)` на каждом чтении»
    **предмет НЕ закрывает**: при №390 таймаут не будит, потому что ОС не постит
    завершение (см. разбор в шапке). Она полезна против ОБЫЧНЫХ зависаний и
    остаётся требованием README, но от №390 не спасает;
  - для моста это не краевой случай, а основная работа: `pipe_bidirectional`
    только и делает, что продолжает чтение после частичных данных.

  Итог: мост, собранный сегодня, вешал бы файбер на каждом закрытом пире. Ф.2
  открывается после закрытия №390, то есть **после тега**. Сдавать «мост с
  известным зависанием» нельзя — это пример, его читают как образец.
