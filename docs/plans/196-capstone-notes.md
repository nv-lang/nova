...
**ПОСТСКРИПТУМ ИНТЕГРАТОРА (2026-07-17, тот же день): снос B10h/B10l ОТМЕНЁН, реестр обратно 46 → 48.**
Перепись-доказательство батча-2 было невалидным для `d45_inferred_return_type.nv`: параллельная
d45-регрессия (ba9a8a2f3, ложный Unit-канал) резала фикстуру на ЧЕКЕРЕ до codegen — её трафик по
B10h/B10l был замаскирован. После d45-фикса (fc06bdee2) снос дал CC-FAIL (undeclared identifier:
let-биндинг с пустым C-типом): generic newtype/named-tuple конструктор (`Counter16[T](v)`) продюсер Q1
честно НЕ каналит (generics-гейт) — легаси-ветки остаются единственным источником. Обе ветки
восстановлены интегратором в merge-волне d45-фикса. **Урок протокола (нормативно для следующих батчей):
детач+panic-перепись валидна только если КАЖДАЯ фикстура корпуса доходит до codegen — перед переписью
проверять, что корпус компилируется целиком (0 CODEGEN/CC-FAIL), иначе 0-хит недоказателен.**
Путь закрытия B10h/B10l — расширение ctor-продюсера на generic-формы (снятие gs-гейта с выводом
тип-аргументов), не снос по маскированной переписи.
