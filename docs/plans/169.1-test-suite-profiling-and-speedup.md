...
| 169.1.2 | [consolidate-tests](169.1.2-consolidate-tests.md) | **Консолидация по темам** (объединяет бывшие 169.1.2/.3/.4): склейка по теме (atomics/sync/…) с префиксом файла `plan_*`, neg→`neg/`, `panics`-клаузула фазой. priv(file)/rename коллизий. | ~−400 CU |
...
`[M-169.1.2-consolidate]`, `[M-169.1.2-panics-clause]`.
