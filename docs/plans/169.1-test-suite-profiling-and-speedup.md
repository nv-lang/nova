...
- Профиль: `docs/dev/research/12-test-suite-profile-2026.md`.
...
- `docs/dev/test-conventions.md` обновлён: секция `_slow`-тестов.
...
- ✅ A2: Таблица top-30 по реальным данным (260 тестов); `docs/dev/research/12-test-suite-profile-2026.md`.
...
- ✅ A7: D298 в `spec/decisions/09-tooling.md`; `docs/dev/test-conventions.md` обновлён.
...
✅ Ф.2: 260 тестов измерено (basics/generics/concurrency/plan57/plan103_*/plan110/plan55/plan56/plan140/plan152_5). Медиана 36.7s, p90 47.5s; compile dominates 96%. Профиль: `docs/dev/research/12-test-suite-profile-2026.md`.
...
✅ Ф.5: D278 добавлен в `spec/decisions/09-tooling.md` — бюджет ≤120s CI, пороги (total≥3s/run≥2s/compile≥500ms). `docs/dev/test-conventions.md` обновлён.
...
- ✅ A2: Таблица top-30 построена по реальным данным (260 тестов); `docs/dev/research/12-test-suite-profile-2026.md`.
...
- ✅ A6: `nova test` (default) PASS без _slow; `nova test --include-slow` PASS со всеми.
- ✅ A7: D278 в `spec/decisions/09-tooling.md`; `docs/dev/test-conventions.md` обновлён.
...
- A7.7: Таблица кандидатов и решений добавлена в
  `docs/dev/research/12-test-suite-profile-2026.md` (секция Ф.7).
...
- ✅ A8.5: CU reduction: baseline ~2812 → Cat-A+basics 2416 (-14%) → Cat-B 1314 (-53%). Cat-B: 75 dirs, ordinal-suffix rename, `scripts/tools/catb_convert.py`.
...
Стратегия: ordinal-suffix rename — `Counter` в N файлах → `Counter1`/`Counter2`/.../`CounterN` (по алфавиту имени файла). Скрипт: `scripts/tools/catb_convert.py`.
...
> [docs/dev/research/14-nova-tests-module-census-2026-06-19.txt](../dev/research/14-nova-tests-module-census-2026-06-19.txt).
...
[docs/dev/research/14c-nova-tests-negative-census-2026-06-19.txt](../dev/research/14c-nova-tests-negative-census-2026-06-19.txt).
...
[docs/dev/research/14b-nova-tests-multimodule-candidates-2026-06-19.txt](../dev/research/14b-nova-tests-multimodule-candidates-2026-06-19.txt).
...
> **Канонический план — [173](173-error-system-unify-harden.md)** (извлечён 2026-06-20 в top-level). Раздел
> ниже — исходный дизайн; актуальная версия (с исправленной формулировкой риска) в Plan 173.
...
**Риск:** ре-вход рантайма после N паник (НЕ «пропуск cleanup» — `nv_panic` идёт через
`_nova_fail_top` первым, `defer`/`on_exit` отрабатывают) — `_nova_fail_top`/GC-arena между
тестами после panic-longjmp. Подробно (исправленная формулировка) → [Plan 173](173-error-system-unify-harden.md).
...
| 169.1.2 | [consolidate-tests](169.1.2-consolidate-tests.md) | **Консолидация по темам** (объединяет бывшие 169.1.2/.3): склейка по теме (atomics/sync/…) с префиксом файла `plan_*`, neg→`neg/`. priv(file)/rename коллизий. | ~−102 CU достигнуто |
| → [173](173-error-system-unify-harden.md) | **`panics`-клаузула** (была фазой 169.1.2, поднята в top-level [Plan 173](173-error-system-unify-harden.md)): `test "…" panics "X"` инверсия PASS/FAIL → runtime-panic в folder-module. | −78 CU (📋 отложен, гейт 172) |

Рекомендованный порядок: **169.1.1** (инфра, разблокирует CI) → **169.1.2**
(склейка по темам; `panics`-фаза рискованная — последней).
Маркеры: `[M-169.1.1-lane-flags]`, `[M-169.1.1-ci-workflow]`,
`[M-169.1.2-consolidate]`, `[M-173-panics-clause]`.
