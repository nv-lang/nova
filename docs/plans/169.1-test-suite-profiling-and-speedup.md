...
- `[M-169.1-consolidate-remaining]` — категория (A), 39 папок, **−239 CU**
  (1314 → ~1075). Метод: расширить `scripts/catb_convert.py` либо ручная
  конвертация крупнейших (`plan91_12`/`plan91_15`/`plan103_2`/`plan67`).
- `[M-169.1-panics-clause]` — реализовать `panics`-клаузулу (parser+AST+codegen+
  миграционный скрипт), консолидировать 114 runtime-panic тестов (**−78 CU**).
  Самостоятельная задача (тянет на под-план). Дизайн — выше.

  **Оценка агентского времени** (wall-clock, вкл. пересборки nova-cli ~1.5–2 мин):
  - Ф.1 parser+AST (`panics "str"`, `TestDecl.panics`): ~30–45 мин.
  - Ф.2 codegen (инверсия setjmp-веток emit_c.rs:17218-17253 + `strstr`): ~30–45 мин.
  - Ф.3 миграция 114 файлов (скрипт `fn main`→`test panics`, смена module): ~45–90 мин.
  - Ф.4 верификация (прогон затронутых папок + регресс): ~20–30 мин.
  - **База: ~2–3 ч**, если не выстрелит риск ниже.

  **Главный риск — GC/fail-frame consistency между тестами после panic-longjmp.**
  ...
  `_nova_fail_top`, `_nova_current_handler_iframe` (уже чинил его leak →
  STATUS_BAD_STACK, см. §16/runtime), GC-arena, scheduler-state. Тот же класс,
  что race-investigations — может раздуть до полудня. Особо подозрительны
  concurrency-паники: `supervised_cancel_double_bind`, `fiber_stack_overflow`,
  `select_all_closed`. Если рантайм рассыпается — нужен `nova_runtime_reset()`
  между тестами (доп. инфра-задача → затея дороже −78 CU, отложить).

  **Диапазон:** оптимистично ~2–3 ч / реалистично ~4–5 ч / пессимистично — день.
  ...
  **ROI-замечание:** −78 CU за ~2–3 ч (в лучшем случае) — среднее соотношение.
  Категория (A) `[M-169.1-consolidate-remaining]` даёт −239 CU дешевле по
  времени за CU (механический rename, без рантайм-рисков) → делать ПЕРВОЙ.
- `[M-169.1-consolidate-partial-D]` — категория (D): вынести neg в `neg/`,
  объединить pos-остаток (~−80 CU).
- `[M-169.1-test-lane-flags]` — CLI category-селекторы: `--positive`/`--slow`/
  `--compile-error`/`--panic`/`--timeout`/`--exit`/`--full`. Аддитивная модель
  (OR), дефолт = positive∩fast, фильтр по `EXPECT_*` маркеру + `_slow` суффиксу.
  Обобщить `SlowLane` (Plan 156). Дизайн — выше. Отдельная задача.
- `[M-169.1-ci-regression-workflow]` — `.github/workflows/nova-test-regression.yml`:
  PR/push = `nova test` + `nova test std` (быстро); nightly cron = `nova test
  --full` + `nova test std`. Закрывает дыру молчаливого накопления поломок.
  Зависит от `[M-169.1-test-lane-flags]` (нужен `--full`). Отдельная задача.
