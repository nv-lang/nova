...
|---|----------|-----|---------|
| 169.1.1 | [test-lane-flags + CI](169.1.1-test-lane-flags-and-ci.md) | CLI category-селекторы (`--positive`/`--slow`/`--compile-error`/`--panic`/`--timeout`/`--exit`/`--full`) + CI авто-регресс (PR быстрый + nightly полный) | закрывает процессную дыру |
