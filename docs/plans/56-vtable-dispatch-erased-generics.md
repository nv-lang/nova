...
1. **Multiple bounds (`T: Hash + Display`)** — не deferred, **must
...
2. **Self type в bound method signature** — `Hash.eq(other Self) -> bool`.
...
3. **Default implementations** — bound может предоставить default
   method (`fn Compare @ne(other Self) -> bool => !@eq(other)`).
   Vtable должна включить default thunk если concrete K не override'ит.
4. **Recursive bound dependencies** — `Hash: PartialEq` (hash
   requires eq). Vtable Hash содержит pointer на PartialEq vtable
   ИЛИ inline eq method. Решение: **inline** (flatten dependencies).
5. **Self-type в return position** — `Compare @max(other Self) -> Self`.
   Возвращает тот же runtime type — vtable thunk возвращает `void*`,
   caller cast'ит обратно.
6. **Effect rows в bound methods** — `Hash @hash() Io -> u64` (если
...
|---|---|---|
| Ф.1 | 6 | runtime vtable struct (Hash, Compare, Display, multi-bound, custom user bound, Self type) |
...
вызывает **bound method** (e.g. `key.hash()` где K имеет Hash bound),
...
```c
/* Auto-generated vtable для Hash protocol.
...
```c
/* HashMap[nova_str, nova_int] — K=nova_str → Hash vtable. */
...
Compiler detects MyKey satisfies Hash, generates:
```c
...
`Hash.eq(other Self) -> bool` — `Self` resolves to receiver type
...
`fn f[T Hash + Display](x T)` — vtable refs **обе** vtables:
```c
...
- **Missing bound method**: при call `k.hash()` если K bound не имеет
  hash → `type K does not satisfy Hash bound: missing method hash`.
- **Ambiguous bound** (multiple protocols define same name): `method 'hash'
  ambiguous: defined in Hash AND CustomHasher — disambiguate с
  protocol prefix`.
- **Vtable construction failure**: `cannot construct vtable Hash
...
- [ ] Self type substitution работает (test: `Compare.eq(other Self)`).
...
> - Erased K (внутри `fn f[K Hash]`) → vtable call, ~1ns
...
- ⏸️ Multi-bound dispatch (`T: Hash + Display` — 2+ vtables через
  ABI).
- ⏸️ Self type substitution в vtable (thunk cast обоих self/other).
- ⏸️ Default method handling (Compare.ne default = !eq).
