...
3. **nova check std:** p202-компилятор **30 FAIL** vs main-бинарь **21 FAIL** — дельта +9,
   ВСЕ 9 в `std/src/http/**`, ВСЕ — один и тот же `import resolution: in imported module
   'tls.tls' (…nova-tls/src/tls/client.nv): 85:9: expected identifier, got 'if'`.
   **Диагноз: НЕ регрессия Plan 202, а environment-skew.** Сиблинг-репа nova-tls
   (commit 0e13b2b, Plan 201-агент) мигрирована на синтаксис D188 v3 (`consume stream {`
   без `= expr` + `return EXPR(X)` в арме), парсер которого добавлен в main коммитом
   `7375ed427` — который **НЕ ancestor plan-202** (`git merge-base --is-ancestor` = нет;
   ветка форкнута с 48ac2d96a, раньше). Любой бинарь моей ветки — с моими правками ИЛИ без —
   парсит nova-tls@0e13b2b одинаково с ошибкой; main-бинарь (после 7375ed427) — парсит.
   Провал — parse-ошибка ВНУТРИ зависимости, к module-registry keying отношения не имеет.
   Остальные 21 FAIL идентичны у обоих компиляторов (все — `neg/`-фикстуры std, fail-by-design).
   **Дельта исчезает при слиянии plan-202 в текущий main** (парсер D188 v3 приедет из main).
   Замер воспроизведён дважды: параллельно и чистым последовательным прогоном — идентичные
   цифры/списки.
