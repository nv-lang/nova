...
> Roadmap отделён от executable steps намеренно (как Plan 23 в конце
...
> либо остаются open Q в Plan 23.
...
- Effect storage TLS (`_nova_handler_*`) — **NO**, пока scope-snapshot
  ...
  нужен per-fiber state — отложено к Plan 23.2.
...
- **Work-stealing Chase-Lev deque** — Plan 23.2.
- **TLS migration для effect handlers** — Plan 23.2.
- **Blocking pool** — Plan 23.3.
- **`std.sync` API** (`Atomic[T]`, `Mutex[T]`) — Plan 18 + 46.
- **`realtime nogc` pinning** — Plan 23.4.
- **Memory model spec** для shared mut — Plan 23.5.
...
- **Plan 23.2+** — work-stealing, TLS migration, blocking pool, std.sync impl.
