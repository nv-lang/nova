...
- A8.3: Все 126 смешанных папок разделены: негативные в `neg/`, позитивные объединены.
- A8.4: `nova test` (default) — 0 регрессий относительно pre-Ф.8 baseline.
- A8.5: Число compile units (строк в `--results-file`) сократилось не менее чем в 5× для позитивных тестов.
- A8.6: D298 в `spec/decisions/09-tooling.md` обновлён: folder-module test convention + `neg/` subdir convention задокументированы.
