...
**Статус:** 🔨 Ф.1-Ф.2 РЕАЛИЗОВАНЫ 2026-07-17 (диагностика + фиксы влиты в ветку `fix-lsp-cpu`,
собран `nova-lsp.exe`, юнит+интеграционные тесты nova-lsp зелёные); Ф.3 (включение) ждёт
владельца/интегратора — сервер остаётся ВЫКЛЮЧЕН (`nova-lsp-v14.exe.disabled`,
`nova.lsp.enabled=false`) до явной приёмки.
