...
imports.rs/резолвер — параллелить нельзя); далее в очереди окон: 214.1 (catch-panic-hook снят — реализуется композицией supervised+Stop в .nv, без рантайм-хука; пересмотр 2026-07-23).
