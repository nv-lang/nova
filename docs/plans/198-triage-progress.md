...
сохранившийся в scratchpad и побайтово согласующийся с агрегатами census
(1905/1403/111/18/13). Он использован как source of truth для per-file DELETE-списка
(committed TSV намеренно не хранится, doc-hygiene — см. методологию выше).

### DELETE — исполнено: 1158 файлов удалено

Из 1403 файлов, классифицированных STALE в Ф.1:

1. **Перед удалением** — синк `github/main` (fast-forward на `1b4e25627`, принёс
   compiler-codegen изменения волн 196.2/196.3) и **пересборка** компилятора
   (`nova-cli`, release). Компилятор с Ф.1-прохода (Jul 11) успел измениться →
   потребовалась ревалидация, не слепое исполнение старого вердикта.
2. **`nova_tests/contracts/*` (218 STALE-файлов) — ИСКЛЮЧЕНЫ из сноса, ОСТАВЛЕНЫ
   KEEP.** Пересобрал компилятор с `--features z3-backend` (`NOVA_Z3_LIB_DIR` →
   vcpkg главного репо) и перепрогнал `contracts/` под `NOVA_SMT_BACKEND=z3`
   (per явный caveat census: «требуется z3-перепрогон перед Ф.2-сносом»). Результат:
   **PASS: 77, FAIL: 2** (оба — `trivial_string_len_positive`/`_fail`, KEEP-SPECIAL
   Ф.3(a)-deferred), **SKIP: 4** (trivial-only негативы). Находка: весь non-neg
   `contracts/` (~200 файлов census посчитал STALE поштучно) — это **ОДИН merged
   folder-module CU**, который целиком CODEGEN-FAIL **только** из-за
   `trivial_string_len_positive.nv` (`E_STR_NO_LEN`, retired API, backend-агностично)
   — тот самый deferred Ф.3(a)-файл, сидящий в ТОМ ЖЕ CU. Т.е. census-цифра «218
   STALE» — каскадный артефакт ОДНОГО общего блокера, а не 218 независимо мёртвых
   тестов. Реальный per-file вердикт для non-neg contracts/ станет известен только
   после Ф.3(a) (byte_len() allow-list решение владельца, ещё не реализовано).
   **Не гадал — оставил всё как есть.**
3. **Остальные 85 директорий (1185 файлов) — ревалидированы свежим (post-merge)
   trivial-backend бинарём** (`nova test --full <85 dirs>`, 3 батча — один упал
   на известном ICE в `plan48_mpm`, что подтвердило census: ICE валит ВЕСЬ
   процесс, не только файл). Найдено **27 файлов**, которые census пометил STALE
   по **TIMEOUT** (артефакт перегруженного параллельного batch-прогона исходного
   census), но которые **PASS** на свежем прогоне без нагрузки (`plan59` × 2,
   `plan62` × 2, `plan70` × 19, `plan79` × 5). Эти 27 **исключены из сноса**
   (не STALE — ложное срабатывание census).
   `plan60` (3 STALE-файла) — не ревалидирован отдельно: entangled в ОДИН
   merged CU с 3 ICE-файлами того же plan60 (compile юнита без ICE-файлов
   невозможен, см. census про co-equal-module ICE), доверился Ф.1-вердикту.
   `plan65` — ревалидирован точечно (карантин ICE-файла `neg/f2_time_after_removed.nv`
   на время прогона, затем восстановлен) — все 16 STALE-файлов подтверждены.
4. **Итог: 1185 − 27 = 1158 файлов удалено** (`git rm`, коммит `011aadde5`,
   ветка `triage-198`). `nova_tests/` 3450 → 2292 файла.
5. **N/A-support (111 файлов) — НЕ тронуты.** Ни один из них не привязан (через
   `rep`) к STALE-родителю в удалённом наборе — все живые cfg/doc/modules/…
   support-файлы делят поддерево с MIGRATE-потребителем (`*_use.nv` и т.п.),
   подтверждено построчной сверкой per-directory (`nova_tests/cfg` и др.: MIGRATE>0,
   STALE=0 везде, где N/A-support>0). Смешанные N/A-каталоги (`doc`, `modules`,
   `plan107`, `plan160`) намеренно НЕ разбирались file-by-file на этой волне —
   риск неверно снести support-файл, реально нужный MIGRATE-потребителю, выше
   пользы; support-файлы инертны (не самостоятельный test-unit, `nova test` их
   не подбирает), оставление не стоит ничего.

### Находка для Ф.4c-очереди (не чинил — вне объёма Ф.2 execution)

Несколько «100%-STALE»-директорий имеют **ОДНУ** общую error-сигнатуру на ВСЕ
файлы (не 8/54/21 разных причин, а 1) — т.е. это тоже merged-CU-каскад, не 8/54/21
независимо стухших теста:
- `nova_tests/basics` (8 файлов, теперь удалены) — `error: use of undeclared
  identifier 'apply'` в сгенерированном `.c` — **похоже на реальный codegen-баг**
  (не retired-API), уже отмечено в census как кандидат в Ф.4c. Удалил per
  approved-STALE-вердикт (владелец уже одобрил Ф.1); репро-код доступен через
  `git log`/`git show` на удалённых путях при заведении Ф.4c-тикета.
- `nova_tests/syntax`, `nova_tests/types`, `nova_tests/runtime` — аналогично
  один-каскад-на-директорию, но error-сигнатуры (`E_METHOD_REDEFINITION`,
  `undefined identifier`, `E_UNKNOWN_STATIC_METHOD`) выглядят как настоящий
  retired-API/module-дрейф, не баг компилятора — низкий приоритет для Ф.4c.

### MIGRATE (1905 файлов) — НЕ исполнено эту волну, отложено

**Причина отказа от исполнения в этой волне — масштаб, не лень:** корректная
миграция требует per-directory решения target (`spec_tests/conformance/` —
ОДИН merged CU на ВСЕ ~2100 файлов после миграции, требует domain-prefixed
имён и ручного dedup — vs `std/<модуль>/*_test.nv` — per-module CU) + проверку
на дубли с существующими `std/**/*_test.nv` + верификацию green-на-новом-месте
для каждой directory. Это авторская/ревьюерская работа над ~150 директориями,
не механическое перемещение файлов — один агент-проход без суб-агентов не может
сделать это добросовестно без риска сломать conformance CU или потерять
покрытие через слепой dedup. **Файлы НЕ тронуты, `nova_tests/` их сохраняет
(KEEP по умолчанию до отдельной волны Ф.2-continuation).** Полный per-file
список classification (МИГРЕЙТ vs остальное) сохранён в scratchpad этой сессии
(`198_final_verdict_trimmed.tsv`) — рекомендуется скопировать в постоянное
хранилище (не терять) перед следующей волной, раз committed-TSV намеренно не
хранится в git.

### KEEP-SPECIAL (18 soundness/deferred файлов) — НЕ перенесены, отложено

Плана-документ ([198-nova-tests-triage.md](198-nova-tests-triage.md)) сам
помечает целевую директорию как **открытое решение владельца**: «весь блок в
`spec_tests/conformance/` (рек.) или отдельная `spec_tests/soundness/`?» — не
решено. Переносить (и перевешивать CI-ratchet `contracts-z3.yml`
soundness-guard) на угадываемое место — было бы именно тем «гадать», которое
запрещено инструкцией. Все 18 файлов остаются в `nova_tests/contracts/` +
`nova_tests/plan140_4/` нетронутыми; `contracts-z3.yml` не менялся.
4 из 18 (`trivial_string_len_*`, `f26_newtype_*`) дополнительно блокированы
Ф.3(a)/(b) deferred-решениями (компиляторные фиксы, не file-move).
