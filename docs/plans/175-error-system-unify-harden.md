...
> **Top-level umbrella-план.** Создан 2026-06-20. **Статус:** 📋 PROPOSED.
> **Маркер:** `[M-175-error-system]`. **Запуск:** «**выполни план 175**» (план самодостаточен).
> **Decision-gate:** Ф.0 (дизайн) требует sign-off владельца перед Ф.2+. **Подписано 2026-06-20:**
> Model 1 (defer-kernel) + синтаксис `supervised(strategy:…, max_restarts:…, period:…, cancel:…)`.
> Остаток Ф.0: `period`-обязательность + набор `strategy`.
...
2. **supervisor-стратегии** (sign-off 2026-06-20 — реализовать): синтаксис
   `supervised(strategy: one_for_one, max_restarts: N, period: T, cancel: tok) { … }` (parens, `:`-именованные;
   заменяет несогласованный спек-постфикс `} strategy = …`). Erlang/OTP-семантика: ребёнок упал → рестарт
   по стратегии; превышение `max_restarts` за `period` → супервизор падает сам (эскалация). Старт:
   `one_for_one` + `{max_restarts, period}`; `one_for_all`/`rest_for_one` — позже. Обновить parser (`parse_supervised`
   9529), AST, runtime (restart-loop в `nova_rt`), 08-runtime.md (заменить постфикс-форму).
...
| supervisor-стратегии | **✅ РЕАЛИЗОВАТЬ (sign-off 2026-06-20)**, НЕ ретрактить. Синтаксис: `supervised(strategy: one_for_one, max_restarts: N, period: T, cancel: tok) { … }` — parens + `:`-именованные аргументы (расширяет реализованный `supervised(cancel:…)`); **заменяет** несогласованный спек-постфикс `} strategy = …, max_restarts = …` (08-runtime.md:168, `=`, не реализован). Под-решения Ф.0/Ф.3: (a) `period` обязателен (OTP `{intensity, period}` — `max_restarts` без окна почти бесполезен) — рекоменд. да; (b) набор `strategy` — `one_for_one` на старте, `one_for_all`/`rest_for_one` позже. |
