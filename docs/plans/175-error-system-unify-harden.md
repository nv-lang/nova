# Plan 175 — Система ошибок и cleanup: унификация + hardening (panic/fail/defer/on_exit), production-grade

> **Top-level umbrella-план.** Создан 2026-06-20. **Статус:** 📋 READY — Ф.0 закрыта (sign-off 2026-06-20); Ф.1 можно стартовать.
> **Маркер:** `[M-175-error-system]`. **Запуск:** «**выполни план 175**» (план самодостаточен).
> **Decision-gate: Ф.0 ЗАКРЫТА (sign-off 2026-06-20).** Model 1 (defer-kernel); supervisor — все 3 стратегии
> (`one_for_one`/`one_for_all`/`rest_for_one`), `period` обязателен (`Duration`), синтаксис
> `supervised(strategy:…, max_restarts:…, period:…, cancel:…)`. План готов к запуску («выполни план 175»).
> **Поглощает/завершает:** [173](173-panics-clause.md) (panics-clause), [174](174-question-mark-return-only.md)
...
|---|---|---|
| `spawn`/`supervised` | throw ребёнка ловится внешним `with Fail` (scope-гранулярность); panic неперехваченный → abort; первая ошибка → siblings cancel cooperatively (USER beats CANCEL) | **нет supervisor-стратегий** (one_for_one/max_restarts заявлены 08-runtime.md:168, парсер берёт только `supervised(cancel:…)`, `parser/mod.rs:9529`); **нет per-fiber catch** (first-wins, нельзя узнать какой fiber упал) |
...
- **Тесты pos:** `nova_tests/err175_f1/`: panic сквозь `with Fail` → процесс падает с `panic:` (fn-main +
...
- **Тесты pos:** `nova_tests/err175_f2/`: `defer(o)` Success/Failure/Panic ветки; errdefer/okdefer-эквиваленты;
...
2. **supervisor-стратегии** (sign-off 2026-06-20 — реализовать): синтаксис
   `supervised(strategy: one_for_one, max_restarts: 3, period: 5.seconds(), cancel: tok) { … }` (parens, `:`-именованные;
   заменяет несогласованный спек-постфикс `} strategy = …`). **Все 3 стратегии:** `one_for_one` (рестарт упавшего),
   `one_for_all` (упал один → рестарт всех), `rest_for_one` (упавшего + стартовавших после него). `period` **обязателен**,
...
- **Тесты pos+neg:** `nova_tests/err175_f3/`: child throw → siblings cancelled + MultiError; per-element Result;
...
- **Тесты pos+neg:** `nova_tests/err175_f4/`: primary+suppressed видны в handler; typed Failure dispatch; cleanup-fail во время body-fail → MultiError (не overwrite); chain переживает голый throw в unwind.
...
5. **Нейминг (sign-off 2026-06-20):** (a) протокол `Consumable[E]` → `Cleanup[E]`, метод `@on_exit` → `@cleanup` — везде (`std/prelude/protocols.nv`, codegen, consume-тесты, доки, amend D188/D194); часть Ф.2 (consume = сахар). (b) эффект D185 `Cleanup` → `ScopeTrace` — parser + codegen-dispatch (`on_scope_enter`/`on_scope_exit`), 3 теста `plan110/cleanup_*`, доки, amend D185 (имя честное: наблюдает, не чистит; ambient OTel/APM-трейсинг consume-scope'ов). (c) модернизировать `ScopeTrace.on_scope_enter` `timeout_ms int` → `Duration`. **Эффект НЕ ретрактится** (рабочий, 3/3 PASS) — только переименован.
5. Поглотить риск [Plan 173](173-panics-clause.md): `nova_runtime_reset()` между panic-тестами в одном процессе (re-entry hazard 173.md:56-73) — естественно рядом с `nova_scope_exit`.
- **Тесты pos+neg:** `nova_tests/err175_f5/`: double on_exit → `D188-on-exit-double-invocation`; exit-timeout 3 уровня; cleanup-timeout наблюдаем.
...
| supervisor-стратегии | **✅ РЕАЛИЗОВАТЬ (sign-off 2026-06-20)**, НЕ ретрактить. Синтаксис: `supervised(strategy: one_for_one, max_restarts: 3, period: 5.seconds(), cancel: tok) { … }` — parens + `:`-именованные (расширяет реализованный `supervised(cancel:…)`); **заменяет** несогласованный спек-постфикс `} strategy = …` (08-runtime.md:168). **Закрыто:** (a) все 3 стратегии сразу — `one_for_one` / `one_for_all` / `rest_for_one`; (b) `period` **обязателен**, тип `Duration` (идиом `N.seconds()`/`N.millis()`, std/time), пара `{max_restarts, period}` = OTP intensity/period. |
...
| Нейминг: observability-эффект | **✅ sign-off 2026-06-20:** D185 `Cleanup` effect → **`ScopeTrace`** (НЕ ретрактить — рабочий, 3/3 PASS plan110; имя было мисномером: наблюдает, не чистит) → освобождает `Cleanup` под протокол. Заодно `on_scope_enter` `timeout_ms int` → `Duration`. ScopeTrace = ambient OTel/APM-обсервабилити всех consume-scope'ов (span на enter/exit с outcome). |
...
`[M-172-errdefer-okdefer-dead-surface]`), [173](173-panics-clause.md), [174](174-question-mark-return-only.md).
...
`[M-175-error-system]`. Возможная декомпозиция в суб-планы при росте: Ф.3 → 175.1 (structured-concurrency),
Ф.4 → 175.2 (MultiError/typed-outcome).
