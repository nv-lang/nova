...
> **Decision-gate:** Ф.0 (дизайн) требует **sign-off владельца** перед Ф.2+.
...
2. **supervisor-стратегии**: реально реализовать `one_for_one`/`max_restarts` (заявлены 08-runtime.md:168) или
   ретракнуть из спеки (решение в Ф.0/Ф.3).
...
| Вопрос | Решение |
|---|---|
| Унификация: одна модель или две поверхности | **Model 1 «defer — ядро»** (рекоменд.); `consume`/`Consumable` = сахар. Fallback Model 2 если разом велико. |
...
| supervisor-стратегии | решить в Ф.0/Ф.3: реализовать one_for_one/max_restarts ЛИБО ретракнуть из 08-runtime.md (не оставлять «заявлено, но нет»). |
