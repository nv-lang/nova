...
### 6.1 Cheap-first (вместо растущих стеков) — `[M-fiber-arena-raise-cap]`
Если потолок 16k когда-нибудь упрётся — сначала **дешёвый win, БЕЗ растущих стеков и БЕЗ GC**:
**уменьшить per-fiber virtual-reserve (8MB→1-2MB) + поднять `NOVA_FIBER_SLOT_COUNT`** →
~50-100k fiber'ов за ~полдня. Virtual ≈ бесплатен на 64-бит; физическая RAM ленивая (платишь
