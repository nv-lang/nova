...
**Статус:** 🔨 Ф.0-Ф.2 РЕАЛИЗОВАНЫ (2026-07-16, ветка `p208-impl`, owner-go получен). Ф.0 (спека D422
...
§«Статус реализации» и `docs/plans/208-impl-progress.md`). **Ф.3 (дженерики `.nv` + auto-derive
компактной Display-формы) — НЕ начата.** **Ф.4 (снос `conv.h` → буфер-примитивы Ф.1) — НЕ начата**,
заблокирована на V1-упрощении #3 (примитивный форматный путь остаётся на `conv.h`, ничего пока не
мертво). Авторитетный полный `spec_tests/conformance`-гейт — за оркестратором/интегратором (не
гонялся здесь, только таргетные изолированные фикстуры).
