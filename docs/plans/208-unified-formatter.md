...
§«Статус реализации» и `docs/plans/208-impl-progress.md`). **Ф.3 (дженерики `.nv` `[]T`/`Vec[T]`/
`Option`/`Result` Display/Debug + auto-derive компактной Display-формы) — СДЕЛАНА** (волна 2,
`docs/plans/208-impl-progress.md` §«Ф.3»; попутно найдены и починены 3 тихих пре-существующих
дефекта: `Vec[T]` структурно не удовлетворял Display/Debug, `Option`/`Result` вообще не имели
`@display`, auto-derive Display на примитивном поле звал ретрактированный `str.from`). **Ф.4 (снос
...
big-bang волной, не безопасными мелкими шагами (см. `208-impl-progress.md` §«Ф.4 — статус:
...
НЕ починено, НЕ регрессия 208). Авторитетный полный `spec_tests/conformance`-гейт — за
