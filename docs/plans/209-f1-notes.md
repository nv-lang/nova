...
- A2: `split_tu(finalized: &str, threshold: usize) -> (String, Vec<String>)` —
  структурный сегментатор (глубина `{}` + строки/комментарии), классификатор
  decl/def, auto-generated common.h прототипы, дедуп decl-only совпадающих по
  имени с def. Unit-тесты на синтетический `.c`.
- A3: debug-assert уникальности top-level имён + "каждый вызываемый символ
  имеет прототип в common.h" (простая проверка внутри/около split_tu).
