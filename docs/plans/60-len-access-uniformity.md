...
> **Status:** proposed (2026-05-17, revised). Production-grade unification, не quick clean-up. Реализует **D-блок D117** (новый) — «size-like accessors всегда method-вызов».
...
4. **Diagnostic в type-checker** на field-access `T.accessor` где `T` имеет одноимённый метод и accessor ∈ {`len`, `cap`, `byte_len`, `is_empty`, `count`, `size`}:
   ...
   42  |     println("${vec.len}")
        ...
        = see Plan 60 / D117 for migration rationale
   ```
...
- [ ] D117 new block в `spec/decisions/03-syntax.md`.
...
- [x] Diagnostic для bare `.len` включает: (a) sample fix snippet, (b) link на D117 / Plan 60, (c) note про bound method value, (d) span подсвечивает имя метода.
- [x] При наличии Plan 36.D ergonomics — `nova explain E_SIZE_ACCESSOR_FIELD` показывает full migration rationale.

### Spec parity

- [x] D117 принят в `spec/decisions/03-syntax.md`.
...
2. **`vec.first` / `vec.last`** (option-returning) — тоже подпадают под D117?
   **Decision (proposed):** **нет** — это не size-accessors, это element-accessors. Семантика разная (могут возвращать `Option[T]`, могут throw — D117 только про cardinality). Оставить как method-only по convention'у, но без жёсткого D-block enforcement.
...
|---|---|---|---|---|
| **Rust** | `vec.len()` method | `s.len()` method (bytes) | `map.len()` method | none — D117-like |
...
Уникальное преимущество Nova над Rust: D117 — это **explicit spec'ed contract**, в Rust это implicit convention (никакого rustc-error если ты определишь field `len` на своём struct). LLM, читающий `spec/decisions/03-syntax.md#D117`, имеет однозначный сигнал; LLM в Rust-проекте может ошибиться и определить публичное поле `len`.
