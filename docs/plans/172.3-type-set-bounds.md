...
тонкими обёртками** (×~10 типов × N методов). Прямой триггер — Plan 172.6 (primitive
...
но без type-set bound это невозможно — приходится плодить обёртки (вариант A в Plan 172.6).
...
- **Разблокирует Plan 172.6 (primitive parse API), вариант B** — после 172 per-type обёртки `int.try_parse`/`u32.try_parse`/… схлопываются в один `fn[T IntNumber] T.try_parse`. До 172 — Plan 172.6 идёт вариантом A (ядра + обёртки).
