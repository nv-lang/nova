...
**Статус:** 📋 СОГЛАСОВАН 2026-07-14 (наблюдение + дизайн подтверждены владельцем). **После:** [194](194-contract-execution-model.md). Ф.0/Ф.1/Ф.1b — чекпоинт волны удалён при закрытии, см. git-историю (статус актуален в [README.md](README.md), не здесь).
...
- **Ф.1b Sized-int trap-дефолт** (решение A, см. ниже): `nova_<T>_checked_add/sub/mul` для sized-типов
  (или generic per-type через `__builtin_*_overflow`); лоуэрить типизированный `+`/`-`/`*` в них
  (сейчас сырой C = signed-UB). Пиновые тесты overflow-паники на i8/i16/i32/i64/u8/…/u64.
...
## ★ Решение (владелец 2026-07-15): trap-дефолт для ВСЕХ Ints (вариант A, Swift-модель)

Найдено при ревью: `effects.h` имеет лишь `nova_int_checked_add/sub/mul`, emit_c (`~27161`/`~28033`)
хардкодит `nova_int_checked_*`. **Значит `+`/`-`/`*` трапят только для `int`; sized-типы (`i8`..`i64`,
`u8`..`u64`) — сырой C-`+` (signed = UB, unsigned = тихий wrap).** Дыра звучности.

Прайор-арт (обзор 2026-07-15): Swift — всегда трап (все размеры); Rust — debug-паника / release
*defined* wrap (не UB); Zig — safe-паника / fast-UB; Go — всегда defined-wrap; C — signed-UB. **Никто,
кроме C, не оставляет signed-overflow как UB.** Наш sized-int = единственный «плохой» вариант.

**Решение — вариант A (Swift-модель): `+`/`-`/`*` трапят на переполнении у ВСЕХ Ints** (не только `int`).
Обоснование: (1) консистентность с уже-трапящим `int`; (2) убирает signed-UB; (3) always-on-safety БЕЗ
Rust'ова перф-компромисса «release-wrap» — у нас есть **Z3-элизия (`optimized`)**, снимающая
доказуемо-безопасные проверки (недоказуемые остаются). Это козырь 194 над Rust: safety без налога.

**Реализация (в объёме 206):** добавить `nova_<T>_checked_add/sub/mul` для sized-типов (или generic
per-type через `__builtin_*_overflow`), лоуэрить типизированный `+`/`-`/`*` в них (сейчас — сырой C).
Перф-паритет: где проверка доказуемо-избыточна — `optimized` Z3-элизия снимет, как для `int`.
Фаза: **Ф.1b — sized-int trap-default** (между Ф.1 интринсиком и Ф.2 обёртками).

## Границы
Не трогает Z3-элизию. `div`/`neg` — вынесены в подплан **[206.1](206.1-div-neg-trap.md)** (иной механизм:
у `__builtin` нет div-overflow; div-by-zero сейчас = SIGFPE-крэш, P1). 206 ДОБАВЛЯЕТ явные политики
(методы) + дедуплицирует overflow-детект в один примитив + закрывает sized-trap (решение A выше).
