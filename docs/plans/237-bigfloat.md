...
**Статус:** 🔨 В РАБОТЕ 2026-07-31 (окно big-pickle/opencode в репе nova-bigint; зависимости 235/236 сданы). **Приоритет:** P3 (не блокирует v0.1; стек ПОСЛЕ [235](235-bigint.md) и [236](236-bigdecimal.md) — BigInt-фундамент первичен, BigDecimal-конверсии расширяют покрытие).
...
## 0а. Конкурентный контекст — откуда какие ставки (ревью 2026-07-30)

**MPFR** (Multiple Precision Floating-point Reliable) — C-библиотека произвольной точности
поверх GMP (big-целые), золотой стандарт области (INRIA, 25+ лет; ей считает константы сам
GCC; Rust `rug` — просто её обёртка). Её главное свойство — **correct rounding**: каждая
операция, включая трансцендентные, доказанно даёт результат, округлённый точно до последнего
бита. Мы её не подключаем (C-зависимость против «чистый .nv»), но берём у неё СЕМАНТИКУ.

| | MPFR / Rust `rug` | Go `math/big.Float` | Python `mpmath` | **наш 237 (V1)** |
|---|---|---|---|---|
| модель точности | контекст на операции | prec **у значения** | глобальный `mp.prec` | **контекст на операции** |
| трансцендентные | ✅ все, correctly-rounded | ❌ нет в stdlib | ✅ богатейшие | ❌ V2+ (паритет Go) |
| Inf/NaN | полная IEEE-модель | Inf есть, NaN → паника | есть | нет — trap D423 |
| зависимости | GMP+MPFR C-сборка (боль на Windows) | чистый Go | чистый Python | **чистый .nv** |
| мутабельность | мутабельный | мутабельный (`z.Add(x,y)` пишет в z) | — | **иммутабельный** |
| rounding-режимы | 5-6 | 6 (нет tiesToZero) | — | **7** |

**Ставки дизайна (что у кого взято и что отвергнуто):**
- **Семантика — MPFR/Java-MathContext:** явный `ctx` на КАЖДОЙ операции. Отвергнуты:
  Go-модель «prec у значения» (результат зависит от prec приёмника — источник тонких багов)
  и mpmath-модель глобального мутабельного контекста (нарушает referential transparency).
- **Форма пакета — Go `math/big`:** чистый язык поверх собственного BigInt, без C-зависимостей
  (Go этим и победил; `rug` проигрывает именно сборкой GMP/MPFR).
- **Nova-специфика:** иммутабельность (нет Go-класса ошибок alias-мутации приёмника; лимбы
  шарятся через GC) + отсутствие тихих Inf/NaN (trap D423; Go сам паникует на NaN — индустрия
  дрейфует сюда).

**Где V1 честно слабее (осознанная цена, тропы в §6):** скорость — школьная O(n²) против
GMP-ассемблера, на порядки; трансцендентных нет (как и у Go stdlib); correctly-rounded
гарантирован только для `+ - × ÷` (однократное округление точного результата) — `sqrt` через
Ньютона даёт «сошлось до стабильности», НЕ доказанное последнее-битовое округление (см. риски).
...
// РЕШЕНО владельцем 2026-07-30 (Ф.0(2) закрыт): суффикс 'bf' УБРАН — вторая дверь без
...
| `sqrt`: Ньютон со стоп-критерием даёт «сошлось до стабильности», а НЕ доказанный correctly-rounded последний бит (MPFR гарантирует; известное ограничение класса V1) | Ф.5-тесты против MPFR-эталона ловят расхождения последнего бита; при находках — документировать как ±1 ulp; V2 — Tuckerman-проверка остатка |
