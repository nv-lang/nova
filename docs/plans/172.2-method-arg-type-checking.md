...
# Plan 172.2 — Method-argument type-checking + scalar-narrowing enforcement

**Статус:** ✅ closed 2026-06-26 (реализовано — checker-side method-arg narrowing через `check_instance_overload` + миграция unicode на `codepoint = u32` (D327); гейт зелёный). Closure — §10.
**Umbrella:** [Plan 172 — Переработка компилятора: типовая система](172-compiler-rework.md).
**D-блок:** **D327** (spec/decisions/02-types.md — «Codepoint = `u32`»; предв. D311 не использован, занят в ходе impl).
**Ветка:** `plan-172-unified-type-engine` (commit `b2de1218c`).
**Приоритет:** P1 (soundness).
**Часть:** [Plan 172.1 — Unified type engine](172.1-unified-type-engine.md) — этот план первый
конкретный шаг 172.1 (часть U.1-U.3 на узком кейсе method-arg narrowing).
...
## 9. Целевая архитектура и этапы — см. Plan 172.1

Целевая архитектура (единый типизированный IR) и пошаговая дорожная карта U.1-U.7 живут в
[Plan 172.1 — Unified type engine](172.1-unified-type-engine.md) §3-§4. Этот план (172.2) —
**первый конкретный шаг** 172.1 на узком кейсе method-arg narrowing; раздел §8 выше — детальная
карта «как есть», на которую 172.1 ссылается.

---

## 10. Реализация (закрытие, 2026-06-26)

**Выбор Q1 — вариант (C), checker-side.** Narrowing на аргументах метода enforce'ится в чекере
в `check_instance_overload` ([types/mod.rs](../../compiler-codegen/src/types/mod.rs)): на ВЫБРАННОЙ
перегрузке (single-overload или единственная type-compatible) подставляем concrete type-args
получателя в каждый параметр (`build_recv_subst` + `subst_typeref`), затем `assignable`, и эмитим
`[E_IMPLICIT_NARROWING]` **только** на `Compat::Narrowing` — зеркало free-fn loop в `f1_check_call`,
то же правило `would_narrow_into` (single source; см. §2). Покрывает **builtin** `Vec.push` (хотспот
из §1) — receiver-инференс + slice-ctor inference дают конкретный `Vec[u32]` → `u32`-параметр.
Permissive на Bad/Unknown/unsubstituted-generic и на synthesized-арг без реального span (desugared
`v[i]=x` и т.п.) → 0 ложных срабатываний (критерий §5.4). Соблюдён Q4 (alias/generic — skip).

**Правило narrowing (D54, подтверждено):** `would_narrow_into` (types/mod.rs:288) — `int → u32`
= signed→unsigned ⇒ **narrowing** (ошибка и у методов, и у free-fn); `u32 → int` = safe widening
(range fits) ⇒ **OK**; литералы идут отдельным range-check путём (`general_category(0x41)` валиден).

**Миграция std/unicode на `codepoint = u32` (D327).** Сужения, которые вскрыл механизм, закрыты
не `as`-обёртками, а правильным типом потока: codepoint теперь `u32` сквозняком (Vec[u32] storage,
параметры/арифметика u32, char только на str-границе). 8 файлов: `cp_utils.nv` (`decode_at -> (u32,
int)` — codepoint как u32 на границе декода; maps u32-keyed), `normalize.nv`, `case.nv`,
`category.nv`, `ranges.nv`, `graphemes.nv`, `words.nv`, `sentences.nv` (`sbcat(cp u32)`),
`collate.nv` (maps `HashMap[u32,str]`, `contraction_lookup/has_longer(first u32)`,
`push_implicit(cp u32)` + `ro c = cp as int` для weight-арифметики). `int`-ключи у `comp_map`
((a<<21)|b — packed-pair > 32 бит) и category-коды (WB_*/SB_* — класс/мера) ОСТАВЛЕНЫ `int` (D327
§bit-packing / §measure). `int`-литералы в публичные cp-функции адаптируются (range-check, не
narrowing).

**Codegen-prerequisite (часть 172.2):** `HashMap[u32, V]` был сломан в codegen — `prim_builtin_method`
([emit_c.rs](../../compiler-codegen/src/codegen/emit_c.rs)) имел hash-builtin только для
`nova_int`/`nova_char`/`nova_bool`/`nova_f64`, но не для `uint32_t`/прочих sized-int. `key.@hash()`
для u32-ключа не находил builtin и mis-dispatch'ился на чужой `@hash` (`Nova_str_method_hash` от V=str
→ undefined symbol при link). Латентная дыра (тот же класс, что фикс `char` в Plan 152.8) — никогда
не срабатывала, т.к. до D327-миграции codepoint-мапы были `int`-keyed. Фикс: расширил hash-арм на
ВСЕ целочисленные C-типы (`uint8_t`/…/`int64_t`) → `nova_int_hash` (C-промоушн к int64,
детерминирован/lossless). Ключевое сравнение HashMap идёт прямым `==` (operator-lowering), не
`.eq()`, → eq-арм не трогали.

**Гейт (зелёный):**
- `nova_tests/unicode` + `nova_tests/strings` → PASS, FAIL:0. unicode — folder-module (один CU,
  entry `plan152_0`), 255 PASS test-блоков, **conformance зелёный**: UAX#15 Normalization (NFC/NFD),
  UTS#10 CollationTest_SHIFTED, UAX#29 Word/Sentence/Grapheme, category/case conformance, Hangul.
- narrowing count (`grep -c E_IMPLICIT_NARROWING` в прогоне) = **0**.
- §7.5 baseline-DELTA: импортёры unicode (plan154/plan159) — **0 новых FAIL**. (4 CC-FAIL в plan159
  `f2/f3` DCE-generic-const — **pre-existing**, идентично падают на baseline `main`; не от 172.2.)
- detect172: POS `u172_2_method_arg_narrowing_pos` (u32-var / `as u32` / in-range литерал → OK) +
  NEG `neg/n_method_arg_narrowing` (`Vec[u32].push(int_var)` → `E_IMPLICIT_NARROWING`).

**Followups (НЕ в 172.2, в backlog-followups.md):**
- `[M-match-arm-mixed-int-width-sentinel-coerce]` — ✅ **ЗАКРЫТ 2026-07-21** (ветка
  `p-fix-match-arm-width`, sonnet). `infer_match_common_primitive` теперь unify'ит
  safe-widening-совместимые арм-ширины к более широкой стороне (D54 `would_narrow_into`)
  вместо bail; genuine-несовместимые ширины (`i32`/`u32`) → hard `E_MATCH_ARM_WIDTH_MISMATCH`.
  D-амендмент D433 (`spec/decisions/02-types.md`). Детали/фикстуры/гейты —
  `docs/plans/backlog-followups.md` (запись убрана из OPEN-view после уборки).
- `[M-option-eq-some-literal-elem-adapt]` — `Option[u32] == Some(int_literal)` не адаптирует литерал
  → CC-FAIL (`NovaOpt_nova_int` vs `NovaOpt_uint32_t`). Обойдено в POS-фикстуре через `v[i]` index.
  P2.
