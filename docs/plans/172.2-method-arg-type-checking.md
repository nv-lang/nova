...
- `[M-match-arm-mixed-int-width-sentinel-coerce]` — `match o { Some(v:u32)=>v, None=>-1 }` в
  `-> int`-функции тихо коэрсит `-1` к u32 (0xFFFFFFFF). `infer_match_common_primitive`
  (types/mod.rs:7361) бейлит на расхождении арм → legacy re-derive по первой арме. Это §1
  («авто-выведенный неверный тип») + §4 (тихая дыра). Миграция `compose_pair → Option[u32]` убрала
  триггер из unicode, но не из компилятора. P1.
