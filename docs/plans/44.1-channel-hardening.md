...
# План 44.1: Channel hardening — production parity с Go/Rust
...
- `NOVA_SELECT_MAX_ARMS`: 16 → 32 с комментарием «Plan 44.1 Ф.1 заменит
  на per-call storage».
...
- **B1/B2/B5-storage/T1/T2:** Plan 44.1 Ф.1 — атомарность + mutex +
...
- Plan 44.1 functional + perf tests PASS.
- ~150 строк, 1 commit.
...
- Plan 44.1: status Ф.1 → ✅ (все 4 prerequisites закрыты:
  storage ✅ + B1 ✅ + T2 ✅ + B2 ✅).
...
- Plan 23 (M:N runtime) — отдельный план, intermixing scheduler + Plan 44.1
  channel layer. Plan 44.1 Ф.1 — prerequisite, не Plan 23 в целом.
...
`SelectCtx storage` на стеке (Plan 44.1 Ф.3 final): `n × sizeof(SelectSlot)`
...
После Plan 44.1 design review относительно Go runtime channels
...
**Что было:** Plan 44.1 sync.h использует одинаковый `acq_rel` для всех
atomic ops.
...
**Что было:** Plan 44.1 говорит «is_closed() fast-path → следующая
...
**Что было:** Plan 44.1 Этап 2 audit говорит «select_park регистрация
...
**Что было:** Plan 44.1 говорит «stop_cb берёт mutex; если sched.h
...
question** в Plan 23 design space. Plan 44.1 Ф.1 **может не закрыть это
сам**.
...
**Что было:** Plan 44.1 Этап 6 — pthread stress tests. Не упоминает
Boehm requirement.
...
**Что было:** Plan 44.1 не специфицирует — если send-arm проиграл, и
...
**Что было:** Plan 44.1 имеет только writer-side close. Dropped receiver
...
**Что было:** Plan 44.1 Этап 7 закрывает Ф.1 с 257/257 PASS.
...
| Unbounded channel | Linked-list-of-blocks à la crossbeam. ~250 LOC. **Real use case:** log/metric pipelines. | Plan 44.2 |
...
### Anti-patterns identified в текущем Plan 44.1
...
Plan 44.1 говорит «add to both for unified wake». Trade-off: +4 байта на
...
Plan 44.1 Ф.3 final: select storage на fiber stack via compound literal.
...
### Что Plan 44.1 делает лучше Go/Rust (зафиксировать как сильные стороны)
...
- Документация в spec D94 и Plan 44.1 явно перечисляет:
...
| C5 | False sharing — Plan 44.1 хвастался «no padding is strength» | strength claim | **inverted: P1** | crossbeam padding gives **300× perf** under contention (alic.dev measurement). При M:N с 8+ thread'ами Plan 44.1 будет в 100-1000× slower без padding. |
...
- **A3 lockorder: P0 → P2 (под contract B).** Round 2 показал: lockorder
  нужен только если когда-либо держим ≥2 locks simultaneously. Plan 44.1
...
Текущий Plan 44.1 audit 1 говорит «stop_cb берёт mutex; edge cases в
...
1. **Auto-close on dropped receiver** (Tokio yes, Go no, Plan 44.1 = Go).
...
- **Documentation:**
  ...
  - spec D94: bootstrap-ограничения + Plan 44.1 hardening + что отложено.
  - Plan 44.1: implementation log per этап.
  - project-creation.txt + simplifications.md + discussion-log.
...
4. **Heap fallback при n_arms > 256** — overlaps с Plan 44.1 Ф.3 «no cap»
...
- MSVC: `<threads.h>` с VS2019 16.8+, **но `<stdatomic.h>` только с VS2022 17.5+**.
  Plan 44.1 audit r1 ошибся утверждая что MSVC VS2019 16.8 покрывает обоих.
...
**Plan 44.1 не собирётся** на: mingw-w64, MSVC VS2019, Apple Clang без
...
**Cost:** ~120 LOC. **P0 — без этого Plan 44.1 не собирается на 3 из 4
...
Plan 44.1 acceptance criterion «<50 ns round-trip uncontended» **fails on
...
macOS support. Если macOS не target — явно doc'нуть в Plan 44.1.

**Решение для bootstrap:** добавить macOS в Plan 44.1 supported platforms.
Cost ~30 LOC.
...
```c
/* channels.h: */
/* INVARIANT (Plan 44.1 R3-1): BaseWaiter chain MUST be GC-reachable от
...
short critical sections (glibc benchmark). Plan 44.1 critical sections
...
**Решение:** doc в Plan 44.1 / spec D94:
...
peak waiters для Prometheus. Plan 44.1 не имеет.
...
wraparound. Plan 44.1 Этап 5 Dockerfile UBSan variant нужен filter.
...
Plan 44.1 silently changes internals; user-facing API unchanged. **Doc
explicitly** в Plan 44.1 Acceptance + spec D94: «No source-level breakage».

#### R3-16 — CI bench reproducibility methodology

`<50ns` acceptance без specification hardware/methodology = unfalsifiable.

**Решение в Plan 44.1 Этап 7:**
...
1. **R3-2 C11 toolchain portability** — Plan 44.1 не собирётся на mingw,
...
1. broadcast::channel — 400 LOC + Loom verification → Plan 44.2+.
...
### Архитектурное решение по toolchain support (Plan 44.1 1.0)
...
Решение задокументировано в spec/overview.ru.md.

---

## Production audit — round 4 (2026-05-12)

После закрытия Ф.1 + Linux Docker validation + создания Plan 44.2 — round
...
Plan 44.1 Ф.1 implemented M:N prerequisites (atomics + mutex + selectdone
CAS) **предполагая** что concurrent GC будет работать. Но Plan 44.2
...
Plan 44.1 audit раунды 1-3 это **не зафиксировали** потому что считали что
`_NOVA_GC_DISABLE` концептуально отдельный план. Реально это
**hard dependency**.

**Решение:** В Plan 44.1 invariant explicitly: «Plan 44.1 atomics correctness
depends on `_NOVA_GC_DISABLE` removal which is Plan 44.2's contract.
Until Plan 44.2 closes, Plan 44.1 Ф.1 single-thread-safe only. TSan tests в
Этап 6 deliberately bypass fiber scheduler.»

Это **doc-only fix** в Plan 44.1 (~10 LOC). Real implementation fix —
Plan 44.2.

**Cross-plan implementation order (corrected):**
1. **Plan 44.2 first** (P0 fixes: P41-2, P41-3, P41-5, P41-11).
...
**Plan 44.1 estimated под mutex-everywhere:**
...
Plan 44.2 raises stack 2MB, mitigating до 500k+ arms. Но pre-Plan 44.2
...
- Или document «soft cap ~13k arms pre-Plan 44.2, ~500k post-Plan 44.2» в spec D94.
...
### Plan 44.1 status post-round-4

**Не invalidate Ф.1 closure.** Round 4 — это **documentation + minor
polish** issues, не correctness gaps. Implementation valid.

**Cross-plan dependency** explicit'но зафиксирован — Plan 44.2 должен
land first для real M:N readiness, **независимо** от Plan 44.1 audit
result.

### Production positioning после Plan 44.1 + Plan 44.2 closed (with P0 fixes)
...
После Plan 44.2 design + 4 prior round'ов — fresh audit нашёл 2 quick
fixes реализуемые **сейчас**, не upstream'ом в Plan 44.2.
...
**CVE-2017-1000366 mitigation.** Single 4KB guard page (Plan 44.2 P41-5)
...
**Эффект:** даже **до** Plan 44.2 implementation user code защищён от
stack-clash. После Plan 44.2 — defense-in-depth: clash-protection
...
3. **No per-field cache padding (crossbeam over-padding):** Plan 44.1
...
**Plan 44.1 уses `tail = (head + count) % cap`.** Modulo на каждый send.
...
| 3 | **R5-1 strengths over Go documented** | P2 | Plan 44.2 + spec |
...
1. **Stack-allocated select arms** — Plan 44.1 Ф.3 final.
2. **`on_select_lost` callback** — Plan 44.1 Ф.2 B7.
3. **No cache padding bloat** — Plan 44.1 R2 C5 access-group padding.
4. **BaseWaiter common prefix** — Plan 44.1 R2 C1 strict-aliasing safe (cleaner чем Go's sudog type pun).

### Что Plan 44.1 **может** лучше Go/Rust (long-term)
...
- **Linux Docker:** 261/261 PASS (perf bench skipped — Boehm/Docker
  SEGV not Plan 44.1 bug).

### Plan 44.1 final status (round 5)

**Ф.2 + Ф.3 + Ф.1 closed.** Round 5 = 2 quick fixes implemented +
documentation updates. **No architecture changes needed.** План
готов к production use под Plan 23 M:N **с Plan 44.2 closing first**
...
за scope этой сессии. Открыты как Plan 44.1 round 9+ или поглощены в
