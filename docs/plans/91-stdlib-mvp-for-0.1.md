...
| 91.8a | [Protocol canon renames + Ordering removal + default bodies (D183)](91.8a-protocol-canon-renames.md) | `Iter→Iterable`, `Display→Display`, `Equal.eq→equals`, `Compare.cmp→compare -> int`, remove `Ordering`, default body parser syntax + embed override | ✅ part 1 2026-05-29 (default body codegen synthesis — followup 91.8a.2) |
| 91.8a.2 | [Default body codegen synthesis + protocols refactor + From identity blanket (D183 amendment)](91.8a.2-default-body-codegen-and-from-blanket.md) | Orthogonal protocols (Equal holds equals default, Compare standalone), Display.fmt default, `fn[T] T.from(t T) -> T => t` blanket, lazy synthesis at use-site, Self в param-type | ✅ part 1 + part 3 MVP synthesis 2026-05-29 (Equal.equals + Display.fmt MVP inline fallbacks); general lazy synthesis + cache + Plan 101 mono extension — followup [M-91.8a.2-default-body-general] |
| 91.8b | [Operator dispatch через protocols (D363, ex-D184)](91.8b-operator-dispatch-protocols.md) | `==`→`@equal`, `<`/`>`/`<=`/`>=`→`@compare`; удалить `@eq/@lt/@le/@gt/@ge` magic methods | ✅ 2026-06-17 (D46+D184; str @eq→@equal; 6/6 PASS) |
| 91.8c | [Generic array API: sort/min/max + _by (D185)](91.8c-generic-array-api.md) | `fn[T Compare]` sort/binary_search/min/max + callback `_by` variants; `reverse_of/position_of/count_of/find_of` | ✅ 2026-06-17 (D185; 13/13 PASS; `_of` suffix; [M-91.8c-pdq-sort] followup) |
| 91.14 | [Debug protocol + `${x:?}` format spec (D229)](91.14-debug-printable-and-format-spec.md) | `protocol Debug { fn @debug(sb consume StringBuilder) -> StringBuilder }`; `${x:?}` interpolation desugars to `@debug`; `#impl(Debug)` manual impl | ✅ 2026-06-17 (D229; 21/21 PASS; [M-91.14-derive-debug] followup) |
