...
**ПРЕДМЕТ — СЕМЕЙСТВО, А НЕ ОДНА ПОДПИСЬ, и это решено вторым указанием подряд.** Владелец
сказал ту же фразу дважды: первый раз с выделением `search.nv:169`, второй — без выделения. Два
одинаковых слова подряд читаются как класс; писать фазу на одну строку значило бы получить то же
слово через неделю про `@lines`, потом про `@matches`.
...
**Открытые вопросы:** какой именно метод владелец имел в виду ВТОРЫМ указанием — выделения не
было (адресат — владелец через окно b5; умолчание: фаза пишется на все пять, потому что два
одинаковых слова подряд читаются как класс; срок — к началу работы). Ответ может сузить
объём, но не расширить: шире пяти накопителей предмета нет.
