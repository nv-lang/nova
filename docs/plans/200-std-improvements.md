...
**Статус:** 📋 СОГЛАСОВАНО-направление 2026-07-16 (владелец: «есть же items.clone()»). НЕ слепой снос — у `from`
ДВЕ роли (core.nv:210, маркер `[M-lint-findings-static-conversion]` уже висит):
1. **same-T копия** — чистый дубль `items.clone()` (D9) → ретракт очевиден;
2. **типо-направленная конструкция** — `Vec[f32].from([1, 2])` (целевой тип типизирует литерал) и element-width
   конверсия `Vec[u8].from(int-vec)` (NOTE в коде) — `clone()` это НЕ покрывает.

**Карта:** инвентаризация 95 call-сайтов (nova_tests заморожен — не мигрировать); same-T → `.clone()`;
литеральные → typed-binding `ro v []f32 = [1.5, 2.5]` (сверить, что D55/контекст покрывает и expression-position);
width-конверсия → явная (map/`as`-элементы) или осознанный остаток. Ретракт декла — ПОСЛЕДНИМ шагом, когда роль-2
развязана. Закрывает `[M-lint-findings-static-conversion]`-часть про Vec.from.
