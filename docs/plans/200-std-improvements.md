...
**Статус:** ⚠️ ПЕРЕОТКРЫТ 2026-09-15 — консолидация НЕ ЗАКРЫЛА КЛАСС: нашлась ПЯТАЯ копия, и она не в `.nv`. Владелец, увидев `extern "nova" fn str.from_codepoint(cp int) -> str` (`std/src/runtime/char.nv:36`): «такой функции через extern "nova" быть не должно, у нас куча подобных функций есть на нова». **ПОЧЕМУ ЭТО НЕ НОВЫЙ ПУНКТ, А ЭТОТ:** предмет тот же — лестница `cp < 0x80 / 0x800 / 0x10000`, — и та же работа уже свела её в `char @encode_utf8()` (`runtime/defaults.nv:112`, живые потребители: `string_builder.nv:135,340,366`, `write_buffer.nv:111`). **ПОЧЕМУ КОПИЮ НЕ УВИДЕЛИ, и это дороже самой копии:** считали копии В NOVA-ИСХОДНИКЕ, а пятая подпёрта C (`Nova_str_static_from_codepoint`, `emit_c.rs:43585`, алиас в `string_builder.h:105`). Мерка перечисляла РЕАЛИЗАЦИИ НА ЯЗЫКЕ вместо всех реализаций лестницы — тот же класс «проверка не мерит свой предмет», что и весь сентябрьский разбор. **ЗАМЕР ОКНА b5, сверить при взятии:** вызовов `str.from_codepoint(` в `std/src` НЕТ НИ ОДНОГО (только упоминания в комментариях), а комментарий `char.nv:35` обещает потребителей-декодеров, которых амендмент D54 уже перевёл на `@to_char()`; снаружи std зовут `spec_tests/conformance/from_codepoint_test.nv` и `from_codepoint_invalid_neg.nv`. **ОБЪЁМ:** перенести на Nova поверх `@encode_utf8()`, снять extern-объявление, поправить лживый комментарий. **ГРАНИЦА, ЧТОБЫ ПОРУЧЕНИЕ НЕ ПРОЧЛИ ШИРОКО:** `extern "nova"` в std 456 штук, из них 340 в `sync.nv` и 54 в `math.nv` — атомики, барьеры, GC, фиберы; они законны, потому что ВВОДЯТ возможность, которой у языка нет. Незаконен хук, который ДУБЛИРУЕТ то, что язык уже умеет, — и критерий именно такой. Прежний статус: ✅ СДЕЛАНО 2026-07-19 (worktree `nova-p20018`, ветка `p200-18-utf8`, слияние
...
**Статус:** ✅ КОМПОЗИТ СДЕЛАН 2026-07-21 (sonnet-агент прерван перезапуском окна, добит интегратором; civil PASS 1/0, duration/checksums δ0; D321-амендмент тем же слиянием). `Date @at(h,...)` ОТЛОЖЕН — стрельнула ловушка №2: `[M-concrete-instance-arity-overload-mangle]` (один C-символ на инстанс-перегрузки конкретного типа); вернуть после фикса. Вторая находка исполнения: `[M-static-multi-overload-chain-call-type]` (chain на мульти-перегрузочном статике берёт не тот тип; обход — биндинг). Мотив: `datetime(2026, 6, 8, 10, 0, tzinfo=MSK)` у Python — один
...
## Пункт 22 — канонические итератор/диапазон-идиомы: `.collect()` / `[a..]` вместо ручных форм

**Статус:** 📋 РЕЕСТР (наблюдения владельца 2026-07-24). **Модель:** sonnet (миграция) — линт-часть
компиляторная (см. ниже, вне file-disjoint-объёма 200; здесь — std-миграция + канон в конвенции).

**Что:** std/nova-http дрейфует от канонических итератор/диапазон-идиом к многословным ручным
формам. Каждая подформа: (а) канон короче и проверенно-эквивалентен; (б) std-миграция сайтов;
(в) компиляторный линт-энфорсер `W_MANUAL_*` (маркер, НЕ в объёме 200 — линкуется); (г) канон
закрепить в `nv-coding-style` (семья §29/§30 «ручная форма vs канон»).

| Подформа | Ручная форма (дрейф) | Канон | Сайтов | Линт-маркер |
|---|---|---|---|---|
| **collect** | `mut v = T.new(); for x in it { v.push(x) }` | `mut v = it.collect()` | 37 в 23 файлах | `[M-manual-collect-lint-missing]` (`W_MANUAL_COLLECT`) |
| **slice-to-end** | `s[a..s.byte_len()]` / `v[a..v.len()]` · `x[0..b]` | `s[a..]` · `x[..b]` · `[..]` | 37 + ~96 | `[M-manual-slice-bounds-lint-missing]` (`W_MANUAL_SLICE_TO_END`) |
| **coalesce** ✅ | `match X { Ok(v)=>v, Err(_)=>D }` | `X ?? D` | 37 мигрировано | `W_MANUAL_COALESCE` ВЛИТ (Haiku-волна), остаток `[M-manual-coalesce-corpus-remainder]` (121) |

**Проверено пробами (бинарь `f7f99cec2`):** `scheme.chars().collect()` компилируется без аннотации
(тип `[]char` выводится) и равен ручному циклу; `s[2..] == s[2..s.byte_len()]`; `X ?? D` — уже канон
(D86). Условия линтов БЕЗ ложных срабатываний — в маркерах (пустой ctor + тело ровно `push(loop_var)`;
end — голый `len()`/`byte_len()` на том же receiver; identity-рука match).

**Порядок (каждая подформа — свой коммит, НЕ мега):** (1) canon-строка в `nv-coding-style` (семья
§30); (2) компиляторный линт по маркеру (агент, гейт интегратора — вне 200); (3) std-миграция сайтов
пофайлово. coalesce уже на шаге 3 (остаток-маркер). Расширения collect (`.map(f).collect()` /
`.filter(c).collect()`) — позже, не в первую версию.

**Спека/конвенция:** канон-идиомы — правка `nv-coding-style` (согласовано наблюдением владельца
2026-07-24), не язык-меняющее. **Приёмка (на подформу):** линт даёт ожидаемое число срабатываний
(0 ложных); мигрированные сайты `nova test` затронутых пакетов δ0; байт-паритет НЕ требуется
(семантика-эквивалентная замена, но C может отличаться — проверять поведенчески).

---

## Пункт 23 — `@split_ascii_whitespace()` и семья: ленивый итератор вместо жадного `[]str`

**Статус:** 📋 ЗАПЛАНИРОВАН 2026-09-15 по слову владельца: «это должен быть интератор»
(о `std/src/runtime/string/search.nv:169`).

> **В РАБОТЕ с 2026-09-15, окно `p200-23` (дерево `nova-p200`, ветка `p200-23-ws-iter`),
> назначено интегратором — ТОЛЬКО ПЕРВЫЙ МЕТОД `@split_ascii_whitespace`.** Остальные
> четыре свободны и пойдут по образцу, который даст этот. Написано: тип
> `AsciiWhitespaceIter` + `#impl(Next[str])` + адаптеры (`vec_iter/core.nv`, рядом со
> `SplitIter`), конструктор в одну строку (`search.nv`), амендмент D251 (`03-syntax.md`)
> — прежний амендмент 2026-07-11 ЯВНО оставлял этот метод эагерным, без его отмены
> слияние было бы язык-меняющим без спеки, — и 7 фикстур в `vec_iter/core_test.nv`.
> **ПРОГНАНО 2026-09-15, 16:16–16:49** (слот отдан интегратором в 16:15):
> `nova test std` → PASS 78 FAIL 0; `nova test spec_tests` → 8 отказов, ВСЕ в
> `spec_tests/soundness/` с `E_D78_MODULE_PATH_MISMATCH`, и они НЕ мои — те же
> восемь падают на ЧИСТОМ дереве моей базы (`271b8eb24`, отдельный worktree,
> контроль прогнан 16:46–16:49); `nova lint --deny std/src` → 292 файла, 0 находок.
> **Проба в обе стороны сделана:** сузил байтовый класс (убрал `\x0B`) — покраснела
> РОВНО фикстура на границу класса, остальные шесть держались; вернул — снова зелено.
> **Цикл модулей резолвер ПРИНЯЛ:** `vec_iter` импортирует `is_ascii_ws` из
> `runtime.string`, который импортирует типы из `vec_iter`; утверждение шапки
> `core.nv` о легальности циклов до сих пор ни разу не проверялось — теперь проверено,
> план Б (копия предиката) не понадобился.
>
> **ЦЕНА ЛЕНИ ИЗМЕРЕНА — фаза оправдана.** Вопрос ставился так: платит ли ленивость
> потребителю, которому нужны ПЕРВЫЕ ДВА слова длинной строки. Замер (2000 слов,
> 50 итераций, clang `--mode release`, три прогона):
>
> | форма | прогон 1 | прогон 2 | прогон 3 |
> |---|---|---|---|
> | ленивая `.take(2).collect()` | 339 мкс | 23 мкс | 20 мкс |
> | жадная (весь `Vec`, затем два слова) | 16559 мкс | 25311 мкс | 23162 мкс |
> | ленивый `for`-in по ВСЕМ словам | 2744 мкс | 3638 мкс | 4383 мкс |
>
> Ленивая форма дешевле жадной **в 49–1150 раз** на первых двух словах (первый прогон
> — прогрев, дальше разрыв растёт). Второй ряд важен не меньше: когда слова нужны ВСЕ,
> но материализовать их не надо, ленивый обход дешевле жадного сбора **в 5–7 раз** —
> то есть лень не только экономит на `take`, но и снимает построение `Vec` там, где
> он не нужен. **Замер сделан прямой программой, а не через `nova bench`:**
> `bench/micro/strings.nv` не собирается вообще (внутренняя ошибка компилятора), и это
> НЕ моя правка — доказано контролем: содержимое с `main` по тому же пути падает так
> же. Заведено отдельной строкой реестра. Бенчи на этот метод в `bench/micro/strings.nv`
> дописаны и лягут в дерево готовыми к работе, как только тот дефект починят.
> **Потребители вне std, замер ДО правки: 1 файл, 3 вызова** —
> `spec_tests/conformance/d251_str_surface.nv:78,81,82` (в `examples/` и `docs/guide/`
> ноль; пакеты — отдельные репозитории, план их не трогает). После правки те же три,
> каждый с `.collect()`.
>
> **Найдено по пути, заведено строкой реестра **№1111** (221.1, номер выдан интегратором 2026-09-15):** в std ДВА
> предиката «ascii whitespace», расходящихся ровно по `\x0B` — `is_ascii_ws`
> (`search.nv:101`, включает) против `char @is_ascii_whitespace()` (`defaults.nv:79`,
> исключает ради паритета с Rust). Перенос взял ИСХОДНЫЙ байтовый класс дословно,
> ни один из двух предикатов не тронут: решение «свести или различить» — владельца.

**ПРЕДМЕТ — СЕМЕЙСТВО, И ГРАНИЦА МЕЖДУ СЛОВОМ ВЛАДЕЛЬЦА И МОИМ РАСШИРЕНИЕМ ПРОВЕДЕНА ЯВНО.**
Владелец назвал ПОИМЁННО ДВЕ: `@split_ascii_whitespace()` (`search.nv:169`) и `@lines()`
(`search.nv:186`) — обе фразой «это должен быть итератор», двумя указаниями подряд.
**Остальные три (`@match_indices`, `@matches`, `@to_code_points`) — РАСШИРЕНИЕ ИНТЕГРАТОРА,**
по правилу «чинится КЛАСС, а не носитель»: они тот же накопитель в том же семействе, и
починка двух названных при трёх одинаковых рядом — ровно та починка носителя, которую
оговорка реестра запрещает. **Расширение помечено, а не растворено:** если владелец скажет
«только две» — три снимаются одной правкой, и порядок фазы этого не ломает, потому что
названные им идут первыми.

**Расхождение внутри ОДНОГО файла, и ровно по этой оси.** Ленивое уже сделано:
`@split(sep) -> SplitIter` (`search.nv:96`), `@splitn` (`:107`), `@rsplit -> RSplitIter` (`:113`),
`@rsplitn` (`:118`), `@split_terminator` (`:163`), плюс `@chars`/`@iter` (`chars.nv:118/124`,
`CharsIter`); типы в `collections/vec_iter/core.nv:379` и `:470`.

**Жадные — пять, все настоящие накопители `out.push(...)`** (замер окна b5, сверить при взятии):

| метод | место | чего требует итератор |
|---|---|---|
| `@split_ascii_whitespace() -> ro []str` | `search.nv:169` | класс байтов, а не подстрока (назван владельцем прямо) |
| `@lines() -> ro []str` | `search.nv:186` | терминатор строки |
| `@match_indices(needle str) -> []int` | `search.nv:209` | позиции |
| `@matches(needle str) -> ro []str` | `search.nv:262` | совпадения |
| `@to_code_points() -> []int` | `chars.nv:129` | кодпоинты |

**`@bytes()` (`core.nv:76`) В РАЗРЯД НЕ ВХОДИТ, и это не оговорка, а различение:** он не копит, а
возвращает ВИД на ту же память (`unsafe { []u8.new(@ptr, @byte_len()) }`). Лень к нему неприменима
— лениться нечему. Счёт «шесть жадных методов» был бы счётом по ПОДПИСИ (`-> ro []…`) вместо
предмета (накопление), и дал бы шестую работу, которой нет.

**Чего не хватает, и это не подпись:** у `SplitIter` разделитель — ПОДСТРОКА; каждому из пяти нужен
СВОЙ тип итератора, иначе форма не выражается. Это и есть объём фазы.

**Чего не хватает, и это не подпись:** у `SplitIter` разделитель — ПОДСТРОКА, а здесь нужен
класс байтов («пропусти пробелы, отдай слово»). Значит нужен свой тип итератора рядом с
`SplitIter`, иначе форма не выражается.

**Цена перехода измерена окном b5:** в `std/src` вызовов `split_ascii_whitespace` НЕТ НИ ОДНОГО —
внутри std переход бесплатен. Снаружи (spec_tests, пакеты) не мерено; первая работа — померить.

**Не делаем в этом пункте:** не трогаем `@bytes()` (см. различение выше) и не заводим линт —
энфорсер формы это пункт 22, у него свой объём и свои маркеры. И не переносим правку на
пакеты: план про std.

**ПОРЯДОК ВНУТРИ ФАЗЫ ДИКТУЕТСЯ ЦЕНОЙ, А НЕ УДОБСТВОМ:** первым идёт
`split_ascii_whitespace` — у него в std НЕТ НИ ОДНОГО вызова, то есть переход бесплатен и
служит образцом для остальных четырёх. `@lines()` берётся ПОСЛЕ него: у него живой
потребитель (`std/src/io/core.nv:224`), и на нём впервые проверяется, что `.collect()` у
вызывающего не портит читаемость.

**Критерии (машинные):** фикстура, где итератор потребляется лениво и даёт те же слова, что
нынешний массив (посимвольное сравнение, а не длина); фикстура на пустую строку и на строку из
одних пробелов — края, где жадная и ленивая формы расходятся чаще всего; `nova test std spec_tests`
зелёный; потребители снаружи std перечислены ЧИСЛОМ до правки и после.

**Открытые вопросы:** входят ли три НЕ названные владельцем (`@match_indices`, `@matches`,
`@to_code_points`) — адресат владелец; умолчание «входят», потому что класс один и
починка носителя приёмкой не считается; срок — к началу работы. Вопрос закрыт по первому
указанию: предмет второго назван прямо — `@lines()`.
