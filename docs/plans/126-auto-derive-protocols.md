...
| `Hash` | `@hash() -> u64` | same |
| `Equal` | `@equal(other Self) -> bool` | same (TBD method name verification) |
| `Compare` | `@compare(other Self) -> int` | same |
| `Display` | `@display(sb StringBuilder) -> ()` | std/prelude/protocols.nv:313 |
...
// fn Vec3 @display(sb StringBuilder) -> () { ... memberwise format ... }
...
```nova
#impl(Eq+Hash+Clone+Compare+Display)   // short form via Eq alias
type X value { ... }

#impl(Equal + Hash + Clone)             // long form
type X { ... }
```
...
- Protocol is **known built-in** (Equal / Hash / Clone / Compare / Display).
...
```
type Outer value {
    inner Inner    // ← Inner НЕ #impl(Equal)
}
#impl(Equal) type Outer { ... }   
// ❌ E_AUTO_DERIVE_FIELD_LACKS_PROTOCOL: 
//    field `inner` (type `Inner`) does not implement `Equal`
```

### 2.5 Heap-record auto-eq (D109 amend)

Currently heap-records `==` is identity comparison. После Plan 126:
- `#impl(Equal)` на heap-record → auto-derive memberwise eq (overrides identity).
- Без `#impl(Equal)` → identity default (unchanged).

Это **обратно совместимо** — old code без `#impl(Equal)` работает identity-eq как раньше.
...
- `type A value { b B }; type B value { a A }` + `#impl(Clone)` → 
  `E_AUTO_DERIVE_CYCLE`.
...
- Verify Equal method name (`@equal` vs `@eq`).
- Audit existing manual implementations (StringBuilder, etc.) для conflict-check.
- Cycle detection design.
- **Эстимат:** 0.3 dev-day.

### Ф.1 — Clone protocol declaration
- Add `Clone` к `std/prelude/protocols.nv`.
...
- **A126.1** ✅ `Clone` protocol declared в std/prelude/protocols.nv.
- **A126.2** ✅ `#impl(Equal)` synthesize'ит `@equal` для value-record.
- **A126.3** ✅ `#impl(Hash)` synthesize'ит `@hash` для value-record.
- **A126.4** ✅ `#impl(Clone)` synthesize'ит deep `@clone`.
- **A126.5** ✅ `#impl(Compare)` synthesize'ит lexicographic `@compare`.
- **A126.6** ✅ `#impl(Display)` synthesize'ит memberwise `@display`.
...
- **A126.11** ✅ Heap-record `#impl(Equal)` overrides identity-eq.
- **A126.12** ✅ Tuple `#impl(Equal)` works (Plan 120 + 124.8).
...
- **Q126.1** — `Equal.@equal` vs `@eq` — verify в Ф.0 что точно в спеке.
