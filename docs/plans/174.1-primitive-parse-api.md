# Plan 174.1 — Primitive parse API (One-Engine, radix-only `parse`)

**Статус:** ✅ ПОВЕРХНОСТЬ SHIPPED 2026-07-08 (ветка `parse-174-1`) — итоговый канон ОТЛИЧАЕТСЯ от черновика ниже, см. §0-ИТОГ. Остаток (codegen-хардкод `T.try_from(str)`, float-канон) — `[M-174.1-parse-engine-structural]`.
...
## §0-ИТОГ (2026-07-08, решение владельца в ходе исполнения; supersedes §2-§4 ниже)

Канон: **конверсия — метод НА ИСТОЧНИКЕ**, не статик-конструктор на целевом типе
(две итерации за день: сперва `T.parse(s)` static, затем owner-amend → `s.to_T()`).

- **Числа:** `str @to_int(radix=10)` / `@to_i64` / `@to_u64` / `@to_u32` / `@to_u8`
  (все → `Result[T, ParseIntError]`; u32/u8 range-checked — закрытие truncation-класса
  на unsigned-стороне) + `str @to_f64() -> Result[f64, ParseFloatError]`.
  Живой набор по фактическим потребителям, НЕ полный SignedInt/UnsignedInt type-set:
  `fn[T SignedInt] T.parse(...)` (вариант B, D310) упёрся в живой codegen-гэп
  path-call на type-set-ресивере (`[P67-LEGACY] Path call return type unknown`).
  `str.@parse_int` (D177/D178/D325-триада) РЕТРАКТИРОВАН из публичного API;
  движки `@parse_int_core`/`@parse_uint_core` (u64-домен, `-`⇒InvalidDigit) —
  приватные в `std/runtime/string/parse.nv`.
- **Байты:** `str.from_bytes`/`from_bytes_lossy`/`from_bytes_unchecked`(+steal)
  РЕТРАКТИРОВАНЫ → `[]u8 @to_str()` (Result[str, Utf8Error]) / `@to_str_lossy()` /
  unsafe `@to_str_unchecked()` / unsafe consume `@into_str_unchecked()`.
  Плюс `@decode_utf8()` — same-body алиас to_str для CU с коллизией имён
  (`[M-174.1-to-str-name-collision-codegen-bug]`).
- **char:** `char @to_str()` (extern-движок `str.from(char)` остаётся внутренней
  опорой интерполяции и Display-default).
- **Домены:** `str @to_complex()` (ретракт `Complex.try_from(s)`), `str @to_version()`
  (ретракт `Version.from(s)`), `str @to_url()` (ретракт `Url.parse(s)`, std/http),
  `str @to_cron()` (ретракт `Cron.parse(expr)`).
- **Компиляторные фиксы той же волной** (все три дефекта до-существующие, вскрыты
  гейтами): чекер-резолв конкретно-слайсовых receiver-методов (`"[]u8"`-ключ vs
  `"Vec"`-нормализация → P67-ICE в fn-main CU), DCE-якорение `[]`-receiver-методов
  (type∧name-пересечение не могло сработать), interp-DCE seed-лист (протухший
  `as_str` вместо `into_str` ронял ЛЮБОЙ fn-main CU с `${…}`; ревью владельца:
  мёртвые имена убраны, маркер `[M-dce-seed-name-list]`).
- **Гейты:** conformance 70/0 (834/834) после каждой из трёх волн; `nova check std`
  116/20 байт-в-байт бейзлайн; targeted std (http 8/0, math/data/time/encoding 11/1 —
  единственный FAIL = `_experimental/semver_range` c до-существующим нерезолвируемым
  `import semver`); cargo test --lib 905/912 (7 FAIL до-существующие, проверено на
  baseline-worktree). Фикстура conformance `plan174_1_typed_numeric_parse.nv`
  (7 test-блоков: границы max/min, overflow→Err, пустая строка, мусор, знак,
  radix, no-trim, unsigned `-`⇒InvalidDigit).

---

> ⚠️ **Выровнен под Plan 177 / D325 (2026-06-25).** Действует единое правило «вся падающая публичная std → `Result`» (D325, R1-R5). Конкретно для 174.1: **(1)** radix-форма = **`T.parse(s, radix)` → Result** (без `try_`-префикса: нет инфаллибл-сиблинга `parse`, R2/R3). **(2)** Бросковой `T.from(s)`/`T.into()` **НЕ генерится** — D77 amend 4-way→2-way (остаются `try_from`/`try_into`, оба Result); throw на call-site = `try_from(s)!!`. **(3)** str-движок схлопывается в один `@parse_int → Result`; bare `@parse_int`(Fail) + `@parse_int_opt`(Option) ретрактированы (D178-retract). **Сам rename/удаление SHIPPED-форм = Plan 177 Ф.2b (compiler-gated, emit_c.rs).** D309 подчинён D325: D325 = канон нейминга, D309/174.1 = примитив-специфика (radix-движок + range-check).
...
// D325: один Result-движок на домен; bare(Fail)+_opt(Option) РЕТРАКТИРОВАНЫ (D178-retract). Целевые имена ниже; rename/удаление SHIPPED-форм = Plan 177 Ф.2b (compiler-gated, emit_c.rs).
...
## 3. Разрешение спеки D74 ↔ D77 (под D325 / Plan 177)
...
- **D309 (предв.) — ПОДЧИНЁН D325:** D325 (Plan 177) задаёт канон нейминга (R1-R5: всё падающее → Result, `try_` только для `from`/`try_from`). D309/174.1 — **применение к примитивам**: «radix-free decimal ⇒ `try_from` (Option через `.ok()`, throw через `!!`); radix-bearing integer ⇒ `parse(s, radix)`; float/bool radix не имеют ⇒ `parse` нет». Инвариант: вся логика в str-движках (один Result-движок), type-level — тонкие делегаты. Per-type обёртки — **interim** (схлопнутся после Plan 172.3).
- **D178 — RETRACT (D325):** bare `parse_int`(Fail) + `parse_int_opt`(Option) удаляются; одна форма `@parse_int → Result` (rename = Plan 177 Ф.2b). **D73 / D25 — UPHELD** без изменений.
...
## 7.7. Оценка остатка (2026-07-06) — что поглощено 177, что живо, что сделано

**Grep-факты на входе (реальное состояние, не план):**

| Компонент | Статус | Факт |
|---|---|---|
| `str.@parse_int → Result[int, ParseIntError]` (radix 2..=36, i64-overflow guard) | ✅ **ПОГЛОЩЕНО 177 Ф.2b** | `std/runtime/string/parse.nv` — единственная форма; bare/`_opt`/`try_`-твины ретрактированы (D178/D325). radix-параметр `radix int = 10` уже есть. |
| `str.@parse_uint` (u64-домен, `-`⇒InvalidDigit) | 🔴 **ЖИВ** (не создан) | в parse.nv нет; нужен для unsigned type-level обёрток. |
| `std/runtime/parse_prim.nv` (type-level try_from/parse) | 🔴 **ЖИВ** (не создан) | файла нет; `T.try_from(s)`/`T.parse(s,radix)` идут через codegen-хардкод, НЕ через .nv. |
| codegen-хардкод `T.try_from`/`T.try_parse` | 🔴 **ЖИВ** | `emit_c.rs` `try_parse`-ветка (Option, факт. `:~29234`) + `try_from`-ветка (Result, `:~29290`) + mirror type-infer (`:~40390`). План указывал `:28006/:28061/:40131` — **фактические символы сместились**. |
| **Баг truncation** (`i8.try_from("999")`→`Ok(-25)`) | ✅ **СДЕЛАНО (этот заход)** | range-check `emit_parse_range_check` (min/max sub-width) в ОБЕ ветки хардкода перед narrowing-кастом → out-of-range = Err/None. Тесты `nova_tests/runtime/plan174_1_parse_truncation.nv` (i8/i16/i32/u8/u16/u32 границы + full-width не задет). |
| Typed error у `T.try_from` (вместо flat-string) | 🔴 **ЖИВ** | хардкод `try_from` эмитит `Err((nova_str){flat "T.try_from: parse error"})` (`:~29350`), НЕ typed `ParseIntError` — требует routing в Nova-движок (структурный fix). |
| `T.parse(s, radix)` primitive-поверхность (range-checked hex/oct) | 🔴 **ЖИВ** (не создан) | нет `parts[1]=="parse"`-ветки для примитивов; radix только у str-движка. |
| Float-канон (§4.1: no-trim pre-check, locale-независимость, f32 strtof, full-consume, inf/nan) | 🔴 **ЖИВ** | хардкод f64/f32 = `nova_str_to_f64` (strtod, триммит, locale-зависим). |
| `char.try_from(s)` + `ParseCharError` | 🔴 **ЖИВ** | хардкод `nova_str_to_char`; typed error нет. |

**Вариант B (Ред. 2, 172.3 ✅ CLOSED — D310 type-set bounds доступны):** целевой движок =
generic `fn[T SignedInt] T.try_from(s)` / `T.parse(s, radix)` с `T.MIN/T.MAX` в generic-теле
(~22 per-type → ~6-8; `SignedInt`/`UnsignedInt` в prelude `protocols.nv`; unsigned MIN-строки D310
уже добавлены в `numeric_type_constant_mapping` `emit_c.rs:37970-37974`). Догма «T.MAX в generic не
резолвится» опровергнута 172.3 §5. Per-type остаются только f64/f32/bool/char.

**Сделано этим заходом (concrete correctness win, gateable):**
- **Truncation-фикс** `emit_parse_range_check` (`emit_c.rs`) в обеих хардкод-ветках (try_from/Result +
  try_parse/Option). Sub-width i8/i16/i32 (signed, оба бортика) + u8/u16/u32 (upper-bound); full-width
  int/i64/uint/u64 не задет (движковый i64/u64 overflow-guard покрывает). Тесты POS (20 кейсов).

**ОТЛОЖЕНО (живой остаток — крупный, координируемый; НЕ упрощение, а честный defer):** полный
структурный вариант-B движок = `str.@parse_uint` + `parse_prim.nv` (generic try_from/parse) +
**удаление codegen-хардкода** (routing в Nova-движок → typed `ParseIntError`/`ParseFloatError`/
`ParseBoolError`/`ParseCharError` вместо flat-string) + `T.parse(s,radix)`-поверхность + float-канон +
миграция `json.nv:464`. Причина defer: (1) зона хардкода `emit_c.rs` parse — под **тройной
координацией 172.1 (hardcode-зона D-трек) × 177 Ф.2b (SHIPPED-имена)** (зонт §2/§3.1); удаление
хардкода + generic-резолв T.MIN/T.MAX в Nova-теле — крупная связанная правка, риск конфликта с
конкурентным D-треком 172.1. (2) Требует нового str-движка + per-type/generic .nv + migration +
float-канон одним изменением, чтобы не оставить «две двери в десятичное» (D9). Truncation-баг
(named acceptance) закрыт СЕЙЧАС в существующей архитектуре как изолированный корректностный фикс.
Маркеры: `[M-174.1-parse-api]` (OPEN), новый `[M-174.1-parse-engine-structural]` (generic-движок +
хардкод-removal + typed-errors + float-канон + radix-поверхность).
...
- **Plan 177 / D325** — задаёт нейминг-канон (вся падающая std → Result); 174.1 = его per-type реализация для примитивов (radix-движок + range-check). См. баннер вверху.
