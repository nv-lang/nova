...
# Обратные кавычки ВНУТРИ ДВОЙНЫХ оболочка ИСПОЛНЯЕТ: здесь она пыталась
# запустить команду `Node`, и в выводе пробы стояло «Node: command not found».
# Тот же класс, что строка реестра 1190 («описания наших собственных гейтов
# исполнялись оболочкой»), и мой ВТОРОЙ случай за смену: первый был `SerError`
# в соседней строке, и, починив его, я не проверил остальные — починка
# НОСИТЕЛЯ вместо класса, ровно то, что запрещает правило.
echo "   грузятся ВМЕСТЕ, и молчание про два имени Node есть ответ, а не слепота."
