# Plan 174.4 — Effect-registry: compile-time размер вместо хардкода 32

> **Top-level план.** Создан 2026-06-20. **Статус:** ✅ Ф.1 DONE 2026-07-04 (Ф.2 static-indices — follow-up).
> **Маркер:** `[M-174.4-effect-registry-size]`.
> **Запуск:** «**выполни план 174.4**» (самодостаточен). **Приоритет:** P2 (латентный correctness + память).
...
`[M-174.4-effect-registry-size]` (Ф.1 размер; Ф.2 статические индексы + удаление рантайм-registry).

## 9. Реализация Ф.1 (2026-07-04)

**Что сделано** (compile-time точный размер + устранение silent-drop):

1. `compiler-codegen/nova_rt/effects.h` — `#define NOVA_MAX_EFFECT_STORAGES 32` обёрнут в
   `#ifndef … #endif` (32 = fallback для hand-written / bootstrap-хедеров). `nova_register_effect_storage`:
   silent-drop заменён на `fprintf(stderr, …) + abort()` при переполнении (теперь = индикатор бага codegen'а,
   т.к. размер точный). Комментарии про «dynamic-resize / Production» убраны.
2. `compiler-codegen/src/codegen/emit_c.rs` — в преамбулу (строка 1 сгенерированного `.c`) эмитится маркер
   `/* nova-effect-count: N */`, где `N = effect_schemas.len().max(1)` (built-in Fail/Time/Mem + user;
   splice в finalize). `.max(1)` закрывает edge `N==0` (C запрещает `[0]`; на практике built-in гарантируют
   N≥3). **Не** эмитим `#define` в тело `.c` — см. п.3 (ABI).
3. `compiler-codegen/src/test_runner.rs` — `effect_count_define_arg()` читает маркер из `.c` (строка 1) и
   прокидывает `-DNOVA_MAX_EFFECT_STORAGES=N` (prefix `-D`/`/D`) во ВСЕ три toolchain-ветки (clang/MSVC/gcc).
   **Ключевой ABI-момент (§7 blast-radius):** сгенерированный `.c` и runtime `effects.c`/`runtime.c`/`fibers.c`
   компилируются как **отдельные TU в одном cc-вызове**; `#define` только в `.c` дал бы `NovaEffectSnapshot`/
   `NovaEffectRegistry` разного размера в разных TU → OOB-запись в 32-словный TLS-registry → segfault
   (обнаружено и исправлено при разработке). `-D` на весь вызов → все TU согласованы; при отсутствии маркера —
   единый fallback 32 из `#ifndef`.

**Проверка:** pos-тест `nova_tests/effect_registry/registry_size.nv` (40 user-эффектов + built-in = 45; registry
44 записи > 32) — 2 теста PASS (spawn наследует handler'ы всех 40 эффектов; outer-handler высокоиндексного
Eff40 не затирается завершившимися фиберами). Conformance `--positive --compile-error` = 38/0. Broad-regression
sample (basics/atomics/gc/contracts/plan110/generics/…) идентичен parent-бинарю f55dd55b (PASS 11 / FAIL 4 —
все 4 fail'а pre-existing branch-breakage: retired `str.len()`, undeclared `apply`, `E_TRY_IN_FAIL_FN`).
Spec: Q-note в [04-effects.md D11](../../spec/decisions/04-effects.md).
