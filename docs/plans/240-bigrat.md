...
**Статус:** 🔨 В РАБОТЕ (отдан 2026-08-02 по слову владельца «отдавай в работу»; исполнитель — opencode/big-pickle, ветка p240 в nova-bigint). Открытые вопросы §6 закрыты рекомендациями плана как решениями (владелец отдал план в работу без правок §6).
...
- **Ф.0 Пины (внутри той же волны, ДО ядра):** (а) value-record с двумя BigInt-полями —
  мини-фикстура компилябельности; (а2, добавлен 2026-08-02) статик-конструкторы
  (`BigRat.zero/one/new`) + instance-методы (`@equal`) на ОДНОМ value-record типе в
  пакетном CU — №271 `[M-static-selfreturn-value-mangle-conflict]` стрельнул на
  NAMED-TUPLE форме BigInt тем же днём; на value-record 236/237 прецедентно зелено,
  но пин обязателен (красный → СТОП-репорт, не обход); (б) операторный desugar `+ - *` на новом типе (дёшево,
...
| Смежные свежие блокеры семьи №271/№272 (2026-08-02, named-tuple ветка p-tuple) | 240 строится на value-record — вне их зоны; НЕ переводить BigRat на кортеж до закрытия №271/№272/№267; интерполяцию `"${rat}"` в тестах не использовать (только явный @to_str) |
