...
Все ниже — **КАНДИДАТЫ (идеи для реализации), НЕ согласованы**; порядок = убывание ценности.
...
(в) ДЕЛАЕМ СЕЙЧАС**: catch-panic-хук на границе соединения по прецеденту test-frame (НЕ user-facing
catch — D13/D325 в силе): паника хендлера → лог полной трассы + ответ 500 + закрытие соединения +
слот жив; ручка в ServerPolicy (дефолт 500; off = Go-стиль лог+обрыв). Состав: extern-примитив в
nova_rt + обёртка в serve_connection + D-амендмент (узаконить единственный хук, в том же слиянии) +
регресс «паника под нагрузкой → 500, слот вернулся, accept живёт». Компилятор-окно «catch-panic-hook»
— следующее за «net2stream+imports». **Требование к 222.7 (той же волной): admission НА
