...
> 1. **222.8 разделена** — (а) OpenAPI-генерация универсальными метаданными (JSON-формат в polaris/.nv) — К РЕАЛИЗАЦИИ; (б) route-верификация — ОТЛОЖЕНА за v0.1, http-хардкод в компилятор ЗАПРЕЩЁН; вернуться после решения про общий comptime-reflection facility.
