...
Источник: [docs/dev/research/http-frameworks-gap.md](../dev/research/http-frameworks-gap.md) — сравнение
...
### 222.8 — compile-time OpenAPI + верификация «маршрут ↔ хендлер» → **отдельный файл [222.8](222.8-openapi-gen.md)** 🔴 наибольшая ценность (дверь одна — см. РЕШЕНИЕ ниже)

**Ф.0-архитектура СОГЛАСОВАНА ПО СУЩЕСТВУ 2026-07-27** (итерации с владельцем) — см.
[222.8](222.8-openapi-gen.md): `#impl(Reflect)` → синтез `T.reflect() -> TypeShape`
(формато-независимое дерево формы типа, std) той же derive-машиной auto_derive.rs; бандл-extractors
одним record'ом (арность всегда 1 — Гэп №2 не задействуется); in-process `/openapi.json` с живого
Router'а (aide-модель), эмиттер пишет JSON напрямую из TypeShape. Четыре отвергнутых подхода
задокументированы в 222.8 §0 (call-site-атрибут, внешний AST-тул, json_schema-на-Serialize,
промежуточный JsonSchema-тип). **ТРЕБУЕТ D-амендмента** тем же слиянием, что Ф.1 (новый
auto-derive протокол, класс D435-прецедента); финализация имён/D-номера — 222.8 §6.
