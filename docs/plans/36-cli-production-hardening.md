...
### [36.D.1] nova test — multiple paths + display name от cwd ✅ ЗАКРЫТ 2026-06-18
...
5. `nova-codegen test-all` команда удалена из бинарника (была заглушка с устаревшим интерфейсом после изменения `TestAllOpts`).
...
- `nova test std nova_tests` — обе директории прогоняются одним вызовом ✅
- `nova test nova_tests/plan118` — конкретная поддиректория работает ✅
- `nova test std/collections/hashmap.nv` — один файл работает ✅
- display name содержит cwd-relative path (без искусственного `std/` префикса через флаги) ✅
- `--include-stdlib` удалён (clean break — exit 2 `unexpected argument`) ✅
- `nova-codegen test-all` удалена (−213 строк, сборка компилируется без ошибок) ✅
- **«Без упрощений как для прода»**: `nova test std nova_tests` со всеми тестами → 0 FAIL (verify full regression) ✅
- Tests: `nova_tests/plan36_d1/` с positive и negative тестами поведения multi-path CLI ✅

**Коммиты:**
- `f15b45bc` — feat(36.D.1): nova test multi-path + display name from cwd
- `8c3d7332` — chore: exclude std/_experimental/ from nova test discovery
- `c6aa5e7f` — chore(nova-codegen): remove stale test-all/test-build commands
