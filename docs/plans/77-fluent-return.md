...
>
> **🔄 ПЕРЕОТКРЫТ 2026-05-21 — Ф.8** (строгие дорожки `-> @`): исходная
> проверка `check_fluent_return` верифицирует только **хвост** тела —
> ранний `return <значение>` проходит мимо. Гарантия `-> @` остаётся
> неполной. Ф.8 закрывает дыру строгой проверкой «дорожек». См. раздел
> «Ф.8» в конце документа.
>
> **✅ ЗАКРЫТ 2026-05-21 (Ф.1–Ф.7) — выбран вариант B (`-> @`).** Реализовано:
...
## Ф.8 — строгие дорожки `-> @` (переоткрытие, 2026-05-21)

### Зачем

Реализованный `check_fluent_return` проверяет только **хвост** тела
`-> @`-метода (последнее выражение обязано быть `@`). Ранний `return`
с обычным значением **не проверяется** — дыра в гарантии:

```
fn Acc mut @job() -> @ {
    if @.full { return @.snapshot() }   // ← возвращает ДРУГОЙ объект,
    ...                                 //   а сигнатура обещает receiver
    @
}
```

Гарантия `-> @` («метод возвращает сам receiver, тот же объект»)
обязана быть полной — иначе sound alias-tracking в consume-checker
(Plan 73, builder-chain) держится на честном слове.

### Идея — две дорожки

Внутри `-> @`-метода каждая переменная принадлежит одной из двух
**дорожек**, и дорожки **никогда не пересекаются**:

- **Дорожка R (receiver)** — «сам receiver»: `@`, `let c = @`,
  переменная-алиас R, результат `-> @`-вызова на R-переменной.
- **Дорожка V (value)** — всё остальное (любое вычисленное значение).

Правила:
1. Каждый `return` **и** хвост тела обязаны быть дорожки R.
   `return <V>` → ошибка (ловит ранний return).
2. Для `mut`-переменной нельзя сменить дорожку: `b = @`, где `b` из V
   → ошибка (V := R); `c = a`, где `c` из R, `a` из V → ошибка
   (R := V).
3. Неизменяемые `let` повторно не присваиваются — уже ловится отдельно.

Пример из обсуждения (всё с `// error` теперь ловится компилятором):

```
fn Acc mut @job() -> @ {
    let a = compute()   // a — дорожка V
    let c = @           // c — дорожка R (алиас receiver'а)
    b = @               // error: b (V) := @ (R)
    c = a               // error: c (R) := a (V)
    return a            // error: return дорожки V
    @                   // ok — хвост дорожки R
}
```
...
Дорожка R = это **класс алиасов `@`**. Машинерия классов алиасов уже
есть в consume-checker'е (`ConsumeCtx.aliases`, canonical-name,
`dissolve_alias_class`). Ф.8 переиспользует её: «переменная в дорожке
R» ≡ «переменная в классе алиасов `SelfAccess`».

### Подзадачи

- **Ф.8.1 — разметка дорожек.** Пройти тело `-> @`-метода, для каждой
  переменной определить дорожку (R = `@` / `let x = @` / алиас R /
  результат `-> @`-вызова на R-переменной; V = остальное).
  Переиспользовать alias-class из consume-checker.
- **Ф.8.2 — проверка возврата.** Каждый `return` **и** хвост тела
  обязаны быть дорожки R. Закрывает дыру раннего `return <значение>`.
- **Ф.8.3 — запрет смены дорожки.** При присваивании `mut`-переменной
  дорожка левой части обязана совпадать с дорожкой правой
  (`V := R` и `R := V` → ошибка).
- **Ф.8.4 — spec + тесты.** Дописать в D132 раздел «строгие дорожки».
  Тесты: позитив (`return c`, где `c = @`; несколько `return @` в
  ветках), негатив (`return a`; `b = @`; `c = a`).

### Граница

Ф.8 работает только внутри тела `-> @`-методов. Обычные методы и
`-> Self` не затрагиваются. Внешние (external/runtime) `-> @`-методы
тела не имеют — проверка к ним неприменима (как и сейчас).

## Ссылки

- Plan 73 (`73-consume-qualifier.md`) — D131 `consume`, источник задачи.
- Plan 80 (`80-must-consume-linear.md`) — линейные значения; растёт из
  «Потенциала лучше Rust» этого документа.
