...
| indexed read | `p.at(i) -> T` | **Nova-сахар** (не Rust) `= p.offs(i).read()`; заменяет `p[i]` |
...
- **Ф.2 — недостающие методы:** `.offs`/`.wrapping_offs`/`.dist`/`.read`(any-T)/`.write`(any-T)/`.at`/`.set`/`.read_unaligned`/`.write_unaligned`/(`.copy_to`/`.copy_from`). Struct read/write — закрыть gap. (`.cast`-метод НЕ вводим — `as` остаётся.)
...
- **pos** `nova_tests/ptr177/`: `.read()/.write()` на примитиве И **struct/record/tuple/sum** (close gap);
  `.offs(n)` (на `*mut T` → `*mut unsafe T`)/`.at(i)`/`.set(i,v)`/`.dist(q)`/`p as *U`/`.read_unaligned()`/`.wrapping_offs(n)`; `*mut unsafe T` writable; `==`/`!=`; auto-deref `.`.
- **neg:** `*p`/`p+i`/`p[i]`/`p<q` → `[E_POINTER_OP_USE_METHOD]` (fix-it на метод); `.write()` на голом `*unsafe T`/`*T` → `E_POINTER_RO_ASSIGN`; `p[i]` (нет `@index` на `*T`) → понятная ошибка.
