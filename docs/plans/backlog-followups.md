...
| `[M-ro-launder-via-mut-binding]` | **OPEN 2026-07-23 (найдено владельцем: «параметры по умолчанию ПОЛНОСТЬЮ ro, это не про биндинг; параметр не может быть присвоен в mut-переменную» — проверено пробами, владелец прав, интегратор ошибался).** `mut b = a`, где `a` — ro-связанный источник (параметр по D176-дефолту ЛИБО явный `ro`-локал), **молча стирает и L1, и L2 ro** для кучевых типов, и запись через `b` **видна вызывающему/оригиналу**. D246 обязан ловить это как `E_READONLY_COERCE` (P8 «ro-content-источник → mut-content-цель»): по P7 «L1 dominates» ro-биндинг ДЕЛАЕТ содержимое ro, значит ro-связанное кучевое значение И ЕСТЬ ro-content-источник. Код ошибки СУЩЕСТВУЕТ и работает на типовом `ro T`, но к L1-ro источникам не применяется. **Это баг против ДЕЙСТВУЮЩЕЙ спеки, не язык-изменение** — D-амендмент не нужен (полезен лишь явный ORACLE-случай G). Матрица проб (свежий бинарь `6221c669b`): (A) `fn f(v []int) { mut w = v; w[0] = 99 }` → компилируется, у вызывающего `arr[0]` стал 99 — **ORACLE F3 обойдён одной строкой**; (B) прямая запись `v[0] = 99` в тот же параметр → корректно `E_READONLY_CONTENT` ✅ (F3 сам по себе жив); (D) то же на `ro`-ЛОКАЛЕ (`ro a = [1,2,3]; mut b = a; b[0] = 99`) → **ORACLE F1 обойдён идентично** — дыра ОБЩАЯ, не параметро-специфичная; (E) чистый value-record без кучевых полей → PASS, копия, вреда нет (единственная безопасная ветка); (F) `Vec[int]`-параметр → `mut w = v; w.push(...)` — у вызывающего Vec вырос; (G) **value-запись с кучевым полем — точная форма `ServerResponse { headers HeaderMap, body []u8 }`** → `mut resp = r; resp.header(...)` пишет в HeaderMap ВЫЗЫВАЮЩЕГО. **Следствие для кода:** паттерн `mut X = X0` (`nova-http/src/middleware/cors.nv` `decorate_simple`, `nova-http/src/client/client.nv:332-335`) держится на этой дыре; там аргумент — временное значение, наблюдаемого ущерба нет, но форма опасна и в общем случае течёт. **Объём фикса:** `mut b = a` с ro-связанным `a` кучевого типа → `E_READONLY_COERCE`; кандидатов `mut X = <идентификатор>` в `std/src` + `examples` + `nova-http/src` — **246** (надмножество: часть — value-типы, часть — не-ro источники). Behavior-changing → мега-CU conformance полным фильтром + флагман-examples. | чекер (`assignable`/binding-init, `types/mod.rs`; P8 не применяется к L1-ro источникам) | **P1 (звучность модели мутабельности)** |
| `[M-fn-newtype-return-position-broken]` | **OPEN 2026-07-23 (найдено интеграторской пробой ПОСЛЕ слияния `4c26fe2a0`; на бинаре, собранном из этого коммита).** Три куска fntypes закрыты для позиций *параметра* и *локального биндинга*, но **позиция ВОЗВРАТА (`-> Handler`) сломана**. Репро (5 строк, `type Handler fn(int) -> str` + `fn base(x int) -> str` + `fn make() -> Handler => base`): (а) `make()(3)` → **ICE** `[P67-LEGACY] Path call return type unknown (no parts) — checker must annotate` (`compiler-codegen/src/codegen/emit_c.rs:56229`); (б) `ro h Handler = make(); h(3)` → **CC-FAIL** `passing 'int' to parameter of incompatible type 'nova_str'` — C-типы аргумента и возврата ПЕРЕПУТАНЫ местами; явная аннотация биндинга НЕ спасает (проверено), т.е. дело не в выводе типа, а в том, что `resolve_fn_typeref` не применяется к результату вызова. Что РАБОТАЕТ (проверено тем же прогоном): `ro h Handler = base` + `h(7)`; `fn use_it(h Handler) -> str => h(3)` с передачей и замыкания, и именованной fn; `type Fmt1 alias fn(int) -> str` как параметр. **Блокирует напрямую:** [222.4](222.4-middleware.md) — вся форма middleware это `fn(next Handler) -> Handler`, т.е. fn-newtype В ПОЗИЦИИ ВОЗВРАТА; без этого куска плоская форма §4а не собирается. Также [222.3 §5а](222.3-extractors.md) (снос `handler_fn`) — любая фабрика, возвращающая `Handler`. | codegen (`resolve_fn_typeref` не покрывает результат вызова) + чекер (аннотация Path-call) | **P1 (блокер 222.4)** |
