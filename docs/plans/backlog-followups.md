...
| `[M-d78-duplicate-decl-module-swallow]` | **OPEN 2026-07-13 (research module-naming, эксперимент).** Реестр модулей керит по ДЕКЛАРАЦИИ (не по каноническому пути): два модуля с одинаковой rev-3-декларацией (`a/neg/x.nv` + `b/neg/x.nv` → оба ПРИНУДИТЕЛЬНО `module neg.x` — иначе E_D78_MODULE_PATH_MISMATCH) в одном пакете → импорт по пути находит оба файла, но экспорты ВТОРОГО тихо исчезают («undefined identifier» с якорем на строку module-decl, БЕЗ duplicate-diagnostic). Роутинг импортов путевой — глотает именно реестр. Корпус спасается дисциплиной (neg/rt-поддеревья: файлы пока разноимённы). Fix-направления (решение владельца, D78-тред): (1) керить реестр по каноническому пути (decl = чистая чексумма; класс исчезает); (2) минимум — hard-error диагностика дубля с обоими путями. Repro: research-заметка 2026-07-13 §2а. Проверить ту же ось в C-mangling (`Nova_<modpath>_` — если modpath=decl, дубль в одном CU коллидирует и там). **Дом: [Plan 202](202-module-registry-path-keyed-and-root-module.md)** (Ф.1/Ф.1b; одобрен 2026-07-13, не запущен). | Plan 202 | P1 |
