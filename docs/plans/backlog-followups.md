...
| `[M-139-f2-ptr-field-producers]` — ✅ **ЗАКРЫТ Plan 139.2 Ф.0+Ф.2 (2026-06-12)** | `as_bytes` (Ф.0), `split` + `from_bytes_lossy` + `from_bytes_unchecked` + `from_bytes_unchecked_steal` (Ф.2) **мигрированы в Nova-body**. **Прошлый root-cause был НЕВЕРЕН:** настоящий разблокер — НЕ `[]T→Vec` flip (он уже состоялся через `Vec.from_raw_parts`, landed Ф.0), а две точечные вещи: (1) codegen-лоуэринг `str { ptr, len }` record-литерала в C compound-literal `(nova_str){.ptr=…,.len=…}` (str ∈ RUNTIME_DEFINED_TYPES skip-list → не было schema → emit-null-stub; добавлен спец-кейс в `emit_record_lit`, emit_c.rs); (2) consume-обязательство `consume bytes []u8` у steal — закрыто новым `Vec[T] consume @into_raw() -> *mut T` (инверс `from_raw_parts`, vec_owned.nv). `split` строит zero-copy sub-views `str{ptr:@ptr+off,len}` (privacy type-based: str type-method видит priv @ptr), `from_bytes_*` читают `(ptr,len,cap)` через публичные Vec-геттеры `@as_ptr/@len/@cap`, alloc+copy+NUL (D26 §3), steal — zero-copy reuse при cap>len. `from_bytes_unchecked` private helper `str.alloc_copy` (str static method → privacy). 8/8 plan139_2 PASS (split_ok+from_bytes_ok+2 neg), 0 регрессий. Остаётся C ТОЛЬКО `@hash` (`[M-139.1-hash-irreducible-crypto-seed]`, security) + operator-lowered `@concat`/`@compare` (`[M-139.1-operator-lowered-methods]`, Ф.3 scope) |
