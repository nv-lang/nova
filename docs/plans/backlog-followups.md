...
| `[M-method-value-arg-in-generic-combinator-infer]` | **OPEN 2026-07-24 (найдено при оценке предложения владельца о сахаре `a.map^method()`).** Method-value как аргумент generic-комбинатора не даёт вывести method-level type-param. Репро: `ro a Option[int] = Some(42); a.map(int.@to_str)` → **`cannot infer method-level type argument \`U\` for `Option.map`**. `int.@to_str` = `fn(int) -> str` (D35 unbound-instance method-value), `map[U](f fn(T)->U) -> Option[U]` должен вывести `U := str` из возвращаемого типа method-value — но инференс через method-value-аргумент не протекает. Замыкание работает: `a.map(|v| v.to_str())` PASS. **Значимость:** это «правильная одна дверь» для подъёма arg-less-метода над функтором (`a.map(int.@to_str)` вместо `a.map(|v| v.to_str())`) — без неё владелец рассматривал новый сахар `a.map^method()` (отвергнут: глиф `^` занят XOR + вторая дверь к method-values). Починка инференса закрывает потребность существующим механизмом. **Зона:** чекер, вывод generic-return из method-value-аргумента (`types/mod.rs`, инференс type-arg комбинатора). Arg-bearing (`v.clamp(lo,hi)`) этим НЕ покрывается (method-value не связывает аргументы — остаётся замыкание). | чекер (`types/mod.rs`, method-value type-inference) | **P3 (эргономика; замыкание-обход есть)** |
| `[M-fmt-rich-spec-primitive-fresh-sb-redundant]` | **OPEN 2026-07-24 (наблюдение владельца при трейсе codegen интерполяции).** rich-spec примитив (`${255:#x}`, `${n:5}` и т.п.) в `emit_int_display_spec_call` (`emit_c.rs`, тж. f64/f32-аналоги) **безусловно** создаёт свежий `Nova_StringBuilder_static_new(16)`, рендерит в него `int_display_spec`, потом `consume_into_str` → caller `append` в главный interp-SB. **Избыточно для примитивов:** `*_display_spec` паддит выравнивание относительно `mark = sb.byte_len()` (mark-relative `pad_in_place`), значит корректно пишет прямо в главный SB — что ДОКАЗЫВАЕТ голый путь (`${n}` → `int_display_spec(_nv_interp_sb, …)` напрямую). Цена на КАЖДОЕ rich-spec поле примитива: лишняя heap-аллокация StringBuilder + материализация str + копирование append; для N полей — N лишних аллокаций. **Корень:** Plan 208 Ш3 девиртуализовал ГОЛЫЙ примитивный путь в прямую запись, а RICH-spec путь ту же оптимизацию НЕ получил — остался на старой двухшаговой форме (render→into_str→append). **Фикс:** передавать главный interp-`sb` прямо в `int_display_spec`/`f64_display_spec`/`f32_display_spec` (как голый путь), убрать fresh-SB+into_str+append для примитивов. **НЕ трогать композит/юзер-тип tail** — там fresh-SB обоснован V1-упрощением #1 (рендер во fresh + внешний `nova_fmt_pad`). Гейт: поведенческая идентичность (вывод форматирования тот же, C изменится — байт-паритет НЕ применим) + профиль/подсчёт аллокаций. | codegen (`emit_c.rs` `emit_int_display_spec_call` + f64/f32) | **P3 (перф, вывод корректен)** |
