...
| `[M-novavtable-read-write-pointer-collision]` | **OPEN (найдено 2026-07-16, sonnet, Plan 196 Зона TEST — попутно, при написании D61-пин-теста `spec_tests/conformance/d61_effect_handler_direct_call.nv`).** Codegen: нуль-арный `.read()` (и одноарный `.write(v)`) на ЛЮБОМ handler-ЗНАЧЕНИИ эффекта (`NovaVtable_<Eff>*`, D61 §8 «`Effect[E].op(args)` — прямой вызов, минуя with-стек») **молча мисдиспатчится** в СОВЕРШЕННО НЕСВЯЗАННУЮ legacy-ветку `B11d_typed_pointer_methods` (`emit_c.rs` ~51360-51384, «typed pointer `(*T).read()->T`») вместо корректного `B11ac_novavtable_effect` (~51777, `effect_schemas`-lookup). Root: guard на входе в B11d-блок (~51368) — `obj_ty.ends_with('*') && !obj_ty.starts_with("Nova_") && !obj_ty.starts_with("NovaArray_") && obj_ty != "void*"` — не исключает префикс `NovaVtable_` (он НЕ начинается на `"Nova_"` буквально — нет подчёркивания сразу после `Nova`), поэтому ЛЮБОЙ `NovaVtable_X*`-ресивер ошибочно проходит проверку и ловится B11d РАНЬШЕ, чем управление доходит до B11ac ниже по каскаду. Эмпирически: `effect Foo { read() -> int }`; `h.read()` эмитит `NovaVtable_Foo r = (*(gauge));` (структурная COPY vtable вместо `gauge->read(gauge->ctx)`) — CC-FAIL (`invalid operands to binary expression`) при использовании возврата, а без использования — тихий wrong-value (структура вместо int). **Живых call-сайтов сегодня НЕТ** (census `docs/plans/196.5-stage-d-census.md` §3.2: единственный корпусный direct-call-на-handler — `examples/effects/effects_d61.nv`'s `switcher.flip()`, имя не коллидирует) — чисто латентный баг, пойман только потому, что пин-тест сознательно перебирал разные имена операций. Fix: добавить `!obj_ty.starts_with("NovaVtable_")` в guard B11d (~51368) — однострочный, но ВНЕ Зоны TEST (эта строка emit_c.rs принадлежит Зоне GEN/frozen, не трогал). Тест избегает `read`/`write` как имён операций (см. коммент в файле). | Plan 196 Зона TEST / codegen (emit_c.rs B11d) | P2 |
...
| Маркер | Суть | Home | Pri |
|---|---|---|---|
| `[M-str-primitive-static-arity-overload]` | **OPEN 2026-07-17.** Primitive-static резолв АРНОСТЬ-СЛЕПОЙ: 2-арг `fn str.new(buf,len)` рядом с 0-арг `str.new()` НЕ разрешается — call-сайт падает в codegen-фолбэк `E_UNKNOWN_STATIC_METHOD str.new(...)` («валидные str.from… резолвятся раньше» — списковый/имя-первый путь примитивов). Generic-типы починены П5/200 (`pick_static_params`, `Sigs::static_methods`) — primitive-путь ходит СТАРОЙ тропой без overload-pick. Вскрыто rename'ом `wrap_owned`→`str.new` (канон Type.new, владелец 2026-07-16): rename ОТКАЧЕН до фикса (см. NB в std/src/runtime/string/core.nv), checksums-CU был красный 6/6 → после отката PASS 3/0. Фикс: свести primitive-static резолв на тот же `pick_static_params`-механизм (одно окно / resolved_callees); затем вернуть rename. | Plan 196-семья / callnorm+codegen | **P1** |
| `[M-d216-unsafe-map-single-file-gaps]` | **OPEN 2026-07-17.** Карта D216 §21 (E_UNSAFE_UNUSED/A11) не знает: (а) `as`-каст указателя (`@ptr as *mut u8`) — историческая `str @bytes`-форма красная в single-file check; (б) вызов `unsafe fn` через GENERIC-STATIC оверлоад/slice-sugar (`[]u8.new(...)` 2-арг view) — атрибут `export unsafe fn Vec[T].new` НЕ собирается (collect ключует статики литеральным именем типа) → атрибут инертен: ни энфорса E_UNSAFE_CALL_REQUIRES_WRAP, ни признания обёртки. ЛАТЕНТНО: реальные CU-пути (импорты/тесты/CI) зелёные; single-file `nova check string/core.nv` красный ВСЕГДА (обе формы). Фикс: (а) ptr-cast в карту; (б) collect unsafe-статиков generic-форм + PER-OVERLOAD (иначе safe-арности `Vec.new` везде потребуют unsafe) — резолв через `resolved_callees` (одно окно, 196.7-семья). До фикса вар.(б) владельца (`unsafe fn` view-new) семантически заявлен, но не энфорсится. | D216 §21 / checker | **P2** |
| `[M-check-folder-enumerator-skips-no-prelude]` | **OPEN 2026-07-17.** `nova check std/src/runtime/string` (папка) → «no .nv files to check» — enumerator тихо скипает модуль (подозрение: фильтр `#no_prelude`); vec-папка чекается нормально. Следствие: single-file-долги (сосед-маркер) копились невидимо; таргетная проверка «PASS 3/0» ветки p200-vecstr НЕ покрыла string-модуль. Фикс: чекать все .nv в папке + честный вывод «skipped N (причина)» вместо тихого пропуска. | nova-cli check | **P2** |
...
| `[M-174.6-rawptr-extern-unsafe-infer]` | **Найдено аудитом планов ≥150 (2026-07-16).** D424 (`spec/decisions/02-types.md:9033`, Plan 174.6 M4, решение владельца 2026-07-15) нормативно вводит правило «raw-ptr `extern`/`external` fn ⇒ `unsafe fn` по инференсу» + снимает `E_UNSAFE_UNUSED` carve-out — но сама спека помечает **«Статус реализации: PROPOSED 2026-07-15 (спека/дизайн)»**: checker-enforcement (инференс unsafe-классификации по сигнатуре extern, удаление carve-out, миграция-свип уже-лишних `unsafe {}`, pos/neg-тесты) ещё НЕ реализован. Маркер был назван в самой спеке, но никогда не заводился в backlog. | Plan 174.6 M4 / checker | P2 |
