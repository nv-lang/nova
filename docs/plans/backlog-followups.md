...
| Маркер | Суть | Home | Pri |
|---|---|---|---|
| `[M-newtype-over-fn-type-unsupported]` | **OPEN 2026-07-23 (проба интегратора; дизайн-решение владельца по `Handler`).** `type Handler fn(ServerRequest) -> ServerResponse` (newtype над FN-типом) **не парсится**: `expected identifier, got '('` — newtype принимает только ИМЕНОВАННЫЙ underlying-тип. Нужны ТРИ куска: (1) парсинг fn-типа как underlying у newtype (D52-амендмент); (2) авто-подъём `fn → Handler` — добавить fn-типы в таблицу «родов» obvious single-wrapper coercion (D55-амендмент; структурное совпадение `fn(A)->B` с `fn(A)->B`), однонаправленно (обратно НЕ коэрсить); (3) **call-through**: `next(req)` вызывается напрямую, как Go `type HandlerFunc func(…)` (который И вызывается, И имеет методы) — тогда метод-обёртка `@handle` удаляется как вторая дверь. Обоснование форварда вызова (в отличие от int-newtype, который НЕ форвардит `+`): вызов — единственная осмысленная операция над функцией. **Потребитель:** [222.3 §5а](222.3-extractors.md) — снос `handler_fn` (22 сайта + 14 тест-файлов) блокирован этим. | D52/D55 / парсер+чекер | **P2** |
| `[M-alias-of-fn-type-not-callable]` | **OPEN 2026-07-23 (проба интегратора при обсуждении формы `Handler`).** `type Handler alias fn(int) -> str` ОБЪЯВЛЯЕТСЯ без ошибок, но значение такого типа НЕ вызывается: `fn wrap(next Handler) => … next(x) …` → `[E_CALL_NOT_CALLABLE] 'next' names a local variable of type 'Handler' here, not a function`. Алиас по D52 обязан быть ПРОЗРАЧНЫМ (`X` и `Y` совместимы) → вызов через алиас fn-типа должен работать как через сам fn-тип. Класс: резолв вызова смотрит на ИМЯ типа, не разворачивая алиас. Репро 5 строк. NB: для http-`Handler` выбран НЕ алиас, а newtype+коэрсия (222.3 §5а) — но баг общий, касается любого алиаса fn-типа. | D52 alias / резолв вызова | **P2** |
...
| `[M-method-value-static-ret-type-ice]` | **OPEN 2026-07-23 (эмпирика интегратора; ПОПРАВКА к снятой записи — см. ниже).** Спека D35 §Method values (Plan 11 Ф.4) специфицирует ТРИ формы: closure, unbound-instance `Type.@method` (`fn(Recv,…)->R`), static `Type.method` (`fn(…)->R`). **Проверено по одной:** ✅ unbound-instance на user-типе (`Account.@add`) — PASS; ✅ unbound-instance на ПРИМИТИВЕ, прямой метод (`int.@max`) — PASS; ❌ **static-форма (`ro mk = Account.new`) — ICE**: `[P67-LEGACY] method call .balance return type unknown — checker must annotate` (emit_c.rs:55789, `obj_ty=""`) при ЛЮБОМ последующем методе на результате, даже через локал (`ro acc = mk(7); acc.balance()`), т.е. НЕ цепочко-специфично: **чекер не аннотирует тип возврата вызова через static-method-value**. Формы 1-2 работают → дыра узкая, только форма 3. Репро 6 строк. | D35 Method values / checker-аннотация | **P2** |
