...
| Маркер | Суть | Home | Pri |
| `[M-d180-consume-propagation-match-payload-mut-rebind]` | **OPEN 2026-07-17 (нашёл владелец, читая TLS-регресс-тест; подтверждено интегратором по спеке+коду).** Цепочка `match rvalue { Ok(x) => ... } → mut y = x → y.consume_method()` ОБХОДИТ статическую consume-дисциплину. Три сцепленных пробела: **(а)** owning-семантика биндингов пейлоада при rvalue-скрутини нигде не специфицирована — D157 расписывает только place-match (view-default, `Some(consume f)` для move); **(б)** чекер НЕ enforc'ит D156-пропагацию consume-обязательства через Result/Option-пейлоад в match/if-let: pattern-bound значение consume-типа не помечается consume-obligated → вся D180-дисциплина ниже по потоку молчит (симптом-родня: `[M-tls-tests-consume-keyword-d180-drift]` — W_CONSUME_KEYWORD_UNNECESSARY там, где по букве consume обязателен); **(в)** `mut stream = session` (alias/move consume-значения) компилируется вопреки D180 Rule 2 (`E_VIEW_BINDING_FORBIDDEN`), при этом ЛЕГАЛЬНОГО мутабельного владеющего ребиндинга не существует вовсе (Rule 4: `consume mut X` = parse error) — т.е. буква спеки не даёт выразить текущий гейт-зелёный код (examples/tls/echo_client.nv:44-48, aggregator live.nv:158-163, tls-тесты). Исполнение корректно (фактический move, память под GC), но логическая линейность течёт: double-close/use-after-close на этом пути статически НЕ ловятся. **Нужно дизайн-решение владельца** (язык-меняющее → D-амендмент в том же слиянии): вариант А — enforce по букве: consume-паттерны обязательны и для rvalue (`Ok(consume tcp)`), амендмент Rule 4 (разрешить `consume mut X` / mut-owning форму), миграция всех сайтов; вариант Б — узаконить неявный move для rvalue-пейлоад-биндингов и `mut X = consume_var` (Rust-стиль implicit move, D157/D180-амендменты). Рекомендация интегратора: А (философия D180 «visible ownership transfer на каждом binding-site» — прямо заявленный Nova edge; и ровно этого ожидал владелец интуитивно). До решения — НЕ стоп-волна (существующий код под гейтом опирается на текущее поведение). | D180/D157/D156, checker consume-flow | **P1 (design-blocked)** |
...
| Маркер | Суть | Home | Pri |
|---|---|---|---|
| `[M-d55-str-literal-coercion-name-gated]` | **OPEN 2026-07-17 (нарушение [compiler-conventions.md §3](../compiler-conventions.md), подтверждено владельцем).** D55-амендмент «str-литерал → `[]u8`» специфицирован ТИПО-направленно (любая `[]u8`-позиция: call-arg на любой `[]u8`-параметр, `let`/`const` с `[]u8`-аннотацией, return-позиция, element `[][]u8` — [02-types.md §Str-литерал→[]u8](../../spec/decisions/02-types.md)), но РЕАЛИЗОВАН ИМЯ-направленно: `emit_c.rs::synthesize_write_str_lit_bytes_coercion` триггерит ТОЛЬКО на методе, названном буквально `write`, с одним позиционным str-литералом, на приёмнике с уже зарегистрированным `write`. Это name-keyed спец-кейс под builtin — ровно анти-паттерн §3 (семья `resolved_cat_of` Vec→Array; тот же класс, что 196.x диспатч-по-имени). **Спека уже правильная (типо-направленная) → D-амендмент НЕ нужен, реализация должна догнать спеку.** ФИКС (одно окно, типо-направленно): коэрсия ключуется на РАЗРЕШЁННОМ ожидаемом типе `[]u8` (из `resolved_*`-канала параметра/аннотации/return, НЕ из имени метода) → str-литерал в такой позиции переписывается в `.bytes()` (переиспользуя существующий рабочий путь, как делает нынешний спец-кейс). Safety-gate «приёмник без write» становится ненужным: тип-приёмника уже даёт `[]u8`-позицию однозначно, ложной коэрсии на неродственном типе быть не может. Покрыть все 4 позиции спеки. Только ЛИТЕРАЛЫ (переменная → явный `.bytes()`, D176 — намеренно, см. D55 §«почему литерал»). Модель: sonnet по карте (codegen-резолв позиции), гейт — оркестратор. | D55 / Plan 208 / codegen | **P2** |
