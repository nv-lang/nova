...
| Маркер | Суть | Home | Pri |
|---|---|---|---|
| `[M-p200-17-remaining-3]` | **OPEN 2026-07-20 (остаток Пункта 17/200; 6/9 влито `72394c274`).** Три std-файл-модуля с инлайн-тестами НЕ конвертированы в папка-модули (`X.nv`→`X/core.nv`+`X/core_test.nv`): `std/src/encoding/base64.nv`, `std/src/runtime/fmt_buf.nv`, `std/src/testing/handlers.nv`. haiku-попытка (ветка p200-17-tests-v2, удалена; диагнозы интегратора): fmt_buf — при сплите терялась export-видимость `int_fmt_into`/`f64_fmt_shortest_into`/`f32_fmt_shortest_into` для string_builder.nv (undefined identifier); handlers — parse-ошибка в пире (core_test.nv:87 `expected fn/... got identifier` — затянут не-test-код); base64 — прогон пустой (0 тестов). haiku «чинил» опустошением тестов — отклонено. Нужен sonnet: конверсия С СОХРАНЕНИЕМ export-поверхности + честный перенос всех test-блоков. Инлайн-тесты в этих 3 = остаточное нарушение test-conventions. | Plan 200 П17 | **P3** |
