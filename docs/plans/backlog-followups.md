...
- `[M-153.1-append-as-slice-ccfail]` (P3, pre-existing, найден 2026-07-06 при .of→.new миграции): `nova_tests/plan153_1/append_as_slice` CC-FAIL `passing 'const nova_str' to parameter of incompatible type 'nova_int'` (.c:6046) — идентичен на .of()/.new() версиях (A/B), не связан с конвенцией; вероятно вскрыт одним из мержей волны 172-181 (append/as_slice кодоген). Локализовать бисекцией по мержам.
