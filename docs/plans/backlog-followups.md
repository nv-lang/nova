...
| `[M-nested-generic-receiver-method-mono]` | **OPEN 2026-08-01 (проба при добавлении `transpose` — решение владельца добавить flat_map_err+transpose).** Метод на ВЛОЖЕННО-generic приёмнике `fn Option[Result[T, E]] @transpose() -> Result[Option[T], E]` компилируется чекером, но codegen ломает мономорфизацию возврата: `Ok(Some(v))` эмитится как `Ok(v)` — CC-FAIL `passing 'nova_int' to parameter of incompatible type 'NovaOpt_nova_int'` (вложенный `Some` теряется при инстанциации `Result[Option[T], E]`). **Минимальный репро (5 строк, воспроизведён):** декла выше + `Some(Ok(5)).transpose()`. NB: receiver-форма `Option[Result[T, E]]` сама вводит T/E — дублирование `[T, E]` на методе даёт честный `E_DUPLICATE_GENERIC_DECL` (это работает верно). Простой generic-приёмник (`Result[T,E] @flat_map_err[F]`) монообразуется нормально — влит той же волной. **Блокирует:** `transpose` (одобрен владельцем, движущий кейс — строгая конфиг-политика «порта нет=дефолт / порт-мусор=ошибка»); добавить в prelude сразу после фикса (D26-нота 2026-08-01 уже помечает). Родня mono-семьи: `[M-generic-body-calls-generic-mono-placeholder]` (другой кейс — generic-вызов из generic-тела). | codegen (mono вложенно-generic приёмника/возврата) | **P2** |
...
| `[M-vec-seq-missing-import-ice-not-e7320]` | **OPEN 2026-07-31 (ревью флагмана). ДИАГНОЗ УТОЧНЁН/ПЕРЕОТКРЫТ 2026-08-01 (окно p-ice-pack, п.10) — честный СТОП, факты новые, root cause line НЕ найдена.** Было: `.map(...)` на `Vec[UserType]` БЕЗ явного `import std.collections.vec_seq.{map}` падает ICE `[P67-LEGACY] method call .map return type unknown` (emit_c.rs:58906) вместо чистой `E7320 no method`; на `Vec[str]` — корректная E7320. **НОВЫЙ ФАКТ (важнее заголовка):** это НЕ про отсутствие импорта `vec_seq` конкретно — заголовок вводит в заблуждение. Проверено фактом: `xs.totally_bogus_method_xyz()` (метод, которого не существует НИГДЕ, ни в vec_seq, ни где-либо ещё) ТОЖЕ молча проходит `nova check` на `Vec[UserRecordType]`/`Vec[UserValueType]` (heap ИЛИ value record, без разницы) — ВНЕ ЗАВИСИМОСТИ от явной аннотации типа (`ro xs []Item = [...]` не помогает). На `Vec[int]`/`Vec[str]` та же самая проверка (`.totally_bogus_method_xyz()`) КОРРЕКТНО даёт `[E7320] no field or method ... on type Vec`. Значит баг общий: чекер ПОЛНОСТЬЮ пропускает валидацию МЕТОДА для Vec-ресивера, когда элемент — user-декларированный тип (heap ИЛИ value record), независимо от конкретного имени метода. Трассировка (types/mod.rs `check_instance_overload` ~12563 → `infer_arg_ty` ~25288 → normalization `TypeRef::Array(_,_) => "Vec"` ~12628 → `self.sig.method_table.get("Vec")` ~12767 → `sig_complete_builtin` blok ~12780): КАЖДАЯ ветка, прочитанная статически, ДОЛЖНА вести себя одинаково для str/int и для user-record элемента (ни одна не консультирует element-тип отдельно) — то есть расхождение происходит ЛИБО в фактическом наполнении `scope`/`self.sig` в рантайме (не видно статическим чтением), ЛИБО в ещё не найденном более раннем gate/коротком возврате, который перехватывает Member-call ДО `check_instance_overload` специально для record-элементных Vec. Не хватило бюджета окна на runtime-трассировку (eprintln + пересборка, ещё один цикл ~2.5 мин на попытку). Репро (минимальный, std-only, оба варианта): `type Item { v int }` / `type Item value { v int }`, `ro xs []Item = [Item{v:1}, Item{v:2}]; xs.totally_bogus_method_xyz()` — PASS check (должен быть E7320); тот же код с `[1,2,3]`/`["a","b"]` вместо Item — корректно даёт E7320. Диагностика ОБЯЗАНА быть E7320 (+ дополнительно подсказка про `vec_seq` для комбинаторов типа `.map`, если метод РЕАЛЬНО существует там). Компиляторная очередь, чекер-канал — начать трассировку СРАЗУ с `check_instance_overload`'s `let Some(recv_ty) = BoundCtx::infer_arg_ty(...)` (сверить факт: точно ли `recv_ty`/`type_name` одинаковы для обоих случаев через eprintln, не полагаться на статическое чтение). |
...
| `[M-bigint-family-migrate-named-tuple]` | **РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-08-01, исполнить после ICE-фикса:** `type BigInt value {sign, limbs}` → named tuple `type BigInt(sign Sign, limbs []u32)` (D215: семантика та же — stack value + copy; выигрыш: позиционный конструктор, явный контракт размещения). Заодно оценить BigDecimal/BigFloat (те же value-records). Блокеры (обновлено 2026-08-01 после захода p-tuple-migration): f().field-ICE и positional-defaults ЗАКРЫТЫ (ICE-пачка); НОВЫЕ блокеры, вскрытые самой миграцией: №247 [M-named-tuple-compare-operators-no-dispatch] и №248 [M-named-tuple-cu-recv-method-misresolution] — оба в 196-волне; после них миграция чисто механическая (код написан и проверен окном, откачен ради зелёного дерева — см. его отчёт). Миграция механическая: декларация + сайты `{ sign: …, limbs: … }` → `BigInt(…, …)`; nova-bigint репа. |
...
| `[M-detach-box-while-loop-read-after]` | **P1 OPEN 2026-08-01 (окно p-shutdown, sonnet; НОВАЯ воспроизводимая форма box-класса). ДИАГНОЗ ЗАФИКСИРОВАН 2026-08-01 (окно p-ice-pack, sonnet, п.2) — честный СТОП, фикс НЕ сделан (см. ниже почему).** Минимальное std-only репро (без polaris/Router — воспроизводится с `std.runtime.sync.Semaphore`):<br>`fn probe() Detach { mut inflight = Semaphore.new(4); mut running = true; while running { detach { ro _ = inflight.try_acquire() }; running = false }; ro avail = inflight.available_permits(); println("avail=${avail}") }` → `error: use of undeclared identifier '_nova_detach_0_box_inflight'`. **Корень (факт, не гипотеза):** `emit_detach` (emit_c.rs ~14887, комментарий `[M-conformance-megacu-intermittent-run-crash]`) heap-box'ит MUTABLE-захваты (`{ty}* {bv} = (ty*)nova_alloc(...); *{bv} = ...;`), эмитя ОБЕ строки через `self.line(...)` В ТЕКУЩЕЙ C-позиции — т.е. ВНУТРИ C-блока `while`-тела, когда `detach{}` там вложен. Куча (`nova_alloc`) переживает выход из блока, но САМ C-идентификатор `bv` — блок-scoped, обнуляется на `}` конца `while`; `var_boxed` (Rust-side карта "имя→box-выражение", используемая ПОЗЖЕ каждым `ExprKind::Ident` того же имени) переживает scope framework'а компилятора и продолжает подставлять уже-невидимый `bv` в код ПОСЛЕ цикла → undeclared identifier. Тот же класс уже чинился для `emit_effect_handler_literal` ([M-175-handler-lit-boxed-var-c-scope-leak]) — но ТАМ фикс (`&cap_name` вместо нового heap-box для inline-хендлеров) неприменим здесь: detach — честный fire-and-forget, что и заставило emit_detach ИЗНАЧАЛЬНО уйти от `&stack_local` к heap-box (см. тот же комментарий, `[M-conformance-megacu-intermittent-run-crash]`) — `&cap_name` вернул бы РОВНО ту UB-регрессию. **Настоящий фикс** — box-указатель должен объявляться (bare, без инициализатора) в C-scope, ДОМИНИРУЮЩЕМ и точку detach{}, и все послецикловые чтения: естественный кандидат — сразу после СОБСТВЕННОГО C-объявления `cap`'а (оно уже в нужном scope по конструкции — Nova требует что переменная объявлена ДО захвата). Это требует retroactive-insert в `self.out` (запомнить byte-offset+indent конца `Stmt::Let`-объявления `mut`-локали, `self.out.insert_str` туда bare-decl, сдвинуть все ПОЗЖЕ сохранённые offset'ы) ЛИБО полноценный pre-scan «будет ли эта mut-локаль когда-либо box-нута» перед эмитом её же `Stmt::Let` (решение сейчас целиком LAZY/reactive — первый closure/handler/detach решает по факту, ни один из трёх site'ов не знает заранее). И то, и другое — новая инфраструктура поверх ОГРОМНОГО `emit_stmt`/`Stmt::Let` (тысячи строк, много под-веток: destructure, ghost, value-record, ...) — риск регрессии несоразмерен окну (11 пунктов). Обход (цикл в отдельную функцию с mut-параметром) остаётся единственным путём. Канал для следующего окна: emit_detach (НЕ emit_lambda — тот, per исходный бриф, уже починен) + либо offset-hoist в emit_stmt, либо pre-scan «captured-later» перед Stmt::Let-эмитом mut-локали. Репро в комментарии serve() (polaris master, defect 1) — теперь ЕСТЬ std-only минимальная форма выше, воспроизводимая без polaris. Передано окну p-soundness-pack. |
...
| `[M-consume-param-spawn-defer-active]` | **P1 OPEN 2026-08-01 (то же окно). ЧАСТИЧНО ЗАКРЫТ 2026-08-01 (окно p-ice-pack, п.11) — чекер-половина исправлена, codegen-половина честный СТОП, новые факты.** Было: перенос consume-параметра в `spawn` → `_defer_N_0_active` undeclared («Plan 217: consume-param call-arg»). Четвёртое репро в serve()-комментарии. **Найдено И ПОЧИНЕНО попутно (реальный, независимо ценный чекер-баг):** `spawn consume c { body }` (D415 §4 «already-bound» форма) ложно давал `[D133-not-consumed]` на ВНЕШНЕМ `c` при выходе из ЕГО СОБСТВЕННОГО объемлющего scope — хотя владение синхронно переходит в spawn-child ИМЕННО в момент `spawn`-statement'а. Корень — ДВА слоя: (1) `Stmt::ConsumeScope`'s `re_consume=false`-ветка НИКОГДА не отмечала переиспользуемый внешний биндинг Consumed; (2) `ExprKind::Spawn`'s `consume_walk_isolated_expr` снапшотит/восстанавливает `ctx.states` вокруг ВСЕГО тела (звучит для mut-захватов, НЕВЕРНО для ownership-move — тот синхронен на самом spawn-statement) — восстановление ОТМЕНЯЛО фикс (1). Оба починены (types/mod.rs, `Stmt::ConsumeScope` + `ExprKind::Spawn` arms). **НЕ доведено до конца:** сам заявленный codegen-баг (`_defer_N_0_active`) НЕ воспроизведён в чистом виде — минимальный репро БЕЗ `@cleanup` бьётся в ДРУГОЙ (тоже реальный) баг `undefined symbol: Nova_<Type>_consume_cleanup` (codegen безусловно эмитит вызов cleanup независимо от того, объявлен ли он), а репро С `@cleanup` — в ТРЕТИЙ баг `use of undeclared identifier '_nv_tmp_N'` (temp объявлен в одном C-блоке, использован в НЕ-доминирующем sibling-блоке — тот же СТРУКТУРНЫЙ класс, что у [M-detach-box-while-loop-read-after] п.2, другой конкретный сайт). Чтобы попасть ИМЕННО в `_defer_N_0_active`, нужен репро ближе к реальному TcpListener consume-param (у него, видимо, более полная close/cleanup-машинерия) — не хватило бюджета окна. Репро (чекер-фикс подтверждён, codegen-баги задокументированы): docs/plans/repro/m_consume_param_spawn_defer_active.nv (референс, НЕ в тест-раннере — build ломается на баге #1 выше). Следующему окну: начать с TcpListener-точного репро (не hand-rolled Handle-типа), канал — emit_c.rs consume-scope/cleanup codegen (Plan 217 «cleanup дизармлен» + temp-scope hoisting, зеркало п.2's диагноза). |
...
| `[M-operator-dispatch-checker-channel-196]` | **OPEN 2026-08-01 (честная граница волны унификации, вопрос владельца):** унификация свела 13 операторов в BINOP_TABLE/UNOP_TABLE + один резолвер (emit_c −128), но РЕШЕНИЕ «оператор→метод» всё ещё в emit-слое по C-типам операндов — не в чекере. Полный §0/196-канон: чекер резолвит операторное выражение в конкретный callee (resolved_callees, как обычный вызов), mono-регистрация — фазой, emit только исполняет; таблица остаётся единственным источником имён/арности. **ТРЕБОВАНИЕ ВЛАДЕЛЬЦА 2026-08-01: ОДНО окно на всё однообразное** — эта 196-волна включает РАЗОМ: 13 операторов таблицы + compound-assign ветку (emit_c ~32046, AssignOp::Add|Sub|… — сахар a=a.plus(e), обязан идти через ту же таблицу) + семью сравнений == != < <= > >= (@equal/@compare, поглощает [M-operator-dispatch-unify-eq-compare-family]) + value-record путь (поглощает [M-operator-dispatch-unify-value-record-path]); byte-parity регресс на каждый пласт; **ПЛЮС решение владельца 2026-08-01 (исследование по языкам, корпус: 0 потребителей): `!` СУЖАЕТСЯ ДО bool — ретракция строки `@not` в D46-таблице ТЕМ ЖЕ слиянием (язык-меняющее), @not выкидывается из UNOP_TABLE и DCE-сидов, попытка перегрузки `!` = понятная ошибка с подсказкой «bitwise — overload `~`/@bitnot», две фикстуры-носителя (d46_operator_overload_at_methods @not-часть, m128_neg_not_operator_only_pos @not-часть) удаляются/сужаются до @neg/@bitnot; neg-фикстура на запрет перегрузки обязательна.** Очередь: сразу после ICE-пачки. |
| `[M-mem-effect-demote-to-namespace]` | **OPEN 2026-08-01 (вопрос владельца «зачем Mem эффект, почему не неймспейс»; владелец: ОК на понижение):** эффект-статус `Mem` (D76) декоративен — нет vtable/хендлера/мока/подписи, все три определяющих свойства эффекта отсутствуют, опы и так inline в rt-счётчики; собственный rationale D76 («vtable исказил бы измерение») — аргумент против эффекта. Понизить до неймспейса/статик-функций (прецедент RawMem); синтаксис вызова `Mem.alloc_count()` может сохраниться. D-амендмент к D76 + правка D26-таблицы ТЕМ ЖЕ слиянием (язык-меняющее). `Trace` НЕ трогать: не реализован, решение «эффект ради экспортёров или нет» — по факту реализации (зафиксировано в D26-ноте при амендменте). Компиляторная очередь, мелочь. |
| `[M-default-handler-factory-yield-point-ban]` | **OPEN 2026-08-01 (вопрос владельца про гонку в момент ленивой инициализации; владелец: «запрет — согласен»):** тело `#default_handler`-фабрики обязано не содержать yield-point'ов (транзитивно, D64-класс скана): суспензия посреди «проверил-NULL → строю → записал» отдаёт поток второму файберу, тот видит ещё-NULL слот и строит второй экземпляр (double-construction; для stateful-дефолта — расщепление состояния). `check_default_handlers` сегодня ловит только циклы фабрик (E_DEFAULT_HANDLER_CYCLE), суспензию — нет. Плюс факт из per-fiber-снапшот механики (effects.h:1571): лениво установленный дефолт уезжает в снапшот ТЕКУЩЕГО файбера, база потока восстанавливается — фабрика и так исполняется по разу на холодную файбер-линию, т.е. обязана быть чистой; запрет yield-point это статически подпирает. Диагностика класса E_DEFAULT_HANDLER_SUSPEND с цепочкой вызовов. Чекер-канал; учесть слепоту D64-скана на instance-method receiver ([M-175-realtime-ban-method-call-blind] — тот же класс). Компиляторная очередь. |
| `[M-println-lowering-onto-nv-print-str]` | **ЗАМОРОЖЕНО ДО СОГЛАСОВАНИЯ — владелец 2026-08-01: «без согласования со мной не делай»; НЕ брать в окна из очереди, старт только по явному GO владельца (дизайн печати/эффект-статус — его решение).** OPEN 2026-08-01 (вопрос владельца «print/println может быть на nv?»; §3 maximize-nv):** печать переносится в std: крошечное ядро `print_str(s str)` в .nv (ambient-статус как у нынешнего extern println — эффект-вопрос ортогонален), lowering `println(a,b,…)` = статический to_str каждого аргумента (существующие .nv-диспетчи интерполяции) → конкат → один вызов print_str; emit_println-простыня и семейство nova_print_int/f64/… уходят из emit_c/rt (минус легаси, дух 196). Интерполяция остаётся сахаром. Компиляторная волна после 196-операторной (тот же класс). **RESEARCH ГОТОВ (2026-08-01, к GO-решению): [docs/research/2026-08-01-println-on-nv.md](../research/2026-08-01-println-on-nv.md)** — инвентарь (~190 LOC emit_println-семьи + 6 rt-хелперов), 3 дефекта статус-кво (юзер-тип = %lld-мусор D229-класса; char-var = code-point; параллельный диспетч дрейфует от интерполяции), НАХОДКА: мокабельный stdout УЖЕ есть (std/io/console.nv Io.write_out + #default_handler + mock_io) — новый extern print_str не нужен; путь A = синтез интерп-строки + переиспользование emit_interpolated_str целиком; ЦЕНТРАЛЬНЫЙ вопрос к GO — эффект-статус печати (рекомендация: оп Io через #default_handler ambient как Log → капчур println в тестах бесплатно; пин совместимости с №131 explicit-Io). |
| `[M-effect-op-body-module-fn-undeclared]` | **OPEN 2026-08-01 (№244; найден на примере 11-json-logs):** тело effect-op хендлер-литерала, зовущее МОДУЛЬНУЮ свободную fn, эмитит вызов необъявленного C-идентификатора (`use of undeclared identifier 'line'`) — модульные fn не попадают в контекст эмиссии op-тел. Обход: инлайн тела по опам (пример задокументирован по месту). Класс emit-контекста хендлеров. |
| `[M-closure-in-fn-array-path-call-p67-ice]` | **OPEN 2026-08-01 (№245; попутная находка окна p-ice-pack при разведке п.5):** closure, хранимый в `[]fn()->str`, вызванный `h(v)` внутри — P67-LEGACY ICE «Path call return type unknown, no parts». Репро-описание в PROGRESS окна (nova-pice). |
| `[M-named-tuple-compare-operators-no-dispatch]` | **OPEN 2026-08-01 (№247; окно p-tuple-migration):** для named-tuple ABI (NovaTuple_X) диспетч бинарных операторов покрывает Eq/Neq/Add/Sub/Mul/Div, но НЕ Lt/Le/Gt/Ge (~emit 34058-34105) → `<`/`>` на кортеже падают в raw C-сравнение структур (CC-FAIL invalid operands); у value-record/heap-record сравнение диспетчится (асимметрия). БЛОКИРУЕТ миграцию BigInt (тест bigint_test «operators < >»). Ложится пластом в 196-операторную волну (семья сравнений — добавить NovaTuple-форму). |
| `[M-named-tuple-cu-recv-method-misresolution]` | **P1 OPEN 2026-08-01 (№248; окно p-tuple-migration, изолировано bisection'ом):** «тип объявлен D215 named tuple» + в том же CU `.div_rem()` (Result-of-tuple метод) + StringBuilder → E_RECV_METHOD_MISMATCH: single-key fallback резолвит имя метода в ЧУЖОЙ тип (last-wins) — класс №129 single-key registries. Исчезает при возврате типа в value record. БЛОКИРУЕТ миграцию BigDecimal/BigFloat. В 196-волну вторым пунктом (или mono_method_registry-канал). |
| `[M-uniform-clone-copy-class-discussion]` | **ТЕМА ОБСУЖДЕНИЯ (владелец 2026-08-01, «не для немедленного выполнения»):** единообразное копирование — явный `.clone()` для ВСЕХ нетривиальных копий (стековых и кучевых), неявная копия только «Copy-классу» (рекурсивно-скалярные; = Rust Copy/Clone канон, наш эталон). Анализ интегратора: deep-value энфорс D246 §72 уже прошёл 80% пути; на рекурсивно-стековых clone обязан компилироваться в ту же битовую копию (zero-cost — гарантию в D). Развилки к решению: (1) Copy-класс автоматически по структуре [рекомендация] vs атрибут #copy; (2) охват: только ro→mut-инициализация [рекомендация] vs все копирующие позиции; (3) тайминг: после v0.1 [рекомендация], направление D-заметкой сейчас. НЕ дефект — в baseline стража номеров. |
| `[M-clone-value-return-no-extra-copies-verify]` | **РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-08-01 (уточнено): ret-слот механизм У НАС УЖЕ ЕСТЬ** — используется точечно при возврате на стеке значений с кучевыми полями (оптимизация против лишних аллокаций), но НЕ как единое решение. ЗАДАЧА (одно окно, по правилам 196): (1) найти и инвентаризировать существующий механизм в кодогене (все места, где он включается, и где НЕ включается); (2) исследовать расширение охвата — все value-return/clone-пути; (3) сделать ЕДИНООБРАЗНЫМ: один путь эмиссии ret-слота (не точечные кейсы), решение о применении — фазой/каналом, не сайд-эффектом ветки; (4) доказательство по сгенерированному C: ноль лишних копий/аллокаций на матрице форм (рекурсивно-стековый / с кучевыми полями / clone / обычный return). Прежний текст (sret/RVO-гипотеза): `.clone()` и вообще возврат value-типов НЕ должен делать лишних копий — целевая механика: скрытый ret/out-параметр (в C это sret/RVO: caller даёт слот, callee пишет прямо в него; clang NRVO убирает внутренние временные). Задача: ДОКАЗАТЬ по сгенерированному C, что для (а) рекурсивно-стековых clone == один memcpy в слот назначения и (б) value-return вообще — без промежуточных копий; где ABI/эмиссия дают лишнюю копию — внедрить явный ret-параметр в кодогене. Перф-задача, не дефект (baseline стража); слот — после v0.1 либо попутно 196-волне если она рядом. |
