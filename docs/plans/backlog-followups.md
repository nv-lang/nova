...
| `[M-138.2-value-pos-arraylit-vec-gate]` | Ф.0b ROOT-CAUSE (для Ф.0-final): `[]T` в ПОЗИЦИИ-ЗНАЧЕНИЯ (`[]T.with_capacity(n)`, `[]T.new()`, array-literal `[..]`, `arr.push`) НЕ Vec-gate-aware — D38 array-static-method dispatch (emit_c.rs:19863-19925) безусловно роутит в `nova_array_new_<suffix>` (int64-erased NovaArray), БЕЗ `contains_key("Vec")` guard, тогда как `[]T` в ТИПОВОЙ позиции (поле/return type) уже резолвится в типизированный Vec под gate (emit_c.rs:2074). Рассинхрон ломает любую fn, что строит `[]T` value-side и хранит/возвращает под typed-Vec тип (Ф.0b hashmap `new_buckets` пострадал → переписан на явный `Vec[Slot]`). Ф.0-final ДОЛЖЕН Vec-gate'нуть value-position dispatch (с graceful #no_prelude fallback). t3 — guard. **NOT a defect — это и есть scope флипа; маркер = checklist-item для Ф.0-final.** | plan-138.2 Ф.0-final | P1 |
