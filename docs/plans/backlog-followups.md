...
| `[M-p200-17-remaining-1-fmtbuf]` | **РАЗБЛОКИРОВАН 2026-07-20** — блокер `[M-imports-entry-folder-module-self-cycle-empty-exports]` ЗАКРЫТ (worktree `nova-entryfix`/ветка `p-fix-entry-exports`, не влито в main). `base64`/`handlers` конвертированы и слиты (см. `docs/history/simplifications-closed.md`) — счёт Пункта 17 теперь 8/9. Остался ТОЛЬКО `std/src/runtime/fmt_buf.nv`: split на `fmt_buf/{core.nv,core_test.nv}` ранее УЖЕ был выполнен и export-поверхность/приватность СОХРАНЕНЫ корректно (verified: `core_test.nv` резолвит module-private `int_fmt`/`FmtSpec`/`bool_fmt`/`char_fmt`/`fmt_f64`/`f64_fmt_into` из `core.nv` без ошибок), но упирался в резолвер-баг (`nova test std/src/runtime/fmt_buf/core_test.nv` CODEGEN-FAIL `undefined identifier` в `string_builder.nv`) и был РЕВЕРТНУТ (fmt_buf.nv остался единым файлом с 8 инлайн-тестами). Резолвер-фикс верифицирован на file-entry репро (`nova test std/src/runtime/fmt_buf.nv` PASS 8/8) — split снова можно пробовать, НЕ выполнено в этой волне (fix-волна была узко про резолвер, не про П17-конверсию); следующий заход — переприменить split и прогнать `core_test.nv` тем же путём. | Plan 200 П17 | **P3** |
