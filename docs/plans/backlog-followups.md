...
| `[M-http-compress-errorkind-crosspkg-collision]` | **OPEN 2026-07-21 (найдено при закрытии `[M-forin-crosspkg-char-to_str-blanket-collision]`, findings A-S4, worktree `nova-forin`, ветка `p-fix-forin-crosspkg`, sonnet).** ЧЕСТНЫЙ остаток nova-http сьюта — 3 CC-FAIL (`src/client/client`, `src/serdejson/serdejson`, `src/transport/real`): `clang: error: use of undeclared identifier 'Nova_ErrorKind'` / `unknown type name 'Nova_ErrorKind'` на сгенерированном `_nv_scr_NNNN->kind` доступе. Это ГЕНУИННЫЙ cross-package баг (в отличие от for-in находки выше, которая ИЗНАЧАЛЬНО была приписана cross-package, но оказалась same-CU): `http.ErrorKind` (std/http/error.nv) И `compress.ErrorKind` (внешний git-пакет `nova-compress`) — ОБА `type ErrorKind enum` (sum-type, качественно требует C-мангл-дизамбигуации по D381 `[M-sync-crossmodule-samename-type-collision]`, УЖЕ решённой системе — см. закрытый маркер в этом же файле). D381's `colliding_type_names`-детекция (`emit_c.rs::emit_module`, ~5029-5072) сканирует ТОЛЬКО `module.peer_files` ОДНОГО `Module`-дерева (одного top-level `module`-namespace, `http.client`/`http` root-peers) — она НЕ видит типы из ДРУГОГО пакета (`compress`), даже если они попадают в тот же C compile-unit через git-зависимость. Отсюда: `Nova_ErrorKind` (немодуль-квалифицированное имя) остаётся неоднозначным между двумя ОТДЕЛЬНЫМИ определениями — компилятор эмитит ссылку без forward-decl (или конфликтующую с чужой), clang рапортует undeclared/unknown-type. Комментарий в самом `nova-http/src/client/client.nv` (строка ~23-24) уже АНТИЦИПИРУЕТ эту находку («auto-decompress gzip/deflate — decode exists (Plan 179) but wiring compress into this CU collides http/compress ErrorKind C-mangling (codegen)» — `[M-178-autodecompress-needs-179]` → `[M-codegen-nominal-type-name-collision]`), т.е. это НЕ новая находка волны A-S4, а уже известный (задокументированный носителем) блокер, которым эта волна лишь подтвердила механизм. НЕ исправлено этой волной — требует расширения D381-collision-scan за границы ОДНОГО `Module`-дерева (архитектурная работа: нужно понять, как несколько top-level Nova-модулей/пакетов мержатся в один C compile-unit ДО `emit_module`, и провести коллизи-скан НА ЭТОМ мердж-уровне, а не внутри `emit_module`'а одного дерева) — вне объёма и бюджета этой волны (найдено ПОСЛЕ основного фикса, доп. время не запрашивалось). Гейт: 3/6 nova-http сьюта остаются CC-FAIL детерминированно (не флаки, не регрессия — pre-existing, задокументировано автором пакета заранее). Зона: вероятно multi-package CU merge (найти, где несколько `Module` объединяются перед `emit_module`) + `emit_c.rs::emit_module`'s `type_def_modules`-скан (расширить область видимости за пределы одного peer_files-дерева). | floating (codegen, cross-package type collision) | **P1 (честный остаток nova-http гейта)** |
...
| `[M-protocol-embed-vtable-missing-method]` | **OPEN 2026-07-21 (найдено при закрытии `[M-protocol-box-callarg-vtable-incomplete]`, Plan 221 A-B4/A-B6, worktree `nova-boxvt`, ветка `p-fix-box-vtable`, sonnet).** Отдельный, НЕ исправленный этой волной баг: протокол с `use`-embed (`type Greeter protocol { use Base  @greet()->int }`, где `Base` — другой протокол с `@base_greet()->int`) — вызов ВСТРОЕННОГО (embedded) метода через protocol-box (`g.base_greet()`, `g Greeter`) даёт `clang: error: no member named 'base_greet' in struct NovaVtable_Greeter`. Воспроизведено И на call-argument, И на let-binding коэрсии (`mut g Greeter = sp; g.base_greet()`) — баг НЕ специфичен для call-arg позиции (в отличие от уже закрытого маркера выше, с которым он изначально был перепутан/слит в одну запись). Гипотеза корня (не подтверждена глубокой инвестигацией, только чтением кода): чекер разворачивает embeds в плоский список методов через `flatten_protocol_methods` (комментарий у `TypeDeclKind::Protocol` в `compiler-codegen/src/ast/mod.rs:1140-1144`), но codegen'овская `protocol_method_registry.insert(t.name.clone(), (type_params, methods.clone()))` (`emit_c.rs`, обе точки регистрации — generic- и non-generic-протокол пре-пассы) кладёт СЫРОЙ AST-`methods` без учёта `embeds` — vtable-struct (строится из этого реестра в `emit_protocol_box_typedef`) поэтому не содержит полей для embedded-методов. Требует: (а) подтвердить гипотезу (найти `flatten_protocol_methods`, проверить, вызывается ли где-то codegen-side), (б) либо звать тот же flatten codegen-side при регистрации, либо явно проходить `embeds`-цепочку при построении vtable-struct. Репро НЕ сохранено в дереве (тривиально воспроизводимо по описанию — `use`-протокол + вызов embedded-метода через любую protocol-box коэрсию). Зона: codegen `emit_c.rs` (`protocol_method_registry` registration + `emit_protocol_box_typedef`), возможно `ast/mod.rs`/`types/mod.rs` (найти `flatten_protocol_methods` для зеркалирования). | codegen / protocol-dispatch | P2 |
