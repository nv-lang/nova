...
| `[M-lint-redundant-to-str-in-interpolation]` | **OPEN 2026-07-31 (ревью владельца по bigdecimal):** `${x.to_str()}` в интерполяции — лишний вызов, интерполяция сама диспетчеризует to_str/Display пользовательского типа (проверено пробой на BigInt). Линт W_-класса (семья manual_*): матчить `.to_str()`-хвост выражения непосредственно внутри `${...}`. Заодно: `.plus(...)/.times(...)/.minus(...)` там, где есть операторный сахар (@plus/@times — зарезервированные имена, 03-syntax.md:1188) — кандидат на соседний W_METHOD_FORM_OF_OPERATOR (проверить шум на std перед включением). Сайты bigdecimal уже переписаны интегратором (bigint внутренности — при том же линт-окне). P3, компилятор-очередь. |
