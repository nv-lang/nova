...
| `[M-statuscode-ctor-result-vs-contract]` | **OPEN 2026-07-24 (вопрос владельца).** `StatusCode.new(int) -> Result[StatusCode, HttpError]` (порт Rust `from_u16`) навязывает `?`/`!` даже на заведомо валидных литералах. Разделить по границе доверия: (1) недоверенный провод/парсинг → `StatusCode.from_u16(int) -> Result` (Result корректен — сетевой мусор восстановим, контракт бы паниковал фибру = DoS); (2) известно-валидный путь → `StatusCode.of(code int) -> StatusCode requires 100<=code<=599` (контракт: нарушение = баг вызывающего, паника уместна); (3) частые литералы → консты `StatusCode.OK`. **Семантика (contracts.md):** `requires`-нарушение panic-класс (нерекаверабл), `Result` восстановим — поэтому НЕ заменять Result контрактом целиком (регрессия звучности на сетевом входе). **Проба 2026-07-24:** compile-time-снятие контракта gated на z3-SMT (не в дефолт-сборке) — без него `.of(999)`/`#verify` падают РАНТАЙМ-паникой, не compile-error; с z3 → compile-error. Попутно: `.new`→Result против ctor-конвенции (`.new`=канонический конструктор), переименовать в `.from_u16`. **Часть [222.5 §4б](222.5-respond.md).** | nova-http `status.nv` + z3-gate | **P2** |
