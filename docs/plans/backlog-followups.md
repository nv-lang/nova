...
| `[M-sync-crossmodule-samename-type-collision]` | **Codegen (pre-existing language-gap, exposed by 178/179 merge). НЕ пофикшен — минимальный sound обход + debt.** Пользовательские sum/record-типы мангли́рует по ПРОСТОМУ имени: `Nova_{name}`, tag `NOVA_TAG_{name}_{V}`, конструкторы — БЕЗ квалификации модулем (`ResolvedType::Named.module=[]` для неквалиф. user-типов; `from_type_ref` берёт только синтакс-path). ⇒ два РАЗНЫХ типа с одним простым именем из РАЗНЫХ модулей в одном CU **коллидируют** (одна `Nova_X` C-структура/tag-enum на всех; registry `find_variant_v2` возвращает вариант выжившего entry). Merge 178/179 свёл в один positive-CU `spec_tests.conformance` **ТРИ** разных `ErrorKind` (std.io / std.http / std.encoding.compress; общие имена вариантов `Other`/`InvalidData` с РАЗНОЙ арностью → merged-union unsound) → ICE `emit_c.rs:[P67] Ident 'PermissionDenied' not a sum-variant` при эмиссии io `kind_from_errno`. **Target-form фикс = module-qualified type-naming (resolver populate `module` + codegen квалификация + tag/ctor/registry-ключи) = крупная переделка (масс. baseline-churn), НЕ sync-fix.** **Обход (sound, §0):** http/compress-conformance-фикстуры вынесены в свои module-CU `spec_tests/http` (d358) + `spec_tests/compress` (d333/334/335/336) — как d323-паттерн; io `ErrorKind` (d322) остаётся в conformance. d359-neg (compress-import) остаются в conformance/neg (компилируются индивидуально → без коллизии). | codegen/resolver | P1 |
...
| `[M-176-generic-wrapper-mono-inference]` | Codegen: generic wrapper-тип с protocol-bounded полем (`BufReader[R]`/`BufWriter[W]`) при inference-only конструкции (`BufReader.new(src)`, R выводится из аргумента) НЕ материализует мономорфизированные методы → эмитит NULL-stub (`emit_generic_static_method_stub`) → runtime-крах. **Обход:** явные type-args (`BufReader[BytesReader].new(src)`). Фикс: регистрировать generic-type-instance при inference из ctor-аргумента (mirror turbofish-path в `emit_c.rs:25875` + `infer_expr_c_type`, gated на stub-only). **NB (2026-07-04):** value-record-НЕЗАВИСИМ — эмпирически подтверждён (`Wrap.new(s)` inference-конструкция даёт NULL-stub `Nova_Wrap_static_new(){ return NULL; }` и на heap-error `WErr` тоже, не только на value-record); это ОТДЕЛЬНЫЙ от value-record-mono keystone корень (type-arg inference из ctor-arg), НЕ закрыт keystone-фиксом. Value-record-в-`Result` часть этого gap'а (CC-FAIL `unknown type NovaRes_<ok>_NovaValue_<E>`) — ЗАКРЫТА (см. `[M-valuerecord-result-vtable-mono]`); осталась только inference-часть. | Plan 176 Ф.1 / codegen | P1 |
...
| `[M-valuerecord-receiver-generic-method]` | Codegen (ОТДЕЛЬНЫЙ корень от value-record-`Result`-mono keystone): метод-generic метод (`fn VDec mut @take[T](x T) -> Result[T, DeErr]`) на value-record РЕСИВЕРЕ мономорфизируется некорректно — call-site передаёт ресивер ПО ЗНАЧЕНИЮ (`Nova_VDec_method_take____nova_int(d, …)`) туда, где mono ждёт `NovaValue_VDec *` (mut-ресивер = указатель) → CC-FAIL «passing 'NovaValue_VDec' to parameter of incompatible type 'NovaValue_VDec *'»; вдобавок возврат стирается в `void*` и type-param не подставляется (`Nova_T* v`). Дефект method-generic-mono подстановки (receiver-ABI + return-type subst), НЕ vtable-forward-decl. Оценка typed-IR-зависимости — открыта; фикс кандидат: применить `prepare_method_recv` (value-record→`&d`) + протянуть type-subst в return-тип в method-generic mono-пути. Нужен для value-record `Deserializer`-методов (180). | Plan 180 / codegen | P2 |
| `[M-176-xmod-payload-variant-ctor]` | Checker: cross-module конструкция payload-варианта литералом (`io.SeekFrom.Start(5)`, `io.ErrorKind.Other(5)` из другого модуля) → `[P67-LEGACY] Path call return type unknown for method=Start` (не аннотируется возвратный тип variant-конструктора). Pattern-match не затронут. **Обход:** статические конструкторы (`SeekFrom.start/end/current`). Фикс: аннотировать return-type qualified payload-variant-ctor в checker. | Plan 176 Ф.1 / checker | P2 |
...
- **[M-178-timeout-needs-173]** (REFINED 2026-07-06) — эмпирически: raw-substrate ЕСТЬ в main (`CancelToken` + `supervised(cancel:)` + `Time.sleep` + cancel-safe net-park), bespoke two-fiber race ХЭНД-РОЛИТСЯ. НО gated НЕ на «173 вообще», а на ДВУХ отсутствующих примитивах: (1) `within`/`with_timeout`/`race2` берут ЧИСТЫЙ `fn()->T` (no effect-row) → НЕ обёртывают `Http`-effectful `send()` (effect-mismatch); (2) `supervised(deadline:)` — **ноль usages/impl во всём репо** (только `supervised(cancel:)`). Нужен effect-polymorphic deadline-combinator (`fn() Http -> T`) ИЛИ `supervised(deadline:)`. Плюс consume-`Response` через scope-boundary = linearity-hazard. Timeout-by-default НЕ приземлён — честный refined-gate.
- **[M-178-autodecompress-needs-179]** (BLOCKED-BY-CODEGEN 2026-07-06) — НЕ gated на Plan 179 (decode `gzip_decode`/`zlib_decode`/`inflate` ЕСТЬ и работают). Реальный блокер: **codegen манглит nominal-типы по КОРОТКОМУ имени без module-qualification** → `std.encoding.compress.ErrorKind` и `std.http.ErrorKind` эмитят ОДИН `struct Nova_ErrorKind`/`NOVA_TAG_ErrorKind_Other` → `redefinition of enumerator` CC-FAIL, как только оба в одном CU. Auto-decompress-в-client ПРИНУЖДАЕТ co-presence (client→compress→ErrorKind + client→http→ErrorKind). Логика auto-decompress написана и корректна, но НЕ линкуется. Fix = либо module-qualified type-mangling в codegen (Rust, крупно/рискованно, §0 P67-зона), либо renname одного `#stable ErrorKind` (governance). Тот же барьер накрывает план-овский `ErrSource.Compress(CompressError)`. [M-codegen-nominal-type-name-collision]
...
Live-socket runner `std.http.servernet.handle_connection` (read framed request → `serve_once` → write → close) НАПИСАН и codegen-компилится, но **loopback-smoke не ЛИНКУЕТСЯ**: `lld-link: undefined symbol Nova_NetError_static_IoError` (референс из `std.net.net_error`) в net+http combined-CU — DCE/emission-баг: конструктор payload-варианта не эмитится когда http тоже в CU (net-only echo-тесты линкуются). Форс-референс не помог. [M-codegen-cross-module-ctor-emission]. Плюс `with Net{…}`-блок мис-выводит result-тип в `Result[(),HttpError]` при HttpError-в-CU (обход: `()` tail) [M-178-with-tail-bang-codegen — расширен]. → real-socket smoke снят; серверная ЛОГИКА полностью покрыта mock'ами.
...
- **[M-codegen-nominal-type-name-collision]** (P1) — nominal-type C-mangling по короткому имени без module-qualification → одноимённые exported-типы из разных модулей (compress `ErrorKind` ↔ http `ErrorKind`) = `redefinition` в общем CU. Блокирует auto-decompress + `ErrSource.Compress`. Fix = module-qualified `Nova_<mod>_<Type>` mangling (крупно, все match/ctor-сайты).
- **[M-codegen-method-return-turbofish]** (P2) — generic-МЕТОД с type-param только в return-позиции игнорит turbofish `resp.m[T]()` → монеморфизирует в `nova_int`, silent-miscompile (`void* w`). Free-fn turbofish OK. Обход: free-fn.
...
- **[M-codegen-cross-module-ctor-emission]** (P2) — payload-variant static-ctor (`NetError.IoError`) не эмитится в net+http combined-CU (net-only OK) → undefined-symbol link-fail. Блокирует http-server real-socket link.
