...
|---|---|---|---|
| `[M-embed-dir]` | **PROPOSED 2026-07-15 (owner-gated, запрос владельца при работе над 187).** `embed()` (D412) вшивает ОДИН файл; целую папку (фронт целиком в бинарь) одним вызовом нельзя — сейчас `embed()` на каждый файл + ручной роутинг. Предложение: `embed_dir("dir")` → иммутабельная карта `путь→[]u8` (нулевая копия, .rodata-блобы + статич. таблица), догоняет Go `//go:embed`+`embed.FS`. Дизайн/survey (Go/Rust/Zig/C23/.NET/Java) — [research 2026-07-15](../research/2026-07-15-embed-dir-proposal.md). Не срочно (single-embed + самодостаточный html покрывают MVP флагмана); удобство, не блокер. Язык-меняющее — D412-амендмент в том же слиянии при вводе. | floating (proposed / lang) | P3 |
