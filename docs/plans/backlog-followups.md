...
| `[M-fiber-arena-sigsegv-install-race]` | **OPEN 2026-07-16 (TSan-смоук Linux-build волны — ПОДТВЕРЖДЁННЫЙ data race).** `fiber_arena.c`: install-once SIGSEGV-хендлера через голый check-then-set `_sigsegv_installed` — два потока могут одновременно пройти check и оба установить хендлер (или увидеть частичную запись флага). Фикс дешёвый и локальный: atomic exchange (CAS/`nova_abool`) либо pthread_once-эквивалент; НЕ архитектурный (в отличие от runq-улова — тот в Плане 211 §5 п.4). Гейт фикса: тот же ручной TSan-смоук чист по этому месту. | floating (runtime, дешёвый фикс) | P2 |
