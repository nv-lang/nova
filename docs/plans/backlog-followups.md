...
- **[M-174-retract-with-timeout]** (Plan 174 / §3a п.4, OPEN 2026-07-06; **гейт-сверка Ф.3 п.5 2026-07-09**) — удалить `within[T]`/`with_timeout[T]` (+`race2`? нет — остаётся) из `std/concurrency/cancellation.nv` (fn + ~7 тест-ссылок): субсумированы `supervised(timeout:)` (D408). Deadline-параметры приземлены, НО полный гейт п.5 = «после **Plan 175**» (Monotonic/Duration + мокабельность) — **Plan 175 = READY, не начат** → удаление остаётся отложенным (подтверждено сверкой Ф.3 п.5). Плюс cancellation.nv независимо сломан retired-API-дрейфом (`str.len`/`ro`-field-mutation) → делать вместе с починкой/переписью файла после 175. Home: Plan 173 Ф.3-остаток.
