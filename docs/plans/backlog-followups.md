...
| `[M-vec-spelling-maplit-desugar-cap-ice]` | **OPEN (vec-sweep, 2026-07-06).** Добавление pre-sizing statement `.cap(n)` в desugar `[k:v]`-map-литерала (AST-transform в `desugar.rs::build_map_block`/`build_record_map_block`, ПОСЛЕ `HashMap.new()`, ДО insert_new-цикла) роняло компилятор: **ICE** `"method call \`.insert_new\` return type unknown"` (P67-LEGACY, emit_c.rs:42872) — но ТОЛЬКО при компиляции НЕСКОЛЬКИХ файлов вместе в одной CU (`nova_tests/map_literals/` folder-module); файл-виновник в ИЗОЛЯЦИИ компилируется чисто (другая, PRE-EXISTING ошибка атрибуции). Investigation не довела до точной причины (не triggер — ни chaining/separate-statement, ни presence-vs-absence самого `.cap()`: убор pre-sizing statement из desugar'а НЕ убрал ICE в folder-context, но временны́х рамок на глубже не хватило). **Принятый обход:** pre-sizing убран из map-literal desugar'а целиком (`let _ = n;` — только `.new()` + amortized growth в insert_new-цикле); perf-only регрессия (иногда лишний rehash для больших map-литералов), correctness не затронута. `nova_tests/map_literals` folder ВСЁ ЕЩЁ падает с этим ICE (пред-существующий independent от моего revert — см. репро в тексте маркера); НЕ входит в гейты этого захода (spec_tests/conformance чист). Требует отдельного расследования с debug-tooling. | codegen (checker per-statement type-inference cache?) | P1 |
...
| `[M-vec-neg-panic-cascaded-cc-fail]` | **OPEN, НЕ ПОЛНОСТЬЮ ИЗОЛИРОВАНО (vec-sweep, 2026-07-06).** `nova test --full std/collections/vec` показывает 2 FAIL вместо ранее-известного 1 (`access.nv` e7320 [M-vec-access-e7320-as-bytes-str]): добавился `neg/vec_of_empty_panic` CC-FAIL (`passing 'Nova_Vec____nova_byte *' to parameter of incompatible type 'nova_str'`). Rollback перегрузки `Vec[T].new(ptr,len,cap)` (см. `[M-vec-new-static-arity-overload]`) НЕ убрал эту ошибку — значит НЕ она причина. Гипотеза (не подтверждена до конца из-за нехватки времени): каскад от того же pre-existing `access.nv` e7320-сбоя — весь `vec`-folder-module компилируется ОДНОЙ CU, и type-check failure в `access.nv` может «отравлять» совместно генерируемый C для соседних тест-бинарников в том же батче. `spec_tests/conformance` (основной гейт этого захода) ЧИСТ (55/1, дельта 0 к baseline) — эта регрессия видна ТОЛЬКО в `--full std/collections/vec` прогоне. Нужен отдельный agent/сессия с debug-tooling для точной изоляции. | vec-sweep follow-up / codegen | P1 |
...
- **[M-174-retract-with-timeout]** (Plan 174 / §3a п.4, OPEN 2026-07-06) — удалить `within[T]`/`with_timeout[T]` (+`race2`? нет — остаётся) из `std/concurrency/cancellation.nv` (fn + ~7 тест-ссылок): субсумированы `supervised(timeout:)` (D408). UNBLOCKED (deadline-параметры приземлены), но cancellation.nv независимо сломан retired-API-дрейфом (`str.len`/`ro`-field-mutation) → делать вместе с починкой/переписью файла. Home: Plan 173 Ф.3-остаток.
...
- **[M-178-servernet-live-net-substrate-segfault]** (P2, pre-existing net-runtime) — живой loopback сокет (supervised{spawn+spawn} над TcpListener/TcpStream) детерминированно сегфолтит (~100ms, 5/5) на baseline И current, на чистом net (без http). Также plan83_12 net-тесты ICE `[P67-LEGACY] Path call return type unknown for method=bind` (direct `.bind().is_ok()`-паттерн) и `.unwrap()` на `Result[_,NetError]` эмитит `Nova_Fail_fail(NetError*)` vs `nova_str`-сигнатура. Net live-socket substrate в этом worktree широко сломан — не связано с Plan 178 codegen. Fix — за net-substrate-планом.
...
- `[M-net-redesign-owner-directive]` (P1, директива владельца 2026-07-06): «Сеть мне не нравится, как реализовано — как всё сделаешь, будем переделывать сеть». Переделка std/net после завершения трека А (172.12/172.13). Перед стартом получить у владельца критерии (API / libuv-слой / модель эффектов). Связанное, что войдёт в переделку: `[M-178-servernet-live-net-substrate-segfault]` (детерминированный сегфолт двух волокон через локальный сокет), P67-LEGACY ICE на net-путях (plan83_12 bind), комбинатор таймаута (эффект-полиморфный deadline — сделать ДО переделки, пригодится). План 116 (TLS) — ПОСЛЕ новой сети, не на старой.
