...
|---|---|---|---|
| `[M-embed-dir]` | **APPROVED/queued 2026-07-15 (владелец: вариант A).** `embed_dir("dir")` → иммутабельная `EmbeddedDir` (`get(path)->Option[[]u8]` + `paths()->[]str`, нулевая копия, .rodata-блобы + статич. таблица), Go `//go:embed`+`embed.FS`-эталон. Дизайн/survey — [research 2026-07-15](../research/2026-07-15-embed-dir-proposal.md) §3 вариант A. Реализация: обход папки в `embed_resolve.rs` + эмиссия N блобов+таблицы в `emit_c.rs` (прецедент single-embed) + std-тип `EmbeddedDir` (nv). Язык-меняющее → D412-амендмент в том же слиянии. Первый потребитель — флагман 187 (frontend→бинарь). Не блокер (single-embed покрывает MVP). | floating (lang / queued) | P3 |
