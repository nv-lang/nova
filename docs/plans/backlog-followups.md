...
| `[M-test-runner-tempdir-race-jobs]` | **OPEN P2 (наблюдалось многократно 2026-07-22).** Тест-раннер при `--jobs N` собирает `.c`→`.exe` многих фикстур в ОБЩИЙ temp-каталог; изредка два job'а конфликтуют на файле → `cannot open output file`/RUN-FAIL, при перепрогоне/изоляции — PASS. Недетерминизм (флака): сегодня a_q3_println_debug_record и другие спорадически краснели, перепрогон 528/0. ОПАСНО: маскирует реальные регрессии (интегратор чуть не принял флаку за regression polish — спасли перепрогон+бисекция) + может случайно покраснить CI. НЕ баг компилятора (он детерминирован) — инфра test_runner.rs. Fix: уникальный temp-dir per-job (изоляция), убрать гонку. Пост-релиз (не портит v0.1 — язык/компилятор детерминированы; для тегов интегратор перепрогоняет+подтверждает зелёное). | compiler-codegen/src/test_runner.rs | P2 |
