...
| `[M-runtime-sync-guard-consume-p67]` | **OPEN 2026-07-19 (найдено побочно при П18-приёмке, worktree `nova-p20018`, sonnet).** `std/src/runtime/sync_test.nv:108-113` (`test "Mutex: lock/unlock guard + with_lock"`, `consume guard = mu.lock(); ...; guard.unlock()`) роняет компилятор ЦЕЛЫМ процессом: `internal error … [P67-LEGACY] Ident 'guard' not in var_types / not a sum-variant — unknown type`. Воспроизведено СТАНДАЛОНЕ (`nova test std/src/runtime/sync_test.nv`, изолированно от остального `runtime`-каталога) на ДВУХ независимых бинарях — чистый неизменённый main (`d9af43662`, БЕЗ моих правок) и П18-мёрж-бинарь (`703b525b7`) — идентичная ошибка, та же строка `emit_c.rs` (номер плывёт по дрейфу кода: 53558 на main / 53619 на мёрже) → подтверждённо НЕ связано ни с `[M-tuple-fixarr-typedef-order]`-фиксом, ни с П18 UTF-8-правками (обе зоны не пересекаются с sync.nv/consume-guard путём). **Возможная РЕГРЕССИЯ (не расследовано глубже):** соседняя запись `[M-vec-ext-method-untyped-let-breaks-chain-dispatch]` (закрыта 2026-07-17) утверждает «δ0 GREEN на ... `std/src/runtime/{char,sync}_test.nv`» — если это верно на тот момент, регрессия влетела в окне `2026-07-17..2026-07-19`. `nova test std/src/runtime` (директория целиком) сейчас недоступен как приёмочная команда — folder-module CU тянет `sync_test.nv` вместе со всеми co-equal файлами каталога. Не расследовано: какой именно `consume`-паттерн триггерит (bare `consume guard = ...` без промежуточного типа?), связь с P67 сум-variant fallback (`emit_c.rs`, тот же класс, что `[M-198-f4c-1-into_str-primitive-chain-p67]`). | codegen emit_c (P67-LEGACY, consume-guard ident) | **P1** |
