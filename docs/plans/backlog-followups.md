...
| `[M-detach-consume-escape-unchecked]` | **OPEN P1 (находка владельца 2026-07-22 чтением examples/flagship/aggregator/src/main.nv).** `detach { ... f(stream) ... }` где `stream` — consume-тип (`TcpStream`) из ОБЪЕМЛЮЩЕГО scope: detach fire-and-forget может выполниться ПОСЛЕ выхода из scope-владельца stream → use-after-consume/use-after-free. Чекер НЕ ловит: `detach` не имеет consume-escape-анализа, который ЕСТЬ у `spawn consume` (D415 §4, types/mod.rs ~23335-23464). Флагман собирается 5/5 именно потому что дыра пропускает небезопасный паттерн. Fix: detach-consume-escape-анализ симметрично spawn — требовать `detach consume stream { ... }` (явный move) при захвате consume-переменной; голый `detach {}` с consume-захватом = E-ошибка. + переписать сам флагман main.nv на `detach consume stream`. **NEG-ФИКСТУРА (директива владельца 2026-07-22): взять ТОЧНЫЙ флагман-паттерн (`detach { ro _ = handle_connection(stream, routes()) }` с consume-stream из объемлющего scope) как `spec_tests/conformance/neg/detach_consume_escape_neg.nv` с EXPECT_COMPILE_ERROR — компилятор ОБЯЗАН отвергать такую работу с consume.** БЛОКЕР ТЕГОВ (consume-safety нельзя выпускать дырявой). | types/mod.rs (consume-analysis, класс D415) | **P1** |
...
| `[M-coalesce-return-fallback-unparsed]` | **OPEN 2026-07-23 (найдено пробой при инвентаре match→`??`).** D86 §«Fallback может быть» явно перечисляет **`return ...` для раннего выхода из enclosing fn** как легальный fallback оператора `??`, но парсер его отвергает: `ro x = find(n) ?? return None` → `error: unexpected `return` in expression`. Остальные три формы D86 работают (проверено на бинаре `6221c669b`): значение-fallback ✅ (Result и Option), `?? panic("…")` ✅, `?? throw MyErr { … }` ✅. Причина, вероятно, в том, что `return` — это Stmt, а не Expr в грамматике, тогда как `throw`/`panic` — выражения типа `never`. **Блокирует:** миграцию 15 из 69 сайтов ручного `match X { Some(v) => v, None => return None }` на канон `??` (`nova-http/src/client/wire.nv:119,120,128,147` и родня) — и, соответственно, лечится ЛИБО фиксом парсера, ЛИБО ретракцией `return`-формы из D86. **Решение владельца требуется:** чинить парсер (D86 остаётся как есть) или ретрактировать форму (тогда 15 сайтов остаются `match`'ем законно). | парсер (`return` как Expr) ИЛИ спека D86 (ретракт формы) | **P2** |
| `[M-manual-coalesce-lint-missing]` | **OPEN 2026-07-23 (владелец: «это нужно добавить в линт»).** Ручной `match X { Ok(v) => v, Err(_) => D }` / `{ Some(v) => v, None => D }` — дрейф от канона `X ?? D` (D86; амендмент 2026-07-07 ретрактировал `unwrap_or`-близнецов ИМЕННО в пользу `?? v`), линта нет. Инвентарь по `nova-http/src` + `std/src` + `examples`: **69 сайтов с identity-рукой** (`Ok(v) => v`) — nova-http 34, std 34, aggregator 1; по виду fallback: 50 значение · 15 `return` (заблокированы `[M-coalesce-return-fallback-unparsed]`) · 4 `panic`. **Отдельно 189 сайтов НЕ-identity** (`Ok(_) => false` и родня) — это НЕ `??`, а `.is_ok()`/`.is_err()`/`.map(f) ?? d`; линт обязан их различать, иначе 189 ложных срабатываний. Инфраструктура готова: реестр `ConvRule { id, summary, ast, text }` в `compiler-codegen/src/lints.rs` (8872 строки, 36 правил), ближайшие образцы того же класса — `W_MANUAL_CLAMP`/`W_MANUAL_MIN_MAX` (`conv_manual_min_max`, ~строка 5058), прецедент clippy `manual_unwrap_or`. Предлагаемый id — `W_MANUAL_COALESCE`. | линты (`lints.rs`, новое ConvRule) + миграция 69 сайтов | **P2** |
| `[M-assoc-const-out-of-body-syntax]` | **OPEN 2026-07-23 (решение владельца, D200-амендмент в том же слиянии — spec/decisions/02-types.md).** Каноническая форма associated-const меняется с in-body (`type X { const N = v }`) на **out-of-body `const Type.NAME <Тип> = <значение>`** — консистентно с `fn Type.new`/`fn Type @method` (всё привязанное к типу объявляется вне тела; тело = только layout инстанса), ближе к rustc. Парсер сейчас: `const Config.VERSION int = 2` → `expected type, got '.'` (квалифицированное имя в const-декларации не парсится). Нужно: (1) парсер принимает `Type.NAME` как имя const-декларации (module-scope, `export`-совместимо); (2) резолв кладёт в const-table типа (тот же путь, что in-body D200 §Семантика — zero-storage, `.rodata` symbol `Type_NAME`, `E_CONST_INSTANCE_ACCESS` на instance-доступ); (3) in-body форму либо оставить парситься (deprecated) до Ф.4, либо сразу отвергнуть — решить на Ф.0. Миграция сайтов = 0 (in-body нигде не used). Sum-type/generic — та же схема (`const Status.VERSION`, `const Box.TAG`; T-dependent `Box[int].SIZE` — синтаксис на Ф.4). **Ортогонально** `[M-d200-assoc-const-composite-value]` (составное значение — codegen, не парсер). **Блокирует** константный слой [222.5](222.5-respond.md) вместе с ним. | парсер + резолв (D200 out-of-body) | **P2** |
| `[M-d200-assoc-const-composite-value]` | **OPEN 2026-07-23 (вопрос владельца «каким синтаксисом заводится StatusCode.OK»).** D200 (associated const `const NAME T = …` в теле типа, доступ `Type.NAME`) реализован ТОЛЬКО для СКАЛЯРНОГО constexpr-значения. Составное (record-литерал `{ code: 200 }`) → codegen-ошибка «assoc const `StatusCode.OK` codegen failed: **non-constant exp**». Матрица проб (бинарь `3ff4cab73`): скаляр `const VERSION int = 2` → `Cfg.VERSION` ✅ PASS; `const OK StatusCode = { code: 200 }` → `StatusCode.OK` ❌ non-constant exp; newtype с телом `type StatusCode int { const OK = 200 }` ❌ parse (newtype не несёт `{}`-тело); module-level `export const OK StatusCode = { code: 200 }` ✅ PASS, но доступ голым `OK` (не `StatusCode.OK`, паттерн uuid.nv); через конструктор `= StatusCode.mk(200)` ❌ `E_CONST_NOT_CONSTEXPR`. **Следствие:** namespaced `StatusCode.OK` (как Rust `StatusCode::OK`) сегодня НЕ завести для value-обёртки. **Блокирует [222.5](222.5-respond.md)** — там в дизайне (§2, строки 23-25 `#impl(IntoResponse)`) уже написан `StatusCode.OK`. **Фикс:** дореализовать D200 codegen до составных constexpr-значений (record-литерал из констант) — финализация draft (D200 §Status «финализ. в Ф.4»), не новый дизайн; assoc-const emit'ит top-level C-symbol `Type_FieldName` в .rodata — нужен путь для struct-инициализатора, не только скаляра. | компилятор (assoc-const codegen, D200; `spec/decisions/02-types.md`) | **P2 (блокер константного слоя 222.5)** |
