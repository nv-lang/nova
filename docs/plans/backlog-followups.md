...
| `[M-187-sustained-live-tls-resource-death]` | **OPEN 2026-07-17 (НТ-марафон 10×-от-текущего, по просьбе владельца).** Сервер флагмана УМИРАЕТ НАВСЕГДА после **273 подряд** SSE weather-live прогонов (loadtest BLOCK 4 ×500): №1-273 зелёные → №274+ все 000 (BLOCK 5/6/7 красные по наследству; RESULT 288/230, все 230 = одна точка смерти). ДО марафона BLOCK 1-3 идеальны: 1202/1202 (12 комбо ×100, вкл. live×200) — серии по 100 НЕ добивают, только сплошной марафон. Класс: НАКОПИТЕЛЬНАЯ ресурсная деградация (НЕ concurrency-wedge — тот от одновременности и замитигирован): ~1470 суммарных прогонов / ~770 weather-live (≈3000 TLS-соединений) до смерти. Смерть на TLS-марафоне → подозреваемые: mbedTLS ctx/session leak (nova-tls `@close` — освобождается ли шим-сессия?), fd/сокеты (CLOSE_WAIT-накопление), fiber-слоты арены. **КОРЕНЬ ЛОКАЛИЗОВАН (2026-07-17, диагностический марафон + resource-монитор, 540 live-прогонов):** это УТЕЧКА ПАМЯТИ на live-TLS-пути, НЕ сокеты. Тренд (CSV, 18 замеров): working-set 18→264 МБ (≈0.45 МБ/live-прогон), private-commit ~11→770 МБ (≈1.4 МБ/live-прогон) — оба РАСТУТ ЛИНЕЙНО и неограниченно; OS-хендлы (301→плато 680), потоки (плато 100), TCP estab/closewait/timewait (1-10, колеблется, соединения ЗАКРЫВАЮТСЯ) — ВСЕ ограничены. → нативная heap/TLS-буферная утечка, не fd. Смерть №274 (пред. марафон) = исчерпание памяти ~940 МБ при ~670 суммарных live. Прайм-подозреваемый: mbedTLS session/ctx в nova-tls `TlsStream.@close` (шим-сессия `mbedtls_ssl_free`/`_config_free` не вызывается?) ЛИБО GC не собирает per-connection TLS-буферы (Boehm + FFI-owned mbedTLS-память вне GC-скана). Repro: `loadtest.ps1 -Iterations 700` (BLOCK 4, live) → смерть ~670; ИЛИ короткий `-Iterations 60` + монитор → slope ~1.4МБ/прогон подтверждается без смерти. Зона: nova-tls `stream.nv @close` + `native/tls_c_shim.c` (free-парность alloc). | Plan 187 / **nova-tls** (mbedTLS free-парность) | **P1** |
