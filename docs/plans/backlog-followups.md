...
| `[M-176-conformance-cu-map-closure]` | Codegen (pre-existing, exposed by Ф.2): добавление std.fs-импортирующего файла в folder-module `spec_tests.conformance` (компилируется как ОДИН большой CU, ~35k строк) ДЕТЕРМИНИРОВАННО ломает codegen map-closure в ДРУГОМ тесте (d102 named-args) → CC-FAIL `use of undeclared identifier 'f'` (closure-env `_env->f` эмитится как bare `f` в `NOVA_CLOS_CALL_ii(f, x)` при MapIter). Root — CU-content-dependent closure-env-access codegen (НЕ fs-код: d102 не импортит fs; убрал d323 → conformance снова 38/0; вернул → падает). **Обход:** d323 conformance-тесты вынесены в ОТДЕЛЬНЫЙ module `spec_tests/d323` (свой малый CU — PASS, как nova_tests/fs); main conformance чист. Фикс: детерминизировать closure-env-field-resolve в codegen (bare `f` → `_env->f` когда `f` — captured, не param). | Plan 176 Ф.2 / codegen | P1 |
