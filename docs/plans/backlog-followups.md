...
| `[M-boehm-large-buffer-retention-fiber-reuse]` | **OPEN 2026-07-17 (эскалация из закрытого по гейту `[M-187-sustained-live-tls-resource-death]`, worktree nova-tls-leak).** Explicit `gc.collect()` (`GC_gcollect()`, полный синхронный mark-sweep) НЕ возвращает `gc.heap_size()` к плато под нагрузкой «много коротких fiber'ов за итерацию, каждый churn'ит один относительно крупный (КБ-масштаба) GC-буфер» — Boehm считает буферы reachable СРАЗУ ПОСЛЕ полной сборки (настоящий retention, не «редко собирает»). Гипотеза (не доказана до конца): conservative-scan false-positive retention через переиспользуемые fiber-стеки (`compiler-codegen/nova_rt/fiber_arena.c` — протухшие байты в переиспользованном stack-слоте МОГУТ случайно выглядеть как указатель на объект прошлой итерации). Эмпирическая опора: размер буфера ПРОПОРЦИОНАЛЕН скорости роста (16КиБ→2КиБ снизил slope ~56КБ/итер→~19КБ/итер на loopback-repro) — типично для conservative-GC false-positive economics. Repro-рецепт (temp-инструмент не сохранён, восстановить заново): loopback client+server TLS/TCP-пара в одном .nv-процессе, `supervised`+2×`spawn`+`Channel` НА КАЖДУЮ итерацию, 3000 итераций; TCP-only контроль (тот же каркас, без крупных буферов) — плато; ЛЮБОЙ TLS-путь — линейный рост, не устраняемый `gc.collect()` каждые 50 итераций. Зона: `compiler-codegen/nova_rt/fiber_arena.c` + `alloc_boehm.c` — компилятор/рантайм, не конкретный пакет. Не блокирует прод (офиц. TLS-гейт CLEAN под реальной сетевой нагрузкой после отдельного nova-tls фикса), но систематический риск для ЛЮБОГО high-churn per-connection/per-request паттерна. Требует opus-разведки GC/fiber-arena слоя. | floating (GC/рантайм) | P2 |
| `[M-parfor-capture-callee-name-collides-std-local]` | **OPEN 2026-07-20 (найдено на examples/mini_aggregator.nv).** Захват-анализ spawn-ctx у `parallel for` резолвит СВОБОДНЫЙ идентификатор-ИМЯ ВЫЗЫВАЕМОЙ module-fn по имени БЕЗ скоупа: если у ЛЮБОЙ другой функции CU есть ЛОКАЛ с тем же именем (репро: fn `probe` vs `uint64_t probe = mag` — локал std float-format движка), в ctx-структуру эмитится поле этого чужого типа (`uint64_t probe;`) и присваивание `_nova_spawn_1_ctx->probe = probe;` из области, где имени нет → CC-FAIL `use of undeclared identifier`. Сам вызов при этом эмитится ПРАВИЛЬНО (mangled прямой вызов в drain) — поле ctx чисто паразитное. Родня 196.6 (per-name глобальные карты без per-fn скоупинга). Репро: examples/mini_aggregator.nv @ 2bc5cd8bb^-форма с fn probe (переименована в ask_source = обход); минимальный: standalone CU, module-fn с именем, совпадающим с локалом любой std-fn, вызов из parallel-for-тела. Зона: capture-emission (emit_c spawn-ctx сбор свободных имён). | codegen / capture-analysis | P2 |
| `[M-vr-binop-wrapper-decl-order-standalone-cu]` | **OPEN 2026-07-20 (найдено на examples/mini_aggregator.nv).** В standalone-CU (`nova build examples/mini_aggregator.nv`) выражение `t0 + 120.to_millis()` (Monotonic + Duration, value-record binop) эмитит обёртку `nova_vr_binop_Nova_Monotonic_method_plus(...) { return Nova_Monotonic_method_plus(&a, b); }` ДО объявления `Nova_Monotonic_method_plus` → implicit int → CC-FAIL `returning 'int' from a function with incompatible result type 'NovaValue_Monotonic'`. В много-файловом CU (examples/flagship/aggregator: `ro shared_deadline Monotonic = t0 + budget`) порядок объявлений другой и всё собирается — чистая ошибка упорядочения деклараций standalone-эмиссии. Обход: `supervised(timeout: ...)` вместо `deadline: t0 + d` (так в mini). Сродни [M-tuple-fixarr-typedef-order] (топо-порядок эмиссии, закрыт 134248143) — вероятно, тот же класс решения: единая точка splice для vr_binop-обёрток после деклараций методов. | codegen / emit-order | P3 |
| `[M-p67-path-call-const-receiver-method-ice]` | **OPEN 2026-07-20 (найдено на examples/mini_aggregator.nv, той же волной что parfor-capture/vr-binop выше).** `const BUDGET_MS int = 120` + `BUDGET_MS.to_millis()` → ICE `[P67-LEGACY] Path call return type unknown for method=to_millis — checker must annotate` (emit_c.rs:52880). ЛИТЕРАЛ `120.to_millis()` работает; const-идентификатор-ресивер у generic-extension (`fn[T Ints] T @to_millis()`) — падает. Та же семья «checker must annotate», что `[M-flagship-monotonic-now-bare-binding-ice]` — но ТОТ закрыт (попутно M-176 Path→Member фиксом 67717dcb1/747a79c65, заморожен фикстурой examples/flagship/aggregator/regressions/monotonic_now_bare_binding/); const-ресивер-shape — ЖИВОЙ остаток семьи, эта строка = его home. Обход в mini: литерал вместо const. | codegen / checker-annotate (P67-LEGACY семья) | P3 |
...
| `[M-protocol-box-callarg-vtable-incomplete]` | **OPEN 2026-07-20 (найдено попутно при POS-регрессионном тестировании фикса `[M-checker-protocol-typed-arg-any-bypass]`, sonnet, worktree `nova-protoany`).** Codegen's box/vtable-коэрсия конкретного типа в protocol-типизированную позицию (`NovaBox_<Protocol>`/`NovaVtable_<Protocol>`) ломается, когда коэрсия происходит В ПОЗИЦИИ АРГУМЕНТА ВЫЗОВА (`fn f(x SomeProtocol); f(concrete_value)`) — `clang: error: passing 'Nova_X*' to parameter of incompatible type 'NovaBox_Proto'` / `no member named '<embedded-method>' in struct NovaVtable_Proto`. Два независимых репро (оба подтверждены на НЕИСПРАВЛЕННОМ `main`-бинаре — предсуществующий баг, не регрессия): (1) protocol с `use`-embed (`use Base` + own method) — vtable для outer-протокола не содержит embedded-метод; (2) NAMED protocol БЕЗ embed, 2 метода, `mut`-параметр — та же ошибка несовместимого типа. Контраст: `spec_tests/conformance/standalone/f2_protocol_dispatch_method_survives.nv` — та же box/vtable-механика через LET-КОЭРСИЮ (`mut g Greeter = sp`, single-method) РАБОТАЕТ; гипотеза (НЕ подтверждена до конца): implicit box-коэрсия реализована для let-binding target-позиции, но не (полностью) для call-argument target-позиции — требует отдельной codegen-инвестигации (emit_c.rs, protocol-box emission site). Репро-файлы не сохранены в дереве (временные, удалены после подтверждения — воспроизвести по описанию тривиально). Зона: codegen `emit_c.rs`, protocol-box/vtable emission. | codegen / protocol-dispatch | **P2** |
...
| `[M-176-consume-through-result-match]` | Checker: must-consume (D133) НЕ отслеживается когда consume-значение извлечено из `Result` через match-паттерн (`match File.open(p){ Ok(f)=>{…} }` — незакрытый `f` НЕ даёт `D133-not-consumed`). Enforced для consume-**параметров** и прямых `consume x = Ctor()` (BufWriter). Затрагивает `File` (176) И `TcpStream` (net, Result[TcpStream]) одинаково — pre-existing. Neg-тесты 176 демонстрируют D133 через consume-param форму. Фикс: протянуть linearity-tracking через pattern-binding payload consume-типа. | Plan 176 Ф.2 / checker | P1 |
| `[M-196-freefn-arity-overload-default-ret-mismatch]` | **OPEN (найдено 2026-07-16, sonnet, Plan 196 Facet C волна-продолжение — попутно, вне выбранного атома).** Two free-fn overloads sharing a name, where the SHORTER one's arg-count coincides with the LONGER one's REQUIRED (non-default) arg-count — `fn f(x int) -> int` + `fn f(x int, tag str = "d") -> str` — make `f(5)` genuinely arity-ambiguous (both `bind_call_args` successfully). The checker's own overload resolver (`types/mod.rs::f1_check_call`, `Ident(n)` arm, ~10832-10889) already recognizes this: when ≥2 candidates are BOTH arity+type-compatible it deliberately does NOT write `resolved_callees` (comment: "`[M-172.1-free-fn-multi-overload-ambiguous]`: 0 or ≥2 compat → codegen"), deferring to codegen. **The actual call-site DISPATCH in codegen picks correctly** (mangled-name-suffix routing lands on the right, exact-arity C function — verified in generated C). But the SEPARATE **return-type inference** used to codegen a CONTAINING expression (e.g. `f(5) == 5`, needs to know `f(5)`'s C type to choose int-`==` vs `Nova_str_method_equal`) goes through the frozen legacy path (`infer_call_ret_c`, name-keyed, arity-blind) and silently derives the WRONG overload's return type — `nova_str` instead of `nova_int` for `f(5)`. Repro pinned CC-FAIL (str/int type clash in the emitted comparison — lucky catch: if both overloads returned type-compatible C types instead, this would be a SILENT wrong-answer, not a compile error). Root class = the free-fn twin of the ALREADY-CLOSED `[M-vec-new-static-arity-overload]` (that fix — `pick_static_params` in `callnorm.rs` + turbofish-per-overload mono in `emit_c.rs` — covers STATIC methods only; `sigs.free` in `callnorm.rs` still unconditionally drops any name with >1 registered signature, `collect_sigs` ~L138-140). Needs either (a) the checker's ambiguity gate promoted to a diagnosed `E_AMBIGUOUS_OVERLOAD` for this narrow arity-collision shape (cleaner: turns a latent miscompile into an honest error) or (b) extend the `pick_static_params`-style per-call-site arity disambiguation to `sigs.free` + thread the picked candidate's return span through the SAME return-type channel Channel 1 already reads (`resolved_callees`→`fn_ret_by_span`), bypassing the frozen `infer_call_ret_c` re-derivation for this case. Not fixed this wave (different mechanism than the chosen atom — checker `f1_check_call` + frozen `infer_call_ret_c`, not `callnorm.rs`'s Block-rewrite). No live corpus instance found (grepped std/examples for same-name free-fn pairs with this exact arity collision — none) — pinned via isolated repro, not committed as a red conformance fixture (would need `EXPECT_COMPILE_ERROR`/negative harness to land safely without leaving a real CC-FAIL in the tree). | Plan 196 Facet C (callnorm/argbind) | P1 |
...
| `[M-175-value-record-const-ref]` | Codegen: `value`-record module-const, использованный КАК ЗНАЧЕНИЕ (`Duration.ZERO`), эмитит undeclared `Duration_ZERO` — `emit_const_decl` даёт символ `ZERO`, а ref-путь `Type.CONST` мангл-имя `Type_CONST`; const-МЕТОД (`Duration.ZERO.is_zero()`) инлайнится в `nova_fn_Type_CONST_method` (объявлен, тело не эмитится → link-error). Value-record-специфичный gap value-const emission/DCE, НЕ time-специфичный (обнаружен Plan 175 Ф.1b). **Обход:** `Duration.from_nanos(0)` вместо `Duration.ZERO`. Фикс: эмитить value-const как `static const NovaValue_X CONST = {…}` + выровнять ref-мангл + reachability-root на const-value-use. | Plan 175 Ф.1b / codegen | P2 |
...
| `[M-176-io-forward-bounded-generic]` | Checker: форвард bounded-generic параметра в другую generic-fn с тем же bound (`fn f[R Read](r){ read_to_end(r) }`) → «R does not satisfy Read» (bound не проносится через форвард). **Обход:** инлайн циклов в io-хелперах. Фикс: проносить bound R:Bound при передаче R-типизированного аргумента в `[R2 Bound]`-fn. | Plan 176 Ф.1 / checker | P2 |
...
| Маркер | Суть | Home | Pri |
|---|---|---|---|
| `[M-p200-17-remaining-1-fmtbuf]` | **OPEN 2026-07-20 (сузился из `[M-p200-17-remaining-3]`; sonnet-заход, worktree `nova-p17rest`/ветка `p200-17-rest`).** `base64`/`handlers` конвертированы и слиты (см. `docs/history/simplifications-closed.md`) — счёт Пункта 17 теперь 8/9. Остался ТОЛЬКО `std/src/runtime/fmt_buf.nv`: split на `fmt_buf/{core.nv,core_test.nv}` был выполнен и export-поверхность/приватность СОХРАНЕНЫ корректно (verified: `core_test.nv` резолвит module-private `int_fmt`/`FmtSpec`/`bool_fmt`/`char_fmt`/`fmt_f64`/`f64_fmt_into` из `core.nv` без ошибок), НО `nova test std/src/runtime/fmt_buf/core_test.nv` CODEGEN-FAIL с `undefined identifier int_fmt_into/f64_fmt_shortest_into/f32_fmt_shortest_into` внутри СОВСЕМ ДРУГОГО файла — `string_builder.nv` (не импортируется fmt_buf вообще). Root-cause НАЙДЕН и это НЕ ошибка конверсии — компиляторный баг import-резолвера, см. новый `[M-imports-entry-folder-module-self-cycle-empty-exports]`. Split РЕВЕРТНУТ в этой волне (fmt_buf.nv остался единым файлом с 8 инлайн-тестами) — конвертировать нельзя, пока баг не починен. | Plan 200 П17 | **P3** |
| `[M-imports-entry-folder-module-self-cycle-empty-exports]` | **OPEN 2026-07-20 (найдено при `[M-p200-17-remaining-1-fmtbuf]`, Explore-агент, репро воспроизводимо).** Баг в `compiler-codegen/src/imports.rs::resolve_imports_inline_ex`: когда CU-entry — файл folder-модуля A (`nova test A/core_test.nv`), `entry_key` (=A) кладётся в `in_progress` (imports.rs:723) и НЕ убирается до конца всей функции (imports.rs:1133), включая drain отложенных `pending_peer_preludes` (imports.rs:1075-1105). Если ЛЮБОЙ другой файл, транзитивно затянутый в тот же CU через auto-injection (напр. `.ptr()` → `needs_vec_injection`, imports.rs:572/582-591/998-1008 → `import std.collections.vec` → его peer'ы не `#no_prelude` → `pending_peer_preludes` → `std/prelude/collections.nv:212` `export import std.runtime.string_builder` → string_builder.nv), сам импортирует модуль A обычным `import` (`string_builder.nv:28` импортирует `std.runtime.fmt_buf`) — `resolve_one`'s cycle-guard (imports.rs:1646-1650, `if in_progress.contains(&module_key) { return Ok(()) }`) молча возвращается БЕЗ заполнения `visible_acc`, т.е. импортёр не видит НИ ОДНОГО экспорта модуля A в этом CU → `undefined identifier`. Комментарий imports.rs:1131-1132 явно постулирует инвариант «entry's exports not cached... entry is never dedup'd as an import by others in the same resolve call» — этот инвариант ЛОЖЕН для сценария «entry — folder-модуль, чьи экспорты нужны ДРУГОМУ файлу, транзитивно затянутому в тот же CU auto-injection'ом (не прямым импортом)». Репро: `std/src/runtime/fmt_buf.nv` (сплитнуть в `fmt_buf/{core.nv,core_test.nv}`, core_test.nv зовёт `buf.ptr()`) + `nova test std/src/runtime/fmt_buf/core_test.nv`. НЕ регрессия сегодняшней волны — баг предсуществующий в import-резолвере, просто не встречался раньше (никакой из 6 уже слитых П17-конверсий не имел внешнего cross-module потребителя своих экспортов). Блокирует `[M-p200-17-remaining-1-fmtbuf]`. | `compiler-codegen/src/imports.rs` (resolve_imports_inline_ex/resolve_one) | **P2** **РАСШИРЕНО интегратором 2026-07-20: касается и FILE-модуля как entry** — `nova test std/src/runtime/fmt_buf.nv` красный тем же undefined-каскадом в string_builder.nv (цикл существует с кросс-импорта мостов `3a257094f`; до него fmt_buf никто не импортировал — потому не всплывало). |
| `[M-standalone-out-of-tree-interp-sb-typedef]` | **OPEN 2026-07-20 (найдено при верификации zero-alloc appendов).** Компиляция ОДИНОЧНОГО .nv-файла ВНЕ дерева проекта (напр. `nova test C:/…/scratchpad/f.nv`) с ЛЮБОЙ интерполяцией, требующей StringBuilder (`"${1.5}"` — f64 достаточно), даёт CC-FAIL: `must use 'struct' tag to refer to type 'Nova_StringBuilder'` + `initializing nova_str with int` — typedef SB не эмитится/не в порядке для out-of-tree standalone-CU. Дискриминатор: тот же файл БЕЗ интерполяции компилится; файлы ВНУТРИ дерева — компилятся. Репро — 4 строки (`test { ro a = 1.5; assert("${a}" == "1.5") }` из temp-каталога). НЕ регрессия zero-alloc волны (воспроизведён без её кода). UX-дефект быстрых проб. | codegen standalone-CU / emit-порядок | **P3** |
| `[M-d55-const-bytes-lit-not-constexpr]` | **OPEN 2026-07-18 (честный под-маркер, найден при закрытии `[M-d55-str-literal-coercion-name-gated]` — см. `docs/simplifications.md`).** Str-литерал→`[]u8` коэрсия (D55 amend) теперь типо-направленная и покрывает call-arg/`let`/return/`[][]u8`-element — НО scope-local `Stmt::Const`/module-level `Item::Const` с явной `[]u8`-аннотацией (`const c []u8 = "hi"`) НЕ покрыты. Причина не в str-литерал-гейте: `const`-инициализатор эмитится через `emit_const_expr_typed`/`emit_const_expr` (`emit_c.rs`) — ОТДЕЛЬНАЯ constexpr-only ветка, не проходящая через `emit_expr_with_target_type` (choke-point, который несёт всю остальную D55-литерал-коэрсию) вообще. `[]u8` — heap-аллоцированный `Vec`-указатель (len/cap/data), не C-scalar-constexpr — представимость `[]u8`-значения как compile-time C-константы (initializer) отдельный архитектурный вопрос, шире одного str-литерала (тот же вопрос встал бы для int-литерала на []u8 через `as`, будь такое легально). Не тихий пропуск — явно задокументирован. | D55 / codegen (`emit_const_expr_typed`) | P3 |
| `[M-slice-ext-receiver-for-in-elem-type]` | **OPEN 2026-07-18 (найден интегратором на живом коде, ФИКС-ВОЛНА ЗАПУЩЕНА тем же днём — p-fix-slice-ext-forin).** Итерация голого ресивера `for r in @` внутри slice-расширения (`fn []TaskResult @to_report`) роняет тип элемента в nova_int-fallback: `r.elapsed_ms` → CC-FAIL «member reference base type nova_int», match по `r.status` резолвит теги в ЧУЖИЕ суммы (NOVA_TAG_OnceState_Done/NetError_Cancelled вместо TaskStatus) — второе окно гадает при пустом канале. Обход-носитель: examples/.../domain/domain.nv (аннотированный биндинг `ro results []TaskResult = @`), снимается фикс-волной. Родня: [M-vec-ext-method-untyped-let-breaks-chain-dispatch] (f3_check_member_ctx). | checker types/mod.rs, for-in elem-инференс SelfAccess | **P1** |
...
- **[M-180-serde-field-attributes]** OPEN (NEW, Plan 180 Ф.6) — field/variant-level `#serde(rename/rename_all/skip/default/flatten/deny_unknown_fields/alias)` synth-consumption (Ф.3b). The AST/grammar are general (parsed by `parse_serde_attr`), so these currently reject as `E_SERDE_BAD_ATTRIBUTE` (not silent); adding consumption is a self-contained follow-up. Record-DTO round-trips on canonical names without it.
