...
| `[M-p67-path-call-const-receiver-method-ice]` | **OPEN 2026-07-20 (найдено на examples/mini_aggregator.nv, той же волной что parfor-capture/vr-binop выше).** `const BUDGET_MS int = 120` + `BUDGET_MS.to_millis()` → ICE `[P67-LEGACY] Path call return type unknown for method=to_millis — checker must annotate` (emit_c.rs:52880). ЛИТЕРАЛ `120.to_millis()` работает; const-идентификатор-ресивер у generic-extension (`fn[T Ints] T @to_millis()`) — падает. Та же семья «checker must annotate», что `[M-flagship-monotonic-now-bare-binding-ice]` (задокументирован в examples/flagship/aggregator/src/app/aggregate.nv, строка ~191 — БЕЗ своей backlog-строки; эта строка = home обоих repro-shapes семьи). Обход в mini: литерал вместо const. | codegen / checker-annotate (P67-LEGACY семья) | P3 |
