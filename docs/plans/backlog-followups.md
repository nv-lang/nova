...
| `[M-176-xmod-payload-variant-ctor]` | Checker: cross-module конструкция payload-варианта литералом (`io.SeekFrom.Start(5)`, `io.ErrorKind.Other(5)` из другого модуля) → `[P67-LEGACY] Path call return type unknown for method=Start` (не аннотируется возвратный тип variant-конструктора). Pattern-match не затронут. **Обход:** статические конструкторы (`SeekFrom.start/end/current`). **Шире, чем «cross-module» (найдено Ф.4, 2026-07-09):** тот же ICE (`Path call return type unknown (no method segment)`) триггерится и В ТОЙ ЖЕ декларирующей модуле, когда метод вызывается ПРЯМО на свежесконструированном enum-значении без промежуточного `ro`-байндинга — даже на nullary-варианте (`NetError.ConnectionRefused.to_error_kind()`), не только на payload-ctor. Существующий обход (`ro x = Type.Variant(...)` / `ro x = Type.Variant` перед `.method()`) уже применяется по всей кодовой базе (`std/net/error_test.nv` и т.д.) — новый код (`std/net/d302_neterror_iokind_test.nv`) следует тому же паттерну. Фикс: аннотировать return-type qualified variant-ctor (payload И nullary) в checker до chained method-call, не только при отдельном байндинге. | Plan 176 Ф.1/Ф.4 / checker | P2 |
...
| `[M-175-type-const-max-shadows-builtin]` | Checker/codegen (обнаружен Plan 175 Ф.1c, 2026-07-06): user type-const с именем `MAX`/`MIN` (`export const MAX Duration = {…}`) **шэдоуит builtin numeric `.MAX`/`.MIN`** в type-set-bound generics. Repro: с `Duration.MAX` в CU, `fn[T Ints] f(x T) -> bool => x == T.MAX` (spec_tests d310) мис-типизирует `T.MAX` (int) как `NovaValue_Duration` → binary-`==` уходит в value-record structural-eq → `((x).nanos == (INT32_MAX).nanos)` CC-FAIL «member reference base type 'int' is not a structure». VALUE резолвится верно (INT32_MAX), мис-аннотирован ТИП. **Блокирует** публичные `Duration.MAX`/`Duration.MIN`-консты (Plan 178 `@timeout(Duration.MAX)`). **Обход:** консты НЕ введены; saturation-границы через internal `i64_max()`/`i64_min()`-fn (D317-функциональность полная). Фикс: builtin numeric `.MAX`/`.MIN` должен иметь приоритет над user type-const для numeric type-params в member-const-резолюции checker'а (172-зона, owner-gated). | Plan 175 Ф.1c / checker | P2 |
...
| `[M-vec-new-static-arity-overload]` | ✅ **РАБОТАЕТ (vec-sweep, 2026-07-06)** — риск из задания НЕ материализовался. `Vec[T].from_raw_parts(ptr, len, cap)` переименован в перегрузку `Vec[T].new(ptr *T, len int, cap int) -> Self` (арность-диспетчинг 0-арг `new()` vs 3-арг `new(ptr,len,cap)` на generic-типе) — скомпилировалось и прошло `spec_tests/conformance` (55/1, дельта 0 к baseline) без изменений в диспетчинге. Прецедент `[M-153.6-collect-static-generic]` в этом конкретном случае не сработал (арность, не method-level generic — вероятно другой код-путь). | vec-sweep / codegen static-overload | ✅ DONE (verified working) |
...
- ~~**[M-178-servernet-live-net-substrate-segfault]**~~ ✅ **CLOSED 2026-07-11 (оригинал — segfault).** Исходный детерминированный `SIGSEGV` живого loopback-сокета закрыт **Планом 183** (`with Net = real_net()` — тест `std/http/servernet/rt/handle_connection_smoke.nv` теперь несёт эффект-хендлер, стр. 37). Репро (release+dev C-codegen) даёт `PASS` детерминированно: соло, ×300 подряд, 6 волн × 24 параллельно, ×150 под насыщением всех 16 ядер — **0 segfault / 0 hang** (924 прогона суммарно). Остаточный `plan83_12` P67-LEGACY ICE (`method=bind` direct `.bind().is_ok()`; `.unwrap()` на `Result[_,NetError]`) — ОТДЕЛЬНЫЙ codegen-долг, идёт в `[M-net-redesign-owner-directive]`. Разведочная гипотеза о teardown-hang → см. `[M-net-close-teardown-hang]` ниже.

- ~~**[M-net-close-teardown-hang]**~~ ✅ **CLOSED 2026-07-11 (root-cause устранён аудитом close-пути; hang-репро НЕ подтверждён на этой машине — честная оговорка).** Разведка подозревала: `handle_connection_smoke` иногда ЗАВИСАЕТ в teardown (`nova_supervised_drain_main_scope → nova_runtime_shutdown → nova_evloop_close`) ПОСЛЕ печати `PASS`, гипотеза — cross-thread close мигрировавших work-stealing'ом волокон через старый `nova_loop_defer_close`. **Детерминированный hang НЕ воспроизведён** (0 hang / 924 прогона: соло/параллельно/под насыщением CPU, dev+release). Однако **аудит close-пути вскрыл РЕАЛЬНЫЙ ordering-дефект** ровно в подозреваемой зоне: `nova_runtime_shutdown` (`compiler-codegen/nova_rt/runtime.c` per-worker cleanup) звал `uv_loop_close(&w->loop)` **ДО** финального `nova_loop_drain_closes`/`nova_loop_drain_calls`. Отложенный close/call-джоб (net.c `TcpListener/TcpStream.close()` через `nova_loop_defer_close`; cross-thread net-op issue через `nova_loop_defer_call`, Plan 183 / `[M-183-net2-loop-affinity-cross-thread-op]`) обычно обслуживается собственным `while(!stop)`-циклом воркера через `_worker_async_cb`. Если `w->stop` взводится в узком окне между постановкой джоба в очередь и следующей итерацией воркера — джоб остаётся в очереди на момент join. Прежний порядок (drain ПОСЛЕ close) звал `close_cb`/`fn` слишком поздно: `uv_run` для этого loop больше никто не крутит → `close_cb` не срабатывает → живой `uv_tcp_t`/`uv_udp_t` (и его сокет-fd) **утекал НЕЗАКРЫТЫМ** (`uv__finish_close` не выполняется). Это лик, а на потенциально-медленных путях (EBUSY-спин в `nova_evloop_close`, до 100 попыток) — правдоподобный источник наблюдённого разведкой TIMEOUT. **Fix (`7bd766963`):** дренаж call+close очередей + bounded-прокачка loop (≤64 тика, `uv_run` NOWAIT) ДО `uv_loop_close`, пока loop ещё открыт и воркер уже joined (single-thread, гонки нет). Ограниченное число тиков — застрявший хендл остаётся ликом (best-effort), не превращается в hang shutdown. Гейты: cargo build чисто; conformance 91/0; репро ×30+ post-fix = 0 hang/segv; err173 28/0; std/net+std/http keystone δ0 (единственный FAIL — pre-existing `[M-tls-handshake-test-panic-undefined-multifile]`, не связан). **ОГОВОРКА (плейбук §4а Lesson #9):** «verified 0 hang» — для ЭТОЙ машины/сборки (16 ядер, Windows, clang, dev+release); детерминированного hang-репро получить НЕ удалось, поэтому фикс закрывает НАЙДЕННЫЙ ordering-дефект, а не эмпирически-локализованный hang. Если hang проявится на другом железе — root cause искать заново (state-dump `NOVA_WATCHDOG_DUMP_SECS`), не считать этот маркер гарантией.
...
- **[M-174.1-to-str-name-collision-codegen-bug]** (NEW 2026-07-08, OPEN) Диспатч facade-метода `[]u8 @to_str()` в CU, где есть ЧУЖОЙ `T @to_str()` (D410-конвенция: NetError/Path/Url/SerError/IoError…), уводит вызов в чужое C-тело (репро: свежий минимальный тип + fn; лечится ТОЛЬКО переименованием). Обход в std: same-body алиас `@decode_utf8()` на коллизионных сайтах (tcp.read_text, io read_to_string/lines, fs read_text, Path, serde, encoding/url). Как чинить: канал разрешения метода по конкретному ресиверу, не по имени (зона 172.12 typed-IR). Priority: P2 (обойдено, но класс жив для user-кода).
