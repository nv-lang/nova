...
| `[M-194-verified-mode-gate]` | Найдено при разведке 194 волн 2-4 (2026-07-15). Режим `--contracts=verified` («не доказал не-`#debug` контракт статически → **compile-error**») — флаг ПРИНИМАЕТСЯ (A2.1), но семантика НЕ реализована: сейчас `verified` ведёт себя как `optimized` (недоказанное уходит в рантайм, не в ошибку сборки). Нужна новая инфра «Z3-fail→compile-error» поверх существующей команды `nova verify-contracts` (SMT JSON-диагностика). Владелец 2026-07-15: `checked`+`optimized` делаем в волне 194, `verified` — отдельным атомом позже. | Plan 194 (атом-followup) | P2 |
