...
| `[M-187-http-serde-setcookie-serialize-collision]` | **OPEN 2026-07-14 (187 прогон А, sonnet).** Генерик `json_encode[T]` (std.encoding.serde) резолвит `.serialize()` НЕ по типу: в одном CU с пакетом `http` коллизия с `http.cookie.SetCookie.serialize()` ломает ЛИНКОВКУ любого `#impl(Serialize)`-типа. Семья `[M-178-server-typed-body]`/D84 method_overloads. Обход в примере: main.nv рендерит JSON вручную (report_json.nv остался typed). Блокирует «typed-serde в http-CU» повторно. Repro: examples/flagship/aggregator (ветка flagship-187). | floating (compiler-codegen resolve/mono) | P1 |
