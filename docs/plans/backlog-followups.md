...
| `[M-187-supervised-nested-fiber-slot-race]` | **OPEN 2026-07-14 (187 прогон А, РАНТАЙМ).** Вероятностная гонка fiber-scheduler'а на вложенных `supervised`: HTTP-сервер примера ЧАСТО виснет на 2-3-м последовательном запросе (NOVA_RUNTIME_DUMP: застрявший слот); единственный запрос к свежему процессу — 100% надёжен. Смягчено, не закрыто. Фикс в nova_rt/fibers.h; шаблон инвестигации — reference-mn-race-case-study. Блокер непрерывной работы флагман-сервера. | floating (runtime M:N) | P1 |
| `[M-187-errorkind-parsejsonerror-variant-collision]` | **OPEN 2026-07-14 (187 прогон А).** Codegen: два sum-типа (`std.io.ErrorKind` / `std.encoding.json.ParseJsonError`) с одноимённым вариантом `UnexpectedEof` РАЗНОЙ арности коллидируют в одном CU (семья D381 bare-variant/арность). Обход: изоляция чистых данных в свой folder-module (src/domain), чтобы report_json не тянул std.net. | floating (codegen D381-семья) | P2 |
| `[M-187-nested-spawn-scope-var-cc-fail]` | **OPEN 2026-07-14 (187 прогон А).** Codegen: `spawn`, вложенный в `spawn` под одним `supervised`, даёт CC-FAIL. Обход: accept-loop в одном spawn, соединения последовательно (усугубляет видимость slot-race выше). | floating (codegen spawn-scope) | P2 |
