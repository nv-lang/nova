...
| `[M-138.2-flip-erased-base-body-mono]` | **Ф.0-final блокер #1 (флип откатан 2026-06-11).** Под универсальным флипом erased `fn[T] []T @m` base body (T НЕ в `current_type_subst`) лоуэрит голый type-param `inner` в dangling `Nova_<T>*` → schema-less `Vec____Nova_T_p` mono. Legacy NovaArray erase'ил unresolved-element в `nova_int`; нужна принципиальная erased-element-mono для Vec. Spike-fix (emit_c.rs ~5110) = heuristic string-инспекция `Nova_`-prefix + отсутствие в schemas → форс `nova_int`; работало, но хрупко → откатан. Production-fix = principled erased-element-mono (как у NovaArray). | plan-138.2 Ф.0-final | P1 |
| `[M-138.2-flip-value-pos-arraylit-vec-gate]` | **Ф.0-final блокер #2 (= ранее `[M-138.2-value-pos-arraylit-vec-gate]` из Ф.0b; флип откатан 2026-06-11).** `[]T` в value-позиции (`[]T.with_capacity`/`.new`/array-literal/`.push`) НЕ Vec-gate-aware — D38 array-static-method dispatch (emit_c.rs:19863-19925) безусловно → `nova_array_new_<suffix>` (int64-erased NovaArray), БЕЗ `contains_key("Vec")` guard, тогда как типовая позиция (поле/return) уже типизированный Vec под gate (emit_c.rs:2074). Под флипом рассинхрон универсален (Ф.0b локализовал в hashmap → `new_buckets` на явный `Vec[Slot]`). Production-fix: Vec-gate'нуть value-position dispatch (graceful #no_prelude fallback). | plan-138.2 Ф.0-final | P1 |
| `[M-138.2-flip-array-ext-vec-recv-routing]` | **Ф.0-final блокер #3 (флип откатан 2026-06-11).** User `fn[T] []T @method` расширение регистрирует sentinel под `("[]T", method)` + erased под `("NovaArray_<X>", method)`, но НЕ под Vec C-именем (`Vec____<X>`). Под флипом Vec-receiver промахивается по `(rt, method)` lookup → `__mono_method__[]T__method` утекает undefined в линкер. Spike-fix добавил Vec-receiver alt-key fallback на `("[]T", method)` в 3 mono-routing местах (emit_c.rs ~22041/~22084); работало для проверенных, не покрывало полный element-class матрикс → откатан. Production-fix = Vec-receiver routing для `[]T`-extension методов (класс 138.4 G-A/G-B, но для extension base bodies). | plan-138.2 Ф.0-final | P1 |
