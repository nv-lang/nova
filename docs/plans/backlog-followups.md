...
| Маркер | Суть | Home | Pri |
|---|---|---|---|
| `[M-method-reference-value-codegen-gap]` | **OPEN 2026-07-23 (эмпирическая проба интегратора по вопросу владельца про `int.@to_seconds`).** Форма `Type.method` КАК ЗНАЧЕНИЕ (method reference: `apply(int.to_str, 5)` при `fn apply(f fn(int) -> str, …)`) **проходит чекер**, но падает в C: `use of undeclared identifier 'int_to_str'` — кодоген эмитит имя, которого не существует, вместо референса на мономорфизированный метод. Дискриминатор: свободная fn как значение РАБОТАЕТ (`twice(inc, 1)` → PASS 1/0), т.е. инфраструктура функций-значений есть; дыра именно в методах. **Класс: чекер пропускает то, чего кодоген не умеет** — как минимум нужна честная диагностика (`E_METHOD_REF_UNSUPPORTED`), как максимум — доделать эмиссию (тогда `a.map(int.to_seconds)` вместо `a.map(|x| x.to_seconds())`, и это общее решение для filter/sort/итераторов, а не только Option). Репро — 3 строки. | codegen / method-values | **P2** |
