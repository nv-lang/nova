...
| `[M-atomicint-record-field-typedef-collision]` | **OPEN (найдено 2026-07-11 при D415 detach-миграции err173_2/detach_effect_ok_test.nv).** Опаковый extern-тип с собственным runtime-C-представлением (`AtomicInt` — `type AtomicInt(*())`, `nova_rt/*.h` уже даёт конкретный `typedef … Nova_AtomicInt;`), использованный КАК ПОЛЕ пользовательского record'а (`type Ctr { n AtomicInt }`), даёт CC-FAIL: `typedef redefinition with different types ('struct Nova_AtomicInt' vs 'struct Nova_AtomicInt' (aka 'Nova_AtomicInt'))`. Root (по сгенерированному `.c`): codegen для ЛЮБОГО именованного типа-поля синтезирует `typedef struct Nova_<T> Nova_<T>;` forward-declare (обычный путь для heap-record-полей, нужен pointer-field до полного тела), но для `AtomicInt` этот forward-declare с ИМЕНОВАННЫМ struct-tag коллидирует с уже-`#include`нутым runtime-typedef'ом (вероятно anonymous-struct `typedef struct { … } Nova_AtomicInt;` — не тот же tag). Репро (standalone CU, 1 файл, без периферии): `type Ctr { n AtomicInt } test { mut c = Ctr { n: AtomicInt.new(0) } ... }` → тот же CC-FAIL. Затрагивает, вероятно, ЛЮБОЙ `#share`-опаковый extern-тип (все `Atomic*`, возможно `Mutex`/`WaitGroup`/…) как field — до сих пор НИКТО в кодовой базе не клал sync-примитив полем record'а (0 прецедентов до этой находки), поэтому дремал незамеченным. Обход (применён в detach_effect_ok_test.nv): не оборачивать — захватывать atomic напрямую (`mut c AtomicInt`), не через record-поле. Фикс: forward-declare-путь для record-полей должен распознавать opaque-extern-типы с готовым runtime-typedef'ом (не генерить свой `struct Nova_<T>` forward-decl, использовать typedef-имя напрямую как уже-complete type, раз оно `#include`н из `nova_rt`). | codegen / record-field forward-decl | P2 |
