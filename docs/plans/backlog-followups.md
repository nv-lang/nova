...
| `[M-open-range-len-source-hardcoded]` | **OPEN 2026-07-24 (наблюдение владельца: «в компиляторе хардкод — для Vec брать `@len()`, для str поле `@len`?»).** ДА, подтверждено кодом: лоуэринг открытого диапазона `[a..]` в `emit_c.rs` — хардкод-свитч по C-типу. str-путь (~34828): `if obj_ty == "nova_str"` → `len_expr = "_s.len"` (ПРЯМОЕ чтение priv-поля byte-длины), рантайм `nova_str_slice_to_end_{chk,nochk}`. Vec-путь (~34745): `obj_ty.starts_with("Nova_Vec____")` → СИНТЕЗ AST-узла `obj.len()` (Member+Call). `NovaArray_` — своя ветка. **`else → Err("slice on unsupported type")`** — то есть пользовательский тип open-range-резать НЕЛЬЗЯ вообще. Против конвенции «максимум из .nv» (feedback-maximize-nv-sourcing): «где взять длину» и «какой рантайм звать» захардкожены по каждому lang-item раздельно. **Смягчающее:** str/Vec/array — примитивы с РАЗНЫМИ C-раскладками (стек-значение `{ptr,len}` / кучевой хэндл / инлайн-массив), срез физически разный рантайм → какой-то диспатч неизбежен; текущий вид — bootstrap-прагматика (комментарий в коде: «для bootstrap проще запретить open-ended для str-slice»). **Кандидат на унификацию:** протокол `HasLen`/`Sliceable` в .nv (каждый тип отдаёт длину методом), лоуэринг `[a..]` → единообразный вызов протокол-метода вместо `if obj_ty=="nova_str"`; побочно откроет open-range-срез для пользовательских типов. Не срочно (примитивы, закрытый набор), но помечено как долг хардкода. | codegen (`emit_c.rs` open-range lowering) — унификация под протокол | **P3** |
| `[M-manual-slice-bounds-lint-missing]` | **OPEN 2026-07-24 (владелец: «`after_scheme[2..]` работает — линт должен ругаться на длинную запись»).** Открытые формы диапазона канон (спека 02-types.md:6761): `arr[a..]` (до конца, end=len), `arr[..b]` (от начала, start=0), `arr[..]` (весь). Проверено: `s[2..] == s[2..s.byte_len()]`, `v[1..] == v[1..v.len()]` (PASS). Линта нет (`W_MANUAL_SLICE_TO_END` в lints.rs = 0). **Три редукции:** (1) `recv[a..recv.len()]` / `recv[a..recv.byte_len()]` -> `recv[a..]` (**37 сайтов** std+nova-http: `data/semver_range.nv` x7, `encoding/url.nv` много, `fs/path.nv`, `math/complex.nv`); (2) `recv[0..b]` -> `recv[..b]` (симметрично, `url.nv:98,152`); (3) `recv[0..recv.len()]` -> `recv[..]`. **Условие БЕЗ ложных срабатываний:** end — ГОЛЫЙ вызов `.len()`/`.byte_len()` на ТОМ ЖЕ receiver-выражении, что и срез (не `x[a..y.len()]` — другой receiver; не `x[a..x.len()-1]` — арифметика); для (2) start — литерал `0`. `.byte_len()` для str, `.len()` для Vec/slice — матчить по факту вызова, тип знать не нужно. Инфраструктура: реестр `ConvRule` в `lints.rs`, семья `W_MANUAL_MIN_MAX`/`W_MANUAL_CLAMP`/`W_MANUAL_COALESCE` (последний влит 2026-07-24 — прямой прецедент, тот же класс «ручная форма vs канон», clippy). Id — `W_MANUAL_SLICE_TO_END`. Fix-it машинный (замена однозначна). | линты (`lints.rs`, новое ConvRule) + миграция ~40 сайтов | **P2** |
