...
| `[M-checker-protocol-typed-arg-any-bypass]` | **OPEN 2026-07-20 (вскрыто [M-208]-расследованием, sonnet).** `resolved_cat_of_depth` (`compiler-codegen/src/types/mod.rs`) мапит ЛЮБОЙ `TypeDeclKind::Protocol` expected-тип → `ResolvedType::Any`, из-за чего `assignable_direct` пропускает ЛЮБОЙ аргумент для protocol-typed параметра **без структурной проверки** (протокол-соответствие не верифицируется). Симптом-прецедент: `.debug(sb)` где `sb: StringBuilder` (реализует только `Write`, НЕ `Fmt`) компилировался БЕЗ ошибки — на C `Nova_StringBuilder*` шёл туда, где ожидался `Nova_FmtCtx*` (оба начинаются с указателя на offset 0 → **тихая type confusion**, пустая строка вместо крэша, вместо чистой compile-error). ОБЩИЙ (не Vec/Fmt-специфичный) пробел: любой protocol-typed параметр принимает несоответствующий аргумент. Zero-tolerance — реальная дыра звучности типизации (маскирует type confusion). Фикс: `assignable_direct`/`resolved_cat_of_depth` должны СТРУКТУРНО проверять, что аргумент реализует протокол (а не деградировать в `Any`). Широкий/рискованный (protocol-dispatch горячий путь) — отдельная волна, нужна аккуратная neg-фикстура-матрица (protocol-параметр × несоответствующий/соответствующий аргумент). Зона: checker `types/mod.rs`. | checker/звучность | **P1** |
