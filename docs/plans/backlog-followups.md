...
| `[M-clone-value-return-no-extra-copies-verify]` | **РЕШЕНИЕ ВЛАДЕЛЬЦА 2026-08-01 (продолжение clone-обсуждения):** `.clone()` и вообще возврат value-типов НЕ должен делать лишних копий — целевая механика: скрытый ret/out-параметр (в C это sret/RVO: caller даёт слот, callee пишет прямо в него; clang NRVO убирает внутренние временные). Задача: ДОКАЗАТЬ по сгенерированному C, что для (а) рекурсивно-стековых clone == один memcpy в слот назначения и (б) value-return вообще — без промежуточных копий; где ABI/эмиссия дают лишнюю копию — внедрить явный ret-параметр в кодогене. Перф-задача, не дефект (baseline стража); слот — после v0.1 либо попутно 196-волне если она рядом. |
