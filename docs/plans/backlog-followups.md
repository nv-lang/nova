...
| Маркер | Суть | Home | Pri |
| `[M-primitive-receiver-bounded-blanket-dispatch]` | **OPEN 2026-07-15.** Bounded-бланкет `fn[T Ints] T @method` на ПРИМИТИВНОМ ресивере мис-диспатчится в одноимённый **concrete**-метод чужого типа в том же CU (codegen name-keyed фолбэк, не `resolved_callees`). Репро: Пункт 10/200 заменил `checked_add_i64(a,b)`→`a.checked_add(b)` в `duration.nv`, где рядом `Duration @checked_add` → CC-FAIL `passing int64_t to NovaValue_Duration` (`duration.c:8568`; `NovaOpt_int64_t` vs `NovaOpt_NovaValue_Duration`). Семейство D164/196.7 (name-collision dispatch), но **НЕ покрытый случай**: D164 — unconstrained bare-`fn[T]` бланкет; ЗДЕСЬ — **BOUNDED** `[T Ints]` + примитивный ресивер + concrete-коллизия. Фикс = ОДНО ОКНО: диспатч method-call примитивного ресивера читает `resolved_callees` (чекер уже выбрал `i64.checked_add`), а не name-keyed фолбэк. **Блокирует Пункт 10/200.** | Plan 196 (одно окно) / codegen | **P1** |
| `[M-primitive-concrete-overload-receiver-dispatch]` | **OPEN 2026-07-16.** ТРЕТИЙ вариант name-collision dispatch-семейства (D164 → 196.8 → этот): ДВА concrete same-name метода на РАЗНЫХ примитивах (`int @clamp` defaults.nv:180 / `f64 @clamp` :182), ресивер `i64` (точного матча нет) → чекер легально коэрсит в `int`, codegen name-keyed `method_receivers` last-wins берёт **f64** → `Nova_f64_method_clamp(i64,…)` — ТИХАЯ порча (i64 через double: `i64.MIN`/`-i64.MAX` неразличимы) → RUN-FAIL `duration.nv:1276` (saturating_* вернул i64.MIN вместо -i64.MAX). Репро: worktree `nova-p200dur` (Пункт 10/200), `duration.c:8580/8604/8628`. Фикс = ОДНО ОКНО: dispatch читает `resolved_callees` (чекер уже выбрал int-оверлоад по коэрсии), не name-keyed; зеркало 196.7/196.8. **Блокирует Пункт 10/200** (вместе с уже закрытым 196.8). | Plan 196 (одно окно) / codegen | **P1** |
| `[M-187-high-concurrency-connection-wedge]` | **OPEN 2026-07-15 (loadtest.ps1 10× под нагрузкой).** M:N-сервер флагмана (examples/flagship/aggregator, 1 процесс, 16 воркеров) ВИСНЕТ под МАССОВОЙ ОДНОВРЕМЕННОЙ нагрузкой соединений: ~20 одновременных держит; 40 — часть 000 но ВОССТАНАВЛИВАЕТСЯ; **80 — 21/80 прошло, дальше 000 НАВСЕГДА** (single-req тоже 000, не восстанавливается). Контраст: sustained-ПОСЛЕДОВАТЕЛЬНАЯ (50× SSE weather-live) + 10 раундов по всем 12 комбо — идеально. Клинит именно ВЫСОКАЯ ПАРАЛЛЕЛЬНОСТЬ входящих соединений (accept-loop spawn'ит per-conn fiber под supervised; на пике remote-park/scheduler заклинивает). Родня pending_remote/accept-park (83.x). Демо-нарратив = concurrency → P1. Repro: `.\examples\flagship\aggregator\loadtest.ps1` (BLOCK 5, Concurrency 80) ЛИБО `seq 1 80\|xargs -P80 curl /api/run`. Диагностика: watchdog-дамп (NOVA_WATCHDOG_DUMP_SECS=3) при висе → какой слот/park застрял. Зона: runtime 83.x (M:N под высокой connection-concurrency) + возможно accept-loop в main.nv (bounded-drain). | Plan 187 / runtime 83.x | **P1** |
...
| `[M-187-http-serde-setcookie-serialize-collision]` | **OPEN — перепроверено эмпирически 2026-07-16 (sonnet, worktree nova-serdecheck), баг ЖИВ.** Генерик `json_encode[T]` (std.encoding.serde) резолвит `.serialize()` НЕ по типу: в одном CU с пакетом `http` коллизия с `http.cookie.SetCookie.serialize()` ломает ЛИНКОВКУ любого `#impl(Serialize)`-типа. Семья `[M-178-server-typed-body]`/D84 method_overloads. Обход в примере: main.nv рендерит JSON вручную (report_json.nv остался typed). Блокирует «typed-serde в http-CU» повторно. Repro: examples/flagship/aggregator (ветка flagship-187). **Проверка закрытия серде-волной (`98e3663cc` и соседи) — ОТРИЦАТЕЛЬНАЯ:** `98e3663cc` — другой баг (`[M-serde-encode-pointer-op-regression]`, type-qualified fn_ret для mono blanket-методов), к этой коллизии не относится и её не задевает. Проба (main.nv: `snapshot_dto_json(dto)` → `snapshot_to_json(dto)` — уже существующий typed-serde путь из `report_json.nv`, никакого нового кода) на текущем main-бинаре компилятора (`nova.exe`, commit `2381b8ce2`) даёт ТУ ЖЕ линковочную ошибку: `lld-link: error: undefined symbol: Nova_SetCookie_method_serialize`, `referenced by ...main.c: nova_fn_8encoding5serde11json_encode____NovaValue_SnapshotDto`. Обход снимать НЕЛЬЗЯ. | floating (compiler-codegen resolve/mono) | P1 |
...
| `[M-consume-block-cancelerror-bare-cu]` | Найдено при проверке consume-block вопроса владельца (2026-07-15). Re-consume block-выражение, ЭСКЕЙПЯЩЕЕ consumable-значение (`consume conn = consume lst { …; lst.accept() }`), в CU БЕЗ cancel/effect-контекста даёт **CC-FAIL** `use of undeclared identifier 'Nova_CancelError'` (cleanup-путь эмитит cancel-error инфру безусловно). В effect-контексте (`with Net`) инфра есть → tcp_share_test зелён; в голом CU — крэш C-компиляции. Scope-STATEMENT форма (`consume r = make() { body }`, d188) не задета (не эскейпит значение). Fix: объявить/гейтить Nova_CancelError в consume-block cleanup независимо от контекста, ЛИБО declare Nova_CancelError всегда. **FIX QUEUED (владелец 2026-07-15): следующим codegen-заходом СРАЗУ ПОСЛЕ слияний 209 Ф.3 / 206 / D39 (все на `emit_c.rs` — во избежание 4-way конфликта); НЕ паркуется.** | floating (codegen/consume-block) | **P2 — FIX QUEUED** |
