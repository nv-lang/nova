...
| `[M-fiber-arena-raise-cap]` | Cheap-first вместо растущих стеков: уменьшить per-fiber virtual-reserve (8MB→1-2MB) + поднять `NOVA_FIBER_SLOT_COUNT` → ~50-100k fiber'ов за ~полдня, БЕЗ GC/growable-stacks (2 `#define`; virtual ≈ бесплатен на 64-бит, RAM ленивая). Риск: меньший стек → deep-recursion overflow → guard-page чистый краш. Делать ТОЛЬКО если потолок 16k реально упрётся. Plan 146 §6.1. | Plan 82/146 | P3 |
...
| **Plan 139 — ✅ CLOSED 2026-06-11** + **Plan 139.1 — ✅ CLOSED 2026-06-12** | umbrella str=value-record закрыт Ф.0-Ф.7; spec финализирован (D26 MAJOR AMEND + D216 §1 + D228 + D52); 0 new FAIL. **Plan 139.1 (E1/E4 completion):** корневой `[M-139-f0-lang-item-decl]` ✅ **ЗАКРЫТ И УДАЛЁН** — `type str value priv {ptr *u8,len int}` объявлен (`std/prelude/core.nv`), privacy fires, ABI-alias `nova_str`, 3 neg-фикстуры PASS; **БЕЗ новой checker-инфры** (переиспользован value-record Plan 124.8). E1 → ✅ FULL. E4 → 🟡 PARTIAL (producer-формы re-homed под Plan 138.2 Ф.0 `[]T→Vec` flip — `@ptr` необходим, но недостаточен). История фаз в simplifications.md (Plan 139.1 Ф.A/Ф.B/Ф.C). Маркеры ниже — gated/permanent followups, не блокеры. |
...
| `[M-139-f1-trim-view]` | Plan 139 Ф.1 followup — `str @trim()` Nova-body возвращает аллоцированную копию (byte-loop + from_bytes_unchecked); бывшая C `nova_str_trim` возвращала zero-copy slice-view без alloc. **REGATE Plan 139.1 Ф.D (2026-06-12):** `@ptr` field-access РАЗГЕЙЧЕН (lang-item landed Ф.A), но view-форма producer-зависима так же, как E4 producer-формы — нужен Nova-конструируемый slice-record над `@ptr`. Re-homed под **Plan 138.2 Ф.0 (`[]T→Vec` flip)**, более НЕ gated на (закрытый) `[M-139-f0-lang-item-decl]`. Контент идентичен, разница только перф (alloc vs view) |
| `[M-139-f2-ptr-field-producers]` | Plan 139 Ф.2 followup — `as_bytes`/`split`/`from_bytes_lossy`/`from_bytes_unchecked`/`from_bytes_unchecked_steal` остаются C-примитивами. **ROOT CAUSE УТОЧНЁН Plan 139.1 Ф.B (2026-06-12):** `@ptr` field-access РАЗГЕЙЧЕН (Ф.A landed lang-item), но НЕДОСТАТОЧЕН. Producer-формы требуют либо (а) конструктора `NovaArray_*` из raw-parts (`as_bytes`/`split` строят `NovaArray_nova_byte*`/`NovaArray_nova_str*`-header, алиасящий `@ptr` — `[]u8`/`[]str` → `NovaArray_*` compiler primitive, emit_c.rs:5173, НЕТ Nova-surface конструктора), либо (б) `[]T`→`Vec` universal flip (Plan 138.2 Ф.0, НЕ приземлён — даст Nova-конструируемый `Vec`-record). `from_bytes_unchecked` дополнительно требует `alloc(len+1)+copy+\0` (D26 §3 NUL-инвариант) — aliasing-zero-copy нарушил бы copy-контракт `readonly` arg. **Настоящий разблокер = Plan 138.2 Ф.0 (`[]T`→Vec flip), не `@ptr`.** `to_bytes`/`to_chars` мигрированы в Nova-body. C as_bytes уже zero-copy (array.h:638) — контракт сохранён |
| `[M-139.1-hash-irreducible-crypto-seed]` | Plan 139.1 Ф.B — `str @hash()` НЕустраним: SipHash-1-3 + скрытый per-process random crypto seed (`nova_hash_seed_k0/k1`, nova_rt.h). DoS-resistance (hash-flooding защита) ТРЕБУЕТ чтобы seed НЕ был Nova-видим — экспонировать его на Nova-сторону = security-регрессия. Остаётся `external fn` (C `nova_str_hash`). Не followup-к-миграции, а постоянная C-граница (записан для аудита) |
| `[M-139.1-operator-lowered-methods]` | Plan 139.1 Ф.B — `str @concat()`/`@compare()` (и `@eq`/`@lt`/`@le`/`@gt`/`@ge`) лоуэрятся НАПРЯМУЮ BinOp-operator codegen (`+`/`<`/`==`/comparison-synthesis, emit_c.rs), НЕ через method-dispatch. Миграция тела метода на Nova НЕ retire'ит C-fn (`nova_str_concat`/`nova_str_cmp` остаются для операторов) и добавляет perf-регрессию (push-loop vs memcpy). Чистое retirement требует совместной миграции operator-lowering + method — orthogonal, низкий приоритет (C-формы оптимальны). Home: будущий str-operator-Nova-body план |
| `[M-139.1-len-d117-method-only]` | Plan 139.1 Ф.B — `str @len()` НЕ мигрируем на field-read `=> len`: `s.len` field-style access = HARD ERROR `E_SIZE_ACCESSOR_FIELD` (D117, emit_c.rs:17625 — size-accessor method-only). Поле `len` value-record'а перекрыто D117-баном на field-style len/cap/is_empty. Метод routes на `nova_str_byte_len` (O(1) field-read в C — уже оптимально). Не блокер; записан для аудита (объясняет почему `@len` остаётся external) |
