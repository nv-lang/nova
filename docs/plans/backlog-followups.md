...
| `[M-method-value-blanket-not-found]` | **OPEN 2026-07-23 (там же).** Method value НЕ находит БЛАНКЕТ-методы: `int.@to_str` → честная чекерная ошибка `method value: no method 'to_str' on type 'int'` (диагностика КОРРЕКТНАЯ, не ICE), хотя `5.to_str()` работает через бланкет `fn[T Display] T @to_str()`. Т.е. lookup формы `Type.@method` смотрит только прямые методы типа, минуя бланкеты/type-set-bounds. Ограничивает `a.map(int.@to_str)`-идиому именно там, где она наиболее полезна (Display/Ints-бланкеты). | D35 Method values / резолв бланкетов | **P3** |
