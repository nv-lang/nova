...
| Маркер | Суть | Home | Pri |
|---|---|---|---|
| `[M-187-watchdog-idle-server-kill]` | **OPEN 2026-07-15 (найдено при браузер-смоуке флагман-сервера).** Диагностический supervised-watchdog (Plan 83.11 Phase A, `fibers.h:2871`, дефолт 5с) срабатывает на ЛЕГИТИМНОМ простое accept-loop: scope `count=0 pending_remote=1` (сервер парконулся на `uv_accept`, ждёт соединение = НОРМА). По коду (`fibers.h:2925-2937`) дамп one-shot non-fatal, НО эмпирически процесс УМИРАЕТ после `nova_runtime_dump_state` (главный поток) — любой idle >5с Nova-сервер падает. Обход: **`NOVA_WATCHDOG_DUMP_SECS=0`** (проверено — сервер держит 15с+ простоя). Затрагивает ЛЮБОЙ долгоживущий сервер (accept-loop), не только флагман. Возможно тот же корень у live real_net `pending_remote`-park (#4 сессии). Фикс: watchdog не должен считать legitimate long-lived remote-park (accept) зависанием И/ИЛИ разобраться, что в `nova_runtime_dump_state` на главном потоке фатально (заявлен non-fatal). Зона: `nova_rt/fibers.h` + `runtime.c`/`runtime.h` (dump). Семья 83.x (supervised/remote-park), сосед `[M-187-supervised-nested-fiber-slot-race]` (закрыт 83.4.5.12). | Plan 187 / runtime 83.x | **P1** |
| `[M-187-tls-cross-pkg-consume-cleanup]` | **OPEN 2026-07-15 (изолировано при фиксе TLS-диаманта; ОТДЕЛЬНЫЙ баг — диамант снят).** Codegen: определение `Nova_<T>_consume_cleanup` эмитится ТОЛЬКО для consume-сайтов КОРНЕВОГО пакета; `consume` ВНУТРИ внешнего пакета (nova-tls консьюмит `std.net TcpStream` в `TlsStream.connect`) не регистрирует тип для эмиссии → **undefined symbol `Nova_TcpStream_consume_cleanup`** при линковке. Repro: pure-std `consume st = s` at scope-exit → линкуется ✓; `TlsStream.connect(tcp, cfg)` → undefined ✗. **Блокирует:** weather-live end-to-end (TLS-handshake) + `examples/tls/echo_server.nv` линковку (type-check PASS, линк FAIL). Фикс: регистрировать тип для эмиссии `consume_cleanup`-определения независимо от пакета consume-сайта (usage-discovery сквозь границы пакетов). Зона: `compiler-codegen/src/codegen/emit_c.rs`. | Plan 187 / codegen | **P1** |
| `[M-compress-checksum-cleanup]` | **OPEN 2026-07-15 (найдено владельцем при ревью nova-compress).** (1) **Инлайн-тесты в модуле** — ✅ ЧАСТИЧНО: вынесены из `nova-compress/src/checksum.nv` в `checksum_test.nv` (`47b0a64`, push public); `std/encoding/compress/checksum.nv` тоже несёт 8 инлайн-тестов — но std/compress УДАЛЯЕТСЯ по 205-endgame (чинить при удалении, не отдельно). (2) **`crc32` ТРОИТСЯ** → **РЕШЕНИЕ владельца: ДЕДУП, использовать std** (не self-contain): `nova-compress/src/checksum.nv` + `std/encoding/compress/checksum.nv` → **импорт `std.checksums.crc32`** вместо своей копии; **`adler32` промоутнуть в `std/checksums/adler32.nv`** (тоже общий чексум, зеркало crc32-промоушна) → оба каноничны в std, пакеты используют. Тесты crc32-векторов → `std/checksums/crc32_test.nv` (уже есть); adler32-тесты → `std/checksums/adler32_test.nv`. Порядок: (а) промоут adler32 в std + гейт; (б) `compress/checksum` (std) на импорт; (в) синк nova-compress + push. Не correctness (конвенция+DRY). | Plan 205 / 179 (compress) | P3 |
...
| `[M-cas-return-witnessed-value]` | Найдено при дизайне плана 206 (2026-07-14). `AtomicI*.compare_exchange`/`_weak` (`std/src/runtime/sync.nv`) возвращают `-> bool`, ВЫБРАСЫВАЯ свидетеля провала: C-примитив `atomic_compare_exchange_strong(&obj,&expected,desired)` при провале ПИШЕТ фактическое прочитанное значение в `expected` (нужно для пересчёта в CAS-цикле без повторного `load()` + окна гонки), а мы его теряем. Rust отдаёт `Result<T,T>` (`Err(actual)`). По принципу 206 «примитив не теряет информацию» — пересмотреть на `-> Result[unit, T]` (`Err(actual)`). **Дорос до плана — см. [207](207-atomic-cas-witnessed-value.md).** | Plan 207 | P3 |
