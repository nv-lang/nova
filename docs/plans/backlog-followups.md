...
| `[M-187-sustained-live-tls-resource-death]` | **OPEN 2026-07-17 (НТ-марафон 10×-от-текущего, по просьбе владельца).** Сервер флагмана УМИРАЕТ НАВСЕГДА после **273 подряд** SSE weather-live прогонов (loadtest BLOCK 4 ×500): №1-273 зелёные → №274+ все 000 (BLOCK 5/6/7 красные по наследству; RESULT 288/230, все 230 = одна точка смерти). ДО марафона BLOCK 1-3 идеальны: 1202/1202 (12 комбо ×100, вкл. live×200) — серии по 100 НЕ добивают, только сплошной марафон. Класс: НАКОПИТЕЛЬНАЯ ресурсная деградация (НЕ concurrency-wedge — тот от одновременности и замитигирован): ~1470 суммарных прогонов / ~770 weather-live (≈3000 TLS-соединений) до смерти. Смерть на TLS-марафоне → подозреваемые: mbedTLS ctx/session leak (nova-tls `@close` — освобождается ли шим-сессия?), fd/сокеты (CLOSE_WAIT-накопление), fiber-слоты арены. Посмертного лога НЕТ (loadtest стартовал сервер без redirect — доработано, следующий марафон принесёт причину: паника/OOM/wedge). Repro: `loadtest.ps1 -Iterations 300` (BLOCK 4, live). Диагностика: перезапустить марафон с логом + мониторить fd/память процесса по ходу (каждые 50 итераций). | Plan 187 / nova-tls + runtime | **P1** |
| `[M-187-docker-linux-runtime-hang]` | **OPEN 2026-07-17 (Docker-волна 187, гейт оркестратора).** Docker-образ флагмана СОБИРАЕТСЯ (126МБ, ветка docker-187: Dockerfile+README+AGGREGATOR_BIND; находка: std/ = Rust-compile-time зависимость компилятора), но сервер ВНУТРИ контейнера НЕ живёт: (1) дефолтная fiber-арена → шторм `fiber_arena guard page mprotect failed` (гипотеза vm.max_map_count: guard-VMA на слот; обход NOVA_MAX_FIBERS=2048 — фейлы уходят); (2) и с малой ареной: LISTEN 0.0.0.0:8187 есть, соединение доходит (CLOSE_WAIT, rx_queue=1), запрос НЕ читается; **PID1 State=D, wchan=do_exit, 34 потока — процесс завис в выходе сразу после старта**, логи 0 байт. Linux-рантайм СЕРВЕРНОГО профиля (долгоживущий M:N в контейнере) не готов — [M-nova-linux-build] гонял только hello/checksums. Родня: TSan runq init/grab race, план 211. Нужен отдельный заход «Linux M:N server profile» (арена/mprotect + exit-hang + Boehm под WSL2). Блокирует финал волны 2 (187). | Plan 187 / runtime 83.x (Linux) | **P1** |
...
| `[M-198-f4c-1-privfile-type-not-discriminated]` | **OPEN 2026-07-17 (Plan 212 пункт 7, sonnet, живой репро на бинаре `696d834b4`).** `priv(file) type` НЕ файл-дискриминируется в checker-резолве (D307 покрывает fn/method-символы, но не типы) — use-site в одном файле резолвит одноимённый `priv(file) type` из ДРУГОГО файла того же folder-module. Репро (2 файла, `known_red.privtype_file_discrimination`, разные формы `Rect`): `a.nv`'s собственный `area(r Rect) -> int => r.w * r.h` даёт `[E7320] no field or method w on type Rect` / `note: Rect has fields: name, tag` — это поля `b.nv`'s `Rect`. Фикстура: `spec_tests/fixtures/known_red/privtype_file_discrimination/{a,b}.nv` (walker-skip, красный намеренно). Родственно орфан-упоминанию `[M-170.1-priv-file-types-methods]` (169.2-аудит, `[M-169.2-red-audit-orphaned]` выше — никогда не заводился отдельным маркером) — та же природа бага, теперь с воспроизводимым репро. | Plan 198 Ф.4c / checker (D307 типы) | **P1** |
