...
| `[M-tuple-fixarr-typedef-order]` | **OPEN 2026-07-18 (вскрыт исполнением 200 П18 haiku-агентом; независимый минимальный репро подтверждён интегратором на чистом main).** Кортеж с фикс-массивом `(int, [4]u8)` генерирует C-typedef'ы в НЕПРАВИЛЬНОМ порядке: typedef кортежа (`struct { nova_int f0; _NovaFixArr_4_9_nova_byte f1; }`) эмитится РАНЬШЕ typedef'а самого `_NovaFixArr_4_9_nova_byte` → `unknown type name` CC-FAIL. Класс: любой `(T, [N]U)` в языке — секция tuple-typedef'ов не упорядочена по зависимостям от секции fixed-array-typedef'ов (родня [M-codegen-emission-nondeterminism], но здесь не флап, а детерминированная поломка зависимости). Репро: `fn f() -> (int, [4]u8) {...}; ro (n, b) = f()` — 8 строк, standalone. **Блокирует 200 П18** (ветка `p200-18-utf8`, worktree nova-p20018, коммит bdae7f4e9 — механика готова, грепы чистые, ЖДЁТ этого фикса). Фикс: dependency-aware порядок эмиссии typedef-секций (FixArr раньше tuple, либо топологический порядок по ссылкам). | codegen emit_c / typedef-порядок | **P1** |
