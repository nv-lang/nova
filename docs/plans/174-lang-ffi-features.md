...
> **Status:** 📋 READY (umbrella). Создан 2026-06-27; **Ред. 2 — 2026-07-03**: полная сверка семейства
...
|---|---|---|---|
| **174.1** | [Primitive parse API](174.1-primitive-parse-api.md) | Один generic-движок str→примитив (**вариант B** — Ред. 2: type-set bounds ДОСТУПНЫ, [172.3 ✅ CLOSED 2026-06-28](172.3-type-set-bounds.md)); radix-only `parse`; фикс truncation-бага; float-канон (§4.1). D309. | 📋 proposed; координация 172.1-legacy-зона + 177 Ф.2b (§2) |
| **174.2** | [`?` return-only](174.2-question-mark-return-only.md) | **Остаток = spec-closure + cross-carrier диагностики** (§3.2). Codegen/checker-ядро (`[E_TRY_IN_FAIL_FN]`, удаление throw-mode, баннер D4) **реализует [173 Ф.1 п.2](173-error-system-unify-harden.md)** — направление «173 завершает 174.2», НЕ наоборот. Amend D85. | 📋 PLANNED; синхронизация с 173 Ф.1 |
...
| **174.4** | [Effect-registry size](174.4-effect-registry-compile-time-size.md) | Compile-time N вместо хардкода 32 (`effects.h:971`; silent-drop 33-го эффекта подтверждён :996-1002). **Carrier-независим** — исполним сейчас, параллельно 174.3. Без D-блока (+ обязательный Q-note). | 📋 READY (P2) |
| **174.5** | [Pointer-ops methods](174.5-pointer-ops-methods.md) | Методы `.read/.write/.offset/.dist/…` вместо операторов; `unsafe T`→`uninit T`; write-cap fix (жив: `types/mod.rs:12093`, `emit_c.rs:26117/39336`). Amend D216-pointer (+ D352 для uninit-семантики). | 📋 PROPOSED; после носителя (02-types = зона 172) |
| **174.6** | [C-FFI ABI types](174.6-ffi-abi-types.md) | C-ABI тип-лист (рекурсивный) + `E_FFI_NON_C_ABI_TYPE` + fn-ptr ABI-тег `*extern "C" fn`. Amend D282-FFI (+ D353 для ABI-тега). Spec-часть — сейчас; checker — фазой M1. Поглощает `[M-172.1-extern-cname-dedup-overloads]`. | 📋 PROPOSED; spec сейчас / checker после M0 |
...
5. **174.6** — M0 (spec: amend D282 rule 2 + D216 cross-amend) — сразу **после sign-off renumber D282
   (§6)**: без него amend зацементирует коллизию двойного номера (либо M0-amend пишется сразу в новый
   номер D355). M1-M3 (checker/тег/тесты) — после M0, не ждёт 172.1-финала (checker-слой аддитивен).
...
- **D138 → D238/D240** (Index/MutIndex; D138 = межпакетный импорт!) — §1.1, §3.
...
174.5 — `d216_pointer_methods.nv` (+ uninit; имя — по итогу renumber-решения §6, при renumber = `d354_*`);
174.6 — `d282_ffi_abi.nv` (дизамбиг от `d282_blanket_protocol.nv`; при renumber = `d355_*`). Прогон: `nova test spec_tests` (один CU) + targeted-тема.
...
- **🔴 Владельцу (sign-off до старта 174.5 и 174.6 M0): двойные D-номера В СПЕКЕ** — D216
  (02-types.md:4314 anon-tuple-mono И 02-types.md:7893 typed-pointers; 174.5 амендит второй) и D282
  (08-runtime.md:8155 extern-ABI И 02-types.md:13099 blanket-protocols; 174.6 амендит первый).
  Прецедент разруливания — D109/D110/D111; **приёмники renumber — D354/D355** (НЕ D352/D353 — те
  заняты новыми блоками семьи, см. выше). Без renumber spec_tests-файлы `d216_*`/`d282_*` смешивают
  два несвязанных D под одним номером.
...
3. Далее по §2-порядку — три ПАРАЛЛЕЛЬНЫЕ ветки старта: **174.3 (P1)** ∥ **174.4** ∥ **174.6 M0**
   (после sign-off renumber §6); затем 174.2-остаток (синхрон с 173 Ф.1) → 174.6 M1→M2→M3 →
...
4. **Prerequisite-check на входе каждого под-плана** (стоп + эскалация при незакрытом гейте):
   ...
   свой + зона 172; 174.5/174.6 ← sign-off renumber D216/D282 (§6).
