...
| 104.7 | Tree-sitter grammar (`tree-sitter-nova` repo) | Grammar, queries (highlights/folds/indents/injections), Helix/Zed/Neovim distribution | ~3 dev-day |
...
### 104.7 — Tree-sitter grammar — ~3 dev-day

**Separate repo:** `nv-lang/tree-sitter-nova` (or similar). Standard tree-sitter project layout (`grammar.js`, generated `parser.c`).
