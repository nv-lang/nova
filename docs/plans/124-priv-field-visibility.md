...
| 124.6 Test access + pub_to | 🟢 **AMENDED+REIMPLEMENTED 2026-06-16** — design changed from fn-level `#test_access` + field-level `#visible_to` → (1) implicit same-module test access (test block in same folder), (2) `#[test_access(T)]` before `test "…" {}` block for cross-module opt-in, (3) `#[pub_to(Y,Z)]` on TypeDecl for type-level friend access; `TestBlockGuard` RAII + `type_pub_to` map in checker; 8 new fixtures (5 pos + 3 neg); 7 pre-existing fixtures migrated to `priv(type)`; A6.1-A6.8 all ✅; plan124_6 PASS 6/CC-FAIL 9 (CC-FAIL = gc.h missing, expected in worktree) |
