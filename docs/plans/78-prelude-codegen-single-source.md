...
>
> **Статус:** ✅ **ЗАКРЫТ 2026-05-22** — Ф.1–Ф.5 все выполнены
> (954 PASS / 0 FAIL). Ф.1 — `method_routing` переклассифицирован как
> легитимный реестр C-реализации (вариант 2); Ф.2 — хардкод-зеркало
> `sum_schemas` для Option/RuntimeError устранено (variants приходят из
> `.nv` — mono-типы / `emit_type_decl` из `errors.nv`); Ф.3 — print/
> println deferred на Plan 68; Ф.4 — panic/exit/assert аудит (legitimate
> codegen). Зеркало вариантов prelude-sum-типов в codegen ликвидировано
> полностью (Result — ещё Plan 62.A.bis Ф.4).
>
...
### Ф.2 — Удалить `sum_schemas` pre-populate (P2) ✅ ВЫПОЛНЕН 2026-05-22
...
#### Ф.2 — итог (✅ ВЫПОЛНЕН ПОЛНОСТЬЮ 2026-05-22)

Эмпирический метод плана (удалить → полный прогон). Финальный
прогон — **954 PASS / 0 FAIL**.

- **`sum_schemas["Option"]` + `["NovaOpt_nova_int"]` — УДАЛЕНЫ ✅.**
  Option полностью мономорфизирован (Plan 14/59): pattern-matching
  резолвит variants через mono-тип `NovaOpt_<T>`, legacy
  `sum_schemas["Option"]` не читался. Зеркало устранено без регрессий.

- **`sum_schemas["RuntimeError"]` — УДАЛЕНО ✅** (со второго подхода).
  Первый эмпирический заход дал 2 CC-FAIL — диагноз: `emit_type_decl`
  для `RuntimeError` (он в `RUNTIME_DEFINED_TYPES`) делал полный
  `return Ok(())`, поэтому sum-schema из декларации не строилась, и
  хардкод pre-populate был единственным источником.
  **Фикс:** ветка `RUNTIME_DEFINED_TYPES` в `emit_type_decl` теперь
  для **sum-типов** строит `sum_schemas` + `record_variant_field_*`
  **из самой декларации** (`std/prelude/errors.nv`) — C-struct
  по-прежнему не эмитится (живёт в `nova_rt/array.h`). Хардкод
  pre-populate (variants + record-variant-метаданные
  `IndexOutOfBounds`) удалён целиком. Единственный источник правды —
  `.nv`. Прогон зелёный.

**Зеркало `sum_schemas` для всех prelude-sum-типов устранено**
(Result — Plan 62.A.bis Ф.4; Option + RuntimeError — Plan 78 Ф.2).
...
### Ф.5 — spec + docs + verification (P2) ✅ ВЫПОЛНЕН 2026-05-22

- **Spec.** Отдельный D-блок не вводится: prelude как source of truth
  для type-checker/resolve уже зафиксирован (D26 + Plan 62); Plan 78 —
  **codegen-internal** чистка (codegen перестал держать параллельное
  зеркало), не изменение языка. Solution-record — этот план + маркер
  `[M-legacy-sum-schemas-retained]` в `simplifications.md`.
- **`simplifications.md`.** `[M-legacy-sum-schemas-retained]` —
  обновлён: variant-зеркало `sum_schemas` устранено целиком (Ф.2),
  routing переклассифицирован как legitimate (Ф.1). print/println —
  Ф.3 уже зафиксировал ссылку на Plan 68.
- **Acceptance — выполнено.** `grep 'sum_schemas.insert.*("Option"|
  "Result"|"RuntimeError"|"NovaOpt")'` по `emit_c.rs` → **пусто**:
  хардкод-зеркало вариантов prelude-sum-типов устранено. `method_routing`
  в `init_hardcoded_baseline` — остаётся как legitimate реестр
  C-реализации (Ф.1, вариант 2 — аналог «Ф.4 legitimate»-исключения).
- **Полный прогон — 954 PASS / 0 FAIL**, ноль регрессий.
...
## Аменд 2026-05-23 (Plan 95 + Plan 95.bis + Plan 99) — финальный

**Plan 99 (закрыт 2026-05-23):** последние 6 closure-applying
Option/Result-методов выведены на Nova-body: `Option.map[U]`/
`unwrap_or_else`/`ok_or[E]`, `Result.map[U]`/`map_err[F]`/
`unwrap_or_else`. **14/14 builtin Option/Result-методов на Nova**.
Реестр `MethodRouting` C-routing — **только для `unwrap`** (Fail-
handler dispatch, Plan 61 lineage). Closure invoke через
`NovaClosBase` + explicit cast — паритет Rust `FnOnce`-mono.
Декомпозиция: 99.1 (foundation method-level generic), 99.2
(contextual ctors), 99.3 (atomic per-method migration), 99.4
(tests+spec+close). См. [Plan 99](99-option-result-closure-applying-methods-nova-body.md).

## Аменд 2026-05-23 (Plan 95 + Plan 95.bis)

**Plan 95 (закрыт 2026-05-23):** `Option.is_some`/`is_none` +
`Result.is_ok`/`is_err` **выведены из `method_routing`-реестра в
Nova-тело** в `std/prelude/core.nv` (`=> match @ { ... }`). Узкий
санкционированный пересмотр Ф.1 для подмножества чистых тег-предикатов.

**Plan 95.bis (закрыт 2026-05-23):** ещё 5 «чистых» методов выведены
тем же каналом: `Option.unwrap_or`/`or`, `Result.unwrap_or`/`ok`/`err`.
Прежние C-трамплины (включая `NOVA_ARRAY_IMPL`-macro для Option.or,
`#define`-алиасы `Nova_Result_method_unwrap_or`/`_ok`, inline emit
`Result.err()`) удалены — single-source. Все 9 перенесённых методов
получают routing через `MethodRouting::DeclaredBody`.

Реестр `MethodRouting` (HardcodedRuntimeFn / `<inline>` sentinel)
**остаётся в силе** для C-реализуемых-only методов:

- `unwrap` (Fail-handler dispatch, Plan 61);
- `unwrap_or_else`/`map`/`ok_or`/`map_err` (closure-applying +
  method-level generic + Plan 98 inference).

Plan 95 ввёл активную ветку `MethodRouting::DeclaredBody` (scaffold-only
до Plan 95) для Nova-body методов на builtin sum-типах; Plan 95.bis
расширил скоуп без изменения механизма. См. spec
[08-runtime.md](../../spec/decisions/08-runtime.md) и
