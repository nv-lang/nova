...
> `method_routing` переклассифицирован), **Ф.2 🟡 частично**
> (`sum_schemas["Option"]`/`["NovaOpt_nova_int"]` pre-populate удалён —
> 954/0; `["RuntimeError"]` — honest-defer, нужна миграция
> RuntimeError-read-сайтов на registry), **Ф.3 ✅** (deferred на
> Plan 68), **Ф.4 ✅** (аудит). Осталось: **Ф.2-остаток** (RuntimeError)
> + **Ф.5** (spec/docs).
>
...
### Ф.2 — Удалить `sum_schemas` pre-populate (P2) 🟡 ЧАСТИЧНО 2026-05-22
...
#### Ф.2 — итог (🟡 ЧАСТИЧНО ВЫПОЛНЕН 2026-05-22)

Эмпирическое удаление (по методу плана: удалить insert → полный
прогон):

- **`sum_schemas["Option"]` + `["NovaOpt_nova_int"]` — УДАЛЕНЫ ✅.**
  Полный прогон **954 PASS / 0 FAIL** — ноль регрессий. Option
  полностью мономорфизирован (Plan 14/59): pattern-matching резолвит
  variants через mono-тип `NovaOpt_<T>`, legacy `sum_schemas["Option"]`
  не читается. Зеркало устранено.

- **`sum_schemas["RuntimeError"]` — ОСТАВЛЕН (honest-defer ⏸).**
  Удаление дало **2 CC-FAIL** (`plan62/runtime_error_from_prelude`,
  `runtime/error_runtime_error` — `error: initializing 'nova_int'`).
  RuntimeError — non-generic value-type с record-вариантами
  (`IndexOutOfBounds {index,length}`); его pattern-matching call-sites
  читают payload-типы вариантов из legacy `sum_schemas`, а не из
  sum-schema-registry. Чистая чистка требует **миграции
  RuntimeError-read-сайтов** (`pattern_bind_typed` tuple- И
  record-ветки + `pattern_cond` — урок Plan 59) на `lookup_sum_schema`
  / registry-`DeclaredFromPrelude` (из `errors.nv`). Это отдельная
  фокус-работа — план Ф.2 явно допускает honest-defer. **Остаток
  Ф.2.**
