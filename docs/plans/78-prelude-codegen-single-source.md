...
## Аменд 2026-05-23 (Plan 95)

`Option.is_some`/`is_none` + `Result.is_ok`/`is_err` **выведены из
`method_routing`-реестра в Nova-тело** в `std/prelude/core.nv`
(`=> match @ { ... }`). Это **узкий санкционированный пересмотр Ф.1**
для подмножества чистых тег-предикатов; реестр `MethodRouting`
(HardcodedRuntimeFn / ExternalFn / `<inline>` sentinel) **в силе**
для C-реализуемых-only методов:

- `unwrap` (Fail-handler dispatch, Plan 61);
- `unwrap_or`/`unwrap_or_else`/`map`/`ok_or`/`map_err` (closure-applying
  спец-codegen);
- `ok`/`err` (boxing в Option из Result-payload).

Plan 95 ввёл активную ветку `MethodRouting::DeclaredBody` (scaffold-only
до Plan 95) для Nova-body методов на builtin sum-типах. См. spec
[08-runtime.md](../../spec/decisions/08-runtime.md) «Plan 95
(закрыт 2026-05-23)» и [Plan 95](95-builtin-sum-method-mono.md).
