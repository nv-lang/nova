...
> **Top-level** под-план. Создан 2026-06-26. Статус **🟢 Ф.1 + Ф.3 LANDED (2026-07-04)** — inflate/gzip/zlib **decode** (pure-Nova) + deflate/gzip/zlib **encode** (pure-Nova, levels + streaming) готовы; **Ф.2 brotli = честный build-gate `[M-179-brotli-vendor-lib]`** (vendor-lib отсутствует эмпирически, см. «Ф.2 assess»). (Ред. 2 — 2026-07-03: аудит ground-truth/7-языков/siblings; stale-номера сняты, D-карта актуализирована, flush/level/FFI-фиксы.) Маркер **[M-179-std-compress]**.
>
...
- **Ф.2 — brotli decode (C-FFI). 🔴 ЧЕСТНЫЙ BUILD-GATE `[M-179-brotli-vendor-lib]` (net-zero, 2026-07-04).** **ЭМПИРИЧЕСКАЯ ПРОВЕРКА vendor-lib (§7.7, Ф.0-verify выполнена):** `libbrotlidec` **ОТСУТСТВУЕТ** — grep по всему worktree (`find -iname '*brotli*'`) = 0; main-repo vcpkg (`compiler-codegen/vcpkg_installed/x64-windows-static/lib/`, куда указывает gate-env) содержит только `gc/gccpp/gctba/cord/atomic_ops*/libz3` — **brotli.lib нет**; brotli-headers нет; в `target/`/`nova_rt/` нет (единственный vendored native = `target/libuv-cache/libuv.lib`). → **это НАСТОЯЩИЙ build-gate, НЕ мнимый блокер: C-FFI без либы НЕ фейкается (§7.7/§0).** Ф.2 стартует ТОЛЬКО после vendor-коммита `libbrotlidec` (google/brotli BSD-2/MIT, статика как libz3/libuv — §6). Реализация (когда vendor будет): **C-FFI к `libbrotlidec`** — НЕ pure-Nova V1 (§3.0 Q2). FFI в `ffi.nv` (`extern "C"`); тонкий Nova-API `brotli_decode(data, max_output)` + streaming `BrotliReader` (**consume value**, Q6 — держит C-instance, D133). **BOMB-CAP** инкрементально (`max_emit`-капинг; window ≤16 MiB фикс-bounded lgwin≤24 — задокументировано, output-cap ≠ window-cap, критик-gap). **Vendor** libbrotli (§6). spec: D337. pos: RFC 7932-vector decode; streaming-chunked==one-shot; round-trip. neg: bomb→`Bomb` (cap поверх C-stream); max-lgwin+bomb→`Bomb` не OOM; truncated→`UnexpectedEof`; malformed→`InvalidData(str)` (из `brotli_dec_error`); **BrotliReader не consume→`EXPECT_COMPILE_ERROR`** (D133 — единственный consume-кодер). DEP: **Ф.1**, libbrotli vendor (§6), Ф.0-vendor-verify, **координация 174.6 M1** (E_FFI_NON_C_ABI_TYPE: extern-сигнатуры без `[]u8` — §3.6; в std нет ни одного extern с `[]u8`, grep=0). **Координация Plan 178:** закрывает `br`-ветку 178-decompress.
...
- **`[M-179-brotli-vendor-lib]` — vendor `libbrotlidec`** (google/brotli BSD-2/MIT, статика как libz3/libuv) — **build-gate Ф.2** (эмпирически отсутствует 2026-07-04; §6/Ф.2). Без него Ф.2 (brotli decode C-FFI) не стартует.
