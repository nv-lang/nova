...
| 6 | README.md | сборка + запуск локально (Windows/Unix), порт, env. ~~Path-dep реальность~~ **Ред.6-поправка: план 204 ЗАКРЫТ (D420)** — зависимости в git-форме с nova.lock; для локальной разработки — `[replace]`; раздел «клонируй три репы» больше НЕ нужен (нужна только nova + компилятор; deps фетчатся) |
