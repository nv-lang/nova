...
5. **Ред.5, два уровня (снято противоречие с §7 п.2 «локальный сервер»):**
   (а) ОБЯЗАТЕЛЬНО: публичная ссылка = фронт-статика с запечёнными replay-прогонами
   НАСТОЯЩЕГО бека (реальные события+тайминги; хостинг статики — страница проекта /
   Pages / cloudflare-инфра nv-lang.org); кнопка Live — подсказка «запусти локально/Docker»;
   (б) ОПЦИЯ (отдельное решение владельца, не MVP): публичный Live-бек (VPS) — постоянная
   эксплуатация, в объём не входит. Читается «за 5 секунд» — про (а).
