...
### Acceptance Ф.6
- [x] `nova test --filter p53` → PASS (Ф.6.1 mojibake fixed)
- [ ] **Ф.6.2 deferred → Plan 53.1:** structured Suggestion требует
      stmt-span refactor в `walk_stmt` (сейчас в let-pattern не пробрасывается
      span containing stmt для replacement). Net-LOC > estimated ~80;
      низкий ROI пока let-without-suggestion работает корректно.
- [x] `let mut { x } = ...` тест с mutation works (Ф.6.3)
- [x] `Channel.new` round-trip тест — send+recv через destructured handles (Ф.6.5)
- [ ] **Ф.6.6 deferred → Plan 53.1:** Unknown-field validation требует
      расширения `BoundCtx` (record_decls registry + type-inference
      arbitrary RHS expressions + Levenshtein helper). Net-LOC ~150+,
      не ~40 как оценивалось; CC-error при unknown field user-recoverable
      (вижу `no member named X in struct Nova_Pair` — название поля
      понятно из контекста).
- [x] Ф.6.4 — Channel.new special-case удалён, общий path работает (через
      `Nova_ChannelPair` schema-registration).
- [x] 0 регрессий vs main baseline (433 PASS, 41 FAIL — baseline без изменений).

### Закрытие Ф.6
Production-hardening доставлен: Ф.6.1, Ф.6.3, Ф.6.4, Ф.6.5. Ф.6.2 и Ф.6.6
вынесены в Plan 53.1 (low-priority polish) после оценки реальной сложности
(требуют инфраструктурных изменений type-checker, превышающих исходный
LOC-estimate). Базовый сценарий — `let { tx, rx } = Channel.new(0)` —
работает без special-case через generic pattern_bind_typed path.
