...
# План 18 (rev 3, DRAFT): Stdlib roadmap для Nova

> ⚠️ **СТАТУС: DRAFT, не финализирован.** Не использовать как авторитет
> для реализации. Текущая утверждённая версия —
> [18-stdlib-roadmap.md](18-stdlib-roadmap.md). После финализации этот
> черновик переносится в основной файл, .draft.md удаляется.
>
> **Дата создания черновика:** 2026-05-09.
> **Что меняет vs rev 2:** учтены результаты чтения spec/decisions
> (D40/D53/D62/D70/D73/D78/D79/D80), переосмыслены `std.sync`, `std.fmt`,
> убраны несуществующие в Nova концепты (Display/Debug/ToStr).
...
## Verification (когда будет финализирован и реализуется)
...
1. Решить Q1–Q8 выше (часть может потребовать новых D-блоков в spec)
2. Согласовать с пользователем структуру каждого модуля (опциональные методы, возможные переименования)
3. Когда финализируется — переименовать `18-stdlib-roadmap.draft.md` → заменить содержимое `18-stdlib-roadmap.md` (или объединить), удалить `.draft.md`
4. Обновить запись в [docs/plans/README.md](README.md) — статус "proposal, не начат" → "финализирован, ждёт codegen-разблокировки"
