...
> **2026-05-16 ревизия:** Ф.7.1, Ф.7.2, Ф.7.3, Ф.7.5, Ф.7.7, Ф.8 закрыты 2026-05-15/16.
> Оставшиеся шаги — только то, что реально не сделано.
...
#### Шаг 2 — Ф.7.6: MONO_INSTANTIATION_DEPTH_LIMIT через CLI

Magic-number глубины инстанциации (сейчас hardcoded 64) вынести в CLI-флаг
`--mono-depth=N` (default 64) через nova-cli → BuildOpts → emit_c.rs.
При превышении — compile-error с указанием места первой инстанциации.

**Зависимости:** нет. **Объём:** ~50 строк (CLI + emit_c.rs).

#### Шаг 3 — Ф.4: spawn + closure-capture в mono (within/race)

Два конкретных бага:

**[M-spawn-closure-capture-mono]:** В mono'д generic fn body, closure-параметр
`body fn() -> T` захватывается в spawn ctx, но spawn body использует `body`
без `_c->body` rewrite. Capture-substitution не применяется к function-typed
параметрам в mono pipeline.
Fix: в `emit_monomorphized_fn` при обработке spawn-body — переписывать все
captured fn-params через `_c->` (аналогично non-generic spawn).

**[M-mono-spawn-fwd-decls]:** `emit_monomorphized_fn` создаёт новые spawn-body
функции (`_nova_spawn_N`), но pre-scan `collect_spawn_forward_decls` отработал раньше.
Fix: drain mono worklist до spawn pre-scan в `emit_module`, или добавить forward-decl
инлайн при первом появлении spawn-body в mono context.

**Acceptance:** `cancellation.nv` (`within[T]`, `race[T]`) компилируется и тесты PASS.
**Объём:** ~100 строк.

#### Шаг 4 — Ф.5 retry: `retry.nv` + `cancellation.nv` тесты

`std/concurrency/retry.nv` — generic метод с closure-параметром, не проверявшийся
через C codegen. После Ф.4 должен компилироваться.
Написать `nova_tests/concurrency/retry_test.nv`.

Риск: `[M-time-effect-schema-mismatch]` (runtime `NovaVtable_Time` имеет
`sleep/now/after`, handlers.nv ожидает `now_ms/now_ns`) — блокирует import
std.testing.handlers. Если retry.nv зависит от Time effect — решить здесь
(согласовать schema), иначе зафиксировать как Plan 49 followup.

**Зависимости:** Ф.4 (шаг 3). **Объём:** ~60 строк тестов + диагностика.
