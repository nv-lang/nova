...
>
> **СТАТУС (2026-05-16):** В работе. Ф.0-Ф.3 DONE, Ф.6 DONE, Ф.7.5/7.7 DONE.
...
>
> **Регрессия:** 397/397 PASS на release-сборке (2026-05-16, после std/collections).
>
...
#### Шаг 1 — Ф.7.1: method-call inference в `infer_expr_c_type`

`let r = obj.method(args)` в generic-контексте типизирует `r` как `void*` вместо
реального возвращаемого типа. Метод-ветка в `infer_expr_c_type` не делает mono
inference. Fix: вызывать `resolve_mono_type_args` + `apply_type_subst_to_ref`
к `fn_decl.return_type` из `mono_method_decls` — зеркало free-fn пути (~12450-12478).

**Зависимости:** нет. **Объём:** ~30 строк.

#### Шаг 2 — Ф.7.2: static generic methods через sentinel routing

`Type.method[T]()` (без instance receiver) — sentinel `__mono_method__T__m`
попадает в C-output как undefined symbol. Sentinel detection в `emit_call`
покрывает instance, но пропускает static-путь.
Fix: расширить sentinel-detection branch (~emit_c.rs:8621) на static dispatch.

**Зависимости:** нет. **Объём:** ~20 строк.

#### Шаг 3 — Ф.8: встроенные методы примитивов (hash/eq/ord)

Разблокирует `HashMap[int, int]`, `HashMap[str, int]` без явных impl.
Примитивы получают автоматические методы от компилятора:

- `nova_rt.h`: `nova_int_hash`, `nova_bool_hash`, `nova_f64_hash`, `nova_str_hash` (FNV-1a inline C)
- `emit_c.rs`: `prim_builtin_method(c_ty, method)` — перехват перед general resolver;
  hash → C fn call, eq/lt/le/gt/ge → inline оператор
- `infer_expr_c_type`: `hash` → `nova_int`, `eq/lt/...` → `nova_bool` для primitive receiver

Нужно сделать **до Ф.7.3** — erased fallback пока ещё нужен как safety net.
**Acceptance:** `nova_tests/modules/hashmap_basic.nv` PASS с `HashMap[str,int]` и `HashMap[int,int]`.
**Объём:** ~120 строк (nova_rt.h + emit_c.rs).

#### Шаг 4 — Ф.7.3 + Ф.7.4: убрать silent fallback → удалить erased-эмиттеры

**Ф.7.3:** `resolve_mono_type_args` → `Err` вместо silent `register_erased_instance`.
Сообщение: имя fn, какой type-param не выведен, hint про turbofish.
Требует чтобы Ф.3 (DONE) и Ф.8 (шаг 3) были готовы — erased fallback больше не нужен.

**Ф.7.4:** После Ф.7.3 erased path мёртв. Удалить:
- `emit_generic_fn_erased` (~4864)
- `emit_generic_method_erased` (~4204)
- `erased_type_ref_c` (~4166)
- `register_erased_instance` (~4470)
- `generic_fns: HashSet` (~329)

Ожидаемый результат: −400 LOC. Закрывает acceptance criteria «Erased-эмиттеры удалены».
**Объём:** Ф.7.3 ~20 строк (правок) + Ф.7.4 −400 LOC (удаление).

#### Шаг 5 — Ф.7.6: MONO_INSTANTIATION_DEPTH_LIMIT через CLI/env

Magic-number глубины инстанциации вынести в CLI-флаг `--mono-depth=N` (default 64)
через nova-cli arguments → BuildOpts → emit_c.rs. При превышении — compile-error
с указанием места первой инстанциации.

**Зависимости:** нет. **Объём:** ~50 строк (CLI + emit_c.rs).

#### Шаг 6 — Ф.4: spawn + closure-capture в mono (within/race)

Два конкретных бага в взаимодействии spawn и mono-эмиттера:

**[M-spawn-closure-capture-mono]:** В mono'д generic fn body, closure-параметр
`body fn() -> T` захватывается в spawn ctx, но внутри spawn body codegen
обращается к `body` напрямую вместо `_c->body`. Fix: в `emit_spawn_body`
(`emit_c.rs`) после mono-подстановки переписывать все captured-переменные
через `_c->` rewrite — аналогично тому, как это делается в non-generic spawn.

**[M-mono-spawn-fwd-decls]:** `emit_monomorphized_fn` создаёт новые spawn-body
функции (`_nova_spawn_N`), но `collect_spawn_forward_decls` (pre-scan) их не видит.
Fix: drain mono worklist **до** spawn pre-scan в `emit_module`, или добавлять
forward-decl инлайн при первом появлении spawn-body в mono context.

**Acceptance:** `cancellation.nv` (`within[T]`, `race[T]`) компилируется и
тесты проходят. `[M-race-closure-array]` снят.
**Объём:** ~100 строк.

#### Шаг 7 — Ф.5 retry: `retry.nv` + `cancellation.nv` тесты

`std/concurrency/retry.nv` — generic метод с closure-параметром, никогда не
проверявшийся через codegen. После Ф.4 должен компилироваться.
Написать `nova_tests/concurrency/retry_test.nv` с покрытием retry с backoff.

Также: `[M-random-effect-codegen]` (Time effect не объявлен в handlers.nv —
runtime pre-registration не совпадает с методами) — проверить, блокирует ли
retry.nv, и если да — зафиксировать как отдельный followup Plan 49 или починить здесь.

**Зависимости:** Ф.4 (шаг 6). **Объём:** ~60 строк тестов + диагностика.

#### Шаг 8 — Финальный regression

- Полный `nova test --release` без новых FAIL
- Проверить все acceptance criteria (таблица ниже)
- Обновить статус в шапке плана, `project-creation.txt`, `simplifications.md`
- Commit + discussion-log
