...
| Protocols (D73/D77/D109) | (none) | From, Into, TryFrom, TryInto, Hash, Equal, Compare, Display |
...
std/prelude/protocols.nv       — From, Into, TryFrom, TryInto, Hash, Equal, Compare, Display
...
**62.C bis (2026-05-20) — RuntimeNoneError ЗАКРЫТО:**

- `type RuntimeNoneError` (unit-type per D85) задекларирован в
  `std/prelude/errors.nv` + re-export через facade. Блокер снят
  Plan 72 P1-B (bodyless-type support в parser — `type Name` без
  тела теперь parsable). PRELUDE_VERSION 8 → 9. Тест
  `plan62/runtime_none_error_from_prelude`.
- `Never` НЕ мигрируется сюда — bottom-тип должен быть строчным
  keyword'ом `never`, не капитализированным prelude-типом (Plan 75
  cleanup, отдельная задача).
- Runtime throw остаётся string-payload (`nova_rt/effects.h`
  `nova_throw_runtime_none_error`); structured `Nova_RuntimeNoneError`
  payload не строится — тот же mechanism что у `ReadBufferError`.
  2 codegen-gap'а вокруг RuntimeNoneError (`!!`-handler-arm skip,
  bodyless-type-as-value) — `[M-runtime-none-error-throw-codegen]`,
  тангенциальны к prelude-миграции.
...
### Plan 62.E — Protocols: From/Into/TryFrom/TryInto/Hash/Equal/Compare/Display (2-3 days)
...
> file). `From[T]` / `Into[U]` / `Hash` / `Equal` / `Compare` /
> `Display` перенесены в `std/prelude/protocols.nv` через formal protocol
> declarations. **Deferred (тогда):** `TryFrom[T,E]` / `TryInto[U,E]` —
> Plan 56 Ф.2.7 effect-free enforcement запрещал `Fail[E]` сигнатуру в
> protocol-методах.
>
> **Status update 2026-05-20 (62.E.bis — ЗАКРЫТО):** `TryFrom[T, E]` /
> `TryInto[U, E]` задекларированы (8/8 протоколов). Effect-блокер снят
> Plan 56 Ф.2.7-revert (D122 amended). Декларации используют форму
> `try_from(t T) -> Result[Self, E]` (D77 Result-форма, migration
> path b) — re-export через facade `std/prelude.nv`, PRELUDE_VERSION
> 7 → 8. См. §«DEFER 62.E» ниже + табличную строку 62.E bis.
...
- [x] Создать `std/prelude/protocols.nv` (без `fn`-префикса в protocol
  ...
  export type Hash protocol { hash() -> u64 }
  export type Equal protocol { eq(other Self) -> bool }
  export type Compare protocol { cmp(other Self) -> Ordering }
  export type Display protocol { fmt(sb StringBuilder) -> () }
  ```
- [x] Auto-derive remains в compiler (D73/D77/D109) — declarations additive.
      Bound clauses `[T Hash]` теперь могут резолвиться через
      cross-file resolve (Plan 15 generic-bounds infrastructure).
- [x] `std/prelude.nv` re-export (6-я строка):
      `export import std.prelude.protocols.{From, Into, Hash, Equal, Compare, Display}`.
- [x] `std/collections/hashmap.nv`: убрана local-декларация `Hash`
      (дубликат с prelude — `duplicate top-level name Hash` блокировал
      все 37 файлов импортирующих HashMap). `HashMap[K Hash, V]`
      bound теперь резолвится через cross-file resolve в prelude.
- [x] `compiler-codegen/src/lints.rs::collect_protocol_names`: HashSet
      shrink на 4 имени (`Hash`, `Iter`, `From`, `Into` — теперь
      file-declared и captures'ятся for-loop'ом ниже через R27
      auto-import). Оставлены `Ord`, `Eq`, `ToStr` (legacy aliases для
      back-compat с `nova_tests/types/generics.nv:90 TwoBounds[K Hash,
...
- [x] Positive tests: `nova_tests/plan62/protocols_bound_resolution.nv` —
      6 test-блоков (Hash/Equal/Compare/From/Into/Display
      bound resolution через prelude auto-import). All PASS.
- [x] Regression: 703/0/44 (baseline 702 + 1 new test) + 16/16 unit tests.

**DEFER 62.E → ЗАКРЫТО в 62.E.bis (2026-05-20)**

> **Resolution (2026-05-20):** выбран **path (b)** — `TryFrom`/`TryInto`
> объявлены с возвратом `Result[Self, E]` (без `Fail[E]` effect-row).
> User-решение: «try_from должен возвращать Result». Декларации
> раскомментированы в `std/prelude/protocols.nv` + добавлены в facade
> re-export. Effect-блокер при этом независимо снят Plan 56 Ф.2.7-revert
> (D122 amended). PRELUDE_VERSION 7 → 8. Текст ниже — исходный анализ
> блокера (historical).
...
- [x] HashMap[K Hash, V] продолжает работать после removal local
      Hash из hashmap.nv — pass.
- [x] TryFrom/TryInto migration — ЗАКРЫТО 62.E.bis (2026-05-20):
      D77 Result-форма, re-export через facade, PRELUDE_VERSION 7 → 8.
...
| 62.E | From[T], Into[U], Hash, Equal, Compare, Display (6/8 protocols) | `protocols.nv` |
...
|---|---|---|
| **62.A.bis Ф.4** ✅ ЗАКРЫТ 2026-05-21 | Remove pre-populated `sum_schemas[Result]` | Closed 2026-05-21 — Plan 59 Ф.7.5 increment 2 полностью мономорфизировал Result (`NovaRes_<ok>_<err>*`); nested-pattern Result-variant typing переведён на `novares_ok_err` (Plan 59 commit `2b184a3c06a`). Legacy hardcoded `sum_schemas["Result"]` HashMap pre-population **удалена** из emit_c.rs. Полный прогон **887 PASS / 0 FAIL**. Оставлены как live infrastructure (не legacy): `init_hardcoded_baseline` Result-entry (несёт method_routing для 9 Result-методов — активный источник dispatch), `nova_rt/array.h` `#define Nova_Result*` алиасы (bare `Nova_Result*` ещё эмитится codegen'ом). `sum_schemas["Option"]` оставлен — Option-mono отдельная инициатива. См. `[M-legacy-sum-schemas-retained]` ✅ RESOLVED. |
...
| **62.C bis** ✅ ЗАКРЫТ 2026-05-20 | RuntimeNoneError migration | Closed 2026-05-20 — блокер снят Plan 72 P1-B (bodyless-type support в parser). `type RuntimeNoneError` (unit-тип, D85 — ошибка `Option!!` на None) задекларирован в `std/prelude/errors.nv` + re-export через facade. PRELUDE_VERSION 8 → 9. Тест `plan62/runtime_none_error_from_prelude` (Some-path + Fail[RuntimeNoneError] сигнатура + None-throw catchability). `Never` НЕ мигрирован — bottom-тип должен быть строчным keyword'ом `never` (Plan 75 cleanup). 2 codegen-gap'а вокруг RuntimeNoneError (`!!`-handler-arm skip + bodyless-type-as-value) вынесены в `[M-runtime-none-error-throw-codegen]`, не блокируют migration. plan62 36/36 PASS. |
...
| **62.E bis** ✅ ЗАКРЫТ 2026-05-20 | TryFrom[T, E] / TryInto[U, E] protocols | Closed 2026-05-20 — effect-блокер устранён ранее (Plan 56 Ф.2.7-revert, D122 amended: эффекты в protocol-методах разрешены под mono-dispatch). `TryFrom[T, E]` / `TryInto[U, E]` задекларированы в `std/prelude/protocols.nv` через **migration path b** — форма `try_from(t T) -> Result[Self, E]` (D77, обычный возврат Result; user-выбор «try_from должен возвращать Result»). Добавлены в facade `std/prelude.nv` export-list (8/8 протоколов). PRELUDE_VERSION 7 → 8. Тесты: `plan62/tryfrom_tryinto_from_prelude` (positive) + `tryfrom_bound_unsatisfied_neg` (negative — bound enforcement). Codegen-баг `match Result[Record,E]` payload typing — pre-existing, вынесен в `[M-result-record-payload-match]` (Plan 59 sum-mono), не блокирует migration. plan62 35/35 PASS. |
...
| 62.E | `lints.rs::collect_protocol_names` | 4 | Hash, From, Into, Iter (re-count overlap with 62.D). Total 4 new removals. |
...
- [x] `std/prelude/protocols.nv` declares From/Into/TryFrom/TryInto/Hash/Equal/Compare/Display.
...
- **[Plan 56](56-vtable-dispatch-erased-generics.md)** — protocols в prelude (From/Into/Hash) могут получать vtable-dispatch infra. Не блокер.
...
- [spec/decisions/03-syntax.md#d73](../../spec/decisions/03-syntax.md#d73), [#d77](../../spec/decisions/03-syntax.md#d77), [#d109](../../spec/decisions/03-syntax.md#d109) — From/TryFrom/Hash.
