...
> `with_capacity`, `@insert`); `Hash` protocol (D72 bounds, Plan 15
> — enforcement готов).
>
...
- **`K: Hash`.** Выведенный или ожидаемый `K` проверяется на bound
  `Hash` (механизм Plan 15). Диагностика: «тип ключа `K` не реализует
  `Hash`».
...
- Ключ всегда `str` → `Hash` выполняется тривиально.
...
```nova
ro h HashMap[str, bool] = []     // ✅ пустая мапа (type из контекста)
...
Если `K = f64` или `K = f32` и компилятор разрешает `f64: Hash`,
...
- **Enforce `K: Hash`** — выведенный/ожидаемый `K` проверяется на
  bound `Hash` (механизм Plan 15). Диагностика: «тип ключа `K` не
  реализует `Hash`».
...
- Диагностика actionable:
  ...
  - `K` не `Hash`: «тип ключа `K` не реализует `Hash`».
...
- **Исправить `with_capacity` контракт и реализацию** (критический баг):
  ...
  ```nova
  ro cap = next_pow2(max(4, ceil_div(min_capacity * 4, 3)))
...
| Type-checker map-литерал + Hash + NaN-warn + dup-lint (Ф.2) | ~230 |
...
- [ ] `K: Hash` enforced для `[k:v]`; нехешируемый ключ — compile error.
...
- [Plan 15](15-generic-bounds-enforcement.md) — `Hash` bound
...
**Проблема:** `Hash.hash() -> u64` для primitives использует
