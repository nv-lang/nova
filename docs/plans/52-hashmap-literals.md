...
`std/collections/` (`deque.nv`, `lru.nv`, `queue.nv`, ...). `.c`-файлы,
которые встречаются рядом в рабочем дереве, — это **сгенерированный
вывод** nova-codegen (`/* Generated by nova-codegen. Do not edit. */`,
gitignored), а не рукописные C-сайдкары; трекаются только `.nv`.
Литерал ничего не меняет: HashMap остаётся обычным Nova-кодом, литерал —
синтаксический сахар над его публичным API. Никакой части HashMap в
компилятор не переезжает.
