...
Decision-документ: [`docs/dev/research/09-precise-gc-decision-2026.md`](../dev/research/09-precise-gc-decision-2026.md)
...
| **Ф.1** Heap bitmaps → [144.1](144.1-heap-layout-bitmaps.md) | Codegen эмитит per-type pointer-offset bitmap; allocator пишет layout-id в заголовок каждого объекта. (Лёгкая сторона — layout известен.) **Аналитическая (emit-nothing) половина ✅ ДОСТАВЛЕНА** ([Plan 144.1](144.1-heap-layout-bitmaps.md), спец [D375](../../spec/decisions/08-runtime.md#d375-ex-d277-renumber-2026-07-03--per-type-gc-pointer-offset-bitmaps-plan-1441-аналитическая-половина-ф1-emit-nothing), 2026-06-15): per-type bitmap вычислен layout-точно + отдан через CLI `nova gc-layout-analyze`, ничего не эмитит. **Runtime-потребление** (layout-id в заголовке + точный tracer) **гейтится на Ф.5** [M-144.1-heap-bitmaps]. | Каждая heap-аллокация несёт layout-id; precise heap-tracer метит объект точно при заданных корнях; unit-тест на 3-4 типах (record/sum/nested-ptr). [✅ аналитика: 5 фикстур + 21 unit-теста, emit-nothing; ⏳ runtime-AC → Ф.5] |
