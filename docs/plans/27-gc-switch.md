...
> **Статус:** в работе. Ф.1 ✅. Ф.1.5 ✅. Ф.2 ✅. Ф.4 ✅. Ф.6 ✅. Б.2 ✅. Б.3 ✅. Б.4 ✅. Б.5 ✅. Б.6 ✅. Б.7 ✅. Б.8 ✅.
> **Создан:** 2026-05-11. **Обновлён:** 2026-05-11 (Ф.6 + Б.2–Б.8 completed).
...
## Audit 2026-05-12 — gaps до «честно production»

Аудит после Plan 32 closure показал недозакрытые фазы:

### Ф.D — vcpkg path graceful fallback (новая)

`test_runner.rs:718-724` хардкодит путь
`cg_include.join("vcpkg_installed").join("x64-windows-static")`. На clean
machine без локального vcpkg bootstrap → cryptic linker error.

Сделать: `detect_boehm_lib()` helper аналогично `detect_or_build_libuv`:
1. Проверить `$NOVA_GC_LIB_DIR` env override.
2. Проверить локальный vcpkg path.
3. Под Windows — попробовать `vcpkg` global install (`%VCPKG_ROOT%`).
4. Если не найдено — graceful error с hint: «build via:
   `cd compiler-codegen && vcpkg install bdwgc:x64-windows-static`».

### Ф.5 — Linux libgc detect + hint

`Toolchain::Gcc` сейчас просто эмитит `-lgc -lpthread`. Под Linux без
`libgc-dev` юзер получит linker error без hint'а.

Сделать: в `detect_boehm_lib()` для Linux:
1. Проверить `/usr/include/gc.h` существует.
2. Если нет — hint: «install via: `sudo apt install libgc-dev`
   (Debian/Ubuntu) or `brew install bdw-gc` (macOS)».
3. macOS — symmetric check `/opt/homebrew/include/gc.h` или
   `/usr/local/include/gc.h`.

### Ф.E — Plan 25 G3a status sync

`docs/plans/25-production-readiness-roadmap.md` строки 122-177 описывают
G3a как «runtime — Plan 27». Plan 27 README говорит «closes G3a», но
Plan 25 свою сторону не обновил.

Сделать: добавить в Plan 25 G3a секцию status block:
«✅ ЗАКРЫТ Plan 27 Ф.4 (default=Boehm) 2026-05-11. См. Plan 27.»
...
Сделать:
1. Интегрировать `gc_test_helpers.c` (или эквивалент через Plan 32
...
- 2026-05-12 — аудит выявил Ф.3/Ф.5 не закрыты + vcpkg path issue + drift в Plan 25.
- Добавлены Ф.D/Ф.E/Ф.F/Ф.5-revisit как explicit subtasks.
...
- **2026-05-11** — Ф.6 + Б.2–Б.8 реализованы: AllocConstraint/GcKindTag/SkipReason +
  parse_alloc_constraint + parse_timeout_ms + Outcome::Skipped + Outcome::Pass расширен
  (captured_stdout/stderr/retries) + Summary.skip + emit_event (TAP skip / JSON skip) +
  print_summary (slowest-10 / JUnit skipped + retry system-out / JSON skip) +
  run_all (filter_from / shuffle / list_only) + nova-cli wiring (--list / --filter-from /
  --shuffle). 171/171 PASS baseline сохранён.
