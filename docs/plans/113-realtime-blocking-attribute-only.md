...
| A5 | `#realtime nogc` works (allocation banned in body) | ⚠️→✅ | **Corrected 2026-08-02 (№273, [M-realtime-nogc-silent-no-enforce]):** the cited `negative_capability/realtime_nogc_alloc` fixture does not exist in the tree (`nova_tests/negative_capability` has no such file) — this row's original ✅ was inaccurate. Ground truth found: `Vec[int].of(...)` (D259 canonical variadic ctor) was missing from `nogc_blacklisted_call`'s name blacklist, AND the path-extractor in `check_capabilities_at` never unwrapped an explicit-generic-typed receiver (`Vec[int].new()` → `Member{obj: TurboFish{..}, ..}`), so `nova check` silently accepted GC-allocating calls in `#realtime nogc fn` bodies. Both fixed in №273 (`compiler-codegen/src/types/mod.rs`); real regression fixtures now at `spec_tests/conformance/neg/d172_realtime_nogc_alloc_neg.nv` + `d172_realtime_nogc_turbofish_neg.nv`, positive at `spec_tests/conformance/d172_realtime_blocking_attrs.nv`. Spec documented at [D172 §7](../../spec/decisions/06-concurrency.md#d172-realtimeblocking-sync-class-annotation-system-plan-1036) (was previously undocumented in D172 despite being parsed/partially enforced). |
