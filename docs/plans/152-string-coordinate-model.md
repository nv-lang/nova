...
(152.3a ∥ 152.6) → 152.4 → 152.3b/152.5.
...
| 152.7 интерполяция + форматирование (D256) | [152.7-interpolation-formatting.md](152.7-interpolation-formatting.md) | A(сохранить)/B(улучшить) |
...
- **D256** (NEW) — формат-спеки интерполяции (Rust-style mini-language) + опц.
...
- **G5.** Spec обновлён: D249–D255 + амендменты D26(MAJOR)/D238/D58; все 5 Q-вопросов
  **закрыты** записями-решениями (§4): Q-string-indexing (заново), Q-string-len,
  Q-unicode-data, Q-string-collation, Q-module-internal-visibility; `docs/strings.md`
  + `docs/strings-internals.md` + migration-гайд написаны.
...
- D-блоки **D249–D256** зарезервированы под этот план (+ AMEND D26/D238/D58/D44/D183/
  D229). Если spec'и ещё на D248 — бери D249+. Другие агенты — с **D257**. Решения по
  Q закрыты в §4 — перенеси их записями в `spec/decisions/` и `spec/open-questions.md`
  (Q-string-indexing переоткрыть и закрыть заново).
...
**Порядок и фазы.** 152.0 → 152.1 → 152.2 → (152.3a ∥ 152.6) → 152.4 → 152.3b/152.5.
...
| D256 (NEW) | формат-спеки + опц. `Write`-sink (152.7) |
