...
> **Создан:** 2026-06-13.  **Статус:** ✅ **PHASE A + PHASE B ЗАКРЫТЫ** (2026-06-16), все sub-plans 152.0–152.7 + 152.7.1 закрыты; 152.8 открыт (post-merge, P2/P3).
...
| длина в codepoint'ах | `utf8.RuneCount` | `.chars().count()` | `[...s].length` | `codePointCount` | ✅ `char_len` | `as_chars().count()` |
...
```
   ...
   ro []u8 (Vec[u8])      CharsIter {buf str,pos}     GraphemesView (std/unicode)
   O(1) [i]/len/slice     iterator (поток), O(n)      UAX#29, O(n), Unicode-data
        ▲                        ▲                            ▲
...
- **I7. Каждый метод — ровно в одном слое.** Нет дублей (плоский `char_at` на `str`
  И `nth` на `CharsIter`); удалённое со `str` живёт в линзе.
...
| 152.7.1 Write-sink (D258 AMEND) | [152.7.1-write-sink.md](152.7.1-write-sink.md) | B (закрыт 2026-06-16) |
| 152.8 `char`/code-point → 32-бит (оптимизация памяти) | [152.8-char-u32-codepoint.md](152.8-char-u32-codepoint.md) | B (после merge) |
...
- Папка `std/runtime/string/` (module-per-file, как `std/collections/`):
  ...
  - `chars.nv` — `CharsIter` (+ опц. `CharIndicesIter`, Phase B) + decode-курсор.
...
- `str` **не** реализует `Index[int,V]`; `str[i]` целым → `E_STR_NO_INT_INDEX`
  (fix-it: `as_chars().nth(i)` / `as_bytes()[i]` / `s[a..b]`). `str[a..b]` —
...
- **Итератор `@as_chars() -> CharsIter` (NEW, вариант C — Q1 апрув автора).**
  `CharsIter value priv {buf str, pos int}`: `mut next()->Option[char]` (`Next[char]`),
  `iter()->CharsIter` (`Iter`, self), `count()->int` O(n), `nth(i)->Option[char]` O(n),
  `is_empty()`. **codepoint'ы — поток, не коллекция:** НЕ `Index[int,char]`, нет
  позиционных `at`/`len` (иначе `for i in 0..len {at(i)}` = O(n²) footgun). `str @iter()
  => @as_chars()`. `char_indices()` — отдельный `CharIndicesIter` (опц., Phase B).
- Бэар `@len()` со `str` убран → `E_STR_NO_LEN` (fix-it: `as_bytes().len()` /
  `as_chars().count()`). `@byte_len() -> int` (O(1)) остаётся как шорткат (имя
  однозначно). Нет `@char_len`/`@char_at` на `str`. Поле `len` в layout — остаётся.
- `@find`/`@rfind` → **байт**-offset.
- `for c in s` → `char` (= `s.as_chars()`).
- Конвенция `as_` (ленивый итератор/вью, borrows) vs `to_` (owned копия) — кодифицируется.

**D-блоки:** D249 (модель), D250 (`CharsIter` итератор + конвенция `as_`/`to_`).
...
- **transform:** `@replace`, `@replacen(from,to,n)`, `@repeat`, `@pad_left`/
  `@pad_right`/`@pad_center` (ширина в **codepoint'ах** → `as_chars().count()`).
...
- **Unicode-aware** (делегат в `std/unicode`, 152.4): `@is_alphabetic`, `@is_numeric`,
  ...
  `@is_control`, `@general_category()->GeneralCategory`, `@to_uppercase()->[]char`/
...
| **152.5b** eq_ignore_case (D254) | ✅ | `fab43385` — `str`/`char @eq_ignore_case` (Unicode full-case-fold, делегирует `fold_case`; "STRASSE".eq_ignore_case("straße")==true); plan152_5 t5+n_ PASS |
...
- **D250** (NEW) — `CharsIter` (codepoint-итератор, вариант C) + конвенция `as_`/`to_`.
...
- **Q-string-len** — **ЗАКРЫТО: нет бэар `len()`.** Длина — свойство представления:
  ...
  для аллокаций); codepoint-длина — `as_chars().count()` (O(n)); бэар `s.len()` →
...
- **152.1:** `str[i]`→`E_STR_NO_INT_INDEX`; нет `@len()`→`E_STR_NO_LEN`; `as_bytes()`
  O(1) индекс; `CharsIter` без `Index[int]`; `for c in s`==`char`; `find`==байт-offset;
...
Полные драфты **D249** (координатная модель) и **D250** (`CharsIter` + конвенция
...
|---|---|
| D249 (NEW) | координатная модель — линзы, без int-index/`len()`, char-итерация |
| D250 (NEW) | `CharsIter` (итератор) + конвенция `as_`/`to_` |
...
| D240 | `MutIndex` — `str` иммутабелен; `CharsIter` не Index-индексируем — не реализуют (R8) |
| D241/D242 | `Next`/`Iter` — `CharsIter` реализует `Next[char]`/`Iter` |
| Q-string-indexing | **ЗАКРЫТО заново** (был «вариант B») → линзы/byte-coords |
| Q-string-len | **ЗАКРЫТО** — нет бэар `len`; `byte_len()` + `as_chars().count()` |
