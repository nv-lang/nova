...
> **Статус:** 🟣 **SUPERSEDED by [Plan 95](95-builtin-sum-method-mono.md)**
> (2026-05-23). Ф.0 ВЫПОЛНЕН — GATE-STOP 2026-05-22: аудит показал, что
> чистая реализация требует инфраструктуры «Option/Result в generic-
> method-mono». Эта инфраструктура декомпозирована в **Plan 95**; цель
> Plan 93 (перенос предикатов `is_some`/`is_none`) реализуется как
> **Plan 95 Ф.4**. Plan 93 отдельно не возобновляется. См. «Итог Ф.0».
...
Перенос `is_some`/`is_none` в `.nv` — это «прелюдия на самом языке» +
...
этого не зависит: компилятор-на-Nova вызывает `opt.is_some()`
одинаково — C-трамплин за методом или Nova-тело; выхлоп — тот же C,
C-рантайм остаётся (прецедент: компилятор Go на Go, рантайм Go —
частично C/asm). → Приоритет **P3 корректен**; не помечать как
self-hosting-блокер и не двигать вперёд работ, реально приближающих
self-hosting (переписывание самого компилятора).
