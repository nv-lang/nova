...
├──────────────────────────────────────────────────────────────────────────┤
│  Tls (Plan 116, GATE: https/h2 ALPN)  │     Net (tcp+udp+dns)     │  Time (173)  │   ← транспорт
├──────────────────────────────────────────────────────────────────────────┤
│                              libuv (park/wake)                              │
└──────────────────────────────────────────────────────────────────────────┘
```
...
export fn Body consume @drain(self) Http -> Result[(), HttpError]     // дочитать остаток+release БЕЗ материализации (conn в пул); ex-@discard (§13.3)
...
- **Force-close:** компилятор требует потребить `Body` (D133) → соединение всегда возвращается в пул либо корректно закрывается. `consume @drain()` — дешёвый «мне не нужно тело» (ex-`@discard`, §13.3).
...
export fn RequestBuilder @header(name str, value str)        -> RequestBuilder     // ЗАМЕНЯЕТ все прежние (set) → HeaderMap.@insert; частый intent (§13.1)
export fn RequestBuilder @append_header(name str, value str) -> RequestBuilder     // ДОБАВЛЯЕТ ещё (multi-value) → HeaderMap.@append
...
export fn RequestBuilder @multipart(form Multipart) -> RequestBuilder              // multipart/form-data (File-part gate Plan 176)
...
**Под-планы при росте:** server → **178.1**, h2 → **178.2**. HTTP/3/QUIC — вне scope (§11).
...
| Q3 | must-consume `Body` | **Response `Body` must-consume (D133): `consume @close/bytes/text/json/drain/copy_to/into_reader`.** Незакрытый = **compile-error**. | Чинит главный Go-footgun `resp.Body`-leak на compile-time; бьёт reqwest(`Drop`)/fetch/OkHttp(runtime) |
...
| Q20 | json (де)сериализация | **`@json[T]` GATED на Plan 180 Ф.4 (serde record-DTO; план существует).** dynamic `.json()->JsonValue` — СЕЙЧАС над существующим `std/encoding/json`. Из §8-acceptance landed-фаз `json()` ИСКЛЮЧЁН пока gate открыт. | Нет reflective serde в Nova; honest-gate, не «available» |
...
| Q30 | Tls effect-leak | **`Http`-seam ИНКАПСУЛИРУЕТ `Tls`/`Net`/`Time` (ex-TcpNet/DnsNet — слиты в Net, §13.2); user видит ТОЛЬКО `Http`.** `real_http()` объявлен требующим их в scope. | Version-transparency: иначе http↔https меняют тип fn |
...
- **Ф.0.5 — PREREQ: URL-промоут + net byte-surface. «сейчас».** (1) Промоут url.nv → `std/http/url.nv` (Q6); **фикс `decode_query`-bug**; `Url.parse`/Result (D325); **ОБЯЗАТЕЛЬНО (§8): фикс `encode_query` multi-byte + IPv6-bracket + строгий host/SSRF-валидатор** (§3.8 — не «доводка»). (2) net byte-surface (ПОЛНЫЙ список §3.10): `read_bytes`/`write_bytes`/`write_all_bytes` на Stream+ReadHalf+WriteHalf; `real_tcp_net`+`mock_tcp_net`+C-shim; `str`-варианты сохранить. **HARD-BLOCKER для Ф.1.** pos: `Url.parse` round-trip; `decode_query` round-trip (был idle); `encode_query` multi-byte UTF-8 round-trip; `read_bytes` байт-в-байт incl. не-UTF-8. neg: malformed-port→`InvalidUrl` (source `ParseUrlError{InvalidPort}`); truncated-`%X`→`InvalidUrl` (source `InvalidPercentEncoding`); octal/hex-IP-obfuscation→reject. (В `HttpError.ErrorKind` таких kind'ов НЕТ — детали живут в `ParseUrlError` через `ErrSource.Url`, §3.9; форма `ParseUrlError`-kinds фиксируется при промоуте url.nv.) DEP: Ф.0, net.
  - **✅ Статус часть (2) net byte-surface — ПРИЗЕМЛЕНО 2026-07-04.** Additive `[]u8`-поверхность: `TcpStream`/`TcpReadHalf`/`TcpWriteHalf`.`read_bytes`/`write_bytes`/`write_all_bytes` + `UdpSocket`.`send_to_bytes`/`recv_from_bytes` — **публичные обёртки** над `str`-методами (НЕ effect-ops: `str.from_bytes_unchecked` на write, `.to_bytes()` на read; C-транспорт length-delimited → байт-чисто incl. embedded-NUL/не-UTF-8; обёртка вместо effect-op избегает vtable-erasure `[]u8`). `str`-варианты сохранены. **Net-унификация (§13.2) ⭐ ПРИЗЕМЛЕНА:** `TcpNet`+`UdpNet`+`DnsNet` → **единый `Net`** (реконсиляция к спеке D62) + один `real_net()`/`mock_net()`; **`AddrNet` РЕТРАКТИРОВАН** — addr-ops (`loopback`/`v4`/`from_str`/`@port`/`@ip`/`@is_v4`/`@is_v6`/`@to_str`) теперь pure `.nv` над `extern "C"` (FFI≠эффект); DNS `lookup` остаётся в `Net`. **Разблокирует 176 Ф.4(b).** Мигрированы ВСЕ call-sites (nova_tests/plan91_12·15·16; `with AddrNet=…, TcpNet=…` → `with Net = real_net()`). **Blast-radius:** 0 codegen-ссылок (grep net-эффектов по `**/*.rs` = 0); 100% .nv+C-runtime. Гейт: conformance 38/38; 19 non-slow + 7 slow net-тестов PASS; byte-surface mock-round-trip PASS. **Отклонения/followups (не «упрощения»):** (a) `bind` коллизия при слиянии → op-имена `tcp_bind`/`udp_bind`; (b) `SocketAddr` value-record rep-change **отложен** (byte-baseline-guarded, НЕ блокирует HTTP — план §3.10/§13.2) → `[M-net-socketaddr-value-record]`; handle-rep сохранён; (c) `socket_addr_parse` FFI: tuple `(int,CSocketAddr)` → `int`+TLS-accessor `socket_addr_parse_result()` (старый tuple-ABI полагался на legacy `_NovaTuple2`-erasure, снятую type-encoded tuple-naming Plan 172); (d) **починен pre-existing Plan-172 ICE** `[P67-LEGACY] Path call return type unknown for method=InvalidAddr/IoError` (sum-variant Path-call type-resolution в emit_c.rs — net не компилился ВООБЩЕ на main); (e) real-socket byte-round-trip slow-тест **заблокирован** pre-existing Plan-172 codegen-gap (`NetError.IoError` payload-variant mis-lower в undefined `_static_`-wrapper при byte-call-graph) → mock-тест валидирует поверхность; `[M-net-payload-variant-static-lowering]`; (f) кастомные partial `effect Net`-хендлеры в user-коде **невозможны** (opaque handles + `split_stream`-tuple) → focus-negative dns-тесты сняты (Err-propagation = тривиальная делегация). Часть (1) URL-промоут — **отдельно, НЕ сделана**.
- **Ф.1 — message-model. «сейчас».** `Method`(enum+`Other`), `StatusCode`(newtype+классы+`reason`+фабрики), `HeaderMap`(case-insens/ordered/multi-value; reject CRLF/NUL Q14; str↔[]u8 Q18; trailer), `Version`, `Url`(Ф.0.5), `Body`(**must-consume** Q3; in-mem`[]u8`\|stream`BodyReader`; bytes/text/json/drain/copy_to/into_reader/trailers consume; charset Q21; bomb-cap), `Mime`/`ContentType`, `Cookie`/`SetCookie`(RFC 6265bis send-инварианты), `HttpError`/`ErrorKind`(Q2). Чистые value+логика. spec: D358, D359. pos: HeaderMap case-insens/multi/ordered; StatusCode класс/reason; Method round-trip; Body.bytes consume; latin1 text-decode. neg: **body не consume→`EXPECT_COMPILE_ERROR`**(Q3); double-consume; CRLF в header→reject; non-token method→reject. DEP: Ф.0.5.
  - **✅ Статус — ПРИЗЕМЛЕНО 2026-07-04.** `std/http/` (module `std.http`, 10 co-equal .nv, чистая Nova, 0 codegen-ссылок): `error.nv`(`HttpError`+OPEN `ErrorKind`+`ErrSource` source-chain), `method.nv`, `status.nv`, `version.nv`, `url.nv`(**промоут** из `_experimental` + `parse`→Result + **строгий host/SSRF-валидатор** (bracket-IPv6/canonical-IPv4/reject hex-octal-decimal-IP-обфускации/control-whitespace) + `encode_query` multi-byte-FIX + self-contained `decode_query` UTF-8), `header.nv`(case-insens/multi/**CRLF-NUL-reject**/**CL+TE-smuggling-reject**), `mime.nv`, `cookie.nv`(**RFC 6265bis send-инварианты**: Secure-over-http/`__Host-`/`__Secure-`/SameSite=None⇒Secure), `body.nv`(**must-consume** `Body` D133), `message.nv`(`Request`/`Response` consume). **Гейт:** conformance **40/0** (+`d358_http_message_model` + neg `d359_body_not_consumed`/`d359_double_consume`); `nova_tests/http/` pos(url/model/body)+neg PASS; zero-regression (additive-only, бинарь не задет). **Амендменты по факту (D358/D359, §5 spec-first):** (a) `SameSite.None`→`Cross` (коллизия с `Option.None` в namespace импортёра); (b) `ErrSource.Url`→`UrlParse` (имя-вариант==имя-тип ломает codegen); (c) `ParseUrlError` tuple-варианты (record-варианты в `Option[sum]` mis-lower eq); (d) `HttpError` non-`value` (Option-of-usertype-поле → forward-ref codegen); (e) `Body` `consume` (НЕ `consume value`: копия оставляла бы поле владельца неразряженным); (f) конструктор Request/Response из СЫРЬЯ (`[]u8`/`BodyReader`) — move consume-переменной в поле не распознаётся checker'ом. **Гейтнуто в Ф.2+ (маркеры, НЕ упрощения):** `Http`-effect на consume-методах Body `[M-178-body-http-effect-surface]`; `@copy_to`/`@json[T]`/`@trailers` `[M-178-body-copy-json-trailers]`; charset-latin1 `@text` `[M-178-body-text-charset]`; `BodyReader` Option-EOF-форма (codegen eq-ordering) `[M-178-bodyreader-option-eof-eq-ordering]`; `ErrSource.Net/Utf8/Io/Compress/Tls` (Ф.2/176/179/116); typed `expires Timestamp` `[M-178-setcookie-expires-timestamp]`; builders `[M-178-message-builders]`. Старый `_experimental/encoding/url.nv` оставлен (0 импортёров; удаление — отдельный коммит).
- **Ф.2 — HTTP/1.1 client. «сейчас».** `Http`+`real_http()`(`Net`+`Time`, h1-парсер .nv; Net = ex-TcpNet+DnsNet §13.2)+`mock_http()` (Q1). `HttpClient`(pooled; builder: deadline/timeout (173), RedirectPolicy (Q9), default-headers, cookie_store (Q10), **Proxy+CONNECT-tunnel (Q23)**, decompress (Q12), **ssrf_guard (Q24)**, **retry (Q16)**). reqwest-builder; convenience `http.get` (Q15). pool+keep-alive+**eviction-on-error**+**idempotent-retry**; chunked TE+**trailers**; auto-decompress **🔴 HARD-GATE на Plan 179 Ф.1** (gzip/deflate/br + bomb-cap; dispatch по Content-Encoding + `ErrSource.Compress` — Ф.2-deliverable ЗДЕСЬ) — identity/chunked самодостаточны; auth (Q11); `error_for_status` (Q4); строгий парсинг (Q14); **1xx-interim loop** (103/множественные 100 — RFC 9110 §15.2); **`TE: trailers` auto** (keep-alive h1); **NO_PROXY-матрица** (D360); **`MockResponse` API** (new/header/body + инъекция delay/error для timeout/retry-neg) — deliverable. Request body: `[]u8`\|stream\|form\|multipart\|json(**gate Q20**). **Decompress gate: Plan 179 Ф.1** (inflate/gzip/zlib = pure-Nova, brotli = C-FFI Ф.2 — 179 Q1/Q2) — пока не приземлён, decompress-acceptance §8.3 gated. spec: D357, D360. pos: GET/POST mock; chunked decode+trailers; redirect-follow; pool-reuse; **reused-dead-conn retry**; 404=Ok (Q4); `error_for_status`; CONNECT-tunnel (plaintext-proxy); **1xx-interim** (103→200; 100-без-Expect); **NO_PROXY-матрица** (регистр/`*`/dot-suffix/CIDR/host:port/loopback-bypass); **TE: trailers шлётся**. neg: redirect-loop→`TooManyRedirects`; timeout→`Timeout`; **auth НЕ утекает cross-origin** (Q9); **POST не реплеится** (Q16); **errored-conn НЕ reused**; malformed status-line→`Protocol`; CL+TE→`Protocol` (Q14); bomb→`BodyTooLarge`; **SSRF private-target→`Blocked`** (Q24). DEP: Ф.1, 173, net byte-surface.
  - **✅ Статус — CORE ПРИЗЕМЛЁН 2026-07-04.** Приземлён **plaintext HTTP/1.1 client-core** (identity + chunked). Структура (ревизия §3/§6 по факту — nested submodules вместо flat `std.http`): **core `std.http`** (message-model Ф.1 + `effect.nv` `Http`-seam + `response_ext.nv` `Response.@header/@content_length/@cookies/@error_for_status`); **`std.http.client`** submodule (`client.nv` `HttpClient`/`HttpClientBuilder`/`RequestBuilder` + verb-shortcuts get/post/put/delete/patch/head + `header/append_header/query/body/text/json(dynamic)/form/bearer_auth/basic_auth` + redirect-loop + auth-strip + convenience `get/post/...`; `wire.nv` pure serialize/parse + **chunked-decode** + smuggling-reject; `mock.nv` `MockResponse`/`MockHttp`/`mock_http`; dynamic `Response.@json()->JsonValue` над `std/encoding/json`); **`std.http.transport`** submodule (`real_http()` над `Net` — effect-over-effect; https→`Err(Tls)` gated). **Гейт:** conformance **40/0** (не тронут); `nova_tests/http/client_test.nv` **14 pos через mock** (GET/POST round-trip; query+default-headers; **chunked-decode**; redirect-follow same/cross-origin; **cross-origin auth-strip** pos + same-origin preserve; too-many→`Err(TooManyRedirects)`; 404=валидный Response; `error_for_status` 2xx-pass + non-2xx→`Err(Status)`; malformed→`Err(Protocol)`; must-consume `.bytes()`/`.drain()`; dynamic json body+parse) PASS; `nova_tests/http_transport/transport_test.nv` (real_http https-gate) PASS; **neg** `neg/response_not_consumed.nv` (Response не consume → compile-error D133) PASS; zero-regression (additive .nv, codegen-бинарь не тронут). **Амендменты по факту (§5 spec-first, §6):** (a) **nested submodules** `std.http.client`/`std.http.transport` вместо flat — ВЫНУЖДЕНО двумя compiler-багами (см. ниже), заодно изолирует `std.net`+`json`-зависимости от lean core; (b) `Response`/`Request` (Ф.1-имена) вместо `HttpResponse`/`HttpRequest`; (c) `ParseUrlError.InvalidPort`→**`MalformedPort`** — whole-program конструктор-namespace коллизия с `NetError.InvalidPort` (любая программа с `real_http` линкует оба); (d) `mock_http().build()` работает НО GC-небезопасен → канон-установка = **inline-handler** `with Http = effect Http { send(..){ m.reply(request) } }` (frame-capture, conservative-GC-safe); `MockHttp.@reply` — deliverable; (e) `error_for_status` материализует+пересобирает Response (in-mem CORE), error-body дренится. **Gated (маркеры, НЕ упрощения):** timeout/deadline-by-default `[M-178-timeout-needs-173]`; auto-decompress `[M-178-autodecompress-needs-179]`; typed `.json[T]()` `[M-178-typed-json-needs-180]`; https/h2 `[M-178-https-needs-116]`; live keep-alive/pool-reuse `[M-178-client-live-pool]` (decision-logic+config есть, live socket-reuse отложен); Proxy/CONNECT/SSRF-guard/cookie-jar/idempotent-retry/1xx-interim/NO_PROXY/trailers/Expect-100 `[M-178-client-policy-surface]` (за CORE). **🔴 Два pre-existing compiler-бага заблокировали часть плана:** (1) **`d357_*`/`d360_*` conformance-фикстуры НЕ добавлены** — single-CU conformance линкует client-value-types рядом с `d43_trailing_block_and_fn`, что триггерит codegen forward-decl баг (`return_type_c`→`infer_expr_c_type` мис-выводит return-тип unit-возвращающего closure-call `d43_run_unit` → эмитит value-тип в forward-decl → `conflicting types` CC-FAIL); поведение D357/D360 покрыто через `nova_tests/http*` `[M-178-conformance-d357-d360-forwarddecl-bug]`; (2) `with`-блок с tail-`!!` (unit) конфликтует interrupt-return-тип с block-value-тип `[M-178-with-tail-bang-codegen]` (обход: закрывать блок `assert(...)`, не голым `!!`); (3) mock `.build()` heap-closure-env не GC-rooted `[M-178-mock-handler-gc-trace]` (обход: inline-handler). **Отложено (честный gate, НЕ в CORE):** typed serde-json, decompress, timeout-by-default, proxy, SSRF, cookie-jar, retry-pool-eviction, 1xx-loop, TE:trailers, Expect:100, https/h2, server (Ф.3).
  - **✅ Статус — Ф.2-enh AUTO-DECOMPRESS ПРИЗЕМЛЁН 2026-07-06** (разблокировано D381 + фикс `[M-codegen-cross-module-ctor-emission]`). `std.http.client`: `Accept-Encoding: gzip, deflate` по умолчанию (opt-out `@no_decompress()`), `finalize_response` прозрачно декодит `Content-Encoding` gzip/`x-gzip` (`gzip_decode`) + `deflate` (`zlib_decode`→raw `inflate` fallback), снимает `Content-Encoding` + переписывает `Content-Length`; bomb-guard `max_decompressed` (64 MiB default, D334; `@max_decompressed(n)`) → `Err(BodyTooLarge)`; decode-fail → `HttpError{Protocol}` + `ErrSource.Compress(CompressError)` (типизированный, `HttpError.from_compress`); `br` закрыт `[M-178-autodecompress-br]`. `CompressError.@is_bomb()` добавлен (bomb-детект без импорта OPEN `ErrorKind`-вариантов). **Гейт:** conformance **54/0** (не тронут); `nova_tests/http_decompress/decompress_test.nv` (круговой gzip+deflate round-trip через mock-encode 179; opt-out; neg bomb→`BodyTooLarge`) PASS; delta-0 (http/compress/io/fs на baseline И current). spec D-амендмент (02-types §D358 Ф.2). **`[M-178-autodecompress-needs-179]` CLOSED.** **Codegen-фикс `[M-codegen-cross-module-ctor-emission]` (keystone):** explicit-receiver payload-variant call `Sum.Variant(x)` (`NetError.IoError(msg)`) мис-роутился в undefined `Nova_<Sum>_static_<Variant>` (payload-вариант зарегистрирован как pseudo-static-overload) → фикс `try_emit_explicit_variant_ctor` в обоих static-emit-сайтах (Path + Member) → `nova_make_<sum>_<variant>`. Разблокировал net+http co-present link (servernet). Живой сокет-смоук `nova_tests/http_servernet/` ЛИНКУЕТСЯ, но runtime-gated pre-existing net-substrate segfault `[M-178-servernet-live-net-substrate-segfault]` (детерминирован на baseline+чистом net) — НЕ линкер.
...
- **NEW D357** — `Http`/`HttpServer` effect-контракт (`spec/decisions/04-effects.md`, рядом с net-блоками D295-DnsNet/D301-TCP-split, D210 Tls): client-seam ops (`send`/`connect_pool`); server-seam; `real_http()` requires `Net`+`Time` (ex-TcpNet+DnsNet — слиты в Net §13.2; +`Tls` для https, **инкапсулировано** Q30); h1/h2-framing = .nv (НЕ effect-op); mock-контракт; **net byte-surface-амендмент** (полный список §3.10, Q5).
...
**spec_tests/conformance — ОБЯЗАТЕЛЬНОЕ D-покрытие (Ред.2):** `spec_tests/conformance/d357_http_effect_contract.nv` (seam-опы, `real_http`-инкапсуляция Q30, mock-контракт) · `d358_http_message_model.nv` (Method/StatusCode/HeaderMap+CRLF-reject/host-валидатор/Cookie-инварианты/Version-OPEN) · `d359_body_must_consume.nv` + `neg/d359_body_not_consumed.nv`/`neg/d359_double_consume.nv` · `d360_client_policies.nv` (redirect/auth-strip/idempotent-retry+POST-never/pool-eviction/proxy-CONNECT/NO_PROXY/SSRF) · `d361_server_contract.nv` (404/405+Allow/path-normalize/Host-mandatory/CL-TE-reject/limits/backpressure) · `d362_http_tls_h2.nv` (gated Ф.4/Ф.5). **Amend-правило (фактчек Ред.2):** net byte-surface амендит **D173 (std/net, 08-runtime:3840) + D301 (TCP split, 04-effects:6029)**; SocketAddr-rep (§13.2) амендит **D364-NetFFI (ex-D292, 02-types:13185)** ⇒ существующий `d364_value_record_handle_wrap.nv` **обновить В ТОМ ЖЕ изменении**; у D173/D301 d-файлов НЕТ → NEW `d301_tcp_split_bytes.nv` (либо покрыть в d357_*). ⚠ Прежняя ссылка «D281» была ошибкой (D281 = field privacy); (D292-коллизия РАЗРЕШЕНА 2026-07-03: netFFI→D364; ModuleSigTable сохраняет D292; net-effect-family ex-D291→D365). Ф.0-verify: grep фактических net-D перед правкой; локалы с префиксом `dNNN_`; прогон `nova test spec_tests` отдельной командой.
...
0. **🔴 ОБЯЗАТЕЛЬНО: «без упрощений, как для прода».** Ни одного «решим потом» на критическом пути (парсинг, security: smuggling/injection/auth-strip/bomb-cap/SSRF/cookie-send-инварианты, must-consume, timeout-by-default, retry-idempotency, proxy-CONNECT) — на КАЖДОЙ приземлённой фазе (Ф.0–Ф.6). Каждая behavior-change — **pos+neg + аргумент звучности**; «звучность по построению»/корпус НЕ заменяют edge-тесты. **Гейт корректности (Ред.2):** `spec_tests/conformance` зелёный (d357–d362 + amended d292 / NEW d301) + pos/neg-фикстуры фазы + **nova_tests baseline-delta = 0** (baseline = parent-коммит через temp-worktree/commit+reset, ТОТ ЖЕ бинарь; nova_tests сам по себе НЕ гейт; флака ≠ регрессия; батчами <10мин). **Явно gated, НЕ «решим потом»:** `json()` (Q20, нет serde), gzip/br-decompress (Q12, нет codec) — исключены из landed-acceptance пока их зависимость открыта; identity/chunked-путь самодостаточен. **Явно scoped-out с rationale (НЕ violation):** non-UTF-8/latin1 charset (Q21), HTTP/3/WebSocket/server-push/h2-PRIORITY/h2c-Upgrade/zstd (§11).
...
8. **spec:** D357–D362 в `spec/decisions/` + spec_tests d357–d362 зелёные (amend d292 / NEW d301); Q9 закрыт; docs; §1a; Plan 116 forward-refs reconciled → 178; **Version-OPEN-extensibility в D358; BodyReader-pull в D359; NO_PROXY-матрица + loopback-bypass в D360** (Ред.2).
...
- **Plan 176 (fs/io)** — `Body.copy_to`/download-to-file + multipart-File-part + cert-from-file гейтят на fs; `impl io.Write`/`io.Read` для `Conn` — координация; **эффект-полиморфизм протоколов ✅ решён 2026-07-03** (176 Q15: конформер несёт свой эффект легально — D122+D62+mono; амендмент D42/D15 + D322-правило = 176 Ф.1; `BodyReader`↔io.Read — scope-out `[M-178-bodyreader-io-read]`); **176 Ф.4a NetError→IoError-projection трогает `ErrSource.Net`** — НЕ пересекать коммиты (176:330); `str.from_bytes` ← **176 Ф.0.5** (внешний prereq Ф.1).
...
- **`[M-178-bodyreader-io-read]`** — io.Read-conformance/adapter `@as_read()` для `BodyReader` (pull `@next_chunk` ↔ buf-форма `@read(buf)`; нужен для BufReader-over-Body). Эффект НЕ блокирует (176 Q15) — чисто shape-адаптер.
...
**Sequencing:** H1 — в Ф.0.5 (уже). **H2 + `@header`-семантика + discharge-таксономия** — приняты (sign-off 2026-06-27), см. §13. typed-json — по landing serde (Plan 180). Остальное MED/LOW — followup/polish.

---

## 13. Дизайн-решения по API-ревью (sign-off 2026-06-27) — логика выбора

Три fold-now-правки из §12 приняты. Записаны **с обоснованием выбора** (почему так, а не иначе).

### 13.1. Header-builder: `@header` = REPLACE, `@append_header` = APPEND (вариант B)

```nova
export fn RequestBuilder @header(name str, value str)        -> RequestBuilder   // ЗАМЕНЯЕТ все прежние (set) → HeaderMap.@insert
export fn RequestBuilder @append_header(name str, value str) -> RequestBuilder   // ДОБАВЛЯЕТ ещё (multi-value) → HeaderMap.@append
```
**Логика выбора (B, НЕ A=additive «@header=append + @set_header=replace»):**
- **Короткое имя делает частый intent.** `.header("Authorization", t)` = «поставить заголовок»; в Go `Header.Set`, fetch `Headers.set`, OkHttp `.header()`, Swift `setValue` короткое/основное имя = **replace**. Append (Accept, Set-Cookie) — редкий и теперь явный.
- **Footgun «тихая дупликация» убран в корне.** Вариант A его оставляет (короткий `@header` всё ещё append'ит → Go/fetch/OkHttp-юзер молча получает 2 заголовка). reqwest `.header()` append'ит — но 1 пример против 4 (Go/fetch/OkHttp/Swift) + sound-довод «короткое = частое».
- **Стоимость нулевая** (план не реализован → смена семантики = правка текста). **Само-согласованность:** builder = тонкий фасад над `HeaderMap.@insert`/`@append`.

### 13.2. net: слияние TcpNet/UdpNet/DnsNet → единый `Net` (реконсиляция к спеке D62) + `AddrNet`-retract в pure

```nova
// addr-операции БЕЗ эффекта (не делают I/O) — AddrNet ретрактируется:
...
export fn SocketAddr @port() -> u16   // + @ip/@is_v4/@is_v6/@to_str — все pure
// ЕДИНЫЙ эффект Net (owner-decision 2026-07-03 — реконсиляция к спеке D62, где Net канонизирован):
fn connect(a SocketAddr) Net -> Result[TcpStream, NetError]   // сигнатуры несут ОДИН Net (не Tcp/Udp/Dns)
with Net = real_net() { … }                                   // один handler ~37 ops (tcp+udp+dns)
```
**Логика выбора:**
- **Эффект = контракт «делаю I/O / parking / нужна capability».** `loopback`/`@port`/`v4`/`from_str` — чистые операции над значением-адресом (построить/распарсить/прочитать поле): нет syscall, нет parking, детерминированы. Навешивать `AddrNet` — **врать сигнатурой** (как no-op `set_nodelay`, §12 issue #7) и плодить церемонию `with` без нужды → `AddrNet` ретрактируется, операции pure.
- **Правило размещения (owner 2026-06-27):** метод живёт **В эффекте** ⟺ неразрывно завязан на него (I/O / parking / нужна capability); иначе — **pure `.nv`**. **FFI ≠ эффект** (чистый `extern "C"` для struct-операции — не эффект). Прогон: ВСЕ `AddrNet`-операции (`loopback`/`loopback_v6`/`v4`/`from_str`-numeric/`@port`/`@ip`/`@is_v4`/`@is_v6`/`@to_str`) — pure → `.nv`, `AddrNet` ретрактируется ПОЛНОСТЬЮ. **Единственная** address-операция, неразрывно завязанная на эффект — **DNS `SocketAddr.lookup(host, port) DnsNet -> Result[[]SocketAddr, NetError]`** (резолв host→адреса = сетевой I/O + parking) — **остаётся в `DnsNet`** (уже там).
- **`SocketAddr` → Nova value-record** `{family, addr-bytes, port}` (НЕ `value { priv handle CSocketAddr }`) — nv-sourcing: address-логика в `.nv`, как Rust `SocketAddr`-enum. Pure-операции тогда — чистая Nova **без FFI**; конверсия в C `sockaddr` — только на границе syscall'а внутри `connect`/`bind`.
- **Data vs effect (снять неоднозначность):** эффект-функции (`connect`/`bind`/`send_to`/`lookup`) **берут `SocketAddr` как ДАННЫЕ** (параметр/возврат) — это НЕ делает тип эффектным (как `File.@write(str) Fs` зависит от `str`, но `str` чистый). Критерий применяется к **операции** (нужен ли ей рантайм эффекта), а не к «упоминается ли тип»: `connect` → syscall+parking → в эффекте; `loopback`/`@port` → только считает → pure. Конверсия Nova↔C `sockaddr` живёт **внутри** `connect`/`recv_from` (FFI-граница), C-форма — лишь в момент syscall'а. Модель Rust `std::net::SocketAddr`.
- **Закреплено как net-задача:** §3.10 (rep-правка) · §6 Миграция · §9 (амендмент-список) — едет с byte-surface (`str`→`[]u8`), byte-baseline-guarded, **НЕ блокирует HTTP**.
- **⭐ Слияние TcpNet/UdpNet/DnsNet → единый `Net` (owner-decision 2026-07-03).** Прежняя формулировка «umbrella сохраняет гранулярные сигнатуры» **ЗАМЕНЕНА на слияние типов** по итогам исследования (spec/design/blast). **Обоснование:** (1) спека **D62 УЖЕ канонизирует ОДИН `Net`** (`04-effects.md:101/116/2822/2982`; пример языка `fn fetch(url) Net Fail -> Response`; sandbox-канон `forbid Net, Db, Fs`) — дробление на TcpNet/UdpNet/DnsNet (Plan 91.12, ex-D291→D365) было **необлагороженным отклонением от спеки**; слияние = **возврат кода к канону**, а не усложнение. (2) D62 «Почему» явно ценит короткий effect-row («растёт до 8-10 имён — тяжело читать»); гранулярность-ради-гранулярности в спеке отсутствует как принцип. (3) Утраченная «protocol-forbid»-точность (`forbid TcpNet` при `allow DnsNet`) нереалистична — реальный sandbox = `forbid Net` целиком; SSRF-guard (Q24) — value-логика, от эффектов не зависит. (4) Пир-прецедент: Deno (сильнейший capability-пир) = единый `--allow-net` (не делит tcp/udp/dns); единственный, кто делил resolve≠connect — deprecated Java SecurityManager. **Цена (принята):** 116 `real_tls()` требует `Net` вместо `TcpNet` — теряется описательная точность «TLS только TCP, не DTLS» (это НЕ гарантия — никто не пишет `forbid UdpNet` при TLS; DTLS-over-UDP = отдельный future-concern §11-116); единый `mock_net()` вместо триады `mock_tcp/udp/dns` (fokус-тесты дают дефолтные op-стабы). **AddrNet ретрактируется в pure независимо** (ниже — ортогонально). **Миграция:** едет ТЕМ ЖЕ net-sweep, что `str`→`[]u8` + SocketAddr-rep (§3.10/§6/§9) — 100% `.nv`+docs (grep net-эффектов по `**/*.rs` = 0; ноль codegen-риска), маргинальная доп-стоимость; отдельным заходом = второй полный sweep. Маркер `[M-net-merge-to-single-effect]` (backlog).
- **Закрывает H2** (`loopback(0)` без `with` — pure-addr; один `Net`-эффект + один `real_net()`-handler). Default-installed `real_http` для скрипт-`http.get` — оставшаяся часть H2, отдельным пунктом.

### 13.3. Discharge-таксономия (owner sign-off 2026-06-27)

| глагол | роль | слой |
|---|---|---|
| `@cleanup(o ScopeOutcome)` | **АВТО**-хук, зовёт компилятор в конце `consume X = e {}`, outcome-aware | протокол `Cleanup[E]` ([Plan 173 sign-off](173-error-system-unify-harden.md) #5: `Consumable[E]`→`Cleanup[E]`, `@on_exit`→`@cleanup`) |
| `@close()` | **РУЧНОЙ** teardown (no-arg), зовёшь сам | net/channels/File/простой http |
| `@drain()` | дочитать остаток → release (conn reusable, в пул) | http Body/HttpResponse |
| `@finish()` | завершить **ПИСАТЕЛЬ** (chunked-terminator) | ResponseWriter |
| ~~`@discard`~~ | — | **убран** (Body.`@discard`→`@drain`, §3 обновлён) |
