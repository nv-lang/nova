...
**spec_tests/conformance — ОБЯЗАТЕЛЬНОЕ D-покрытие (Ред.2):** `spec_tests/conformance/d357_http_effect_contract.nv` (seam-опы, `real_http`-инкапсуляция Q30, mock-контракт) · `d358_http_message_model.nv` (Method/StatusCode/HeaderMap+CRLF-reject/host-валидатор/Cookie-инварианты/Version-OPEN) · `d359_body_must_consume.nv` + `neg/d359_body_not_consumed.nv`/`neg/d359_double_consume.nv` · `d360_client_policies.nv` (redirect/auth-strip/idempotent-retry+POST-never/pool-eviction/proxy-CONNECT/NO_PROXY/SSRF) · `d361_server_contract.nv` (404/405+Allow/path-normalize/Host-mandatory/CL-TE-reject/limits/backpressure) · `d362_http_tls_h2.nv` (gated Ф.4/Ф.5). **Amend-правило (фактчек Ред.2):** net byte-surface амендит **D173 (std/net, 08-runtime:3840) + D301 (TCP split, 04-effects:6029)**; SocketAddr-rep (§13.2) амендит **D292-NetFFI (02-types:13185)** ⇒ существующий `d292_value_record_handle_wrap.nv` **обновить В ТОМ ЖЕ изменении**; у D173/D301 d-файлов НЕТ → NEW `d301_tcp_split_bytes.nv` (либо покрыть в d357_*). ⚠ Прежняя ссылка «D281» была ошибкой (D281 = field privacy); ⚠ в спеке **ДВА блока «D292»** (07-modules ModuleSigTable + 02-types Net-FFI) — дубль номера, разрулить нотой при амендменте (прецедент D109/D110/D111). Ф.0-verify: grep фактических net-D перед правкой; локалы с префиксом `dNNN_`; прогон `nova test spec_tests` отдельной командой.
