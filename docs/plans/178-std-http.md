...
### 13.2. net: `AddrNet`-операции → чистые (без эффекта) + umbrella `real_net()`
...
// umbrella: один with вместо четырёх (устанавливает TcpNet+UdpNet+DnsNet разом):
with Net = real_net() { … }   // ≡ with TcpNet=real_tcp_net(), UdpNet=real_udp_net(), DnsNet=real_dns_net()
```
**Логика выбора:**
...
- **Umbrella НЕ теряет гранулярность:** I/O-функции по-прежнему объявляют `TcpNet`/`DnsNet` в сигнатуре (видно, что делают I/O); меняется только **установка**. Прецедент в плане: `real_http()` инкапсулирует `Tls/TcpNet/DnsNet/Time` (§3.0 Q30) — `real_net()` симметричен; effect-bundle-install язык уже умеет.
- **Закрывает H2 на ~80%** (`loopback(0)` без `with`, один `real_net()` вместо 4). Default-installed `real_http` для скрипт-`http.get` — оставшаяся часть H2, отдельным пунктом.
