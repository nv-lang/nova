...
- **W2 (дизайн УТВЕРЖДЁН 2026-08-02, 221.1 №254; СПЕКА НАПИСАНА 2026-08-16 — [D464](../../spec/decisions/10-overloading.md#d464--бáунд-как-фильтр-отбора-отсев-без-ранжирования-2026-08-16))** — протокольный резолв:
...
> **W2 РАЗБЛОКИРОВАН СПЕКОЙ (2026-08-16).** Правило, которого W2 ждал, написано:
> [D464](../../spec/decisions/10-overloading.md) — бáунд есть ФИЛЬТР отбора между
> фильтром аргументов и фильтром результата (D84); кандидат с недержащимся бáундом
> отпадает; один выживший берётся молча; несколько без структурного доминирования —
> `ambiguous overload`; ноль — ошибка бáунда (D72). Ранжирования по бáундам НЕТ
> намеренно: «ошибка → ранжирование» — переход обратимый, обратный — нет (лагерь
> rustc/Go; у Swift ранжирование дало экспоненциальный резолвер). Проверка — в
> чекере, кодоген получает готовое решение каналом. Окно 274 реализует ту же норму
> в novac (этап Э2-б3) — два исполнителя одного правила, спека одна.
>
> **W6 ПОДТВЕРЖДЁН УЛИКОЙ, И ОНА НЕПРИЯТНАЯ (2026-08-16, реестр №696).** Single-key
> `method_receivers` (last-wins по имени) не просто жив — он уже дважды пропатчен
> ТОЧЕЧНО: планом 196.7 (закрыт 2026-07-15, грань bare-T бланкета `@to_str`) и
> сегодня, №696 (грань D381-квалификации: коллидирующий тип получает
> квалифицированное C-имя, таблицы методов ключуются голым — диспетч промахивается
> и падает в fallback к ЧУЖОМУ методу: чужая арность, чужой тип результата,
> мискомпиляция без диагностики). Вывод для W6: **закрытие 196.7 было неполным** —
> канал `resolved_callees` не покрыл тела ИМПОРТИРОВАННЫХ модулей (класс №669,
> «канал per-module_env»), и именно там стоял вызов в репро окна 274. Пока канал не
> доведён до всех тел CU, fallback НЕ станет мёртвым кодом, и каждая новая грань
> будет стоить точечного патча. **Точечные патчи на новые грани запрещены** (их уже
> два) — следующая грань идёт этим окном целиком.
