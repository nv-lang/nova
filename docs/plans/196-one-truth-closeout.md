...
`infer_call_ret_c` — НЕ «одно окно, что всё умеет». Это **ВТОРОЕ** окно (legacy codegen-перевывод),
дублирующее чекер. Одно окно = **ЧЕКЕР** (`resolved_types`, D315). Почему чекер лучше:
