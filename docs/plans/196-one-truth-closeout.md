...
**★ ЦИКЛ ЗАКРЫТИЯ КАЖДОГО D (дисциплина, директива владельца 2026-07-12) — ОБЯЗАТЕЛЬНО:** тест → миграция →
закрытие (remove/panic) → **(1) отчёт** (ледер `196.wave2-progress.md`) → **(2) коммит** → **(3) обновление
плана** (D помечен закрытым) → **(4) СИНК С MAIN** (`fetch github main` → merge main в ветку [взять свежее,
вкл. волну-1] → merge D-закрытия в main, push github+gitverse). Частый per-D синк = обе волны сходятся на main,
не расходятся (нет гигантского merge в конце). Тот же per-batch синк применим и к волне-1.
