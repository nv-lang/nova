...
## ★ Волновой план добивания «мест на удаче» (владелец 2026-08-02: «есть план на одно окно? можем всё сейчас?»)

Честный ответ владельцу: ОДНИМ окном нельзя — все волны в одной зоне
(types/mod.rs + emit_c), идут строго последовательно; объём — конвейер.
План (каждая волна = одно окно, гейты по разделу «Гейты» ниже):

- **W1 (В ПОЛЁТЕ)** — операторный канал: 13 операторов + compound + сравнения
  + value-record + сужение `!` (D46-ретракция) → resolved_callees.
- **W2 (дизайн УТВЕРЖДЁН 2026-08-02, 221.1 №254)** — протокольный резолв:
  bound-чек generic-кандидатов (непрошедшие отпадают, не last-wins) +
  специфичность «полное соответствие протоколу, собственный носитель > делегат»
  + E_AMBIGUOUS + Iter-перегрузки комбинаторов в std + T-binding фикс.
- **W3** — bare-варианты enum: общий контекстный резолв (ожидаемый тип из
  return/параметра/скрутини) вместо алфавитного tie-break; E_AMBIGUOUS при
  остатке. Обобщает точечный ok_or-фикс №253; закрывает класс №27.
  Координация с №136-окном манглинга (параллельная сессия) — не дублировать.
- **W4** — P67-аннотации как контракт-фаза: ICE-пачка одним-двумя окнами
  (221.1 №229/№230/№235/№237/№241, №19, №126, №73 — все «checker must
  annotate») → resolved_types обязателен на этих классах узлов, паника
  заменяется ранней диагностикой.
- **W5** — мономорфизация ФАЗОЙ-коллектором (rustc-эталон, worklist до
  эмиссии; закрывает класс №250-фазовости системно); RET-слот-исследование
  примыкает сюда же (инвентаризация → единый путь).
- **W6** — снос single-key fallback + мёртвых веток (B12h и родня) после
  W1/W2/W4: когда чекер аннотирует всё, fallback обязан стать мёртвым кодом
  → удаление + ratchet ВНИЗ (это и есть выход «долга 62947»).

**ПОРЯДОК ПО КРИТИЧНОСТИ (владелец 2026-08-02: «сначала критичные»).**
Критерий: тихая порча данных > тихий неверный тип > поздний ICE > архитектура
> уборка. Все окна — одна зона (types/mod.rs + emit_c), строго последовательно:
W1 (летит) → **S1a звучность захватов** (№150 файбер-гонка + №117 share×fn +
№242§3 escaping — порча данных при молчащем check; + №168 handler-coercion,
блокер тега, соседняя зона эффектов) → **S1b каналы** (№143 Channel[T] —
настоящий дженерик с .nv-декларацией §3 + №144 send-consume P0; ПОДНЯТО
владельцем 2026-08-02: «Channel[T] — тип на дженерике» — договорённость,
не очередь) → **W2** (№254, утверждён критическим) → **W2a** (№259
sentinel-конфляция — вне очереди, ТОП-1 аудита) → **W3** (bare-варианты — тихий чужой тип) → **W4** (P67
ICE-пачка — громкие) → **W5** (mono-фаза) → **W6** (снос fallback, ratchet вниз).
Вне зоны параллельно: BigInt-кортежи (сразу после W1 — ждёт №247 оттуда),
LSP-пачка 104.x, polaris/222-хвосты, Ф.2/Ф.3.

**Параллельная дорожка (звучность, НЕ 196-зона, свои окна):** №150 (файбер-
гонка + модель памяти), №117 (share × fn-значения), №143 (типизация каналов),
№172 (requires-энфорс), №120 (нетипизированные хендлер-параметры).

**Аудит-волна (запущена 2026-08-02, opus, read-only)** — каталог «мест на
удаче» (fallback/tie-break/молчаливые дефолты/текстовые контракты) по
emit_c/types → каждой находке № в 221.1; питает W3-W6 и звучность-дорожку.
...
Следующие пункты требуют проверки перед/в ходе продолжения 196 — структурные ограничения и зависимости, не учтённые в текущем реестре:

### B11q/B11r root-cause — `gs` без баундов, 31 сайт, не точечный фикс

Корень B11q/B11r найден (closeout-волна 2026-07-21): `gs` (generics in scope) несёт ТОЛЬКО имена (`&HashSet<String>`), баунды протоколов теряются до резолва вызова. 31 сайт в `types/mod.rs` — это НЕ «ещё одна ветка 196.2», это редизайн того, как чекер носит generic-параметры. **Без этого никакой `Option[T Debug]@debug`/`Result[T,E Debug]@debug` не переедет из legacy codegen в канал.** Оценка объёма не сделана; риск: фикс может потребовать нового cross-cutting состояния (31 сайт = широкий blast radius). **Рекомендация:** до следующей closeout-волны — спайк на оценку миграции `gs`: (а) что именно нужно хранить (protocol bounds, конкретные типы, другое); (б) минимальное изменение 31 сайта; (в) не упирается ли это в ограничение `ExprId`-ключа (один generic-параметр в разных вызовах имеет разные bounds?).

### СПАЙК ВЫПОЛНЕН 2026-07-30 — параллель ПОДТВЕРЖДЕНА кодом, масштаб измерен

Разведка (opencode/deepseek-v4-flash-free `--variant max`, worktree `nova-p196gs`,
ветка `p196-gs-spike`, коммит `ecfa653aa`); полный документ — [wip/196-gs-spike.md](wip/196-gs-spike.md).
**Ключевые утверждения перепроверены интегратором ПО КОДУ, не по отчёту.**

**(1) Чего не хватает.** `gs: &HashSet<String>` несёт только имена; теряются
`bounds` (главное), `default`, `consume_bound`, `span` — все они ЕСТЬ в
`GenericParam` (`ast/mod.rs:811-826`, проверено). Предложение — носить
`GenericParamInfo` (имя + bounds + default + consume_bound).

**НАХОДКА, которой в плане не было: обходной механизм УЖЕ СУЩЕСТВУЕТ.** Поле
`current_fn_generics: RefCell<Vec<GenericParam>>` (`types/mod.rs:3519`, проверено)
несёт ПОЛНЫЕ `GenericParam` с баундами — но используется ровно одним потребителем,
`resolve_generic_bound_method_return` (`types/mod.rs:11565`), для узкого случая
match-scrutinee. То есть паттерн «протащить полный GenericParam» в чекере уже
реализован и работает; миграция `gs` — обобщение существующего решения, а не
изобретение нового. Это СНИЖАЕТ оценку риска по сравнению с записью плана.

**(2) Масштаб.** 39 сигнатур (план говорил «31»): **22 — чистый pass-through**
(механическая замена типа), остальные читают содержимое и требуют ручной работы.
То есть «31 сайт = широкий blast radius» — верно лишь наполовину: большинство
правок механические.

**(4) Параллель с №150 — ПОДТВЕРЖДЕНА.** Один механизм: «TypeVar с attached
constraints». Protocol bounds — компиляторная проверка «есть ли `@debug` у `T`»;
share-ness — ограничение «можно ли отдать `T` за границу файбера»; в нашей
архитектуре второе выражается как ещё один bound (`T Share`), то есть №150
сводится к частному случаю общей задачи. **Следствие для порядка работ:**
расширение TypeVar проектируется ОДИН раз под оба класса — иначе энфорс №150 и
closeout 196 будут строить одну и ту же абстракцию дважды и разойдутся в деталях.

### Параллель `gs` ↔ №150 — общий механизм TypeVar с constraints

`gs` теряет баунды generic-параметра — share-ness теряет транзитивную достижимость через границу файбера (№150). По сути ОДНА проблема: checker не умеет нести мета-свойства (protocol bounds, shareness, thread-safety) сквозь generic-параметры от объявления до точки использования. Если проектировать решение для B11q (TypeVar с attached constraints), оно потенциально закрывает и №150 (share-ness как constraint на TypeVar). **Рекомендация:** не чинить B11q изолированно — спроектировать расширение TypeVar один раз, под оба класса. Вынести в отдельный дизайн-документ (D-блок) до реализации.

### ~~Мега-CU гейт сломан — d216_ptr_methods_174_5.nv падает на pristine main~~ — СНЯТО (устаревшая посылка)

**Пункт снят интегратором при вливании ветки (2026-07-29): опирался на состояние
closeout-волны 21.07 и на момент написания перепроверки уже был неверен.** Маркер
`[M-d216-write-at-return-type-unknown-cc-panic]` **ЗАКРЫТ 2026-07-21** — фикс
`75ea680f5` (`infer_expr_c_type`, Block-trailing arm не пре-регистрировала
top-level let-локали своего же блока, из-за чего ресивер, связанный ранее в том же
unsafe-блоке, резолвился в `obj_ty=""` и бил в hard-panic P67-LEGACY), влит в main
слиянием `8e843e2ac`. Проверено: `git merge-base --is-ancestor 75ea680f5 main` → да.
Вывод «пока это не починено, 196 не может закрыть ни одну волну с гарантией» —
недействителен, d216 в мега-CU зелёный.

**Что из пункта остаётся в силе:** подпункт (в) — ассерт в `test_runner.rs`
«сумма PASS+FAIL+SKIP = число .nv-файлов». Он НЕ про d216, а про то, что шард-краш
может ТИХО съесть хвост очереди, а счётчик этого не покажет — самостоятельная
P0-эскалация в 221.1, живая. Сюда же: CI-гейт (и `.github/workflows/nova-gate.yml`,
и новый `.sourcecraft/ci.yaml`) проверяет только код возврата и НЕ ассертит строку
`PASS: N FAIL: 0` — та же дыра, что и в локальном фильтре (краш не печатает строку
вовсе). Чинить вместе с (в).

### Wave-1 (ветка-за-веткой) пробуксовывает — Wave-2 (D-driven) как основной

Closeout-волна показала: обе попытки сноса B11q/B11r/B10m наткнулись на живые corpus-хиты на ПЕРВОМ же репро. Wave-1 (ветка-за-веткой `infer_call_ret_c`) пробуксовывает, потому что класс-C-ветки взаимосвязаны — выдёргивание одной ломает другие (те же 31 сайт `gs`). Wave-2 (D-driven, feature → checker channel → delete) архитектурно правильнее, но медленнее. **Рекомендация:** сделать Wave-2 ЕДИНСТВЕННЫм путём; Wave-1 переквалифицировать в мониторинг (гейт-1 строго убывает, но форсировать снос не пытаться). Иначе ресурсы уходят в verify-and-retry (тот же паттерн, что Ф.4a/b/c-retracted).

### Хардкод-аудит — пост-релиз, не в 196

452 сайта (144 долга по §3) — ~8 sonnet-волн. Ни один из них не блокирует v0.1. Внесение в 196 сейчас = распыление. Детектор-сторож (`hardcode-audit.sh`) уже на месте — рост не уйдёт незамеченным. **Рекомендация:** вынести из 196-объёма явно; завести как отдельный подплан после v0.1, с классификацией по категориям (B→E→F→A→D) как указано.
