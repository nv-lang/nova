...
### Мега-CU гейт сломан — d216_ptr_methods_174_5.nv падает на pristine main

Документировано closeout-волной (2026-07-21): `spec_tests/conformance/d216_ptr_methods_174_5.nv` падает P67-LEGACY на чистом main (регрессия p208-sh4-teardown). Любая 196-волна, полагающаяся на `nova test spec_tests/conformance` как авторитетный гейт, работает вслепую — зелёный прогон может быть ложным (из-за недостижения сломанного файла шардом) или красный из-за pre-existing. **Пока это не починено, 196 не может закрыть ни одну волну с гарантией.** **Рекомендация:** (а) изолировать d216 в known_red/ с явным маркером; (б) починить как P1-пререквизит 196 (sonnet-окно); (в) добавить assert в test_runner: сумма PASS+FAIL+SKIP = число .nv-файлов (см. P0-эскалацию 221.1).
