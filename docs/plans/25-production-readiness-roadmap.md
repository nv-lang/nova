...
**Blocker до closing'а:** Plan 83 (M:N runtime) (M:N
...
**Status:** не открыта работа. Сначала Plan 83 (M:N) — он не зависит
...
#### G3c. Дорожная карта GC после Boehm (v1.0+)

**Update 2026-05-14.** Зафиксировано решение по кандидатам на
production GC (после того, как Boehm доказал bootstrap-достаточность).

##### Почему нельзя взять GC из V8 или Go

- **V8 GC** — не библиотека. Намертво вшит в V8 object model
  (каждый объект наследует `HeapObject`, hidden classes, C++ CRTP).
  Наш codegen генерирует plain C — интеграция потребует полной
  переписи codegen на C++ + адаптации всего object model.
  **Нереалистично.**

- **cppgc** (Google выделили часть V8's GC в standalone, BSD-3) —
  теоретически встраиваемая, но требует C++ (`GarbageCollected<T>`,
  `Member<T>` write-barrier wrappers, Visitor на каждом типе).
  Опять переписать codegen с C на C++. **Непропорционально сложно.**

- **Go GC** — не standalone. Глубоко интегрирован с goroutine
  scheduler, growable stacks, runtime. Выдернуть нельзя.

##### Правильный кандидат: MMTk

**MMTk (Memory Management Toolkit)** — Rust-native GC framework,
спроектированный именно для встройки в чужие language runtimes:

- Написан на **Rust** — нативно в нашем toolchain
- Есть **C API** (FFI из compiler-codegen)
- Уже используется: OpenJDK, Ruby, Julia, V8 (эксперимент)
- Generational + concurrent + precise — паритет с Go/V8
- Лицензия: Apache 2.0

Альтернатива — написать свой concurrent GC (больше контроля,
больше работы). Решение — decision point когда дойдём до v1.0.

##### Главный prerequisite: stack maps в compiler

**Любой precise GC (MMTk, свой, cppgc) требует одного:**
компилятор должен для каждого stack frame знать **точно** какие
слоты содержат указатели (GC roots), а какие — int/float/bytes.
Это называется **stack maps** (или pointer maps, GC maps).

Распределение работы примерно:

- **~80% усилий** — добавить stack map emission в `compiler-codegen`
  (для каждой функции emit'ить таблицу: `[offset: u32, is_ptr: bool]`
  per stack slot per safepoint).
- **~20% усилий** — сам GC алгоритм (если MMTk — интеграция через
  C API; если свой — реализация).

Пока stack maps нет — Boehm conservative остаётся единственным
вариантом без massive rewrite.

##### Когда начинать

Decision point: **после того, как language semantics + stdlib
стабилизированы** (v1.0 feature freeze). До этого момента:
- Boehm достаточен для bootstrap workloads
- Инвестиции в GC раньше этой точки — premature optimization
- ABI compiler'а ещё меняется → stack maps придётся переписывать

**Action item v1.0:** spike «stack map emission» как первый шаг G3c.
Когда spike готов — отдельный план «Plan 4X: precise GC».
...
**Status:** не открыта работа. Скорее всего — после Plan 83 (M:N),
...
|---|---|---|---|---|
| G1 | Single-threaded scheduler | Plan 83 (M:N runtime) | **Высокий** | Multi-core unblock |
| G2 | Fixed fiber stacks | TBD после Plan 83 | Средний | 1M fibers target |
...
| G5 | Preemption budget | TBD после Plan 83 | Средний | Long-compute fairness |
...
> real-time use-cases — требуют Plan 83 (M:N runtime) и GC pause
...
4. **Plan 83 (M:N runtime, = G1)** — самый большой рычаг. После него
   Nova competitive на multi-core. См. отдельный roadmap.
5. **G5/G6 preemption** — после Plan 83, потому что M:N меняет
...
- **Performance benchmarks.** Бенчмарки появятся вместе с
  implementation (G1 — Plan 83 bench, G3 — Step 1, G5 — TBD).
...
- **Plan 83** — M:N roadmap, закрывает G1. Самостоятельный план.
