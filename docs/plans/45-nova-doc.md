...
| Ф.21.2 | **Cross-module intra-doc-links не резолвятся** — `resolve_imports_inline` отключён в cmd_doc; `[std.io.println]` → broken. Honest fix требует multi-module DocTree (file_id → module_path mapping), что де-факто = workspace mode. | Ф.6 single-file-only | ⏳ **deferred → Ф.21.7** (одна и та же инфраструктура: multi-module DocTree). Партизанский half-fix давал бы semantic-wrong target_ids, что хуже broken-link. |
