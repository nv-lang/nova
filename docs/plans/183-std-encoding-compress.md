...
# Plan 183 — std/encoding/compress (DEFLATE / zlib / gzip / brotli)

> **Top-level** под-план. Создан 2026-06-26. Статус **proposed**. Маркер **[M-183-std-compress]**.
> **Запуск:** «выполни план 183».
...
> **D-блоки (NEW):** **D333** (codec-контракт: PURE-codec без effect + byte-first + `CompressError`) · **D334** (bomb-cap DoS-инвариант — обязательный `max_output`) · **D335** (streaming incremental coder `feed`/`read`/`finish` + Plan 182 `BodyReader`-мост) · **D336** (checksum-контракт CRC-32/Adler-32/ISIZE-verify) · **D337** (brotli C-FFI-контракт + bomb-over-FFI). ⚠ **D316–D324** (179/180), **D327–D332** (182) ещё **НЕ внесены** в `spec/decisions/` (committed до D326) → берём **D333+** безусловно, gap фиксируем в индексе **как Plan 181 §«D316–D324 зарезервированы»** (Ф.0).
> **HARD-GATES/DEPS:** **этот план Ф.1 = 🔴 HARD-GATE-OPENER для Plan 182 Q12** (auto-decompress gzip/deflate **сейчас**; br — после Ф.2). DEP: Plan 181 (D325 Result-everywhere), Plan 180 (byte-first / must-consume / `io.Write` для followup copy_to), Plan 182 (consumer; schedule-coord). Brotli Ф.2 — gated на vendor `libbrotlidec` (build-infra verify Ф.0).
> **Координация:** Plan 182 (consumer — reconcile Q12/§3.5/§9/§11, **+ amend «C-zlib FFI»→pure-Nova**, см. §6), Plan 181 (D325), Plan 180 (byte-first), `_experimental/checksums/crc32.nv` (промоут/реюз owner-sign-off).
> **Сквозной критерий (§8.0):** **без упрощений, как для прода** — bomb-cap НЕ опционально (§8.0-security-critical, критпуть Plan 182); checksum-verify by-default; streaming-память-bounded доказывается; brotli/encode честно **gated/scoped** с rationale.

---

## 1. Зачем

В Nova **нет `std/encoding/compress`** — ни inflate, ни deflate, ни gzip/zlib/brotli. Это прямо **держит Plan 182 Ф.2** (HTTP-клиент): авто-decompress ответа по `Content-Encoding: gzip|deflate|br` — **🔴 HARD-GATE на этот под-план** ([182:865](182-std-http.md): «gzip/deflate/br… 🔴 HARD-GATE на NEW sub-plan `std/encoding/compress`»; [182:999](182-std-http.md): «gzip/deflate/brotli НЕ существуют»; bomb→`BodyTooLarge` [182:956]). Без compress HTTP-клиент Nova получает на проводе сжатое тело и **не может его прочитать** — а `gzip`/`br` шлёт по умолчанию практически каждый современный сервер (CDN/API). То есть **без этого под-плана REST-клиент Nova нефункционален на реальном вебе**, не «менее эргономичен», а буквально получает байты, которые не декодирует.
...
**Что разблокирует:** (1) **Plan 182 Ф.2 auto-decompress** — `Inflater`/`GzipReader`/`BrotliReader` стримят чанки `BodyReader` chunk-by-chunk (НЕ one-shot — интегрируется с `BodyReader.@next_chunk` [182:224]), с **bomb-cap** (`max_decompressed`, дефолт 100 MiB [182:380]) → `CompressError{Bomb}` на превышении; (2) **server-side encode** — `mw_compress` (gzip/br по `Accept-Encoding`) и request-body compression на upload; (3) **архивы/форматы** — `.gz`/zlib (PNG/zip-член/`tar.gz`), registry-tarball'ы Plan 03.x, gRPC-message-compression. **Приоритет — DECOMPRESSION (inflate):** Ф.1 (raw-DEFLATE + zlib + gzip decode, streaming + bomb-cap) приземляется **первой** и закрывает HTTP-gate; brotli-decode (Ф.2) и encode (Ф.3) — следом; **zstd → followup** (§11).

> **⚠ Reconcile-нота (gate-producer vs gate-consumer):** Plan 182 в трёх местах формулирует кодеки как **«Nova-logic над C-zlib FFI»** ([182:488], [182:865 «Nova-logic над C-zlib FFI (nv-sourcing)»], §3.5). **Этот план решает иначе** (§3.0 Q1): inflate/zlib/gzip = **чистая Nova, БЕЗ C** (прецедент Zig/Go); C-FFI — **только brotli**. Plan 183 — source-of-truth по pure-vs-C (это его домен); **Ф.0 правит формулировку 182** (Q12/§3.5/§9) на «inflate/gzip/zlib = pure-Nova (183 Q1); C-FFI только brotli (183 Q2)» — иначе синтезатор, читающий оба плана, получает прямое противоречие (§6/§9).

## 1a. Где Nova ЛУЧШЕ peers (differentiators — в доку)

- **🏆 bomb-cap-by-DEFAULT (decompression-DoS by construction):** **каждый** decode (`inflate`/`gzip_decode`/`Inflater.feed`) принимает `max_output` и при превышении возвращает **`CompressError{kind: Bomb(limit)}`** — НЕ растёт безгранично. Decompression-bomb (10 KB → 10 GB, классический DoS — `42.zip`) — **известнейшая дыра HTTP-клиентов/архиваторов**; Go `flate.NewReader`/Rust `flate2`/Java `Inflater` по умолчанию распаковывают **без лимита** (нужно вручную городить `io.LimitReader`/счётчик — все забывают). Node добавил `maxOutputLength` лишь после CVE-волны, и он **opt-in**. У Nova лимит — **обязательный параметр API** (`max_output` — see-it-in-the-signature), §8.0-security-критический, прямо служит Plan 182. **Никакой peer не делает cap дефолтом сигнатуры.** *(Escape-hatch `max_output=0`=без-лимита существует для caller-trust low-level, но Plan 182 ВСЕГДА передаёт реальный cap — §3.3.)*
...
- **byte-first done RIGHT:** вход/выход кодеков — **`[]u8`** (компрессия — байтовая операция, текста на этом уровне нет); никакого `str` в API кодека. Совпадает с zlib-ABI и byte-first-курсом std ([180:62]). **PURE codec — БЕЗ effect-триады** (нет I/O: чистая CPU-трансформация байт→байт): просто fallible-функции + value-type coder — сознательно проще net/fs (которым нужен `Http`/`Fs`-seam); конвенция различает effect-subsystem (триада) и pure-codec (нет seam), precedent `json`/`base64`.
...
**Plan 182-интеграция (HARD-GATE 182 Q12).** Стримовые кодеры — value-типы, **оборачивающие любой источник `[]u8`-чанков** (не только `BodyReader`): `feed(chunk)` накапливает вход; `read(max_emit) -> Result[Option[[]u8], _]` тянет ≤ `max_emit` раскодированных байт (`None` = stream-end); `finish()` валидирует завершённость+checksum. Plan 182 `BodyReader.@next_chunk` ([182:224]) → `gz.feed(chunk)` → `gz.read(budget)` → отдаёт раскодированный чанк наружу (park/wake-backpressure на стороне 182-транспорта; сам кодек без effect). **bomb-cap** — first-class: накопленный выход > `max_output` → `CompressError{Bomb}` **немедленно** (не дочитав вход). Plan 182 пробрасывает `@max_decompressed(n)` (дефолт 100 MiB [182:380]) → `max_output`. Это **§8.0-критичный security-инвариант**.
...
### 3.3. One-shot decode (PRIORITY — анблокит Plan 182 Ф.2)

```nova
/// Raw DEFLATE (RFC 1951): голый битстрим без фрейма/чексуммы. `max_output` — bomb-cap
/// (выход > лимита → CompressError{Bomb}). 0 = без лимита (caller-trust low-level; Plan 182
...
### 3.4. STREAMING decode — инкрементальные кодеры (Plan 182 BodyReader)
...
**Plan 182 BodyReader-композиция (контракт-доказательство; glue в `real_http` Plan 182, НЕ в compress — направление зависимости: compress НЕ знает про http, §6):**

```nova
// псевдо-glue в real_http (Plan 182): decompress-обёртка над BodyReader
loop {
    match raw_reader.@next_chunk()? {            // Http-park над транспортом ([182:224])
        Some(enc) => {
...
**Единый feed/read/finish-контракт у всех ридеров** — Plan 182 выбирает кодер по `Content-Encoding` и работает полиморфно. Опц. протокол `ChunkDecoder` (`mut @feed; mut @read; mut @finish; @is_done`) — **деталь Plan 182** (там dispatch по encoding); **compress его НЕ экспортирует** (избегаем навязывания trait; duck-typing/match на стороне http).
...
| Q4 | streaming-модель | **value-кодер `feed`/`read(max_emit)`/`finish`**; единый контракт raw/gzip/zlib/brotli; `read` отдаёт ≤ max_emit (bounded-per-call) | компонуется над любым `[]u8`-источником (Plan 182 `BodyReader`); Python `decompressobj(...,max_length)` — единственный peer с per-call-cap |
| Q5 | bomb-cap (DoS) | **first-class `max_output`** (one-shot + streaming + brotli-FFI); накопл. выход/прогресс-вход > лимита → `Bomb` НЕМЕДЛЕННО; cap капит **И вход** (anti-flood); 0=trust (low-level) | §8.0-security; Plan 182 пробрасывает `@max_decompressed` (100 MiB); Go исторически **без cap** (CVE-класс) |
...
| Q11 | brotli-libs недоступны | **feature-gate**: `brotli_*`/`BrotliReader.new` → `UnsupportedMethod` (не паника); Ф.1 самодостаточна | сборка без C-deps не падает; Plan 182 gzip/deflate работает независимо |
| Q12 | целочисленность/overflow | **`int`(i64) для размеров/length/distance/offset; `u32` только CRC/Adler/ISIZE**; ISIZE-cmp = `mod 2^32`; bit-reader bounds-checked | Plan 180-конвенция [180:74]; >4 GiB-ISIZE-wrap НЕ Checksum; crafted distance/length → `InvalidData`, не OOB |
...
| Q16 | zstd | **followup §11** (не V1) | RFC 8878; HTTP `zstd` редок (2024+); Plan 182 не требует; Zig — единственный pure-в-std |
OPEN: ["МОДУЛЬНАЯ РАСКЛАДКА (folder=один модуль std.encoding.compress): error.nv (CompressError) · checksum.nv (CRC-32 промоут + Adler-32 NEW) · inflate.nv (bit-reader + Huffman + Inflater + one-shot decode) · deflate.nv (Deflater + encode, Ф.3) · gzip.nv (GzipReader/GzipWriter framing) · zlib.nv (ZlibReader/ZlibWriter framing) · brotli.nv (BrotliReader Nova-API) + ffi.nv (extern C, Ф.2) · mod.nv (re-export). Подтвердить раскладку при реализации (§6).","CRC32-VALUE-TYPE: §3.2 решено НЕ делать value-type Crc32/Adler32 в V1 (free-functions реюз). Если при framing-реализации окажется удобнее инкрементальный value-объект — добавить как followup, не ломая free-fn API.","ChunkDecoder-ПРОТОКОЛ: решено НЕ экспортировать из compress (деталь Plan 182 dispatch). Если 182-реализация покажет, что общий protocol-bound нужен на стороне compress для version-transparent-обёртки — пересмотреть в координации с 182."]

---

## 4. Фазы

**Dep-chain:** Ф.0 → **Ф.1 (inflate: raw-DEFLATE + zlib + gzip decode, streaming + bomb-cap)** → Ф.3 (encode) → Ф.6 (tests+docs); **Ф.2 (brotli decode, C-FFI)** параллельна Ф.3 после Ф.1. **«сейчас» (UNBLOCKS Plan 182 Ф.2):** Ф.0, Ф.1, Ф.6(decode-часть). **«позже»:** Ф.2 (brotli), Ф.3 (encode), Ф.6(encode/brotli-часть). Коммит после каждой фазы (§10).

> **🎯 Gate-релевантность:** **Plan 182 Q12 (auto-decompress) — 🔴 HARD-GATE именно на Ф.1.** Ф.1 **самодостаточна** и приземляет gate для gzip/deflate **сейчас**; `br`-ветка 182 gated на Ф.2. Ф.3 (encode) и Ф.2 (brotli) — для полноты, НЕ на критпути 182-decode.

- **Ф.0 — GATE (без кода). «сейчас».** (1) Закрыть §3.0 (готово). (2) **D-номера:** verify, что D316–D324 (179/180) и D327–D332 (182) **ещё не в** `spec/decisions/` (committed до D326 — подтверждено) → взять **D333–D337** безусловно; **записать reservation-ноту СЕЙЧАС** в `docs/plans/README.md`/spec-index **как Plan 181 §«D316–D324 зарезервированы»**: «D327–D332 reserved Plan 182, D333–D337 reserved Plan 183» (чтобы конкурентная 182-работа не схватила D333+ — критик-gap). (3) **🔴 RECONCILE Plan 182:** amend [182:488], [182:865 Q12], §3.5 — «Nova-logic над **C-zlib FFI**» → «inflate/gzip/zlib = **pure-Nova** (183 Q1); C-FFI только brotli (183 Q2)»; обновить указатель «NEW под-план std/encoding/compress» → **Plan 183** (Q12/§9/§11). (4) **verify на main:** (a) `_experimental/checksums/crc32.nv` — RFC-1952-совместим (`0xCBF43926` PASS) → промоут-план (§6); (b) Adler-32 — **НЕ существует** → NEW Ф.1; (c) bit-ops/`[]u8`-slice/`Vec[u8].push` достаточны для LSB-first bit-reader + canonical-Huffman (verify против `vec_seq.nv`); (d) `consume value` для `BrotliReader` (verify против `Body` 182 / `File` 180); (e) **vendor-link-инфра brotli** — подтвердить, что проект собирает vendored static C (как `libz3.lib`/libuv) ДО коммита Ф.2 (simplification-audit: механизм link не верифицирован — Ф.0 проверяет реальный путь). (5) **D333–D337 spec-first** (§5). (6) **Координация:** подтвердить с owner Plan 182, что Ф.1 закрывает Q12-gate для gzip/deflate сейчас; brotli ждёт Ф.2. **GATE.** DEP: Plan 181 (D325), 180 (must-consume/byte-first), 182-schedule.

- **Ф.1 — inflate: raw-DEFLATE + zlib + gzip decode (streaming + bomb-cap). «сейчас». 🔴 UNBLOCKS Plan 182 Q12 (gzip/deflate).** **Чистая Nova (.nv), БЕЗ C** (precedent Zig/Go; §3.0 Q1). Контент:
  ...
  - **🔴 BOMB-CAP (§8.0, D334):** every decode-path принимает `max_output`; превышение output **ИЛИ прогресс-вход** → `Bomb(limit)` ДО аллокации сверх лимита (инкрементально, НЕ post-factum). Plan 182 → `max_decompressed`.
  - **🔴 STREAMING (§8.0, D335, Plan 182 BodyReader):** `Inflater`/`GzipReader`/`ZlibReader` — `feed`/`read(max_emit)`/`finish`; **bounded-per-call** (`read` ≤ max_emit — критик streaming-bound); 32 KiB window + bit-leftover между вызовами; `finish` валидирует checksum/ISIZE + clean-EOF; **EOF-семантика** (Q14: no-BFINAL=need-more; finish-before-BFINAL→`UnexpectedEof`).
...
**D-номера:** D325=Result-everywhere (181), D326=ref-param-mode (172.5) → **D327–D332 заняты Plan 182** → compress старт **D333**. ⚠ D316–D324 (179/180) и D327–D332 (182) **НЕ в** `spec/decisions/` (committed до D326) → **reservation-нота** в индексе (как Plan 181, Ф.0).
...
- **NEW D334** — **bomb-cap (DoS-инвариант)**: каждый decode-путь (one-shot + streaming + brotli-FFI) несёт `max_output`; превышение output **ИЛИ прогресс-вход** (anti-flood: member-flood/giant-header) → `Bomb(limit)`, проверка **инкрементальная** (ДО over-аллокации); Plan 182 пробрасывает `max_decompressed` (100 MiB). **§8.0-critical.**
- **NEW D335** — **streaming incremental coder**: `feed(chunk)` + `read(max_emit) -> Result[Option[[]u8],CompressError]` (`None`=end, **bounded-per-call** — anti-single-huge-alloc) + `finish`; window/bit-leftover/checksum-state между вызовами; **EOF-семантика** (no-BFINAL=need-more; finish-before-BFINAL→`UnexpectedEof`); **must-consume только `BrotliReader`** (C-instance, D133); pure-Nova-кодеры = plain value (Q6). **Контракт-мост с Plan 182 `BodyReader`** (`@next_chunk`→`feed`→`read`) фиксируется здесь.
...
**🔴 Plan 182-reconcile (Ф.0, критик-gap высокой severity):** Plan 182 в [182:488]/[182:865 Q12]/§3.5 формулирует кодеки как «Nova-logic над **C-zlib FFI**» — **противоречит** Plan 183 Q1 (pure-Nova). **Amend 182** (отдельный коммит, sub-task Ф.0): (1) Q12/§3.5/§9 — «inflate/gzip/zlib = pure-Nova (183 Q1); C-FFI только brotli (183 Q2)»; (2) указатель «NEW под-план std/encoding/compress» → **Plan 183 Ф.1 (gzip/deflate gate-open) + Ф.2 (br)**; (3) §11-followup `zstd` → ссылка на Plan 183 §11. Без этого синтезатор обоих планов получает live-противоречие по pure-vs-C.

**Зависимость-направление (§3.4):** **compress НЕ импортирует `std.http`** (избежать цикла); Plan 182 импортирует compress. Glue (`BodyReader`+decoder) живёт в `real_http` Plan 182.
...
6. **spec:** D333–D337 в `spec/decisions/` (+ reservation-нота); docs `encoding-compress.md`; §1a; **Plan 182 reconciled** (Q12/§3.5/§9/§11 → pure-Nova + Plan 183, «C-zlib FFI» amended).
...
**🔴 GATE-релевантность (Plan 182):**
- **Plan 182 Q12 (auto-decompress) — 🔴 HARD-GATE на Plan 183 Ф.1.** Ф.1 (gzip/deflate/zlib decode + bomb-cap + streaming-`BodyReader`) **открывает gate для gzip/deflate сейчас**; `br` gated на Ф.2. Plan 182 → `max_decompressed`→`max_output` (D334). **Streaming-контракт-мост** (`BodyReader.@next_chunk`[182:224] ↔ `feed`/`read`) — D335; verify против Plan 182 §3.5.
- **🔴 Reconcile (критик-gap высокой severity):** Plan 182 «Nova-logic над **C-zlib FFI**» ([182:488]/[182:865]/§3.5) ↔ Plan 183 Q1 (pure-Nova). **Ф.0 amend'ит 182** (§6) — wording-override фиксируется ЗДЕСЬ как координационный deliverable, НЕ просто указатель-апдейт.

**Координировать:**
- **Plan 182 (std/http)** — потребитель Ф.1 (gzip/deflate) + Ф.2 (br); reconcile Q12/§3.5/§9/§11 → Plan 183 + pure-Nova-amend.
- **Plan 181 (D325)** — Result-everywhere (conformant by-construction).
- **Plan 180 (io/fs)** — `copy_to(w mut impl io.Write)` (followup §11, decode-to-file/encode-from-file) гейтит на `io.Write`; large-file-slow-тесты читают fixture-файлы (fs).
- **`_experimental/checksums/crc32.nv`** — промоут (D336/Q15); owner-sign-off при stable-промоуте.
- **`std/encoding/json`** — Plan 182 `json()` отдельный serde-gate (НЕ этот план); layout-сосед.
...
`[M-183-std-compress]`. **Под-планы при росте:** **183.1** (encode-suite + optimal-parse), **183.2** (pure-Nova-brotli). Deferred (с rationale, НЕ на критпути inflate+gzip+zlib-decode = 182-gate):
...
- **`copy_to`/streaming-to-file** (decode-to-`io.Write` / encode-from-`io.Read` — fs-gate Plan 180; download-decompress без буферизации-всего).
