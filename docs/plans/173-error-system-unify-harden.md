...
> **Объединяет** бывш. Plan 172.8 (error-system) + Plan 173 (panics-clause = Ф.6) под номером 173 (2026-06-20).
...
> **Включает** panics-clause (Ф.6, бывш. отдельный Plan 173). **Завершает** [172.7](172.7-question-mark-return-only.md)
...
| 3 | `?` Fail-context throw-mode (D165) ещё живёт в codegen (Plan 172.7 не завершён) | `emit_c.rs:21264-21327` | **P1** |
...
- типы — **Plan 179** (`Monotonic` точка / `Duration` интервал); параметр рядом с `cancel:`/`strategy:`/`max_restarts:`/`period:`;
...
2. **#3 fix:** удалить `?` Fail-context throw-mode (`emit_c.rs:21264-21327`) → `[E_TRY_IN_FAIL_FN]`; `?` строго
   return-only. Завершает codegen-часть [Plan 172.7](172.7-question-mark-return-only.md).
...
**ФУНДАМЕНТ typed-errors — type_id-инфра Plan 61 (переиспользуем; исследование 2026-06-20). `any`/`is` → [Plan 172.8](172.8-any-type-and-is-downcast.md) (зависимость, реализуй первым):**
...
**`any`-представление + `is`/`try_as` — вынесено в [Plan 172.8](172.8-any-type-and-is-downcast.md)** (fat-pointer `{data, vt=NovaTypeInfo[type_id+Display]}`, downcast через `type_id`; сосуществует с мономорфизацией). **Ф.4 зависит от 175.**
...
на ГОТОВОМ type_id-фундаменте, а не с нуля.** Главный риск Ф.4 — materialization MultiError (`any`-boxing+vtable закрывает [Plan 172.8](172.8-any-type-and-is-downcast.md)); сам матчинг готов.
...
| `?` + auto-`From` | **отклонён** (см. [172.7](172.7-question-mark-return-only.md)); explicit `.map_err`. Cleanup-движок рассуждает только о throw+cancel+panic, не о value-`?`. |
...
`[M-172-errdefer-okdefer-dead-surface]`), [172.7](172.7-question-mark-return-only.md).
