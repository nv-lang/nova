...
> **Объединяет** бывш. Plan 174.3 (error-system) + Plan 173 (panics-clause = Ф.6) под номером 173 (2026-06-20).
...
> **Включает** panics-clause (Ф.6, бывш. отдельный Plan 173). **Завершает** [174.2](174.2-question-mark-return-only.md)
...
| 3 | `?` Fail-context throw-mode (D165) ещё живёт в codegen (Plan 174.2 не завершён) | `emit_c.rs:21264-21327` | **P1** |
...
| `parallel for` | десугар в supervised+spawn (`emit_c.rs:8040`) | при ошибке ребёнка `[]T`-результат отбрасывается (**all-or-throw, нет per-element `Result`**); **checker↔codegen рассинхрон** (`[M-parfor-record-result-miscompile]`): результат-запись/non-primitive тайпчекается как `[]T`, но array-mode codegen (`emit_c.rs:8154`) умеет лишь `T ∈ {int,bool,f64,str}` → молча degrade в `unit` → **утечка сырого C-error** (README `check_all → []HealthStatus` не собирается) |
...
## 3a. Пересмотр cleanup/timeout (owner, 2026-06-26)

> **Перекрывает §3.1/§3.2** («bare `defer` — unshielded») и **ретрактирует D192** (force-таймаут на cleanup).
> Итог обсуждения с владельцем 2026-06-26 (кросс-язык Go/Swift/Rust/Kotlin/Java + права программиста).
> Принцип владельца: **«defer всегда должен добегать»**.

**(1) `defer` ВСЕГДА добегает (completes-by-default); щит ЭЛИДИТСЯ для синхронных тел.**
Отмена в Nova **кооперативна** (доставляется только на suspend-точках: I/O / `channel` / `sleep` / `lock`). Поэтому:
- **синхронное** тело defer добегает и так (нет suspend-точки → cancel негде вклиниться) → щит = **no-op** → компилятор его **элидит** (тот же приём D194-элизии) → **ноль стоимости** на горячем пути;
- **suspend'ящееся** тело — cancel замаскирован на время cleanup → не рвётся посередине.
Это **заменяет §3.1/§3.2** «bare defer — unshielded»: bare `defer`/`defer(o)` теперь тоже **completes-by-default**. Разница с `consume` остаётся в **must-consume-контракте + exit-policy**, а НЕ в «добежит/не добежит». Модель **Go/Swift/Rust** (cleanup всегда завершается), не Kotlin (opt-in `NonCancellable` — footgun, на который жалуются).

**(2) НЕТ force-таймаута, прерывающего cleanup — D192 ретракт.**
Никто из 5 языков не force-прерывает cleanup по таймауту: это вернуло бы **partial-cleanup-порчу** (ровно то, что щит предотвращает). D192 в форме «инжект `CleanupTimeoutError` в suspend cleanup'а» (D188 R3) **ретрактируется**. Зависший cleanup = баг программиста; защищаемся **снаружи** (п.3), не разрывом изнутри.

**(3) Bounded-shutdown — дедлайн на SCOPE, не на cleanup.** Лимит времени — параметр `supervised`/`parallel`:
- **`supervised(deadline: Monotonic)`** — **абсолютная точка** (канон): корректно **пропагируется** во вложенные scope (общий конец, а НЕ свежий лимит на каждом уровне — модель Go `context` deadline);
- **`supervised(timeout: Duration)`** — относительный **сахар** = `deadline: Monotonic.now() + d`;
- типы — **Plan 175** (`Monotonic` точка / `Duration` интервал); параметр рядом с `cancel:`/`strategy:`/`max_restarts:`/`period:`;
- **`parallel for` зеркалит** параметры `supervised` (включая `deadline:`/`timeout:`) **и/или** наследует дедлайн enclosing-scope через пропагацию.
По дедлайну → **кооперативная отмена** детей (на suspend-точках) → cleanup'ы после этого **добегают** (под щитом). Превышение **самого** дедлайна при зависшем cleanup → **watchdog-варн** (диагностика «fiber X застрял в cleanup»), **НЕ** force-kill.

**(4) `with_timeout` / `race` (stdlib) — убрать.**
- **`with_timeout[T](timeout_ms int, body fn()->T)`** (`std/concurrency/cancellation.nv:156`) — **субсумирован** `supervised(timeout:)`/`parallel(timeout:)`. Текущая форма не годится: thunk-функция (не блок) + **сырой `int`-ms** (до-179) + **относительный без пропагации** + **flat-string** throw. **Удалить** (fn + ~7 тест-ссылок).
- **`race`/`race2`** (там же:113) — first-wins-паттерн, **stdlib-выражаемый** (`race2` = `supervised(cancel:)` + spawn); текущий `race2` — **заглушка-на-2** (общий `race[T]([]fn)` ждёт array-of-closures, Plan 48 Ф.4). **Убрать сейчас**, вернуть нормальный N-арный `race` позже по спросу.
- Спека **stale**: 06-concurrency.md «race/with_timeout — stdlib поверх примитивов» + `race { … }`-блок (04-effects.md:94, НЕ реализован) — поправить.

**(5) Per-op timeout — fallback** (`ch.recv(timeout:)` и т.п.) для тонкого контроля / не-кооперативных операций. Не замена scope-дедлайна.

**Затрагивает (амендить при реализации):** **D188 R3** (shield → `defer` тоже completes-by-default + elision для sync), **D192 РЕТРАКТ** (нет force-cleanup-timeout), **D71/`parallel for` + `supervised`** (новые `deadline:`/`timeout:`), **06-concurrency.md / 04-effects.md** (stale `race`/`with_timeout`/`race{}`). Зависит от **Plan 175** (`Monotonic`/`Duration`). Координация: **173.0** (рантайм cancel/shield-mask), **173.1** (`parallel`/`supervised` как value + параметры).

---

## 3b. Пересмотр supervisor-модели: restart для файберов под вопросом (owner, 2026-06-26)

> **Перекрывает Ф.0-решение** «supervisor — все 3 стратегии `OneForOne`/`OneForAll`/`RestForOne` + `max_restarts`/`period`».
> Вскрыто вопросом владельца: **Erlang-restart завязан на process-ИЗОЛЯЦИЮ, которой у файберов нет.**

**Проблема.** В Erlang дети — **процессы** (share-nothing, своя память): restart = **чистый сброс с нуля** (свежий процесс из init-функции). У Nova дети — **файберы** на **ОБЩЕЙ памяти**: «restart файбера» = заново запустить его замыкание, но **общее состояние, которое он успел испортить** (полу-мутация, недозакрытый ресурс), **не сбрасывается** → гарантия Erlang «вернулись в известно-хорошее» **НЕ держится**. Показательно: **ни один файбер/корутин-фреймворк** (Kotlin `coroutineScope`/`supervisorScope`, Java `StructuredTaskScope`, Swift `TaskGroup`, Go `errgroup`) **НЕ берёт** Erlang-restart — только cancel/aggregate/shutdown (restart требует изоляции).

**Что переносится на файберы, а что нет:**
- ✅ **переносится** (про жизненный цикл, не изоляцию): отмена siblings, **Escalate** (гасим scope + ошибка наверх), **Stop** (снять упавшего, остальные живут), wait-all + **MultiError**.
- ⚠️ **сомнительно** (Erlang-изоляция): **Restart** (`OneForOne`/`OneForAll`/`RestForOne` + `max_restarts`/`period`) — на shared-memory нет «чистого сброса»; restart переиспользовал бы испорченное общее состояние.

**Решение (направление, owner 2026-06-26):**
- **(a) default — БЕЗ per-fiber restart:** `cancel`-siblings / **Escalate** / **Stop** / aggregate (модель Kotlin/Java/Swift/Go). Звучно для shared-memory.
- **(b) restart — ТОЛЬКО для ИЗОЛИРОВАННЫХ файберов, opt-in:** которые не делят mut-состояние (рычаг изоляции — **[Plan 173.3](173.3-data-race-freedom-share.md)** `#share` / consume-в-spawn). Там restart безопасен — нечего портить/переиспользовать.
- **(c) опционально — restart на уровне ВСЕГО scope** (retry операции целиком из чистой точки), а не отдельного файбера.

**Затрагивает:** Ф.0-решение про стратегии (врезка `supervised(strategy:…)`) — **пересмотреть**: набор по умолчанию = `cancel`/`Escalate`/`Stop` (restart-стратегии — гейт **173.3**, не в базовом); **08-runtime.md:168** (`OneForOne`/`max_restarts` заявлены); координация **173.0/173.2** (supervisor-движок). **Erlang-restart «в лоб» на shared-memory файберах — анти-паттерн.**
...
2. **#3 fix:** удалить `?` Fail-context throw-mode (`emit_c.rs:21264-21327`) → `[E_TRY_IN_FAIL_FN]`; `?` строго
   return-only. Завершает codegen-часть [Plan 174.2](174.2-question-mark-return-only.md).
...
> **⚠️ ВЫНЕСЕНО В СЕМЕЙСТВО 173.0–173.3 (sign-off 2026-06-21).** Эта фаза разрослась в подсистему и вынесена в
> отдельные планы — **они авторитетны**, нумерованные пункты ниже = раннее мышление (частично пересмотрено диалогом):
> - **[173.0](173.0-concurrency-runtime-substrate.md)** — рантайм-субстрат (**ГЕЙТ**): per-slot ошибки + serialized
>   decision-loop + ctx-pinning + закрытие multi-worker drain-гонки. Без него 173.1/173.2 неверифицируемы.
> - **[173.1](173.1-parallel-collect-and-supervised-value.md)** — `parallel for → []T` через канал+`consume`,
>   `supervised`-возвращает-значение, completion-order, плотный (закрывает `[M-parfor-record-result-miscompile]`).
> - **[173.2](173.2-supervision-as-effect.md)** — supervision **как эффект** (`Supervisor`/`on_child_fail(idx,err,attempt)→Decision`),
>   MVP `Restart`/`Escalate`/`Stop`; стратегии = хендлеры.
> - **[173.3](173.3-data-race-freedom-share.md)** — data-race-freedom: атрибут **`#share`** (одна ось; move=`consume`),
>   capture-check, consume-в-spawn, поглощение `#fiber_send`.
>
> **Главные пересмотры vs пунктов ниже:** scope-result — **не** `[]Result`/`all_or_throw`-индекс, а **канал +
> completion-order dense** (п.1); supervisor-стратегии — **не** sum-type-аргумент, а **эффект-хендлер** (п.2).
> Проверено adversarial-стресс-тестом + research-свипом 2026-06-21. Детали и открытые вопросы — в под-планах.
...
7. **🐞 checker↔codegen рассинхрон `parallel for → []T` (P1, `[M-parfor-record-result-miscompile]`):** чекер
   типизирует результат `parallel for` как `[]T` для ЛЮБОГО `T` (D71), но array-mode codegen (`emit_c.rs:8154`)
   умеет лишь `T ∈ {nova_int, nova_bool, nova_f64, nova_str}` (+ iter ∈ Range / array-lit / array-ident) — иначе
   **молча degrade в statement-mode (`unit`)** → emit `return NOVA_UNIT` из `[]T`-функции → **сырой C-compile-error**.
   Эмпирически (2026-06-21): `nova check` PASS, `nova build` FAIL — `returning 'nova_unit' from a function with
   incompatible result type 'Nova_Vec____Nova_HS_p *'`. Это ровно README-пример `check_all → []HealthStatus`, который
   сейчас не собирается. **Фикс (без упрощений):** array-mode codegen на произвольный `T` (record/sum/любой) — это и
   реализует scope-result `[]T` из п.1; **до того** — чекер обязан отвергать неподдержанный результат `parallel for`
   чистым `[E_PARFOR_RESULT_UNSUPPORTED]` (no-silent-degrade), а не утекать C-error. (Отдельно: README использует
   удалённый `let` — D184; пример нерабочий по двум причинам.)
8. **🔲 Открытый surface-дизайн (резолв в ЭТОЙ фазе, не v0.1-блокер):** точный синтаксис scope-result —
...
- **Тесты pos+neg:** `nova_tests/err173_f3/`: child throw → siblings cancelled + MultiError; per-element Result;
  race any_ok; supervisor restart (если оставлен); detach policy; channel closed vs value;
  **`parallel for` → `[]Record`/`[]SumType` собирается и возвращает корректный массив** (закрывает
  `[M-parfor-record-result-miscompile]`). **neg:** до фикса codegen — непримитивный результат `parallel for` →
  чистый `[E_PARFOR_RESULT_UNSUPPORTED]`, НЕ сырой C-error; …
- **Acceptance:** child-fail отменяет siblings + агрегирует by default (≡ Kotlin/Java); detach-эффект enforced;
  ни одна ошибка fiber'а не теряется молча; **`parallel for → []T` работает для любого `T`** (record/sum/примитив),
  чекер и codegen согласованы (no-silent-degrade); **без упрощений**.
...
6. **Инвариант catchability (две ошибки → MultiError НЕ подменяет эффект):** при композиции primary
   остаётся носителем — `nova_rethrow_with_suppressed` (`effects.h:210-218`) **копирует `error_user_type_id` +
   `payload` примери** в проброс, cleanup-ошибки уезжают в suppressed-цепочку. Следствие: `with Fail[Primary]`
   СРАБАТЫВАЕТ (match по `type_id == NOVA_TID_Primary`), **эффект остаётся `Fail[Primary]`, НЕ становится
   `Fail[MultiError]`**; `e.suppressed()` = [cleanup]. Этот инвариант ДОЛЖЕН сохраниться после унификации
   re-dispatch (Ф.2 `nova_scope_exit`) — добавить regression-guard. (Граница: если упала ТОЛЬКО cleanup —
   primary = cleanup-ошибка, пробрасывается ЕЁ тип; это вопрос полноты effect-row, а не подмены примери.)

**ФУНДАМЕНТ typed-errors — type_id-инфра Plan 61 (переиспользуем; исследование 2026-06-20). `any`/`is` → [Plan 174.3](174.3-any-type-and-is-downcast.md) (зависимость, реализуй первым):**
...
**`any`-представление + `is`/`try_as` — вынесено в [Plan 174.3](174.3-any-type-and-is-downcast.md)** (fat-pointer `{data, vt=NovaTypeInfo[type_id+Display]}`, downcast через `type_id`; сосуществует с мономорфизацией). **Ф.4 зависит от 175.**
...
на ГОТОВОМ type_id-фундаменте, а не с нуля.** Главный риск Ф.4 — materialization MultiError (`any`-boxing+vtable закрывает [Plan 174.3](174.3-any-type-and-is-downcast.md)); сам матчинг готов.
...
- **Тесты pos+neg:** `nova_tests/err173_f4/`: primary+suppressed видны в handler; typed Failure dispatch; cleanup-fail во время body-fail → MultiError (не overwrite); chain переживает голый throw в unwind.
  **Catchability-тест (инвариант п.6):** тело бросает `Primary`, `@cleanup` тоже бросает `Cleanup` (две ошибки) →
  внешний `with Fail[Primary]` СРАБАТЫВАЕТ (`e` — `Primary`, type_id сохранён), `e.suppressed()` == `[Cleanup]`;
  эффект объявлен `Fail[Primary]`, НЕ `Fail[MultiError]`. **neg:** упала ТОЛЬКО cleanup (`Cleanup` ≠ `Primary`) →
  `with Fail[Primary]` НЕ ловит (пробрасывается `Fail[Cleanup]`) — подтверждает «primary остаётся носителем, не оборачивается».
- **Acceptance:** D158/D193 обещание выполнено end-to-end; cleanup-ошибка НИКОГДА не перезаписывает body-ошибку;
  **композиция сохраняет type_id примери — `Fail[Primary]` ловится, эффект НЕ становится `Fail[MultiError]`** (catchability-инвариант п.6, под regression-guard); **без упрощений**.
...
| `?` + auto-`From` | **отклонён** (см. [174.2](174.2-question-mark-return-only.md)); explicit `.map_err`. Cleanup-движок рассуждает только о throw+cancel+panic, не о value-`?`. |
| `ScopeOutcome.Failure` тип | **`Failure(any)`** (type-erased payload, D188); `ScopeOutcome[E]` остаётся rejected (D190). |
| MultiError | **материализовать end-to-end** (Ф.4), НЕ ретрактить (production-grade). **Catchability-инвариант:** две ошибки (body+cleanup) → primary остаётся носителем (type_id примери сохраняется через `nova_rethrow_with_suppressed`); ловишь `Fail[Primary]` (НЕ `Fail[MultiError]`), cleanup-ошибки — в `e.suppressed()`. Эффект НЕ подменяется. |
...
`[M-172-errdefer-okdefer-dead-surface]`), [174.2](174.2-question-mark-return-only.md).
...
`[M-173-error-system]` (umbrella) + `[M-173-panics-clause]` (Ф.6) +
`[M-parfor-record-result-miscompile]` (Ф.3 п.7 — checker↔codegen рассинхрон `parallel for → []T`, утечка C-error; P1).
Возможная декомпозиция в суб-планы при росте: Ф.3 → 173.1 (structured-concurrency), Ф.4 → 173.2 (MultiError/typed-outcome).
