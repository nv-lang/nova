...
**ФУНДАМЕНТ typed-errors — type_id-инфра Plan 61 (переиспользуем; исследование 2026-06-20). `any`/`is` → [Plan 177](177-any-type-and-is-downcast.md) (зависимость, реализуй первым):**
...
**`any`-представление + `is`/`try_as` — вынесено в [Plan 177](177-any-type-and-is-downcast.md)** (fat-pointer `{data, vt=NovaTypeInfo[type_id+Display]}`, downcast через `type_id`; сосуществует с мономорфизацией). **Ф.4 зависит от 177.**
...
на ГОТОВОМ type_id-фундаменте, а не с нуля.** Главный риск Ф.4 — materialization MultiError (`any`-boxing+vtable закрывает [Plan 177](177-any-type-and-is-downcast.md)); сам матчинг готов.
