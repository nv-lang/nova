...
| `std/_experimental/math/complex.nv` | `Complex.from(s str)`(throws, :312) → `Complex.try_from(s) -> Result[Complex, ParseComplexError]`; `parse_f64_or_throw`(:368)→`_or_err`(Result); тесты :552-591. Инфаллибл-конструкторы (`from(f64)`/`from_imag`/`from_polar`) — **без изменений**. | малый |
