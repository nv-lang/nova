...
| `std/encoding/json.nv` | `Json.parse`(:630) → `Result[JsonValue, ParseJsonError]`; `throw`→`Err`/`?` по всему лексеру+recursive-descent (~20 сайтов + сигнатуры хелперов с `?`-threading); тесты. | **большой** |
...
>
> **Урок для плана (важно):** «`.nv`-only» (не трогает compiler-source) ≠ «codegen-clean». Из 2 проверенных Ф.2a-файлов **оба** упёрлись в codegen-баги (base64 — пре-существующие латентные; complex — НОВАЯ регрессия от Result-обёртки над named-tuple). Гонять `nova test`, не только `check`. **Вывод: разблокировка Ф.2a де-факто требует сперва починить 2 codegen-бага** (`[M-181-ifexpr-value-materialize-codegen]` + `[M-181-result-over-named-tuple-codegen]`). json.nv (Result над sum-type `JsonValue`) — риск аналогичных сюрпризов; `nova test` до коммита обязателен.
