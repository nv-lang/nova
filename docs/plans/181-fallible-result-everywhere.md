...
| `std/_experimental/math/complex.nv` ⚠ **BLOCKED → Ф.2b (codegen)** | Миграция написана и корректна (`Complex.from(s str)`→`Complex.try_from(s) -> Result[Complex, ParseComplexError]`, `parse_f64_or_throw`→`parse_f64_or_err`(Result), neg-тест :593 расконсервирован, `nova check` ✅), НО `Result[Complex,…]` (Result над **named-tuple** `Complex`) триггерит codegen-ordering баг → **РЕГРЕССИЯ** ранее зелёного `nova test`. Откатил; ждёт codegen-фикс (баг 3 ниже). Инфаллибл `from(f64)`/`from_imag`/`from_polar` — без изменений. | малый, codegen-gated |
| `std/encoding/json.nv` ✅ **DONE (исходник, 2026-06-25)** | `Json.parse`/`Parser.*`/`Lexer.@read_*` → `Result[…, ParseJsonError]` (~15 fn; throw→Err/return Err; `?`-threading; `Lexer.@advance`/`@peek` = Option, без `?`); `JsonValue.from(str)`→`try_from`; ~35 `Json.parse` в тестах → `!!`, 5 neg `with Fail`-блоков → `Err(..)`-match. `nova check` ✅. **⚠ `nova test` блокирован codegen-багами (баг 4 ниже + пре-существующий erasure CC-FAIL `NovaOpt_Nova_Vec`, json.c:7081, [M-91.13]) — НЕ регрессия** (оригинал тоже падал `nova test`). | **большой** |
...
> 3. **Result над named-tuple — codegen type-ordering** (complex.nv, **РЕГРЕССИЯ**): `Result[Complex, ParseComplexError]` (Complex = named-tuple `type Complex(re, im)`) → структура `NovaRes_NovaTuple_Complex_…` использует `NovaTuple_Complex` ДО его typedef (forward-reference) → `unknown type name 'NovaTuple_Complex'`. Оригинал complex `nova test` **проходил** — обёртка `Result` новая, потому регрессия. Маркер **`[M-181-result-over-named-tuple-codegen]`** (codegen: forward-декларация named-tuple типов, embedded в generic-wrapper Result/Vec — раньше, чем wrapper-struct; родственно D123/D216 tuple-mono). Компилятор → **ОТЛОЖЕН**; complex реклассифицирован в Ф.2b, миграция **откачена** до фикса (чтобы не ронять зелёный тест).
> 4. **Анонимный record-литерал как аргумент `Ok(...)`** (json.nv): `Ok({ tok: …, line, col })` / `Ok({ lex, cur })` → `codegen error: anonymous record literal without spread not supported`. В оригинале `{ … }` возвращался напрямую (codegen знал target-тип `TokenWithPos`/`Parser` из return-типа и коэрсил, D55); обёрнутый в `Ok(...)` анон-литерал теряет target-контекст. Маркер **`[M-181-anon-record-in-ctor-arg-codegen]`** (codegen: пробрасывать ожидаемый тип в анон-record-литерал в позиции аргумента конструктора/обёртки). **НЕ регрессия** (оригинал json уже падал `nova test` на пре-существующем erasure-баге `as_array() -> Option[[]JsonValue]`, [M-91.13]). Source-workaround есть (type-annotated binding до `Ok`), но громоздкий, и json упрётся в следующий codegen-баг → не делал; ждёт codegen-фикс.
>
> **Урок для плана (важно):** «`.nv`-only» (не трогает compiler-source) ≠ «codegen-clean». Все **3** проверенных Ф.2a-файла упёрлись в codegen-баги. **Регрессия только у complex** (зелёный→красный, откачен); **base64 и json — пре-существующе-красные** (закоммичены: D325-корректны + `nova check`-чисты, `nova test`-статус не ухудшен). **Разблокировка Ф.2a требует codegen-фиксов** (маркеры `[M-181-ifexpr-value-materialize-codegen]`, `[M-181-result-over-named-tuple-codegen]`, `[M-181-anon-record-in-ctor-arg-codegen]`) — их чинит отдельный агент (compiler). После фикса: re-test base64/json, re-apply complex.
