...
> Practical guide для [D148](../../spec/decisions/03-syntax.md#d148)
...
fn process() Fs Time -> () {
    ...
    defer { conn.graceful_drain() }             // suspend-able (D148)
    do_io(view conn)?
}
```

D148 снимает D90 §5 «no-suspend» — `Time.sleep`, `Channel.recv`,
...
❌ **`spawn`** — error D148-spawn-in-defer. Новый fiber в cleanup утечёт
supervised scope.
...
## Async cleanup + failable composition (D147 + D148 combined)
...
Combines D147 (failable body) + D148 (async/suspend) — production-grade
graceful shutdown.

## Сравнение

| Capability | Rust | TS (ES2024) | Kotlin | Nova D148 |
|---|---|---|---|---|
