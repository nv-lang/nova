...
| [docs/stdlib-module-conventions.md](docs/stdlib-module-conventions.md) | **Designing stdlib modules + C integration** — effect-family architecture (mockable plumbing + type-method facade), value/must-consume types, structured `Result` errors, byte-first, the `extern "C"` `ffi.nv` layer (CStr vs `(*u8,len)`, errno, value-records), `#cfg` platform-split, async park/wake. Complements [ffi-cookbook.md](docs/ffi-cookbook.md) (FFI mechanics) and [nv-coding-style.md](docs/nv-coding-style.md) (`.nv` style). |
