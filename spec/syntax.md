...
- `_prefix` — конвенция «приватное-по-договору» только для полей
  (подробно — [«Видимость: export»](#видимость-export-для-публичных-деклараций)
  ниже, включая настоящую field-приватность через `priv`). Для
  функций/методов **не используется**.
...
export type Account {                    // публичный тип
    ...
    _internal_id u64                     // convention: `_` = приватное-по-договору
                                           // (поле остаётся ФАКТИЧЕСКИ публичным)
}
...
**Поля record:** без `priv` на типе поля `export`-типа публичны по
умолчанию (D47); `_prefix` — конвенция «приватное-по-договору», не
enforced компилятором. Настоящая приватность — модификатор `priv`
на типе (`priv`/`priv(type)`/`priv(file)`, D220/D281): поле физически
