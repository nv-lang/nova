...
центральной идеи (см. [decisions.md D10](../decisions.md)):
...
Любое нечистое вычисление — эффект. Эффект объявляется через `effect`,
имеет операции, и **handler** перехватывает операции и решает,
что с ними делать. Это обобщение `try/catch`, `async/await`,
dependency injection и моков в одну штуку.
...
`resume(value)` — продолжение вычисления с возвращённым значением.
В обычном случае `resume` вызывается один раз. Если **не вызвать** —
эффект «прерывает» вычисление (так работает `Throws`). Если вызвать
...
|---|---|---|
| `Throws[E]` | Может выбросить ошибку типа E | catch, retry, log-and-continue |
| `Io` | Чтение/запись файлов, stdout | mock-fs, capture-stdout |
| `Net` | Сетевые запросы | record/replay, fault injection |
| `Db` | База данных | транзакция, in-memory storage |
| `Fs` | Чтение/запись файлов | virtual filesystem |
| `Time` | Часы, таймеры, задержки | virtual clock, fast-forward |
| `Random` | RNG | seeded RNG для тестов |
| `Mut` | Изменяемое состояние | undo/redo, snapshot |
| `Alloc[R]` | Аллокация в регионе R | арена, GC, pool |
| `Async` | Точки приостановки (fiber yield) | scheduler, single-thread runner |
| `Par` | Параллельный запуск | thread-pool, sequential (для тестов) |
...
Вместо `async/await` — эффект `Async` (см. R7). **Цвет функции
исчезает** — нет деления на «sync» и «async», есть «какие у функции
эффекты».
...
- **Нет глобального изменяемого состояния** — `Mut` это эффект, виден
  в сигнатуре
...
fn transfer(from AccountId, to AccountId, amount money)
    Throws[InsufficientFunds, InvalidAccount]
    Db Time Log
    requires amount > 0
    ensures from != to
    -> TransferReceipt
...
Стандартная библиотека пишется так, чтобы каждая функция полностью
описывала себя:
...
/// effect.Throws[NetError]: on connection failure, timeout, non-2xx
///
/// example:
///     let body = http.get("https://api.example.com/users/1")?
///
/// see also: http.post, http.client
fn http.get(url str, timeout ms = 30000) Net Time Throws[NetError] -> Response
```
...
```nova
// программист написал:
fn withdraw(mut acc Account, amount money) Throws[Overdraft] -> () {
...
- Функции, для которых SMT-проверка контракта не сходится за разумное
  время (см. ограничения SMT в [decisions.md D24](../decisions.md))
...
3. **Семантика контрактов через handler-state** — открытый вопрос.
   ...
   См. [decisions.md D24](../decisions.md).
...
- **Опирается на [decisions.md D24](../decisions.md)** — стратегия
...
Это превращает «sandboxing» из runtime-задачи в compile-time гарантию.
Полезно для:
- Плагинов
- Пользовательских скриптов
- LLM-сгенерированного кода (запретить сеть для сгенерированной функции)
...
`Async` — **обычный эффект**, не специальная конструкция. Цвет функции
исчезает, потому что переезжает в эффекты:
...
Тип возврата `Response`, **не `Future<Response>`**. Код пишется
как синхронный.

Под капотом — **fiber-based scheduler** (как Go/OCaml 5). Когда
операция эффекта `Net` приостанавливается, fiber кладётся в
очередь ожидания, scheduler берёт другой fiber. Память — сегментированный
стек или cactus stack.
...
Подробно — [decisions.md D12](../decisions.md).
...
panic-источники). Подробно — [decisions.md D13](../decisions.md).
...
[D10](../decisions.md) («всё handler») масштабируется до распределённых
...
**стек handler'ов** над эффектами `Db`, `Net`, `Throws`.
...
```nova
fn transfer(from AccountId, to AccountId, amount money) Db Throws -> Receipt {
...
В сигнатуре — `Db` и `Throws`. **Никаких** `@Idempotent`, `@Replicated`,
...
**3. Retry с backoff.** Handler `Net`, перехватывающий `Throws[NetError]`
и повторяющий вызов:
...
fn traced(real Handler[Db]) -> Handler[Db] => Db {
    query(sql, args) => Trace.span("db.query", { "sql": sql }) {
        resume(real.query(sql, args))
    }
    exec(sql, args)  => Trace.span("db.exec", { "sql": sql }) {
        resume(real.exec(sql, args))
    }
}
```
...
| Circuit breaker | Hystrix-библиотека + конфиг | handler с `Throws[Tripped]` |
...
сигнатуру `transfer`, видит `Db Throws` — обычные эффекты. Читая
...
- **Транзакции через границы handler'ов.** Если `Db` обернут в
  репликацию, а вокруг ещё `with Throws[NetError] = retry`, корректное
...
- **Развивает [D10](../decisions.md)** — это иллюстрация центрального
...
## Главные риски (повторено из decisions.md D10)
