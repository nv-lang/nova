...
## Q-buffer. `Buffer` — mutable byte accumulator ⚠️ REPLACED (2026-05-08)

> **Replaced by Q-string-builder + Q-write-buffer + Q-read-buffer**
> в Plan 04 (split на три типа). Унифицированный `Buffer` смешивал
> text+binary domain в одном типе — выявилось при добавлении
> endianness-методов: `write_u32_le` рядом с `add_str` несогласовано;
> `@into() -> str` всегда fallible даже если писали только text. Split
> делает `StringBuilder.@into() -> str` infallible (UTF-8 invariant
> поддерживается типом).
>
> **Status (2026-05-08):**
> - Runtime: StringBuilder/WriteBuffer/ReadBuffer работают (Plan 04
>   Этапы 1-3 закрыты).
> - std/ partial sweep: 14 файлов мигрированы (text-only + Url @into);
>   url.nv decode_query — единственный остаток на Buffer (mixed
>   text+binary, нужен `WriteBuffer.write_char/write_str`).
>
> **Buffer — неудачное решение,** правильно заменено split'ом.
> Удаляется **без backward compat** (Nova не в production, революционный
> язык важнее обратной совместимости). Полное удаление — **Plan 04
> Этап 6**.
>
> **Что осталось сделать (Plan 04 Этап 6):**
> 1. Добавить `WriteBuffer @write_char(c)` и `@write_str(s)` — UTF-8
>    encode в byte buffer.
> 2. Мигрировать url.nv decode_query: Buffer → WriteBuffer + finalize
>    через `str.try_from(bytes)?`.
> 3. Удалить `Nova_Buffer*` из codegen (record_schemas, method dispatch).
> 4. Удалить `nova_rt/buffer.h`.
> 5. Удалить/мигрировать `nova_tests/runtime/buffer.nv`.
> 6. Q-buffer пометить ✅ REMOVED (не REPLACED — он не «заменён»,
>    а **удалён как ошибочный**).
>
...
> Plan 04 (`docs/plans/04-buffer-split-and-external.md`) Этап 6.
