...
## Q-infinite-value-type — dedicated `E_INFINITE_TYPE` для genuinely-infinite value-типов — 🟡 OPEN (minor, 2026-06-15)
...
Сейчас `None` сёрфейсится как **generic** `E_CONST_FN_GENERIC_NEEDS_T_REFLECTION` («размер `N` не
отрефлексировался»), а не как специализированная диагностика `E_INFINITE_TYPE` («тип `N` бесконечен:
value-поле `next` ссылается на сам `N` без указательной индирекции»). Это не дефект (тип отвергается,
компилятор не падает), но сообщение менее ясное, чем могло бы быть. Чистый `E_INFINITE_TYPE` требует
отдельного cycle-detection-прохода в checker'е (по value-полям, с visited-set), эмитящего ошибку ДО
того, как `size_of` форсит size-walk — это решено НЕ делать в рамках robustness-фикса (out of scope:
фикс — про no-crash + emit-accuracy, не про новую диагностику).

### Решение (для V1)

Депт-budget → `None` → graceful generic-ошибка. Dedicated `E_INFINITE_TYPE` — потенциальный следующий
шаг (DX-улучшение), не для V1. Делать, если появится мотивирующий запрос на ясность диагностики.
