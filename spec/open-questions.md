...
```nova
fn User.greet(self) -> str = ...    // метод типа: self явный
fn list_users() Db -> []User = ...   // функция с эффектом: self неявный
```

Два разных синтаксиса для семантически одинаковой вещи (функция,
ассоциированная с типом).

**Предложение.** Унифицировать:

```nova
fn User.greet(self) -> str = ...
fn Db.list_users(self) -> []User = ...   // self = активный handler в скоупе
```

Один синтаксис, два способа доступа к `self`:
- Для data-типа — экземпляр через `u.method()`
- Для эффект-роли — активный handler через `Db.method()`

Эффект в сигнатуре автоматически — через `self` (не нужно дублировать).

**Статус.** Обсудили, ты сказал «надо подумать». Не зафиксировано.

**Тонкие места:**
1. Неявный `self` для эффектов — это «магия» или унификация?
2. Куда отнести функции с несколькими эффектами (`Net Db Log Throws`)?
3. Переписывание стандартной библиотеки — много изменений.
...
**Не зафиксировано** в decisions.md явно. Если возникнет снова —
...
1. **Целочисленное переполнение** — wrapping в release или panic? D13
   говорит «debug — panic, release — wrapping», но это спорно для
   security-критичного кода.
2. **Stack overflow** — может ли быть не-panic, обрабатываемая
   ошибка? Или всегда смерть программы?
3. **OOM** — recoverable? Или процесс умирает? Erlang восстанавливается,
   Java бросает `OutOfMemoryError`. Nova?
4. **Assertion failures** в debug — это Panic или обычные `Throws`?
...
`syntax.md`, `effects.md`, `decisions.md` (D9) актуализированы. `audit.nv`
...
## Q12. Модель concurrency

**Контекст.** D14 зафиксировал `Async` как эффект и fiber-runtime,
[revolutionary.md R7](revolutionary.md) описал structured primitives
(`parallel for`, `race`, `select`, `cancel_scope`, `with_timeout`),
[revolutionary.md R9](revolutionary.md) — supervision. Но **между
этими тезисами есть пробелы**, которые блокируют stdlib (Q9) и AI-first
тезис «сигнатура = полное описание».
...
**Статус: снято.** [D21](../decisions.md) отменён в пользу managed memory
([D6 пересмотрен](../decisions.md)). Префиксов `~T`/`~&T` нет, atomic
...
- **Реализация GC ([D6](../decisions.md)).** Concurrent collector работает
...
CLI-приложений, и хорошо сочетается с [D6](../decisions.md) (managed memory
с concurrent GC).
...
- **Shared heap с concurrent GC** — единая куча на процесс, GC снимает
  вопрос refcount/atomicity для shared ownership ([D6 пересмотрен](../decisions.md))
...
реализации эффектов ([decisions.md:290](../decisions.md)). Брать ещё одну
...
Но `=` уже занят (присваивание, алиасы), и [D17](../decisions.md) фиксирует
...
> С введением [D46 (operator overloading)](../decisions.md) вопрос
...
Требует перегрузки операторов `|`, `&` для конкретного типа. См. [D1](../decisions.md)
...
[D24](../decisions.md). В дизайн-документе языка не фиксируется — может
...
3. **Принцип «не плодить специальные сущности»** ([D17](../decisions.md),
   [D18](../decisions.md), [D22](../decisions.md)) — trailing-closure это
...
**Статус: закрыто.** [D21](../decisions.md) отменён в пользу managed memory
([D6 пересмотрен](../decisions.md)). Циклы освобождаются автоматически
...
См. [D6 (новая версия)](../decisions.md) и discussion-log этап 13.

---

### Историческое тело (не актуально)

**~~Контекст.~~** [D21](../decisions.md) фиксирует: цикл через `~T` без
...
рефакторинге. Сейчас в [D21:1284-1290](../decisions.md#L1284-L1290)
...
2. [decisions.md](../decisions.md) — все принятые решения с обоснованием
