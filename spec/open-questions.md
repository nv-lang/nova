...
- `[M-140-contract-panic-unwind]` — release violation как panic-unwind
  (recoverable) вместо abort, если понадобится graceful degrade сервиса.
- `[M-140-contract-levels]` — per-module opt-out + Eiffel-style раздельная
  гранулярность (pre / post / invariant).
- `[M-140-bounds-as-contract]` — Vec `@index` OOB как
  `requires 0 <= i < @len` (после Plan 140; Plan 138.x/139).
...
**Расширено write-back (2026-06-13, `[M-140.2-elision-writeback §1]` ✅):** frame-check
read-only → len-инвариантный — `v[i]=val` (mut @index) СОХРАНЯЕТ длину, поэтому
write-back циклы (`for i in 0..v.len() { v[i]=f(v[i]) }`) теперь элидируют И
запись, И чтение (length-changing методы по-прежнему frame-unsafe).

**Отложено** (followup `[M-140.2-elision-writeback]`, остаток): общая cross-function
call-site instantiation (caller доказывает произвольный callee `requires` с
`@field`); `@index(Range)` slice-bounds; экзотические не-loop-var индексы.
