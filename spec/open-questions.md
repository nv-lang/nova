...
1. **Default build-policy.** Предлагается **enforce-by-default + явный
   opt-out** (`--contracts=off` восстанавливает legacy zero-cost целиком).
   Альтернативы: opt-in (`--contracts=enforce` по требованию — отвергается,
   возвращает дыру silent-UB по умолчанию); трёхуровневый
   `enforce|warn|off`. **Предложение:** `enforce` default; `off` —
...
3. **Гранулярность opt-out.**
   - **per-fn** `#unchecked` — точечно для hot-path (реализуется Ф.2).
   - **build** `--contracts=off` — глобально, legacy (реализуется Ф.2).
   - **per-module** `#unchecked_module` / манифест — **отложено**; нужен
     ли промежуточный уровень между fn и build? **Предложение:** V1 =
     per-fn + build; per-module — followup `[M-140-contract-levels]` по
     запросу (Eiffel-style гранулярность pre/post/invariant — там же).
