...
- Деление на ноль ✓
- Целочисленное переполнение (`int`: **panic в debug И release** — Plan 33.8 D13, не wrapping; доказуемо-безопасные чеки элидируются Z3 — Plan 140.4 [D271](decisions/09-tooling.md); sized-типы `u8`/…/`i32`: wrap по модулю 2^N — Plan 33.7) ✓ закрыто
