...
**Предложение:** **2** — Rust-style. Это согласуется с принципом «нет
скрытых аллокаций» и упрощает реализацию `nova_str_slice` (zero-copy).
...
## Q-strbuilder. Mutable string accumulator для hot-path

**Контекст.** D26 фиксирует что `s1 + s2` — O(a+b) per call, новая
аллокация. В hot loop `s = s + x` × N → O(N²) time + N allocations.
Это норма для immutable strings, но требует builder для production
кода.

**Прецеденты:**

| Язык | Builder | Capacity-grow |
|---|---|---|
| Java | `StringBuilder` | 2x grow |
| C# | `StringBuilder` | 2x grow |
| Go | `strings.Builder` | append-style |
| Rust | `String` (mutable) | 2x grow |
| Python | `''.join(list)` | manual |

**Предложение.** Добавить `StrBuilder` в prelude:
...
let s = sb.into_str()       // финализация без копии
```

Реализация: внутри `[]byte` (растущий буфер с 2x grow), при
`into_str()` — валидация UTF-8 + zero-copy переход в `str`.

Тонкости:
1. `into_str()` consume'ит builder (одноразовая операция). Альтернатива
   — `to_str()` копирует.
2. `sb.push(s str)` для `str` (быстро, memcpy). `sb.push_char(c char)`
   для одного codepoint (UTF-8 encode 1-4 байта).
3. Capacity hint: `StrBuilder.with_capacity(n int)` для известного
   размера.

**Связь:** [D26](decisions/08-runtime.md#d26), Q-array-api,
[D74](decisions/08-runtime.md#d74) (math как @-методы — здесь
`StrBuilder` это mutable type, методы через `mut @push`).
