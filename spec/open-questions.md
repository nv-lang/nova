...
**ФОРМА ВЫБРАНА ВЛАДЕЛЬЦЕМ 2026-08-09:** `on_timeout: |_| Err(ReadFailed)` — обработчик, а не готовое значение ошибки. Вопрос остаётся в этом файле только до слияния реализации: правило проекта требует D-амендмента в ТОМ ЖЕ слиянии, что и язык-меняющая правка, поэтому решение станет D-блоком вместе с кодом, а не раньше.
