...
frame-check (read-only цикл ⇒ длина инвариантна; Z3 моделит `_field_len_int(v)`
как фиксированное, а Nova переоценивает `0..v.len()` каждую итерацию).

**Отложено** (followup `[M-140.2-elision-writeback]`): общая cross-function
call-site instantiation (caller доказывает произвольный callee `requires` с
`@field`); write-back циклы (`v[i]=f(v[i])`); не-loop-var индексы; `@index(Range)`
slice-bounds.
