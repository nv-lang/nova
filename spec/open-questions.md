...
5. **Запрещён ли `resume`** для `Never`-операций (`Throws.throw`)?
...
|---|---|---|---|
| **One-shot** | Koka, OCaml 5, Eff | дёшево, stack-based | Throws, Db, Log, Time, Random — **95%** реальных эффектов |
...
весь `with`-блок. Это типичный паттерн для `Throws`:

```nova
fn try_parse(s str) -> Option[int] =>
    with Throws[ParseError] = (_) => None {
        Some(parse(s)?)
    }
```
...
`Throws.throw` имеет тип `Never` (нет возвратного значения). `resume`
