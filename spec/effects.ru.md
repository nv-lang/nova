...
## Что печатается при отказе ([D462](decisions/08-runtime.md#d462))
...
Форма **`?? return ...`** (ранний выход из объемлющей функции) зависит
от того, есть ли у функции обёртка для проброса
([D86](decisions/04-effects.md#d86) amend 2026-07-23 + amend 2026-09-01):

* функция возвращает `Option`/`Result` — форма **отвергается**
  (`E_COALESCE_RETURN_FALLBACK`): она была бы второй дверью к `?`. Канон —
  `X?` (та же обёртка наружу), `.ok()?` (Result → функция отдаёт Option),
  `.map_err(...)?` (меняется тип ошибки), `.ok_or(err)?` (Option → функция
  отдаёт Result);
* функция возвращает что-то другое (`bool`, число, `()`, кортеж, свой
  тип) — форма **законна**: пробрасывать нечего, `?` неприменим, и
  альтернатива ей — явный `match` из двух рук (тоже законен).
...
Ранний выход здесь — настоящий `return`: `defer`, `errdefer` и
`ensures`-контракты работают. Ошибка `Err(e)` при этом отбрасывается —
как и у обычного `??`; нужна `e` внутри — пиши `match`. Под generic-параметром
форма отвергается: под `T` может приехать `Option`.
