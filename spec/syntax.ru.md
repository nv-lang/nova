...
ro y ref int = a.b.c                   // локальный алиас: имя для чужого места
```
...
Позиции ограничены: возврат, локальный алиас, тип приёмника (`@` у
стекового типа — это `ref Self`). **Запрещена** в полях, коллекциях
...
Читается ссылка как значение: `ro t = peek(v)` КОПИРУЕТ — именованной
переменной-ссылки, кроме локального алиаса выше, в языке нет.
