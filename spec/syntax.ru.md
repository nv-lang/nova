...
Запись `p*.` разбирается однозначно: за `*` обязана идти точка, а точка не может
начать число (литерала `.5` в языке нет). Пробелы ничего не меняют — `p*.field`,
`p *.field` и `p* .field` одно и то же. Голый `p*` формой не является; значение
целиком берут методом `p.read()`.
