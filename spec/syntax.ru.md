...
Сахар выше (n Type mut @job(a int) -> @) не меняется — это по-прежнему
...
значений: Account.@balance типизируется как n(mut @Account) -> @, а не
n(Account) -> money — тип содержит @, и такой тип НЕЛЬЗЯ привести к
обычной функции (s fn(...) для этого запрещён в обе стороны — нужна лямбда).
Компиляторная форма (n job(mut @Type, a int) -> @) как ОБЪЯВЛЕНИЕ в коде
