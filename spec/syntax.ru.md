...
берут `requires`, `ensures`, `assert`, `debug_assert` и `panic`** — повторять в
каждой строке не нужно. Два таких параметра — отказ по имени. Чтобы указать не
на свой параметр, его передают явно: `requires cond, "msg", loc`,
