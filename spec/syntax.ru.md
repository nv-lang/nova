...
обёрток. Проброс — свойство ВЫЗЫВАЮЩЕГО, а не вызываемого: непомеченный передаёт
свою буквальную строку вызова, помеченный — то, что получил сам; поэтому место
доезжает ровно настолько, насколько расставлены пометки, и останавливается на
первом НЕпомеченном кадре. В непомеченной функции
...
**Оператор `?` следует тому же правилу.** Каждый `?` штампует кадр в трассу
распространения, которую печатает паника ([D437](decisions/08-runtime.md#d437));
в помеченной функции этот кадр несёт место вызова вместо строки самого `?` — и
только это делает кадры тонкой обёртки различимыми. Состав и глубина трассы не
меняются.
