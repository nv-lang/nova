...
**Функция может узнать, откуда её позвали** — `#track_caller` плюс встроенная
`caller_loc()`, возвращающая тип прелюдии `CallerLoc(file str, line int)`
([D468](decisions/08-runtime.md#d468)). Без этого собственная дверь диагностики
называет СЕБЯ, а не виновника: `assert` и контракты получают локацию от
компилятора, а рукописный `ice()`/`expect()` — не мог. Атрибут значит «место
вызова протекает через меня», поэтому он пишется на КАЖДОМ звене цепочки
обёрток: помеченная, вызывая помеченную, передаёт ИСХОДНОГО вызывающего, и
место останавливается на первом НЕпомеченном кадре. В непомеченной функции
`caller_loc()` честно возвращает свою собственную строку. Эффекта не несёт —
значение есть константа места вызова, и именно это позволяет звать её в дверях
`-> never`.

**За атрибутом следует ВСЁ, что печатает локацию в помеченной функции** —
`panic`, `assert`, `debug_assert`, `requires`/`ensures` и `caller_loc()`
одинаково называют МЕСТО ВЫЗОВА. Одно правило, таблицу помнить не нужно:
помеченная функция ведёт себя одинаково во всех своих отказах. Сообщение при
этом несёт ОБА места, поэтому баг самой обёртки остаётся находимым:

```
caller.nv:17: assert failed: text (cond) [in ice at diag.nv:129]
```

**Контракт называет вызывающего и БЕЗ атрибута.** Нарушенный `requires`/`ensures`
сообщает место вызова, а не строку объявления вызываемой функции: нарушенное
предусловие — вина вызывающего. Атрибут затем расширяет это с непосредственного
вызывающего на исходного, сквозь цепочку обёрток. `assert` вне помеченной функции
по-прежнему называет себя — это её собственный инвариант.
