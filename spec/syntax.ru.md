...
**Что ещё следует за атрибутом:** нарушенный `requires`/`ensures` помеченной
функции называет МЕСТО ВЫЗОВА (нарушенное предусловие — вина вызывающего), и так
же ведёт себя `panic`; а `assert` в теле сохраняет свою строку, потому что это
внутренний инвариант функции, и указание в чужую сторону спрятало бы её
собственный баг. Одной фразой: проверка обязательства ПЕРЕД вызывающим указывает
на вызывающего, внутренняя проверка — на себя.
