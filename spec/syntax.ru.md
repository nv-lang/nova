...
обёрток; в непомеченной функции `caller_loc()` честно возвращает свою
собственную строку. Эффекта не несёт — значение есть константа места вызова,
и именно это позволяет звать её в дверях `-> never`.
