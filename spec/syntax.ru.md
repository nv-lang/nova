...
**Escape-последовательностей внутри бэктик-тела ровно три:** `` \` ``, `\\`, `\$`.
Всё остальное после `\` проходит как ДВА символа — именно поэтому
`` regex`\d{3}-\d{4}` `` работает без удвоения. Бэктик-литералу тег не нужен:
голый интерполирует так же, как `"..."`.

Подробно — [D48](decisions/03-syntax.md#d48), с амендментом
[D467](decisions/03-syntax.md#d467) §5–§7 (2026-08-30): набор escape закрыт до трёх
(`\n`/`\t` сняты — форма многострочная, перенос набирается, а их раскрытие ломало пути
и регэкспы), голый бэктик стал законной формой, отступ тела и CRLF нормализуются.
...
Каждое `${expr}` печатается протоколом `Display` — примитивы и
prelude-типы реализуют его сами; пользовательский тип подключается
методом `@display(mut f Fmt)` ([D422](decisions/02-types.md#d422)), а
голое `${x}` вдобавок требует явного согласия `#impl(Display)`
([D186](decisions/02-types.md#d186)). Буквальное `${` в строке — через
escape: `"\${name}"`.

Что именно напечатается для структурного типа, у которого своего
`@display` нет, а ворота `#impl(Display)` стоят: компактная форма
БЕЗ имён полей — `Point(1, 2)`, а для суммы — `Some(5)`, payload как
значение. Именованная форма `Point { x: 1, y: 2 }` принадлежит
`${x:?}` и протоколу `Debug`, а не `Display` ([D422](decisions/02-types.md#d422) §4,
амендмент D109 от 2026-09-18). У примитивов обе формы совпадают
(`42`, `true`) — различие есть только у структурных.

Список протоколов в `#impl` НЕПУСТ: несколько пишутся через `+`
в ОДНОЙ аннотации (`#impl(Display + Equal)`), а `#impl()` и голое `#impl` —
ошибка (`E_IMPL_NO_PROTOCOLS`): аннотация существует, чтобы сказать «автор
согласился», а согласие ни на что не открывает ни одних ворот
([D186](decisions/02-types.md#d186), амендмент 2026-09-18).

> **Исправлено 2026-09-18; здесь стояло три снятых утверждения сразу,** и это
> единственное место страницы, где сказано почему — ниже по тексту правка та же,
> без повтора. Было: «рендерится через `str.from(v)` … `@display(mut w Write)`,
> [D73]». (1) Встроенного `str.from(v)` для примитива не существует — амендмент
> D54 (`03-syntax.md`), жива только ПОЛЬЗОВАТЕЛЬСКАЯ перегрузка `fn str.from(T)`.
> (2) Сигнатуру заменил D422 (2026-07-15, keystone). (3) D73 ретрактирован
> ЦЕЛИКОМ 2026-07-06 (`08-runtime.md`) вместе с `From`/`Into`.
>
> **Как это нашлось и почему важно:** `paradigm.ru.md` всё это время писала
> `@display(mut f Fmt)`, то есть две обзорные страницы противоречили ДРУГ ДРУГУ.
> Расхождение двух обзоров между собой доказывает больше, чем ссылка на D-блок.
> Обзор ниже D-блока по старшинству, но ВЫШЕ компилятора — поэтому человек,
> написавший метод по этой странице, получал отказ и был прав по странице.
> Снятый текст оставлен видимым: он же стоит в старых D-блоках, и читатель,
> пришедший оттуда, иначе решит, что ошибся он.

**Набор escape внутри `"..."` ЗАКРЫТ** ([D467](decisions/03-syntax.md#d467) §2,
2026-08-29): `\n \t \r \\ \" \0 \$ \xNN \u{H…}` плюс `\` перед переносом строки,
который продолжает строку и съедает ведущие пробелы следующей (§3). Всё прочее после `\` —
ошибка, а не сквозной проход: открытый набор означал бы, что добавленная завтра форма
меняет смысл написанного сегодня.

`\xNN` — КОДОВАЯ ТОЧКА U+00NN, не сырой байт (амендмент 2026-08-30): `str` — только
UTF-8, и байтовое чтение позволило бы одним escape собрать невалидную строку. Для сырых
байтов дверь — `Buffer`/`[]byte`.

Подробно — [D44 → «Строковые литералы и интерполяция»](decisions/03-syntax.md#d44),
с амендментом [D467](decisions/03-syntax.md#d467).
...
`==`/`!=` — через `@equal` (протокол `Equal`, `!=` выводится отрицанием); `<`/`<=`/`>`/`>=` — через единый `@compare(o) -> int` (протокол `Compare`, memcmp-стиль: `< 0` / `0` / `> 0`). **Нет `@equal`, но есть `@compare` — равенство берётся оттуда:** у протокола `Equal` тело по умолчанию `@equal(o) => @compare(o) == 0` (`std/prelude/protocols.nv`), и тип, объявивший один лишь `@compare`, сравнивается на `==` именно им, а не пополево (измерено 2026-09-16 тремя пробами; разбор — [D183](decisions/02-types.md#d183-canonical-comparison-protocols--default-method-bodies-plan-918a), раздел «Известные ограничения»). Протоколы при этом ОРТОГОНАЛЬНЫ: `Compare` НЕ встраивает `Equal`, тело по умолчанию висит на самом `Equal`. Индексация `a[i]` / `a[i] = v` — `@index` / `mut @index` (протоколы `Index[K, V]` / `MutIndex[K, V]`, D240); slice-индексация `a[x..y]` — та же `@index`, перегруженная по типу параметра: `x..y` (half-open, не включает `y`) лоуэрится компилятором в `Range { start: x, end: y }`, и вызывается `a.index(r Range)` — на `[]T`/`str` возвращает view без копирования (`std/collections/vec/slice.nv`, `std/runtime/string/slice.nv`). `&&`/`||` **не перегружаются** (short-circuit
...
**Срез требует ДВУХ методов, и это роль, а не одна перегрузка.** `a[x..y]`
разворачивается в `a.index(x..y)`, а открытый конец (`a[x..]`, `a[..]`) —
в `a.index(Range { start: x, end: a.end_index() })`. Поэтому тип, умеющий срез,
обязан отвечать И `@index(r Range)`, И `@end_index() -> int`: первый берёт
диапазон, который ему дали, второй говорит, где у значения конец. Пара и есть
роль `RangeIndex[V]` — [D470](decisions/03-syntax.md#d470), она же амендмент к
[D238](decisions/03-syntax.md#d238), где источником открытого конца назывался
`len()`. Роль структурна (D9/D42): объявлять соответствие не нужно, достаточно
написать оба метода. Тип с `@index(r Range)` и без `@end_index()` получает на
`a[x..y]` отказ `E_SLICE_ROLE_UNSATISFIED`, а не срез по догадке о поле `len`.

**Роль требуется у САХАРА, а не по типу аргумента.** `x[a..b]`, `x[a..]`,
`x[..b]`, `x[a..=b]` — это сахар, и он спрашивает роль. А явное значение типа
`Range` (`ro r = 2..5`, затем `x[r]`) — обычный `Index[Range, V]` по
[D238](decisions/03-syntax.md#d238), и `@end_index()` ему не нужен: у собранного
`Range` открытого конца нет, материализовать нечего. Различие названо потому,
что в реализации оно выглядит небрежностью и его тянет «починить» — подробнее
в [D470](decisions/03-syntax.md#d470) §3.
...
- **Пустой `Vec` и пустой срез не выделяют памяти** ([D232-амендмент
  2026-09-24](decisions/02-types.md#d232-vect--nova-native-generic-growable-array)): `Vec[T].new()`
  и `[]T` нулевой длины — нулевой указатель данных, `cap == len == 0`, выделение только при
  первом `push`/`reserve`/`cap(n)`. При `len == 0` указатель не несёт контракта данных: его не
  разыменовывают и не передают источником `memcpy`; на границе FFI пустота — пара `(NULL, 0)`.
  У пустого `str` указатель не нормирован (литерал `""` интернирован), судят по `byte_len()`.
- **Обычный `[T]` принимает только обычные типы** ([D156-амендмент
  2026-09-24](decisions/02-types.md#d156)): must-consume тип подставляется только в параметр
  `[T consume]`, тело которого компилятор проверяет строго. Контейнер объявляется
  `type Vec[T consume]`; его методы, выбрасывающие или удваивающие элементы, пишутся с обычным
  `[T]` и для must-consume элемента просто недоступны. В таком типе метод с обычным `[T]`
  не берёт `consume x T`: забирающая форма пишется один раз, с `[T consume]`
  (`E_CONSUME_PARAM_UNBOUNDED`, пункт 6 амендмента, 2026-09-25). Метод может иметь пару
  форм, различающихся только маркером, — общую `[T consume]` и быструю `[T]`: обычный `T`
  берёт `[T]`, must-consume — `[T consume]`, результат обязан совпадать (пункт 7,
  [D464-амендмент 2026-09-25](decisions/10-overloading.md#d464--бáунд-как-фильтр-отбора-отсев-без-ранжирования-2026-08-16)).
  Порядок (пункт 8): `for consume x in v` идёт с первого элемента по последний, а очистка
  контейнера с `Cleanup` — с последнего по первый, LIFO как у `defer` (D161).
- **Авто-`@cleanup` освобождает ОДНУ форму биндинга, а не всё, кроме
  ...
  пока её не впишут в список. Обязательство потребления заводится для
  `while Pat = expr`, для результата consume-области и для типизированного
  параметра замыкания/хендлер-опа.
- **`consume`-ПАРАМЕТР освобождается САМ, как и `consume X = e`**
  ([D432 rev-2](decisions/02-types.md#d432), решение владельца 2026-09-16). Если у типа
  объявлен эффект-free `@cleanup`, непотреблённый `consume`-параметр освобождается
  к концу тела функции — ошибкой это больше не является. До этой правки гарантия
  висела на ФОРМЕ ЗАПИСИ, а не на ТИПЕ: одно и то же значение освобождалось само
  в биндинге и требовало явного потребления, будучи параметром. У типа БЕЗ
  `@cleanup` ничего не меняется: строгая линейность D133. `consume`-ПРИЁМНИК
  амендментом НЕ затронут и остаётся открытым вопросом.
...
- **Именованный кортеж: строится по позиции, разбирается по имени**
  ...
  **Умолчания у полей — только В ХВОСТЕ**
  ([D482](decisions/02-types.md#d482-ex-d478-renumber-2026-09-20-умолчания-только-в-хвосте-решение-владельца-2026-09-14),
  решение владельца 2026-09-14): `type BadOrder(a int = 7, b int)` — ошибка
  компиляции, как и `fn g(x int = 0, y int)` по D102. До этого амендмент D215
  чередование ЯВНО РАЗРЕШАЛ, так что D482 — отмена разрешения, а не
  заполнение пробела. Причина не в стройности: проверка арности судит
  КОЛИЧЕСТВО аргументов, а счёт отличает связанное поле от несвязанного
  только пока умолчания идут хвостом.
- **Анонимный тип с ИМЕНОВАННЫМИ компонентами**
  ([D469](decisions/02-types.md#d469), решение владельца 2026-09-04):
  `fn loc() -> (file str, line int)`. Заполняет последнюю пустую клетку решётки
  форм — имя у ТИПА и имена у ПОЛЕЙ суть две независимые оси, и сочетание
  «типа нет, поля названы» до сих пор отсутствовало. Смысл виден на возврате
  нескольких значений: `-> (str, int)` заставляет вызывающего помнить ПОРЯДОК,
  `-> (file str, line int)` называет смысл на месте и переживает перестановку.
  Тождество СТРУКТУРНОЕ и включает имена компонентов: `(file str, line int)` и
  `(path str, line int)` — разные типы. Доступ по имени (`l.file`), разбор
  фигурной формой, конструирование позиционное — всё по общим правилам форм.
  **Формы в языке до этого блока НЕ БЫЛО** (замерено: `expected ')', got
  identifier`), то есть блок её вводит, а не описывает.
  **Деталь приведена в соответствие 2026-09-04 (решение владельца).** Эта
  страница была права всё время, а расходился сам D102: его правило 2 говорило
  «параметр без дефолта связывается позиционно **или** по имени», и на этом
  основании амендмент D215 объявлял себя противоречащим D102, будучи с ним
  согласным. Теперь D102 говорит то же, что и здесь: **обязательный —
  позиционно, дефолтный — по имени**. Тем же решением снят пример
  `ro Point(x, y) = p` (`03-syntax.md`): разбор не называет тип — `(x, y)` для
  позиционной формы, `{ x, y }` для именованной, ровно как сказано строкой выше.
...
- `Display`/`@display(mut f Fmt)` — представление в строку для
  `${expr}`-интерполяции; голое `${x}` требует `#impl(Display)`
...
- `Display` — protocol с instance-методом `@display(mut f Fmt)`,
  представление в строку ([D422](decisions/02-types.md#d422))
...
### Временное `consume`-значение во view-позиции (D476)

Временное значение consume-типа, попавшее во view-позицию — аргументом без
квалификатора или приёмником невинного метода, — раньше отвергалось. Теперь оно
**разворачивается в consume-скоуп**:
...
Скоуп **узкий**: он обнимает то выражение, где временное участвует, а не весь
объемлющий вызов — иначе ресурс держался бы всё время работы соседнего аргумента.
...
Блок есть ВЫРАЖЕНИЕ (D188), поэтому отдельной позиции оператора не нужно —
работает и там, где её нет:

```nova
while consume t = get_luck() { t.more() } { … }
```

Условие цикла перевычисляется каждую итерацию, значит скоуп создаётся и чистится
каждую итерацию; правый операнд `&&` вычисляется условно — скоуп входится условно.

**Чего правило НЕ делает.** Оно не трогает consume-ПАРАМЕТР (`f(make_tx())` при
`f(consume x Tx)` законно и так, владение передаётся без биндинга) и не лечит
именованный биндинг не той формы: `ro opt = get_job()` остаётся ошибкой, законная
запись — `consume opt = get_job()`. У типа БЕЗ `@cleanup` (строго линейного)
десугаринг даёт непотреблённый биндинг, и `D133-not-consumed` срабатывает как
надо.

**Цена, названная вслух:** очистка может нести эффект, и тогда `Fail[E]` обязан
появиться в подписи объемлющей функции — строка, выглядящая как печать, может
потребовать дописать эффект. Взамен ни одна диагностика не имеет права назвать
синтетическое имя: она показывает на выражение, которое написал автор.

Норма — [D476](decisions/03-syntax.md); реализации пока нет ни у одного
компилятора.

### Рекурсивный тип — маркер `indirect` (D477)

Сумма размещается **по значению**, поэтому вариант, чья нагрузка содержит сам тип,
дал бы бесконечный размер. Косвенность объявляется **явно**, маркером на
объявлении:
...
напрямую, `unsafe` не нужен.

**Размещение по значению реализует Карина** (план 274, Т2); у оракула его нет
и не планируется — критерий владельца: менять оракул только ради того, без
чего не собирается Карина, а сумма в куче ей не мешает. **Оракул с D477 §5б
(2026-09-25) маркер ПАРСИТ** — токен принимается и отбрасывается, сумма
остаётся в куче как всегда, — это снимает отказ парсера для кода, написанного
под будущий синтаксис (план 286), но размещения не даёт: `E_RECURSIVE_NEEDS_INDIRECT`
и `E_RECURSIVE_NO_BASE_CASE` ниже — тоже только у Карины.
...
**Место маркера — одно правило на две фигуры:** он ставится на ту единицу, чей
размер считается отдельно. У суммы размер есть максимум по вариантам — маркер на
**варианте**; у `value`-записи размер есть сумма полей — маркер на **поле**:
...
**Маркер отвечает на «сколько занимает», а не на «где кончается».**
`job indirect Job` без `Option` имеет конечный размер и при этом непостроим:
чтобы создать `Job`, нужен готовый `Job`. Базовый случай даёт `Option` либо
вариант-лист; компилятор различает два отказа
(`E_RECURSIVE_NEEDS_INDIRECT` и `E_RECURSIVE_NO_BASE_CASE`) — правки у них разные.

**Обычной записи маркер не нужен:** ссылка на heap-тип по значению занимает
размер указателя, а не размер объекта, поэтому `type Job { job Option[Job] }`
конечна и без него. Обязанность появляется ровно у тех, кто запросил размещение
по значению.

**Рекурсия — повод обязательный, но не единственный.** Маркер ставят и там, где
рекурсии нет, — чтобы ужать толстый вариант: пометка убирает из размера суммы всю
нагрузку, а не только рекурсивную часть.
...
`Decorated` — имя варианта, `(BigRec, Node)` — его нагрузка из двух значений:
сначала `BigRec`, потом ещё один `Node` (это и есть рекурсия — узел несёт
соседний узел). Как `Some(T)`, только значений два.

Тут `Node` весит тег плюс указатель — столько же, сколько `Leaf`. Без маркера
размер был бы бесконечным, а если бы маркер стоял только на рекурсивной части,
каждый `Leaf` платил бы 32 байта за соседний вариант.

Обязанность появляется у бесконечного типа; законное применение шире.

**Маркер законен при любом классе хранения, но работает по-разному:** у стекового
типа под ним появляется прозрачный указатель (в размер входит указатель, в коде
косвенности не видно), у кучевого не меняется ничего — значение и так живёт по
ссылке. Отказа на кучевом нет намеренно: иначе обобщённый код пришлось бы писать
по-разному для `T = value` и `T = heap`, а класс хранения известен только с
инстанциацией.

Норма — [D477](decisions/02-types.md); реализации пока нет ни у одного
компилятора.
...
**Голое имя в образце — ВСЕГДА связывание, даже если так называется константа.**
`match w { MUT_W => 1, _ => 0 }` не сравнивает `w` с `MUT_W`, и потому эта форма
отвергается (`E_MATCH_CONST_PATTERN`): сравнение пишется guard'ом —
`w if w == MUT_W => 1`. Перечень форм образца ЗАКРЫТ
([D19](decisions/03-syntax.md#d19), амендмент 2026-09-18): форма, которой нет в
таблице выше, не принимается.
...
стоят по отдельности: `Some((a, b))`, `Ok((a, b))`, `Err((a, b))` (tuple
внутри payload) и `Some({ a, b })` (record внутри payload) — законные формы,
когда сам payload-тип — кортеж или record
([D157](decisions/05-memory.md#d157), амендмент `[M-216-record-payload-consume]`,
2026-07-21; поправка списка форм D184 — 2026-09-21, реестр 221.1 №1240).
...
Там, где грамматика форсирует перенос, решения нет и маркер не нужен (D157, амендмент 2026-09-24): имя payload использовано ровно один раз — хвостом арма, `return` или аргументом конструктора (`Ok(s) => s`, `Ok(v) => Ok(v)`). Любое иное использование, как `r.close()` выше, по-прежнему требует `consume`. Реализация — план 293.
...
`break` / `continue` — стандартные, без меток; оба — выражения типа `never`
(AMEND D25), вне цикла — ошибка. `break <значение>` — только в `loop`: значение
становится значением цикла, тип `loop` — общий тип всех `break` ([D485](decisions/03-syntax.md#d485)).
...
### Порядок вычисления аргументов вызова — слева направо (D484)

Аргументы вызова вычисляются **слева направо, в порядке записи**, и этот
порядок **наблюдаем**: если два аргумента производят эффекты, программа
обязана увидеть их именно в этом порядке.

```nova
f(say("left"), say("right"))   // left first, then right
```

Правило не новое: оно жило внутри D188, в прозе про точку дизарма, и потому
не находилось — за месяц три поиска подряд прошли мимо. Слово «наблюдаем»
здесь единственное добавленное, и оно не украшение: без него правило было бы подсказкой
компилятору, которую он вправе не исполнить. Норма и её цена — [D484](decisions/03-syntax.md#d484);
там же названы граница (метод запуском не проверен, конструктор блоком не
решается) и то, что нынешний компилятор эту норму НАРУШАЕТ, вычисляя справа
налево, — дефект реестра №685, а не расхождение вкусов.
...
### Доступ через указатель: `p*.field` (D483)

У указателя две разные вещи пишутся по-разному, и это видно в коде:
...
Раньше член цели писался просто `p.field` (авторазыменование), и если у типа
цели оказывался метод с именем примитива, вызов молча доставался примитиву.
Теперь выбирать нечего: слева от `*` — указатель, справа — цель.

Постфикс пишется **слитно**: `p*.field`. Формы `p * .field` и `p* .field`
невалидны, поэтому спутать с умножением нечего.

**Одна звезда на уровень указателя:**
...
Голый `p*` формой не является; значение целиком берут методом `p.read()`.

**`unsafe` для доступа НЕ нужен** (D216, амендмент 2026-09-20): `p*.field`,
`p*.m()`, `p.read()`, `p.write(v)` на `*T` и `*mut T` безопасны — указатель по
норме не бывает null, а null выражается только через `Option[*T]`. `unsafe`
остаётся на адресной арифметике (`read_at`, `offset`, …) и на `*uninit T` — типе,
в который указатель деградирует после арифметики.

**Линейность через указатель** ([D216, амендмент 2026-09-25](decisions/02-types.md#d216-typed-pointer-family--unsafe-model--null-safety-через-npo)):
`p.write(v)` / `p.write_at(i, v)` и `p.read()` / `p.read_at(i)` — копия, только обычный `T`;
`p.write_consume(v)` / `p.write_consume_at(i, v)` — владение в память, для любого `T`;
`p.read_consume()` / `p.read_consume_at(i)` — изъятие, результат владеющий; посмотреть, не
изымая, — `p.view(f)` / `p.view_at(i, f)`: элемент одалживается замыканию видом-параметром.

**Из C указатель приходит только как `Option[*T]`** — голый `*T` в `extern "C"`
запрещён. Иначе «безопасно» было бы обещанием, за которое никто не отвечает:
`match`, снимающий `Option`, и есть то место, где ручается программист.

> **Пока не реализовано, и граница этим правилом НЕ закрыта** (реестр №1185):
> по норме `Option[*T]` — один указатель с `None` = NULL, а выпущенный C сегодня
> даёт структуру с тегом и `nova_int` внутри. Правило верно, кода под ним нет.

Норма — [D483](decisions/02-types.md); реализации пока нет.

### Тип-ссылка: `ref T` и `ref mut T` (D480)

Ссылка на чужое место пишется ОДНИМ словом перед типом, а изменяемость —
вторым, и только когда сквозь ссылку пишут:
...
**Что значит `mut` у параметра** (D326 Р3, амендмент 2026-09-24): право менять ПОЛУЧЕННОЕ значение, а не связь с переменной вызывающего. У стекового (value) типа значение и есть хранилище вызывающего — присваивание ему видно снаружи. У кучевого типа значение — сам объект: изменение объекта видно, а присваивание параметру перепривязывает лишь локальное имя, и на это компилятор предупреждает `W_MUT_HEAP_PARAM_REBIND`.

**Локальную переменную-ссылку объявить нельзя** (D480): `ro y ref int = a.b.c` —
ошибка. Ссылка живёт как тип возврата и как тип приёмника (`@` у стекового типа),
но имени ей не дают. Причина — ей не нашлось применения: у кучевого типа обычная
переменная и так псевдоним, у `value`-типа остаётся полный путь.

> **Ссылку возвращают только на место, которое переживёт вызов.** У `ro`-параметра
> представление выбирает компилятор (копия либо скрытая ссылка), поэтому
> `fn peek[T](x T) -> ref T => x` вернул бы ссылку на умирающее хранилище —
> и отвергается. У `mut`-параметра место принадлежит вызывающему всегда, поэтому
> первая строка законна.

`ro ref T` и `ref ro T` — **ошибка** `E_REDUNDANT_REF_RO`: `ref T` и так значит
«только читать», как `*T` у указателя значит «только читать указуемое». Пара
умолчаний у ссылки и у указателя ОДНА: без слова — чтение, `mut` — запись.

Ссылка **непереселяема**: связал — навсегда, переприсвоить её на другое место
нельзя (потому и нет смысла в слове, описывающем изменяемость самой ссылки).

Позиции ограничены: возврат и тип приёмника (`@` у стекового типа — это
`ref Self`). **Запрещена** в локальных переменных, полях, коллекциях
(`Vec[ref T]`), суммах, `Option[ref T]` и тип-аргументах — иначе ссылка на
стековое место уехала бы в долгоживущую кучу, и понадобились бы лайфтаймы.
Возврат `ref T` из обобщённой функции законен (D480).

Читается ссылка как значение: `ro t = edit(v)` КОПИРУЕТ — именованной
переменной-ссылки в языке нет вовсе.

Подробно — [decisions/02-types.md → D480](decisions/02-types.md#d480-тип-ссылка-запись-ref-t--ref-mut-t-возврат-ссылки-из-обобщённой-функции-решение-владельца-2026-09-20)
и ревизия [D326](decisions/02-types.md#ревизия-d326-plan-184-ref-t--ограниченный-тип).
