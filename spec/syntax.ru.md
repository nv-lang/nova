...
- **Обычный `[T]` принимает только обычные типы** ([D156-амендмент
  ...
  (`E_CONSUME_PARAM_UNBOUNDED`, пункт 6 амендмента, 2026-09-25).
...
запись — пара форм, копирующая (только обычный `T`) и забирающая `consume`, выбор по
правилу 3 D84; `p.read()` / `p.read_at(i)` — копия, только обычный `T`;
`p.read_consume()` / `p.read_consume_at(i)` — изъятие, результат владеющий; посмотреть, не
изымая, — `p.lend(f)` / `p.lend_at(i, f)`: элемент одалживается замыканию видом-параметром.
