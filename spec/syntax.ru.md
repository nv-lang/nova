...
```nova
fn edit[T](mut x T) -> ref mut T => x  // сквозь неё пишут
fn peek[T](x T) -> ref T => x          // сквозь неё читают — см. оговорку ниже

ro y ref int = a.b.c                   // локальный алиас: имя для чужого места
```

> **Вторая строка примера сегодня НЕ компилируется** (замер 2026-09-20):
> компилятор отвечает `E_READONLY_COERCE`, считая не-`ro` всё, что не написано
> словом `ro`, — то есть правило «`ref T` и так только чтение» ему ещё не
> известно. По норме D480 форма законна; отказ на ней — дефект, а не защита.
