...
- **Авто-`@cleanup` освобождает ОДНУ форму биндинга, а не всё, кроме
  перечисленного** ([D432-амендмент 2026-08-21](decisions/02-types.md#d432),
  реестр 221.1 №672). Исключение §2 снимает обязательство только с
  `consume X = e;` с одиночным `Ident`-паттерном — единственной формы, под
  которую кодоген действительно вставляет очистку. Правило говорит о
  ПРОИСХОЖДЕНИИ биндинга, а не перечисляет исключения, чтобы следующая
  добавленная в язык форма попадала под норму сама, а не текла до тех пор,
  пока её не впишут в список. Обязательство потребления заводится и для
  consume-параметра, и для `while Pat = expr`, и для результата consume-области,
  и для типизированного параметра замыкания/хендлер-опа.
- **Правило алиаса идёт за ro-местом В ЭЛЕМЕНТ КОРТЕЖА**
  ([D246-амендмент 2026-08-23](decisions/02-types.md#d246), реестр 221.1 №717,
  дыра 1). Место, укоренённое в ro-имени и лежащее в элементе кортежа, судится
  по объявленному типу ЭТОГО ЭЛЕМЕНТА, а не кортежа целиком:
  `fn wrap(v []int) -> ([]int, int) => (v, v.len())` — это `E_READONLY_COERCE`,
  канон — `-> (ro []int, int)`. Замерено до того, как правило появилось:
  вызывающий писал `pair.0[0] = 999`, и запись доходила до замороженного
  источника. Элемент, объявленный `ro`; элемент со свежим значением (вызов,
  литерал, конструктор); полностью стековый элемент — исключены ровно так же,
  как в голой позиции возврата: судит их та же проверка.
- **`ro` заражает только по алиасу**
  ([D246-амендмент 2026-08-21](decisions/02-types.md#d246), реестр 221.1 №717).
  Локаль, чьё значение — место, укоренённое в ro-имени (`ro al = v`,
  `ro f = v.field`, `ro e = v[i]`), сама ro и не может быть возвращена под
  не-`ro` типом возврата — включая хвост ветви `if`/`match`. Локаль,
  связанная со СВЕЖИМ значением (результат вызова, литерал, конструктор,
  `.clone()`), — обычная, сколько бы немутабельной ни была её привязка.
...
**Функция может узнать, откуда её позвали** — запись прелюдии `CallerLoc`
плюс встроенная `caller_loc()` ([D468](decisions/08-runtime.md#d468)). Без них
собственная дверь диагностики называет СЕБЯ, а не виновника: `assert` и
контракты получают локацию от компилятора, а рукописный `ice()`/`expect()` — не
мог.

Место вызова приезжает ОБЫЧНЫМ параметром с умолчанием — ни атрибута, ни
скрытого канала:
...
`caller_loc()` значит «место этого выражения», а умолчание подставляется НА
МЕСТЕ ВЫЗОВА — поэтому в параметре оказывается строка вызывающего, а не строка
объявления. Одно правило, без режимов. Эффекта не несёт, так что её можно звать
в дверях `-> never`, и в рантайме она не стоит ничего: настоящего вызова нет,
есть константный указатель на статическую запись.

**Внутри функции, У КОТОРОЙ есть параметр типа `CallerLoc`, его автоматически
берут `requires`, `assert`, `debug_assert`, `panic` и `throw`** — повторять в
каждой строке не нужно; `throw` пишет его в поле `site` записи отказа.
**`ensures` — сознательное исключение:** нарушенное постусловие — баг самой
функции, вызывающий не может его вызвать даже теоретически, поэтому оно
по-прежнему называет себя — так назначают вину все контрактные языки (Eiffel,
D, Ada). Одна фраза вместо таблицы: что говорит о ВХОДЕ или наружу — указывает
на вызывающего; `ensures` говорит о собственном ВЫХОДЕ. Два таких параметра —
отказ по имени. Чтобы указать не на свой параметр, его передают явно:
`requires cond, "msg", loc`, `assert(cond, loc)`, `panic("…", loc)`.

**Цепочка пробрасывается РУКАМИ.** Обёртка, которая хочет обвинить своего
вызывающего, передаёт `loc` дальше:
...
Обе записи читаются буквально, и разница видна в тексте. Здесь Nova идёт за C# и
Swift, а не за Rust: неявного проброса нет, а значит нет и скрытого параметра,
который пришлось бы нести системе типов. Цена названа: проброс — дисциплина
автора, компилятор о нём не напомнит.

Сообщение при этом несёт ОБА места, поэтому баг самой обёртки остаётся находимым:
...
**Наборы перегрузок, различающиеся только РЕЖИМОМ параметра, разрешаются доминированием, а не суммой очков** (ось режимов D84; реализовано 2026-09-02, реестр 857). Кандидат побеждает, только если он не менее специфичен в каждой позиции и более специфичен хотя бы в одной: `consume` > `mut` > `ro`. Когда два кандидата меняются в противоположные стороны — например `f(consume a, b, c)` против `f(a, mut b, mut c)` — не доминирует ни один, и вызов отвергается диагностикой `[E_OVERLOAD_AMBIGUOUS_MODE]` с перечнем кандидатов. Молчаливого выбора нет: до этой даты такой вызов компилировался и тихо исполнял один из двух.
...
**Имя с `_`-префиксом нельзя использовать, а `_` нельзя выбросить линейное
значение** ([D461](decisions/03-syntax.md#d461)). Префикс — это обещание, что
значение не нужно, и компилятор держит вас за слово: для любого типа, не только
линейного. А `_` ничего не привязывает, поэтому выбросить им линейный
(`consume`) payload значило бы утечь ресурсом без единого ключевого слова — это
тоже ошибка. Префиксы, зарезервированные за компилятором (`_nova_`, `_at_`,
`__`), исключены: правило про человеческое соглашение.
...
Сахар выше (`fn Type mut @job(a int) -> @`) не меняется — это по-прежнему
единственная форма ОБЪЯВЛЕНИЯ метода. D458 (2026-08-12, реализация — план
[273](../docs/plans/273-one-form-for-methods-and-functions.md), в работе)
формализует, чем `@` является внутри компилятора и в ТИПЕ значения метода:
переменная типа получателя (аналог `Self`), связанная первым параметром, той
же переменной остаётся `@` и в `-> @`/`Option[@]`. Отсюда следствие для
значений: `Account.@balance` типизируется как `fn(mut @Account) -> @`, а не
`fn(Account) -> money` — тип содержит `@`, и такой тип НЕЛЬЗЯ привести к
обычной функции (`as fn(...)` для этого запрещён в обе стороны — нужна лямбда).
Компиляторная форма (`fn job(mut @Type, a int) -> @`) как ОБЪЯВЛЕНИЕ в коде
запрещена (`E_D458_COMPILER_FORM_IN_SOURCE`), но легальна как аннотация ТИПА:
...
**Бáунд при отборе перегрузок — фильтр, не ранжирование** ([D464](decisions/10-overloading.md#d464--бáунд-как-фильтр-отбора-отсев-без-ранжирования-2026-08-16)). Кандидат, чей бáунд не держится на выведенной подстановке, отпадает; один выживший берётся молча; выживших больше одного без структурного доминирования (конкретное над generic, D84) — ошибка неоднозначности, «чей бáунд у́же» не сравнивается; ни одного выжившего — ошибка бáунда (D72). Проверяется в чекере из самого бáунда, до мономорфизации; кодоген получает готовое решение.
...
## Panic — не эффект; fiber умирает, РОДИТЕЛЬ наблюдает

Деление на ноль, выход за границы массива, переполнение — это
**не эффект**, это `Panic`: никакого `Fail[DivByZero]` в сигнатурах, никакого
`try/catch` вокруг собственного кода. Panic убивает **текущий fiber** —
изнутри этого fiber'а перехватить его нельзя:
...
Но смерть ДОЧЕРНЕГО fiber'а **наблюдаема родителем** — это спроектированная
дверь, а не внутренность рантайма
([D416](decisions/06-concurrency.md#d416), эффект `Supervisor`):
`supervised`-scope по умолчанию перебрасывает ошибку первого упавшего ребёнка
на выходе из scope, а под `with Supervisor = policy` родитель инспектирует
каждое падение (`on_child_fail(idx int, err any) -> Decision`) и решает
`Stop`/`Escalate` по каждому ребёнку — готовые политики в
`std/concurrency/supervisor`. Точная формула: *свой panic fiber не ловит;
родитель его супервизирует.* (Прежняя редакция этой страницы говорила
«программист не ловит panic в коде» без супервизорной половины — исправлено
2026-08-14 по D416.)
