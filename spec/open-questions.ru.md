...
- Деление на ноль ✓
- Целочисленное переполнение (`int`: **panic в debug И release** — Plan 33.8 D13, не wrapping; доказуемо-безопасные чеки элидируются Z3 — Plan 140.4 [D272](decisions/09-tooling.md); sized-типы `u8`/…/`i32`: wrap по модулю 2^N — Plan 33.7) ✓ закрыто
...
`s.ends_with(p)`, `s.to_ascii_lower()`, `s.to_ascii_upper()`, `s.trim_ascii()` — все
...
**Решение из обсуждения:** первый компилятор Nova пишется на **Rust**.
~~После self-hosting (v2.0+) переписывается на Nova.~~
**АМЕНДМЕНТ 2026-08-30 (решение владельца; конвенция именования §1.2, внутренние доки):
старый ответ писался до плана 274.** Самоходный компилятор (бренд — **Carina**,
рабочее имя `novac`) УЖЕ пишется на Nova, и его релиз-цель — **Nova Carina 1.0**
(первый релиз, где бинарь `nova` собран самой Nova), а не v2.0+; v0.1 выпускает
оракул (Rust-компилятор, после тега замораживается как неподвижная точка поведения).
...
2. **Skip-семантика.** Поля, которые не сериализуем: отдельный atom
   `@json(skip)`, отсутствие атрибута → не включать, или `priv` field
   modifier (D220) автоматически skip'нут? (`priv` fields — implementation
   detail, не serialize'ятся by default.)
...
> ✅ **ЗАКРЫТ ОКОНЧАТЕЛЬНО — D209 (Plan 108.4, 2026-06-09): `@` ОБЯЗАТЕЛЕН.**
> Голое имя — ошибка разбора `E_PROTO_METHOD_NEEDS_AT`; статический метод — через точку;
> модификатор приёмника (`mut`/`ro`/`consume`) стоит перед `@`. Дом нормы —
> [D209](decisions/04-effects.md), изложение — [02-types.md, «Method-prefix в protocol-блоке»]
> (decisions/02-types.md#d53).
>
> **Прежний ответ, ОТМЕНЁННЫЙ** (оставлен, чтобы вопрос не открыли заново):
> CLOSED by D53 / Plan 17 Ф.1 (2026-05-08) — **обе формы валидны и эквивалентны**.
> Примеры ниже набраны в ТОЙ форме и оставлены намеренно: здесь голое имя — предмет
> разговора, а не образец для подражания (правка 2026-09-16, окно Карины).
...
> **НОРМА ПЕРЕЕХАЛА 2026-09-18: [D481](decisions/03-syntax.md#d481).** Этот
> раздел больше НЕ источник правил — он история. Причина переезда названа в
> самом D481 и стоит того, чтобы знать её здесь: грамматика, закрывшая вопрос,
> три недели жила ТОЛЬКО тут, вне `spec/decisions/`, и её продукция `raw-char`
> не была раскрыта ни одной фразой. Из-за этого строка реестра №675 всё то же
> время утверждала, что лексер нарушает правило, — правила не существовало.
> D481 определяет алфавит `raw-char` и ЗАПРЕЩАЕТ сырой управляющий байт
> (`E_CHAR_RAW_CONTROL`); набор escape там записан ПО ЗАМЕРУ компилятора, и он
> УЖЕ этого раздела: `\b` и `\f` отвергаются, `\uHHHH` без скобок не
> поддерживается. Текст ниже читать только за историей.
...
> Все таблицы `std/unicode` генерируются инструментом **`nova-codegen unicode --ucd-dir
> <UCD>`** из официального Unicode Character Database в компактные `;`-кодированные
> Nova-таблицы. Пин к версии (`const UNICODE_VERSION str = "16.0"`), **ленивая**
> инициализация (парсятся в `HashMap`/range-таблицы при первом вызове через module-level
...
> **Amend (Plan 91.18, 2026-06-19):** bare-имена `to_lower`/`to_upper`/`trim`/`trim_start`/
> `trim_end`/`split_whitespace` теперь Unicode-семантика под `import std.unicode`. Из prelude
> доступны только `_ascii_`-варианты. `s.to_upper()` без import → E7320.
...
**→ Вариант C: оставить prefix `*p`.** Постфиксная мотивация (left-to-right при цепочках) реальна, но
для Nova не окупается, а точные постфиксные формы либо неграмматичны, либо ломают симметрию.

- **B `p*` (чистый постфикс) — НЕГРАММАТИЧЕН** (подтверждено): лексер выбрасывает пробелы и НЕ хранит
  adjacency-флаг (`lexer/mod.rs:309`) → `a*`, `a *`, `a*⏎b` дают идентичный поток `Ident Star …`; а `*` —
  И prefix-deref (`parse_unary:6320`) И infix-mul (`parse_mul:6161`) → `p*` неотличим от начала `p * y`
  без unbounded lookahead. Настоящая ambiguity. (Аналогично мёртв `p^` Pascal: `^` уже занят
  bitwise-xor — `lexer:265` → `BinOp::BitXor`.) **Ни один зрелый язык не взял голый `p*`** — именно
  поэтому постфиксные deref всегда добавляют разделитель (Zig `.`, Pascal/Odin `^`).
- **A `p.*` (Zig) — грамматичен** (ведущая `.` снимает ambiguity; ~3 строки в `parse_postfix:6340`), **но
  не окупается для Nova**: сырой `*p` редок (~57 сайтов, почти всё тест-фикстуры, 1 production-сайт в
  `std/collections/vec_owned.nv`), а форма `(*p).field`, которую постфикс бы починил, встречается **в 1
  файле из 2989** — авто-deref (`p.field`) + индексация (`p[i]`) поглощают весь chaining. A ценой ~50-65
  переписываний ломает симметрию `*T`/`*p` и пару `&`/`*`, зацементированную Plan 147/D246.
- **C `*p` — WIN на всех 4 осях:** парсится (зашипён), эргономика совпадает с реальным usage (deref почти
  всегда bare-unary `*p = v` / `ro v = *p`), доминирующая конвенция (C/C++/Rust/Go/D + **Carbon**, чистый
  лист 2020-х, явно рассмотрел постфикс/whitespace-disambig и **оставил prefix**, добавив `->` сахар),
  симметричен с prefix-типом `*T` и инверсной парой `&`/`*`. Ноль миграции.

**Симметрия «тип справа → deref справа»** (мотив автора; Zig = `*T` + `.*` её валидирует) — реальная
ось дизайна, но конкурирует с «`*` префиксит то, к чему применяется» (`*T`/`*p`; `&x`/`*p`-инверсия).
Тай-брейк решают парсер (B/`p^` мёртвы) + cost/benefit (A не окупается). 

### Отложенная опция
`p.*` (Zig) — единственная грамматичная постфиксная форма; вернуться **только если** сырой deref станет
частым или потребуется chained-навигация (сейчас — нет). Тогда: ~3 строки в `parse_postfix` (ветка
`Dot`→`Star`) + миграция ~50-65 сайтов + spec/docs (D216 §5, `examples/typed_pointers/basic_pointer.nv`).

### Cross-refs

- Plan 118 / Plan 147 (D216 §5, D246) — типизированные указатели `*T` / `*mut T` / `*unsafe T` (prefix-тип).
- Plan 138.5 — pointee-мутируемость в **типе** (`*mut T`) vs reassign-ability в **binding** (`ro`/`mut`);
  ортогонально deref-выражению `*p` этого Q28.
...
**Статус:** 🟡 OPEN, направление согласовано с владельцем 2026-06-21. Зависит от единого
носителя `ResolvedType` ([D315](decisions/02-types.md#d315-resolvedtype--единый-канонический-носитель-типа-plan-1721-2026-06-21))
— ABI/placement выводятся из ОДНОГО окна типа, поэтому это локальная правка ОДНОГО лоуэринга,
а не разбросанные ветки. Делать ПОСЛЕ MVP-консолидации 172.1 (сейчас byte-identical).

**Вопрос:** компилятор должен **единообразно** обращаться со всеми value-типами и
**автоматически** выбирать передачу/размещение, без явных аннотаций ([Q29](#q29-safe-references-ref-t--ref-mut-t--нужны-ли): explicit `ref T` отвергнут).

**Три связанных пункта:**

1. **Единый value-ABI (убрать раздельную обработку).** `type Job value {}`, `Tuple(int, str)`,
   `Tuple(a int, b str)`, named tuple — это ОДИН класс value-типов, но сейчас лоуэрятся РАЗНЫМ
   кодом (`NovaValue_X` vs `NovaTuple_X`; приёмник: heap-record / value-record by-pointer, а
   **named-tuple `ro @` by-value** — `Plan 128`/§4072 02-types.md). Эта раздельность — источник
   багов (владелец, 2026-06-21). Цель: ОДИН путь representation+приёмника+передачи из
   `ResolvedType`. Вектор — U.5/U.6 (свернуть tuple/value-record/sum-schema дубли).

2. **Авто-by-ref передачи параметров.** Большие value-значения передавать **указателем
   автоматически** (вместо копии), когда копия не наблюдаема — это и есть «`ref T`
   автоматически» (закрывает ЗА-аргумент Q29 «передача больших стековых значений»). Foundation:
   `escape_analyze.rs` (Plan 127) уже авто-промоутит value-records; обобщить на by-ref-передачу.

3. **Авто heap↔stack placement, в т.ч. нерекурсивный sum→стек.** Sum-типы сейчас **по умолчанию
   heap** (`Nova_X*`, 02-types.md:2454) — это правильно для **рекурсивных** (деревья/связи). Но
   **большинство sum — перечисления/нерекурсивные** и могут быть авто-свёрнуты в **стек-value
   (tagged-union)**, как уже сделано для билтина **Option (`NovaOpt` stack-value-sum)** vs `Result`
   (heap) — `Plan 62.A.bis`. Foundation готов: детектор рекурсии существует (size-калькулятор,
   08-runtime.md:8211-8235: «рекурсивный heap-тип — указательный лист»), value-стек-механизм —
   D228/D277/D290. Дедик-пункта на это НЕТ — фиксируется здесь. (Узкий частный случай —
   payload-less explicit-base enum → `typedef <c_type>` — это `Plan 105`, proposed.)

**Зачем:** перф (нет лишних аллокаций/копий — bench-цель §2 conventions, «быстрый
сгенерированный код») + меньше кода/багов (одно окно). **Граница:** соундность важнее — placement
выводится консервативно (любая неопределённость/рекурсия → heap, как Plan 127 over-promote).

**Cross-refs:** Q29 (no explicit `ref T`); D315 (ABI выводится); Plan 127 (escape→promote);
Plan 62.A.bis (NovaOpt stack / Result heap); Plan 128 (receiver ABI — раздельность под унификацию);
Plan 105 (explicit-base enum); D228/D277/D290 (value-record stack); 172.1 U.5/U.6 (единый носитель/repr).

---

## Q30. Computed properties: `@name()` getter / `mut @name(val)` setter

**Вопрос:** вводить ли механизм computed properties — методы с именем поля (`@name()` /
`mut @name(val)`), которые прозрачно вызываются при чтении/записи через dot-синтаксис?
...
- ✅ Нет конфликта с Plan 132: `obj.@method` удалён как hard error `E_BOUND_METHOD_REMOVED` —
  синтаксис свободен, старой семантики нет; новая семантика (property-call) ложится на чистое место
...
Вариант B наиболее Nova-идиоматичен: `@` уже означает «магия/оператор» в языке,
`obj.@name` не имеет старого значения (Plan 132 вычистил bound-value до hard error),
синтаксис свободен для новой семантики.
...
**Вопрос:** какие именно политические настройки даёт разработчику новая
модель «contracts enforced in release» (D24 amend, Plan 140), и каковы их
дефолты?

### Контекст (Plan 140, D24 amend, 2026-06-12)

Plan 140 заменил legacy-модель «недоказанные контракты стираются в release
(zero-cost, как `assert`/`NDEBUG`)» на **enforce-with-elision**:
- Z3-**доказанный** контракт элидируется на codegen (zero-cost);
- **недоказанный** — runtime-проверка остаётся и в release; нарушение →
  `nova_contract_violation` (fail-fast abort);
- снять проверку с недоказанного — явно: `#unchecked` (per-fn) или
  `--contracts=off` (build).

Семантика принята (D24 amend). Открытыми остаются три **политических**
параметра — детали, не дизайн; фиксируются по ходу Ф.1–Ф.5.

> **РЕШЕНО (Plan 140 Ф.2, 2026-06-12):** пункты 1 и 3 закрыты как
> реализованные. (1) Default build-policy = **`enforce`**; единственный
> полный build-opt-out — **`--contracts=off`** (`enforce|warn|off`
> трёхуровневый отвергнут — `warn` не убирает silent-UB-риск). (2) Violation
> в release = **abort** (реализовано Ф.1; panic-unwind — followup
> `[M-140-contract-panic-unwind]`). (3) Гранулярность = **per-fn
> `#unchecked` + build `--contracts=off`**; per-module — отложено
> (`[M-140-contract-levels]`).
>
> **ЗАКРЫТО (Plan 140 Ф.5, 2026-06-12):** Q34 closed. Все три политических
> параметра зафиксированы (выше); proven-элидирование измерено zero-cost
> (микробенч `perf_contract_hot_loop`: Z3-elide ≈ `--contracts=off` baseline;
> Trivial-checked ≈ +12% на contract-saturated loop — см. D24 amend Perf-блок).
> Plan 140 закрыт целиком (Ф.0–Ф.5, F1–F7 met). Остаточные followups
> (`[M-140-contract-panic-unwind]`, `[M-140-contract-levels]`,
> `[M-140-bounds-as-contract]`) — по запросу, не блокеры.

### Открытые места

1. **Default build-policy.** ✅ РЕШЕНО (Ф.2): **enforce-by-default + явный
   opt-out** (`--contracts=off` восстанавливает legacy zero-cost целиком).
   Альтернативы отвергнуты: opt-in (`--contracts=enforce` по требованию —
   возвращает дыру silent-UB по умолчанию); трёхуровневый
   `enforce|warn|off` (`warn` не снимает risk). `enforce` default; `off` —
   единственный полный opt-out на build-уровне.

2. **Поведение violation в release: abort vs panic-unwind.**
   - **abort** (предложение, дефолт): `nova_contract_violation` печатает
     структурное сообщение (PRE/POST, fn, src, file:line) и **немедленно
     завершает процесс** (fail-fast, не разматывая стек). Просто, надёжно
     под NDEBUG/LTO, не зависит от состояния fiber-runtime. Согласуется
     с D13 («нарушение контракта — это panic»), но в наиболее жёсткой
     форме (panic = abort, без unwind/`okrescue`).
   - **panic-unwind:** нарушение → обычный Nova-panic, ловится
     supervised-scope / `okrescue`. Гибче (graceful degrade сервиса), но
     дороже и тоньше под release/LTO; контракт-violation становится
     «recoverable», что размывает fail-fast-инвариант.
   - **Предложение:** в release **abort** (контракт-violation = баг
     программы, не recoverable ошибка); panic-unwind — возможный followup
     `[M-140-contract-panic-unwind]` если потребуется graceful degrade.
...
3. **Гранулярность opt-out.** ✅ РЕШЕНО (Ф.2): V1 = **per-fn + build**.
   ...
   - **per-module** `#unchecked_module` / манифест — **отложено**;
     followup `[M-140-contract-levels]` по запросу (Eiffel-style
     гранулярность pre/post/invariant — там же).

### Возможные направления / followups
- `[M-140-contract-panic-unwind]` ✅ **РЕШЕНО (Plan 140.3, 2026-06-14)** —
  переосмыслено: не «abort→unwind» (assert/контракт в файбере уже разматываются
  к fail-frame), а **унификация классификации**: `nova_assert_loc`/
  `nova_contract_violation` теперь тегают fail-frame `error_kind = NOVA_THROW_PANIC`
  (как `nv_panic`, D188) → пойманное consume/supervised классифицируется как **Panic**
  (не recoverable Failure), по spec D13. Commit `60e909a0`.
- `[M-140-contract-levels]` ✅ **РЕШЕНО (Plan 140.3, 2026-06-14)** — per-module
  opt-out (`#unchecked` перед `module X`, голое имя) + Eiffel per-kind
  `#unchecked(requires/ensures/invariant)` (fn И module). `ContractOptOut` +
  per-kind gate `contracts_elided_for(kind)`. Commit `77932ea3`.
- `[M-140-bounds-as-contract]` ✅ **РЕШЕНО (Plan 140.2)** — Vec `@index` OOB как
  `requires 0 <= i && i < @len` (D256/D257).

### Связь
- [D24](decisions/09-tooling.md#d24) — AMEND (Plan 140):
  enforce-with-elision; «стираются в release» retracted.
- [D106](decisions/09-tooling.md) — `verify_status`/`verify_mode`; proven
  feeds release-элидирование.
- [08-runtime.md → D13](decisions/08-runtime.md#d13) — нарушение контракта
  в runtime — это panic (abort-vs-unwind уточняется здесь).
- Plan 140 `docs/plans/140-contracts-enforced-in-release.md` — Ф.0–Ф.5,
  acceptance F1–F7, risk register.

---

## Q35. Сравнение с bool-операндом / chained comparison (`a <= b < c`) — ✅ ЗАКРЫТО (2026-06-13, Plan 150 / D248)

**Вопрос:** что Nova должна делать с выражением `a <= b < c` (chained
comparison) и со сравнением, у которого операнд — `bool`?

### Контекст (2026-06-12, всплыло при Plan 140 `[M-140-bounds-as-contract]`)

Сравнения лево-ассоциативны и возвращают `bool`. Поэтому `0 <= i < @len`
парсится как **`(0 <= i) < @len`** = `bool < @len`. Nova **молча принимает**
`bool < int` (coerce bool→0/1 + сравнение): результат всегда `true`
(`bool` ∈ {0,1} < @len при @len>1) — **вакуумно-истинно**, range-check НЕ
выполняется. Подтверждено: `assert(0 <= 100 < 10)` → PASS (хотя `100` вне `[0,10)`).

Тихий footgun. Особо опасен для контрактов: `requires 0 <= i < @len` стал бы
вакуумным контрактом = ноль безопасности (пишется естественно, выглядит
правильно, не проверяет ничего).

### Варианты

**(a) Hard error на bool-операнд сравнения** (`<`/`<=`/`>`/`>=`; `==`/`!=`
осмысленны для bool — не трогать) → `E_COMPARISON_BOOL_OPERAND` с fix-it-
подсказкой «возможно, имелось в виду `a <= b && b < c`?». Минимально, ловит
footgun, без нового синтаксиса. Прецедент: Rust/C++ (`a < b < c` = type error).

**(b) Полноценный chained comparison** (Python-стиль) — `a <= b < c` ≡
`a <= b && b < c`, средний операнд вычисляется ОДИН раз (нужен temp). Читаемо
для range-check, но: новый парсер-правило + взаимодействие с прочими
операторами. Прецедент: Python (единственный мейнстрим; C/Rust/Go/Java — нет).

**(c) Статус-кво** — `bool < int` валиден. Отвергается: тихий footgun без выгоды.
...
(`==`/`!=` на bool — легальны). Канонная форма диапазона = `&&`. Если в будущем будет спрос на
chaining — отдельное предложение (не сейчас).

**✅ ЗАКРЫТО 2026-06-13** — Plan 150 Ф.0-Ф.1 landed: [D248](decisions/02-types.md#d248-запрет-chained-comparison--relational-операнды-требуют-ordered-категорию-plan-150)
spec + parser (`parse_cmp`/`parse_eq`: ≥2 сравнений → `E_CMP_CHAIN_UNSUPPORTED`, paren-aware) +
checker (`bool`/`unit` relational-операнд → `E_RELATIONAL_OPERAND_NOT_ORDERED`). Полный check-sweep
2938 файлов = 0 ложных срабатываний; 8 негативных + 5 позитивных фикстур (`nova_tests/plan150/`).
...
Boxing-aware фикс size-walk'а ([D280](decisions/08-runtime.md#d280)) сделал ВАЛИДНЫЕ рекурсивные
heap-типы конечными (heap-ссылка = указатель) и убрал stack-overflow. Для **genuinely-infinite
value-self-cycle** — `type N value { next N }` (без указательной индирекции, не имеет конечного layout) —
depth-budget (`MAX_TYPE_SIZE_DEPTH = 128`) в `const_fn_eval` останавливает рекурсию и возвращает `None`
вместо краша. **No-crash гарантирован.**

### Решение (✅ РЕАЛИЗОВАНО 2026-06-15, коммиты `ae1e2906` + `3ffa7714`, ветка `fix-infinite-value-type`)

Добавлен **value-containment cycle-detector** в `compiler-codegen/src/types/mod.rs`,
функция `check_infinite_type`, вызываемая из `check_type_decl`. DFS по inline-containment-графу
(value Record / NamedTuple / Newtype / Alias / Tuple / FixedArray — INLINE; heap Sum / heap Record /
`*T` / `[]T` / str / Option / примитивы — STOP, разрывают цикл) с on-path visited-set; при back-edge
эмитит **`E_INFINITE_TYPE`** с ясным сообщением (имя типа + поле + подсказка: sum / Option / `*T` / `[]T`).
...
ленивом резолве произвольных prelude-методов. **2026-06-16: §2 (lazy-module-resolution) ПРОМОУТ в
[Plan 162](../docs/plans/162-rust-model-module-resolution.md) — см. Q-module-resolution-model.**
...
U.4.6 строит `resolved_type_to_c` (единый C-лоуэринг от канонического `ResolvedType`) как
**byte-identical зеркало** legacy `type_ref_to_c`. Это **ОСОЗНАННЫЙ MVP**, не утверждение, что legacy
«правильный». Причина byte-identical — разделение двух вещей, которые делает «принтер»:

- **C-имя (мангл)** — это **ABI-контракт**: один тип обязан манглиться в одно C-имя ВЕЗДЕ, иначе
  программа не слинкуется. Имя нельзя «улучшить» в одиночку — менять пришлось бы атомарно во всех
  местах. Поэтому при миграции сайтов (U.4.7/U.4.8) выход обязан совпадать до байта; byte-identical —
  это контракт безопасной миграции, а не «legacy верен».
- **Резолв (что это за тип)** — вот ЗДЕСЬ legacy неправильный (делает в codegen, дублируя чекер,
  §0-анти-паттерн). U.4 это **чинит** (резолв уезжает в чекер). То есть архитектуру делаем правильной,
  а вывод (имя) сохраняем.

Но в самом legacy-формировании C-имени есть **bootstrap-компромиссы / возможные неточности**, которые
MVP сознательно зеркалит как есть:
...
Когда `type_ref_to_c` **ретайрится** и `resolved_type_to_c` становится ЕДИНСТВЕННЫМ источником (U.4.8) —
пересмотреть на ОДНОЙ функции, где C-имя «не совсем верное», и перейти на **более правильное
формирование** (например, выбор erasure по структуре `ResolvedType`, а не по C-строке; не-erased
typed-storage для record-массивов). **Делать отдельными изолированными ревьюемыми коммитами ПОСЛЕ
объединения**, потому что:
1. смена мангл-имени = смена ABI → менять консистентно/атомарно, может затронуть рантайм-представление;
2. смешивать «объединить движки» с «изменить семантику» во время миграции = §7-анти-паттерн
   (непрослеживаемые регрессии — не понять, объединение сломало или «улучшение»).

Прецедент дисциплины: баг `uint = -1` (U.5.2) сохранили byte-identical при объединении, починили
**отдельным** коммитом `d58e0092`.

### Решение (для V1 / MVP)

U.4.6 — byte-identical зеркало legacy C-имени (**MVP**); резолв уезжает в чекер (архитектура чинится),
байты вывода сохраняются. Улучшение самого C-имени — **потенциальный следующий шаг ПОСЛЕ U.4.8** (один
источник), не во время объединения. Делать, если появится мотивирующая нужда (typed-storage массивов /
точная erasure) — каждый случай отдельным behavior-change коммитом с D-блоком.

### Связь
- [D315](decisions/02-types.md#d315-resolvedtype--единый-канонический-носитель-типа-plan-1721-2026-06-21) — `ResolvedType` единый носитель; ABI выводится лоуэрингом, не хранится.
- [Plan 172.1 U.4.6/U.4.7/U.4.8](../docs/plans/172.1-unified-type-engine.md) — построить `resolved_type_to_c` до паритета → мигрировать объявленные типы → удалить `type_ref_to_c`.
...
**НЕТ.** По [D84](decisions/10-overloading.md) перегрузки различаются **типами параметров / арностью / возвратом**; `consume` и `ref` — **режимы передачи, НЕ оси перегрузки**. Пробник `fn which(consume p Point)` + `fn which(p Point)` → `error: duplicate definition 'which' with same signature`.
...
5. **Противоречит принятому:** оптимизация временных **невидима/автоматическая** (RVO/NRVO + move-on-last-use, [Plan 172.4 Ф.6](../docs/plans/172.4-value-abi-auto-placement.md)), а НЕ выбирается перегрузкой; формализация `ref` ([D326](decisions/02-types.md#d326)) тоже не делает `ref` user-overload-осью.
...
### Связь
- [D84](decisions/10-overloading.md) — оси перегрузки (`consume`/`ref` не входят).
- D131/D133 ([02-types.md](decisions/02-types.md)) — `consume`-контракт (владение, affine).
- [D326](decisions/02-types.md#d326) — `ref` как режим, не тип, не overload-ось.
- [Plan 172.4 Ф.6](../docs/plans/172.4-value-abi-auto-placement.md) — RVO/copy-elision (невидимая оптимизация временных).
- [Q29](#q29-safe-references-ref-t--ref-mut-t--нужны-ли) — родственное (`ref` как тип отвергнут).
...
1. **`any` / fallible-`race` — варианты на падающих конкурентах.** Базовый `race[T](...funcs []fn() -> T) -> T` (первый ЗАВЕРШИВШИЙСЯ, чистый, канальный) записан в [173.1 §2a](../docs/plans/173.1-parallel-collect-and-supervised-value.md). Остаётся:
...
2. **Supervisor-strategy-набор для файберов — финализировать.** [173 §3b](../docs/plans/173-error-system-unify-harden.md) зафиксировал **направление**: default `cancel`/`Escalate`/`Stop`/aggregate; **Erlang-restart — анти-паттерн на shared-memory** (нет «чистого сброса»). Остаётся:
   - точный API/синтаксис `Stop`/`Escalate` (per-fiber решение vs scope-policy);
   - **restart ТОЛЬКО для ИЗОЛИРОВАННЫХ** файберов (opt-in): как выражается изоляция (рычаг [173.3](../docs/plans/173.3-data-race-freedom-share.md) `#share`/consume-в-spawn), какие стратегии (`OneForOne`/`OneForAll`/`RestForOne`) сохраняются и только для изолированных;
...
воспроизводимая сборка вне машины автора невозможна.

Предложенная схема (владелец одобрил направление 2026-07-13; школа Go/SwiftPM —
без собственного реестра):

1. **Релизная форма:** `tls = { git = "https://github.com/nv-lang/nova-tls", version = "0.1" }` —
   версия резолвится по git-тегам `v0.1.x`, семвер-совместимость (caret-семантика по умолчанию).
2. **`path =` — только dev-режим.** Отдельный `[replace]`-блок манифеста
   (`replace tls = { path = "../nova-tls" }`) отделяет «что требуется» от «где взять сейчас»
   (go-школа); голый path-dep без replace — предупреждение/ошибка при публикуемой сборке.
3. **`nova.lock`** рядом с манифестом: точный тег + commit-хэш каждой зависимости; коммитится;
   расхождение манифеста и lock — ошибка с подсказкой обновления.
4. Реестр пакетов — потом; схема перерастает в него без ломки (имя и семвер уже есть).

Открыто: (а) caret против точного диапазона по умолчанию; (б) формат/имя lock-файла и команда
обновления (`nova update`?); (в) транзитивные зависимости и разрешение конфликтов версий
(MVS по-гошному против semver-unify по-каргошному); (г) checksum-верификация (аналог go.sum).

Реализация — Plan 204 (манифест-резолвер + lock); D-блок в 07-modules/09-tooling — в том же
слиянии, что и код (правило язык-меняющих слияний).
...
## Q-format-spec-to-display — передача формат-спека в @display/@debug (pretty и др.) + движок интерполяции без промежуточных аллокаций — ✅ RESOLVED → [D419](decisions/02-types.md#d419-Fmt-protocol--format-spec-контекст-для-display_fmt-plan-1527-2-2026-07-13) (Plan 152.7.2, 2026-07-13), свёрнут в [D422](decisions/02-types.md#d422) (Plan 208, 2026-07-15)
...
компилятор применяет СНАРУЖИ к выводу display; пользовательский тип спек не видит
(`E_FORMAT_SPEC_UNKNOWN` на всё кроме `:?`); pretty — отдельными методами (`to_str_pretty`);
маркер `[M-91.14-format-dsl-extensions]` протух (перечисляет уже реализованное).

Предлагаемое направление (Rust-модель — Formatter несёт разобранный спек):

1. **Контекст вместо второго параметра — РЕШЕНО, и НЕ так, как предлагалось здесь.**
   Итог — [D422](decisions/02-types.md#d422) (Plan 208, 2026-07-15, keystone):
   ОДИН метод `@display(mut f Fmt)`, без пары `@display` + `@display_fmt` и без
   сохранения `@display(mut w Write)` вторым каноном.

   > **Предлагалось (снято D422):** «протокол-обёртка `Fmt` над `Write`
   > (`@width() -> Option[int]`, `@precision() -> Option[int]`,
   > `@alternate() -> bool`, ...); `@display(mut w Write)` остаётся канонном для
   > типов, которым спек безразличен; тип, желающий реагировать, реализует
   > `@display_fmt(mut f Fmt)` (или Fmt подтипом Write — решить: перегрузка против
   > второго протокола против расширения сигнатуры breaking-волной)».
   >
   > Снято 2026-09-18. **Почему это был не просто устаревший текст:** заголовок
   > вопроса указывал на D419, который D422 свернул, — а «предлагаемое направление»
   > ниже продолжало жить и называло `@display(mut w Write)` каноном. Открытый
   > вопрос, который уже отвечен, возвращает снятую форму: он читается как живое
   > направление. Развилка, названная в снятом тексте («перегрузка против второго
   > протокола против расширения сигнатуры»), закрыта ТРЕТЬИМ вариантом — единой
   > сигнатурой, — и без этой записи следующий читатель открыл бы её заново.
2. **`#` (alternate) = каноничный pretty-флаг**: `${cfg:#}` → многострочный вывод;
   `JsonValue.to_str_pretty` становится `"${v:#}"`-совместимым (метод остаётся как API).
3. **Разделение обязанностей:** width/align/fill ОСТАЮТСЯ пост-обработкой у вызывающего
   (тип не должен уметь паддить); типу передаются только семантические оси:
   alternate/pretty, precision, radix-желание (числовые типы).
4. **Движок интерполяции — прямо-в-sink:** `${x}` для ЛЮБОГО типа лоуэрится в
   `@display(mut sb)` без промежуточной str; `str.from(v)` перестаёт быть движком
   (остаётся публичной конверсией там, где уместна по 174.1-канону, и уходит из
   интерполяционного пути) — снимает аллокацию на фрагмент, о которой владелец.
5. Обновить/закрыть протухший `[M-91.14-format-dsl-extensions]` этим решением.

Открыто: (а) форма контекста (Fmt-протокол против расширения Write против второго метода);
(б) breaking-объём (все @display переломлены D374 недавно — вторую ломку подряд не хочется,
поэтому «второй опциональный метод» выглядит дешевле); (в) какие оси передавать типу.
...
## Q39. Таймаут как ЗНАЧЕНИЕ: именованный обработчик у `supervised`

**ЗАКРЫТ 2026-08-09 → [D449](decisions/06-concurrency.md).** Владелец выбрал форму `on_timeout: |_| Err(ReadFailed)` (обработчик, а не готовое значение ошибки) и поправил интегратора: спека и D пишутся ДО реализации, а не вместе с кодом. Формулировка `CLAUDE.md` «не пушится без D-амендмента в том же слиянии» задаёт КРАЙНИЙ СРОК, а не «одновременно»; интегратор прочитал её как запрет писать раньше. Открытые части вопроса перенесены в раздел «Открыто» самого D449.

**Поставлен владельцем 2026-08-09.** Исходно предложенная им форма:

```nova
ro res Result[HeadersRead, HeaderReadError] = supervised(
        timeout: 10.to_millis(), on_timeout_err: ReadFailed) {
    read_headers(client, max_bytes)
}
```

**Контекст.** Сегодня по [D408](decisions/06-concurrency.md) таймаут приходит
эффектом `Fail[TimeoutError]`, и превратить его в значение можно только обёрткой
снаружи:

```nova
with Fail[TimeoutError] = |_e| Err(ReadFailed) {
    supervised(timeout: 10.to_millis()) { read_headers(client, max_bytes) }
}
```

Неудобство не в длине, а в **разнесённости**: «сколько ждём» стоит в
`supervised`, «что делать, если не дождались» — уровнем выше, в обёртке.

**Что мешает варианту `on_timeout_err:` (значение ошибки).** Конструкция начнёт
менять свой тип в зависимости от набора аргументов: без него `supervised { … }`
даёт тип тела, с ним — `Result[тело, E]`. Такую неоднородность трудно объяснить
и легко забыть. Плюс форма требует, чтобы тело **уже** возвращало `Result`, —
иначе некуда вкладывать `Err`.

**Предложение интегратора — передавать не значение, а обработчик:**
...
* тип конструкции **всегда равен типу тела**; обработчик обязан вернуть то же
  самое, и это проверяет компилятор;
* работает с любым телом, не только с `Result`: тело даёт `int` — обработчик
  даёт `int`;
* обработчик получает сам `TimeoutError` с `deadline_ns` — можно записать в
  журнал, отличить срок от отмены, посчитать меру. При передаче готового
  значения эта информация теряется;
* форма владельца становится частным случаем: `on_timeout: |_| Err(ReadFailed)`;
* аргумент **необязателен**: без него поведение прежнее — `Fail[TimeoutError]`
  летит наружу и ловится где угодно выше.

Это развилка Kotlin (`withTimeout` бросает / `withTimeoutOrNull` возвращает), но
**одной конструкцией вместо двух имён** — ближе к тому, как `supervised` устроен
сейчас (набор именованных аргументов `cancel:` / `deadline:` / `timeout:`).

**Прецеденты в языке:** [D43](decisions/03-syntax.md#d43) (функциональный
параметр выносится за скобки вызова), [D102](decisions/03-syntax.md) (именованные
аргументы), `with Fail[E] = |e| …` (обработчик как значение — уже есть).

**Не решено:**

* нужен ли отдельный `on_cancel:` для внешней отмены (`cancel:`), или отмена и
  срок должны разделять один обработчик;
* что делать, если обработчик сам падает или сам не укладывается в срок;
* совместимость с `deadline:` — обработчик один на область или по одному на
  каждый вид завершения.

**Срок.** К релизу v0.1 не требуется: текущая форма работает, вопрос про
удобство, а не про возможность. Смежное — №457 (реестр 221.1)
(`supervised(timeout:)` не достаёт до `.read()`): пока корень не починен,
обсуждать форму преждевременно — она не изменит того, что механизм не срабатывает.

---

## Q40. Анонимный именованный кортеж прямо в типе поля

**Пожелание владельца 2026-08-09**, дословно: «`entries [](key: K, value: V)` — для
лучшей читаемости кода»; затем уточнение формы: «пожелания `(key K, value V)` без
заведения нового типа».

**Сегодня так нельзя.** Проверено пробами на свежем компиляторе: обе записи —
`[](key: K, value: V)` и `[](key K, value V)` — отвергаются (`expected identifier`).
[D52](decisions/02-types.md) даёт именованный кортёж **только как объявление
типа**: `type X(name1 T1, name2 T2)`. Анонимной формы в позиции типа поля нет.

**Что это дало бы.** Поле читается само по себе:

```nova
entries [](key K, value V)     // видно, что где
entries [](K, V)               // сегодня: .0 и .1 по всему модулю
```

Обходной путь — завести именованный тип (`type IndexEntry[K, V](key K, value V)`)
— владелец его тоже одобрил, но он требует объявления рядом и не годится там, где
пара нужна разово.

**Не решено:**

* совместимость с позиционным доступом: остаются ли `.0`/`.1` наравне с `.key`?
* выводится ли тип при передаче позиционного литерала `(a, b)` в такую позицию;
* нужна ли форма в возвращаемом типе (`-> (key K, value V)`) — там та же
  читаемость и та же нехватка.

**Срок.** К релизу v0.1 не требуется: обходной путь есть и одобрен. Смежное —
№502 (реестр 221.1): обходной путь СЕГОДНЯ тоже не работает,
обобщённый именованный кортёж ломает кодоген. Пока это не починено, оба варианта
одинаково недоступны.
