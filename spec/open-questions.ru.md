...
1. **Контекст вместо второго параметра:** протокол-обёртка `Fmt` над `Write`
   (`@width() -> Option[int]`, `@precision() -> Option[int]`, `@alternate() -> bool`, ...);
   `@display(mut w Write)` остаётся канонном для типов, которым спек безразличен;
   тип, желающий реагировать, реализует `@display_fmt(mut f Fmt)` (или Fmt подтипом Write —
   решить: перегрузка против второго протокола против расширения сигнатуры breaking-волной).
