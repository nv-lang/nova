...
6. **Структурированная конкурентность поверх M:N-планировщика (кодовое имя рантайма —
   ...
   **Возврат КОРТЕЖЕМ — тоже потребление** (амендмент
   [D133](decisions/02-types.md#d133) от 2026-09-14, реестр 221.1 №1092):
   `fn f(consume a T, consume b T) -> (T, T) => (a, b)` отдаёт caller'у оба
   владения, как `return a` отдаёт одно. До амендмента перечень мест
   потребления кортежа не упоминал, и такая функция отвергалась как «не
   consumed до scope-exit» — при том что её тело ЕСТЬ весь её результат;
   в std это два месяца обходили, помечая результат `ro`, то есть ложью о
   владении в подписи (`split_pair`, `net/tcp.nv`).
