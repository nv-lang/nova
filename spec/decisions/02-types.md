...
* **`cap` в нём значит не то, что в коде.** У `HashMap` `@cap()` сегодня —
  геттер (`-> int`), а сеттер `with_capacity` ретрактирован (пометка в самом
  файле). Атрибут проверял наличие имени, а не смысла.
