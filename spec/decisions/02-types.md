...
объявляет неявную коэрсию `I → O`. Две эквивалентные формы записи (семантика ОДНА; направление
— где автору удобнее):
...
#coerce fn Matrix.new(v []f64) -> ro Self => ...           // []f64 → ro Matrix  (view)
```

### Правила (нормативные)

- **R1 Унарность.** Ровно один входной тип: метод-без-параметров ИЛИ статик-с-одним-параметром.
  Иначе `E_COERCE_NOT_UNARY`.
...
- **R4 Один уровень.** Цепочки не разворачиваются: `I→O` и `O→P` объявлены — `I` в
  `P`-позиции НЕ коэрсится (паритет single-wrapper «ровно один уровень»).
...
- **R8 Охват — все типы, включая пользовательские, с первого дня** (решение владельца
  ...
  предотвращает конфликт деклараций.
...
- **R10 `as` НЕ вызывает `#coerce`** (вопрос владельца 2026-07-18, закреплено). `as` —
  repr-каст (численное сужение/расширение D54, newtype-relabel D52, ptr-каст в unsafe):
  НИКОГДА не исполняет код. `#coerce` вставляет ВЫЗОВ функции (view-заголовок/финализация) —
  другой класс; смешение сделало бы `as` небесплатным (C++-ловушка conversion-operator) и
  дало бы ТРЕТЬЮ дверь к паре (голое значение / именованный метод / as) — D9. Явная форма
