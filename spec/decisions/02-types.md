...
- **Композиция вместо копирования** (D310 amendment) — `Ints` = `SignedInts | UnsignedInt`, будущий набор = композиция готовых, а не переписанный вручную список; синтаксическая экономия БЕЗ ослабления identity-модели (структурного совпадения по-прежнему нет).
