...
- **Поля с умолчанием идут ПОСЛЕ полей без умолчания** — см. [D482](#d482-ex-d478-renumber-2026-09-20-умолчания-только-в-хвосте-решение-владельца-2026-09-14).
  До 2026-09-14 здесь стояло ОБРАТНОЕ: «Порядок полей в declaration не ограничен:
  required и optional могут чередоваться (хотя рекомендуется ставить optional в конце)» —
  рекомендация без запрета. D482 это разрешение ОТМЕНЯЕТ.
...
## D475. Амендмент к D422 §5: цифры float производит Nova, C-дверь форматтера закрывается (план 285, 2026-09-13)

**Что говорил D422 §5.** «`float` — ЕДИНСТВЕННЫЙ C-extern, dtoa непортируем». Всё
остальное форматирование — целые, `bool`, `char`, радикс, дополнение — ушло в `.nv`, а печать
чисел с плавающей точкой осталась за `snprintf` и `strtod`.

**Что меняется.** Утверждение «непортируем» больше не верно и снимается. Кратчайшая
round-trip-запись (`FloatKind.Shortest`) для `f64` и `f32` производится кодом на Nova
(`std/src/runtime/fmt_buf/shortest.nv`, порт Ryu), а точная десятичная арифметика живёт в
`runtime.decimal` и обслуживает ОБЕ стороны преобразования. C остаётся у `Fixed` и `Sci` до
завершения плана 285 Ф.4.

**Почему это АМЕНДМЕНТ, а не новое решение.** Поверхность не меняется ни на байт:
`f64_fmt`/`f32_fmt`, `FloatKind`, мини-язык `${x:.2}` и протокол `Fmt` остаются теми же.
Меняется ТОЛЬКО то, кто производит цифры за ней.

**ОДНО НАБЛЮДАЕМОЕ РАСХОЖДЕНИЕ, и оно НЕ регрессия.** В субнормальной ленте
старый C-хелпер печатал НЕ кратчайшее: `4.94066e-324` там, где кратчайшая запись есть
`5e-324` (замер плана 285: 0 расхождений на 200 000 случайных `f64`, 3871 из 4000 в нижней
субнормальной ленте; у `f32` — 4 из 59 786, все субнормальные). Причина: хелпер берёт
ПЕРВУЮ точность, что читается обратно, начиная с `%g` — шести значащих цифр ТОЧНОГО
двоичного значения, — а у субнормали с малой мантиссой двоичная величина отстоит от
короткой десятичной на 1.2 %, и хвостовые нули не появляются. **`Shortest` есть ИМЯ
ВИДА, то есть контракт**, и функция, печатавшая там шестизначное, его не исполняла.
Новый производитель не отступает от поверхности — он начинает её исполнять.

**Что ЕЩЁ НЕ ЗАКРЫТО на момент амендмента, чтобы читатель не счёл дело сделанным:**
у печати ДВЕ двери. `print(x)` для float идёт мимо `f64_fmt` — кодоген выпускает
`nova_print_f64`, зовущий C-хелпер напрямую (реестр 221.1 №1072). До его закрытия
`println(x)` и `println("${x}")` могут разойтись на субнормалях — расхождение, которое план 180
однажды уже закрывал.
...
## D482 (ex-D478, renumber 2026-09-20). Умолчания только в хвосте (решение владельца 2026-09-14)

**Статус:** ПРИНЯТО 2026-09-14 (решение владельца в разговоре; номер назначен
интегратором). **Реализации НЕТ ни у одного компилятора:** сегодня обе формы
принимаются молча.

**Отменяет:** правило о порядке полей в амендменте
[D215](#d215-amend--named-tuple-field-defaults-2026-06-17) (раздел «Семантика», до этого блока — строки
13780–13781). Это именно **отмена явного разрешения**, а не заполнение пробела:
спека о порядке не молчала, она говорила обратное.

### Что

Поле или параметр со значением по умолчанию стоит только ПОСЛЕ всех
обязательных — одинаково у функций, методов и именованных кортежей.

```nova
type GoodOrder(a int, b int = 3)      // OK
type BadOrder(a int = 7, b int)       // E_TUPLE_DEFAULT_BEFORE_REQUIRED

fn f(x int, y int = 0)                // OK
fn g(x int = 0, y int)                // compile error (D102 «Правило — объявление» п.1)
```

Для функций правило уже нормативно с самого
[D102](03-syntax.md#d102-именованные-аргументы-и-значения-параметров-по-умолчанию):
«Параметры с дефолтом идут после параметров без дефолта. `fn f(x int = 0, y int)`
— compile error». D482 распространяет его на поля именованного кортежа, где амендмент
D215 разрешал чередование.

Диагностика — `E_TUPLE_DEFAULT_BEFORE_REQUIRED`. Негативную фикстуру пишет тот, кто
чинит, по общему правилу (новый `E_*` требует фикстуры с line-pinned `nova:expect`).

### Почему

**1. Это не новое ограничение, а инвариант, на который проверка уже молча
рассчитывала.** Проверка арности амендмента D215 судит КОЛИЧЕСТВО аргументов:
`min_arity` — число обязательных полей, `max_arity` — всего полей, выход за окно —
`E_TUPLE_CONSTRUCT_ARITY_MISMATCH`. Счёт отличает связанные поля от несвязанных только
пока умолчания идут хвостом. При чередовании вызов `BadOrder(5)` даёт
`1 ∈ [min_arity=1, max_arity=2]` — счёт сходится, позиционный аргумент уходит в поле
С УМОЛЧАНИЕМ, а ОБЯЗАТЕЛЬНОЕ остаётся несвязанным и молча зануляется
(`BadOrder(5)` → `a=5, b=0`; реестр 221.1 №1096).

**2. Одно правило на все три формы.** Разный порядок у функций и у кортежей
пришлось бы запоминать отдельно, а причина запрета у них общая.

### Цена отмены — ноль, и вот как это измерено

На день принятия чередованием не пользовалось **ни одно из 141** скобочных
объявлений в `novac/src`, `std/src`, `spec_tests`, `examples` (29 из них несут
умолчания). То есть отмена не ломает ничего из написанного.

**История мерки приведена намеренно**, потому что число «ноль» дорого стоит
ровно в той мере, в какой известно, что считали весь предмет. Мерка врала
трижды и каждый раз МОЛЧА:

1. `)` внутри комментария обрубал тело объявления — одно ложное попадание
   (`type FloatSpec(...)`);
2. имя типа искалось глобально, и вызовы одного `Pair` приписывались объявлению
   другого `Pair` из чужого модуля;
3. якорь `^type` не видел `export type` — а именно так объявлен
   `std/src/math/complex.nv:50`.

Прежние числа этой же мерки — 97 и 112 — были взяты по НЕПОЛНОЙ выборке и
сняты. Замечание общего свойства: первоначальное утверждение «спека о порядке
МОЛЧИТ» тоже было результатом односоставного поиска: искали РУССКИЕ слова
(«умолч», «порядок»), а абзац говорит `required`/`optional` по-английски.
**Утверждение об ОТСУТСТВИИ правила нельзя доказать поиском по ОДНОМУ
написанию предмета.**

### Область

Правило нормативно для обоих компиляторов. Порядок работ назначает
интегратор: ни одно объявление в деревьях правила не нарушает, поэтому
отсутствие проверки ничего не ломает сегодня и не требует срочной работы.

### Чего этот блок НЕ решает

Отдельный вопрос — можно ли передавать поле С УМОЛЧАНИЕМ ПОЗИЦИОННО.
Здесь расходятся **ТРИ** стороны, и вопрос ждёт решения владельца:

* D102 «Правило — вызов» п.1: параметр с дефолтом — keyword-only, позиционно —
  compile error; и этот файл (амендмент D215) прямо называет D102 адресом
  конструирования именованного кортежа;
* `std` делает обратное в 126 вызовах из 14 файлов (`std/src/math/int128.nv` — 18,
  `complex_test.nv` — 22, `float_parse/convert.nv` — 12);
* в корпусе лежит **ЗЕЛЁНАЯ** фикстура
  `spec_tests/conformance/m_ice_named_tuple_positional_defaults_pos.nv`, пинающая
  позиционную передачу как «verified-non-bug».

Третья сторона важнее первых двух: кто-то уже принял решение и закрепил его
фикстурой, не изменив спеку.

### Связь

* [D102](03-syntax.md#d102-именованные-аргументы-и-значения-параметров-по-умолчанию)
  — то же правило для функций, источник формулировки.
* [D52](#d52-объявление-типов-revised-newtype-alias-sum-через-leading-)
  — «Construction всегда требует все обязательные поля»: правило вызова, которое
  здесь нарушается молча.
* Реестр [221.1](../../docs/plans/221.1-bug-sweep.md) №1096 — дефект, на котором это
  вышло наружу, и обе его двери.
