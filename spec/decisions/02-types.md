...
> ⚠️ **D72 §«Порядок» — ПРАВИЛО НЕ ДЕЙСТВУЕТ, ТРЕБУЕТСЯ РЕШЕНИЕ ВЛАДЕЛЬЦА (аудит спеки
> 2026-08-16, подтверждено 3/3 скептиками; реестр №702).** Три факта: (1) чекер порядок
> НЕ проверяет — `check_generic_bound_declarations` (`types/mod.rs:26054`) валидирует
> только имена бáундов; (2) std САМ пишет forward-order и компилируется:
> `export type ArrayGen[G Generator[T], T]`, `fn shrink_loop[G Generator[T], T]`
> (`std/src/testing/property.nv:224,304`); (3) D145 §«Bound syntax (через D72)» даёт
> `fn[T From[K], K] T @construct_from(v K)` как валидный пример — по букве этого §
> это ОШИБКА. Обоснование «ради простоты type-checker'а» истекло: чекер без правила
> справляется. **Два пути, выбор за владельцем:** (а) реализовать правило — сломает
> std и D145; (б) снять правило: имена одного generic-списка видны друг другу
> целиком (как rustc: `fn f<T: From<K>, K>` законно). **Рекомендация интегратора —
> (б):** правило никогда не действовало, его отмена не меняет ни одной компилирующейся
> программы; D88 (forward-ref в ДЕФОЛТАХ `[T = X]`) не затрагивается — там порядок
> нужен для вычисления значения, здесь — нет. До решения пример D145 оставлен как
> есть (он отражает поведение оракула), а этот § помечен, не удалён.
