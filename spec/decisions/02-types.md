...
**Особый случай: pointer-returns (D216 amend, Plan 138.5).** Для
возвращаемого **указателя** return-mut-default относится к **pointee**
(target), а не к самому указателю — у указательного типа нет «outer
pointer-mut» (retracted, [D216 §V2.6](#d216-v2-amend-2026-06-04--universal-right-binding-rule-для-type-level-modifiers--unsafe-t-first-class)).

| Return type | Pointee (target) | Перепривязываемость результата |
|---|---|---|
