...
(четыре раунда решений). **Status:** 🟢 ДИЗАЙН ЗАКРЕПЛЁН (sign-off владельца 2026-07-06);
реализация — Ф.0 (эта спека) → Ф.1 (парсер) → Ф.2 (чекер) → Ф.3 (кодоген) → Ф.4 (миграция/тесты).
...
**(Р1) Позиции `ref T`.** Легален: параметр (представление, выбирает компилятор),
возврат (`-> ref Self` — де-сахар `-> @` для value-типов), локал-алиас
(`ro y ref T = x.a.b` — именованная ссылка на место). **ЗАПРЕЩЁН** в: полях record/value,
коллекциях (`Vec[ref T]`), суммах (`enum`), `Option[ref T]`. Диагностика в запрещённой
позиции — `E_REF_TYPE_POSITION` (поле/коллекция/сумма/Option). Причина запрета: ссылка на
стек-сторадж не должна попасть в долгоживущую кучу (без этого нужны лайфтаймы).
...
поднимается в кучу) + запрет захвата ссылок замыканиями/spawn/parallel
(`E_REF_ESCAPE_CAPTURE`, как в исходном D326-R10). Только синхронный lifetime вызова.
