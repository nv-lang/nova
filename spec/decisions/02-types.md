...
## D249. Vec `@index` bounds как элидируемый контракт (Plan 140.2)

> **Status:** 🆕 SPEC LANDED 2026-06-13 (Plan 140.2). Закрывает `[M-140-bounds-as-contract]`. Зависит от
> [Plan 140](../../docs/plans/140-contracts-enforced-in-release.md) (D24 enforce-with-elision) +
> [D248](#d248-запрет-chained-comparison--relational-операнды-требуют-ordered-категорию-plan-150) (`&&`-форма).

### Что

`Vec[T] @index(i int)` (read, powers `v[i]`) и `Vec[T] mut @index(i int, val T)` (write, powers
`v[i] = val`) несут контракт **`requires 0 <= i && i < @len`**. Bounds — НЕ ручная always-on проверка, а
**элидируемый контракт** (D24 / Plan 140): Z3-доказуемый индекс (напр. в `for i in 0..v.len()`) → проверка
**элидируется** (zero-cost); недоказуемый → runtime check и в release (abort) — Rust-style bounds check.

### Семантика

- OOB (`i < 0` или `i >= @len`) → **abort** (contract violation, формат D140.1; включает index/len).
  Раньше — `panic("Vec: index out of bounds")`; теперь — contract-violation abort (оба прерывают).
- `@get(i)` / `@first` / `@last` (Option-возврат) — **БЕЗ** контракта (graceful `None` на OOB) — не меняются.
- Range `@index(r Range)` (`v[a..b]`) — собственный slice-bounds check (inline), вне MVP D249.

### Реализация

`v[i]` / `v[i] = val` направлены через монолизированный `@index` / `mut @index` (вместо прежнего inline
`nv_panic_index_oob`); контракт проверяется/элидируется на call-site через Plan 140 (`proven_contracts`).
C-инлайнер убирает оверхед вызова крошечного метода.

### Связь

- [D24](09-tooling.md#d24) / Plan 140 — enforce-with-elision (proven → zero-cost, unproven → release check).
- [D248](#d248-запрет-chained-comparison--relational-операнды-требуют-ordered-категорию-plan-150) / Plan 150 —
  `&&`-форма (вакуумная `0<=i<@len` — compile-error `E_CMP_CHAIN_UNSUPPORTED`).
- D238 / D240 — `Index` / `MutIndex` протоколы (`@index` magic).
