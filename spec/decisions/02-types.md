...
| `fn f(x T)` — на caller-стороне (`f(value)`) | да | ⛔ ещё нет |
...
> record-coercion для безымянного литерала `{ ... }` реально работает в
> позициях, помеченных ✅. Для ⛔-позиций безымянный `{ ... }` пока даёт
> codegen-ошибку — там пиши `T { ... }`. Полная реализация D55 во всех
> позициях — отдельная задача (investigation в Plan 51 показал, что
> «~900 избыточных мест» — переоценка; основная масса — это перенос
> имени, а не устранение). Sum-coercion по позициям не аудирован
> отдельно.
...
4. **Десугаринг — без промежуточных объектов:** block-expression с
   ...
   ```nova
   { let mut _m = HashMap[str, V].with_capacity(n)
     _m.@insert("debug", true); _m.@insert("verbose", false); _m }
   ```
5. **Пустой `{}`** в map-позиции → `HashMap[str, V].new()`.
