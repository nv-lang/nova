...
В позиции с явным ожидаемым типом `[]u8` **str-литерал** (не переменная, не произвольное
...
| Тип возвращаемого значения | `-> ro T` / `-> mut T` / **`-> consume T`** | возвращаемое значение |
| Тело типа | `type X { … }` / `type X value { … }` / **`type X consume { … }`** / `type X enum A \| B` | сам тип (тот же слот, что `value`/`enum`) |

```nova
// Возврат — канон 2026-08-03: consume ПЕРЕД типом, симметрично ro/mut.
fn Mutex @lock() -> consume MutexGuard
fn RwLock @read() -> consume ReadGuard
fn RwLock @write() -> consume WriteGuard
```
...
Парсер: return-позиция (`-> …`) теперь ест опциональный `consume` СРАЗУ
после `->`, до `parse_type()` — симметрично уже существовавшей обработке
`const`/`@` в той же ветке. Старая постфиксная форма (`consume` СРАЗУ
после типа возврата) — теперь `E_RETURN_CONSUME_POSTFIX_RETRACTED` (hard
error с fix-it на новую форму), не тихий проход.

### Связь
- [D176](#d176-ro-t--тип-модификатор) — `-> ro T`/`-> mut T` в позиции
  возврата (симметрия, которую этот блок распространяет на `consume`)
- [D133](#d133-type-x-consume--обязательная-consume-семантика-must-be-consumed) — `consume`-семантика типа/значения
...
2. **`Fail[E]` очистки обязан появиться в сигнатуре** объемлющей функции.
   ...
   (обзор `spec/effects.md`, раздел «Прямые эффекты, не транзитивные»;
...
- **Чекер** (`compiler-codegen/src/types/mod.rs`): `LinearityRegistry.
  ...
  (молча, per §8а п.5 — диагностика use-after остаётся точной, снабжена
  точкой вставки через существующий span-механизм).
...
`SignedInt ∪ UnsignedInt`), было легально через exemption ([D423](04-effects.md#d423)
...
наборе теперь можно как частично, так и полно; `SignedInt`/`UnsignedInt` остаются в
