...
Прежний контракт требовал **трёх** методов — `new`, `cap`, `insert_new`, — и
проверялся в `types/mod.rs` условием `has_new && has_cap && has_insert_new`.
Он отменяется целиком, по трём причинам:

* **Три требования там, где хватает одного.** Сеттер существует —
  `export fn HashMap[K, V] mut @cap(n int) -> @` (перегрузка по числу
  аргументов: `@cap()` читает, `@cap(n)` устанавливает), и контракт имел в виду
  именно его. Но конструктор с вместимостью делает то же самое одним вызовом,
  а проверка «есть ли метод по имени `cap`» не отличает сеттер от геттера:
  тип с одним лишь `@cap() -> int` проходил её так же.

  > **Поправка 2026-08-09.** Первая редакция этого пункта утверждала, что «`cap`
  > значит не то, что в коде» и что сеттер ретрактирован. Неверно: ретрактирован
  > был `with_capacity`, а `mut @cap(n)` его заменил и жив. Ошибка интегратора,
  > снята владельцем; решение от неё не зависит — довод в числе требований, а не
  > в отсутствии метода.
* **`insert_new` — внутренняя оптимизация**, а не часть публичного договора:
  требовать её от типа значит требовать деталь реализации.
* **`new(cap:)` уже канон.** `HashMap[K, V].new(cap int = 16)` существует и
  вызывается как `Self.new(cap: pairs.len() * 2)` — то есть концепция принята и
  используется, а проверка атрибута от неё отстала.
...
`IndexMap` получает `new(cap int = …)` и обе пометки; `HashMap` уже
соответствует и правки не требует — из его контракта уходят лишь `cap`/
`insert_new` как ТРЕБОВАНИЯ атрибута (сами методы остаются, они полезны сами по
себе).
