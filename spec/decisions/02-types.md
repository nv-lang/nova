...
- **Многострочный** — если варианты на новых строках, `|` **обязателен**
  у каждого варианта (включая первый). Это отличает `enum`/sum от `set`,
  где первый член идёт без `|` (D310):
...
// многострочный — первый член БЕЗ |, последующие через |
type AnyNumber set
    i8 | i16 | i32 | i64 | int
    | u8 | u16 | u32 | u64 | uint
