...
### Амендмент (2026-09-04, решение владельца): операторы над числовым newtype — только внутри одного newtype

**Что обнаружено** (исследование `docs/dev/research/2026-09-04-typed-index-vec.md`, реестр
строка «операторы стирают newtype», проба `docs/plans/repro/904-newtype-operators-erase/`).
Newtype объявлен «типизированно отличным от Y, Go-style» (§Что), и вызовы это соблюдают:
`fn f(r FnRow)` отвергает аргумент `TyId` (`E7301`). Но бинарные операторы — нет:
`FnRow == TyId`, `FnRow < TyId`, `FnRow + TyId` и `FnRow + n` при `n int` проходят чекер,
собираются и исполняются (печатают `true false 2 true 2`). Различие newtype компилятор
знает и применяет на одной двери (вызов), а на другой (оператор) стирает. Ни этот блок, ни
D46 (операторы через `@`-методы) случай не описывали.

**Норма.** Для newtype `X` над числовым представлением `Y`:

1. Бинарные операторы арифметики (`+ - * / %`) и сравнения (`== != < <= > >=`) определены
   между операндами **одного и того же newtype `X`**. Результат арифметики — `X`;
   результат сравнения — `bool`.
2. Числовой **литерал** в позиции операнда адаптируется к `X` (как и при присваивании,
   Plan 200): `a + 1` при `a FnRow` законно, результат `FnRow`.
3. Смешение операндов — `X` с другим newtype `Z`, `X` с **значением** типа `Y` (переменная,
   поле, результат вызова) — **compile error**; диагностика называет явный `as` как путь:
   `a + (n as FnRow)` или `(a as int) + n`, по смыслу места.
4. Унарные операторы и составные присваивания (`-a`, `a += b`) — по тем же правилам.

Это ровно семантика донора: в Go `type FnRow int` даёт `FnRow == TyId` → *mismatched
types*, `FnRow + 1` законно (untyped constant), `FnRow + n` при `n int` → ошибка.

**Почему (а), а не «стирание намеренно».** Смысл newtype над числом — чтобы два
пространства номеров нельзя было спутать; самое частое место такой путаницы — сравнение
`if row_id == ty_id`. Стирание в операторах оставляло бы гарантию newtype дырявой ровно там,
где она нужнее всего, и обесценивало бы типизированные индексы (`IndexVec[I, T]`, то же
исследование): защита на `[]` и вызовах ничего не стоит, если индексы можно сравнить между
собой напрямую.

**Реализация (требование):** операнды оператора проверяются **той же дверью, что аргументы
вызова** — не второй копией сверки; иначе дыра вернётся при следующем операторе. Запись
положена до фикса: спека прежде реализации. Фикстуры — у чинящего: негатив на `==`/`<`/`+`
между двумя newtype и между newtype и `Y`-переменной (line-pinned), позитив на операторы
одного newtype с результатом того же типа и на литерал.
...
> `docs/dev/research/2026-09-04-numeric-widening.md` §3, пробы
> `docs/plans/repro/926-mixed-numeric-doors/float_lit_ok`, `float_lit_neg`).** Пункт 4 перечислял
> только целые цели; компилятор при этом уже принимал `ro x f32 = 1.5` и `b + 1` при `b f64`
> (печатает `3.5 1.5`) и отвергал `ro n int = 1.5` (`E7301`) — поведение разумное, но не записанное.
> Записывается:
>
> - **Float-литерал** (с точкой или экспонентой) в позиции float-типа (`f32`, `f64`) принимается без
>   `as` — продолжение правила для целых; `f32` получает ближайшее IEEE-значение. В позиции
>   целого типа — по-прежнему ошибка (`E7301`), без сужения.
> - **Литерал без точки — «число», а не «целое», пока у него нет типа.** В позиции float-типа и в
>   операнде числового оператора против float-операнда он адаптируется к этому float-типу: `b + 1`
>   при `b f64` есть `b + 1.0`. Донор здесь Go (untyped constant), не Rust (там нужно `1.0`):
>   D55 уже трактует литерал как подгоняемый под цель, и запрет дал бы `1.0` ради формы, не смысла.
> - Оба правила касаются только **литералов**. Значения типа `int`/`f32` с float-операндом другой
>   ширины — по-прежнему ошибка (D405, амендмент 2026-09-04): одинаковый конкретный тип, явный `as`.
...
// Construction — ПОЗИЦИОННО (амендмент 2026-08-05; прежние примеры показывали
// именованные аргументы для полей БЕЗ дефолтов, что противоречит D102).
// ЦИТАТА ПОЧИНЕНА 2026-09-04: здесь стояло «см. ниже "Конструирование и
// деструктуризация"» — раздела с таким именем в spec/ НЕТ НИ ОДНОГО (проверено
// грепом окном 274), то есть правило два месяца опиралось на несуществующий
// адрес. Настоящие адреса: конструирование — D102 (03-syntax.md, редакция
// 2026-09-04: обязательный позиционно, дефолтный по имени); разбор на части —
// D411 (03-syntax.md): фигурная форма для именованных полей, круглая для
// позиционных. Заодно исчезла и причина спора: на день амендмента D102 говорил
// обратное тому, на что амендмент ссылался, и был приведён в соответствие
// решением владельца 2026-09-04.
...
> (`@to_uint()` D430, `max(0, x) as uint`), для битов имени, кроме `as`, нет; доноры
...
> **АМЕНДМЕНТ 2026-09-18 (решение владельца, реестр 221.1 №701 (а)):
> ЭКСПОРТИРУЕМАЯ ФОРМА — ТОЛЬКО КВАЛИФИЦИРОВАННАЯ; ГОЛЫЙ `export ro NAME = …`
> ОТВЕРГАЕТСЯ (`E_EXPORT_RO_UNQUALIFIED`).**
>
> До этого амендмента блок описывал ровно одну экспортируемую форму —
> `[export] ro Type.NAME [Тип] = …`, — а про голую на уровне модуля не говорил
> НИЧЕГО. Молчание спеки читалось как разрешение, и компилятор его принимал:
> слово `export` перед модульным `ro` разбиралось и ТЕРЯЛОСЬ в парсере.
> Владелец модуля собирался молча, а импортёр падал с `undefined identifier`
> на месте использования — то есть отказ приходил ЧУЖОМУ коду, в другом файле
> и без указания на причину.
>
> **Почему форма отвергается, а не чинится.** Научить её экспортироваться
> значило бы завести ВТОРУЮ форму записи для одного и того же. Цену второй
> формы платит не тот, кто её добавил, а каждый читатель языка: он обязан
> знать обе и помнить, чем они различаются. Замер, снятый перед решением:
> в дереве (std, spec_tests, novac, examples) голая экспортируемая форма
> встречается НОЛЬ раз — запрет не ломает ни одной строки, он закрывает дверь,
> в которую никто не ходил, но через которую уходили символы.
>
> **Что НЕ затронуто.** Приватный модульный `ro NAME = …` законен и остаётся
> — в std он живёт десятками (`fs.nv`, `addr.nv`, `unicode/category.nv`).
> Отвергается ровно СОЧЕТАНИЕ `export` + голое имя.
>
> Диагностика обязана называть обе дороги: квалифицированную запись (канон) и
> «снять `export`», если значение задумано модульно-приватным.
>
> Фикстуры: `spec_tests/conformance/neg/d200_export_ro_unqualified_neg.nv`
> (neg), `spec_tests/conformance/standalone/d200_export_ro_private_ok.nv`
> (контроль — приватный модульный `ro` работает; без него запрет был бы
> неотличим от «модульный `ro` сломали вообще»).
>
> **ЗАМЕР, НАЙДЕННЫЙ ПРИ ПОСТРОЙКЕ КОНТРОЛЬНОЙ КЛЕТКИ И НЕ ЗАКРЫТЫЙ ЗДЕСЬ
> (реестр 221.1 №701 (б)):** у самой КАНОНИЧЕСКОЙ формы `ro Type.NAME` сейчас
> не остаётся ни одной рабочей записи. Рантаймный инициализатор
> (`ro Cfg.NAME int = f()`) не собирается — `lld-link: undefined symbol` на
> функцию-инициализатор, в ОДНОМ файле и без всякого экспорта; а constexpr-
> инициализатор запрещён правилом строгого раздела выше
> (`E_RO_FOR_CONSTEXPR_PREFER_CONST`), которое требует писать `const`. То
> есть то, что правило разрешает, компилятор не собирает, а то, что собралось
> бы, правило запрещает. Форма, объявленная здесь канонической, нерабочая —
> это чинится отдельно и НЕ этим амендментом.
...
2. **Relational-операнды требуют ordered-категорию.** `<` / `<=` / `>` / `>=` требуют mutually-ordered тип
   ...
   > Уточнение 2026-09-04 (D405, амендмент того же дня): «ordered-категория» — требование к КАЖДОМУ операнду, а не
   > разрешение сравнивать операнды разных типов внутри категории. Оба операнда `<`/`==` и прочих должны иметь один и тот же
   > конкретный числовой тип: `u32 == i32`, `i64 < i32`, `f32 < f64`, `int < f64` — ошибка компиляции, явный `as`.
   > Литерал адаптируется к типу другого операнда (D55).
...
> называется семейство `Ints`/`Floats` (`Floats` на момент этой записи существовал только
> здесь; объявлен в prelude 2026-09-04 — см. амендмент ниже).
>
...
> **АМЕНДМЕНТ 2026-09-04 (решение владельца; исследование
> `docs/dev/research/2026-09-04-typed-index-vec.md` §6; реестр №896).** Раздел
> «Семантика тела» ниже говорит «мономорфизация per член, как обычный `fn[T]`»,
> но молчал о конформансе, и чекер читал молчание как «нет»: set-blanket метод
> находится диспатчем (`fn[T SignedInts] T @ordinal() -> int`; вызов
> `(7 as i32).ordinal()` резолвится), но не засчитывается при проверке bound'а
> (`fn take[I Ordinal](i I)`, вызов `take(7)` →
> `[E_BOUND_NOT_SATISFIED] type int does not satisfy Ordinal … int is missing:
> ordinal()`). Два ответа на один вопрос «есть ли у `int` метод `ordinal`».
>
> **Норма:** set-blanket метод считается объявленным на **каждом члене** set'а — в
> том числе для проверки конформанса протоколам и generic-bound'ам. Если все
> методы, требуемые протоколом, даны blanket'ами по set'у, каждый член set'а
> удовлетворяет этому протоколу. Это не новое правило, а буква «per член»,
> доведённая до конформанса.
>
> **Реализация (требование, не подсказка):** конформанс спрашивает **ту же
> таблицу методов и тем же путём, что диспатч**. Set-blanket разворачивается по
> членам при регистрации объявления — set конечен, дальше обычный lookup; полный
> скан таблицы методов на каждую проверку (форма правки №111 для blanket'ов по
> протоколу) для set'ов не годится. Проба:
> `docs/plans/repro/896-set-blanket-conformance/`. Запись положена **до** фикса
> №896: спека прежде реализации.
>
> **Тем же днём (решение владельца): set `Floats`.** Амендмент 2026-08-17 ссылался на «семейство
> `Ints`/`Floats`», но set `Floats` нигде не был объявлен — имя существовало только в этой прозе
> (проверено грепом по спеке, std и компилятору). Объявлен: `type Floats set f32 | f64` в
> `std/src/prelude/protocols.nv` рядом с `Ints`; проба `fn[T Floats] T @half() -> T => @ / (2 as T)`
> на `f32` и `f64` — чекер принимает. Назначение — blanket-методы над обеими ширинами float, по
> образцу `fn[T Ints] T @checked_*`. Прелюдийных set'ов теперь четыре: `SignedInts`,
> `UnsignedInts`, `Ints`, `Floats`.
...
- **Синтаксис.** Очередная kind-форма под `type` ([D52](#d52-объявление-типов-revised-newtype-alias-sum-через-leading-)/D53/D406): `type Name set Member1 | Member2 | …`. Диспетчеризация по **первому токену после имени** — контекстный kind-токен `set` однозначно отличает type-set от sum-type (`type X enum A | B`, D406) и остальных форм. Backtracking нет (один токен lookahead). `set` — контекстное слово (только в позиции после `type Name`), НЕ глобально-зарезервированное. Члены — TypeRef через `|`. **Многострочная форма:** если первый член на новой строке — `|` обязателен у каждого члена включая первый (аналогично D406 `enum` и D310 `set`); несколько членов в одной строке допускаются. **Вложенность (D310 amendment, Plan p424, 2026-08-07).** Член МОЖЕТ сам быть другим type-set'ом (`type Ints set SignedInt | UnsignedInt`): на объявлении внешнего набора члены вложенного **разворачиваются** (flatten) в его собственный список, рекурсивно, через ЛЮБУЮ глубину вложенности. **Дедупликация** — по идентичности листового члена (`set Ints | i32` не дублирует `i32`, уже входящий в `Ints`; если один и тот же лист достижим через два разных вложенных набора, входит один раз). **Циклы** — набор, который (транзитивно, через цепочку вложенных членов) содержит сам себя, не имеет конечного разворачивания: прямая самоссылка (`type C set C`) и взаимная (`type A set B`, `type B set A`) обе ловятся диагностикой `E_TYPE_SET_CYCLE` с путём цикла (`A -> B -> A`). **Происхождение листового члена** в диагностиках, читающих развёрнутый список (`E_TYPE_NOT_IN_SET`), помечается указанием вложенного набора, через который он попал (`` member `i32` (via `SignedInt`) ``) — без этого сообщение ссылалось бы на имя, отсутствующее в исходном тексте объявления набора. `~underlying`/representation/structural bounds по-прежнему НЕ вводятся этим амендментом — вложенность = композиция явно перечисленных множеств, не structural-matching; [Q-representation-bound](../open-questions.md#q-representation-bound) остаётся open question, Plan 102.
- **Члены — по ИДЕНТИЧНОСТИ.** Примитивы, любые объявленные конкретные типы (newtype / named-tuple / record), либо (D310 amendment) другой type-set — каждый перечислен ЯВНО. Protocol и effect НЕ конкретны — отклоняются (`E_TYPE_SET_MEMBER_NOT_CONCRETE`). Newtype `type MyI8 i8` **не** член set'а `{i8}` — нужен явный листинг; вложенность НЕ вводит структурное совпадение, она лишь имя для уже явно перечисленного списка. `~underlying` НЕТ (в Nova нет implicit-coercion; D52/D215).
- **Bound = membership-предикат.** В `[...]`-позиции type-set ведёт себя как protocol-bound (D72): `[T SignedInts]`. Композиция с протоколами через `+` (D145, conjunction): `[T SignedInt + Hash]` ⇒ T ∈ set И реализует Hash; проверки независимы, per-member. **Не более одного type-set** в bound-листе (`E_MULTIPLE_TYPE_SETS`); протоколов — сколько угодно.
- **Семантика тела.** Мономорфизация per член (как обычный `fn[T]`, Plan 48 worklist) — над РАЗВЁРНУТЫМ (post-nesting) списком листовых членов, вложенность не меняет модель мономорфизации. `T.MAX`/`T.MIN`/`T.new`/литералы резолвятся per-instance через `numeric_type_constant_mapping` по **Nova-имени** подставленного члена (нужен Nova-name subst-канал T→"i8" ПЕРЕД lookup, отдельный от C-name subst T→"int8_t"). Операторы в теле — **пересечение** легальных для ВСЕХ членов; чекер материализует resolved-тип каждого T-выражения в per-ExprId канал (codegen лоуэрит, не ре-резолвит). Без `nova_int`-fallback (§1): неразрешённый член = диагностика чекера, не угадывание.

### Амендмент (D310 amendment, Plan p424, 2026-08-07, владелец + я) — снятие «Знаковости», вложенные наборы

Один атом, два изменения, одна причина: правило «Знаковость» запрещало ЧАСТИЧНУЮ смесь
signed/unsigned (`set i32 | u32`), пока то же самое, но ПОЛНОЕ объединение (`Ints`,
`SignedInts ∪ UnsignedInts`), было легально через exemption ([D423](04-effects.md#d423)
§R3 — единый `.nv`-бланкет `fn[T Ints] T @checked_add(...)` и его родня, живущий на
`Ints` в проде). Это самопротиворечие: `{i32, u32} ⊂ Ints`, и если тело корректно
мономорфизуется по всем десяти членам `Ints`, оно тем более корректно по любым двум из
них — правило отвергало строго более СЛАБЫЙ случай, разрешая более сильный. Мотивировка
«несовместимые value-domains» (`u64.MAX ∉ i64`) не выдерживает: per-member мономорфизация
уже резолвит `T.MAX`/`T.MIN` по имени ПОДСТАВЛЕННОГО члена (см. «Семантика тела» выше) —
ни один T-выражение никогда не видит значение ЧУЖОГО домена. **Правило «Знаковость» и код
диагностики, которым оно проверялось, СНЯТЫ целиком** — смешивать signed/unsigned в одном
наборе теперь можно как частично, так и полно; `SignedInts`/`UnsignedInts` остаются в
stdlib (полезны там, где однородность нужна ПО СУЩЕСТВУ, напр. тело, полагающееся на
отсутствие отрицательных), но перестают быть ОБЯЗАТЕЛЬНЫМ способом обойти запрет.

Вторая половина того же атома — **вложенность** — устраняла бы ту же ловушку с другой
стороны: без неё `Ints` пришлось бы держать как плоский список десяти имён (или, после
снятия «Знаковости», у автора не было бы способа выразить «сумма двух готовых наборов»
без копирования их членов). Прецедент — Go 1.18+ type-constraint'ы, где составные
ограничения (`constraints.Integer = Signed | Unsigned`) — канон, не частный случай.
Разворачивание НА ОБЪЯВЛЕНИИ (а не лениво на use-site) сохраняет остальную модель D310
нетронутой: мономорфизация, оператор-пересечение и diagnostics ниже по конвейеру видят
тот же плоский список членов, что видели бы, будь он списан руками. Единственная НОВАЯ
диагностика — `E_TYPE_SET_CYCLE`: разворачивание, в отличие от протокольного `use`
(D145), не терпит цикла (protocol composition допускает diamond-embed без цикла по
своей модели; здесь цикл буквально означает «бесконечный список членов», развернуть
нечего).

### Проверки / диагностика (чекер, §1/§6; новые коды в [09-tooling](09-tooling.md))
- `E_TYPE_NOT_IN_SET` — конкретный T не член set'а (фиксируется на **инстанцировании**, не на use-site внутри тела; сообщение перечисляет членов + fix; член, попавший в набор через вложенность, отмечен происхождением — см. «Синтаксис» выше).
- `E_TYPE_SET_MEMBER_NOT_CONCRETE` — член set'а не конкретный тип (protocol / effect).
- `E_TYPE_SET_CYCLE` — (D310 amendment, Plan p424) вложенные члены образуют цикл — прямой (`type C set C`) или через цепочку (`A set B`, `B set A`) — набор не имеет конечного разворачивания.
...
- **Композиция вместо копирования** (D310 amendment) — `Ints` = `SignedInt | UnsignedInt`, будущий набор = композиция готовых, а не переписанный вручную список; синтаксическая экономия БЕЗ ослабления identity-модели (структурного совпадения по-прежнему нет).

### Связь
[D52](#d52-объявление-типов-revised-newtype-alias-sum-через-leading-) (формы `type`, first-token dispatch) · D53 (kind-токен под `type`) · [D72](#d72-generic-bounds-через-t-protocol--protocol-как-тип) (bound = тип в [...]-позиции, **amended**) · [D145](#d145-fnt-префикс--receiver-generic-decl--bounds-plan-101) (`+` multi-bound conjunction, **amended**) · [D237](#d237) (capitalized naming) · [D315](#d315-resolvedtype--единый-канонический-носитель-типа-plan-1721-2026-06-21) (ResolvedType несёт ширину/знак) · [D423](04-effects.md#d423) §R3 (единый `Ints`-бланкет — мотивировка амендмента). [Q-representation-bound](../open-questions.md#q-representation-bound) — частично (explicit-member-set, теперь и nested-explicit-member-set); `~`/repr → Plan 102. Потребитель: Plan 174.1.
...
### Почему Rust/Go/Swift-стиль, не Java/Kotlin/C/Zig
...
| Zig | `u8 + u16 → u16`, `f32 + f64 → f64` (Integer and Float Widening: coercion к типу, вмещающему все значения старого) | Расширяет молча, как C, только без потери значений; сужение — ошибка |
| **Rust/Go/Swift** | compile error → явный cast | Никаких сюрпризов; программист контролирует |
| Nova (D405) | compile error → явный `as` cast | Согласуется с «no implicit conversions» (D54) |

> Поправка 2026-09-04 (владелец): Zig стоял в ряду «compile error → явный cast» — неверно, Zig расширяет и целые, и float неявно (без потери значений); перенесён в свою строку. Доноры решения — Rust/Go/Swift.
...
### Амендмент (2026-09-04, решение владельца): правило — категорией, не списком; сравнение, float, унарный минус

**Что обнаружено** (исследование `docs/dev/research/2026-09-04-numeric-widening.md`, пробы
`docs/plans/repro/926-mixed-numeric-doors/`, строка реестра «D405 запрещает смешанную арифметику,
но три двери вне его списка молчат»). Правило выше перечисляет девять операторов и говорит
«целочисленный тип»; правило relational-операндов требует лишь ordered-категорию; D427 R2 оставляет
unsigned `-x` «не тронутым». Компилятор исполняет букву, и три двери молчат, собираются и печатают
неверное: `u32(4294967295) == i32(-1)` → `true`; `i64 + f64` → `6.5`; `-a` при `a u8 = 200` → `56`.
Первое — ровно ловушка C, ради которой этот блок принят. Контроль: `u8 + i16` → `E_MIXED_WIDTH_ARITH`.

**Норма** (владелец 2026-09-04: «никакого автоматического „бери больший“ — так должно везде
работать»; унарный минус на unsigned — ошибка компиляции):
...
1. Операнды **любого** числового оператора — бинарного арифметического (`+ - * / %`), битового
   ...
   **один и тот же конкретный числовой тип — по имени, не по ширине** (`int` ≠ `i64`, `uint` ≠ `u64`
   ...
   оператор», не перечень. Правил про float до этого амендмента в спеке не было вовсе: D405 говорил
   о целых, D55 п. 4 перечисляет целые цели литералов; литеральные правила для float — открытый
   вопрос, решённый тем же днём амендментом к D55 (float-литерал — к любому float-типу;
   литерал без точки — к float-операнду).
...
3. **Унарный минус определён только для знаковых типов.** `-x` при беззнаковом `x` — ошибка
   компиляции; диагностика называет путь `-(x as iN)`. Заворот по модулю (`-200u8 → 56`) не
   производится. Это амендмент и к D427 R2: «unsigned `-x` не трогается» → «отвергается чекером до
   кодогена»; пометка поставлена там же.
4. Реализация — требование: одна дверь сверки операндов для всех числовых операторов; чекер, не Си
   (`EXPECT_CC_ERROR` для этих случаев — не приёмка). Спека положена до фикса.
...
single-wrapper вкл. переменные, ~~int→i64 widening «невидимо для автора»~~ — **снято 2026-09-10 (решение владельца):** `int` ≠ `i64`, `uint` ≠ `u64`, неявного числового расширения нет (D405 амендмент 2026-09-04, `E_MIXED_WIDTH_ARITH`); R11 ниже говорит о newtype-wrap и sum-lift, не о числах). Пары вида
...
## D469. Анонимный тип с ИМЕНОВАННЫМИ компонентами: `(file str, line int)` (решение владельца 2026-09-04)

> **Status:** active (spec, 2026-09-04). Заполняет пустую клетку решётки D215.
> Номер выдан интегратором; замер до решения — план
> [274.6](../../docs/plans/274.6-fix-tuple-to-record-door.md) §4а.

### Что

Тип, у которого **нет имени, но есть имена у компонентов**:

```nova
fn loc() -> (file str, line int) => ("main.nv", 42)

ro l = loc()
println(l.file)          // доступ ПО ИМЕНИ, а не по индексу
ro { file, line } = l    // разбор фигурной формой (D411)
```

### Почему клетка была пуста, и почему её надо закрыть

Формы записи различаются по ДВУМ независимым осям — есть ли имя у ТИПА и есть
ли имена у ПОЛЕЙ. До этого блока язык покрывал три клетки из четырёх:

| | компоненты именованы | компоненты позиционны |
|---|---|---|
| **тип объявлен** | `type Pair(x int, y int)` (D215) | `type Point(f64, f64)` (D215) |
| **типа нет** | **D469 — эта клетка** | `(1, 2)` (D123) |

Пустота была не решением, а пропуском: анонимный кортеж `(1, 2)` в языке есть,
именованные поля у объявленного типа есть, а их сочетание — нет. Практическая
цена видна на возврате нескольких значений: `-> (str, int)` заставляет
вызывающего помнить ПОРЯДОК, тогда как `-> (file str, line int)` называет
смысл на месте и переживает перестановку.

### Правила — все выводятся из уже принятого, ни одно не новое

1. **Размещение — стек, семантика значения** (D215, allocation contract:
   скобочная форма всегда stack/value). `value` к ней неприменим: переводить с
   кучи нечего.
2. **Тождество — СТРУКТУРНОЕ**, как у анонимного кортежа (D123): два
   вхождения `(file str, line int)` суть один тип. Имя компонента входит в
   тождество — `(file str, line int)` и `(path str, line int)` РАЗНЫЕ типы;
   иначе имя не несло бы контракта, а лишь украшало.
3. **Конструирование — позиционное** (D215, амендмент 2026-08-05; D102 в
   редакции 2026-09-04): `("main.nv", 42)`. Именованная форма — только для
   компонентов с умолчанием.
4. **Доступ — по имени** (`l.file`), не по индексу: имена есть, и `.0` на
   именованных компонентах не вводится — одна форма доступа на одну сущность.
5. **Разбор — фигурной формой** `ro { file, line } = l` (D411), как у любой
   формы с именованными полями; круглая `(a, b)` на ней ЗАПРЕЩЕНА по той же
   причине, что и на именованном кортеже: имена — часть контракта, и разбор по
   порядку молча сломался бы при законной перестановке.

### Замер, стоящий за блоком (2026-09-03, до решения)

Формы в языке НЕ БЫЛО, и это проверено, а не предположено: `nova build` на
`fn loc() -> (file str, line int)` даёт `expected ')', got identifier`; грепом
по `spec/decisions/` форма не описана. То есть блок ВВОДИТ форму, а не
описывает существующую, и до его реализации компилятор обязан её отказывать.

### Что это стоит реализации

В `novac` цена названа заранее и мала: шестой вариант перечисления формы записи
плюс арм разбора — классификатор уже различает «имя у поля есть/нет», а форма
уже отделена от тождества (план 274.6, волны В1–В3-1). В оракуле — разбор
скобочного типа с именованными компонентами и структурное тождество по
(имя, тип) вместо одного типа.

### Что отвергнуто

- **Доступ `.0` наравне с `.file`** — две формы доступа к одному полю; язык
  выбирает одну, и раз имена есть, побеждает имя.
- **Считать `(file str, line int)` тем же типом, что `(str, int)`** — тогда
  имя компонента не несёт контракта и его нельзя проверить; отвергнуто той же
  логикой, по которой имена полей именованного кортежа входят в контракт.

## D477. Размещение суммы и маркер косвенности `indirect` (решение владельца 2026-09-13)

**Статус:** ПРИНЯТО 2026-09-13 (решение владельца в разговоре; номер назначен
интегратором). **Реализации НЕТ ни у одного компилятора.** Требование к Карине —
план [274](../../docs/plans/274-novac-self-hosted-compiler.md), Т2; для оракула
работа не планируется (см. §«Область» ниже).

**Амендирует:** [D406](#d406-sum-type-синтаксис-enum-маркер-2026-07-01) (синтаксис
суммы — добавляется маркер), [D241] (`value`-запись — маркер применим и к её полям).

### 0. Чего в спеке НЕ БЫЛО, и почему это пробел, а не умолчание

**Нормы о размещении суммы не существовало.** Проверено поиском по заголовкам
D-блоков во всех файлах `spec/decisions/` 2026-09-13: есть D406 (синтаксис
`enum`), есть [D474](04-effects.md#d474) (размещение литерала обработчика), есть
[D280](08-runtime.md#d280) (ссылка на heap-тип по значению = размер указателя) —
про размещение самой суммы нет ничего.

Фактическое поведение жило **комментарием в коде**: `check_infinite_type`
(`compiler-codegen/src/types/mod.rs`) вырезает суммы из проверки на бесконечный
тип ДВАЖДЫ и намеренно — как субъект цикла и как ребро, — с комментарием «A Sum
is ALWAYS heap». Поведение, держащееся комментарием, не является нормой: его
нельзя ни процитировать, ни опровергнуть, ни унаследовать второму компилятору.

### 1. Норма

**Сумма размещается ПО ЗНАЧЕНИЮ** (на стеке, в поле, в регистре — где живёт
объемлющее значение), а не за неявным указателем.

**Рекурсия разрывается ЯВНО — маркером `indirect` на объявлении.** Тип, размер
которого без косвенности бесконечен, обязан СКАЗАТЬ об этом; молчаливого спасения
кучей больше нет.
...
Помеченный вариант вносит в размер суммы РАЗМЕР УКАЗАТЕЛЯ. Косвенность невидима в
использовании: рукав `match` связывает внутреннее значение напрямую, без
развёртки, и `unsafe` не требуется нигде.

```nova
match t {
    SqlType.TList(inner) => …        // inner : SqlType, не коробка
    _                    => …
}
```

### 2. Где стоит маркер — ОДНО правило на две фигуры

**Маркер сидит на той единице, чей размер вычисляется ОТДЕЛЬНО.**

| фигура | как считается размер | место маркера | пример |
|---|---|---|---|
| сумма | МАКСИМУМ по вариантам | **вариант** | `| indirect TList(SqlType)` |
| `value`-запись | СУММА полей | **поле** | `job indirect Option[Job]` |

Это не два правила, а одно, приложенное к двум фигурам. У записи вариантов нет,
поэтому поле — единственное возможное место. Для суммы вариант выбран по трём
причинам, и каждая проверяема:

1. **Размер суммы есть максимум по вариантам** — ручка, управляющая размером,
   принадлежит той единице, где размер и решается.
2. **Одна аллокация на помеченный вариант**, а не по одной на каждое помеченное
   поле: `TMap(SqlType, SqlType)` даёт ОДНУ косвенность, а не две.
3. **Вариантная пометка лечит и ТОЛСТЫЙ вариант.** У `TDecorated(BigRec, SqlType)`
   пополевая пометка оставила бы `BigRec` внутри и раздула бы ВСЮ сумму до его
   размера — то есть отменила бы то, ради чего сумма размещается по значению.

**Расширение с варианта на поле остаётся возможным**, сужение — нет: если случай
потребует пополевой точности внутри суммы, маркер на поле добавляется как
уточнение, и написанный код не ломается.

### 3. Чего маркер НЕ делает

`indirect` отвечает на вопрос **«сколько это занимает»** и НЕ отвечает на
**«где это кончается»**.

```nova
type Job value {
    name str
    job indirect Job          // размер конечен — и тип НЕПОСТРОИМ
}
```

Чтобы построить `Job`, нужен готовый `Job`; первый взять неоткуда. Базовый случай
даёт `Option` либо вариант-лист:

```nova
type Job value {
    name str
    job indirect Option[Job]   // { name: "a", job: None } строится
}
```

**Диагностики РАЗНЫЕ, и это нормативное требование, а не пожелание:**

* `E_RECURSIVE_NEEDS_INDIRECT` — рекурсия без маркера. Сообщение называет ЦИКЛ
  (последовательность типов) и предлагает маркер в конкретной позиции.
* `E_RECURSIVE_NO_BASE_CASE` — маркер есть, базового случая нет. Сообщение
  предлагает `Option` либо вариант без рекурсивной нагрузки.

Два отказа смешивать нельзя: правки у них разные, и автор, получивший первое
сообщение на второй случай, поставит маркер и упрётся снова.

### 4. Где маркер НЕ нужен

Только там, где размещение по значению **запрошено автором**: `value`-записи и
суммы. **Обычная запись** (`type X { … }`) конечна уже сегодня — ссылка на
heap-тип по значению занимает размер указателя, а не размер объекта
([D280](08-runtime.md#d280)). Новая обязанность ложится ровно на тех, кто просил
размещение по значению, и ни на кого больше.

### 5. Область: Карина — да, оракул — нет

**Карина** — требование самохостинга (план 274, Т2), наравне с
[D473](08-runtime.md#d473) и [D474](04-effects.md#d474).

**Оракул** — работа НЕ планируется. Критерий владельца 2026-09-13: «причина менять
оракул одна — требования Карины, т.к. она без этого не может собраться». Сумма в
куче сборке Карины не мешает; перевод оракула на размещение по значению — широкий
радиус (замер плана
[172.14](../../docs/plans/172.14-value-abi-perf-placement.md) Ф.2: 4387 сайтов
`nova_make_*` в 73 единицах компиляции) и к тегу v0.1 не приближает.

**Следствие, названное вслух:** до реализации в Карине ни один компилятор не
соберёт код с `indirect`. Положительная фикстура на D477 покраснеет у оракула,
поэтому в корпус до реализации идут только ОТРИЦАТЕЛЬНЫЕ (рекурсия без маркера —
отказ), а положительные приезжают тем же слиянием, что и реализация. Обратный
порядок кончается фикстурой, которую придётся отключить.

### 6. Что обязан сделать реализующий

**Детектора рекурсии для сумм НЕТ ни у одного компилятора** — это установлено
разведкой 2026-08-22 (поправка в плане 172.14) и проверено по коду: единственный
настоящий детектор `check_infinite_type` суммы исключает. **Работа начинается с
написания детектора**, а не с его использования; каркас обхода (DFS с `on_path`,
ловящий и взаимную рекурсию) лежит рядом.

**Приёмка — четыре пункта, и каждый машинный:**

1. рекурсивная сумма БЕЗ маркера → `E_RECURSIVE_NEEDS_INDIRECT`, а не
   `stack overflow` и не тихая куча;
2. она же С маркером → собирается и ИСПОЛНЯЕТСЯ (не «собирается»: предмет —
   проходит ли значение целиком);
3. нерекурсивная сумма → в сгенерированном C НИ ОДНОГО `nova_alloc` под неё
   (проверяется грепом по выпущенному C, а не по исходнику компилятора);
4. обратная проба: правило снято — фикстура (1) зеленеет.

Плюс негативная фикстура на `E_RECURSIVE_NO_BASE_CASE` с построчным
`nova:expect`, потому что это новый `E_*`.

### Связь

* [D406](#d406-sum-type-синтаксис-enum-маркер-2026-07-01) — синтаксис суммы,
  куда добавляется маркер.
* [D280](08-runtime.md#d280) — почему обычная запись в маркере не нуждается.
* План [274](../../docs/plans/274-novac-self-hosted-compiler.md) Т2 — требование
  к Карине, тесты и порядок работ.
* План [286](../../docs/plans/286-db-driver.md) — первый потребитель
  (`SqlType.TList`); там же разобрано, почему отвергнуты `[]T`, `Box[T]` и
  указатель.
