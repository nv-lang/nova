...
| Ф.3 | Дженерики `.nv` (`[]T`/`Vec[T]`/`Option`/`Result` Display/Debug) + auto-derive record/sum/tuple (компактная `TypeName(a, b)` форма Display, отличная от именованной Debug-формы) | ⏳ pending — НЕ начата |
| Ф.4 | Зачистка: оставшийся `conv.h` → `.nv`; удаление мёртвого `nova_fmt_*` | ⏳ pending — **заблокирована на Ф.2's V1-упрощении #1** (см. ниже): примитивный форматный путь (bare + rich-spec, `emit_interpolated_str`/`emit_format_spec_value`) сознательно НЕ перевязан на буфер-примитивы Ф.1 — `conv.h`'s `nova_fmt_*`/`nova_*_to_str`/`nova_*_to_debug_str` остаются ЖИВЫМИ (не мёртвыми), так что «удалить мёртвый nova_fmt_*» пока буквально нечего удалять |
