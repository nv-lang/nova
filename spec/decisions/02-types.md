...
(четыре раунда решений). **Status:** 🟢 РЕАЛИЗОВАНО (заходы 1-5, 2026-07-07), КРОМЕ Р13/Р14.
Факт по правилам: Р1✅ Р2✅ Р3✅ Р4✅ Р5✅ Р6✅ Р7✅ Р8✅ Р9✅ Р10✅ Р11✅ Р12✅ —
**Р13/Р14 (перегрузка по оси режима) ОТКРЫТЫ** (сейчас `f(x T)` vs `f(mut x T)` — дубль-
дефиниция; нужен mode-axis в overload-ключе + диспатч по mut-биндингу; см. точку
возобновления в docs/plans/184-ref-type-revision.md). Носитель — `TypeRef::Ref(T)`; коды
...
> **⚠️ СТАТУС Р13/Р14 (заход-5, 2026-07-07): НЕ РЕАЛИЗОВАНО — точка возобновления.**
> Сейчас `f(x T)` и `f(mut x T)` дают ошибку «duplicate definition `f` with same signature»:
> ось режима (mut/consume) НЕ входит в overload-сигнатурный ключ различения (различается
> только тип/арность/возврат, D84). Р13/Р14 требуют: (1) включить ось режима параметра в
> ключ различения перегрузок; (2) диспатч по mut-биндингу аргумента (mut→mut-версия, иначе
> ro; consume-версия — только в последней точке использования). Прецедент — receiver-mutability
> (Plan 135). Затрагивает overload-signature + resolver — рискованная зона, отдельный заход.
