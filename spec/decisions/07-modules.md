...
| [D369](#d369-межпакетный-импорт--только-через-объявленную-зависимость) | Межпакетный импорт — только через объявленную `[dependencies]`-зависимость |
...
| Код | Кogда |
|---|---|
| `E_D78_MODULE_PATH_MISMATCH` | declaration не matches rev-3 (`parent.target`) — hard error. Покрывает и rev-1 legacy form (full path от package), и любые иные несоответствия. |

**Bug history:**
- До **2026-06-01** compiler принимал И rev-3 И rev-1 legacy form
...
- **2026-06-01 `[M-D78-strict-removal]` ✅:** после полной миграции
  ...
  retired (на практике никогда не сработает — rev-1 form ловится раньше
  как mismatch). См. `compiler-codegen/src/manifest.rs`
  `check_module_path_with_kind` и [Эволюция D29](#эволюция-d29) ниже.
...
#### Циклические импорты — запрещены — ОТМЕНЕНО

> **ОТМЕНЕНО D291** (Plan 162, 2026-06-16): межмодульные циклы РАЗРЕШЕНЫ —
> cycle-detection заменена cycle-guard'ом («Amended Plan 42 Rule A»). Действующая
> норма — в [D291](#d291); текст ниже оставлен КАК ИСТОРИЯ решения.
>
> **Почему отменённое ОСТАВЛЕНО, а не затёрто** (и эта фраза здесь стоит нарочно,
> чтобы через месяц цитату не «прибрали» как мусор): читатель, пришедший в D29
> по ссылке или поиском, должен понять, что он не сумасшедший, а прочёл УСТАРЕВШЕЕ.
> Стёртое правило возвращается: следующее окно, не найдя ответа, почему циклы
> нельзя, предложит запретить их заново.
>
> **В силе ОСТАЁТСЯ соседнее правило той же фразы выше** («Wildcard и циклические
> импорты запрещены»): запрет `import x.*` НЕ отменялся никогда. Одна фраза
> несёт ДВА правила, из которых отменено только одно, — поэтому она и не правится
> удалением слова: вычеркнув полфразы, потеряли бы действующий запрет.
>
> *Правка редакционная (2026-09-18, окно Карины, форма согласована с интегратором):
> норма не меняется — она принята D291 два с половиной месяца назад, а текст D29
> всё это время говорил обратное. Найдено агентом-читателем при разборе форм импорта.*
...
> **ТЕКСТ ВЫШЕ И ПРИМЕР С КРЕСТОМ — ИСТОРИЯ, А НЕ НОРМА.** Сегодня такой цикл
> НЕ ошибка: резолвер держит cycle-guard (D291), и выделение в `app.shared`
> остаётся СОВЕТОМ по устройству, а не требованием компилятора.
...
- **rev-3 strict removal `[M-D78-strict-removal]` ✅** (2026-06-01):
  ...
  `bench/`; включая переименование `plan100.3/` → `plan100_3/`) rev-1
  acceptance branch удалён из `check_module_path_with_kind`. rev-1 form
  теперь падает с `E_D78_MODULE_PATH_MISMATCH` (single hard error для
  любого несоответствия rev-3). Warning `W_D78_REV1_DEPRECATED` retired
  (на практике никогда не сработает). `ModulePathCheck::Rev1Deprecated`
  enum-variant kept как dead variant для ABI stability и potential
  per-package opt-in legacy mode в будущем.
...
## D479. Две записи одного элемента разрешаются одинаково, и отказ называет первый неразрешившийся сегмент (2026-09-18)

**Статус:** принято (решение владельца 2026-09-18; поднято окном Карины при разборе
строки реестра №1150, номер выдан интегратором).

### Задача

Один и тот же элемент можно назвать двумя записями, и обе законны:

```nova
import path.mod.{Pointt}
ro a = Pointt.MAX            // через селективный импорт (D29)

import path.mod
ro b = mod.Pointt.MAX        // через qualified namespace (D289)
```

Нормы, говорящей, что `Pointt` и `mod.Pointt` — ОДИН элемент, в спеке не было. Следствие
измерено на живом носителе: дверь `novac`, судившая ГОЛОВУ пути, для первой записи
спрашивала про `Pointt`, а для второй — про `mod`, то есть задавала РАЗНЫЕ вопросы об
одном элементе и могла дать разные диагностики на одну и ту же ошибку.

**Класс:** одна сущность, две записи, и ничто не обязывает их отвечать одинаково.

### Решение — нормируется НАБЛЮДАЕМОЕ

**1. Тождество.** Обе записи разрешаются в ОДНУ декларацию. Наблюдаемое тождество, и
проверяется оно тремя равенствами, а не чтением устройства:

- одинаковый результат разрешения (тот же элемент);
- одинаковый НАБОР диагностик (не «похожий» — тот же);
- одинаковый ТЕКСТ отказа для обеих записей.

Форма записи — свойство ТЕКСТА программы, а не её смысла; всё, что различает две записи
для пользователя, есть дефект.

**2. Отказ называет ПЕРВЫЙ неразрешившийся сегмент** и не называет последующие. Если
`mod` не разрешился — речь о `mod`, и о `Pointt` говорить нечего: его не в чем искать.
Если `mod` разрешился, а `Pointt` нет — речь о `Pointt`, и имя модуля в сообщении
уместно лишь как адрес поиска.

**Почему второе правило стоит в ЭТОМ блоке, а не в своём:** оно есть следствие первого.
Как только две записи обязаны отвечать одинаково, вопрос «о чём говорит отказ» перестаёт
быть свободным: сегменты у записей разные, и одинаковым ответ будет только тогда, когда
он привязан к ПЕРВОМУ несостоявшемуся шагу разрешения, а не к позиции в тексте.

### Обоснование

Наш способ добиться этого — ОДНА дверь разрешения пути; две двери к одному корню
расходятся на первой же правке (274.4, и там же измерено, как это выглядит). Но способ
намеренно оставлен ЗДЕСЬ, а не в норме: норма обязана пережить переписывание резолвера,
а требование «одна дверь» связало бы руки без выгоды для пользователя.

Доноры: rustc — путь резолвится в `Res::Def(def_id)`, и `use`-псевдоним не создаёт второй
сущности; Go — `pkg.Name` и импортированное имя указывают на один объект.

### Приёмка

Негативная фикстура на КАЖДУЮ из двух записей, а не на одну: норма о тождестве двух форм,
доказанная на одной из них, не доказана вовсе — тот же класс, на котором сгорел фикс
№1147 (починку проверили на том носителе, на котором делали, а второй остался нетронут).

Четыре клетки: (1) обе записи с ВЕРНЫМИ именами — приняты, результат один; (2) опечатка в
ГОЛОВЕ — отказ называет голову, в обеих записях; (3) опечатка в СРЕДНЕМ сегменте
(`mod.Pointt` при верном `mod`) — отказ называет тип, а не модуль; (4) сравнение ТЕКСТОВ
отказа для двух записей одной и той же ошибки — тексты совпадают.

### Связь

- [D29](#d29-модули-и-импорты) — селективная форма импорта.
- [D289](#d289-import-m--last-segment-qualified-namespace-option-b-amend) — `import m`
  связывает последний сегмент как namespace; без него второй записи не существовало бы.
- [D381](08-runtime.md#d381) — DEF/REF-резолюция имени типа для C-манглинга: смежное
  тождество (какой модуль ОБЪЯВИЛ имя), не заменяющее это.
- Реестр 221.1 №1150 — носитель, на котором расхождение стало видимым.
...
> **Амендмент (Plan 223, владелец 2026-07-23) — `src/` невидим ВЕЗДЕ (D78
> rev-5).** Plan 195 (выше) канонизировал `src/` в **манифест-режиме**
> (`[lib] src = "src"` — `nova-http` объявляет `module http.server`, слово
> `src` в module-path не участвует), но это оставляло асимметрию: приложение
> БЕЗ собственного `[lib] src` (напр. демо/example-программа, чей ближайший
> предок-манифест резолвит плоский корень — `[lib] src = "."` или ключ
> отсутствует), нарочно разложенное по канону `<root>/src/{main.nv,app/…}`
> (как Rust/npm/Python/Zig), теряло симметрию: `src` **протекал** в
> декларацию как обычная папка (`<root>/src/main.nv` → `module src.main`,
> `<root>/src/app/*.nv` → `module src.app`) — раскладка-канон, но имя
> зарезервированного каталога видно снаружи, чего нет у манифест-режима.
> Решение владельца: **одно правило без исключений — «`src/` невидим
> ВЕЗДЕ»**, слово `src` остаётся только на диске (никогда в module path),
> симметрично для пакета (манифест) И приложения (entry-файл без
> собственного `[lib] src`, где `src` встречается на пути ниже уже
> выведенного корня).
>
> **Правило (rev-5):**
> 1. Если путь от уже выведенного `source_root` (манифест-режим — уже
>    учитывает `[lib] src`, если задан; entry-режим без манифеста или с
>    плоским `[lib] src = "."` — сам этот корень) до entry-файла проходит
>    через каталог, буквально названный `src`, — **первый** (ближайший к
>    корню) такой каталог становится ЭФФЕКТИВНЫМ корнем модулей для этого
>    файла (и всех его folder-module-соседей); сегмент `src` НИКОГДА не
>    входит в module path. Пример (флагман-агрегатор, `examples/`-пакет с
>    `[lib] src = "."`): `examples/flagship/aggregator/src/main.nv` →
>    `module main` (не `src.main`, не `examples.flagship.aggregator.src.main`);
>    `src/app/*.nv` → `module app`; `src/api/*.nv` → `module api`;
>    `src/domain/domain.nv` → `module domain`. Отсутствие имени
>    пакета/владельца в этих декларациях — сознательное следствие: у entry-
>    приложения нет отдельного library-имени, ради которого стоило бы
>    добавлять сегмент (симметрично тому, как манифест-режим уже не
>    добавляет `http.src.server`, а прямо `http.server`).
>
>    **↑ ЭТОТ АБЗАЦ ОТОЗВАН — D78 rev-6 (ниже, план 267).** Сброшенный
>    корень ПОЛУЧАЕТ имя — имя папки-владельца: `module aggregator`,
>    `module aggregator.app`. Читайте rev-6; здесь текст оставлен, чтобы
>    видеть, от чего отошли, и почему: безымянный корень делал приложение
>    особым случаем языка и разваливал соседние файлы на отдельные модули
>    (реестр 221.1 №564, №531).
> 2. Entry-файл ВНЕ `src/` (путь от `source_root` до файла НЕ проходит через
>    каталог `src`) — поведение не меняется (вывод корня как раньше, полностью
>    byte-identical).
> 3. `src` — зарезервированное имя module-каталога ВНУТРИ уже установленного
>    `source_root`: если после применения правила 1 (или уже в манифест-
>    режиме, где `[lib] src` сам относит `source_root` в `src/`) на пути к
>    файлу встречается ЕЩЁ ОДИН каталог `src` — compile error
>    `E_MODULE_DIR_SRC_RESERVED` (`src/src/` делало бы правило 1
>    неоднозначным — какой из двух `src/` считать корнем). Симметрично
>    обоим режимам.
> 4. Манифест-режим с нетривиальным `[lib] src` (`nova-http`, `std`, …) не
>    меняется — `source_root` уже указывает НА `src/`, поведение вне этого
>    каталога прежнее; правило 1 применяется лишь если каталог `src`
>    встретится ЕЩЁ РАЗ глубже (см. правило 3).
> 5. Path/module enforcement (ниже) генерирует suggestion без сегмента
>    `src` — согласовано с правилом 1 (suggestion никогда не предлагает имя,
>    в которое `src` протекает как обычный сегмент).
>
> Язык-меняющее (резолюция модулей путём file path ↔ module name) — правило
> реализовано в том же слиянии, что и этот амендмент (`compiler-codegen/src/
> manifest.rs::apply_src_transparency` + `expected_module_path_rev3`'s
> `bare`-режим).
...
#### Виртуальный корень: `src/` сбрасывает имя пакета (амендмент D78 rev-6, план 267, владелец 2026-08-10)

**Одной строкой.** Корень исходников — либо сам пакет, либо его папка
`src`. Слово `src` в имя модуля не входит НИКОГДА. Имя модуля = имя корня
+ папки по пути от корня; файлы прямо в корне — один модуль с именем
корня.

**Что меняется по сравнению с rev-5.** rev-5 объявил `src/` невидимым, но
оставил открытым вопрос, КАК ЗОВЁТСЯ то, что лежит в сдвинутом корне.
Фактический ответ rev-5 («никак»: `package_name` становился пустым, и
каждый файл судился как одиночный модуль по имени своего файла) породил
две беды сразу.

Первая — приложение оказалось особым случаем языка. У библиотеки
`nova-http` корень зовётся `http`, вложенное — `http.server`; у
приложения корень не звался никак, а вложенное было голым `app`. Одно и
то же дерево папок читалось двумя разными правилами в зависимости от
того, есть ли у пакета манифест.

Вторая — и она стоила нам двух дефектов — соседние файлы приложения
перестали быть одним модулем. Флагман `examples/flagship/http_proxy_chain/src/`
состоит из шести файлов, которые пользуются типами друг друга без
импортов (`ConnectFailed` объявлен в `upstream.nv`, применён в
`upstream_socks5.nv`). `nova build` их собирал — он проверяет декларацию
ТОЛЬКО у entry-файла. `nova check` на каталоге краснел на каждом
(реестр 221.1 №564), а языковой сервер писал `unknown type ConnectFailed`
на верном коде (№531, третья причина). Один корень: одиночный путь
проверки не восстанавливал реальную единицу компиляции.

**Правило rev-6.**

1. **Значение `[lib] src` — только `"."` или `"src"`.** Любое другое —
   ошибка чтения манифеста `E_MANIFEST_SRC_VALUE`, hard error. Проверяет
   КОМПИЛЯТОР при разборе манифеста, не страж-скрипт: чужой пакет
   приезжает зависимостью и стражей у нас не спрашивает. Сверено по
   факту: во всех репозиториях 2055 манифестов, значений ровно два —
   `"."` (1956) и `"src"` (99); третьего не написал никто, миграция
   нулевая.

2. **Папка, буквально названная `src`, сбрасывает корень** — где бы она
   ни лежала (это и есть rev-5). Новое: **сброшенный корень получает
   имя** — имя папки, которая владеет `src`.
   `examples/flagship/http_proxy_chain/src/` → корень зовётся
   `http_proxy_chain`.

3. **Файлы прямо в сброшенном корне — root peers этого корня** (rev-4,
   слово в слово, только имя пакета берётся с диска, а не из манифеста):
   все объявляют `module http_proxy_chain`, делят namespace без импортов
   между собой.

4. **Вложенные папки получают префикс корня**, как у любого пакета:
   `src/app/*.nv` → `module http_proxy_chain.app`,
   `src/app/dns/*.nv` → `module http_proxy_chain.app.dns`.

**Что отзывается.** Пример rev-5 «`examples/flagship/aggregator/src/main.nv`
→ `module main`; `src/app/*.nv` → `module app`» — ОТОЗВАН. Правильно:
`module aggregator` и `module aggregator.app`. В `main` не было принципа:
это было имя файла `main.nv`, случайно совпавшее с гошным `package main`,
и обобщалось оно скверно — у каждого приложения корневые файлы звались бы
одинаково, а уникальности имён нет.

**Граница соглашения — названа явно, чтобы её не искали.** Вложенное
приложение без собственного манифеста может назвать корень только `src`.
Назовёт `source` — слово войдёт в имя модуля. Это не дефект: без
манифеста угадывать нечего, а с манифестом (п. 1) выбор всё равно из двух
слов. Именно поэтому запрет на третье значение и сброс корня — одно
решение, а не два: пока манифест не мог сказать ничего, кроме `.` и
`src`, соглашение и манифест обязаны называть корень ОДНИМ словом.

**Диагностика.** Несовпадение декларации в сброшенном корне —
по-прежнему `E_D78_MODULE_PATH_MISMATCH`; перечень ожидаемых форм теперь
включает root-peer-форму сброшенного корня (`module <owner-dir>`).
Неверное значение `[lib] src` — `E_MANIFEST_SRC_VALUE` с перечислением
двух допустимых.

**Приёмка (план 267).** Позитивная фикстура: многофайловый сброшенный
корень + вложенная папка, типы видны без импортов, `nova build` и
`nova check <каталог>` дают ОДИН вердикт. Негативная: `[lib] src =
"source"` → `E_MANIFEST_SRC_VALUE`. И шаг, сравнивающий диагностику
языкового сервера с компилятором на МНОГОФАЙЛОВОМ модуле с внешней
зависимостью — на одиночном файле без импортов оба зелены, и такой шаг
не доказал бы ничего (см. №531).

Язык-меняющее (резолюция модулей путём file path ↔ module name).
...
- Межпакетный резолв: первый сегмент import-пути — имя объявленной
  ...
  [D369](#d369-межпакетный-импорт--только-через-объявленную-зависимость).
...
#### Shadow распространяется на МЕТОДЫ (Plan 138.2 Ф.0c, 2026-06-11)

«User-declaration wins» — **full-shadow**: когда user re-declares имя
generic-типа из prelude / явного импорта с **другой арностью** (напр. user
`type Vec { x, y }` arity 0 поверх `Vec[T]` arity 1 из `import
std.collections.vec_owned.{Vec}`), user-тип выигрывает **целиком** — вместе
с его (отсутствующими) методами. Imported-generic'а методы (`Vec[T] @push`,
`@get`, …) **НЕ leak'ают** на user-redeclared receiver: их type-refs `Vec[T]`
не арити-чекаются против non-generic user-`Vec` (иначе E7310 `type Vec is not
generic, but 1 argument provided`), и record-литерал `Vec{x,y}` монорфизируется
в user-struct, а не через import-template. Вызов `v.push(1)` на user-`Vec` —
честная **no-method ошибка**, не silent-routing на prelude-`Vec`. Дискриминатор:
merged-метод несёт carrier-арность импорта (`Vec[T]` receiver → `generics.len()
≠ user-arity`); метод, написанный самим user'ом на его redeclared-типе, несёт
совпадающую арность и сохраняется. Mitigation против over-skip: правило fire'ит
ТОЛЬКО когда user объявил то же имя в entry-peer-files И merged-арность отличается
(легитимный `import …Vec` БЕЗ user-redeclare не затрагивается). Реализация —
type-checker `is_shadowed_import_method` (types/mod.rs) + codegen guards на
generic-registration / method-prepop / emit (emit_c.rs). Это уточняет
 implementation-уровень shadow с types-only на types+methods.
...
## D369. Межпакетный импорт — только через объявленную зависимость
...
- **`std` — неявное исключение:** стандартная библиотека доступна без
  записи в `[dependencies]` (как Rust `std`). Машинерия D369 — для
  **не-`std`** пакетов.
...
- [D78](#d78-package-tooling-novatoml-novalock-registry-chain-workspace)
  — `nova.toml` / `[dependencies]` / workspace; D369 фиксирует **правило
...
## D471. `[ffi] cxx` — пакет, линкующийся с библиотекой на C++

**Статус:** принято 2026-09-09, реализуется (заказ владельца; носитель — пакет
`nova-duckdb`).

**Контекст.** Статический архив на C++ **не несёт свою стандартную библиотеку**.
MSVC подхватывает её по указаниям внутри объектных файлов; в ELF-цепочках таких
указаний нет, и стандартную библиотеку добавляет **драйвер** — но только если его
позвали как `g++`/`clang++`, а не как `gcc`/`clang`. Nova зовёт C-драйвер всегда:
понятия C++ в компиляторе не было вовсе (проверено 2026-09-09 — ни `c++`, ни
`stdc++` в `link_prep.rs`; драйвер выбирается через `ToolchainPref`, и все три
варианта C-шные).

Замер, породивший блок: `nova-duckdb` зелен на Windows (21/21) и красен на Linux
— каждый символ `duckdb_*` неразрешён, и отказ указывает на код пользователя, а не
на причину.

**Решение.** Шестой ключ секции `[ffi]`:
...
`cxx = true` означает: **линковка идёт через C++-драйвер**. `clang` → `clang++`,
`gcc` → `g++`; на MSVC поведение прежнее. По умолчанию `false` — ни один
существующий пакет не меняет поведения.

**Почему флаг, а не список библиотек по системам.** Рассматривался вариант
`libs.linux = ["stdc++"]` и отвергнут по трём причинам.

1. **`cxx` — правда о библиотеке, а список — правда о платформе.** «Этот пакет
   линкуется с C++» есть свойство зависимости; «на Linux добавь `stdc++`» —
   следствие, которое инструментальная цепочка знает лучше автора пакета и знает
   по-разному на разных дистрибутивах.
2. **Список исключений растёт, понятие — нет.** Сегодня Linux, завтра macOS,
   послезавтра MinGW против MSVC; каждый автор пакета выводит заново то, что
   система уже знает.
3. **Список порождает МОЛЧАЛИВЫЙ отказ.** `stdc++`, дописанный в `libs`, на
   Windows не находится, и `first_missing_ffi_lib` уводит пакет в
   `SkipReason::FfiLibNotFound` — то есть лечение одной поломки заводит другую,
   причём беззвучную (реестр 221.1 №1055 — тот же корень).

**Отсутствие драйвера — громкий отказ, а не тихий откат.** Если `cxx = true`, а
C++-драйвера нет ни рядом с C-драйвером, ни в `PATH`, сборка **останавливается с
диагностикой, называющей искомое имя**. Молчаливый откат на C-драйвер даёт стену
неразрешённых символов, указывающую на код пользователя, — ровно тот отказ, из-за
которого этот блок и написан.

**Граница.** Блок НЕ вводит ни компиляции `.cpp` в составе пакета, ни выбора
версии стандарта C++, ни флагов вида `-std=c++17`: `c_shims` остаются C. Речь
только о **драйвере на шаге линковки**. Компиляция C++-исходников — отдельный
вопрос, и его этот блок сознательно не решает.

### Связь

- [D214](#d214) — секция `[ffi]`, куда добавляется ключ.
- Реестр 221.1 №1055 — разбор `nova.toml` построчно; `cxx` булев и потому этой
  ямы не касается, но живёт в той же секции.
