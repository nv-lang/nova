...
- **Стек / escape analysis** — компилятор держит на стеке всё, что не
  ...
  Тот же обход отвечает на вопрос «убегает ли значение» для косвенного возврата в буфер вызывающего — [D473](08-runtime.md); третий разбор для этого не заводится.
...
`if let`. Plain `Ok(x)` в этом случае — **ошибка** (правило СУЖЕНО амендментом 2026-09-24,
план 293, ниже в этом D-блоке: без маркера законен биндинг, использованный ровно один раз
как форсируемый перенос — хвост арма, `return`, аргумент конструктора):
...
### Амендмент (2026-09-24, план 293) — маркер `consume` не нужен там, где выбора нет

> **Решение владельца 2026-09-24** («да» на вариант 1 исследования
> [16](../../docs/dev/research/16-consume-pattern-auto-move.md)). Решение 2026-07-18
> «вариант А: enforce по букве» НЕ отменяется, а сужается: маркер документирует
> РЕШЕНИЕ, а там, где грамматика форсирует перенос, решения нет. Прецедент —
> D409 (`03-syntax.md`, «`-> @`: автоматический возврат приёмника»): форма с одной
> семантикой автоматизируется, класс ошибок исчезает по построению.

Биндинг payload-паттерна must-consume типа БЕЗ маркера `consume` законен тогда и
только тогда, когда имя использовано **ровно один раз** и это использование —
**форсируемый перенос**:

- хвост арма (`Ok(s) => s`);
- `return s`;
- аргумент конструктора — варианта суммы или литерала записи (`Ok(v) => Ok(v)`,
  `Some(v) => Some(v)`, `Ok(s) => Conn { s }`).

Любое иное использование — вызов метода (`Ok(s) => s.close()`) или функции, второе
упоминание, захват замыканием, view-использование — по-прежнему
`E_CONSUME_PATTERN_REQUIRED` с тем же машинно-применимым предложением «insert
consume». Видимость передачи владения переезжает с паттерна на точку решения:
`consume stream = match …` остаётся маркированным.

Правило СИНТАКСИЧЕСКОЕ и потому работает и в обобщённых телах, где тип payload не
известен: форма `Ok(v) => Ok(v)` в `map_err` законна по построению, а не по
молчанию backward-compat D156 (исследование 16, факт №3).

`?` на `Result` с must-consume payload формой переноса в этом правиле не считается
(умолчание плана 293, открытый вопрос).
...
## D465. Affine-значение и протокольный БОКС: бокс забирает владение, заимствованный бокс — ошибка

> **Решение владельца 2026-08-14**, записанное в реестре 221.1 №572
> (интегратор, по поручению владельца). Спека пишется ДО реализации:
> реализация — окно чекер-канала плана
> [196](../../docs/plans/196-one-truth-closeout.md) вместе с №598 и №571.
> Амендит [D131](#d131-consume--квалификатор-логической-линейности); границ
> [D72](02-types.md#d72) НЕ касается — см. §«Чего это решение НЕ меняет».

### Задача

[D131](#d131-consume--квалификатор-логической-линейности) держит линейность
на **именах биндингов**: у переменной есть `VarState`, и потребление
переводит имя в `Consumed`. Протокольный бокс имени не имеет. Поэтому
обязательство в него не переходит — бокс копирует ДОСТУП к значению, не
копируя ОБЯЗАТЕЛЬСТВО, и сбежавший бокс переживает потребление оригинала.

Репро, воспроизведённое трижды (2026-08-11, 2026-08-19, 2026-08-26):

```nova
type Res consume { fd int }
fn Res @kind() -> int => @fd
fn Res consume @close() -> () => ()

type Sink protocol { @kind() -> int }

fn keep(x Sink) -> ro Sink => x        // бокс СБЕГАЕТ из view-borrow наружу

fn main() Io -> () {
    consume r = Res { fd: 3 }
    ro escaped = keep(r)
    r.close()                          // оригинал ПОТРЕБЛЁН
    print("after close: ${escaped.kind()}")   // печатает 3
}
```

Собирается без единого слова — ни ошибки, ни предупреждения. В пробе
`@close()` пуст, поэтому падения нет; но ровно этой формой написаны наши
настоящие ресурсы, и с непустым `close()` это use-after-close — та самая
ошибка, ради предотвращения которой `consume` и существует.

### Что решено

**1. Протокольная позиция для affine-значения — это MOVE.** Бокс ЗАБИРАЕТ
владение: оригинал помечается потреблённым, а сам бокс становится
consume-значением и несёт обязательство дальше. В репро это делает
`r.close()` честной ошибкой use-after-consume (D131), потому что `r`
потреблён передачей в `keep`.

**2. ЗАИМСТВОВАННЫЙ бокс на affine-значении — ошибка компиляции,
`E_CONSUME_PROTOCOL_BORROW`.** Форма репро (`ro`-вход в протокол, из
которого бокс может сбежать) отвергается. Довод не вкусовой: у Nova нет
лайфтаймов ([D75](06-concurrency.md#d75) — borrow-checker отвергнут
сознательно), и удержать заимствование в сбегающем боксе языку попросту
нечем. Там, где Rust требует `&dyn Trait + 'a`, у нас нет `'a`.

**Эталон.** В Rust `Box<dyn Trait>` ПЕРЕМЕЩАЕТ значение: после этого
оригинала нет, и «положить в бокс и продолжать пользоваться» невозможно по
построению. Правило 1 — это оно; правило 2 закрывает то, что в Rust держит
лайфтайм, а у нас держать нечем.

**Отвергнутый вариант (а): «affine-значение вообще не кладётся в
протокольный бокс».** Делает consume-типы второсортными — ресурс нельзя
убрать за интерфейс — и расходится с move-семантикой языка всюду
остальном.

### Чего это решение НЕ меняет

**Обобщённая граница `[T Protocol]` не затронута.** Протокол в сигнатуре
бывает двух родов, и они ведут себя по-разному:

| Род | Форма | Что происходит | Это решение |
|---|---|---|---|
| Граница | `fn f[T Write](x T)` | мономорфизуется; бокса нет, сбегать нечему | НЕ касается |
| Бокс | `fn f(x Write)` | динамика; бокс может сбежать | ПРАВИЛА 1 и 2 |

Различение существенно, а не педантично. **Замер по std 2026-08-26:**
границ около 24 (`Write` 9, `Read` 10, `Display` 5), бокс-позиций — 2.
Если бы правило задевало ГРАНИЦЫ, оно запретило бы передачу `File` и
`TcpStream` в любое `Write` — главный идиом ввода-вывода языка, то есть
ровно тот останавливающий всех ложный отказ, против которого говорит
реестр 221.1 №703. Целясь в боксы, правило стоит единицы мест.

Также остаётся законной **двойная передача consume-значения в НЕ-consume
параметр** обычного (не протокольного) типа: это view-borrow, и дефект был
не в ней, а в ПОБЕГЕ бокса за пределы заимствования.

### Приёмка

1. Цепочка репро выше падает на `r.close()` честной ошибкой
   use-after-consume — не на `keep(r)` и не молча.
2. Форма `fn keep(x Sink) -> ro Sink` отвергается
   `E_CONSUME_PROTOCOL_BORROW` — негативная фикстура с line-pinned
   `nova:expect`.
3. Проба «подсунь заведомо негодное»: сломать условие — обе фикстуры
   краснеют; вернуть — зеленеют. Фикстура, которая была бы зелёной и без
   фикса, не доказывает ничего.
4. Радиус измерен КОМПИЛЯТОРОМ, а не грепом: временная диагностика на
   каждую бокс-позицию с affine-аргументом плюс подсчёт по корпусу — тем
   же приёмом, каким №634 мерил лазейку D410. Грепная оценка выше (2 места
   в std) — вход, а не замер: греп видит только параметры и только четыре
   имени протоколов, а репро — ВОЗВРАТ.
5. Ложных отказов по std и корпусу — ноль; ненулевое число означает, что
   правило задело границы, а не боксы.

### Препятствие, названное честно

Анализ потребления (`ConsumeRegistry`) знает, какие параметры consume и
какие view, но НЕ знает, какие ПРОТОКОЛЬНЫЕ — для этого ему нужна таблица
типов. Это отдельная конструкция, а не строчка, и потому работа идёт
каналом чекера плана 196 (чекер резолвит один раз → `resolved_types` →
кодоген читает), а не наращиванием вывода в эмиттере.

### Смежное

- [D131](#d131-consume--квалификатор-логической-линейности) — линейность на
  именах биндингов; корень дефекта в том, что бокс имени не имеет.
- [D133](02-types.md#d133) — type-level `consume`; те же обязательства с
  другой стороны.
- [D53](02-types.md#d53) и [D42](02-types.md#d42) — протокол как тип,
  структурное соответствие для plain protocol-typed параметра.
- [D72](02-types.md#d72) — обобщённые границы `[T Protocol]`, которых это
  решение не касается.
- [D75](06-concurrency.md#d75) — почему borrow-checker отвергнут; отсюда
  отсутствие лайфтаймов и, как следствие, правило 2.
- Реестр 221.1 №572 (этот дефект), №598 (заимствующий метод потребляет свой
  же приёмник — тот же корень: линейность живёт на именах), №571
  (`value`-тип в бокс не кладётся вовсе).
