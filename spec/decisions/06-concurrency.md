...
- **`Supervisor.on_child_fail` — D416§2 обратное измерение:** та же
  методика (64×20×5 раундов), но `on_child_fail` вместо `with Fail[T]`, тот
  же неатомарный `cnt = cnt + 1` — **0 потерянных обновлений** (счётчик
  точен на каждом раунде). Подтверждает: D416§2 сериализация — не просто
  формулировка спеки, а измеренный факт рантайма (пин-фикстура
  `supervisor_on_child_fail_serialized_pin_test.nv`).
...
ссылкой на измерение Ф.0 §1). **Исключение (D416§2):** эффект с именем
`Supervisor` пропускается по имени — `on_child_fail` сериализован на
drive-файбере scope'а (§2 D416, измерено §1 выше), гарантия запинена
`supervisor_on_child_fail_serialized_pin_test.nv` (неатомарный счётчик,
64×20 раундов, `assert(cnt == n)` на КАЖДОМ раунде — не просто печать
факта, как в исходной пробе Ф.0).
...
- **Точка (4) плана (именованный граф вызовов / `infer_effects`)** — план
  предполагал вариант «M:N-небезопасность» как транзитивное свойство,
  поднимаемое эффект-машиной через ИМЕНОВАННЫЕ вызовы произвольной
  глубины (аналог Rust `Send`, распространяемого через сигнатуры). Ф.3
  проверила: существующий `infer_effects` поднимает ТОЛЬКО объявленные
  эффекты сигнатуры (`Net`/`Fs`/`Db`/`Time`/`Detach`/…), у него НЕТ
  понятия «M:N-safe leaf» — это была бы НОВАЯ эффект-категория, отдельное
  окно. §3(а) вместо этого — call-site-специфичный механизм (проверка
  ИМЕННО у вызова функции с `spawn`-инвокацией СВОЕГО параметра), уже
  накрывающий обязательный критерий (repro B) и оба структурных риска
  scale150-аудита (`race2`, `Application.register_finalizer`,
  `BackgroundTasks.tasks` — 0 живых вызовов на момент аудита), но НЕ
  общий транзитивный граф произвольной глубины (`f` передаётся в `g`,
  `g` передаёт в `h`, `h` зовёт в `spawn` — 2+ хопа не отслеживаются).
- **Точка (6) плана (гейт на коэрсии fn→Handler, D52 стирание)** — НЕ
  реализована отдельным гейтом. §3(в) ловит handler-ЛИТЕРАЛ, написанный
  ПРЯМО на месте `with X = …` (100% реальных 28 сайтов — этой формы). Если
  `with X = some_precomputed_handler` (имя, не литерал, потенциально
  стёртое из обычного `fn`-значения через D52-коэрсию) — §3(в) НЕ
  прослеживает его до исходного замыкания (тот же принципиальный лимит,
  что у simple-`Ident`-случая в §3(а), только там `closure_free_vars`
  снэпшот частично закрывает именно `let`/`ro`-биндинг литерала; для
  handler-позиции этот снэпшот не подключён). Живых нарушителей этой формы
  не найдено — честно оставлено как известный, а не молчаливый пробел.
- **Класс «замыкание как ПОЛЕ структуры»** (`BackgroundTasks.tasks
  []fn()->()`, `ServerResponse.upgrade`/`.background` и т.п.) — НЕ
  проверяется: запись значения в поле записи/коллекции и последующее
  извлечение — на 2+ хопа глубже call-site-механизма §3(а). Структурный
  риск (scale150-аудит), не подтверждённое нарушение (0 живых
  mut-захватывающих сайтов на дату аудита) — остаётся риском, не забытым
  багом.
