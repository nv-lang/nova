...
- **Точка (6) плана (гейт на коэрсии fn→Handler, D52 стирание) — ЗАКРЫТО
  ...
  (D416§2) сохранён без изменений. **Транзитивная установка ЗАКРЫТА
...
- №168: `with`-handler в install-позиции resolvится через СНЭПШОТ, взятый
  ...
  отслеживаются».
- Класс «замыкание как ПОЛЕ структуры» (§5 выше) не тронут этим окном —
  остаётся структурным риском, не подтверждённым нарушением.
