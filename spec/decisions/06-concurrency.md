...
В спеке (`syntax.md`, `04-effects.md` D62):
- `let r = spawn { body }` имеет тип = тип `body`.
- Пример из D62: `let users = spawn fetch_users()` — `users: []User`.

Это создаёт **противоречие** с параллельностью `supervised`: если spawn
возвращает результат синхронно, scope не может ждать всех spawn'ов в
конце — все они уже завершились к моменту записи результата. **D71
разрешает противоречие через контекстно-зависимые типы:**

| Контекст | Что возвращает `spawn body` | Когда доступно значение |
|---|---|---|
| `let r = spawn { body }` **вне** `supervised` | type-erased `nova_int` (см. ниже) | сразу (eager-blocking) |
| `spawn { body }` **внутри** `supervised` | unit / `((nova_int)0)` placeholder | результат не возвращается caller'у |
| `spawn body` без `let r =` | unit | — |

**Type erasure (bootstrap-упрощение).** Поле результата в
`NovaSpawnCtx_X` — `nova_int _nova_result`. Все типы конвертируются
через `(nova_int)(value)`:

- `nova_int` → передаётся напрямую.
- `nova_bool` → 0/1.
- `nova_str` / records / arrays → pointer cast'ится в `nova_int` (lossy:
  тип теряется).
- Возвращаемое значение читается как `nova_int`, и call site
  использует его без приведения обратно — что **корректно только для
  int/bool результатов**.

`let r = spawn { "hello" }` сегодня даёт `r: nova_int = address`, не
`nova_str`. Это **известная регрессия type-system'а**, для bootstrap-
тестов хватает int-результатов.

**Внутри supervised** результат **не доступен caller'у** — placeholder 0.
Реальный обмен данными — через захваты `mut`-переменных или (в
будущем) через каналы:

```nova
let mut result = 0
supervised {
    spawn {
        result = compute()   // mut capture by-pointer
    }
}
// здесь result содержит значение
```

**Production-fix.** Полная реализация: вывести тип `body`, типизировать
поле `_nova_result` соответственно (T для scalar, T* для struct/string),
вернуть из spawn-call значение нужного типа. Внутри `supervised` —
открытый вопрос (handle с `.value()` blocking-read? handle, который
имплицитно блокирует при первом use? отказ от значения вообще?).
...
- **Тип результата `spawn`.** Сейчас `nova_int`-erasure (см. п. 2). Полная
  реализация — вывести тип body, типизировать `_nova_result` соответственно.
  Открытый под-вопрос: что возвращает `spawn body` внутри `supervised`?
  Варианты: (a) unit как сейчас + результаты через mut-захваты; (b)
  `Handle[T]` с blocking `.value()`; (c) implicit-await при первом use.
  Спека (syntax.md, D62) намекает на (c) для согласованности с примерами,
  но это «implicit await» = «цвет функции» что D62 запрещает. Не закрыто.
