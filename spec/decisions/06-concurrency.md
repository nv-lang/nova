...
## D455. Исход отмены выражен ЗНАЧЕНИЕМ области: `supervised(cancel:)` даёт `Outcome[T]` (2026-08-10)

> **Status:** accepted (решение владельца, 2026-08-10, вариант «C» из четырёх
> предложенных). Продолжает [D449](#d449) («срок как ЗНАЧЕНИЕ, а не как эффект»)
> и уточняет [D75](#d75-supervisedcancel-tok--структурная-отмена-с-внешним-токеном).
> Реализация — [план 266](../../docs/plans/266-cancellation-outcome.md), до тега
> v0.1.
>
> **Отклонение реализации от этого текста, временное (2026-08-11).** Код в
> ветке плана 266 несёт имена `Finished`/`Aborted`, а не `Done`/`Cancelled`.
> Причина — НЕ вкус: голый паттерн `Done(x)` в `match` резолвится по ИМЕНИ, а
> не по типу значения, и совпадает с `OnceState.Done`, захардкоженным в
> рантайме; проба дала молча неверный результат (`Done(42)` → `0`, чтение
> неинициализированной памяти). Это дефект компилятора — реестр 221.1 №567,
> К1, чинится до тега. **Решение владельца 2026-08-11: баг компилятора не
> определяет именование языка** — после фикса №567 имена ВОЗВРАЩАЮТСЯ к
> `Done`/`Cancelled`, как записано ниже. Отклонение зафиксировано здесь, чтобы
> временное не стало постоянным молча.

### Что

Область с именованным аргументом `cancel:` возвращает не `T`, а **исход**:

```nova
ro out = supervised(cancel: tok) {
    serve(listener)          // тело вычисляет T
}
match out {
    Done(v)   => println("завершилась сама: ${v}")
    Cancelled => println("отменена снаружи")
}
```

**Тип результата зависит от НАЛИЧИЯ `cancel:`**, и только от него:

| форма | тип результата |
|---|---|
| `supervised { … }` | `T` — как сейчас |
| `supervised(timeout: …) { … }` | `T`; истечение срока — `Fail[TimeoutError]` (D408) |
| `supervised(timeout: …, on_timeout: h) { … }` | `T`; срок переведён в значение обработчиком (D449) |
| `supervised(cancel: tok) { … }` | **`Outcome[T]`** = `Done(T) \| Cancelled` |

Прецедент формозависимости уже принят: `on_timeout:` меняет поведение области
самим своим присутствием (D449). Здесь то же правило, применённое к `cancel:`.

### Почему

1. **Сегодня исход не выражен вовсе.** Отмена ничего не бросает (в отличие от
   срока), а значение области при отмене неотличимо от значения при успешном
   завершении. Единственный способ узнать правду — вспомнить и вызвать
   `tok.is_cancelled()` ПОСЛЕ области (`06-concurrency.md:1275`, «читает флаг без
   yield, не throws»). Это инвариант на честном слове: ничто не заставляет
   вызывающего спросить, и ничто не краснеет, если он забыл.
2. **Из этого следует невозможность проверить корректное завершение.** Тест
   graceful shutdown сегодня может утверждать только «программа не упала».
   Утверждать «область была отменена и ресурсы закрыты» не на чем — и потому
   записи [№554](../../docs/plans/221.1-bug-sweep.md) (перехватывается только
   `SIGINT`) и фаза Ф.2 плана 265 (сигналы как эффект) упираются в отсутствие
   наблюдаемого исхода, а не в сами сигналы.
3. **Согласовано с уже принятым.** D449 постановил: истечение срока выражается
   ЗНАЧЕНИЕМ, а не эффектом. Отмена — событие того же рода (штатное завершение
   по внешней воле), и выражать её иначе значило бы держать два разных ответа на
   один вопрос «чем кончилась область».
4. **Платит только тот, кто попросил.** Без `cancel:` тип прежний, ни одна
   существующая строка не меняется. Это отличает решение от «пусть отмена
   бросает `Fail[Cancelled]`»: там платили бы все, включая тех, кто отмену не
   заказывал.

### Что отвергнуто и почему

* **`on_cancel:` по симметрии с `on_timeout:`.** Обработчику нечего
  перехватывать: отмена не бросает. Форма выродилась бы в «выполни это при
  отмене», то есть в очистку, — а очистка у нас уже есть и ровно одна:
  `defer`/`@cleanup`/`consume`. Два способа на одно не самообеспечиваются
  (довод D410, применён в D452).
* **Отмена бросает `Fail[Cancelled]`** (путь Kotlin `CancellationException`,
  Swift `CancellationError`). Штатное завершение сервиса начало бы выглядеть
  отказом; `Fail`, который положено ловить всегда, перестал бы быть сигналом
  беды. Плата ложится на весь код, а не на того, кто заказал отмену.
* **Оставить `tok.is_cancelled()` единственным ответом.** Это статус-кво, то
  есть инвариант на честном слове; см. «Почему» §1.
* **Отмена как событие управления** (продолжение после области не выполняется).
  Ничего не добавляет в типы, но и не оставляет вызывающему места сказать «если
  отменили — вот что делать»; для сервиса, которому надо дожать соединения и
  сообщить об этом, этого мало.

### Открыто (решается реализацией, но НЕ молча)

* **Сочетание с `Result`.** Если тело области уже возвращает `Result[T, E]`, тип
  становится `Outcome[Result[T, E]]`. Не вкладывать ли иначе — вопрос формы,
  ответ обязан быть записан амендментом, а не выбран реализацией.
* **Имя типа и вариантов.** `Outcome[T]` / `Done` / `Cancelled` — рабочее
  предложение. Если в прелюдии уже занято близкое имя, назвать иначе и записать.
* **Сочетание `cancel:` с `timeout:` в одной области.** Тогда исходов три:
  завершилась, отменена, истёк срок. Форма — за амендментом.

### Прецеденты

Rust: отменённость видна в ЗНАЧЕНИИ (`JoinError::is_cancelled`), а не в
исключении. Go: `ctx.Err()` — тоже значение, но требует опроса, то есть ровно
наш нынешний недостаток. Kotlin/Swift: исключение — путь, который мы отвергли
выше. [D449](#d449) — наш собственный прецедент «срок как значение».
