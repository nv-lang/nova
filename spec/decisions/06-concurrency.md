...
переезжает в эффекты, как `Throws`, `Net`, `Db`. Под капотом —
...
fn fetch_all(urls []str) Net Async Throws -> []Response =>
...
```nova
fn fan_out(urls []str) Net Async Throws -> []Response =>
    parallel for url in urls {
        fetch(url)
    }
// эффект Async, не Par
...
```nova
fn read_file_sync(path str) Blocking Throws[IoError] -> []byte =>
    blocking {
