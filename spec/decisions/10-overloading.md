...
#### Ось линейности `[T]` / `[T consume]` (амендмент 2026-09-25)

Объявления, различающиеся только маркером линейности параметров типа, — законная
пара, а не дубль; обычный `T` берёт форму `[T]`, must-consume — `[T consume]`.
Правило и довод, почему это не ранжирование бáундов, —
[D464, амендмент 2026-09-25](#амендмент-2026-09-25--ось-линейности-пара-t--t-consume-решение-владельца-план-246).
...
* Не вводит ранжирование по бáундам — намеренно (см. следствие 2). Единственное
  исключение — ось линейности `[T]` / `[T consume]`, амендмент 2026-09-25 ниже.
...
### Амендмент 2026-09-25 — ось линейности: пара `[T]` / `[T consume]` (решение владельца, план 246)

**Правило.** Объявления одного имени, различающиеся ТОЛЬКО маркером линейности
параметров типа — обычный `[T]` против `[T consume]`, в скобках носителя или в
скобках функции, при совпадении всего остального, включая протокольную часть
бáунда (`[T Display]` / `[T consume Display]`), — законная пара, а не дубль
[D84](#d84-перегрузка-функций-и-методов-четыре-оси-резолв-по-самому-специфичному-матчу).
Выбор:

1. **Обычный `T`** — обе формы проходят фильтр 2б; **доминирует форма с обычным
   `[T]`**.
2. **Must-consume `T`** — форма с обычным `[T]` фильтр 2б не проходит
   ([D156, амендмент 2026-09-24](02-types.md#d156) п. 1); применима единственная —
   `[T consume]`.
3. **Одна форма берёт всех, кого допускает её бáунд.** Есть только `[T consume]` —
   её берёт любой `T`. Есть только обычный `[T]` — его берёт только обычный `T`, а
   must-consume `T` получает ошибку бáунда (`E_CONSUME_TYPE_ARG_UNBOUNDED`, D156 п. 1):
   линейному `T` эта дверь закрыта.
4. **Несколько параметров типа** — доминирование по позиционному вектору, как у оси
   режима (реестр №857): форма побеждает, если ни в одной позиции не шире и хотя бы в
   одной у́же; доминирующей нет — `ambiguous overload` с перечнем кандидатов.
5. **Порядок с осью режима (уточнение владельца 2026-09-25).** Линейность решает
   ТОЛЬКО между формами, одинаковыми во всём остальном, и применяется ПОСЛЕ оси режима
   D84; в один вектор доминирования с режимом она не входит. Пример — три формы
   `fn Vec[T] @m(x T)`, `fn Vec[T consume] @m(x T)`, `fn Vec[T consume] mut @m(consume x T)`
   при обычном `T`: временное аргументом — ось режима берёт третью; `ro`-привязка —
   третья исключена правилом 3, из первых двух линейность берёт первую. Пара, различающаяся
   и линейностью, и режимом (`push(v T)` на `[T]` / `push(consume v T)` на `[T consume]`),
   решается осью режима; линейность у неё — только фильтр 2б (must-consume `T` не видит
   формы `[T]`).
6. **Обобщённый вызывающий (решение владельца 2026-09-25, вариант (а)).** Форма
   выбирается в чекере по бáунду, который вызывающий объявил, а не заново при
   мономорфизации: внутри `fn g[T consume](v Vec[T])` вызов `v.index_of(x)` берёт
   `[T consume]` — для `T` вызывающего не доказано, что он обычный, — и эта форма
   остаётся при любой подстановке, в том числе `int`. Внутри тела с обычным `[T]` —
   форма `[T]`. Цена названа: быстрая форма не работает в обобщённом коде над
   `[T consume]`; выигрыш — выбор делает чекер, кодоген читает (норма раздела «Где
   проверяется»).
7. **`#impl` на паре.** Протокол реализует ИМЯ метода типа, а не одна из форм: `#impl(P)`
   на любой форме пары делает тип конформным, и вызов через протокол выбирает форму по
   тем же правилам 1–6. Протокол, объявленный с обычным `[T]`, остаётся недоступен
   must-consume `T` (план 246, «Правки владельца 2026-09-25 (день)», п. 2).

**Обязанность автора пары.** Для обычного `T` обе формы дают одинаковый наблюдаемый
результат: форма `[T]` — специализация (быстрый путь: прямое чтение элемента вместо
вида через `view_at`, D216-амендмент 2026-09-25), а не другой смысл. Компилятор этого
не доказывает; держится фикстурой, сравнивающей обе формы на обычном `T`.

**Почему это не ранжирование, запрещённое следствием 2.** Следствие 2 запрещает
сравнивать ПРОТОКОЛЬНЫЕ бáунды: протоколов открытое множество, порядок по «у́же» —
частичный и требует рассуждать о следовании протоколов, на этом и растёт резолвер Swift
и спека C#. Ось линейности другая: у неё ровно два значения, и множество типов,
допускаемых обычным `[T]` (обычные, D156 п. 1), целиком вложено в множество
`[T consume]` (любые). Порядок полный, сравнение всегда определено и не зависит от
протоколов — это структурное правило того же рода, что «конкретное над generic». Сам
D464 держал этот переход открытым: «ошибка → ранжирование» позже возможен без ломки
программ; пары, отвергавшиеся как дубль, становятся законными, ни одна принятая
программа не меняет смысла.

**Что по-прежнему решает следствие 2** — всё, где различается протокольная часть:
`[T]` против `[T Display]`, `[T Display]` против `[T Debug]`, и смешанная пара
`[T consume]` против `[T Display]` — для обычного `Display`-типа это неоднозначность.

**Где проверяется.** В чекере, как весь фильтр 2б: выбранная форма уходит в канал,
кодоген её читает. Реализация отсева обычного `[T]` для must-consume `T` не опирается
на отказ D156 п. 1 (оракул его не проверяет, решение владельца 2026-09-25 — проверку
делает Карина): выбор обязан исключать форму `[T]` сам.

**Первый потребитель** — поиск и сравнение `Vec` (`contains`, `index_of`, `position`,
`rposition`, `is_sorted`, `is_sorted_by`, `binary_search`, `binary_search_by`,
`binary_search_by_key`): общая форма `[T consume]` смотрит элементы видом, быстрая `[T]`
читает напрямую (план 246, «Правки владельца 2026-09-25 (день)», п. 3).
