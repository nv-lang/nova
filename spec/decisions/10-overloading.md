...
> **Статус:** ✅ IMPLEMENTED (Plan 184 заход-6, 2026-07-07). Режим параметра — ось
> перегрузки наравне с receiver-mut (прецедент Plan 135). Факты реализации:
...
3. **consume-версия** участвует в резолве **ТОЛЬКО когда аргумент в последней точке
   ...
   *Реализация (заход-6):* consume-версия выбирается для **owned-временного** аргумента
   ...
   `is_rvalue_temp`), где ни один живой биндинг не потребляется молча. Именованный биндинг
   в его последней точке (полный last-use через consume-чекер) — расширение внутри того же
   правила (маркер `[M-184-consume-dispatch-named-lastuse]`); сейчас такой аргумент идёт в
   ro/mut-версию, а явное `consume`-владение выражается конструктор-/вызов-формой.
