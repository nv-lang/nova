...
| D226 | 02-types.md | Value-record allocation contract (Plan 124.8 Ф.2/Ф.4, 2026-06-02) — `type X value { ... }` stack-allocated reference type с copy-on-pass semantics. `value` — contextual keyword (Ident-based match, backward compat для variables/fields named 'value'). Composable с consume/priv в любом order (canonical: value consume priv). V1 codegen — heap-backed transparent fallback (semantic identical); V2 [M-124.8-value-codegen-stack] для proper stack allocation. Plan 124.8 amends 6 D-blocks (D33/D52/D175/D176/D215/D222) + retract D225 + D226 NEW. 23/23 plan124_8 fixtures PASS (16 positive + 7 negative). Industry alignment: Kotlin/Java Valhalla `value class` pattern + Nova edge — single declaration syntax с explicit allocation modifier (unique in industry). |
