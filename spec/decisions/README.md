...
| D223 | 02-types.md | Priv-escape hatches `#test_access` + `#visible_to` (Plan 124.6, 2026-06-02) — fn-level `#test_access(TypeX, TypeY...)` grants priv access body fn ко всем listed types (test escape hatch); field-level `#visible_to(OtherType)` grants methods OtherType access ко marked priv field (per-field friend declaration). Combined predicate `priv_field_access_allowed(tname, &visible_to)` — type-method-scope OR test_access OR visible_to. RAII `PrivTestAccessGuard` tracks current_fn_test_access. 7/7 plan124_6 PASS (4 positive + 3 negative). Reuses Plan 124.1-124.4 error codes; hints mention escape hatches |
| D224 | 02-types.md | Type-level priv flip для named tuples (Plan 124.7, 2026-06-02) — `type Secret priv (key str, salt str)` symmetric extension D220 §3.3.1 на named tuple form (D215). Existing `TypeDecl.default_field_priv` re-used; new parser shim `parse_named_tuple_fields_with_default(default_priv)` propagates type-level default к per-field resolution. Explicit `pub` overrides; `priv pub`/`pub priv` → E_PRIV_PUB_CONFLICT. 8/8 plan124_7 PASS (5 positive + 3 negative); regression Plan 120 + plan124_1/4 unchanged. **Plan 124 umbrella now FULLY CLOSED** (all 7 sub-plans 124.1-124.7) |
