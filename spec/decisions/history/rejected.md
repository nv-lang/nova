...
### `a ?? |err| ... ?? 1` — замыкание с доступом к ошибке в fallback
**Отвергнуто 2026-08-30** (предложено владельцем при разборе вопроса
«правильно ли, что `??` теряет ошибку на `Result`»). Две причины, вторая
замерена ЗАПУСКОМ, а не выведена рассуждением:

1. **Это возврат `unwrap_or_else`**, ретрактированного 2026-07-07 как дубль
   оператора ([D86 AMEND](../04-effects.md#d86), «ниша … 0 использований»).
2. **Форма МОЛЧА ЗНАЧИТ НЕ ТО, ради чего предлагается.** `??`
   правоассоциативен (замерено, см. таблицу приоритетов в D38), а тело
   замыкания — выражение. Значит `a ?? |e| f(e) ?? 1` разбирается как
   `a ?? (|e| (f(e) ?? 1))`: хвостовой `?? 1` уезжает ВНУТРЬ замыкания, и цепочка,
   ради которой форма и нужна, без скобок означает другое.
   **Замер 2026-08-30:** `ro r = a ?? |e| 5 ?? 1` при `a = None` собирается, и
   `println("r=${r}")` печатает УКАЗАТЕЛЬ (`r=140701139968008`) — то есть `r`
   оказался ЗАМЫКАНИЕМ, а не числом. Правдоподобно и неверно — худшее
   свойство синтаксической формы.

Где живёт ниша «ошибка нужна»: `@flat_map_err` (`Result` → `Result`),
`match` (`Result` → `T`), `@ok()` (выбросить причину СЛОВОМ, а не молчанием).
См. [D86](../04-effects.md#d86), амендмент 2026-08-30.
...
двоеточие опускается. См. [D17](../02-types.md#d17), `spec/syntax.ru.md`.
...
**Отвергнуто.** Прецеденты — Python, Kotlin, Swift, C++. Привлекательно
эргономикой, но ломает несколько принципов Nova:

1. **«Всё видно в сигнатуре».** Дефолт не виден на call-site —
   читатель не знает, какое значение использовалось. Анти-AI-friendly.
2. **«Один очевидный путь» (D9).** `f(x)` или `f(x, default_y())` —
   оба валидны, выбор путает.
3. **Refactoring fragility.** Менять дефолт = тихое изменение
   поведения у всех вызывающих, кто «забыл» передать.
4. **Конфликт с overloading (D46).** Резолюция перегрузки усложняется
   при default-значениях.
5. **Конфликт с контрактами (D24).** `requires`/`ensures` зависят от
   значений — SMT-кодировка усложняется.
...
Все опции **видны на call-site**, дефолты переиспользуются между
средами, refactoring безопасен. См. [syntax.md → «Опциональные
параметры»](../../syntax.md).

Если когда-то вернёмся — только в named-form вызова и только для
public API, отдельное D-решение.
