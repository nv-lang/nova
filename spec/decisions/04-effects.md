...
> **AMEND (2026-09-25, D25; номер и форма — интегратор, текст — окно Карины):
> `break` и `continue` — выражения типа `never`.** Черновик лежал с 2026-08-19
> (`docs/plans/wip/amend-never-break-continue.md`, замер всех четырёх форм `??`)
> и влит этой правкой; черновик удалён — один дом.
>
> 1. **Тип.** В точке, где стоит `break` или `continue`, выражение имеет тип
>    `never` — наравне с `throw`, `panic`, `return` и `interrupt`. Значит оно
>    совместимо с любым ожидаемым типом.
> 2. **Позиция не ослабляется.** `never` не отменяет правила «только внутри тела
>    цикла» (`for`/`while`/`loop`): `break`/`continue` вне цикла — ошибка
>    компиляции `E_BREAK_OUTSIDE_LOOP`. Тело операции handler-литерала — отдельная
>    функция: цикл вокруг `with` для него не цикл.
> 3. **Значение цикла — [D485](03-syntax.md#d485).** Если `break` несёт значение
>    (`break e`), оно уходит в тип ЦИКЛА, а не в позицию, где написан `break`;
>    тип самого `break e` остаётся `never`. Этот пункт от D485 не зависит и правки
>    при её изменении не требует.
> 4. **Следствие для D86 — без правки D86.** `x ?? continue` и
>    `x ?? break` законны по уже существующему правилу «fallback типа `never`».
> 5. **`return` при `??` остаётся ретрактированным** (AMEND 2026-07-23 к D86). Это
>    не противоречие: `return` отклонён не по типу, а потому что у проброса есть
>    канон (`X?`, `.ok_or(..)?`); у «пропустить итерацию при `None`» такого канона
>    нет. Пример `lookup(id) ?? return …` выше иллюстрирует ТИП `never`, а не
>    законность формы.
> 6. **Что становится законным сверх `??`:** `if c { continue } else { v }` в
>    позиции значения, рука `match` вида `A => continue` рядом с руками-значениями
>    — любая позиция, где `never` уже допустим для `throw`.
>
> **Реализация.** Карина: позиция и тело руки `match` — волна 274.7 В10-б
> (2026-09-25); `never` в любой позиции значения — часть 3 В10, тем же слиянием,
> что D485. Оракул формы в позиции выражения не читает (`x ?? continue` —
> «unexpected `continue` in expression», замер 2026-08-19); релиз оракула на паузе,
> расхождение — в пользу нормы.
...
> ⚠️ **ПЕРЕКРЫТО [D85](#d85-операторы--и----унифицированное-поведение-для-result-и-option)** (пометка 2026-08-17 по аудиту самосогласованности; подтверждено тремя скептиками). Текст выше описывает СНЯТУЮ семантику «`?` ⇒ throw, требует `Fail[E]` в сигнатуре». D85 переопределил `?` как **return-стиль без эффектов** (`Ok(v) => v`, `Err(e) => return Err(e)`), и это ЭНФОРСЕНО: в функции с эффектом `Fail` оператор `?` запрещён диагностикой `E_TRY_IN_FAIL_FN` — там `!!` или `throw`. Старый текст сохранён как история: он объясняет, почему `Fail` вообще связан с пробросом, но нормой БОЛЬШЕ НЕ ЯВЛЯЕТСЯ.
...
> **✅ ENFORCED (Plan 221.1 п.11, №428, 2026-08-07): «Fail транзитивен через
> границы вызовов» (§Правило 2 выше) означает буквально **резолвленное
> свойство callee**, не «текст `!!`/`throw` дословно виден в теле САМОЙ
> `export fn`». Найдено: `export fn api(x int) -> int => level1(x)` PASS'ил
> `nova check` без единой ошибки, хотя `level1` зовёт `level2` зовёт
> `level3`, и только `level3` несёт `risky(x)!!` — ни `level1`, ни `level2`
> НИКОГДА не декларировали `Fail`, а энфорс (`check_bang_requires_fail_
> block`/`_expr`, №113) искал буквальный `ExprKind::Bang` в СИНТАКСИСЕ
> ТЕЛА `api`, никогда не смотрел на эффекты вызываемых функций. Тот же
> провал — для callee, УЖЕ явно объявившего `Fail[E]` в своей сигнатуре:
> `export fn api() -> int => declared_fail(x)` (без `?`/`!!` на месте
> вызова вовсе) ТОЖЕ PASS'ил — старый чекер не читал `callee.effects`
> НИКОГДА, ни при каком hop-count'е. «Явная декларация обязательна»
> (§Правило 2) была декорацией для любой формы, кроме буквального `!!`
> в собственном теле `export fn` — тот же класс дыры, что №131 закрыл
> для raw effect-op call (см. блок выше), но для Fail-транзитивности.
>
> **Механика:** `fail_reach.rs` (`compiler-codegen/src/types/`) —
> отдельный дополнительный проход `check_module_impl` (`mod.rs`, рядом с
> `fiber_safety::run`), запускается ПОСЛЕ финализации `resolved_callees`
> (§0/196 чекер-канал: методы/generic-dispatch резолвятся ТАК ЖЕ, как
> у `fiber_safety`, не именем-строкой). Вычисляет least fixed point
> `requires_fail(F)` над ЛОКАЛЬНЫМ графом вызовов: fn `F` «требует Fail»,
> если (а) её СОБСТВЕННАЯ сигнатура уже несёт `Fail[…]`, ЛИБО (б) её тело
> (включая ВЛОЖЕННЫЕ closure/handler-литерал-опы — эффект-capability
> которых, по D28 «Что такое эффект-capability», это capability
> ОКРУЖАЮЩЕЙ функции, не своё собственное) содержит недогашенный `!!`,
> ЛИБО (в) её тело зовёт (транзитивно, любая глубина) другую локальную
> `F`, для которой `requires_fail` уже true — если только тот КОНКРЕТНЫЙ
> вызов не обёрнут локальным `with Fail = … { … }` (Правило 2
> «Альтернатива через with» выше, погашение НЕ меняется этим
> амендментом). Для `export fn` это ТЕПЕРЬ hard error
> `[E_BANG_REQUIRES_FAIL]`, если `requires_fail(fn)` истинно, а
> `fn.effects` не несёт `Fail`. Приватная граница (D28 auto-inference,
> §Правило 2 «экономика приватных fn») НЕ ТРОНУТА этим амендментом —
> приватная fn по-прежнему остаётся под молчаливым выводом; правится
> ИСКЛЮЧИТЕЛЬНО граница `export`, ровно как и раньше задумывал №113.
> `check_defer_bodies`' (D158) собственный one-hop `fn_effects`-lookup
> имел ТУ ЖЕ дыру для `defer`/`errdefer` тел — закрыт тем же fixed
> point'ом (`fail_reach::requires_fail_by_name`, имя-based вариант — этот
> чекер-проход запускается РАНЬШЕ в пайплайне, до готовности
> `resolved_callees`, поэтому резолвит вызовы по имени, тем же способом,
> что уже использует `call_target_name`/`fn_effects`, не regression по
> точности).
>
> **Находка (НЕ пофикшено этим слиянием — доложено интегратору):**
> ужесточение вскрыло РЕАЛЬНУЮ, ранее невидимую дыру в `std`: `std/src/
> encoding/json.nv`'s приватный `Lexer @char_at` (`.next()!!` на
> `Option[char]`, строка 458) транзитивно достижим из ЭКСПОРТНОГО
> `Json.parse`/`JsonValue.try_from` (через `Parser.new`→`parse_value`),
> которые публично НЕ объявляют `Fail` — если инвариант «`p` всегда в
> границах» когда-либо нарушится, паника уйдёт наружу из `Json.parse`
> без единого предупреждения в его подписи. Транзитивно задевает
> `std/src/crypto/jwt.nv`, `encoding/serde/json.nv`, и (через folder-
> module/prelude merge) любой compile-unit, который где-либо касается
> `Json.parse` — включая `examples/flagship/aggregator`. Сама природа
> находки — РАЗ (один корень, `Lexer.char_at`), но радиус — широкий;
> исправление публичного API `Json`/`Jwt` — отдельная волна, не в
> объёме №428 (задача этого окна — чекер, не `std`-код).
>
> **Кандидаты того же класса, ПРОВЕРЕНЫ, НЕ пофикшены этим слиянием**
> (см. `docs/plans/wip/PROGRESS-p428.md` за полной таблицей): raw
> effect-op транзитивность (№131, `check_raw_effect_op_declared`) —
> та же дыра, подтверждена пробой, требует ТОЙ ЖЕ архитектуры,
> обобщённой на произвольное имя эффекта (не только `Fail`) — оставлено
> отдельным окном. `E_UNDECLARED_TRANSITIVE_EFFECT` (`--strict-effects`,
> §Правило 1) — уже читает `callee.effects` (properly property-based), но
> та же неполнота АДРЕС того же корня («частные fn эффекты не всегда
> транзитивно выведены») — другой, смежный дефект, не «форма вместо
> свойства». `E_DETACH_REQUIRES_EFFECT` (D50) — НЕ тот же класс: энфорс
> безусловен для ЛЮБОЙ fn (приватной или нет), несущей `detach { }`
> буквально в теле — обхода через приватный helper НЕТ структурно,
> фикс не нужен.
...
4. **`RuntimeError` sum** даёт типизированный set встроенных ошибок.
   ...
   ломает существующие caller'ы — **ОШИБКОЙ КОМПИЛЯЦИИ**, не
   предупреждением. Это **фича** — программист обновляется консистентно.

   > ⚠️ **АМЕНДМЕНТ 2026-08-16 (решение владельца; реестр №703).** Здесь
   > стояло «через non-exhaustive match **warning**» — ослабление, которое
   > противоречило поверхностной спеке (там исчерпаемость — ошибка) и
   > которое реализация не исполняла ВОВСЕ: непокрытый вариант проходил
   > `check` и `build` и давал ТИХО неверный результат (`-> str` возвращал
   > пустое, код 0). Найдено окном 274, воспроизведено интегратором.
   > **Почему именно ошибка, а не предупреждение — довод о необратимости**
   > (тот же, что в [D464](10-overloading.md)): «ошибка → предупреждение»
   > ослабить позже можно, ни одна программа не сломается; «предупреждение
   > → ошибка» ломает чужой код и потому не делается никогда — язык
   > остался бы с тихой порчей навсегда. Плюс: предупреждение бесполезно
   > по природе отказа (результат — мусор, а не подозрительный код), и на
   > «это ошибка» уже опирается конвенция novac П21 («ветка «всё остальное»
   > на закрытом множестве запрещена: новый вариант уедет молча»).
   > Реализация — чекер-канал плана 196; приёмка по №703: исчерпаемость на
   > ВСЕХ закрытых множествах (сумма, type-set, `is T`-матч), neg-фикстура
   > на каждую форму, диагностика ПЕРЕЧИСЛЯЕТ непокрытые варианты
   > (прецедент rustc E0004).
...
> ---
>
> **AMEND (2026-09-01, решение владельца): ретракция выше СУЖЕНА, а не
> отменена. Форма `X ?? return R` ЗАКОННА там и только там, где
> возврат объемлющей функции — НЕ `Option`/`Result`.**
>
> Основание ретракции было ОДНО и измеренное: форма была ВТОРОЙ
> ДВЕРЬЮ к [`?`](#d85-операторы--и--унифицированное-поведение-для-result-и-option-throw-стиль-через-) — 12 из 15 сайтов
> корпуса имели канон короче. **Где функция возвращает не `Option`/`Result`,
> первой двери не существует:** `?` нечего пробрасывать наружу, и
> основание к этим местам не относится. Это та самая последняя строка
> таблицы выше (`bool`, кортеж и т.п.), где стояло «явный `match` — законен,
> не дефект»; теперь там законны ОБЕ записи.
>
> **Замер, на котором стоит этот амендмент (`novac`, 2026-09-01).** В самом
> языке, пишущем себя, шаблон встречается в ТРЁХ формах обхода, и ни одна
> из них не стоит в функции с обёрткой:
>
> * **11** ручных `match … => return` (`check/calls.nv`, `check/match_arms.nv`,
>   `check/methods.nv`, `sem/harvest.nv`) — все в `-> bool` и `-> ()`;
> * **9** обходов через ФИКТИВНОЕ ЗНАЧЕНИЕ: `?? no_ty()` (где `no_ty()` —
>   `TyId(-1)`), а следом сравнение с тем же sentinel'ом и `return`
>   (образец — `check/binds.nv:171`);
> * **2** таких же обхода с ФИКТИВНЫМ КОРТЕЖЕМ: `?? (no_vrow(), 0)`, затем
>   `if vr.1 == 0 { return false }` (`sem/mangle.nv:102`, `check/match_arms.nv:79`).
>
> Вторая и третья формы важнее первой: запрет НЕ убрал шаблон, а вытеснил
> его в форму, которую владелец сам правил вручную 2026-08-21 и назвал
> в коде (`novac/src/sem/slots.nv:460`) «a detour that turns "absent" into a
> number to be remembered». Фиктивное значение хуже ручного `match`: оно
> живёт в обычной переменной и утечёт дальше, если проверку забыли, —
> то есть ретракция в этой части обменяла ясную строку на молчаливый риск.
>
> **Граница разрешения, дословно.** Форма законна, когда ОДНОВРЕМЕННО:
> операнд `??` — `Option`/`Result` (иначе сам `??` — ошибка типов); возврат
> объемлющей функции — НЕ `Option`/`Result`; ни один из двух типов НЕ
> упоминает generic-параметр. **Generic отвергается НАМЕРЕННО:** под `T`
> может приехать `Option`, и тогда форма снова становится второй дверью —
> ровно тем, что ретракция снимала; разрешать там — отдельное решение со
> своим замером.
>
> **Семантика.** `X ?? return R` вычисляет `X`; на `Some(v)`/`Ok(v)` даёт `v` и
> вычисление продолжается; на `None`/`Err(_)` выполняет `return R` из
> ОБЪЕМЛЮЩЕЙ функции. Ошибка `Err(e)` ОТБРАСЫВАЕТСЯ — как и у обычного
> `??`; если `e` нужна внутри `R`, пишится `match`, а не эта форма. Ранний
> выход НЕ обходит ничего из того, что делает обычный `return`: кодоген
> строит `Stmt::Return` и отдаёт его штатной эмиссии, поэтому `defer`,
> `errdefer` и `ensures`-контракты работают (проверено пробой с `defer`:
> обе ветви печатают defer-строку).
>
> **Что НЕ меняется.** Таблица замен выше действует для всех первых
> четырёх строк без изменений — там `X ?? return R` по-прежнему
> отвергается `E_COALESCE_RETURN_FALLBACK` с машинно-применимой подсказкой.
> Остальные три fallback-формы (значение, `throw`, `panic`) — без изменений.
> Линт `W_MANUAL_COALESCE` продолжает ловить ручной identity-`match` как дрейф
> от `X ?? D` — его предмет (fallback-ЗНАЧЕНИЕ) с этим амендментом не
> пересекается.
...
**АМЕНДМЕНТ 2026-08-30: это РЕШЕНИЕ, А НЕ НЕДОДЕЛКА** — абзац выше читался
как ограничение реализации, и вопрос «не сделать ли `??` только для `Option`»
всплывал повторно (владелец, 2026-08-30). **`??` остаётся и для `Option`,
и для `Result`.**

*Откуда взяты основания (чтобы читатель нашёл контекст, а не искал его зря):*
*разбор с подсчётами по корпусу пришёл от агента-координатора, пересланный
*владельцем; НЕ от окна 274 — оно само это опровергло, и владелец подтвердил.
*Сообщение коммита `d97b71c48` называет источником окно 274 — это НЕВЕРНО;
*историю не переписываем, поэтому поправка стоит здесь. На сами основания это
*не влияет: каждое из них перепроверено здесь чтением дерева и запуском
*(адреса `core.nv:229` и `:247`, счёт по корпусу, разбор замыкания — сборкой).* Основания, все четыре — замеренные, а не вкусовые:

1. **Отбрасывание пишет АВТОР, а не компилятор.** `??` в исходнике есть
   явное «причина мне не нужна». Класс «молчание читается как успех» (№770)
   сюда не относится: там молчит МЕХАНИЗМ, здесь говорит человек.
2. **Единственный донор с ОБЕИМИ обёртками — Rust — делает так же:** `unwrap_or`
   есть и у `Option`, и у `Result`, с той же потерей причины. C#/Swift/Kotlin
   ограничены `null` НЕ по суждению, а потому что `Result` у них не в ядре языка;
   записано здесь именно чтобы этим больше не аргументировали.
3. **Дыры в возможностях нет, и у каждого случая СВОЯ дверь:**
   `Result` → `Result` с чтением ошибки — `@flat_map_err` (`std/src/prelude/core.nv:229`,
   заведён решением владельца 2026-08-01, замкнув симметрию
   `map`/`map_err` ↔ `flat_map`/`flat_map_err`); `Result` → `T` из ошибки — `match`,
   и он законен: во всём корпусе таких сайтов ПЯТЬ (три — `panic("... ${e}")`,
   два — тест, возвращающий ошибку значением); а «выбросить причину СЛОВОМ» —
   `@ok()` (`core.nv:247`, 49 употреблений), он уже стоит в таблице замен этого блока.
4. **Цена правки несимметрична выгоде.** Окно для несовместимого изменения
   закрывается тегом v0.1; `??` в корпусе — порядка 240 сайтов, а выгода была бы
   стилистической.

**Что здесь НЕ решается и решаться не может.** Сегодня `??` ВООБЩЕ не проверяет
типы операндов: `ro n = 5` и `n ?? 99` проходит `check`, собирается и печатает 99
(реестр №839, блокер тега). Значит само различение «Option или Result» сегодня
НИЧЕМ НЕ ПРИНУЖДАЕТСЯ, и то, какой из двух ответов верен, будет ЗАКОДИРОВАН
ФИКСОМ №839. Поэтому решение принимается ДО фикса, а не после: фикс обязан
требовать слева `Option` ИЛИ `Result`, а справа — ЗНАЧЕНИЕ нагрузки.

#### Сравнение `?`, `!!`, `??`

| Оператор | На `Some(v)` / `Ok(v)` | На `None` / `Err` | Эффект |
|---|---|---|---|
| `expr?` | разворачивает в `v` | early-return из enclosing fn ([D67](#d67--оператор-семантика-для-result-через-fail-для-option-через-ранний-return)) | **без эффекта**; требует, чтобы enclosing fn возвращала `Result`/`Option` ([D85](#d85-операторы--и--унифицированное-поведение-для-result-и-option-throw-стиль-через-), `E_TRY_IN_FAIL_FN`) |
...
> **Амендмент 2026-08-17 (аудит самосогласованности, реестр 221.1 №442).** Ячейка «Эффект» у `expr?` до сегодня читалась «требует `Fail[E]` если `expr` это `Result`» — это семантика ДО [D85](#d85-операторы--и--унифицированное-поведение-для-result-и-option-throw-стиль-через-). D85 сделал `?` return-стилем: эффекта нет вовсе, а `?` внутри функции, объявившей `Fail`, — ошибка `E_TRY_IN_FAIL_FN`. Строка `expr!!` НЕ меняется: `!!` остаётся throw-стилем и `Fail[E]` действительно требует. Здесь текст ИСПРАВЛЕН, а не помечен: таблица нормативна и читается как справка — пометка рядом с неверной ячейкой оставляет неверную ячейку.
...
**Статус:** закреплён 2026-07-15 (owner sign-off, Plan 206 наблюдение 2026-07-14 + решение 2026-07-15). Ф.2 (`checked_*`/`saturating_*`/`wrapping_*` бланкеты + миграция overflow-зависимого std-кода) landed той же волной 2026-07-15 — см. §R4. Amends [D310](02-types.md#d310-type-set-bounds-plan-1723) (`Ints` — единый бланкет для ВСЕХ signed+unsigned членов; изначально формулировалось как exemption от `E_TYPE_SET_MIXED_SIGNEDNESS`, но D310 amendment Plan p424 2026-08-07 сняло то правило целиком — §R1 ниже переформулирован), расширяет [D13](#) (trap-дефолт `+`/`-`/`*`, ранее только для безграничного `int`) на ВСЕ sized-типы. `div`/`neg`/`mod` — вне рамок, вынесены в подплан [206.1](../../docs/plans/206.1-div-neg-trap.md).
...
**(R1) Type-set `Ints`.** `std/src/prelude/protocols.nv`: `type Ints set i8|i16|i32|i64|int|u8|u16|u32|u64|uint` — полное объединение `SignedInts ∪ UnsignedInts`, единый бланкет для ВСЕХ members: per-member мономорфизация (D310 §«Семантика тела») уже резолвит `T.MAX`/`T.MIN` per-instance, и знак-агностичные сравнения (`rhs < 0`) для unsigned членов константно `false` — никакого междоменного сравнения не возникает. Тот же случай, что иллюстративный `AnyNumber` в самом тексте D310.
> **Амендмент (D310 amendment, Plan p424, 2026-08-07, я):** на момент этого решения (2026-07-15) D310 запрещало ЧАСТИЧНЫЙ signed/unsigned микс (`{i32, u32}` и т.п. — не полный `Ints`), и `Ints` было единственным явно разрешённым исключением («full-union exemption», проверялось в `E_TYPE_SET_MIXED_SIGNEDNESS`). Сама эта норма — R1's обоснование выше — стала решающим доводом СНЯТЬ правило целиком: `{i32,u32} ⊂ Ints`, и если тело корректно мономорфизуется по всем десяти членам `Ints` (без цросс-доменных сравнений — именно это доказывает R1), оно тем более корректно по любым двум из них. D310 amendment убрало запрет и код диагностики; `Ints` продолжает существовать без изменений (полный список тот же), но больше не единственный легальный способ смешать знаковость — частичный микс (`set i32 | u32`) теперь тоже компилируется. D423 (эта запись) не меняется по существу — `Ints`/`checked_add`/trap-политика остаются нормативными как были.
...
> **Амендмент 2026-09-04 (решение владельца; см. D405, амендмент того же дня).** «Unsigned `-x` НЕ трогается» заменено: унарный минус определён только для знаковых типов, `-x` при беззнаковом `x` — ошибка компиляции (путь `-(x as iN)`), до кодогена; заворот по модулю не производится. Норма — в D405; здесь только пометка, чтобы у правила не было двух домов.

**(R3) `.nv`-бланкеты политик (`std/src/prelude/protocols.nv`, `fn[T Ints]`, ЕДИНЫЙ бланкет и для signed, и для unsigned — как в D423 §R4, НЕ split SignedInts/UnsignedInts).** `@overflowing_div`/`@overflowing_neg`-компиляторный интринсик (D423 §R2 паттерн) сюда НЕ применяется — у деления/negation нет аппаратного флага, поэтому бланкеты написаны напрямую на `.nv`, не как обёртка над generic-интринсиком:
...
## D430. Проверяемое числовое сужение — `to_<T>` чейн-семья + `RangeError` ([M-numeric-try-narrowing], 2026-07-20)
...
**Форма:** `(300u32).to_u8() -> Result[u8, RangeError]` — метод на ИСХОДНОМ значении (владелец, 2026-07-20: чейн, не static-конструктор на целевом типе). Имя: приставка `try_` = проверяемая версия, `to_<T>` = целевой тип — тот же принцип именования, что `str.to_i8`/`to_u16` (Plan 174.1/numeric-parity-2) и `checked_*`-семья (D423). `as` остаётся быстрым обрезающим кастом, без изменений.
...
> **АМЕНДМЕНТ 2026-09-04 — (R6) `to_<T>` для float (решение владельца; исследование
...
> `fn[S Floats] S @to_<T>() -> Result[T, RangeError]` для каждого целого `<T>` из
...
> часть усекается к нулю, затем проверяется диапазон цели: `(2.5).to_i16() == Ok(2)`,
> `(-2.5).to_i16() == Ok(-2)`, `(70000.5).to_i16() == Err(AboveMax)`,
> `(-1.0).to_u16() == Err(BelowMin)`, `+∞` → `Err(AboveMax)`, `-∞`/NaN → `Err(BelowMin)`. Это зеркало `as`
...
> `x.5`, NaN, ±∞ для `f32` и `f64`. Пока семья не заведена, вызов `f.to_i16()` на float
...
> **АМЕНДМЕНТ 2026-09-05 — (R7) имя `to_<T>`, ошибка `RangeError enum AboveMax | BelowMin`
> (решение владельца; исследование `docs/dev/research/2026-09-04-numeric-widening.md` §5.7).**
>
> **Имя.** Семья переименована `try_to_<T>` → `to_<T>`. Правило R3 конвенций (D325-семья,
> `nv-coding-style.md`): префикс `try_` — только чтобы отличить фаллибельный вариант
> одноимённого инфаллибельного; в одиночной фаллибельной операции префикса нет, `Result` сам
> сообщает о фаллибельности (R2). Инфаллибельного `@to_u8() -> u8` на целых не существует — его
> роль у `as`, — значит `try_` здесь был лишним, и весь живой std это подтверждает: `str @to_int()
> -> Result`, `str @to_i8() -> Result`, `int @to_char() -> Result`, `str @to_url()` — все
> фаллибельные, все без `try_`. Этот же блок ссылался на `str.to_i8` как на прецедент имени —
> прецедент говорил против префикса. Переименованы все вхождения в спеке, std и фикстурах (как
> амендмент D310 2026-08-17 переименовывал `SignedInt` → `SignedInts`): `(300u32).to_u8()`,
> `c.to_u8()`, `f.to_i16()`.
>
> **Ошибка.** `RangeError` был unit-типом («факт без данных»). Критерий R4: «больше одной причины
> отказа или вызывающему нужна причина → `Result`; единственный исход „нет" → `Option`». Причин у
> сужения две, и вызывающему они нужны: значение **выше** максимума цели и **ниже** минимума
> (отрицательное в беззнаковое — частный случай второй). Записывается
> `type RangeError enum AboveMax | BelowMin`: `(300u32).to_u8() == Err(AboveMax)`,
> `(-1i32).to_u8() == Err(BelowMin)`, `(70000.5).to_i16() == Err(AboveMax)`, NaN/±∞ у float —
> `AboveMax` для `+∞`, `BelowMin` для `-∞`, NaN — `BelowMin` (не число — не влезает ни с какой
> стороны; выбор произволен и записан, чтобы не было двух). Полезной нагрузки с типом источника
> нет намеренно — она сделала бы ошибку generic. Имена вариантов длинные (`AboveMax`, не `Above`),
> чтобы не сталкиваться голыми именами вариантов с чужими суммами в одном CU (реестр №136).
> `?`/`.ok()`/`match` работают как прежде; `Err(RangeError)` как паттерн больше не существует.
>
> **Рядом, решение отдельное:** `CharFromError` (`int @to_char()`) — тот же unit-тип с тремя
> причинами (выше `0x10FFFF`, ниже `0`, суррогат `D800–DFFF`); предложена та же форма
> `enum AboveMax | BelowMin | Surrogate`, см. исследование §5.7.

### Границы

**Известный, НЕ новый разрыв, задетый при написании тестов:** generic type-set-bound бланкет-метод (`fn[S Ints] S @to_<T>()`), вызванный на receiver'е, который сам — bound переменная `for`-цикла (`for v in vec { v.to_u8() }`), падает в pre-existing `[P67-LEGACY]` "method call return type unknown" (тот же класс, что уже задокументирован в заголовке `std/src/math/overflow_policy_test.nv` для inline-conversion-call receiver — здесь тот же checker-gap для другого receiver-shape, `for`-bound Ident, а не Call). НЕ specific для `to_*` (общий generic-blanket-dispatch разрыв, вне зоны этой волны) — обходится в тестах чтением элемента в типизированную `ro`-локаль перед вызовом (тот же обход, что весь остальной файл уже применяет). Followup НЕ заводился отдельно — покрыт существующим общим "generic-blanket receiver must be typed local" классом (`[P67-LEGACY]`, см. D423/D427 §«Известные разрывы»/§R4.3).
...
## Amend D431 — `E_DEFAULT_HANDLER_SUSPEND`: fabric construction-time yield-point ban (№246, [M-default-handler-factory-yield-point-ban], owner decision 2026-08-01)

**Статус:** ✅ LANDED (окно p-eff-hygiene, 2026-08-01). Владелец: вопрос про гонку в момент ленивой инициализации → «запрет — согласен».

### Правило

Тело `#default_handler`-фабрики — **construction-time код, обязан быть свободен от yield-point'ов**: любой прямой или транзитивный вызов suspend-опа (`Time.sleep`, любой `Net.*`-оп; канальные `recv`/`send` — см. «Известное ограничение» ниже) ДО того, как фабрика вернёт свой `effect X { ... }`-литерал, — compile error `E_DEFAULT_HANDLER_SUSPEND`. Op-тела ВНУТРИ литерала (`op(...) => ...`/`op(...) { ... }`) — **исключение**: они исполняются при ДИСПЕТЧЕ (когда потребитель зовёт `X.op(...)`), не при СТРОЙКЕ (когда фабрика лениво конструирует и устанавливает handler) — суспензия там нормальна и ожидаема (пример: `real_time()`'s `sleep`-оп зовёт `time_sleep_ms`, реально паркующий fiber, — это op-тело, не construction-time код фабрики, и остаётся легальным).

### Обоснование

Механика §1 D431-текста выше (`if (!_nova_handler_X) { _nova_handler_X = <default-ctor>(); }`, врезанная в тело диспетчера) — это НЕ атомарная check-and-set: между «слот ещё NULL» и «слот записан» лежит ПОЛНОЕ исполнение тела `<default-ctor>()`. Если это тело суспендирует fiber (паркуется на suspend-опе), кооперативный планировщик отдаёт OS-поток ВТОРОМУ файберу, который может пройти ТУ ЖЕ проверку `if (!_nova_handler_X)`, увидеть слот всё ещё NULL (первый файбер не успел записать результат) и запустить фабрику ВТОРОЙ раз — double-construction. Для чистой (stateless) фабрики это просто трата работы; для stateful-дефолта (хендлер с closure-состоянием) — расщепление состояния между двумя независимыми экземплярами, наблюдаемое потребителем как «то один инстанс, то другой» без предсказуемого порядка.

Дополнительное обоснование из per-fiber-снапшот механики (`nova_effect_snapshot_save`/`_restore`, `nova_rt/effects.h:1571`, ссылка на текущий номер строки — см. файл): установленный дефолт лениво попадает в снапшот ТЕКУЩЕГО файбера при первом использовании; база потока (`_nova_handler_X` TLS-слот) восстанавливается на выходе. Из этого следует, что фабрика и без явного запрета УЖЕ ожидается исполняющейся РОВНО ОДИН РАЗ на холодную файбер-линию — запрет yield-point'ов лишь статически подпирает предположение, которое рантайм-архитектура и так делает неявно.

### Реализация

`check_default_handlers` (`compiler-codegen/src/types/mod.rs`) получил пятую проверку (после `E_DEFAULT_HANDLER_DUPLICATE`/`UNKNOWN_EFFECT`/`ARITY`/`RETURN_TYPE`, §"Валидация" п.4 выше) — `E_DEFAULT_HANDLER_SUSPEND`:

1. **Direct-скан.** `direct_suspend_call_in_fn`/`_block`/`_stmt`/`_expr` — AST-walk фабрики, детектирующий прямой вызов `is_suspend_op_path` (`Time.sleep`/любой `Net.*`, path-extraction скопирован с `check_capabilities_at`'s D63/D64-извлечения). **КРИТИЧНО:** явный рукав `ExprKind::HandlerLit { .. } => None` — скан НЕ рекурсирует в тело литерала (та самая op-тела/construction-time граница из правила выше).
2. **Транзитивный скан.** CU-wide fixed-point closure (`suspend_call_closure`) над NAMED-fn call-графом — АЛГОРИТМ переиспользован БЕЗ изменений от `thread_affine_closure`/`own_fiber_call_names_of_fn` (D441 §5 №167, A-V10 closure) — тот же класс механики: leaf-предикат (здесь — прямой suspend-вызов вместо `#thread_affine`-тега) + iterative dataflow до fixpoint. `own_fiber_call_names_of_fn` ПЕРЕИСПОЛЬЗОВАН verbatim; единственная новая поверхность — leaf-предикат (п.1 выше), поскольку `own_fiber_call_names_expr` собирает только bare-`Ident`-имена вызовов (не умеет узнавать `Path`/`Member`-shaped `Effect.op(...)`-вызовы).
3. **Диагностика.** Цепочка вызовов от фабрики до suspend-опа (образец — `E_THREAD_AFFINE_IN_FIBER`, №167): прямой хит — `` `#default_handler(X)` fn `f` calls suspend-op `Time.sleep` directly ``; транзитивный — цепочка `` `f` → `g` → `Net.connect` ``.

### Известное ограничение (унаследовано, НЕ починено этой волной)

Скан — **синтаксический**, та же слепота, что у D64 realtime-ban (`check_capabilities_at`, «(Только receiver-Path формы; instance-method через obj.method требует type-инференции, отложен.)» — [M-175-realtime-ban-method-call-blind], №252 в `docs/plans/221.1-bug-sweep.md`): вызов suspend-опа через instance-method на произвольном expression-receiver (`ch.recv()`, `tx.send(v)` — канальные recv/send) НЕ резолвится (нужна receiver-type-инференция на этом checkpoint'е, которой у `check_default_handlers` нет — то же архитектурное ограничение, что у D64/D63). `Time.sleep`/`Net.*` (Path-form, `Effect.op(...)`-shaped) — покрыты полностью; канальные `recv`/`send` — нет. Фабрика, суспендирующая ТОЛЬКО через канальный `.recv()`/`.send()`, пройдёт этот гард незамеченной — известный, документированный, унаследованный gap, а не новая дыра этой волны.

### Фикстуры

`spec_tests/conformance/neg/d431_default_handler_suspend_neg.nv` (`#default_handler`-фабрика зовёт `Time.sleep(...)` ДО возврата литерала → `E_DEFAULT_HANDLER_SUSPEND`) · `spec_tests/conformance/d431_default_handler_suspend_pos.nv` (чистая фабрика, op-тело С суспендящим `Time.sleep`-вызовом — компилируется). Существующие 5 std-фабрик (`real_io`/`real_time`/`real_fs`/`real_net`/`real_os`) — construction-time код у всех пяти чист (просто строят и возвращают литерал) — остаются зелёными, `nova check std/src` байт-идентично канону (PASS 147/FAIL 26/WARN 60).

### Связь

[D474](#d474) (представление литерала в C: одна vtable, один контекст, три случая размещения — дефолт без захватов становится статической константой, то есть перестаёт строиться на каждом потоке) · Amends [D431](#d431-default_handlerx--ambient-lazy-default-handler-factory-для-эффектов-plan-175-ф2-v2-2026-07-2122) (§"Валидация" п.4 — пятая проверка) · родня [D441](06-concurrency.md#d441) §5 №167 (A-V10 `#thread_affine`-closure — переиспользованный алгоритм) · [D63](#d63)/[D64](#d64) (`forbid`/`realtime` suspend-effect scan — источник path-extraction И источник унаследованной слепоты) · `docs/plans/221.1-bug-sweep.md` №246 (эта волна) · №252 (`[M-175-realtime-ban-method-call-blind]`, унаследованная слепота, номер выдан задним числом по правилу №217) · `docs/plans/backlog-followups.md` (`[M-default-handler-factory-yield-point-ban]` → CLOSED).

## D474. Представление литерала обработчика в C: одна vtable, один контекст, три случая размещения

> **Создан:** 2026-09-10, по разбору владельца на живом носителе
> (`nova-duckdb/src/duckdb.c:7503-7517`). **Дом:** здесь, рядом с [D431](#d431)
> (`#default_handler`), потому что предмет тот же — как обработчик existует в
> порождаемом C. Родня: [D473](08-runtime.md) (косвенный возврат) — тот же род
> решения: механизм без поверхности, целиком выводимый компилятором.

**Грамматической формы у этого решения НЕТ и не должно быть**, как и у D473:
попросить или запретить конкретное размещение из исходника нельзя. Блок нормирует
то, что эмиттер обязан делать, и то, чего он делать не вправе.

### 0. Что представляет обработчик

Литерал `effect X { op() => … }` порождает ДВЕ вещи: таблицу операций
(`NovaVtable_X`) и **контекст захватов**. Контекст — это окружение замыкания:
переменные, захваченные литералом из области, где он написан. Механизм захвата
тот же, что у обычного замыкания (D431, «захваты используют РОВНО ту же схему,
что closures»).

**Контекст ОДИН НА ОБРАБОТЧИК, а не на операцию.** Все операции одного литерала
получают один и тот же `ctx` первым аргументом (`nova_rt/effects.h:1891-1897`), и
это нормативно: операции захватывают из ОДНОЙ области видимости, и разные
контексты рассинхронизировали бы их. Пример из std — `mut_clock(start_ms)`
(`std/src/testing/handlers/core.nv:262`): `now()`, `now_monotonic()` и `sleep()`
читают и двигают один `current_ms`; разъедини их контексты, и часы разойдутся.
Индивидуален контекст на ЭКЗЕМПЛЯР: два вызова `mut_clock(0)` дают два
независимых контекста.

### 1. Три случая размещения, и они нормативны

**Замер, с которого начался разбор** (`duckdb.c:7503`): дефолтный обработчик
`real_time()` выполнял ДВЕ `nova_alloc` за вызов — под таблицу и под контекст,
у которого ноль захватов и потому единственное поле `char _dummy`. Это не
неэффективность одного места, а следствие того, что эмиттер выделял контекст
БЕЗУСЛОВНО. Отсюда три случая, различаемые по свойству, а не по имени эффекта:

1. **Дефолт (`#default_handler`) БЕЗ захватов — статическая константа файлового
   уровня, ноль аллокаций.** Содержимое такой таблицы неизменно и одинаково для
   всех потоков, поэтому экземпляр обязан быть один на программу, а не один на
   поток. Диспетчер операции берёт его как запасное значение:
   `p = _nova_handler_X; if (!p) p = &g_default_X;`.
2. **Обычный обработчик БЕЗ захватов — одна аллокация, `ctx = NULL`.** Выделять
   блок под пустую структуру запрещено: `char _dummy` существует только чтобы C
   не отверг пустой тип, и аллокация под него — работа без предмета.
3. **Обработчик С захватами — ОДНА аллокация на таблицу и контекст вместе.**
   Время жизни у них общее (жизнь обработчика), поэтому делить их на два блока
   незачем: контекст размещается в хвосте того же блока.

### 2. Правило выравнивания — часть нормы, а не деталь

Смещение контекста в склеенном блоке — `align_up(sizeof(NovaVtable_X),
alignof(NovaCtx_<id>))`, а НЕ просто `sizeof(NovaVtable_X)`. Сегодня таблица
состоит из указателей, её размер кратен восьми, и любой нынешний захват (целое,
указатель, `f64`) лёг бы верно и без выравнивания. **Правило записано именно
потому, что носителя ещё нет:** первый же захват с выравниванием шестнадцать даст
тихую порчу, и найдётся она не там, где сделана. Это тот же класс, что у D473
«обязанность по типу»: проверка стоит ДО появления носителя, а не после.

### 3. Что этими правилами НЕ меняется

* **Семантика `with`.** Вход перезаписывает слот, выход восстанавливает ПРЕЖНЕЕ
  значение — NULL либо прежний дефолт. Поэтому проверка `if (!p)` на диспетчере
  ОСТАЁТСЯ: она снимает не только «первый вызов», но и «мы вышли из `with`».
  Убрать её значит изменить восстановление, то есть семантику D431, — это
  отдельное решение, и здесь оно не принимается.
* **Отсутствие обработчика — паника.** Эффект без `#default_handler` при вызове
  операции без активного `with` даёт `nv_panic` с текстом, называющим эффект и
  операцию (реестр 221.1 №158). Ни один из трёх случаев выше этого не касается.
* **Хранение слота.** `_nova_handler_X` остаётся потоко-локальным со снимком на
  файбер (`nova_register_effect_storage`, `nova_supervised_step`): статическая
  константа случая (1) — это ЗАПАСНОЕ значение, а не замена слота.

### 4. Что блок запрещает

* **Аллокацию контекста при нуле захватов** — ни под `char _dummy`, ни под что
  бы то ни было; при нуле захватов `ctx` есть NULL.
* **Два блока там, где время жизни одно** — таблица и её контекст выделяются
  вместе.
* **Общий изменяемый контекст на потоки.** Случай (1) законен ровно потому, что
  захватов нет и содержимое неизменно. **Дефолт С захватами константой не
  становится** — он строится на поток, как и раньше; иначе один контекст
  разделили бы все потоки, и это гонка, а не оптимизация.
* **Смещение хвоста без выравнивания** (см. §2).
* **Различение случаев по имени эффекта или типа** — решает свойство «есть ли
  захваты», и только оно.

### 5. Чего этот блок НЕ утверждает

**Ускорения не обещано и не измерено.** Основание — устранение работы без
предмета (две аллокации на пустоту, построение дефолта заново на каждом потоке) и
сокращение чтений потоко-локального слота с четырёх до одного на операцию.
Наблюдаемого выигрыша на профиле никто не показывал; если он понадобится как
довод, его надо замерить на горячем пути `Time`, а не вывести отсюда.
...
## D448. Эффекты в сигнатуре — НИЖНЯЯ граница; совместимость типов функций по включению

> Принято владельцем 2026-08-09 по разбору «как реализовать модули и фреймворки
> на Nova, если они понятия не имеют, какие эффекты будут в пользовательском
> коде». Уточняет [D50](06-concurrency.md#d50) и правило `--strict-effects`.

### Что

Перечень эффектов в объявлении функции означает **«как минимум это»**, а не
«ровно это и ничего сверх». Соответственно совместимость типов функций
проверяется **по включению**: значение типа `fn() A B -> T` подставимо всюду,
где требуется `fn() A -> T`, потому что требование `A` выполнено — наличие `B`
сверх требования препятствием не является. Частный случай: `fn() -> T` не
предъявляет требований и потому принимает функцию с любыми эффектами.

```nova
fn run_handler[T, E](body fn() Fail[E] -> T) Fail[E] -> T => body()

fn user_handler() Time Fail[FwErr] -> int => 42   // умеет БОЛЬШЕ, чем требует
                                                  // параметр
ro v = run_handler(user_handler)                  // законно: Fail[E] есть
```

### Зачем — иначе на Nova нельзя писать фреймворки

Библиотека, принимающая пользовательский код (роутер HTTP, планировщик,
тест-раннер, `retry`, итератор с обратным вызовом), **не может знать** эффекты
этого кода: обработчик волен ходить в базу, в файлы, в сеть, спать и писать
логи в любом сочетании. Если перечень в типе параметра трактовать как точное
совпадение, библиотека отвергает пользовательский код за то, что тот делает
больше, чем предугадал её автор. Предугадать нельзя — значит трактовка неверна.

### Почему это НЕ ослабляет гарантии

«Ничего сверх» выражается отдельной конструкцией — `forbid Net, Db, Fs { … }`
(см. выше в этом файле). Раз запрет выражается явно и отдельно, перечень в
сигнатуре свободен нести смысл требования-минимума; две конструкции не
пересекаются по назначению:
...
### Что проверяет `--strict-effects`

Что объявлено **не меньше**, чем используется: функция, вызывающая `Time`-код,
обязана объявить `Time`. Это и есть исходное правило («объявлять надо как
минимум те эффекты, которые используются»), и оно не противоречит трактовке
выше — обе про нижнюю границу, с разных сторон.

Строгий режим **не** должен требовать точного совпадения эффектов при
подстановке функции в параметр: такое требование делает его несовместимым с
функциями высшего порядка (см. дефект ниже).

### Состояние реализации на 2026-08-09

* эффекты **не влияют на представление**: `fn f(x int) Time -> int` и
  `fn f(x int) -> int` дают побайтно одинаковую C-сигнатуру — проверено прямой
  сборкой. Значит правило подстановки затрагивает ТОЛЬКО чекер, кодоген не
  трогает вовсе;
* **вне** строгого режима подстановка уже работает по включению (проверено:
  `apply_it(timed_one)` при параметре `fn() -> T` компилируется);
* **в** строгом режиме — отвергается, что противоречит настоящему решению;
  заведено как №488 (реестр 221.1);
* `forbid` — ключевое слово в парсере есть (`KwForbid`), живых носителей в `.nv`
...
## D456. Граница эффекта — публичный API на Nova, а не таблица системных вызовов (план 268, владелец 2026-08-11)

**Одной строкой.** Операция эффекта — обычная функция Nova, у которой отнят
ровно один элемент: получатель `@`, потому что у эффекта нет инстанса. Всё
остальное, что умеет язык, операции эффекта доступно и от неё ожидается.

### Что доступно и должно использоваться

`Result`/`Option`, записи и суммы, коллекции и кортежи, параметры-функции,
именованные типы вместо голых чисел, квалификаторы `consume`/`mut` на
параметрах. Отсутствует ровно `@`: получателя нет, потому что эффект — не
значение, а точка диспетчеризации.

**Исключение — теперь НАЗВАННЫЙ ОТКАЗ ЧЕКЕРА, а не наблюдение задним числом.**
Установлено пробой, а не рассуждением: до владельческого решения 2026-08-12
ГЕНЕРИКИ в эффектах принимались чекером на ОБЕИХ осях обобщения и падали уже
в СИ-компиляторе — худший из возможных исходов. Ось 1 (реестр 221.1 №570) —
generic-параметр на ОПЕРАЦИИ эффекта: `type Wrap effect { around[T](body
fn() -> T) -> T }`, две инстанциации (`int`, `str`) — `nova check` давал
зелёный, `nova build` падал ошибкой СИ-компилятора (кодоген выпускает одну
стёртую функцию `Nova_Wrap_around`, возвращающую `Nova_T*`, вторая
инстанциация не приводится к типу). Ось 2 (реестр 221.1 №614) —
generic-параметр на САМОМ эффекте: `type Store[T] effect { ... }` — с
подставленным типом в обработчике чекер (неверно) винил автора обработчика
(`E_HANDLER_OP_RETURN_TYPE_MISMATCH`), с голым `T` `nova check` был зелёным, а
`nova build` падал `unknown type name 'NovaVtable_Store'`. Обе оси — один
класс: rank-2 полиморфизм через vtable-слот с ОДНОЙ подписью, точное правило
значится открытым вопросом ([Q6](../open-questions.ru.md#q6)); поддерживал их
только bootstrap-интерпретатор через стирание, а интерпретатор ретрактирован.

**Владелец 2026-08-12 (реестр 221.1 №570/№614):** до закрытия Q6 ОБЕ оси
отвергаются ЧЕКЕРОМ по имени, на объявлении, а не пропускаются до
СИ-компилятора — `E_EFFECT_OP_GENERIC_UNSUPPORTED` (ось 1, `types/mod.rs`
`check_effect_generic_refusal`) и `E_EFFECT_GENERIC_UNSUPPORTED` (ось 2, тот
же проход; отказ на объявлении исключает generic-эффект из схемы
`check_handler_op_declarations`, так что его обработчики больше НЕ получают
второй, вводящий в заблуждение диагноз). Исключение из этого отказа —
компилятор-интринсик `Fail[E]` (и однотипные `Effect`/`Ask`/`Alloc`): это не
пользовательский эффект, идущий через общий handler-literal vtable-путь, а
sugar-цель `throw`/`!!` со своей рантайм-машинерией (`arity_exempt`,
`types/mod.rs`) — отказ по имени её не касается.

> **Амендмент 2026-08-17 (реестр 221.1 №710).** На этом месте до сегодняшнего
> дня стояло ещё и «и она уже работает». Проба показала, что это правда
> наполовину, и вторая половина опасная. `Fail[E]` действительно работает для
> любого `E` — но только если параметр операции в handler-литерале
> **аннотирован**: `fail(e ParseErr) -> never` даёт верный результат, а
> `fail(e) -> never` типизируется как `str` и чекером, и кодогеном, так что
> запись проходит через строковый заголовок МОЛЧА — `e.byte_len()` на
> `ParseErr` компилируется, программа печатает мусор, диагностики нет ни
> одной. **Норма не меняется:** `E` — тип бросаемого значения, для любого `E`,
> и вывод типа параметра обязан брать его из `with Fail[E] = …`. Меняется
> честность записи: до закрытия №710 (канал плана 196) аннотация фактически
> обязательна, и это дефект реализации, а не форма языка. Двумя абзацами ниже
> этот же D-блок запрещает «обещать возможность, которой нет» — правило
> поймало собственный текст, и запись исправлена, а не подогнана.

Единственный носитель в `std` — `Db.in_transaction[T, E]`
(`std/src/data/sql.nv`) — **убран**, не переписан: транзакция — это
`consume`-тип `Tx` с `@cleanup` (операция возвращает `Result[Tx, DbError]`,
тело пользователя выполняется СНАРУЖИ границы эффекта, `T` её не пересекает,
ранг-2 не возникает); проектирование `Tx` — план 268, форма записана в плане
273 §4-тер. Написано здесь прямым текстом именно потому, что этот D-блок сам
запрещает ложные обоснования: обещать возможность, которой нет, — та же
ошибка, что и объяснять форму ограничением, которого нет.

### Чего на границе эффекта быть не должно

Каждый пункт — не вкусовщина, а форма, у которой есть цена; цена названа.

1. **Отрицательный errno или код возврата вместо ошибки.** `env_set(...) -> int`,
   «0 или `-errno`», заставляет автора мока знать POSIX, чтобы написать мок.
   Правильно — `Result[(), IoError]`.
2. **Пустое значение как признак «нет» или «ошибка».** `cwd()` пустая строка =
   ошибка, `home_dir()` пустая = «нет такого». Sentinel — это `Option`/`Result`,
   от которого отняли проверяемость.
3. **Обход по индексу** (`env_len` + `env_key_at` + `env_val_at`,
   `arg_count` + `arg_at`). Три операции обязаны договориться между собой:
   мок отдаст пять ключей и четыре значения — и не покраснеет НИЧЕГО.
   Правильно — одна операция, возвращающая коллекцию.
4. **Out-параметры** (`stream_peer_addr(stream, out mut []u8)`). Результат
   функции — это возвращаемое значение; буфер наружу — способ обойти систему
   типов, а не оптимизация.
5. **Сырые ручки как `int`** (`read(fd int, …)`). Дескриптор — именованный тип;
   `int` позволяет передать в `read` номер строки.
6. **Счётчики рядом с данными** (`argv []u8, argc int`). Коллекция знает свою
   длину; отдельный счётчик — это приглашение соврать.
7. **`str`, в котором лежат не-UTF-8 байты.** Тип, который врёт, обманывает не
   человека, а компилятор: он перестаёт проверять ровно то, ради чего
   существует. Наружу — `[]u8`; `str`-удобство живёт в обёртке.
8. **Позиционные простыни и булевы флаги-переключатели.**
   `process_run(program, argv, argc, env, envc, use_env, cwd)` — семь
   параметров, два счётчика, один флаг. Правильно — запись-описание.

### Где живут C-формы

В `extern "C"`-объявлениях `ffi.nv` (module-private) и внутри `real_*`-обработчика.
**Обработчик — переводчик, а не сквозной канал.** Если C-форма видна в
декларации эффекта, перевод просто не написан, и его придётся писать каждому,
кто пишет мок.

### Чего НЕ делать: не брать эталон из существующих файлов

Прямое указание, потому что «посмотри, как сделано у соседа» — самый частый
способ размножить дефект. На 2026-08-11 в `std` **нет** эффекта, годного в
образцы:

| эффект | опер. | сырых | богатых | out-пар. | курсор | ручка-`int` |
|---|---:|---:|---:|---:|---:|---:|
| `Fs` | 26 | 24 | 0 | 5 | 2 | 13 |
| `Net` | 25 | 15 | 10 | 6 | 0 | 0 |
| `Os` | 17 | 16 | 1 | 0 | 5 | 0 |
...
`Net` — недоделанная миграция: богатые возвраты соседствуют с шестью
out-параметрами. `Db` выглядит правильно, но это **набросок без единого
реального пользователя** (и `Fail` в `std` не применяется) — форма не проверена
подключением к настоящей БД. Эффект судится ПО ЭТОМУ ПРАВИЛУ, а не по сходству
с существующим файлом.

### Исключения — только названные

Форма из списка запретов допустима там, где она действительно верна (счётчики
метрик — правдоподобный случай), но обоснование пишется КОММЕНТАРИЕМ У
ДЕКЛАРАЦИИ. Молчаливое исключение запрещено: именно так `Os` и `Fs` доехали до
сегодня, а объяснение в шапке `os/effect.nv` («vtable эффекта стирает богатый
возврат до `nova_int`/`nova_str`») к этому дню стало ПРОСТО НЕВЕРНЫМ — `Net`
возвращает `Result[TcpStream, NetError]`, `Db` возвращает `[]DbRow`. Ложное
обоснование хуже отсутствующего: оно учит следующего автора ограничению,
которого нет.

### Приёмка

Правило нормативно с момента принятия: **новый эффект, нарушающий его,
не принимается.** Существующие приводятся волнами (план 268), под храповик —
счётчик нарушений по каждому пункту, которому разрешено только убывать.

**Связи:** план [268](../../docs/plans/268-effect-boundary-reform.md) ·
`docs/dev/module-conventions.md` (FFI-слой `ffi.nv`, module-private) ·
D453 (`process_run` — носитель п. 6 и 8) · записи реестра №568 (форма границы),
№569 (`ffi.nv` без машины).

## D459. Операция эффекта объявляется словом `fn` (план 272, владелец 2026-08-16; форма-мост варианта C′)

**Одной строкой.** Внутри `type X effect { … }` каждая операция пишется так же,
как всякая функция языка, — начиная со слова `fn`.
...
type Cache effect {
    fn fetch(key str) -> Option[int]
    fn store(key str, value int) -> ()
}
```

Прежняя форма — та же строка без `fn` (`log(msg str) -> ()`) — снимается.

**Примечание 2026-09-21 (интегратор, правка формулировки без смены нормы):** до правки
здесь стоял `type Cache[K, V] effect` — generic-параметр на самом эффекте. Этот блок вводит
РОВНО слово `fn` (граница выше это и говорит) и generic-форму не рассматривает вовсе; пример
скопирован из более старого дизайн-документа D53 без сверки с D456, принятым пятью днями
раньше D459 и запрещающим ОБЕ оси generic-обобщения в эффектах ЧЕКЕРОМ до закрытия
[Q6](../open-questions.ru.md#q6) (`E_EFFECT_GENERIC_UNSUPPORTED`/`E_EFFECT_OP_GENERIC_UNSUPPORTED`).
Найдено окном Карины 2026-09-21 при разборе формы объявления эффекта для 274.5 §5б. D456
остаётся действующей нормой без изменений; правка меняет только иллюстрацию.

### Зачем, и это не выравнивание ради красоты

**Одна форма объявления функции на язык.** Сегодня объявление внутри эффекта —
единственное место, где функция объявляется БЕЗ `fn`. Читатель, дошедший до тела
эффекта, обязан знать, что здесь синтаксис другой; инструмент — тоже. Слово `fn`
снимает исключение, а не добавляет шум: оно возвращает эффект в общий язык.

**Это МОСТ к метод-форме (остальной план 272), и мост нужен раньше неё.** Полная
форма даёт операции получателя (`fn TcpListener mut @accept() -> int`), и её
невозможно записать, пока объявление не начинается с `fn`: голова строки занята
именем операции. То есть `fn` — не первый шаг из вежливости, а то, без чего
следующий шаг не записывается.

**Требование самохостинга, а не пожелание.** Решением владельца 2026-09-13 план
272 ОБЯЗАТЕЛЕН для Карины (`novac`) — она пишет свой код в этой форме. Пока
оракул её не принимает, Карина либо пишет форму, которой её же компилятор не
понимает, либо пишет старую и потом мигрирует весь свой корпус.

### Граница блока — что сюда НЕ входит

Этот блок вводит РОВНО слово `fn` перед операцией. Не входят и остаются работой
остального плана 272:

* получатель у операции (`fn TcpListener mut @accept()`) и всё, что с ним
  связано, — режимы `mut`/`consume` у получателя, правило вывода типа;
* вызов операции метод-формой (`lst.accept()`) вместо `Net.accept(lst)`;
* снятие рукописных методов-обёрток (22 по `net`/`fs`/`os`/`io`).

Граница названа затем, что без неё «272 сделан» прочтётся как сделанное целиком,
а сделана будет одна восьмая.

### Приёмка

1. **Обе формы различимы компилятором:** объявление с `fn` принимается;
   объявление без `fn` ОТВЕРГАЕТСЯ названной диагностикой, а не падает разбором.
   Негативная фикстура с `nova:expect` на строке — обязательна.
2. **Корпус переведён целиком:** ни одного объявления операции без `fn`. Счёт
   машинный; на день принятия решения их 303 из 350 (47 уже написаны с `fn`).
3. **Перевод ничего не меняет в поведении:** эмиссия C до и после миграции
   совпадает, прогон корпуса даёт те же числа. Это проверяется, а не
   предполагается: миграция механическая, и молчаливое расхождение здесь было бы
   дороже самой работы.

**Связи:** план [272](../../docs/plans/272-effect-method-form.md) (метод-форма
целиком) · D456 (граница эффекта — операция это обычная функция, у которой отнят
только `@`) · D458 (`02-types.md`, одна форма для методов и функций — та же
мысль на уровне типов) · план [274](../../docs/plans/274-novac-self-hosted-compiler.md)
(самохостинг, ради которого форма стала обязательной).
