...
**Исключение, установленное пробой, а не рассуждением: ГЕНЕРИКИ в операции
эффекта сегодня НЕ РАБОТАЮТ.** Проверено 2026-08-11 на минимальном примере
(`type Wrap effect { around[T](body fn() -> T) -> T }`, две инстанциации — `int`
и `str`): `nova check` даёт зелёный, `nova build` падает ошибкой СИ-компилятора
— кодоген выпускает одну стёртую функцию `Nova_Wrap_around`, возвращающую
`Nova_T*`, и вторая инстанциация не приводится к типу. Спека этого и не
обещала: генерик-операции требуют rank-2 полиморфизма, и точное правило значится
открытым вопросом ([Q6](../open-questions.md#q6)); поддерживал их только
bootstrap-интерпретатор через стирание, а интерпретатор ретрактирован.
Следовательно, до закрытия Q6 **операция эффекта generic-параметров не несёт**;
`Db.in_transaction[T, E]` в `std/src/data/sql.nv` — объявление, которое проходит
проверку и не собирается (реестр 221.1 №570). Написано здесь прямым текстом
именно потому, что этот D-блок сам запрещает ложные обоснования: обещать
возможность, которой нет, — та же ошибка, что и объяснять форму ограничением,
которого нет.
