...
> `?` будет **авто-конвертить** тип ошибки через `From` (Rust-паритет) —
> `Err(e) => return/throw E'.from(e)` при `E ≠ E'`, если на `E'` есть не-identity
> `From[E]`. Снимает требование явного `.map_err()` (ниже секция «Совместимость с
> `Fail[E]`»). Реализация gated на 172. NB: устаревшая секция `## D4` выше (и дубль
> `####`) противоречит D85 (до-D85) — чинится в рамках Plan 174.
