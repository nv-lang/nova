...
>
> **⚠️ Принято изменение 2026-06-20 ([Plan 174](../../docs/plans/174-question-mark-auto-from.md)):**
> `?` приводится в точности к Rust — **(A) return-only**: только на `Result`/`Option`,
> проброс значением; в Fail-эффект-функциях `?` запрещён (там `!!`/`throw`) — откат мутного
> throw-режима D165. **(B) авто-`From`**: `Err(e) => return Err(E'.from(e))` при `E ≠ E'`,
> если на `E'` есть не-identity `From[E]`; снимает явный `.map_err()`. Реализация gated на
> 172. NB: устаревшая секция `## D4` выше (и дубль `####`) противоречит D85 — чинится в Plan 174.
