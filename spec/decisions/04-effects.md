...
> Финальный синтаксис: `type X effect { ... }`. `effect` — kind-токен
> (по [D53](02-types.md#d53)) И keyword (по [D61](#d61)). Старая
> формулировка про «`protocol` для эффектов» устарела:
> protocols-как-структурные-контракты в Nova больше нет (см. [D62](#d62)),
> остались только nominal effects. `type X { методы }` без полей
> по-прежнему запрещено.

### Правило

#### Чёткое разделение `type` vs `type X effect`
...
// behavior (эффекты) — kind-токен effect
type Db effect {
    query(q Sql) -> []DbRow
    exec(q Sql)  -> ()
}
...
Слова `effect` и `handler` зарезервированы как keyword'ы.
