...
(`Fail[E]`, `Io`, `Async`, `Db`, `Net`, `Log`, `Alloc[R]`, `Mut`),
...
`Fail` без параметров — сахар над `Fail[Error]`, допустим только в
...
> `type Db effect { ... }`, не `protocol Db { ... }`. Семантически
...
2. Каждый `throw` → `Fail[E]` добавляется.
...
которая вызывает public `C` с `Db Fail`, то `A` получает `Db Fail`.
...
fn bad() Fail[A] Fail[B] -> ()           // ОШИБКА (если разные параметры —
