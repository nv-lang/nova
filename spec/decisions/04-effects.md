...
sum-type. `Fail` без параметра — сахар над `Fail[Error]`,
допустим только в приватных функциях. В публичных API `Fail[E]`
обязателен.
...
2. **Вызывать `return value`** → продолжение с подменой (управление
