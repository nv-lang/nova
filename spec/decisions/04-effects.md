...
| [D18](#d18-унификация-effect--это-обычный-type) | Унификация: `effect` — это обычный `type` |
...
объявление типа ([D18](#d18-унификация-effect--это-обычный-type)):
...
- [D18](#d18-унификация-effect--это-обычный-type) — эффект объявляется
  через обычный `type`, не специальный keyword.
...
### Что
Эффект — это **именованный тип**, не keyword. Имена в PascalCase.
...
Имя эффекта — обычный идентификатор типа в PascalCase. Синтаксис тот
же, что у любого `type` ([D18](#d18-унификация-effect--это-обычный-type)).
...
```nova
type User { id u64, name str }                              // record-тип
let alice = User { id: 1, name: "alice" }                   // record-литерал

type Logger { log(msg str) -> () }                          // эффект-тип
...
[D18](#d18-унификация-effect--это-обычный-type) для полной семантики.
...
- [D18](#d18-унификация-effect--это-обычный-type) — эффект объявляется
  через обычный `type`, литералы различаются по содержимому.
...
- [02-types.md → D42](02-types.md#d42) — `protocol` как структурный
  контракт; эффект-тип может одновременно работать как `protocol`.
...
### Что
Ключевое слово `effect` отменяется. Эффекты объявляются через обычный
`type` — компилятор различает «эффект» и «структурный интерфейс»
**по контексту использования**, не по форме объявления.
...
```nova
effect Db {                              // ОТМЕНЕНО — keyword effect
    query(sql str, args []any) -> []Row
    exec(sql str, args []any) -> ()
}
...
```nova
type Db {                                // унифицировано — обычный type
    query(sql str, args []any) -> []Row
    exec(sql str, args []any) -> ()
}
...
**Один синтаксис** для всех именованных типов с операциями. Слово
`effect` зарезервированным **не является** — можно использовать как
имя переменной, функции, поля.
...
1. **Меньше keyword'ов.** В Nova сознательно держится минимум
   зарезервированных слов.
2. **Гибкость ролей.** Один тип может играть роль эффекта или
   параметра — программист выбирает стиль на месте, не на уровне
   объявления.
3. **Согласованность с принципом «не плодить специальные сущности».**
   Эффект — это **роль**, не сущность. Сущность — это `type` с
   операциями.
4. **Симметрия.** Объявление `type X { op(...) -> ... }`, литерал
   `X { op(...) => ... }`, использование между `)` и `->` или как
   параметр — единый набор правил.

### Что отвергнуто

- **`effect X { ... }` keyword.** Дублировал `type X { ... }`, не
  добавляя информации. Лишний keyword без выгоды.
...
1. **Семантическая зависимость в парсинге.** Раньше `effect`
   ...
   `{...}` (двоеточие vs стрелка). Это работает, но добавляет одно
   правило в грамматику.
2. **Тип может «случайно» использоваться как эффект.** Если у функции
   в сигнатуре стоит `Logger`, и в скоупе нет активного handler'а —
   ошибка компиляции «эффект Logger не предоставлен». Это нормально,
   но раньше было ясно, что Logger — эффект, по слову `effect`. Теперь —
   по сигнатуре функции.
3. **«Эффект» как термин остаётся в документации**, но в коде нет
   соответствующего keyword'а. Это отражает реальность: эффект — это
   **роль**, не сущность.
...
- [02-types.md → D17](02-types.md#d17) — единый синтаксис объявления
  `type`.
- [02-types.md → D42](02-types.md#d42) — `protocol` как структурный
  контракт; эффект-тип одновременно может работать как `protocol`.
- [03-syntax.md → D19](03-syntax.md#d19) — стрелка `=>` в match-arms,
  та же что в handler-литералах.

### Эволюция

Раньше существовало два keyword'а: `effect X { ... }` для эффектов и
`type X { ... }` для всего остального. Унифицировано — `effect`
отменён, эффекты объявляются через `type`. Различение идёт по
контексту использования (позиция в сигнатуре) и по содержимому литерала
(двоеточие → record, стрелка `=>` → handler).

В одном из ранних черновиков D18 пример handler-литерала был записан
со стрелкой `->` (`Db { query(s, a) -> resume(...) }`) — это устарело.
