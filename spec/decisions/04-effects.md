...
Генерики и параметры-функции (операции высшего порядка — `Db.in_transaction`
принимает `body fn() Db Fail[E] -> T`), `Result`/`Option`, записи и суммы,
коллекции и кортежи, именованные типы вместо голых чисел, квалификаторы
`consume`/`mut` на параметрах. Отсутствует ровно `@`: получателя нет, потому
что эффект — не значение, а точка диспетчеризации.
