...
| [D67](#d67--оператор-семантика-для-result-через-fail-для-option-через-ранний-return) | `?` оператор: семантика для `Result` через Fail, для `Option` через ранний return |
...
- **Отдельный оператор `??!` для Option** — отвергнуто: `??` уже
  используется как coalesce ([D4](#d4)), цена ещё одного символа
  выше пользы.
