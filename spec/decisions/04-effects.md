...
>
> **⚠️ Уточнение 2026-06-20 ([Plan 174.2](../../docs/plans/174.2-question-mark-return-only.md)):**
> `?` подтверждён **return-only** — только на `Result`/`Option`, проброс значением; в
> Fail-эффект-функциях `?` запрещён (там `!!`/`throw`), мутный throw-режим D165 убирается.
> Авто-`From` конверсия ошибки (Rust-стиль) **рассмотрена и ОТКЛОНЕНА** — держим explicit
> `.map_err` (причины — в Plan 174.2, Часть B). NB: устаревшая секция `## D4` выше (и дубль
> `####`) противоречит D85 — чинится в Plan 174.2.
>
...
**Status:** ✅ ACTIVE (Ф.1 — единый источник + split). Typed-surface (`timestamp()->Timestamp` / `monotonic()->Monotonic` / `sleep(Duration)`), infallibility-by-contract (Q15), non-serializable `Monotonic`, overflow-политика (→ D317) и monotonic non-regression (→ D318) вносятся последующими фазами плана 175 (Ф.1c/Ф.2/Ф.3) — здесь зафиксирована ТОЛЬКО механика Ф.1 (без смены поведения int-провода).
