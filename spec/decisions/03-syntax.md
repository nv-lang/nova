...
> **АМЕНДМЕНТ 2026-09-18 (решение владельца, реестр 221.1 №762/№1157):
> ПРОДУКЦИЯ `pattern` РАСКРЫТА, ПЕРЕЧЕНЬ ФОРМ ЗАКРЫТ, ИМЯ КОНСТАНТЫ В ПОЗИЦИИ
> ОБРАЗЦА — НЕ ФОРМА (`E_MATCH_CONST_PATTERN`).**
>
> До амендмента грамматика выше называла `pattern`, но нигде его не
> раскрывала: единственным перечнем форм была ОПИСАТЕЛЬНАЯ таблица обзорной
> страницы (`spec/syntax.ru.md`, «Виды паттернов»). Из-за этого вопрос «а
> можно ли в образце написать имя константы?» не имел ответа ни в одну
> сторону, и компилятор отвечал на него ДВУМЯ разными способами сразу.
>
> ```
> pattern      = literal | binding | '_' | constructor | record-pat
>              | tuple-pat | array-pat
> binding      = IDENT            // ВСЕГДА свежее имя, никогда не сравнение
> ```
>
> Список ЗАКРЫТ: форма, не порождаемая этими продукциями, отвергается.
>
> **ЧТО ИМЕННО ОТВЕРГАЕТСЯ И ПОЧЕМУ — ЗАМЕР, А НЕ ВКУС.** Запись
> `const MUT_W = "mut"` … `match w { MUT_W => 1, _ => 0 }` до запрета значила
> РАЗНОЕ в зависимости от РЕГИСТРА имени: ALL-CAPS разбиралось как
> `Pattern::Variant`, варианта не находилось, и сборка падала тремя ошибками C
> (чужой язык, чужой этап); строчное разбиралось как `Pattern::Ident`, то есть
> СВЯЗЫВАНИЕ — программа собиралась МОЛЧА и печатала `mut=1 other=1`, то есть
> первая ветка матчила всегда, а `_` была мертва. Автор в обоих случаях писал
> сравнение.
>
> **КАНОН, который диагностика обязана называть:** `x if x == MUT_W => …` —
> сравнение пишется guard'ом, который в грамматике выше уже есть.
>
> **ПОЧЕМУ ОТКАЗ, А НЕ НОВАЯ ФОРМА.** Завести образец-константу значило бы,
> что `NAME => …` читается по-разному в зависимости от того, объявлена ли выше
> константа с таким именем. Смысл образца стал бы зависеть от контекста,
> невидимого в самой строке: читатель ветки не смог бы отличить сравнение от
> связывания, не сходив за объявлениями. Цена отказа — четыре лишних символа в
> редкой записи; цена формы — постоянная неоднозначность в частой.
>
> **ГРАНИЦА:** имя, которое ЕСТЬ вариант суммы, остаётся ВАРИАНТОМ — проверка
> спрашивает варианты первыми. Иначе константа, названная именем варианта,
> убила бы законный разбор.
>
> Фикстуры: `neg/d19_const_pattern_upper_neg.nv` (громкая половина),
> `neg/d19_const_pattern_lower_neg.nv` (тихая — она важнее),
> `standalone/d19_pattern_forms_still_legal.nv` (контроль: связывание свежим
> именем, канон-guard, вариант суммы, тень параметра — всё законно).
...
**АМЕНДМЕНТ 2026-09-18 (интегратор; реестр 221.1 №676). База turbofish'а —
имя ТИПА, а не имя значения, и одного взгляда вперёд для этого мало.**

Правило Bootstrap выше — «после `Ident[...]` смотрим post-`]` token; `.IDENT(`
→ turbofish» — разрешает неоднозначность ОДНИМ синтаксисом, а она синтаксисом
не разрешается. `Vec[int].new()` и `xs[i].len()` написаны ОДИНАКОВО:
идентификатор, скобки, одно имя внутри, точка, метод, круглые скобки. Отличает
их только то, чем является база — типом или значением, — и об этом в самих
токенах нет ничего.

**Норма:** `base[X].method(...)` есть turbofish тогда и только тогда, когда
`base` именует ТИП. Если `base` именует ЗНАЧЕНИЕ, запись есть индексация с
вызовом метода на ЭЛЕМЕНТЕ. При совпадении имён (в области видимости есть и тип
`X`, и значение `X`) побеждает ЗНАЧЕНИЕ — по общему правилу затенения.

**Что было до амендмента, и почему это стоило дорого.** Побеждал turbofish, и
`xs[i].len()` МОЛЧА терял индекс: вызов уходил на внешний вектор, выражение
печатало длину контейнера вместо длины элемента. Молчаливость здесь не деталь —
у неё две половины, и худшая именно тихая: когда у контейнера НЕТ метода с тем
же именем, компилятор отвергает запись (`E_RECV_METHOD_MISMATCH`) и дефект
чинится в тот же час; когда есть — а `len` есть у всякого `Vec` — не
отвергается ничто, и программа отвечает неверным числом.

**Граница ловилась не там, где причина.** Литеральный индекс `xs[0]` работал
(литерал не может быть именем типа), поле `h.datas[i]` работало (база —
`Member`, а не `Ident`), промежуточный биндинг `ro p = xs[i]` работал (нет
post-`]` продолжения). Три зелёные клетки из пяти делали дефект похожим на
частный случай вывода типов, и полтора месяца он так и значился — «ресивер
берёт тип контейнера». Настоящая причина лежала в РАЗБОРЕ, то есть раньше на
целую фазу, и объясняет все пять клеток одним предложением.

**Реализация (оракул):** парсер собирает имена, объявленные в файле
ЗНАЧЕНИЯМИ (`ro`/`mut`/`consume`-биндинги, `for`-переменные), и такое имя базой
turbofish'а быть не может. Огрубление названо намеренно: множество собирается
по ФАЙЛУ, а не по области видимости, потому что разбор спекулятивен и стек
областей пришлось бы откатывать вместе с позицией. Недобор имён оставляет
прежнее поведение, перебор отнял бы законный turbofish — поэтому форма сбора
узкая: имя берётся только вплотную за ключевым словом.

**Известная граница, названная, а не забытая:** параметры замыканий и
биндинги образцов в этот сбор не входят, поэтому `x[i].m()`, где `x` —
параметр замыкания, остаётся разобран по-старому. Закрывается тем же способом
(добавлением этих форм в сбор) и ждёт своего носителя в корпусе.

Граница: `spec_tests/conformance/standalone/p676_var_index_receiver.nv` — пять
клеток, по одной на каждый разобранный выше случай.
...
>
> **СНЯТО ТЕМ ЖЕ ДНЁМ.** Запись ниже — ранняя редакция 2026-09-05; решением владельца позже тем же
> днём (амендмент выше, «Последняя фраза выше снята»: ветки `try_from` для числовых/char конверсий
> нет, `u8.try_from(c char)` заменён на `c.to_u8()`) она перекрыта. Текст сохранён как история;
> нормой не является. Пометка — 2026-09-09, исследование
> `docs/dev/research/2026-09-09-postfix-type-conversion.md` §5.5.
>
...
4. **Точка дизарма — момент передачи владения** (вход в consuming-вызов,
   ...
   порядок вычисления аргументов слева направо — нормативная
   формулировка и её наблюдаемость: [D484](#d484)). Всё, что в EXPR
...
> **АМЕНДМЕНТ D470 (2026-09-07): источник открытого конца — `@end_index()`, а не
> `len()`.** Три строки выше называют источником `arr.len()`; заменено вызовом
> `@end_index()` — второго метода роли `RangeIndex[V]`. Причина: поле или метод с
> именем `len` не обязан быть концом пространства индексов (счётчик, ёмкость), и
> признак «умеет срез» на этом основании ошибался молча. См. [D470](#d470).
...
- **split-семейство** (search.nv, byte-scan + zero-copy views): `@split(sep)`,
  ...
  > **Амендмент 2026-09-15 (`@split_ascii_whitespace` → ленивый; план 200 §23):**
  > `@split_ascii_whitespace()` возвращает **ленивый итератор**
  > `AsciiWhitespaceIter` (семья `vec_iter`) вместо эагерного `ro []str`; слова —
  > те же zero-copy виды, выдаются по одному, массив — `.collect()`. Семантика
  > разбиения НЕ менялась: runs ASCII-WS, пустые сегменты пропускаются,
  > ведущие/хвостовые WS игнорируются. Это отменяет оговорку амендмента
  > 2026-07-11 «`@split_whitespace`/`@lines` — без изменений» РОВНО для первого из
  > двух: `@lines()` пока эагерный (та же фаза плана 200 §23, отдельным шагом —
  > у него живой потребитель `std/src/io/core.nv`).
  >
  > **Почему СВОЙ тип, а не `SplitIter` с другим полем:** у `SplitIter`
  > разделитель — ПОДСТРОКА (`sep str`), здесь — КЛАСС БАЙТОВ («пропусти пробелы,
  > отдай слово»). Общий тип нёс бы мёртвое поле в каждом из двух случаев и был бы
  > беднее обоих; разделяются формы, а общей остаётся структура (`value`-запись +
  > `#impl(Next[str])` + байт-скан по zero-copy видам).
  >
  > **Класс байтов при переносе сохранён дословно** — space + `\t\n\v\f\r`
  > (9..13), то есть ШИРЕ, чем `char @is_ascii_whitespace()` (`runtime/defaults.nv`),
  > который исключает `\x0B` ради паритета с Rust. Два предиката в std расходятся
  > ровно по этому байту; перенос берёт исходный (`is_ascii_ws`, search.nv), а не
  > char-версию, и на расхождение заведена фикстура.
...
## D470. Роль среза `RangeIndex[V]`: срез решает МЕТОД типа, а не имя типа

**Статус:** нормативно с 2026-09-07 (решение владельца; план 284, номер
назначен интегратором). Блок меняет ПРИЗНАК применимости среза и не меняет
его семантику. Блок общий с планом 281 — §6.

### §1. Что было, и почему это дефект, а не стиль

Признак «этот тип умеет срез» жил в кодогене тремя зашитыми фактами разом
(`compiler-codegen/src/codegen/emit_c.rs`, Range-арм `ExprKind::Index`, сверено
2026-09-07):
...
Он ошибался в ДВЕ противоположные стороны, и обе ошибки тихие:

1. **имя типа в условии** — каждый новый тип со срезом надо было дописывать в
   кодоген; `str` из общего пути был исключён ИМЕНЕМ и держал собственную
   ветку с собственным рантайм-хелпером;
2. **структурное поле как признак** — тип, у которого поле `len` есть, но
   значит не длину (счётчик, ёмкость, вместимость кольца), проходил проверку
   МОЛЧА и получал неверный срез; а тип с методом `@index(r Range)`, но без
   поля, до общего пути не доходил вовсе.

Второе — не гипотеза: под старым правилом ровно так и работали две фикстуры
корпуса (`openrange_len_field_heap.nv`, `openrange_len_field_value.nv`), и они
были ЗАДУМАНЫ как доказательство того, что признаком служит поле.
...
Тип, удовлетворяющий роли, умеет срез; не удовлетворяющий — не умеет, и это
говорит компилятор, а не догадка кодогена о структуре. Протоколы в Nova
СТРУКТУРНЫ ([D9](#d9), [D42](#d42)): объявлять соответствие не требуется, оно
следует из наличия методов, — поэтому роль не вводит новой обязанности для
того, кто уже написал оба метода.

### §3. Отношение к D238 `Index[K, V]`

Роль **строго сильнее** `Index[Range, V]`: та требует только `@index`, эта —
`@index` И `@end_index`. Отсюда следствие, которое надо назвать прямо: тип,
реализующий `Index[Range, V]` и НЕ отвечающий `@end_index()`, законно
отвергается на `x[a..b]`, и это не регресс, а разница между «умею взять
диапазон, который мне дали» и «умею сказать, где мой конец». Сахар
открытого конца требует второго.

`Index[K, V]` для прочих `K` (целочисленный ключ, ключ словаря) роль не
затрагивает — там открытого конца не бывает, и D238 действует как записан.

**Граница роли — СИНТАКСИЧЕСКАЯ форма среза, а не тип аргумента.** Роль
требуется там, где стоит сахар: `x[a..b]`, `x[a..]`, `x[..b]`, `x[a..=b]`. А
значение типа `Range`, переданное явно (`ro r = 2..5` и затем `x[r]`), —
обычный `Index[Range, V]` по D238, и роли НЕ требует: у собранного `Range`
открытого конца нет, материализовать нечего, спрашивать `@end_index()` не о
чем. Граница названа здесь потому, что в реализации она выглядит как
небрежность: рядом с проверкой лежит готовая переменная «индекс есть
диапазон», которая ловит ОБЕ формы, и переиспользовать её — первое, что
захочется сделать читателю. Это завело бы новый отказ на форме, которая
работает.

### §4. Почему `V`, а не `Self`; почему `-> int`, а не параметр `I`

**`V`, а не `Self`.** Срез не обязан быть тем же типом. В дереве уже есть
`str @bytes() -> ro []u8`, а невладеющий вид выражается режимом `ro` и/или
другим типом. Жёсткий `Self` выставил бы такие типы за роль, и им пришлось бы
заводить свою дверь — ровно то расхождение, которое блок убирает. `str` —
`RangeIndex[str]`; `Vec[T]` — `RangeIndex[Vec[T]]`.

**`-> int`, а не параметр.** `Range` не обобщён:
`std/src/collections/range/core.nv` — `export type Range value { ro start int;
ro end int }`, полуоткрытый, границы `int`. Параметр, всегда равный `int`, был
бы запасом на будущее, а не свойством языка. Обобщение `Range` — предмет плана
281, см. §6.

### §5. Развёртка сахара — АМЕНДМЕНТ К D238

D238 записывает развёртку открытого конца через `v.len()`:

```nova
v[2..]   // D238, БЫЛО: v.index(Range { start: 2, end: v.len() })
```

Заменяется на вызов метода роли:

```nova
v[2..]    // → v.index(Range { start: 2, end: v.end_index() })
v[..3]    // → v.index(Range { start: 0, end: 3 })
v[..]     // → v.index(Range { start: 0, end: v.end_index() })
v[a..=b]  // → v.index(Range { start: a, end: b + 1 })
```

Причина замены — §1 п.2: `len` бывает не длиной, а метод отвечает за себя сам
и умеет вычислять границу (кольцевой буфер, вид со смещением). Для `str` и
`Vec[T]` это тривиальная обёртка: `@end_index() => @byte_len()` и
`@end_index() => @len`.

**`str` больше не исключение.** `s[a..b]` разворачивается в `s.index(a..b)` и
попадает в тело `str @index(r Range)` вместе с его контрактом. Наблюдаемое
следствие, названное здесь потому, что оно наблюдаемое: выход за границы теперь
ловит `requires` строкового метода, а не C-хелпер, и текст паники —
контрактный. Это паритет: у `Vec[T]` так с плана 194, и все три его негатива в
корпусе пришпилены к контрактному тексту.

### §6. Что придёт с планом 281 и как роль обобщится

План 281 вводит `Ordinal`, типизированный индекс и `Range[I Ordinal = int]`.
Когда это ляжет, роль станет `RangeIndex[I, V]` — `@index(r Range[I]) -> V`,
`@end_index() -> I`. Это то же движение, а не новая работа: блок один и тот же,
и разделы про `Ordinal`, `IndexVec` и alias-`Vec` пишутся сюда волной 281.
Здесь они НЕ написаны намеренно — их проектирует другое окно, и блок, который
объявил бы за него, был бы догадкой в нормативном тексте.

### §7. Признак спрашивается у реестра МЕТОДОВ, и это не деталь реализации

Замер окна 274, 2026-09-07. Соответствие роли проверяется по реестру МЕТОДОВ
(`all_methods` в эмиттере — та же карта, которой в этом файле задаются все
прочие вопросы «есть ли у типа такой метод»), и НИКОГДА по реестру полей
(`record_schemas`).

Разница не косметическая, у неё есть цена, уже однажды уплаченная. 2026-07-24
предыдущая попытка втянуть `str` в общий путь пошла через запись
`record_schemas["str"]` — и расретрачила `str.from(5)`, который по
[D410](#d410) обязан оставаться ошибкой компиляции: постановка имени `str` в
реестр полей оживляет НЕСВЯЗАННЫЙ путь резолва `.from`, ключующийся на «есть ли
это имя в `record_schemas`». Поймала фикстура `neg_str_from_retracted`, правку
откатили. Роль спрашивает методы, поэтому о полях `str` в этом месте не
становится известно ничего, и капкан остаётся закрытым по построению, а не по
внимательности.

### §8. Что этот блок НЕ меняет

- **семантику среза строки** — по-прежнему байтовый диапазон ([D249](#d249)),
  никакой «умной» индексации по кодпоинтам блок не вводит;
- **охрану границы кодпоинта** — разрез не по границе UTF-8 остаётся паникой
  рантайма (проверка зависит от ДАННЫХ, а не от границ, и потому не может быть
  контрактом); она лишь переехала из C-хелпера в тело Nova-метода с тем же
  текстом;
- **числовые границы как контракт** — `requires` у `@index` остаётся
  контрактом со своей политикой доказательства и снятия;
- **быстрые C-хелперы** — тип вправе держать свою реализацию ВНУТРИ тела
  метода; блок судит признак применимости, а не способ исполнения.

## D476. Временное consume-значение во view-позиции ДЕСУГАРИТСЯ в scope-block, а не отвергается (решение владельца 2026-09-13)

**Статус:** ПРИНЯТО 2026-09-13 (решение владельца в разговоре; номер назначен
интегратором). **Реализации НЕТ ни у одного компилятора.**

**Амендирует:** [D133](02-types.md#d133) — два отказа заменяются формой;
опирается на [D188](#d188-cleanupe-protocol--consume-x--expr-body-scope-block)
(scope-block, к которому десугаринг сводится) и
[D432](02-types.md#d432) (авто-`@cleanup`).

### 1. Что меняется

Сегодня временное значение consume-типа во view-позиции ОТВЕРГАЕТСЯ. Таблица D133
(`02-types.md:5839`, дубль `:6037`): `f(make_tx())` при `f(t Tx)` даёт
`D133-consume-rvalue-in-view`, при `f(mut t Tx)` — `D133-consume-rvalue-in-mut-view`.
Приёмник метода — такая же позиция с режимом (`:5780` «Помечается на: receiver /
param метода»), поэтому `get_luck().print()` при `fn Job print()` отвергается тоже.

**Новая норма: вместо отказа — РАЗВЕРНУТЬ в consume-скоуп.**
...
### 2. Главное свойство: это НЕ новое исключение

Амендмент [D432](02-types.md#d432) §2 (реестр 221.1 №672) формулирует норму через
ПРОИСХОЖДЕНИЕ биндинга и прямо запрещает перечислять освобождённые формы: «форм
биндинга в языке больше, чем три, и каждая неназванная молча освобождалась
исключением, которое кодоген не подкрепляет».

D476 **ничего не добавляет** к множеству освобождённых форм. Он приводит новую
форму к той ЕДИНСТВЕННОЙ, что в нём уже есть (`consume X = e` с одиночным
`Ident`-паттерном). Это и есть причина, по которой решение безопасно: множество
исключений не растёт.

### 3. Правило целиком — семь положений

**П1. Применяется ТОЛЬКО во view-позиции.** `f(make_tx())` при `f(consume x Tx)`
— законная строка (владение передаётся без биндинга, D133 таблица `:5833`), и
десугаринг туда НЕ лезет: обернув её в скоуп, мы отдали бы владение внутрь `f` и
следом попытались прибрать снаружи. Правило замещает собой РОВНО два отказа —
`D133-consume-rvalue-in-view` и `-in-mut-view`, — и не трогает ничего больше.

**П2. Скоуп УЗКИЙ: он обнимает то выражение, в котором участвует временное, а не
весь объемлющий вызов.**
...
Цена широкого скоупа названа, чтобы выбор не выглядел вкусовым: если `get_luck()`
держит соединение или замок, а `g()` медленна, широкий вариант молча расширил бы
критическую секцию на чужой вызов — и сказать «почисти раньше» было бы нечем, ведь
имени значению не дали.

**П3. Несколько временных — ПЛОСКО, а не вложенно.**

```nova
f(get_luck(), get_luck())
// эквивалентно:
{
    consume arg1 = get_luck()
    consume arg2 = get_luck()
    f(arg1, arg2)
}
```

Вложенность нужна только при скоупах РАЗНОЙ длины; здесь они одинаковы, и плоская
форма даёт ту же очистку LIFO в конце блока. **Синтетические имена обязаны быть
непроизносимыми** — иначе затенят пользовательские.

**П4. Блок ОТДАЁТ значение.** Это не новая возможность: D188, амендмент
2026-07-13, уже объявляет форму re-consume выражением — «Блок — ВЫРАЖЕНИЕ:
`ro s = consume X { …; <tail> }` — значение блока = tail-выражение». D476
распространяет то же на binding-форму.

**П5. Хвост-НЕ-ресурс очистку НЕ дизармит.** D188 правило 3 перечисляет законные
пути выноса владения (tail-значение `X`, `return X`, `X` ровно один раз в
consume-позиции) и добавляет: «Все ОСТАЛЬНЫЕ пути выхода … — cleanup
exactly-once». Хвост `t.id()` даёт `int`, то есть попадает в «остальные»: очистка
срабатывает, блок отдаёт значение. **Сказано ЯВНО**, потому что у формы
re-consume хвост несёт ДВОЙНУЮ роль (значение и признак дизарма), и асимметрия,
не названная вслух, читается как недосмотр.

**П6. Позиции без места для оператора особого правила НЕ требуют** — следствие П4.
Раз блок есть выражение, десугаринг идёт в позицию ВЫРАЖЕНИЯ:

```nova
while consume t = get_luck() { t.more() } { … }
```

Условие цикла перевычисляется каждую итерацию → скоуп создаётся и чистится каждую
итерацию. Правый операнд `&&` вычисляется условно → скоуп входится условно. То же
у скрутиния `match` и у `return`. Ни `if`, ни `while`, ни `&&`, ни `match` не
нуждаются в оговорке.

**П7. Порядок очисток ВЫВОДИТСЯ, а не назначается.** Порядок вычисления аргументов
гарантирован во всех функциях (решение владельца 2026-09-13), поэтому очистка идёт
строго LIFO, обратно порядку вычисления — как у defer-ядра
[D314](#d314-единое-ядро-cleanup--defer-как-примитив-defer-kernel). **Нормативная
строка, без которой правило недоформулировано:** очистка временного — ТОЖЕ
наблюдаемый эффект, и она встроена в тот же порядок, а не живёт отдельной
очередью. Иначе `f(make_a(), make_b())` оставляет неотвеченным, что раньше —
очистка `a` или `b`.

### 4. Где правило НЕ действует, и это не дыра

**Типы БЕЗ `@cleanup`** (строго линейные; канонический носитель — `StringBuilder`,
он `@cleanup` не объявляет по замыслу, потому что потребление извлекает ценность).
Десугаринг даёт непотреблённый биндинг, и `D133-not-consumed` срабатывает как
надо. Правило деградирует верно; записано, чтобы не прочли как пробел.

**Именованный биндинг не той формы.** `ro opt = get_job()` остаётся ОШИБКОЙ и
после D476: десугаринг лечит ВРЕМЕННОЕ значение, а не биндинг. Законная запись —
`consume opt = get_job()`. Норма прежняя (D432 §2 амендмент №672: обязательство
снимает только форма `consume X = e;`), D476 её не трогает.

### 5. Диагностики: синтетическое имя НЕ называется НИКОГДА

Десугаринг заводит биндинг, которого в исходнике НЕТ. **Ни одна диагностика не
имеет права назвать синтетическое имя** — она обязана показывать на выражение,
которое написал автор.

Без этого получаем класс «компилятор ругает код, которого человек не писал», и он
проявляется двумя разными путями: требованием дописать `Fail` на строке, где
написана печать (§6), и жалобой на непотреблённый биндинг, которого автор не
заводил (§4). Оба случая выглядят как дефект компилятора, а не как сообщение об
ошибке автора.

### 6. Эффект очистки попадает в ПОДПИСЬ объемлющей функции

По амендменту [D432](02-types.md#d432) от 2026-08-04 эффекты авто-вставленного
`@cleanup` — ПРЯМЫЕ эффекты объемлющей функции; `Fail[E]` очистки обязан появиться
в её сигнатуре, а в `export fn` его отсутствие есть ошибка компиляции с указанием
на биндинг.

**Следствие названо вслух, потому что оно неочевидно:** `get_luck().print()`
внутри `export fn` может потребовать дописать `Fail` — ради строки, которая
выглядит как печать.

**Рассмотрен и ОТВЕРГНУТ обходной путь** «ограничить десугаринг типами с
эффект-чистым `@cleanup`»: он завёл бы ДВА СОРТА consume-типов с разной
эргономикой, и граница между ними невидима в месте использования. Правило
применяется единообразно, а цена гасится §5 — диагностика обязана показать на
`get_luck()`, а не на синтетический биндинг.

### 7. Область: Карина — да, оракул — нет

**Карина** — форма обязательна (план
[274](../../docs/plans/274-novac-self-hosted-compiler.md), перечень видов
[274.9](../../docs/plans/274.9-carina-linearity-consume.md) §2.7).

**Оракул** — работа НЕ планируется, по критерию владельца 2026-09-13: «причина
менять оракул одна — требования Карины, т.к. она без этого не может собраться».
Замер того же дня: в 93 файлах `novac/src` методов `@cleanup` — НОЛЬ,
consume-скоупов — НОЛЬ, единственный consume-тип (`Emitter`) держит
`StringBuilder`, у которого `@cleanup` нет по замыслу. То есть десугаринг сборке
Карины не даёт ничего, и оракулу тем более.

**Следствие, названное до того, как на него наступят:** положительная фикстура на
D476 покраснеет у оракула — он формы не знает. До реализации в корпус идут ТОЛЬКО
отрицательные (сегодняшний отказ `D133-consume-rvalue-in-view`), а положительные
приезжают тем же слиянием, что и реализация. Обратный порядок кончается
фикстурой, которую придётся отключить, а отключённая фикстура выглядит проверкой,
ею не являясь.

### 8. Приёмка

1. `get_luck().print()` и `f(make_tx())` во view-параметр — СОБИРАЮТСЯ и
   ИСПОЛНЯЮТСЯ, `@cleanup` зовётся РОВНО ОДИН раз (счётчик в теле очистки, а не
   «собралось»);
2. `f(get_luck().id(), g())` — очистка срабатывает ДО вызова `g()` (П2);
   доказывается печатью из `@cleanup` и из `g()` и сверкой порядка строк;
3. `f(make_a(), make_b())` — очистки идут LIFO и в общем порядке эффектов (П7);
4. `f(get_luck())` при consume-параметре — скоуп НЕ заводится (П1): владение
   уходит в `f`, двойной очистки нет;
5. тип без `@cleanup` — по-прежнему `D133-not-consumed` (§4);
6. `ro opt = get_job()` — по-прежнему ошибка (§4);
7. **негативная на ТЕКСТ диагностики** (§5): ни одно сообщение не содержит
   синтетического имени; проверяется грепом по выводу, а не чтением;
8. обратная проба на каждую: правило снято — (1) снова отказ, (2) порядок
   меняется, (4) двойная очистка.

### Связь

* [D133](02-types.md#d133) — два отказа, которые правило замещает.
* [D188](#d188-cleanupe-protocol--consume-x--expr-body-scope-block) — форма, к
  которой десугаринг сводится; её правило 3 даёт §П5.
* [D432](02-types.md#d432) — авто-`@cleanup`, амендмент №672 (происхождение
  биндинга) и амендмент 2026-08-04 (эффекты очистки в подписи).
* План [274.9](../../docs/plans/274.9-carina-linearity-consume.md) §2.7 — разбор
  решения, пять вопросов и их ответы; волна K4 несёт тесты.

## D481 (ex-D478, renumber 2026-09-20). Char-литерал: алфавит `raw-char` и запрет сырого управляющего байта (2026-09-18) {#d481}

**Решение владельца 2026-09-18** по замеру интегратора (реестр 221.1 №675).
Блок делает две вещи: переносит НОРМАТИВНУЮ грамматику char-литерала сюда и
определяет то единственное, чего в ней не было, — алфавит `raw-char`.

### §1. Почему блок вообще понадобился

Грамматика char-литерала жила только в `spec/open-questions.ru.md`, раздел
`Q-char-literals`, помеченный «✅ ЗАКРЫТО (2026-05-07)» с оговоркой «оригинальный
текст ниже сохранён для истории». То есть нормативного текста в `spec/decisions/`
не было вовсе, а `08-runtime.md` до сегодняшнего дня числил вопрос ОТКРЫТЫМ —
два документа говорили о статусе разное.

**И это не формальность, а причина настоящего дефекта.** Продукция `raw-char` не
была раскрыта ни одной фразой, поэтому запрета, который лексер мог бы нарушить,
не существовало. Строка реестра №675 три недели утверждала, что «лексер принимает
то, чего спека не разрешает», — судила по правилу, которого нет. Пробел в спеке
дороже неверного правила: неверное правило спорит, отсутствующее — молчит, и
каждый читатель достраивает своё.

### §2. Грамматика — нормативна ЗДЕСЬ

```
char-literal = "'" ( raw-char | escape ) "'"
raw-char     = любой Unicode scalar, КРОМЕ `'`, `\` и управляющих (§3)
escape       = '\' ( "'" | '"' | '\' | 'n' | 'r' | 't' | '0' | 'u{' hex+ '}' )
```

**Набор escape — ЗАКРЫТЫЙ, и он записан ПО ЗАМЕРУ компилятора, а не переписан
со старой страницы.** Замер 2026-09-18: `\n`=10, `\t`=9, `\r`=13, `\0`=0,
`\'`=39, `\"`=34, `\\`=92, `\u{1F600}`=128512 — принимаются; **`\b` и `\f`
ОТВЕРГАЮТСЯ** (`unknown char escape`), хотя старая страница их называла, и форма
`\uHHHH` без фигурных скобок не поддерживается. Старый текст описывал замысел,
код — факт; нормой становится факт.

**НАЗВАННЫЙ ПРОБЕЛ, СОЗНАТЕЛЬНО НЕ ЗАКРЫТЫЙ ЗДЕСЬ:** набор escape у char-литерала
и у строкового ([D467 §2](#d467)) РАЗНЫЙ — у строки есть `\xNN` и `\$`, у char
их нет. Сводить их в один набор — отдельное решение с отдельной ценой, и делать
это походя, внутри блока про управляющие байты, было бы тем самым «заодно», из-за
которого правки потом не отменить по частям.

### §3. Сырой управляющий байт ОТВЕРГАЕТСЯ

`raw-char` не может быть управляющим символом: **U+0000–U+001F и U+007F**. Сюда
попадают табуляция, CR, LF и вертикальная табуляция — то есть ровно те формы,
которые до сих пор принимались молча. Диагностика — **`E_CHAR_RAW_CONTROL`**, и
её текст обязан называть замену: `'\t'`, `'\n'`, `'\r'`, `'\0'`, `'\u{…}'`.

**Довод — не стройность, а замер.** Символ, которого в исходнике НЕ ВИДНО,
читатель не может ни заметить, ни отличить от соседнего: `'<TAB>'` и `'<VT>'`
выглядят одинаково, а `'<LF>'` вообще разрывает строку. Окно, писавшее лексер
novac, на этом обожглось: собранный бинарь зациклился на выделении памяти, пока
Boehm не умер с «Too many heap sections», и причиной был именно невидимый байт
в литерале (`novac/src/lex/lex.nv:235`, маркер `[LEGACY-#675]`). Байт при этом
компилятор давал ПРАВИЛЬНЫЙ — ошибка была не в семантике, а в том, что автор
написал не то, что думал. Такой класс не ловится ни тестом, ни ревью: глазу не
на что смотреть.

### §4. Что остаётся законным

Пробел (U+0020) — обычный `raw-char`, он видимый и однозначный. Строковый литерал
этим блоком НЕ затронут: его правила живут в D467, где сырой перевод строки уже
запрещён (`E_STR_RAW_NEWLINE`), а прочие управляющие байты внутри `"…"` —
вопрос того блока, не этого.

### §5. Что это ретрактирует

* `Q-char-literals` в `spec/open-questions.ru.md` перестаёт быть источником нормы
  и получает отсылку сюда; его текст остаётся историей;
* формулировка `08-runtime.md` «char-литералов (`'a'`) — ещё открытый вопрос»
  снимается: вопрос закрыт этим блоком.

## D484 (амендмент D188). Порядок вычисления аргументов: норма поднята из прозы в самостоятельную и объявлена НАБЛЮДАЕМОЙ (2026-09-20) {#d484}

**Номер, дом и форма — интегратор; текст — окно Карины.** Форма именно
амендмента, а не нового блока: правило уже существует, и второй дом на одно
правило разошёлся бы с первым на первой же правке.

### §1. Правило УЖЕ несущее, и живёт оно внутри другого правила

[D188](#d188), пункт 4 («Точка дизарма — момент передачи владения»),
03-syntax.md:9544, дословно:

> …после того, как ВСЕ аргументы ЭТОГО вызова вычислены — Nova's порядок
> вычисления аргументов слева направо

Это не оговорка: на этом порядке СТОИТ само правило точки дизарма. Пример
D188 — `return Ok(wrap(stream, risky()))` — разбирает, что происходит, если
`risky()` бросит ДО входа в `wrap`, и вопрос «до» имеет смысл только при
определённом порядке.

### §2. Что этот блок делает: поднимает норму из прозы

Правило, спрятанное внутри правила про cleanup, находит только тот, кто уже
знает, что оно там. Отсюда — норма получает собственный адрес, а в D188 п.4
ставится отсылка сюда.

**Нормативная формулировка:** аргументы вызова вычисляются **слева направо, в
порядке записи в исходнике**. Порядок **НАБЛЮДАЕМ**: если два аргумента
производят эффекты, программа обязана увидеть их именно в этом порядке.

Слово «наблюдаем» — единственное, что здесь ДОБАВЛЕНО к D188, и оно несёт всю
нагрузку: без него правило есть подсказка компилятору, которую он вправе не
исполнить; с ним — правило, которое можно нарушить и поймать тестом.

### §3. Почему подъём понадобился — замер о том, как прячется правило

**Одну и ту же ошибку сделали трижды, и дважды она записана.** 2026-08-16
интегратор заключил по грепу, что спека молчит, и сам же поправил себя в
реестре (строка 685, дословно: «СПЕКА НЕ МОЛЧИТ — ОНА ЭТО ОБЪЯВЛЯЕТ, И ПРАВИЛО
НЕСУЩЕЕ… ошибка была моя: узкий греп»). 2026-09-20 ту же ошибку повторили ОБА
окна независимо: окно Карины грепом, интегратор — подтверждением его замером.
Нашлось правило не грепом, а РЕЕСТРОМ, где месячной давности поправка ждала
читателя.

Вывод, ради которого §3 существует: **правило, лежащее внутри чужого правила,
неотличимо от отсутствующего для всех, кроме автора.** Оно не защищено даже
тем, что записано, — три поиска подряд его не нашли. Поэтому цена подъёма не
в стройности, а в находимости.

### §4. Оракул НАРУШАЕТ эту норму — это дефект, а не расхождение вкусов

Реестр 221.1 **№685**: «порядок вычисления аргументов объявлен спекой и
нарушен кодогеном; на нём же стоит правило точки дизарма». Замер той строки —
`third / second / first`. Замер окна Карины 2026-09-20 на своей программе:
Карина печатает `left`, `right`, `3`, оракул даёт обратный порядок.

**Отсюда следствие для учёта расхождений, и оно противоположно первой
редакции этого блока:** это НЕ осознанное расхождение двух реализаций. Норма
одна, Карина её исполняет, оракул её нарушает — значит строка в
`novac/divergences.allow` была бы неправдой: она говорит «расходимся
НАМЕРЕННО». Правильный адрес — №685, и он уже заведён.

**НАЗВАННЫЙ ПРОБЕЛ:** ни одна фикстура корпуса не содержит вызова с двумя
эффективными аргументами, поэтому дифференциал на 45 фикстурах сошёлся, а
нарушение нормы оракулом не судится ничем. Фикстура — работа той же волны.

### §5. Что реализовано и где граница — ровно проверенное

Волна F65 (Карина, 2026-09-20) сняла отказ компилировать вызов с двумя и
более эффективными аргументами: каждый такой аргумент опускается в свой локал
в порядке исходника ДО вызова, потому что C порядок вычисления аргументов не
определяет. То есть волна **исполняет норму D188**, а не вводит новую.

* **Свободный вызов `f(a(), b())`** — проверен запуском, норма держится.
* **Вызов метода `x.m(a(), b())`** — идёт тем же путём, но **запуском не
  проверен**; здесь сказано «ожидается то же», а не «доказано».
* **Конструктор и литерал записи** — этим блоком не решаются; порядок внутри
  литерала со spread имеет свой текст (03-syntax:4674).

Граница названа списком намеренно: «слева направо» звучит как правило обо
всём, и через неделю его процитируют про конструктор.

## D485. `loop` как выражение: `break <значение>` и тип цикла (2026-09-25) {#d485}

**Номер — интегратор (2026-09-25); текст — окно Карины; предмет — слово владельца
2026-09-25: «`loop` может возвращать значение: `ro a = loop { break 2 }`».** Новый
блок, а не амендмент: формы `break <значение>` в спеке не было нигде
(`syntax.ru.md`, «Циклы»: «`break` / `continue` — стандартные», без операнда;
`open-questions.ru.md` Q-labeled-loops: «value-carrying `break outer x` — вне
scope»). Тип самого `break`/`continue` — `never`, и это живёт в AMEND D25
(04-effects.md, D25, раздел «`never` — почему `throw` совместим с любым типом»); здесь — только
значение и тип ЦИКЛА.
...
ro first = loop {
    i += 1
    if i * i > 50 { break i }     // значение цикла
}
println(first)                    // 8
```

* `break <выражение>` — `break` и выражение **на одной строке**. `break`, за
  которым строка кончается, — голый (грамматика строчная, D452).
* `continue` значения не несёт никогда.
* Меток нет: адресный `break outer` — открытый вопрос Q-labeled-loops, и значение
  с меткой остаётся вне scope до его решения.

### §2. Где значение законно

**Только в `loop`.** `break <значение>` в `for` и `while` — ошибка компиляции
`E_BREAK_VALUE_OUTSIDE_LOOP_EXPR`: у этих циклов есть выход, не проходящий через
`break` (условие ложно, коллекция кончилась), и значения на этом выходе нет. Так же
в Rust; довод тот же, а не заимствование ради заимствования.

`break` целится в **ближайший** охватывающий цикл. Значение из вложенного `for`
во внешний `loop` передать нельзя — это адресный `break`, см. §1.

### §3. Тип `loop`

| что в теле | тип выражения `loop { … }` |
|---|---|
| ни одного `break`, целящегося в этот цикл | `never` (выход — только `return`/`throw`/`panic`) |
| только голые `break` | `()` |
| `break e1`, …, `break eN` | общий тип `e1 … eN` — тот же, что у веток `if`/`match` |

* Смешивать в одном цикле голый `break` и `break <значение>` — ошибка
  `E_LOOP_BREAK_MIXED`: голый `break` означал бы значение, которого нет.
* Значения разных типов, не сводимые к общему по правилу веток, — ошибка
  `E_LOOP_BREAK_TYPE_MISMATCH`, с позициями обоих `break`.
* Литерал без контекста типизируется по [D44](#d44-числовые-литералы) как в ветке
  `if`; ожидаемый тип (`ro a u8 = loop { break 1 }`) проходит внутрь, как в ветку.

### §4. Где `loop` стоит

`loop` со значением — выражение и стоит везде, где стоит `if`-выражение:
инициализатор, аргумент, хвост тела, операнд. `loop` без значения в позиции
оператора — прежняя форма, ничего не меняется.

`consume`-биндинги внутри тела — по правилу циклов [D156](02-types.md#d156-generic-t-consume-bound--collection-aware-iteration)
и 05-memory («Циклы — пессимистично»): `break <значение>` передаёт значение
ИЗ цикла так же, как `return` передаёт его из функции.

### §5. Реализация и расхождение

* **Карина:** часть 3 волны 274.7 В10 (`loop` целиком), тем же слиянием, что этот
  блок. Приёмка — фикстуры против нормы; смоук с оракулом — только на голом `break`.
* **Оракул** формы не читает (`break i * 10` — `E_STMT_SEP_MISSING`, замер
  2026-09-25) и `loop` типизирует как `never` безусловно. Релиз оракула на паузе;
  расхождение — в пользу нормы, строка реестра заводится при первой фикстуре,
  которая его покажет.
