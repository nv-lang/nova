...
3. **Вынос владения — tail/return самого `X`, либо (амендмент v3 ниже)
   `X` РОВНО ОДИН раз аргументом consume-параметра внутри tail/return
   EXPR.** Легальные формы выноса — tail-значение `X` (при биндинге-
   приёмнике) или `return X` (ГОЛЫЙ `X`), а также — амендмент v3,
   2026-07-13, Plan 201, §«Амендмент v3» ниже — tail/return **EXPR**, где
   `X` встречается ровно один раз, аргументом consume-параметра
   (`return Ctor(x, other)`, `return Ok(TlsStream.wrap(stream, session))`).
...
2. **Вхождение — в consume-позиции.** Единственное вхождение обязано быть
   АРГУМЕНТОМ параметра, помеченного `consume`, у вызова (свободная fn,
   метод `obj.method(...)`, статический `Type.method(...)` /
   `module.fn(...)`) — НЕ receiver'ом (`X.method()` НЕ подпадает: это
   передача владения через RECEIVER, не через argument-позицию — по-
   прежнему `E_CONSUME_BLOCK_MOVE_OUT`), НЕ view/mut-параметром. Частный
   случай — EXPR = голый `X` (п.3 §«Правила формы» — как в v2,
   БЕЗ изменений).
...
4. **Точка дизарма — момент передачи владения** (вход в consuming-вызов,
   ...
   дизарм → `wrap` теперь владеет `stream`.
5. **Нарушения — тот же `E_CONSUME_BLOCK_MOVE_OUT`, уточнённый текст.**
   Все три failure-режима (>1 вхождение; единственное вхождение НЕ в
   consume-позиции; захватывающее замыкание) остаются ОДНИМ кодом ошибки
   `E_CONSUME_BLOCK_MOVE_OUT` — текст диагностики уточняет причину.
...
**Статус: ПРИНЯТО** (владелец 2026-07-07, «добро п.1, 2»). Реализация в очереди
