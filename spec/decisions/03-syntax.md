...
> f64 не сужается молча). Маркер `[M-154.1-f32-literal-coercion]` ✅.
...
## D253. `std/unicode` — нормализация (UAX #15) + grapheme-сегментация (UAX #29) + case folding/mapping

Added: 2026-06-14  Status: 152.4.1+152.4.2 (нормализация) + 152.4.3 (graphemes) + 152.4.4 (case folding/mapping) IMPLEMENTED 2026-06-14; остаётся только title-casing (нужны UAX #29 word boundaries, `[M-152-word-boundaries]`)
...
SpecialCasing.txt + UnicodeData.txt[12,13] + DerivedCoreProperties.txt
Cased/Case_Ignorable): FOLD/LOWER/UPPER maps (`cp:m1,m2;..`) + CASED/CASE_IGNORABLE
ranges. Ленивые `ro` HashMap/range-таблицы (как нормализация/graphemes).
...
- Case (152.4.4): `case_conformance.nv` — breadth по всем mapped codepoints
  ...
  Выборку (no-locale: Turkic-excl, field-index, 3-cp, Final_Sigma+case-ignorable) пиннит
  **independent hand-oracle** в `case.nv`. Все — `nova-codegen unicode --emit-conformance`.

См. [Plan 152.4](../../docs/plans/152.4-std-unicode.md), Q-unicode-data
(open-questions.md). Phase B остаток: title-casing (`[M-152-word-boundaries]`),
collation (152.5b). `[M-152-case-fold]` ЗАКРЫТ (152.4.4). AMEND D250 §«линзы» —
`as_graphemes` третья линза.
