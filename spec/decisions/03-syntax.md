...
> Проверяемая форма для ЦЕЛЫХ — `@try_to_i8()…@try_to_uint()` (D430, blanket по `Ints`):
> `(300 as u32).try_to_u8() == Err(RangeError)`. Для float → целое проверяемой формы **нет**;
...
**Если нужна проверка** out-of-range — для ЦЕЛЫХ есть `@try_to_*` (D430),
...
ro n = i16.try_from(f)?         // МЁРТВАЯ ССЫЛКА — см. амендмент 2026-09-04 выше; для целых: (x).try_to_i16()
```

`as` остаётся **pure** (без Fail-эффекта). Форма, возвращающая ошибку,
— это `@try_to_*` (D430) для целых; для float её нет.
...
>
> **1. `str.from(v)` не существует** (`[E_UNKNOWN_STATIC_METHOD]`). Строка таблицы
> исправлена на `v.to_str()` — и это не выбор из двух, а тот же канон «конверсия —
> метод на ИСТОЧНИКЕ», которым амендмент 2026-08-01 уже заменил `T.try_from(s)` на
> `s.to_*()`. Проба: `(42).to_str()`, `(1.5).to_str()`, `true.to_str()` собираются и
> печатают `42`, `1.5`, `true`.
>
...
> `runtime/string/core.nv:354`) — но она даёт `str`, а не `char`; исключение для
...
>
> **Чем её заводить — НЕ решено, и решать это здесь нельзя.** Два имени спорят:
> `char.try_from(n)` (как было написано) против `n.to_char()` (канон «метод на
> источнике», по которому уже перестроены `str`-парсеры). Выбор — за владельцем;
> вопрос той же природы, что float → целое, и живёт в той же строке реестра, в её
> половине (б) «сама форма — работа по std ПОСЛЕ тега». До ответа таблица честно
> говорит «формы нет», а не отсылает к отсутствующей.
>
...
`str.try_from_codepoint` и аналогичных stdlib-функций где range-check уже
