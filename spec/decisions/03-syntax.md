...
| [D102](#d102-именованные-аргументы-и-значения-параметров-по-умолчанию) | Именованные аргументы `f(name: val)` и значения параметров по умолчанию `fn f(x int = 0)` |
...
> **Status:** active (spec). Реализация — [Plan 46](../../docs/plans/46-named-parameters.md).

### Что

Параметр функции может иметь **значение по умолчанию**; на месте
вызова аргумент может передаваться **по имени**:

```nova
fn connect(host str, port int = 8080, tls bool = false) -> Conn

connect("localhost")                       // дефолты для port, tls
connect("localhost", 9000)                 // позиционно
connect("localhost", port: 9000)           // именованно
connect("localhost", tls: true, port: 80)  // именованные переставимы
```
...
```nova
f(1, 2, flag: true)        // позиционные, затем именованные
f(1, opt: 5)               // часть позиционно, часть именованно
f(required: 1, opt: 5)     // всё именованно
f(1, flag: true)           // opt опущен → дефолт
```

1. **Позиционные аргументы идут первыми**, связываются слева направо.
   Именованный аргумент **не может предшествовать** позиционному —
   `f(opt: 5, 1)` — compile error.
2. **Именованные аргументы переставимы** между собой.
3. **Каждый параметр связывается ровно один раз.** Передать параметр
   и позиционно, и по имени — compile error (`f(1, required: 2)`).
4. **Параметр с дефолтом можно опустить;** параметр без дефолта —
   обязателен (позиционно или по имени).
5. Имя в `name: expr` — это **имя параметра callee**, не выражение.
...
- **D43 trailing-block / trailing-fn.** Trailing-форма связывается с
  **последним** функциональным параметром. Если этот параметр имеет
  имя — передать его *и* trailing-формой, *и* именованным аргументом
  нельзя (правило 3, «связан дважды»).
...
1. **Self-documenting вызовы.** `connect("h", tls: true)` вместо
   `connect("h", false, true)` — позиционные `bool`-флаги нечитаемы и
   это классическая ошибка LLM-генерации. AI-first язык должен делать
   call-site явным.
2. **Убирает builder/option-struct boilerplate** для простых случаев
   «функция с несколькими опциональными настройками».
3. **Включает `supervised(cancel: tok)`** — синтаксис structured
   concurrency ([D75](06-concurrency.md#d75)) опирается на эту фичу.
4. **Call-site evaluation дефолтов** — нет Python-гочи с разделяемым
   mutable-дефолтом.
...
- **Обязательно-именованные параметры** (Swift `func f(name x:)` где
  имя обязательно на call-site) — лишняя церемония; любой параметр
  можно передать и позиционно, и по имени.
...
- [Plan 46](../../docs/plans/46-named-parameters.md) — реализация.
