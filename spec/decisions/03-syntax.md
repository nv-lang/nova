...
> (беглые цепочки `-> @`). Синтаксис тел методов.

### Что

Метод с возвратом `-> @` больше не обязан возвращать приёмник вручную:

- отсутствие хвостового выражения в теле → неявный `return @`;
- голый `return` (без значения) в таком теле → `return @`;
- явные `@` в хвосте и `return @` остаются легальными (обратная совместимость,
  существующие тела не переписываются).
...
3. Чекер: тело/ветки без значения — легальны; кодогенератор подставляет возврат
   приёмника на каждом выходе без явного значения.
