...
> 2. **Инлайн-форма НЕ вводится:** `lo int /// end` остаётся ошибкой
>    (`E_RECORD_FIELD_MISSING_SEPARATOR` — `///` токен, не trivia; парсер видит
>    второе поле без разделителя). Это было бы второе правило привязки, которого
>    нет нигде в языке, и длинная дока в хвост строки не переносится. Текст
>    ошибки дополнен подсказкой: «doc-comment for a field goes on the line
>    ABOVE it».
...
> 6. **Потребители:** LSP hover/outline поля и варианта показывают их doc, как у
>    декларации; `nova doc` рендерит doc поля/варианта в таблице типа.
>
> ```nova
