...
**Явное переопределение — формы владельца 2026-09-02.** Нужны, когда указать надо не свой
параметр, а полученный от кого-то ещё:

| механизм | формы |
|---|---|
| контракт | `requires bool` · `requires bool, str` · `requires bool, CallerLoc` · `requires bool, str, CallerLoc` |
| `assert` | `assert(bool)` · `assert(bool, str)` · `assert(bool, CallerLoc)` · `assert(bool, str, CallerLoc)` |
| `panic` | `panic(str)` · `panic(str, CallerLoc)` |

Формы без `str` берут сообщение автоматически, как сегодня.

**Грамматический слот у контракта свободен, и это проверено, а не предположено:**
сообщение контракта обязано быть строковым ЛИТЕРАЛОМ (`parse_opt_contract_message`,
иначе `[E_CONTRACT_MESSAGE_NOT_STRING]`), поэтому идентификатор в этой позиции сегодня —
ошибка. Различение «литерал → сообщение, значение `CallerLoc` → место» однозначно и не
требует правил приоритета.

### Цепочка — явная

```nova
fn ensure(ok bool, loc CallerLoc = caller_loc()) -> () {
    if !ok { ice("assertion failed", loc) }      // loc ПЕРЕДАЁТСЯ РУКАМИ
}
```
