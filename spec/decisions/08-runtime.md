...
### Правило тела: контракт, `assert` и `panic` берут параметр сами

Решение владельца 2026-09-02. **Если у функции есть параметр типа `CallerLoc`, то
`requires`, `ensures`, `assert`, `debug_assert` и `panic` в её теле по умолчанию
указывают на него** — без повторного упоминания в каждой строке:
...
- **[D462](#d462) — значение поля `site` не меняется.**
