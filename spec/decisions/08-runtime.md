...
**Правило — по владельцу вины, а не одно на всех:**

| механизм | чья вина | место, на которое указывает |
|---|---|---|
| `requires` / `ensures` на помеченной функции | вызывающего: он нарушил обязательство | **место вызова** |
| `assert` / `debug_assert` в теле | самой функции: это её внутренний инвариант | **своя строка, всегда** |
| `panic()` и пользовательская дверь (`ice()`) | по смыслу — о вызывающем | **место вызова** |

Одной фразой: **проверка обязательства ПЕРЕД вызывающим указывает на вызывающего;
внутренняя проверка указывает на себя.** Обратное решение («всё следует за атрибутом»)
отвергнуто по конкретному вреду: `assert`, поймавший баг САМОЙ обёртки, показывал бы
чужую строку, и чинили бы не то.

**Что это значит для записи отказа D462:** поле `site` у отказа, пришедшего из контракта
помеченной функции или из её `panic`, несёт место вызова; у отказа из `assert` в теле —
своё. Формат не меняется, меняется значение одного поля — ровно то, о чём говорит раздел
«Границы» ниже.

**Область изменения — только помеченные функции.** У непомеченной всё остаётся как
сегодня. Вопрос «а не должен ли `requires` называть вызывающего ВСЕГДА, без атрибута»
поставлен отдельно (план 280, открытый вопрос 4): ответ там же — умолчание «нет, только
под атрибутом», потому что это смена существующего поведения всей поставки, и её цена
меряется отдельно.
