...
> ```nova
> type str value priv { ptr *ro u8, len int }   // 16 байт, stack, copy-семантика
> ```
> `value` → stack-аллокация, 16 байт, copy-семантика (как было). `priv` →
> поля видны только методам str. `ptr *ro u8` → указатель на иммутабельный
> UTF-8 буфер (D216 §1 `*ro T` — flagship use-case). `len` — длина в **байтах**
...
> иммутабельным буфером (`*ro u8` делает sharing безопасным). См.
...
>   `*ro u8` — нет легального write-path), контракт сохранён.
>
