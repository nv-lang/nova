...
>
> **Методы — Nova-body (Ф.1/Ф.2):** `starts_with`/`ends_with`/`contains`/
> `find`/`rfind`/`char_at`/`char_len`/`trim`/`to_lower`/`to_upper`/
> `to_bytes`/`to_chars` мигрированы из external `nova_str_X` C-функций в
> Nova-тела в `std/runtime/string.nv` (читают байты через `@as_bytes()`
> zero-copy view + `@byte_at`, аллоцируют через `[]u8.with_capacity`+`push`+
> `str.from_bytes_unchecked`). Семантика байт-в-байт идентична C-оригиналам.
> **Irreducible C-примитивы (≤2):** (1) byte-access `nova_str_byte_at` O(1);
> (2) buffer-alloc через `str.from_bytes_unchecked`. UTF-8 decode-курсор —
> один helper `cp_to_char`.
>
