...
`debug_assert`, `requires`/`ensures` и `caller_loc()`. Атрибут значит «место вызова
протекает через меня» без исключений.
...
| механизм | БЕЗ `#track_caller` | ПОД `#track_caller` |
|---|---|---|
| `requires` / `ensures` | **место вызова** (новое) | место вызова, ПРОБРОШЕННОЕ по цепочке |
