...
> `consume`/supervised assert классифицировался как recoverable **Failure**, не
> **Panic**. Теперь `nova_assert_loc` (как `nv_panic`, D188) ставит `error_kind =
> NOVA_THROW_PANIC` → классификация совпадает с panic и с контрактами ([D24](09-tooling.md#d24)
> amend). `[M-140-contract-panic-unwind]`.

> **AMEND (Plan 140.1, 2026-06-12) — assert/debug_assert: location-first
> формат + `file:line`.** `assert`/`debug_assert` failure = panic (D13);
> Plan 140.1 уточняет **диагностический текст** этого panic'а. Прежний
> `assertion failed: <expr>` (`nova_assert` в `effects.h`) — без локации
> (не найти, где упало) и со словом «assertion» — **RETRACT**. Новый
> формат — общий с контрактами ([D24](09-tooling.md#d24), location-first):
> ```
> <file>:<line>: assert failed: <expr>
> ```
> Пример: `foo.nv:10: assert failed: a > 0`. С пользовательским
> сообщением (`assert(a > 0, "must be positive")`, прецедент
> [D84](10-overloading.md#d84)) — тот же принцип, выражение в скобках:
> ```
> <file>:<line>: assert failed: <msg> (<expr>)
> ```
> Пример: `foo.nv:10: assert failed: must be positive (a > 0)`.
>
> Правила: (1) `<file>:<line>:` **всегда** первый, авто-проставляется
> codegen'ом из call-сайта (`__FILE__`/`__LINE__`) — пользователь его
> **не** включает в свой текст; (2) `assert failed:` сохраняется (слово
> `assertion` → `assert`); (3) `debug_assert` использует **тот же** текст
> `assert failed` — отличие только в debug-only-гейте ([D81](#d81)), не в
> сообщении; (4) `<msg>` опционален — без него формат без скобок. Реализация
> прокидывает `file`/`line` в `nova_assert` (Plan 140.1 Ф.2). Этим assert
> и контракты дают **единый** кликабельный location-first диагностический
> формат. См. также [D81](#d81) (assert/debug_assert build-mode) и
> [D24 → «Формат runtime-violation»](09-tooling.md#формат-runtime-violation-plan-1401).
...
> - `std/prelude/protocols.nv` — `From`/`Into`/`Hash`/`Equal`/
>   `Compare`/`Display` (6 formal protocols; `TryFrom`/`TryInto`
...
char-литералов (`'a'`) — закрыт [D481](03-syntax.md#d481) (2026-09-18; до него эта строка расходилась с `Q-char-literals`, где вопрос числился закрытым с мая).
...
> **MAJOR AMEND (Plan 139, ЗАКРЫТ 2026-06-11): `str` — Nova value-type,
> не C-примитив.** `str` теперь — **Nova value-record lang-item**:
> ```nova
> type str value priv { ptr *u8, len int }   // 16 байт, stack, copy-семантика
> ```
> `value` → stack-аллокация, 16 байт, copy-семантика (как было). `priv` →
> поля видны только методам str. `ptr *u8` → указатель на иммутабельный
> UTF-8 буфер (`*T` = ro-pointee, D246 — flagship use-case; immutability
> выражена типом указателя, `*ro u8` избыточен → `E_REDUNDANT_POINTER_RO`).
> `len` — длина в **байтах**
> (D26 §«str.len = bytes»; public `len()` — codepoints).
>
> **ABI (risk-limiter, Ф.0):** value-record лоуэрится в C-структуру
> **layout-идентичную** старому `nova_str`: `typedef struct {const uint8_t* ptr;
> int64_t len;} nova_str;` (было `{const char* ptr; size_t len;}`). На x64
> `const char* ≡ const uint8_t*` и `size_t ≡ int64_t` → все **~354
> рантайм-C-вхождения** (net.c/effects/channels/sync/vtables/string_builder/
> conv/fibers) продолжают работать через `nova_str`-typedef-алиас БЕЗ правок.
> `sizeof(str) == 16`. Typedef определён в `nova_rt.h:56` + `vtables.h:42`.
>
> **Методы — Nova-body (миграция Plan 139.2 Ф.0-Ф.3):** `starts_with`/
> `ends_with`/`contains`/`find`/`rfind`/`char_at`/`char_len`/`trim`/`to_lower`/
> `to_upper`/`to_bytes`/`to_chars` (139 Ф.1/Ф.2), `as_bytes` (139.2 Ф.0),
> `split`/`from_bytes_unchecked`/`from_bytes_lossy`/`from_bytes_unchecked_steal`
> (139.2 Ф.2), `concat`/`compare` (139.2 Ф.3) мигрированы из external
> `nova_str_X` C-функций в Nova-тела в `std/runtime/string.nv` (читают байты
> через `@as_bytes()` zero-copy view + `@byte_at`, аллоцируют через
> `[]u8.with_capacity`+`push`+`str.from_bytes_unchecked`). Семантика байт-в-байт
> идентична C-оригиналам. **9/10 str-методов — Nova-body.** Остаётся C только
> `@hash` (SipHash-1-3 + crypto-seed, DoS-resistance — `nova_str_hash`).
> **Operator-lowering ОСТАЁТСЯ на C (option (b), 139.2 Ф.3):** операторы
> `+`/`<`/`<=`/`>`/`>=`/`==`/`!=` над `nova_str` лоуэрятся ОТДЕЛЬНО, напрямую в
> `nova_str_concat`/`nova_str_lt`/…/`nova_str_eq` (perf hot-path: один alloc+
> 2×memcpy / один memcmp vs Nova push-loop / byte-loop); ПРЯМЫЕ method-вызовы
> `s.concat(t)`/`s.compare(t)` идут в Nova-body. См.
> [02-types.md «Amend (Plan 139.2 Ф.3)»](02-types.md#d246-три-оси-мутабельности-l1-binding--l2-view--l3-pointee)
> + `[M-139.1-operator-lowered-methods]` (reframed).
>
> **eq/hash/clone (Ф.3) — content, не pointer.** `==`/`!=`/`<`/`<=`/`>`/`>=`
> лоуэрятся напрямую в BinOp codegen → `nova_str_eq`/`nova_str_lt`/… (memcmp/
> byte-loop). `emit_field_eq` спец-кейсит `cty == "nova_str"` → `nova_str_eq`
> ПЕРЕД Plan 141 field-by-field — поэтому str-in-tuple/record/sum и
> str-keyed `HashMap` (hash через `nova_str_hash` SipHash-1-3 по байтам)
> content-keyed автоматически. `@clone` = 16-байт handle-copy над общим
> иммутабельным буфером (`*u8` ro-pointee делает sharing безопасным). См.
> [D228](02-types.md#d228) content-eq override.
>
> **Литералы интернируются (Ф.6):** идентичные литералы делят один
> `static const uint8_t[]` буфер + один `static const nova_str` (per-CU
> rodata dedup; content-hash символы; семантически невидимо т.к. eq —
> content). См. §«Compile-time литералы интернируются» ниже.
>
> **GC (Ф.5):** `str` value — 16-байт stack-значение `{ptr,len}`; `ptr`
> указывает в heap (RawMem/`nova_alloc`, GC-tracked) либо rodata (литералы/
> interned — никогда не собираются). Conservative stack-scan покрывает
> value-record str на стеке (ptr-поле — обычный указатель в 16-байт значении).
>
> **Открытые followups (никогда не silently dropped):**
> `[M-139-f0-lang-item-decl]` — полная Nova-декларация + privacy-enforcement
> (`s.ptr`→E_PRIV_FIELD); сегодня str — compiler-builtin, layout известен
> codegen'у, но privacy-gate + member-access `@ptr` field-доступ требуют новой
> lang-item checker-инфры. Гейтит: `[M-139-f1-trim-view]` (zero-copy trim-view),
> `[M-139-f2-ptr-field-producers]` (`as_bytes`/`split`/`from_bytes_*` остаются
> тонкими C-примитивами — C `as_bytes` уже zero-copy, контракт сохранён).
> `[M-139-f0-rt-header-ptr-sign-casts]` — 59 -Wpointer-sign warnings в
> рантайм-хедерах (source-compatible, подавлены `-w`).
>
> **Q139-блоки (resolved / extracted):**
> - `Q139-gc-stack-scan` → **RESOLVED**: conservative stack-scan достаточен;
>   str-value на стеке — 16-байт `{ptr,len}`, ptr-поле сканируется как обычный
>   указатель; буферы static-rodata (литералы/interned, не собираются) либо
>   RawMem/`nova_alloc` (GC-tracked).
> - `Q139-literal-buffer-lifetime` → **RESOLVED**: литералы — static rodata
>   (program lifetime, никогда не GC'ятся); built-строки — heap, GC-tracked.
> - `Q139-utf8-cursor-primitive` → **RESOLVED**: один helper `cp_to_char`
>   (codepoint→char) + byte-курсор по `@as_bytes()`; единственный decode-helper.
> - `Q139-str-eq-override` → **RESOLVED**: explicit special-case в
>   `emit_field_eq` (`cty=="nova_str"`→`nova_str_eq`) + direct BinOp lowering;
>   str никогда не доходит до Plan 141 field-by-field. См. [D228](02-types.md#d228).
> - `Q139-cstr-nul-termination` → **RESOLVED** (§3 выше, Ф.4): alloc-fallback
>   через `nova_str_terminated_ptr`.
> - `Q139-intern-scope` → **RESOLVED** (§interning ниже, Ф.6): per-CU.
> - `Q139-as-bytes-aliasing` → **EXTRACTED** в `[M-139-f2-ptr-field-producers]`:
>   pure-Nova zero-copy `as_bytes` через `@ptr`-поле gated на
>   `[M-139-f0-lang-item-decl]`; C `as_bytes` уже zero-copy (`ro Vec` view над
>   `*u8` ro-pointee — нет легального write-path), контракт сохранён.
>
> Спецификация value-record: [D228](02-types.md#d228). Указатель-поле:
> [D216 §1](02-types.md). Таксономия value/reference: [D52](02-types.md#d52).
...
> **⚠ AMEND Plan 152.1 (D249, в процессе 2026-06-13):** школа B (всё codepoint-
> indexed) **разворачивается** на байт-координаты + линзы (D249, `docs/plans/152.1`).
> **УЖЕ сделано (Ф.2):** `@find`/`@rfind` возвращают **БАЙТОВЫЙ offset** (композируются
> с byte-slice `s[k..]` за O(1); cp-offset был несоставим). **В процессе:** `str[i]`
> (int) → `E_STR_NO_INT_INDEX` (Ф.1), бэар `len()` → `E_STR_NO_LEN` + `as_chars()` линза
> (Ф.4). Раздел «Почему codepoint-indexing» ниже — **исторический** (school-B
> reasoning), отвергнут D249 в пользу прозрачности стоимости (O(n) под `[i]`/`len` —
> «ложь»). Пример с `i == 6 (codepoints)` устарел: теперь `find` даёт байт-offset.
...
> ⚠️ **НБ (2026-07-13): D73 сам РЕТРАКТИРОВАН 2026-07-06** — протоколы From/Into
> упразднены; актуальный канон = именные конвенции `.from(x)` (конструктор-конверсия),
> `to_str()`/`to_*` (представление, D410), `consume @into_цель()` (владение, D131).
> Migration map ниже — исторический (описывает переход D70→D73 эпохи протоколов).
>
...
> **⛔ РЕТРАКЦИЯ (2026-07-06, решение владельца; переисследование с чистого листа).**
> Протоколы `From[T]` / `Into[U]` (вместе с D77 `TryFrom`/`TryInto` — все четыре)
> **упраздняются**. Мотив: их драйверы — растовские, в Nova мертвы. (1) В Rust
> конверсионные баунды — костыль отсутствия перегрузок; в Nova перегрузки есть (D84),
> и как generic-баунд `From`/`Into` в живом std не используется НИ РАЗУ. (2) `?` у нас
> не делает From-конверсию ошибок (D325: один `XError` на домен, конверсия явная).
> (3) Все 103 вызова `.into()` исполняли роль «представление в строку» — это ось
> `to_str()` (D410), а не передача владения; `v.into()` был единственным вызовом языка,
> чей смысл не виден без вывода типа цели. (4) Вслед уходит вся обслуга: blanket
> identity `From` (синтез per mono), auto-derive `From→Into`, 4-шаговый resolution,
...
> `TryFrom[T, E]` / `TryInto[U, E]` упраздняются (мотивы — в D73-баннере).
> Остаётся `try_from` как **имя-конвенция** фаллибельного конструктора-конверсии
> (только при инфаллибельном сиблинге `from`, R3 D325); `try_into` уходит вовсе.
> Ниже — исторический текст.
...
> **Формат failure (Plan 140.1 — см. [D13](#d13) AMEND).** При нарушении
> печатается location-first диагностика, общая с контрактами
> ([D24](09-tooling.md#d24)): `<file>:<line>: assert failed: <expr>`, а с
> опциональным сообщением (`assert(cond, "msg")`, [D84](10-overloading.md#d84))
> — `<file>:<line>: assert failed: <msg> (<expr>)`. `<file>:<line>:`
> авто-проставляется codegen'ом; `debug_assert` использует тот же текст
> (отличие — debug-only-гейт ниже, не сообщение).
...
- [D24](09-tooling.md#d24) — `requires`/`ensures` контракты;
  ...
  `contracts`. Plan 140.1: assert и контракты делят единый
...
- Heap-record с `#impl(Equal)` → `a == b` вызывает synthesized
  `@equal(other)` (memberwise структурное равенство).
- Heap-record **без** `#impl(Equal)` → identity-eq (unchanged) —
...
- Cyclic type `type A { b B }; type B { a A }` + `#impl(Clone)` →
  `E_AUTO_DERIVE_CYCLE`.
...
- [D72 — Generic bounds](02-types.md#d72) — `Hash` требует `hash() -> u64`.
...
### Амендмент — владение составным ресурсом: `rc`/`split()`/`share()` (Plan 258, 2026-08-09)

> **Найдено аудитом (план [258](../../docs/plans/258-std-net-ownership.md)):** §§1–5 выше
> (типы адресов, `TcpListener`/`TcpStream`/`UdpSocket`/`NetError` листинги, синтаксис
> `namespace X { fn f(self) }`) — снимок ДО byte-surface-миграции (текущий транспорт несёт
> ТОЛЬКО `[]u8`, не `str`), ДО объединения TCP/UDP/DNS в один эффект `Net`
> (`std/src/net/effect.nv`) и ДО механики `into_split()`/`@share()` ниже — ни один из
> перечисленных сигнатур/вариантов `NetError` не совпадает с текущей реализацией
> дословно. Как ОБЩИЙ API-снимок §§1–5 обновлению НЕ подлежат в рамках этого плана
> (объём другого рода, риск непропорционален узкой задаче владения) — авторитетный
> источник точных сигнатур/вариантов ошибок — сам `std/src/net/**` (см. файловую карту в
> шапке каждого `.nv`-файла модуля). Этот амендмент закрывает УЗКУЮ, специфичную для
> владения дыру: §§1–5 не упоминают `into_split()`/`@share()` вовсе (модуль их не имел на
> момент написания D173) — то есть модель владения составным сокетом сегодня не
> задокументирована на уровне спеки нигде.

**Модель.** Один TCP-сокет (`*()`-хендл) имеет ДВЕ раздельные, последовательные схемы
подсчёта владения — не параллельные и не обязанные совпадать в любой момент времени,
а СМЕНЯЮЩИЕ друг друга в момент `into_split()`:

1. **До `into_split()` — счётчик на стороне Nova.** `TcpStream.rc` (`*mut AtomicInt`,
   `std/src/net/tcp.nv`) считает `@share()`-копии ЦЕЛОГО потока (`share()` = `fetch_add(1)`,
   `close()`/`cleanup()` = `fetch_sub(1)`, реальный extern-close — только когда счётчик
   доходит до 0). Каждый `@share()`-алиас — независимое linear-значение со СВОИМ
   consume-обязательством (D188, `03-syntax.md` §«Alias-копия — `@share()`»).
2. **`into_split()` — однократная передача владения от Nova к шиму.** Требует SOLE
   ownership (`rc == 1`, никаких живых `@share()`-копий): проверка ПЕРЕД мутацией —
   `rc.load()`, и `rc.fetch_sub(1)` выполняется, только когда прочитано ровно 1 (Plan 258
   Ф.1, №498) — при живых копиях метод паникует, вовсе не трогая счётчик. Не единая CAS-
   инструкция (`AtomicInt.compare_exchange` через `*mut AtomicInt`-приёмник упирается в
   отдельный, не связанный с владением дефект кодогена — `[E_CODEGEN_TYPE_UNKNOWN]`,
   №TBD), но пары `load`+`fetch_sub` здесь достаточно: `rc` может инкрементить ТОЛЬКО
   владелец текущего `TcpStream`-значения (`@share()` требует обладания им), а оно —
   линейное значение с ровно одним держателем одновременно, так что «прочитан 1» на
   момент проверки означает «других живых копий нет» и уже не может стать ложным к
   моменту `fetch_sub` — создать новую копию `rc` в этот промежуток некому. При успехе
   `rc` переходит в 0 (Nova-сторона исчерпана) и `Net.mark_split(...)` заводит НОВЫЙ
   счётчик на C-стороне (`split_refcount` в `nova_rt/net.c`, `NovaNet2Stream`) со
   значением 2 — по одной единице на `TcpReadHalf`/`TcpWriteHalf`.
3. **После `into_split()` — счётчик на стороне C.** Каждая половина при `close()` вызывает
   `net_tcp_close()`, который декрементирует `split_refcount`; реальный `uv_close` — только
   когда счётчик доходит до 0 (та же «закрывает последний» семантика, что и `rc` в фазе 1,
   но на другом слое и над другим набором совладельцев — половинами, не share-копиями).

**Почему НЕ единый сквозной счётчик.** Nova-side `rc` не может пережить `into_split()` —
после хендофа половины (`TcpReadHalf`/`TcpWriteHalf`) больше не носят поле `rc` вовсе
(`std/src/net/tcp.nv`: `type TcpReadHalf consume value priv { handle *() }`, без `rc`) —
они голые хендл-обёртки, и учёт владения над ними принципиально обязан жить там, где
проходит их park/close-протокол — в C, вместе с libuv-хендлом. Обратный перенос (учёт
split-владения в Nova) потребовал бы либо делить `rc`-ячейку между двумя РАЗНЫМИ Nova-
типами (`TcpReadHalf`/`TcpWriteHalf` сегодня их не имеют — язык-меняющее расширение),
либо дублировать состояние в обоих слоях одновременно, что и есть отвергнутая
«промежуточная» форма (план 258, «Корень: владение учитывается дважды»). Единственная
форма БЕЗ двойного учёта — однократная, направленная (Nova → C) передача владения ровно
в момент `into_split()`, что и реализовано.

**Эфемерный `TcpStream` вокруг half-операций — перф/гигиена, НЕ второй счётчик
владения (аудит p496-net-audit, №496):** обе половины передают операции через ЭФЕМЕРНЫЙ
`TcpStream.from_raw(h)` (свежий, отдельный `rc`-счётчик на каждый вызов, никогда не
читаемый и не мутируемый после конструирования — см. `TcpReadHalf mut @read`/
`TcpWriteHalf consume @close` и остальные `from_raw`-сайты, `std/src/net/tcp.nv`), потому
что `Net`-эффект типизирует свои операции по `TcpStream`, а не по голому хендлу — половины
пользуются только полем `.handle`, счётчик — мёртвый груз (лишняя GC-аллокация на вызов,
задокументированная как перф-заметка в `tcp.nv` над `TcpStream.from_raw`). Это НЕ второй
источник истины о владении (счётчик этих временных значений никогда не участвует ни в
одном решении «закрывать или нет» — решение всегда идёт через `split_refcount` в C) —
однако именно эта конструкция мотивировала план 258 (Ф.2 плана, `docs/plans/258-std-net-ownership.md`)
пересмотреть half-операции на прямые `Net.*_half(...)`-вызовы без промежуточного
`TcpStream`, чтобы устранить саму возможность путаницы. Статус этой правки — план 258 Ф.2:
*решение принято, реализация — после релизного тега* (риск правки инвариантов владения
в сетевом коде непосредственно перед релизом оценён владельцем как выше риска самой
находки, у которой сегодня нет живого носителя кроме уже закрытых №456/№458).
...
> **Amend (Plan 91.18 Ф.2 П2, 2026-06-19):** `pad_left`/`pad_right` переименованы в
> `pad_start`/`pad_end`. Новые имена согласуются с CSS/JS padding convention (start/end
> вместо left/right). Устаревшие имена удалены.
...
> **Amend (Plan 91.18 Ф.2 П5, 2026-06-19):** `@to_str()` переименован в `@into_str()`.
> `as_` конвенция зарезервирована для O(1) zero-copy view; consuming-финализатор использует
> Rust-идиому `into_*`. Устаревшее имя `@to_str()` удалено.
...
// АМЕНДМЕНТ (план 283 Ф.4, 2026-09-07): собственная реализация подъехала. `str @to_f64()`
// больше не зовёт `strtod`/`nova_str_to_f64`/`str_parse_f64` — правильное округление
// десятичной записи в двоичную теперь порт Rust `core::num::dec2flt` (коммит
...
// `strtod`, включая внешний корпус parse-number-fxx-test-data). `strtod` остаётся ТОЛЬКО
// эталоном тестов — из состава `to_f64()` он выведен.
// АМЕНДМЕНТ (план 283 Ф.7, 2026-09-07, решение владельца): дверь `str → f32` ОТКРЫТА —
// `str @to_f32() -> Result[f32, ParseFloatError]`, та же грамматика и тот же словарь
// ошибок. Округление ПРЯМОЕ: десятичные цифры → `f32` за один шаг, ВТОРЫМ НАБОРОМ
// КОНСТАНТ над тем же алгоритмом (одна таблица степеней пяти, один медленный путь).
// Путь «строка → `f64` → `f32`» отвергнут замером, а не вкусом: он округляет дважды, и
// для строки, лежащей выше середины между двумя соседними `f32` меньше чем на разрешение
// `f64`, добавка теряется на первом шаге, значение садится на середину, а округление «к
// чётному» уходит в противоположную сторону — пять из шести таких строк дали другой бит.
// Так же поступают Rust (`impl Lemire for f32` со своей таблицей) и Go (свой `atof32`), а
// в C `strtof` — отдельная функция, а не `(float)strtod(s)`. Компиляторный builtin
// `f32.try_parse` тем же слиянием СНЯТ (`Option` вместо `Result`, лень `strtod` и та самая
// двойная округлённость); негативная фикстура —
// `spec_tests/conformance/neg/f32_try_parse_retracted.nv`.
```
...
> именованного пользовательского типа (record / sum / named-tuple / newtype) и релевантных
> встроенных value-типов (`str`) набор байт-смещений внутри эмитимого C-объекта, по которым лежат
> **GC-управляемые указатели** — per-type pointer-offset «bitmap» (аналог Go `gcdata`). Это
> **heap-сторона** точного GC из [Plan 144 §7](../../docs/plans/144-precise-gc-implementation.md#7)
> («Heap-сторона достижима»): Nova знает layout типов → точная карта смещений указателей.
> Sibling-прецедент [D273](06-concurrency.md#d273) (may-GC, та же emit-nothing-форма, дыра H4/Q15).
> **Ничего не эмитит** в генерируемый C; layout-id в заголовок объекта НЕ пишется — потребляется
> точным mark-sweep'ом в **Ф.5** ([Plan 144.5](../../docs/plans/144.5-nonmoving-precise-gc-online.md)),
> отдельно и под гейтом.

### 1. Что фиксируется

Per-type карта `{ type-name → {pointer_offsets, variants, size, align, unresolved} }`:
- **record / named-tuple / newtype / value-record / `str`**: `pointer_offsets` — байт-смещения
  GC-указательных слотов в declaration-order layout; `variants` пуст.
- **sum-тип**: `pointer_offsets` пуст (тег @0 — скаляр), `variants` держит **отдельный bitmap НА
  КАЖДЫЙ вариант** (keyed по имени + tag-индексу), перечисляя GC-смещения, активные когда выбран
  этот тег.
...
- **`str = {ptr: *u8, len: int}`:** `ptr` (@0) **ЕСТЬ** указательный слот (object-start lookup
  на mark толерантен к interior / не-GC буферу — [§7.6 H1](../../docs/plans/144-precise-gc-implementation.md#76));
  `len` — скаляр. Помечается смещение `ptr`.
- **Вложенные value-records / value-типы:** **РЕКУРСИЯ** — смещения указателей вложенного типа
  встраиваются на смещении поля (offset поля + относительные offset'ы вложенного).
- **`Option[T]` / `Result[T,E]`:** учитывается NPO — когда inner лоуэрится в одиночный указатель
  (`type_ref_to_c` даёт `*`-суффикс), `Option` = один heap-указатель @0; иначе — tagged-inline
  форма (рекурсия в payload, тег-скаляр).
...
пропуск указателя фатален (зеркало дефолта-в-MayGC у [D273](06-concurrency.md#d273)). Тип, чей layout
...
`VariantLayout` / `classify_field`. Introspection-CLI **`nova gc-layout-analyze <path> [--format
text|json]`** (зеркало `gc-effect-analyze` / `consume-analyze`; в бинарь ничего не эмитит).
...
инстанциации, FFI-pointer edge-cases — [Q-gc-layout-precision](../open-questions.md#q-gc-layout-precision).
...
2. **Типы параметров И возврата** `extern "C" fn` должны быть **C-ABI-совместимы**
   ...
   ```
   - **Scalars:** `int`, `uint`, `i8`–`i64`, `u8`–`u64`, `f32`, `f64`, `bool`, `char`.
     `uint` = address-sized unsigned, C-тип `nova_uint` (`= uintptr_t`, size_t-аналог,
     [D130](02-types.md#d130)) — **не** `uint64_t`; отделён от `u64` так же, как `int`
     (address-sized signed, C-тип `nova_int`) отделён от `i64`.
   - **Raw-указатели:** `*T` (любой pointee), `*()`, `CStr` (= `*u8`).
   - **C-ABI fn-указатели:** `*extern "C" fn(C_ABI…) -> C_ABI` — базовый C-ABI случай
     (для передачи Nova-функции как настоящего C-callback: `qsort`-компаратор,
     libuv-хендлер и т.п.). Позволяет fn-указателю быть **параметром/полем** другого
     `extern "C" fn` / value-record. Типы сигнатуры сами рекурсивно C-ABI; ABI-тег,
     коэрция и обоснование — [D353](#d353). Nova-ABI `*fn` (без тега `extern "C"`) —
     **НЕ** C-ABI (Nova-типы в сигнатуре; передаётся Nova-ABI'ем).
   - **value-records и туплы** (анонимные + именованные) — C-ABI **iff ВСЕ поля C-ABI**; передаются
     **by-value** как C-struct. `str` = value-record `{ptr,len}` (D139) → C-ABI. **Циклический**
     value-record (`type Node value { val int, next *Node }`) — C-ABI: поле `*Node` = raw-ptr
     (базовый случай, рекурсия обрывается на указателе). Ключевое слово `value` **обязательно** —
     без него (`type Node { … }`) это **heap** GC-record (`Nova_Node*`, by-reference,
     [02-types.md](02-types.md)), исключённый negative-list ниже как non-C-ABI.
   - **`Option[X]`** — C-ABI **iff `X` — RawPtr** (любой указательный тип: `*T`/`*()`/`CStr`; NPO
     применим к любому указателю: `None`=0, `Some(p)`=p). `Option[non-ptr]` и
   ...
   - Примеры ✅: `(int, CSocketAddr)` · `str` · `Node value {val int, next *Node}` · `Option[*u8]` ·
     `uint`/`f64`/`char` · `*extern "C" fn(*u8, int) -> int` (C-callback-параметр).
     ❌: `Vec[int]` · `Result[int,str]` · `Option[int]` · `()`/zero-field tuple · fn-с-env ·
     Nova-ABI `*fn` (нетегированный) как C-callback.
...
[D216 §10/§20](02-types.md#d216) (C ABI не пробрасывает Nova-исключения): частный случай `Fail`-эффекта
...
## D437 — Диагностика uncaught throw/panic: throw-site + propagation trace (Plan 173 Ф.5 п.7, нормализовано 2026-07-23)

**Статус:** active (документирует поведение, реализованное Plan 173; запрос владельца 2026-07-23 —
дать нормативную опору recover-логу Polaris/222.7).

При неперехваченном throw/панике рантайм печатает в stderr:
1. **Сообщение** (текст паники/ошибки);
2. **Throw-site** — `at <file.nv>:<line>` точки возникновения (`nova_throw_site_mark`);
3. **Propagation trace** — хронологический список `<file.nv>:<line>` точек ПЕРЕДАЧИ ошибки
   (`?`-сайты и re-throw границы разматывания), НЕ полный callstack: функции, не участвовавшие
   в передаче, в списке отсутствуют. Кольцевой буфер `NOVA_THROW_TRACE_CAP = 16` — при
   переполнении старейшие записи вытесняются с пометкой `... (N earlier frames dropped)`.

**Механика:** кодоген штампует `nova_throw_trace_push(file, line)` на каждом `?`-сайте
(happy-path не платит — вызовы лежат на error-ветке); буфер — per-error-state
(`_nova_throw_trace`, effects.h). Печать — `nova_throw_site_dump()` на abort-пути и на
Fail-границе фибра. Парность: Zig error-return-traces (стоимость только на error-path).

**Гарантии для потребителей (Polaris recover-лог):** формат «сообщение + throw-site +
propagation trace» — стабильный минимум диагностики паники; полный Nova-callstack НЕ
гарантируется (решение владельца 2026-07-23: не делаем). Изменение формата/глубины —
амендмент этого блока.

## D462 — Запись отказа: один источник, два рендерера (human по умолчанию, JSON по требованию); амендмент D437 (2026-08-15)

**Статус:** active (решение владельца 2026-08-15: «я бы предложил, чтобы они формировались
в совместимом для json формате, и если никто не разбирает, то печаталось в юзер-френдли
сообщении»). Амендмент [D437](#d437) — тот блок оставляет за собой право менять формат
только амендментом; это он.

### Что

Терминальный отказ (неперехваченный `Fail`, `panic`, typed-`Fail`, составной D158) —
это ОДНА запись со стабильным набором полей:

| поле | смысл |
|---|---|
| `kind` | `fail` / `panic` / `typed` / `cancel` |
| `message` | текст ошибки |
| `payload_type` | имя типа payload'а (только typed) |
| `site` | `{file, line}` точки броска (D437 п.2) |
| `trace` | `?`-цепочка передачи, oldest first (D437 п.3) |
| `trace_dropped` | сколько звеньев вытеснено кольцевым буфером |
| `suppressed` | цепочка подавленных ошибок D158 |

Печатают её ДВА рендерера, выбор — переменной окружения `NOVA_PANIC_FORMAT`:

* **не задана или `text` — ПО УМОЛЧАНИЮ**: прежний человеческий вывод, байт в байт
  с D437 (`panic: <msg>`, `  at <file>:<line> (throw site)`, `  propagation trace …`);
* **`json`**: одна строка JSON с полями выше, ключ-маркер `"nova_failure":1` первым.
...
$ NOVA_PANIC_FORMAT=json ./app
{"nova_failure":1,"kind":"fail","message":"leaf-error","site":{"file":"app.nv","line":29},
 "trace":[{"file":"app.nv","line":19},{"file":"app.nv","line":24}],"trace_dropped":0,"suppressed":[]}
```

### Почему именно так

1. **Данные уже собирались — не было контракта вывода.** Вид отказа, сообщение, точка
   броска, `?`-цепочка и цепочка D158 существовали в рантайме и до этого решения; но
   печатали их ЧЕТЫРЕ разных места своими `fprintf`'ами. Машине текст приходилось
   разбирать догадкой, а инструменту (novac) — клеить свой машинный формат ВНУТРЬ строки
   паники. Теперь печать одна, а формат — выбор потребителя.
2. **Человеческий вывод остаётся умолчанием.** Пользователь не должен читать JSON, не
   попросив; инструмент просит машинный формат явно. Обратной несовместимости нет:
   без переменной окружения вывод прежний.
3. **Переменная окружения, а не флаг компиляции.** Формат выбирает тот, кто ЗАПУСКАЕТ
   (CI, обёртка, IDE), а не тот, кто собирал; пересборка ради машинного лога — абсурд.

### Починка, без которой запись рождалась пустой (амендмент D437 по существу)

D437 обещает throw-site и propagation trace. Замер 2026-08-15 показал, что для самого
частого случая — неперехваченная ошибка из `main` — они НЕ ПЕЧАТАЛИСЬ вовсе, и ровно
это держало красной паник-дорожку (реестр №445).

Механика: тело `main` — файбер со своим fail-frame. Бросок внутри него разматывается
на этот кадр, ошибка репортится в scope, и `nova_supervised_run_impl` перебрасывает её
уже НА ВЫЗЫВАЮЩЕМ C-ПОТОКЕ. Диагностический бакет — per-fiber (Plan 201,
`NovaFiberErrorState`), поэтому переброс читал ЧУЖОЙ, пустой бакет: замерено
`bucket=…3D433D20` на броске против `…3B78C870` на печати. Сообщение переживало переход,
потому что путешествует явным параметром; site и трасса — нет, потому что жили только
в бакете.

Починено: снимок `site`+`trace` кладётся рядом с `first_error` в том же чокпоинте и по
тем же правилам приоритета (`nova_fiber_report_error_kinded`), и восстанавливается в
активный бакет непосредственно перед перебросом (`nova_rethrow_scope_diag`).
Same-fiber переброс (supervised-guard, эмитится кодогеном) снимок НЕ передаёт — его
бакет уже верный; сигнатура `nova_rethrow_scope` ради этого сохранена без изменений.

### Приёмка

* три фикстуры паник-дорожки, красные с 2026-08-12, зелены без правки самих фикстур:
  `neg/f5_uncaught_trace_throw`, `neg/f5_uncaught_trace_panic`,
  `neg/f5_propagation_trace_full` — это и есть приёмка починки, а не фикс носителя;
* страж `scripts/guards/check-panic-report-contract.sh`: человеческий рендер содержит
  `(throw site)` и `propagation trace`, JSON-рендер — обязательные ключи и валидный JSON;
* `nova test --full spec_tests/conformance` — без новых отказов относительно замера
  до починки.

### Границы

Полный Nova-callstack по-прежнему НЕ гарантируется (решение владельца 2026-07-23,
D437). JSON — это ровно поля таблицы выше; добавление поля обратно совместимо,
переименование или удаление — амендмент этого блока.

## D463 — Отказ, доступный из кода: `report()`, `cause()`, `suppressed()`; причина связывается неявно (2026-08-16)

**Статус:** accepted (решения владельца 2026-08-16). Продолжение [D462](#d462):
тот дал запись отказа МАШИНЕ снаружи процесса (`NOVA_PANIC_FORMAT=json`), этот
даёт её КОДУ внутри. План — [173.4](../../docs/plans/173.4-failure-report-contract.md) Ф.3.

**Реализация ждёт №680** (бросок из руки обработчика молча съедается): `cause()`
описывает связь, которая возникает при перебросе, а сам переброс сегодня не
работает. Строить на нём нельзя, пока он не починен.

### Что

Три свободных аксессора той же формы, что уже существующий `suppressed()` —
читают ПОСЛЕДНЮЮ пойманную ошибку:

```nova
report()     -> str          // НОВОЕ: вся запись отказа одной строкой
cause()      -> Option[any]  // НОВОЕ: один шаг цепочки причин
...
with Supervisor = effect Supervisor {
    on_child_fail(idx, err) -> Decision {
        Log.error(report())
        for s in suppressed() { Log.warn(s) }
        if Some(c) = cause() { Log.error("caused by: ", c) }
        // Словарь Decision — Escalate | Stop, ПОЛНЫЙ (D416 §4, ретракт Restart-семейства
        // решением владельца 2026-07-10). Паника приезжает как `str`-сообщение (D416),
        // типизированный бросок — своим типом; вид отказа читается из record().
        return if report().kind == "panic" { Decision.Stop } else { Decision.Escalate }
    }
}
```

**`report()` — тот же рендерер, что печатает терминальный отказ.** Один
переключатель `NOVA_PANIC_FORMAT` управляет и печатью, и тем, что вернёт
`report()`: человеку текст, инструменту JSON. Второго формата не заводится, иначе
они разойдутся — ровно та болезнь, от которой лечит D462.

**Вид отказа нового API не требует — но и нового типа тоже.** ИСПРАВЛЕНО 2026-08-16 по
аудиту спеки (подтверждено тремя скептиками): первая редакция писала «паника приезжает
обработчику типом `Panic`» и `if err is Panic` — это противоречило
[D416](06-concurrency.md#d416) («panic — `str`-сообщение», typed-throw — своим типом,
сужается `err is T`), и типа `Panic` в прелюдии нет. Норма — D416: `err` есть `any`,
паника приходит `str`-ом. ВИД отказа (panic / fail / typed) читается из записи D462:
`report().kind` — то самое поле `kind`, что печатает JSON-рендерер; второго источника
истины о виде отказа не заводится. Пример выше исправлен вместе с этим абзацем
(в нём же стоял `Decision.Restart` — вариант, ретрактированный D416 §4).

### Причина связывается НЕЯВНО — нового слова нет

Обработчик, бросающий ВМЕСТО пойманной ошибки, связывает её причиной сам:

```nova
with Fail[DbErr] = effect Fail[DbErr] {
    fail(e) -> never { throw AppErr { code: 500 } }   // причина = e, проставится сама
}
```

**Почему без явного слова** (решение владельца на вопрос «без слова `from` можно?»).
У Python две цепочки: явная `__cause__` (`raise X from e`) и неявная `__context__`,
которая проставляется сама при броске внутри обработки. Основную работу делает
неявная; `from` нужен в основном чтобы ПЕРЕОПРЕДЕЛИТЬ причину или ПОГАСИТЬ её.

У нас неявная связь **точнее питоновской**: в Python подходит любой `except`-блок,
отсюда жалобы на «прицепилась не та ошибка». В Nova место, где новая ошибка
заменяет старую, — не «любой блок», а ровно две конструкции, которые рантайм уже
отслеживает: рука обработчика (обрабатываемая ошибка это буквально параметр `e`)
и точки конверсии `?` / `!!`.

**Развод с `suppressed` принципиальный, а не эвристический:**

| что случилось | куда едет |
|---|---|
| cleanup (`defer`) бросил РЯДОМ с ещё летящей первичной | `suppressed` (D158, карман) |
| обработчик бросил ВМЕСТО пойманной | `cause` (новая заменяет, унося старую) |

**Чего лишаемся:** указать ДРУГУЮ причину или погасить её. Оба случая редкие, оба
добавляются потом без ломки — неявное остаётся умолчанием. Go выбрал обратное
(`%w` явный), и самая частая ошибка их сообщества — ЗАБЫТЬ `%w`; у неявного
умолчания такого режима отказа нет.

### Отвергнуто, и почему

**Запись `Failure` из семи полей** с типами `Site`/`FailureKind` и свободной
функцией `failure()` — первое предложение интегратора, **снято им же** после
замечания владельца «переусложнено». Разбор чужих решений показал, что оно
противоречит всем: ни Go (`Error() string` + `errors.Unwrap`), ни Rust (`Display`
+ `source()`), ни Java (`toString` + `getCause` + `getSuppressed`), ни Python
(`str(e)` + `__cause__`) не выдают место и трассу ПОЛЯМИ — их отдают ТЕКСТОМ; а
причина везде ОДИН необязательный шаг, а не дерево.

**Неявное связывание `@` в литерале обработчика** (вариант C плана 272) — снято
возражением владельца: у литерала протокола ресивера НЕТ вовсе (значение это бокс
+ vtbl, состояние приходит замыканием), поэтому голое имя там возможно именно
потому, что связывать нечего; прецедентом для метод-формы он не служит.

**`suppressed` — не изобретение.** Java завела `getSuppressed()` ровно потому, что
упавший `close()` в try-with-resources не должен подменять первичную ошибку —
буква в букву наш D158. Go пришёл к тому же через `errors.Join`.

### Границы

`file`/`line` и звенья `?`-цепочки как ОТДЕЛЬНЫЕ значения не выдаются — их нет ни
у Go, ни у Rust, ни у Java. Понадобятся — добавляются полем в JSON без правки
языка. Полный Nova-callstack по-прежнему не гарантируется (D437, решение владельца
2026-07-23).

### Приёмка

* `report()` в обработчике и терминальная печать дают ОДИН И ТОТ ЖЕ текст —
  иначе рендерер размножился;
* `NOVA_PANIC_FORMAT=json` меняет ОБА;
* цепочка из трёх звеньев доходит до `cause()`, и `cause()` даёт РОВНО один шаг;
* cleanup-ошибка попадает в `suppressed()` и НЕ попадает в `cause()`, и наоборот;
* страж `check-panic-report-contract.sh` расширяется проверкой «`report()` ==
  терминальный вывод».

## D468 — Место вызова, доступное коду: `CallerLoc` и `caller_loc()` (2026-09-02)

**Статус:** active (решение владельца 2026-09-02). Семья: [D462](#d462) (запись отказа,
поле `site`) и [D463](#d463) (отказ, доступный из кода). Повод — реестр 221.1 №683.

### Задача

`__FILE__`/`__LINE__` у Nova есть, но только ВНУТРИ кодогена: место вызова
автоматически проставляют `assert`, `debug_assert` и контракты (см. блок про формат
`<file>:<line>: assert failed: …` выше и `nova_contract_violation`). Пользовательскому
коду этот канал недоступен, и следствие измерено на живом носителе: собственная дверь
диагностики — у `novac` это `ice()` — не может назвать виновника. Запись отказа D462
честно проставляет `site`, но это точка броска ВНУТРИ хелпера; поле верное и
бесполезное. То же у любого прикладного `expect`/`require`-хелпера и у `panic()`,
обёрнутого чем угодно.

**Класс:** диагностика указывает на посредника, а не на источник.

### Решение — явная передача, без скрытых каналов

Три части. Ни одна не вводит скрытого параметра, не меняет тип функции и не трогает ABI.

**1. Тип `CallerLoc` в прелюдии — ЗАПИСЬ:**

```nova
type CallerLoc { file str, line int }
```

Запись, то есть ссылочный тип: значение `CallerLoc` физически ЕСТЬ указатель. Именно это
делает передачу дешёвой — одно слово, независимо от числа полей, так что будущая колонка
не удорожает вызов.

*Форму сменил владелец 2026-09-02.* Первая редакция блока предлагала именованный кортеж
[D215](02-types.md#d215) (`type CallerLoc(file str, line int)`) — стековое значение в ТРИ
слова, потому что `nova_str` в эмиссии есть `{ ptr, len }` (`nova_rt/vtables.h:44`) плюс
`line`. Запись вместо кортежа даёт один указатель и, главное, позволяет самому значению
жить в статической памяти (см. «Физическая форма»).

Поле называется `file`, а не `filename`: так оно уже названо во всех наших записях
(структура диагностики `novac`, `at <file.nv>:<line>` рантайма, аргумент
`nova_contract_violation`, ключ `"file"` JSON-схемы), и в нём лежит ПУТЬ, как его получил
CLI, а не короткое имя.

**2. Встроенная `caller_loc()`:**

```nova
fn caller_loc() -> ro CallerLoc
```

`ro` — отдаётся ЧИТАЕМАЯ ссылка: запись живёт в статической памяти, править её нечем и
незачем. Форма возврата настоящая, а не придуманная под этот блок: `-> ro T` встречается
в корпусе 145 раз (греп 2026-09-02).

**Вызова на самом деле нет.** `caller_loc()` не исполняется в рантайме: кодоген эмитит по
одной статической записи на позицию и подставляет константный указатель на неё.
Приём в эмиссии не новый — строковые литералы уже сводятся к общему
`static const uint8_t[]` плюс `static const nova_str` с дедупликацией по хешу
(`emit_c.rs:1744-1764`); здесь то же самое одной ступенью выше. На пути исполнения — ноль
инструкций, в `rodata` — одна запись на различающуюся пару.

**Значение — «место ЭТОГО выражения»**: файл и строка, где `caller_loc()` стоит после
всех подстановок. Одно правило, без режимов и без зависимости от того, где она написана.
**Законна в ЛЮБОЙ позиции выражения — решение владельца 2026-09-03:** написанная просто в
теле, вне умолчания, она возвращает свою собственную строку, то есть служит обычным «где
я сейчас» (аналог `__FILE__`/`__LINE__`). Отдельного правила и отказа в чекере не
требуется — это то же самое правило.

**3. Место вызова приезжает ОБЫЧНЫМ параметром с умолчанием:**

```nova
export fn ice(msg str, loc CallerLoc = caller_loc()) -> never { … }

ice("bad")            // loc = место ЭТОГО вызова
```

Почему одно правило «место этого выражения» даёт здесь место ВЫЗЫВАЮЩЕГО, а не строку
объявления: **умолчание в Nova подставляется НА МЕСТЕ ВЫЗОВА.** Это не проектное
пожелание, а измеренное устройство [D102](03-syntax.md#d102): `callnorm.rs` переписывает
вызов в блок, где умолчание становится `let` со span'ом ВЫЗОВА (`callnorm.rs:693` —
`let sp = e.span`; клонирование и правка умолчания — там же, :818–834). То есть
`caller_loc()` физически оказывается в теле вызывающего, и «место этого выражения» и есть
его строка.

Отсюда важное следствие для читателя спеки: **отдельного правила о цепочке не
существует.** Три правила первой редакции (Р1 «в помеченной функции», Р2 «в
непомеченной», Р3 «цепочка») схлопнулись в одно.

### Правило тела: `requires`, `assert`, `panic` и `throw` берут параметр сами; `ensures` — нет

Решение владельца 2026-09-02, уточнённое им же 2026-09-03 (`throw` добавлен явно,
`ensures` исключён после спора — разбор ниже). **Если у функции есть параметр типа
`CallerLoc`, то `requires`, `assert`, `debug_assert`, `panic` и `throw` в её теле по
умолчанию указывают на него** — без повторного упоминания в каждой строке. Для `throw`
это значит: полученное место пишется в поле `site` записи отказа ([D462](#d462)) — та же
семантика двери, что у `panic`: обёртка сообщает наружу об ошибке в данных вызывающего.
Без параметра `site` — точка броска, как сегодня:

```nova
fn half(n int, loc CallerLoc = caller_loc()) -> int
    requires n > 0, "half wants a positive"      // loc подставится сам
{
    n / 2
}
```

Почему автоматически, а не руками: единственная слабость чисто явной формы — «автор
забыл дописать `loc`» ВНУТРИ своей же функции, а это самый частый случай. Правило её
снимает, и магии при этом не прибавляется: параметр ВИДЕН В ПОДПИСИ, неявность живёт
только там, где явность была бы шумом.

**`ensures` — сознательное исключение (решение владельца 2026-09-03).** Владелец предлагал
включить и его ради единообразия, пригласил контрдовод и принял его. Три основания,
каждое самостоятельное:

- **доноры единодушны.** Eiffel — родина контрактов и самого понятия «назначение вины»:
  предусловие — вина клиента, постусловие — вина поставщика. D — документация дословно:
  «failed `in` contract = баг вызывающего, failed `out` contract = баг функции». Ada 2012
  (`Pre`/`Post`) и C# Code Contracts — так же. Назначение вины — суть контрактов, а не
  украшение к проверке;
- **вызывающий не может нарушить `ensures` даже теоретически:** он управляет только
  аргументами. Если при аргументах, прошедших `requires`, постусловие ложно — сломано
  тело либо слишком слаб сам `requires`, и оба — баги автора функции. Указатель на
  вызывающего был бы ложным следом в ста процентах срабатываний — болезнь №683
  («диагностика указывает не туда») внутри её же лекарства;
- **статическая сторона уже назначила вину так же:** верификация контрактов (Z3-ярус)
  по построению доказывает `ensures` из ТЕЛА функции, а `requires` проверяет на каждом
  вызове. Рантайм-атрибуция, спорящая с математической, путала бы читателя.

Унификация при этом не теряется, а формулируется одной фразой вместо таблицы: **всё, что
говорит о ВХОДЕ или наружу — `requires`, `assert`, `panic`, `throw`, — указывает на
вызывающего; `ensures` говорит о собственном ВЫХОДЕ — указывает на себя.** По той же
причине у `ensures` нет и явной формы с `CallerLoc` в таблице ниже: место вызова в
постусловии не означает ничего.

**Два параметра типа `CallerLoc` — отказ по имени, `[E_AMBIGUOUS_CALLER_LOC]`.** Молчаливый
выбор первого — ровно тот класс, который реестр 221.1 ловит у перегрузок (№857): оракул
выбирал одну из двух применимых кандидатур вместо отказа.

**ЯВНЫЕ ФОРМЫ ПЕРЕОПРЕДЕЛЕНИЯ ОТОЗВАНЫ ВЛАДЕЛЬЦЕМ 2026-09-03 («это можно не делать»).**
Блок предлагал их таблицей — `requires bool[, str][, CallerLoc]`, `assert` в четырёх формах,
`panic(str[, CallerLoc])`, — чтобы можно было указать место, полученное не своим параметром.
Формы сняты; в языке от них не остаётся ничего, и грамматика клаузы не трогается вовсе.

**Что именно потеряно, названо честно:** единственный случай — место, лежащее НЕ в
параметре: поле записи (`node.loc`), элемент коллекции, вычисленное значение. Такой код
теперь называет либо свой параметр, либо собственную строку. Обходной путь однострочный и
остаётся в языке: принять место параметром с нужным умолчанием —
`fn check(node Node, loc CallerLoc = node.loc)`.

**Что сохраняется целиком:** правило тела (`requires`, `assert`, `debug_assert`, `panic`,
`throw` берут параметр `CallerLoc` сами), проброс по цепочке через ОБЫЧНЫЙ АРГУМЕНТ
(`ice(msg, loc: loc)` — это аргумент функции, а не форма клаузы, и он не отозван), исключение
`ensures`, и отказ на двух параметрах.

**Грамматика контракта остаётся как была:** второй слот — только строковый ЛИТЕРАЛ
(`parse_opt_contract_message`), иначе `[E_CONTRACT_MESSAGE_NOT_STRING]`.

*Замер, оправдавший немедленный откат начатой реализации.* К моменту отзыва слот в AST,
трёхэлементная грамматика и проверка типа в эмиссии были уже написаны, и промежуточное
состояние успело показать, чем опасно останавливаться на полпути: `requires n > 0, m` со
строковой ПЕРЕМЕННОЙ парсился и молча игнорировался — форма, принятая компилятором и не
исполненная, то есть класс реестра №885. Откачено в тот же час, целиком.

### Цепочка — явная

```nova
fn ensure(ok bool, loc CallerLoc = caller_loc()) -> () {
    if !ok { ice("assertion failed", loc: loc) }  // ПЕРЕДАЁТСЯ РУКАМИ, ПО ИМЕНИ
}
```

Не передать — законно и означает другое: `ice` получит своё умолчание, то есть строку
ВНУТРИ `ensure`. Обе записи читаются буквально, и разница видна в тексте.

**Решение владельца 2026-09-02:** «оставить цепочку явной — ДА, неявный проброс — слишком
много проблем».

### Почему неявный проброс отвергнут — разбор оставлен намеренно

Первая редакция этого блока вводила атрибут `#track_caller` со скрытым параметром:
пометил функцию — и место вызова протекает через неё насквозь, включая цепочку обёрток.
Форма отвергнута владельцем после разбора, и разбор сохранён, потому что предложение
вернётся.

Цепочка причин, каждая из которых следует из предыдущей:

1. **Скрытый параметр — всё равно часть подписи.** Довод владельца, снявший последнее
   возражение интегратора: «если вызывающая сторона не знает о параметре
   `const CallerLoc*`, то она не знает правильную сигнатуру функции, и вызов вообще
   некорректен». Значит «скрытый» он только для читателя.
2. **Следовательно, свойство переезжает в ТИП функции** — как уже устроена строка
   эффектов. Тогда `fn(int) -> int` и помеченный вариант суть РАЗНЫЕ типы.
3. **Следовательно, всякая высшая функция обязана выбрать сторону** — в `std` их **62**, и
   каждая либо теряет канал, либо раздваивается. **Правило счёта, без которого число нельзя
   проверить:** объявление `fn <имя>(…)` в `std/src`, в списке параметров которого — по
   балансу скобок, многострочные подписи включены, комментарии вырезаны — встречается тип
   `fn(`. Замер 2026-09-02: 62 объявления в 12 файлах; крупнейшие носители —
   `collections/vec/sort.nv` (15), `collections/vec_lazy/core.nv` (10), `runtime/sync.nv` (10).
   Первая редакция называла 95 со ссылкой на реестр №683; **это число не воспроизводится** —
   правила счёта при нём не было, грубый греп даёт 87, точный — 62. Довод от этого не
   слабеет (любое из трёх означает «десятки функций обязаны выбрать сторону»), но число без
   правила счёта в спеке не остаётся.
4. **И существующий код ломается:** функция с контрактом, переданная как значение,
   сегодня компилируется и срабатывает (проба 2026-09-02) — при строгом типе перестала бы.
5. **Протокол и vtable** добавляли к этому третий слой: контракт на методе, попавшем в
   таблицу виртуальных вызовов, — обычное дело, и «запретить по имени» его нельзя.

Владелец: «мне это не нравится, я думаю поэтому в др языках и не стали пробрасывать
параметр неявно». Так и есть: C# (`CallerLineNumber`) и Swift (`#line`) передают место
ЯВНЫМ параметром с умолчанием — ровно принятая здесь форма; Rust держит `#[track_caller]`
и платит за это шимом там, где функция берётся значением.

**Что при этом потеряно, честно:** проброс через обёртку держится на дисциплине автора —
компилятор не напомнит. Это та же цена, которую платят C# и Swift, и владелец принял её
осознанно, взамен получив отсутствие скрытых каналов во всём языке.

### Что НЕ меняется — против первой редакции блока

Перечислено явно, потому что первая редакция это меняла, и следующий читатель обязан
видеть, что решения отозваны, а не забыты:

- **Контракт БЕЗ параметра `CallerLoc` ведёт себя как сегодня** — называет строку своего
  объявления. Универсальной смены поведения контрактов НЕТ; четыре фикстуры, пинующие
  текст контрактной паники, не трогаются.
- **[D437](#d437) — без изменений, амендмент первой редакции СНЯТ.** У `?` нет
  аргументного слота, куда передать место, а его собственная строка и есть честный ответ
  на вопрос трассы «через какие точки прошла ошибка».
- **[D462](#d462) — состав полей записи отказа не меняется.** Значение `site` уточнено
  2026-09-03 в одном случае, см. «Правило тела»: `throw` из функции с параметром
  `CallerLoc` пишет туда полученное место; без параметра `site` — точка броска, как
  сегодня.
- **`extern` — вопроса больше нет:** у внешней функции нет умолчаний Nova, и скрытого
  параметра не существует. Диагностика `[E_TRACK_CALLER_ON_EXTERN]` не заводится.
- **Функция-значение — вопроса больше нет:** тип функции не меняется, брать её значением
  можно как прежде. Диагностика `[E_TRACK_CALLER_AS_VALUE]` не заводится.

### Сообщение несёт оба места

Отказ, которому передали `loc`, печатает место вызывающего первым, а физическую точку
проверки — хвостом:

```
<файл вызывающего>:<строка>: assert failed: <текст> (<условие>) [in <функция> at <файл>:<строка>]
```

Первым идёт то, что нужно пользователю; хвост сохраняет то, что иначе потерялось бы.
Обе константы известны на компиляции, правило «location-first» не нарушено: кликабельная
позиция по-прежнему первая.

### Физическая форма

**Одна статическая запись `CallerLoc` на различающуюся пару «файл, строка»**, указатель на
неё — константа. Дедупликация — тем же приёмом, что у строковых литералов
(`emit_c.rs:1744-1764`).

**Безопасность по GC измерена, а не предположена** (2026-09-02). Живых бэкендов
размещения два: `alloc.c` (malloc) и `alloc_boehm.c` (Boehm) — оба спокойно терпят
указатель на память вне кучи, консервативный сборщик просто не считает такой адрес своим.
Boehm вдобавок запускается с `GC_set_no_dls(1)` (`emit_c.rs:10198`), то есть статические
данные не сканируются вовсе, — и сканировать нечего: запись указывает только на такие же
статические байты литерала имени файла. Третий файл, `alloc_rc.c` со счётчиком ссылок в
заголовке перед телом объекта, в сборку НЕ подключён (греп по `src`, `nova-cli`,
`scripts`, `.github` — ноль упоминаний); если его когда-нибудь подключат, статической
записи понадобится бессмертный счётчик, иначе `nova_release` попробует освободить
`rodata`. Записано здесь, чтобы это не выяснилось на месте.

**Цена:** **24 байта полей** в `rodata` на различающуюся позицию, минус дедупликация. Это
арифметика, а не оценка: `nova_str` есть `{ ptr, len }` (8 + 8) плюс `line` (8). Заголовок
объекта, если запись его несёт, сюда НЕ входит и меряется на Э1. На пути исполнения — ноль,
потому что вызова нет. Функция, не объявившая параметр, не платит
ничего и эмитируется байт в байт как прежде.

### Границы

Блок НЕ вводит: колонку (только файл и строка — колонка добавляется полем в `CallerLoc`,
когда понадобится, и по устройству записи это не удорожает вызов), стек вызовов целиком
(это другая задача — трасса, а не место), скрытые каналы любого рода и какое-либо
изменение типа функции, ABI или формата записи отказа D462.
---

## D473 — Косвенный возврат составного значения в буфер вызывающего (destination-passing); C-суффикс `__sret`

**Source:** [Plan 172.14](../../docs/plans/172.14-value-abi-perf-placement.md) (соглашение),
[Plan 172.15](../../docs/plans/172.15-value-placement-abi-audit.md) (снятие хардкода, Ф.1
закрыта 2026-08-08). **Status:** ✅ ACTIVE 2026-09-10.
**Связь:** [D280](#d280-new--ссылка-на-heap-тип-by-value--pointer-size-8-байт-не-inline-object-size)
(C-кодировка возврата — `Nova_X*`; D473 говорит, ЧЬЯ память за этим указателем, и правило D280
не меняет), [D228](02-types.md) / [D232](02-types.md) (категории значения),
[D447](02-types.md) (`#no_copy`), [D6 §Что сохранилось](05-memory.md) (анализ убегания —
тот же обход).

**Почему блок появился позже механизма.** Механизм работал в эмиттере с плана 172.14 и
**не имел ни грамматической формы, ни нормативного описания** — состояние, названное в
[novac-architecture.md](../../docs/dev/novac-architecture.md) как строка №469 семьи
«спека↔реализация разошлись» (№487). Этот блок закрывает расхождение: он не вводит нового
поведения, а делает существующее нормативным и запрещает то, чем оно уже переболело.

### 0. Терминология — что здесь как называется

Различаются два слоя, и путать их — источник половины недоразумений:

* **Оптимизация** (уровень языка): значение строится СРАЗУ в памяти вызывающего, а не
  создаётся у себя и потом переносится. Общепринятое имя — **destination-passing style**
  (DPS), «передача назначения». Ближайшая родня в C++ — **RVO/NRVO** и copy elision, но
  там устраняется КОПИЯ уже построенного объекта, а здесь меняется соглашение о вызове.
* **Механизм** (уровень ABI): **косвенный возврат через скрытый параметр-назначение**.
  В System V AMD64 это класс MEMORY со скрытым указателем первым аргументом; в LLVM —
  атрибут `sret` (*structure return*); в Swift SIL — indirect result `@out`; в Rust MIR —
  return place.

**В Nova нормативно называется так:** оптимизация — **косвенный возврат в буфер
вызывающего**; **`__sret` — имя суффикса C-СИМВОЛА**, и только оно (§1). Употреблять
`__sret` как имя оптимизации или как понятие уровня языка нельзя.

**Грамматической формы у механизма НЕТ, и это решение, а не недоделка.** В исходном тексте
Nova его не существует: ни синтаксиса, ни атрибута, ни способа попросить или запретить —
класс оптимизации целиком выводится компилятором (§2). Введение атрибута было бы отдельным
решением и требовало бы носителя, которого нет.

### 1. Форма в сгенерированном C

Каждая годная функция получает **два символа**:

```c
/* вариант с назначением: пишет поля в _out, ни одной nova_alloc */
S* f__sret(args..., S* _out);

/* классический: обёртка, выделяет буфер и зовёт вариант выше */
S* f(args...) { return f__sret(args..., (S*)nova_alloc(sizeof(S))); }
```

**Классический символ остаётся всегда** — он адрес функции для тех сайтов, у которых
назначения нет (§3), и точка совместимости для FFI. Имя параметра назначения — `_out`.
Тип возврата у обоих символов один и тот же `Nova_X*` — по D280; D473 не меняет кодировку,
а только источник памяти.

### 2. Годность ФУНКЦИИ — две фазы, и ни одна не смотрит на имя типа

Реализация: `CEmitter::sret_fn_eligible` (`compiler-codegen/src/codegen/emit_c.rs`).

**Фаза I — обязанность, по типу.** Единственное место, где решает тип. Два вопроса, оба
про корректность: (1) возвращается ли значение категории «GC-запись», рождающейся ровно
одной `nova_alloc` — читается соглашение эмиттера (`heap_record_struct_of_c`), а не имя
конкретного типа; (2) обязан ли адрес значения быть стабильным. **Носителей второго
правила у Nova сегодня нет** (разобраны непрозрачные ручки, атомики под `#no_copy` D447,
`#share`-записи, C-структуры `nova_rt/**` — ни один не проходит через размещение значений),
поэтому проверки нет — и это отсутствие намеренное, а не забытое. Когда носитель появится,
проверка встаёт ЗДЕСЬ и только здесь.

**Фаза II — выгода, по свойству возвращаемого значения.** Требуется всё разом:
(1) результат ридонли — не `mut`, не сам получатель; (2) у значения РОВНО ОДИН
производитель в хвосте, и до него ни один стейтмент значения не строит и из функции не
выходит (ранний `return`/`throw` запрещён: он записал бы буфер вызывающего второй раз либо
оставил недописанным); (3) эмиттер умеет перенаправить эту единственную запись в
назначение — способов ровно два: литерал записи (`emit_record_lit` пишет поля в `_out`) и
вызов, сам обладающий этим свойством (назначение пробрасывается в его `__sret`-вариант,
фикспойнт по графу вызовов). Свойство (2) снимается **ТЕМ ЖЕ обходом, что анализ убегания**
(`escape_analyze::sret_sole_tail_producer`, план 127); третьего разбора не заводится, и это
нормативно: см. обратную ссылку в [05-memory.md](05-memory.md).

**Нормативно и проверяемо: ЗАПИСЬ формы на результат не влияет.** Обёртка `unsafe { … }`
прозрачна; выбор между `[]T` и `Vec[T]` безразличен; хардкода имён типов в периметре
косвенного возврата быть не должно **в принципе** (мерило Ф.1 плана 172.15: было
4 хардкода → стало 0). Правило существует потому, что каждый прежний хардкод давал ровно
один класс отказа: функция, отличающаяся от годной только записью, молча теряла
оптимизацию.

### 3. Перепись САЙТА — отдельное условие: у записи должно быть НАЗНАЧЕНИЕ

Годность функции даёт второй символ; она **не** обязывает каждый вызов его звать. Вызов
переписывается в `f__sret(args, &dest)` только там, где у вызывающего есть именованное
место под значение: `ro`-биндинг, который не убегает, с RHS — прямым вызовом годной
функции (`sret_out_dest`, взводится по `expr.id`; точка переписи
`sret_maybe_rewrite_call` сверяет совпадение C-типов, где оба уже конкретны):

```c
S _slot_N = {0};
S* x = f__sret(args, &_slot_N);
```

**Позиция аргумента назначения не имеет** — там нет ни имени, ни слота, — и вызов
законно берёт классический символ. Живой носитель на 2026-09-10, `nova-duckdb/src/duckdb.c`:
`Nova_str_method_bytes__sret` **объявлен** (строка 5653, то есть функция годна по §2), а
сайты вида

```c
(void)(Nova_FmtCtx_method_write(f, Nova_str_method_bytes(_nova_strlit_…)));
```

зовут аллоцирующую обёртку, потому что результат нигде не назван; счёт в том же файле —
19 упоминаний варианта с назначением против 82 обёрток. Это **не** дефект §2 и **не**
нарушение этого блока: аллокация здесь — следствие того, что материализация временного
слота под аргумент в периметр §3 сегодня не входит.

**Расширение периметра сайтов** (временный слот на время вызова для аргумент-позиции)
законно и желательно, но требует того же, что и §2: свойства, а не разбора формы, —
значение не должно переживать вызов. Полное решение принадлежит типизированному IR
([172.12](../../docs/plans/172.12-typed-ir-mono.md)); до него всякое сужение обязано
доказываться выводом анализа убегания, а не сопоставлением AST-узлов.

### 4. Наблюдаемое поведение — тождественно

Для любого входа `A`, где вызов заменён на вариант с назначением, поведение `T(A)`
наблюдаемо тождественно `A`. Разница только в том, ЧЬЯ память принимает значение: буфер
вызывающего вместо свежей `nova_alloc`. Отсюда и предел: механизм применим ровно там, где
значение не убегает за пределы кадра вызывающего, — это и проверяет фаза II.

### 5. Что блок ЗАПРЕЩАЕТ

* **Хардкод по имени КОНКРЕТНОГО типа** в любом месте периметра — ни в фазе I, ни в
  фазе II, ни в точке переписи. Решение принимается категорией и свойством.
  **Уточнение 2026-09-10 при приёмке блока, чтобы запрет не отрицал §2:** сама
  категория читается ПРЕФИКСОМ C-имени (`heap_record_struct_of_c` — это
  `strip_prefix("Nova_")` над одноуровневым указателем), то есть формально тоже по
  имени. Разница не в строгости, а в предмете: префикс — это СОГЛАШЕНИЕ ЭМИТТЕРА об
  именовании категории, одно на все её типы, и меняется он вместе с соглашением;
  `starts_with("Nova_Vec____")` называл ОДИН тип и потому терял соседние. Запрет
  метит второе. Когда у эмиттера появится реестр категорий, читать надо будет его —
  и это признанный долг, а не скрытая оговорка.
* **Зависимость от записи, а не от смысла**: `unsafe`-обёртка, `[]T` против `Vec[T]`,
  именованная `Self{…}` против выведенной `{ … }` — все эти записи обязаны давать
  одинаковый C.
* **Единственный символ.** Годная функция обязана иметь ОБА символа: убрать классический
  нельзя (§1), молча не эмитить вариант с назначением при пройденном предикате — тоже.
* **Порог размера как признак.** Решает не размер, а категория и свойство; где порог
  нужен, он один именованный на все места (Ф.1-бис плана 172.15), а не число в выражении.
* **`__sret` как имя понятия** (§0): в текстах Nova — «косвенный возврат в буфер
  вызывающего»; `__sret` только про C-символ.
* **Третий разбор** для свойства (2): вывод берётся у анализа убегания, а не заводится
  заново сопоставлением форм.
