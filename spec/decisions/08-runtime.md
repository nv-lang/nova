...
## D468 — Место вызова, доступное коду: `#track_caller` + `caller_loc()` (2026-09-02)
...
Две части, и обе обязательны — по отдельности задача не решается.

**1. Атрибут `#track_caller` на функции** — «место вызова протекает через меня
насквозь». Компилятор передаёт помеченной функции точку, из которой её позвали,
скрытым каналом; подпись для пользователя не меняется.

**2. Встроенная `caller_loc() -> CallerLoc`** — читает эту точку. Встроенная в смысле
уже существующей категории: имя признаётся резолвером без объявления, как `size_of` и
`align_of` («t-reflection intrinsics»).

**3. Тип результата — именованный кортеж в прелюдии:**

```nova
type CallerLoc(file str, line int)
```

Именно `file`, а не `filename`: так поле уже называется во всех наших записях
(структура диагностики `novac`, `at <file.nv>:<line>` рантайма, аргумент
`nova_contract_violation`, ключ `"file"` JSON-схемы), и в нём лежит ПУТЬ, как его
получил CLI, а не короткое имя. Кортеж именованный, а не позиционный
(`(str, int)`), потому что у двух значений разного смысла позиционная пара живёт до
третьего поля, а колонка напрашивается уже сейчас; по [D215](02-types.md#d215) такой
тип стековый — на пути паники аллокаций нет.

### Семантика — три правила

**Р1. В помеченной функции** `caller_loc()` возвращает место вызова ЭТОЙ функции —
на кадр выше.

**Р2. В непомеченной функции** `caller_loc()` возвращает место САМОГО ВЫЗОВА
`caller_loc()`, то есть текущие файл и строку. Это честный ответ, а не ошибка: ровно
поведение `__FILE__`/`__LINE__`, и оно полезно само по себе.

**Р3. Цепочка.** Если помеченная `f` вызывает помеченную `g`, то `g` получает место
вызова `f`, а не строку внутри `f`. Иначе обёртка над обёрткой снова показывала бы
среднее звено — тот самый случай, ради которого атрибут и вводится, и он же стоит в
приёмке №683.

```nova
#track_caller
export fn ice(msg str) -> never {
    ro loc = caller_loc()          // Р1: место вызова ice()
    …                              // "file":"resolve.nv","start":412 вместо пустого
}

#track_caller
fn ensure(cond bool, msg str) -> () {
    if !cond { ice(msg) }          // Р3: ice увидит вызывающего ensure, не эту строку
}
```

### Что решено отдельно и почему

**Эффекта нет.** `caller_loc()` не несёт эффект: её значение — константа места вызова,
известная на компиляции. Иначе её нельзя было бы звать в дверях `-> never` вроде
`ice()`, ради которых всё и делается, и любая помеченная функция обзавелась бы
эффектом в строке без всякой пользы для читателя.

**Атрибутная форма, а не значения по умолчанию.** Умолчания в Nova ЕСТЬ
([D102](03-syntax.md#d102), передача только по имени), и swift-подобное
`fn ice(msg str, file str = #file)` выразимо. Отвергнуто не по цене, а по существу:
через обёртку умолчания пришлось бы протаскивать РУКАМИ на каждом звене
(`ice(msg, file: file, line: line)`), то есть правило Р3 держалось бы на дисциплине
автора — ровно то, что проект везде заменяет механизмом.

**Платят только помеченные.** Скрытый параметр появляется у функции с атрибутом;
непомеченные не меняются ни в подписи, ни в вызове.

**`extern` — отказ по имени.** `#track_caller` на `extern "C"` (и на любой внешней
функции) отвергается: скрытому параметру негде жить, ABI чужой. Диагностика
`[E_TRACK_CALLER_ON_EXTERN]`.

**Функция-значение — отказ по имени в этой редакции.** Взять помеченную функцию как
значение (`ro g = ice`) отвергается — `[E_TRACK_CALLER_AS_VALUE]`. Причина: у
косвенного вызова нет статически известного места, и молчаливая потеря атрибута
означала бы, что `loc` внезапно указывает не туда. Форма может быть открыта позже
амендментом (Rust решает это шимом); до тех пор лучше отказ, чем тихое расхождение.

### На что распространяется атрибут — `assert`, контракты, `panic`

Вопрос владельца 2026-09-02: «`#track_caller` только на `caller_loc()`
распространяется? `panic`, `assert`, контракты отдают свои LOC?» Первая редакция блока
об этом молчала — дыра закрыта здесь.

**Замер сегодняшнего поведения** (проба из трёх функций, нарушенный `requires`):

```
panic: ploc3.nv:5: requires failed: half wants a positive (n > 0)
```

Строка 5 — это объявление ВЫЗЫВАЕМОЙ функции. Ни вызывающего (строка 12), ни исходного
виновника (строка 17) в сообщении нет. То есть у контрактов та же болезнь, что у обёрток,
только повёрнутая в другую сторону: указывают не на посредника, а на себя.

**ПРАВИЛО ОДНО НА ВСЕХ — решение владельца 2026-09-02:** в помеченной функции место
вызова получает КАЖДЫЙ механизм, который печатает локацию, — `panic`, `assert`,
`debug_assert`, `requires`/`ensures`, кадр `?` и `caller_loc()`. Атрибут значит «место
вызова протекает через меня» без исключений.

**Что такое «место вызова» — определяется ОДИН раз, иначе таблицы ниже читаются
двусмысленно** (уточнение по вопросу владельца 2026-09-02: «место вызова — это всегда
проброшенное по цепочке?»). У каждого вызова функции, которой нужна локация, есть РОВНО
ОДНО значение, и функция его ПОЛУЧАЕТ; передаёт его вызывающий по такому правилу:

- **вызывающий НЕ помечен** → передаёт буквальную строку своего вызова
  (непосредственное место);
- **вызывающий ПОМЕЧЕН** → передаёт то, что получил сам.

Отсюда: «проброшенное по цепочке» — не отдельный режим вызываемой функции, а СЛЕДСТВИЕ
того, что её вызывающий помечен. Цепочка прозрачна ровно настолько, насколько помечены её
звенья, и останавливается на первом НЕпомеченном кадре. Для одной помеченной функции,
вызванной из обычного кода, оба прочтения совпадают — расходятся они только в цепочке.

**Следствие для контракта БЕЗ атрибута, и оно неочевидно.** Функция с `requires`, сама не
помеченная, получает НЕПОСРЕДСТВЕННОЕ место вызова. Если пометить обёртку `wrap`, а
вызываемую `half` оставить непомеченной, нарушенный `requires` у `half` назовёт строку
ВНУТРИ `wrap`, а не вызывающего `wrap`. Это верно по существу: `wrap` мог сам исказить
аргумент, и обвинять его вызывающего было бы ложью. Чтобы контракт `half` смотрел сквозь
обёртку, помечают `half` — тогда `wrap`, будучи помеченной, передаст ей своё полученное
место.

| механизм в теле помеченной функции | место, на которое указывает |
|---|---|
| `requires` / `ensures` | **место вызова** |
| `assert` / `debug_assert` | **место вызова** |
| `panic()` и пользовательская дверь (`ice()`) | **место вызова** |
| `?` — сайт передачи ошибки (`propagation trace`, [D437](#d437)) | **место вызова** |
| `caller_loc()` | **место вызова** (правила Р1–Р3 выше) |

*Разбор, предшествовавший решению, оставлен намеренно.* Интегратор предлагал расщепить
правило по владельцу вины: контракт — на вызывающего (нарушено обязательство ПЕРЕД ним),
а `assert` в теле — на себя (внутренний инвариант функции). Довод против единого правила
был конкретен: `assert`, поймавший баг САМОЙ обёртки, назовёт чужую строку, и чинить
пойдут не туда. Владелец выбрал единое правило — за предсказуемость: помеченная функция
ведёт себя ОДИНАКОВО во всех своих отказах, и автору не нужно помнить таблицу.

**Названная цена не остаётся неоплаченной — сообщение несёт ОБА места.** Формат отказа из
помеченной функции:

```
<файл вызывающего>:<строка>: assert failed: <текст> (<условие>) [in <своя-функция> at <свой-файл>:<своя-строка>]
```

Первым — то, что нужно пользователю (его строка); в хвосте — где физически сработала
проверка. Это снимает единственный довод против единого правила, ничего не стоит на пути
исполнения (обе константы известны на компиляции) и не ломает правило «location-first»:
кликабельная позиция по-прежнему первая.

**Что это значит для записи отказа D462:** поле `site` у отказа из помеченной функции
несёт место вызова; физическая точка проверки — отдельным полем записи (JSON) и хвостом
человекочитаемой строки. Формат не меняется по составу полей — уточняется значение `site`
и добавляется хвост, ради которого поле физической точки и существует.

**Контракт называет вызывающего ВСЕГДА — решение владельца 2026-09-02, второе.** Вопрос
стоял отдельно («не должен ли `requires` указывать на вызывающего и без атрибута»), и
умолчание было осторожным — «только под атрибутом». Владелец решил иначе, и это верно по
существу: сегодняшний ответ — строка ОБЪЯВЛЕНИЯ вызываемой функции — не помогает никому.
Нарушенное предусловие есть вина вызывающего, значит его место и есть ответ, независимо
от того, помечена функция или нет.

Итоговая раскладка после обоих решений:

Ниже «место вызова» читается по определению выше: полученное значение, а не обязательно
непосредственная строка.

| механизм | функция БЕЗ `#track_caller` | функция ПОД `#track_caller` |
|---|---|---|
| `requires` / `ensures` | **место вызова** (новое) — получает его как всякая функция, которой нужна локация | то же значение, но теперь оно ещё и ПЕРЕДАЁТСЯ дальше тем, кого зовёт она |
| `assert` / `debug_assert` | своя строка (как сегодня) | **место вызова** |
| `panic()` | своя строка (как сегодня) | **место вызова** |
| `?` (кадр `propagation trace`) | своя строка (как сегодня, D437) | **место вызова** |
| `caller_loc()` | своя строка (Р2) | **место вызова** (Р1/Р3) |

То есть атрибут не вводит контрактам новое поведение, а РАСШИРЯЕТ его: без атрибута
контракт называет непосредственного вызывающего, с атрибутом — исходного, сквозь цепочку
обёрток.

**Цена универсальной половины измерена ДО решения** (греп корпуса 2026-09-02): контрактную
панику упоминают **33** фикстуры, её ТЕКСТ пинуют **4**, и **ни одна** не пинует явную
пару `файл:строка` в ожидании. То есть смена трогает четыре фикстуры, а не корпус целиком;
у них меняется номер строки в ожидаемом тексте, и это ровно та работа, которую волна
обязана сделать своими руками, а не объявить регрессом.

**Скрытый параметр появляется и у функций с контрактом** — не только у помеченных
атрибутом. Это следствие решения, и его надо назвать: функция с `requires` платит за
локацию так же, как помеченная. Функция без контрактов и без атрибута не платит ничего.

### Амендмент к [D437](#d437) — кадр `?` в помеченной функции

D437 фиксирует, что кодоген штампует `nova_throw_trace_push(file, line)` на КАЖДОМ
`?`-сайте, и что «изменение формата/глубины — амендмент этого блока». Вот он, и он
узкий: **состав и глубина трассы не меняются** (те же `?`-сайты, тот же кольцевой буфер
на 16 записей, тот же принцип «платит только error-path»), меняется ОДНО — какая пара
`файл:строка` попадает в кадр, когда `?` стоит в теле помеченной функции: место вызова
этой функции вместо строки самого `?`.

**Почему это верно, а не просто единообразно.** Трасса отвечает на вопрос «через какие
точки прошла ошибка», и для ТОНКОЙ обёртки её собственная строка — шум: она всегда одна и
та же и ничего не различает. Место вызова, наоборот, различает вызовы обёртки между собой
— то есть кадр становится информативным ровно там, где сегодня бесполезен.

**Что при этом не теряется:** физическая точка `?` остаётся в хвосте кадра тем же
способом, что и у остальных механизмов (`[in <fn> at <file>:<line>]`), — правило хвоста
одно на все механизмы блока.

**Решение владельца 2026-09-02** («на выходе `?` тоже пробрасывает LOC… я за вызывающего
при `#track_caller`»). Вне помеченной функции `?` ведёт себя как сегодня — D437 без
изменений.

### Как место вызова передаётся физически (решение владельца 2026-09-02)

**Скрытый параметр — `const CallerLoc*`: константный указатель на статическую запись,
`NULL` означает «вызывающий неизвестен».** Не пара `(файл, строка)` и не упакованное
число.

На каждый вызывающий сайт, которому нужно передать место, эмитится одна статическая
константа, и одинаковые пары «файл, строка» дедуплицируются. Приём в эмиссии НЕ новый:
строковые литералы уже сводятся к одному общему `static const uint8_t[]` плюс
`static const nova_str` с дедупликацией по хешу (`emit_c.rs:1744-1764`) — здесь то же
самое одной ступенью выше.

**Почему указатель, а не упакованный `int`** (оба стоят один регистр, так что выбор не
про скорость):

- **не нужна таблица «номер файла → имя»** — в дереве её нет, при упаковке пришлось бы
  заводить; в записи имя лежит прямо, а повторы снимает та же дедупликация;
- **нет бюджета битов** — упаковка потребовала бы поделить разряды между номером файла и
  номером строки, то есть завести предел вида «строк не больше миллиона», который в
  ПОРОЖДЁННОМ коде однажды выстрелит;
- **метка «неизвестно» бесплатна и не соглашение**: `NULL` невозможно спутать с настоящим
  местом (при упаковке пришлось бы резервировать ноль и помнить об этом);
- **`caller_loc()` не собирает значение** — тип `CallerLoc` ровно этой формы,
  разыменование и возврат; при упаковке потребовалась бы распаковка плюс поиск имени;
- **отлаживается**: в C-отладчике видна читаемая запись, а не непрозрачное число.

**Цена, названная честно:** по одной константе (порядка 16–24 байт в `rodata`) на
вызывающий сайт, который передаёт место, минус дедупликация; чтение — одна лишняя
загрузка и только на пути отказа. Таблица файлов, которая была бы нужна при упаковке,
съела бы часть той же памяти.

### Границы

Блок НЕ вводит: колонку (только файл и строка — колонка добавляется полем в
`CallerLoc`, когда понадобится), стек вызовов целиком (это другая задача — трасса, а не
место), и не меняет формат записи отказа D462: поле `site` продолжает существовать,
меняется лишь то, НАСКОЛЬКО точно оно указывает, когда дверь помечена.
