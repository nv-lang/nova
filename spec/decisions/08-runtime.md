...
2. **`into_split()` — однократная передача владения от Nova к шиму.** Требует SOLE
   ownership (`rc == 1`, никаких живых `@share()`-копий): атомарный
   `rc.compare_exchange(1, 0)` (Plan 258 Ф.1, №498) — при живых копиях CAS проваливается
   БЕЗ побочного эффекта и метод паникует, не трогая счётчик. При успехе `rc` переходит в
   0 (Nova-сторона исчерпана) и `Net.mark_split(...)` заводит НОВЫЙ счётчик на C-стороне
   (`split_refcount` в `nova_rt/net.c`, `NovaNet2Stream`) со значением 2 — по одной единице
   на `TcpReadHalf`/`TcpWriteHalf`.
