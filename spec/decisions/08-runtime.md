...
3. **Следовательно, всякая высшая функция обязана выбрать сторону** — в `std` их 95
   (замер в №683), и каждая либо теряет канал, либо раздваивается.
...
**Цена:** порядка 16–24 байт в `rodata` на различающуюся позицию, минус дедупликация; на
пути исполнения — ноль, потому что вызова нет. Функция, не объявившая параметр, не платит
