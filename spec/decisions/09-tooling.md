...
Тест переносится в slow-lane по **характеру**, а не по абсолютным миллисекундам:
- тест intentionally медленный — sleep, stress-loop, bench (как `#[ignore]` в Rust, `-short` в Go), ИЛИ
- Имя содержит `stress`, `bench`, `perf`, ИЛИ
- Тест таймаутит при 60 с лимите.

Функциональный тест, медленный только из-за compile time, остаётся в дефолтном прогоне.
...
### Folder-module test layout (Plan 169 Ф.8, 2026-06-17)
