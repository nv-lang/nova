...
## D130. Opaque / reveal / fuel — controlled SMT unfolding (Plan 33.9)

### Что

Три композирующихся construct'а для контроля unfolding'а recursive
`#pure` функций в SMT-scope верификатора. Решают **matching-loop** /
**proof-context explosion** для contract-heavy modules.

```nova
// 1. #opaque attr — body НЕ раскрывается в SMT по умолчанию.
//    Каллеры видят fn как uninterpreted function (UF).
#opaque
...
{
    // 2. reveal name — statement, раскрывает body в текущем fn scope.
    //    Эффект: в этом fn после reveal Z3 видит body, в других fn — нет.
    reveal list_len

    // ... proof body ...
}

// 3. #fuel(n) attr — recursive opaque fn раскрывается до глубины n
//    (default = 0, без unfold). Композируется с #proof_budget (D113).
#opaque
#fuel(3)
#pure fn ackermann(m: int, n: int) -> int =>
    if m == 0 { n + 1 }
...
#### `#opaque` (attribute на `#pure fn`)

- Body fn'ы **НЕ инлайнится** в SMT formula каллера.
- Каллер видит fn как **uninterpreted function** (UF) с тем же arity и
  return sort'ом, что и signature.
- Body **раскрывается** только если:
  - В fn scope каллера есть `reveal <name>` statement, ИЛИ
  - У opaque fn'ы есть `#fuel(n)` с n > 0 (auto-unfold n уровней).
- **Требования:** `#opaque` совместим только с `#pure`. Conflict с
  `#verify` (verify fn должна быть полностью видна для своего же body
  verification) → compile error `E2410`.

#### `reveal <name>` (statement)

- Контекстуальный keyword: `reveal` парсится как statement-start **только**
  внутри fn body. В expression position — обычный identifier.
- Эффект: между `reveal X` и концом enclosing fn body — body X
  доступен Z3 как axiom.
- Локальность: reveal не propagate'ится на other fn'ы того же module.
- Допустимо несколько reveal в одном scope, но duplicate `reveal X;
  reveal X;` → W2402 (lint Ф.6.3).
- `reveal X` где `X` не помечен `#opaque` → W2403 (lint Ф.6.1).
- `reveal X` где `X` не определён → compile error E2411.

#### `#fuel(n)` (attribute на opaque `#pure fn`)

- `n` — depth до которой opaque recursive fn раскрывается без явного reveal.
- Domain: `0 <= n <= 100` (validated at parse-time).
- `n = 0` (default) — fully opaque, нужен reveal для unfold.
- `n > 0` — Z3 emits chain из n axioms раскрывающих body на n уровнях.
- `#fuel` без `#opaque` — W2403 (noise, fuel не имеет эффекта на
  non-opaque fns).
- `#fuel(0)` explicit — W2403 (эквивалентно отсутствию атрибута; lint Ф.6.4).
- Композируется с `#proof_budget(timeout_ms = N)` для fine-grained
  control: opaque + fuel ограничивает proof-context space, budget — time.

### Соundness rationale

1. **UF abstraction совершенно консервативна.** Если `list_len(xs) ==
   list_len(reverse(xs))` доказывается с opaque list_len через external
   axiom — это **более** строгое требование, чем доказательство с inlined
   body (любая модель, удовлетворяющая axiomatic spec, удовлетворяет и
   concrete body). Возможны false-Unknown'ы (нужен reveal), но НЕ
   false-Proven'ы.
2. **Reveal — локальная информация, не global.** Раскрытие в fn `f`
   не аффектит верификацию fn `g`. Каждый fn — независимый SMT scope.
3. **Fuel chain — bounded unfolding.** N раскрытий = N axioms; Z3 не
   может развернуть N+1. Matching loop ограничен.

### Industry positioning

| Tool | Opaque syntax | Reveal syntax | Fuel syntax |
|---|---|---|---|
| **Dafny** | `{:opaque}` (magic comment) | `reveal X();` | `{:fuel X, n}` |
| **Verus** | `#[verifier::opaque]` | `reveal!(fn_name)` (macro) | `#[verifier::opaque_outside_module]` (different concept) |
| **F\*** | `[@opaque_to_smt]` | `unfold X` | implicit via `[@unifier_hint_injector]` |
| **Liquid Haskell** | `{-@ reflect f @-}` (opposite default) | `_` (no opaque by default) | `{-@ unfold f @-}` |
| **Nova (D130)** | `#opaque` (attribute) | `reveal X` (statement) | `#fuel(n)` (attribute) |

**Nova-уникальные преимущества:**

1. **Единый attribute namespace** — `#opaque` / `#fuel` follow D105
   attribute conventions, нет magic comments (Dafny `{:opaque}`),
   нет macros (Verus `reveal!()`).
2. **`reveal` как statement** — выразительно для пошагового proof
   workflow'а: можно интерливать reveal'ы между proof-steps. В Dafny
   `reveal X();` — call-like syntax; в Verus — macro invocation.
3. **`#fuel(n)` композируется с `#proof_budget`** — orthogonal control
   axes (proof-context size vs proof-time). Dafny `{:fuel}` влияет
   только на context, time — separate global setting.

### Что не входит (deferred)

- **Cross-module reveal** (`reveal other_module.foo`) — V3 (требует
  module import tracking, обсуждается в Plan 33.16 LSP scope).
...
- **Reveal в lemma body** — V2 (lemma уже proof term, reveal внутри —
  pending semantic decision).
- **Pattern hints** (`reveal foo {:pattern ...}`) — V3 (требует pattern parser).
- **`#opaque` на `#trusted external fn`** — semantic noise, lint warning
  (external fns не имеют body чтобы раскрывать).

### Pipeline lints (реализовано Ф.6)

- **E2411 reveal-undef** — `reveal X` где `X` не определён в module.
- **W2403 reveal-non-opaque** — `reveal X` где `X` не помечена `#opaque`.
- **W2403 dead-opaque** — `#opaque` fn никогда не reveal'ится в module.
- **W2402 dup-reveal** — `reveal X; reveal X;` в одном scope.
- **W2403 fuel(0)** — `#fuel(0)` explicit (эквивалентно отсутствию).

### Phasing (по статусу Plan 33.9)

- **V1 (✅ Bootstrap 2026-05-18):** parser + AST + 5 pipeline lints.
  TrivialBackend silently passthrough'ит reveal/opaque/fuel (treats
  opaque как обычный pure fn).
- **V2 (📋 deferred):** Z3 axiomatic encoding в `verify/backend/z3.rs` —
  emit UF declarations + reveal-triggered axioms + fuel chain expansion.
  Требует focused session с test corpus демонстрирующим matching loops.
- **V3 (✅ закрыто этим D-блоком + idiom):** spec amendment + idiom guide.

### Cross-references

- [D105](#d105-doc-атрибуты) — общий attribute namespace.
- [Plan 33.9](../../docs/plans/33.9-opaque-reveal-fuel.md) — implementation plan.
- [Plan 33.6](../../docs/plans/33.6-contracts-production-hardening.md) —
  production hardening (50+ спринтов foundation).
- [docs/idioms/opaque-reveal.md](../../docs/idioms/opaque-reveal.md) —
  pragmatic guide когда / как использовать.
