...
- `examples/flagship/aggregator --strict-effects` — CC-FAIL
  ...
  TlsError]`) — значит дело НЕ в API-дрейфе. Первая гипотеза (lock-файл
  пинует старый коммит nova-tls) ПРОВЕРЕНА И ОТКЛОНЕНА: `examples/
  nova.lock.toml` действительно помечен git'ом modified в рабочем
  дереве, но `git diff` на него даёт ПУСТОЙ дифф (только CRLF-нормали-
  зация, содержимое байт-в-байт то же) — откачен как no-op. Реальный
  корень НЕ установлен до конца в рамках бюджета этого окна: на ЧИСТОМ
  main та же команда даёт `built:` с пометкой `build cache hit — reusing
  generated C` (переиспользует СТАРЫЙ сгенерированный C, не резолвит
  свежо), а в nova-s1a (свежий worktree, чистый кэш) — реальный свежий
  резолв падает. **Вывод (осторожный): предсуществующий баг/рассинхрон,
  МАСКИРУЕМЫЙ build-кэшем на main, НЕ вызван этим окном** (я не трогал
  net/tls/method-resolution/lock-механику) — точный механизм маскировки
  не расследован до конца (не входит в периметр S1a). Вне периметра —
  не исправлено, задокументировано (доложить владельцу/интегратору для
...
- `examples/flagship/aggregator --strict-effects` — красный по
  ПРЕДСУЩЕСТВУЮЩЕЙ, НЕ-от-этого-окна причине (build-кэш на main маскирует
  реальный резолв `TlsStream.read_bytes`, см. Ф.3 — точный механизм не
  расследован до конца, вне периметра) — задокументировано.
