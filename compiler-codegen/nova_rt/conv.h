 ...
 * стандартно ("nan", "inf").
 *
 * КТО ЭТО ЗОВЁТ ПОСЛЕ ПЛАНА 283 Ф.4 (2026-09-07) — важно, потому что роль
 ...
 *      (codegen/emit_c.rs, таблица `try_parse`) эмитит вызов ИМЕННО СЮДА.
 *      Значит у f32 из строки до сих пор ленивая грамматика strtod
 *      (пробелы, "nan"/"inf", hex, локаль), тогда как у f64 она строгая.
 *      Расхождение намеренно НЕ закрыто планом 283 (его раздел «Не делаем»:
 *      двери `str → f32` в спеке нет); удалять эту функцию нельзя, пока
 *      builtin жив. */
