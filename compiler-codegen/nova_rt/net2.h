#ifndef NOVA_RT_NET2_H
#define NOVA_RT_NET2_H

/* Plan 183 Ф.1: nova_rt/net2.c — reworked async TCP/UDP/DNS stdlib substrate.
 *
 ...
 * Invariant: grep -E "__thread|__declspec\(thread\)" net2.c == 0.
 *
 * Park/wake/cancel reused verbatim from the (correct) net.c mechanism: park slot
 * lives in the supervised scope, not in the OS thread → M:N-safe. Cross-thread
 * uv_close routed via nova_loop_defer_close (Plan 83.10.2).
 *
 ...
 *
 * This file lives ALONGSIDE net.c during the migration (Plan 183 Ф.1-Ф.3): the
 * old net.c still backs all current consumers; net2 is the replacement being
 * grown. All internal helpers here are `static` → no link collision with net.c.
 */

#ifndef NOVA_USE_LIBUV
#  error "Plan 183: NOVA_USE_LIBUV required for std/net (net2)."
#endif
