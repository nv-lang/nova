...
nova_str fs_realpath_data(void);

/* Best-effort parent-dir fsync (write_atomic step 5); no-op on Windows. */
nova_int fs_fsync_dir(const uint8_t* path);

/* scandir: entry count (>= 0) or -errno; then iterate with fs_scandir_next(). */
nova_int fs_scandir(const uint8_t* path);
nova_int fs_scandir_next(void);   /* 1 = an entry is cached, 0 = done */
