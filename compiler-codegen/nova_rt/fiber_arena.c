...
/* Код выхода при диагностированном переполнении стека файбера — тот же 101,
 * что у любой другой ошибки программы (`NOVA_PROGRAM_ERROR_EXIT_CODE` в
 * effects.h, №7436). Задан числом, а не включением effects.h: fiber_arena.c —
 * нижний слой и об эффектах не знает; расхождение двух чисел удерживает
 * страж `check-sigsegv-altstack`. */
#ifndef NOVA_FIBER_OVERFLOW_EXIT_CODE
#  define NOVA_FIBER_OVERFLOW_EXIT_CODE 101
#endif
...
static void _arena_sigsegv_handler(int sig, siginfo_t* info, void* uctx) {
    ...
    /* №745, вторая половина: ДИАГНОСТИРОВАННОЕ переполнение — ОШИБКА
     * ПРОГРАММЫ, а для них в этом рантайме уже есть канон: №7436,
     * «управляемый выход, не авария» (`nova_exit_program_error`, effects.h,
     * код NOVA_PROGRAM_ERROR_EXIT_CODE = 101). Обычная паника выходит именно
     * так; этот обработчик был единственным путём, который всё ещё падал.
     *
     * ЧЕМ СТОИЛО ПАДЕНИЕ, ЗАМЕР: `signal(SIG_DFL); raise(sig)` заставлял
     * ядро писать core на ВСЁ адресное пространство процесса, а арена
     * РЕЗЕРВИРУЕТ slot_size × slot_count × воркеры (по умолчанию
     * 4МБ × 16384 на воркер). Измерено 2026-08-23: дампы на 111–140 ГБ,
     * минуты в `do_exit` (`/proc/<pid>/status` → `State: D (disk sleep)`),
     * фикстура `standalone/fiber_stack_overflow` не укладывалась в 64с таймаута
     * И ЗАБИВАЛА ДИСК. Диагностика уже напечатана выше — core к ней
     * не добавляет ничего, кроме минут и гигабайт.
     *
     * `_exit`, А НЕ `exit`: мы внутри обработчика сигнала и могли прервать
     * поток, держащий лок; atexit-обработчики — единственный оставшийся
     * способ зависнуть ПОСЛЕ того, как диагностика уже на экране.
     * stdout/stderr сброшены явно — строкой выше и здесь.
     *
     * NOVA_KILL_ALTSTACK=1 заодно возвращает СТАРОЕ поведение целиком
     * (без альтернативного стека сюда просто не доходят), так что оба
     * режима по-прежнему проверяются на ОДНОМ бинаре. */
    if (in_our_range) {
        fflush(stdout);
        _exit(NOVA_FIBER_OVERFLOW_EXIT_CODE);
    }

    /* НЕ наш фолт (null deref в пользовательском коде, порча кучи) — отдаём
     * действию по умолчанию, как и раньше: там core действительно нужен,
     * потому что причина НЕ диагностирована. */
    signal(sig, SIG_DFL);
    raise(sig);
}
