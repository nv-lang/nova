...
static void _nova_driver_sleep_close_cb(uv_handle_t* h) {
    ...
    if (actual_co != st->expected_co) {
        ...
        if (expected_co && mco_status(expected_co) == MCO_SUSPENDED) {
            ...
            __atomic_store_n(&st->stage, NOVA_SLEEP_DRV_CLOSED, __ATOMIC_SEQ_CST);
            /* №656, серия 6 — wake С ПРОВЕРКОЙ ТОЖДЕСТВА. История ветки в
             * трёх редакциях, каждая с уроком: (1) безусловный goready по
             * mco_status(expected_co) — откачен за риск чужого co; (2) «нет
             * goready вовсе, парковщик перепроверит» — НЕДОСТАТОЧНО: у этой
             * ветки не было собственного load-плеча, и Деккер не замыкался,
             * когда CLOSED публикуется после чтения предиката парковщиком —
             * зависание c330 (PARKED/WAIT, слот 72) поймано ИМЕННО с
             * редакцией 2 в бинаре. (3, эта: два плеча вместе) парковщик
             * публикует НОСИТЕЛЬ до перепроверки (nova_sched_park_until,
             * №656), а эта ветка после SC-публикации CLOSED ЧИТАЕТ
             * parked_co[slot] и будит ТОЛЬКО при тождестве с
             * st->expected_co: чужой/устаревший указатель проваливает
             * сравнение безвредно, а свой — доказуемо жив (файбер не
             * покидает sleep до CLOSED; второго close_cb на один st не
             * бывает — «expected_co is already dead» из комментария выше
             * для сна недостижимо). Хотя бы одно плечо видит другое. */
            nova_sched_wake_expected(st->scope, st->slot, st->expected_co);
            if (st->home_worker_id < 0) nova_runtime_signal_main();
        }
        return;
    }
