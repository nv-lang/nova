//! Запускает все .nv-файлы из ../nova_tests/ через интерпретатор и
//! проверяет, что все `test "..." { ... }` блоки внутри проходят.
//!
//! nova_tests/ живёт на top-level репозитория, не в compiler-bootstrap/ —
...
fn collect_nova_files() -> Vec<PathBuf> {
    // nova_tests/ — на top-level репо (sibling to compiler-bootstrap/).
...
fn all_spec_test_files_pass() {
    ...
    if files.is_empty() {
        panic!("no .nv files found in nova_tests/");
    }
