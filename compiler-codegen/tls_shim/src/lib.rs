...
mod tests {
    ...
    fn pinned_correct_pin_completes_handshake() {
        unsafe {
            ...
            // cross-check против openssl SPKI-pin (используется в Nova-тестах)
            let expected = [0x92u8,0xf7,0x09,0x93,0x97,0xe8,0xc4,0x80,0x7a,0x31,0x10,0x0b,0xf3,0xb7,0xb7,0x94,0x01,0x17,0x0a,0x13,0x85,0x1c,0x92,0xc9,0xfe,0xd5,0x9d,0xe4,0x92,0xcf,0x2c,0x34];
            assert_eq!(pin, expected, "shim SPKI pin must match openssl");
            let cc = tls_client_cfg_new();
