...
//! в Z3 AST через FFI.
//!
//! Поддерживаемые сейчас типы:
//! - `Int` — Z3 Int sort, математически БЕЗГРАНИЧНЫЙ. Nova `int` — i64 в
//!   рантайме; безграничное рассуждение sound, поскольку переполнение
//!   `int` приводит к `panic` (Plan 33.8 Ф.1, spec 04-effects.md): функция
//!   либо вернёт истинный матем. результат, либо умрёт паникой — ложного
//!   значения не вернёт. Sized-типы (u8/u16/u32/u64/i8/i16/i32) — `BitVec`.
//! - `Bool`.
