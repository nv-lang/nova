        ...
        for name in &self.consume_obligations {
            ...
            match state {
                Some(VarState::Live) => {
                    ...
                    let suggestion = Suggestion {
                        message: format!(
                            "consume `{name}` via method (D133-not-consumed quick fix)",
                            name = name),
                        span: exit_span,
                ...
                Some(VarState::MaybeConsumed(at)) => {
                    ...
                    use crate::diag::Suggestion as Sug;
                    let suggestion = Sug {
                        message: format!(
                            "cover `{name}` on all paths: errdefer + okdefer (D133-not-consumed multi-path)",
                            name = name),
                        span: exit_span,
                        replacement: suggestion_text,
                        applicability: Applicability::MaybeIncorrect,
