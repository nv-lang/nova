...
fn cmd_check(path: &PathBuf, explain_cache: bool) -> Result<()> {
    ...
    nova_codegen::chain_norm::normalize_chains_module(&mut module, &std::collections::HashMap::new());
...
fn cmd_compile(path: &PathBuf, output: Option<&std::path::Path>, annotate_source: bool, lint: bool, contracts: &str) -> Result<()> {
    ...
    // / check / codegen — §2; AFTER serde-inject so synthesized bodies share
    // the uniquify invariant). Uniquifies 2nd+ same-scope bindings; no-op
    // otherwise.
    nova_codegen::alpha_rename::alpha_rename(&mut module);
