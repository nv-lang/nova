...
//!
//! **СТАТУС: НЕ ПОДДЕРЖИВАЕТСЯ (kept for reference only).** Tree-walking
//! интерпретатор сейчас не обслуживается и не тестируется: точка входа
//! `nova run` намеренно отдаёт ошибку «not supported». Nova тестируется
//! и шипится исключительно через C-codegen — `nova build <file>` (собрать
//! исполняемый файл) и `nova test` (скомпилировать и прогнать тесты).
//! Код движка оставлен в дереве для справки; разморозить — когда interp
//! вернут в поддержку.
//!
...
impl Interpreter {
    ...
    pub fn eval_expr(&self, expr: &Expr, env: &Env) -> Result<Flow, Diagnostic> {
        match &expr.kind {
            ...
            // Plan 172.5 (D326): call-site `ref <place>` — transparent to
            // const-eval / interp (value of the place; borrow is a codegen
            // concern). `ref` args never appear in comptime-evaluable exprs.
