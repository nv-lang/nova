...
fn codegen_to_c(
    ...
    let (emit_output, warnings) = {
        ...
        // simple name, variant decl index)) — consulted before the name-based
...
/// прогоном; отдельного вызова у неё не было, и проверить её было нечем.
///
...
mod tests {
    ...
    fn diag_head_window_keeps_the_crash_site_not_the_windows_startup() {
        ...
        let got = diag_head_window(&lines).expect("окно не найдено при живой диагностике");
    ...
    fn merged_cu_detail_puts_the_verdict_before_the_long_text() {
        ...
        let verdict_at = got.find("НЕ определён").expect("в строке нет вердикта атрибуции");
        let raw_at = got.find("yyy").expect("в строке нет raw_detail");
