...
fn build_command(tc: &Toolchain, opts: &BuildOpts) -> Command {
    ...
    // Plan 193 Ф.2 gate-3 (mbedtls-vendored, 2026-07-12): `c_file_uses_tls`
    // is a bare text-scan for `tls_client_cfg_new(`/`tls_server_cfg_new(`
    // call sites — it has no notion of WHICH package emitted them. A
    // standalone package that declares its own `[ffi] c_shims` (e.g.
    // nova-tls, extracted from std/tls per Plan 193 but keeping the same C
    // ABI/symbol names for compat) satisfies that scan too, and — before
    // this guard — this built-in monorepo-internal special-case would
    // UNCONDITIONALLY ALSO compile+link the monorepo's own
    // `nova_rt/tls_c_shim.c` (stub or real, gated separately by
    // `NOVA_USE_MBEDTLS`) alongside the package's own shim, producing
    // `lld-link: duplicate symbol: tls_client_cfg_new` et al. — reproduced
    // once mbedTLS actually became linkable (the vendor build-and-cache
    // mechanism below, or a manually dropped-in prebuilt lib, either would
    // trigger it identically; this is pre-existing, not new).
    // A package with its own declared `[ffi] c_shims` is self-sufficient
    // for its native linkage — defer to it, skip the built-in shortcut.
    // The monorepo's own `std/tls` tests are unaffected: `std/nova.toml`
    // has no `[ffi]` section, so `opts.ffi` is `None` there — byte-identical
    // behaviour, 0 regressions.
    let has_own_ffi_c_shims = opts.ffi.map(|f| !f.c_shims.is_empty()).unwrap_or(false);
    let uses_tls = !has_own_ffi_c_shims && c_file_uses_tls(opts.c_file);
