...
impl CEmitter {
    ...
    fn type_ref_to_c(&self, ty: &TypeRef) -> Result<String, String> {
        match ty {
            ...
            TypeRef::Tuple(elems, _) => {
                // Flatten tuples to struct — not implemented in Phase 1
                let _ = elems;
                Err("tuple types not yet supported in codegen".into())
            }
            TypeRef::Func { .. } => Err("function types not yet supported in codegen".into()),
    ...
    fn emit_expr(&mut self, expr: &Expr) -> Result<String, String> {
        match &expr.kind {
            ...
            ExprKind::Member { obj, name } => {
                ...
                Ok(format!("({}{}{name})", o, accessor, name = name))
            }
            ...
            ExprKind::ArrayLit(_) => {
                Err("array literals not yet supported in codegen".into())
            }

            ExprKind::TupleLit(_) => {
                Err("tuple literals not yet supported in codegen".into())
            }
    ...
    fn emit_call(&mut self, func: &Expr, args: &[Expr]) -> Result<String, String> {
        ...
        let func_c = match &func.kind {
            ...
            ExprKind::Member { obj, name: method } => {
                ...
                // 2. String methods: `s.starts_with(...)` → `nova_str_starts_with(s, ...)`
                let obj_ty = self.infer_expr_c_type(obj);
                if obj_ty == "nova_str" {
                ...
                // 3. Direct handler vtable call: `switcher.flip()` where switcher: NovaVtable_X*
                ...
                // 4. General method call: `obj->method(...)` or `obj.method(...)`
                let accessor = if Self::is_value_type(&obj_ty) { "." } else { "->" };
                let obj_c = self.emit_expr(obj)?;
    ...
    fn emit_if_expr(
        ...
        self.indent += 1;
        self.emit_block_into_unit(then, &tmp)?;
        self.indent -= 1;
        match else_ {
            None => {
                self.line("}");
            ...
            Some(ElseBranch::Block(b)) => {
                self.line("} else {");
                self.indent += 1;
                self.emit_block_into_unit(b, &tmp)?;
                self.indent -= 1;
                self.line("}");
            }
            Some(ElseBranch::If(e)) => {
                ...
                self.indent += 1;
                let v = self.emit_expr(e)?;
                self.line(&format!("{} = {}; (void){};", tmp, tmp, v));
                self.indent -= 1;
                self.line("}");
            }
        }
    ...
    /// Emit block statements into an existing `nova_unit` temp variable.
    /// Does NOT emit `return NOVA_UNIT` — just updates the tmp if there's a trailing expr.
    ...
    fn pattern_cond(&mut self, pat: &Pattern, scr: &str) -> Result<String, String> {
        match pat {
            ...
            Pattern::Variant { path, kind, .. } => {
                ...
                let type_name = if path.len() > 1 {
                    ...
                    self.find_variant(&variant_name)
                        ...
                        .unwrap_or_else(|| {
                            ...
                            let scr_ty = self.var_types.get(scr).cloned()
                                .unwrap_or_default();
                            scr_ty
                                .strip_prefix("Nova_")
                                .unwrap_or("")
                ...
                let tag = format!("NOVA_TAG_{}_{}", type_name, variant_name);
                let base = format!("({}->tag == {})", scr, tag);
                match kind {
                    VariantPatternKind::Unit => Ok(base),
                    VariantPatternKind::Tuple { patterns, .. } => {
                        let mut cond = base;
                        for (i, p) in patterns.iter().enumerate() {
                            let field = format!("{}->payload.{}._{}",
                                scr, variant_name, i);
                    ...
                    VariantPatternKind::Tuple { patterns, .. } => {
                        ...
                        for (i, p) in patterns.iter().enumerate() {
                            ...
                            let field_ty = field_types.get(i)
                                .cloned()
                                .unwrap_or_else(|| "nova_int".into());
                            // Bind the inner pattern with the field expression
