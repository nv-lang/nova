...
/// line we build the full string and assert `graphemes()` yields exactly the
...
pub fn render_grapheme_conformance_nv(ucd_dir: &Path, limit: usize) -> anyhow::Result<String> {
    ...
    out.push_str("// `graphemes()` yields the expected cluster sequence (content-checked).\n");
    ...
    for (ci, chunk) in sel.chunks(CHUNK).enumerate() {
        ...
        for (full, clusters) in chunk {
            out.push_str("    {\n");
            out.push_str(&format!("        mut g = \"{}\".graphemes()\n", full));
            for c in clusters {
                out.push_str(&format!(
...
/// (capped, stride-sampled) line, assert `words()` yields exactly the expected
...
pub fn render_word_conformance_nv(ucd_dir: &Path, limit: usize) -> anyhow::Result<String> {
    ...
    let mut out = String::new();
    out.push_str("// AUTO-GENERATED by `nova-codegen unicode --emit-conformance`. DO NOT EDIT.\n");
    out.push_str("// UAX #29 word conformance from WordBreakTest.txt: assert `words()`\n");
    ...
    for (ci, chunk) in sel.chunks(CHUNK).enumerate() {
        ...
        for (full, segs) in chunk {
            out.push_str("    {\n");
            out.push_str(&format!("        mut w = \"{}\".words()\n", full));
            for c in segs {
                out.push_str(&format!(
...
/// tests). For each (uniform-spread) line, assert `sentences()` yields exactly the
...
pub fn render_sentence_conformance_nv(ucd_dir: &Path, limit: usize) -> anyhow::Result<String> {
    ...
    out.push_str("// `sentences()` yields the expected segment sequence (content-checked).\n");
    out.push_str(&format!(
        "// Coverage: uniform spread = {} of {} test lines.\n",
        sel.len(), total
    ));
    ...
    for (ci, chunk) in sel.chunks(CHUNK).enumerate() {
        ...
        for (full, segs) in chunk {
            out.push_str("    {\n");
            out.push_str(&format!("        mut sv = \"{}\".sentences()\n", full));
            for c in segs {
                out.push_str(&format!(
