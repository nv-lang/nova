...
/// UAX #15 conformance check (NormalizationTest.txt). For each data line
/// `source;NFC;NFD;NFKC;NFKD` we assert all four `normalize_*(source)` outputs.
/// Capped at `limit` lines (Part 0 specific cases + Part 1 per-character first)
/// to keep compile/run time bounded — `log()`ged so the cap is not silent.
pub fn render_conformance_nv(ucd_dir: &Path, limit: usize) -> anyhow::Result<String> {
    ...
    let mut cases: Vec<[String; 5]> = Vec::new();
    ...
    for line in data.lines() {
        ...
        total += 1;
        if cases.len() < limit {
            cases.push([lit(cols[0]), lit(cols[1]), lit(cols[2]), lit(cols[3]), lit(cols[4])]);
        }
    }
    let mut out = String::new();
    ...
    out.push_str(&format!("// {} of {} data lines covered (cap).\n", cases.len(), total));
    ...
    out.push_str(&format!(
        "test \"UAX#15 NormalizationTest.txt conformance ({} cases)\" {{\n",
        cases.len()
    ));
    for c in &cases {
