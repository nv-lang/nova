...
impl ConstValue {
    ...
    pub fn to_literal_expr(&self, span: Span) -> Expr {
        let kind = match self {
            ...
            ConstValue::Variant(name, args) => {
                let func = Box::new(Expr {
                    kind: ExprKind::Ident(name.clone()), span, id: crate::ast::ExprId::UNSET, debug_only: false,
                });
        ...
        Expr { kind, span, id: crate::ast::ExprId::UNSET, debug_only: false }
    }
            ...
            if let ExprKind::Ident(n) = &base.kind {
                if (n == "size_of" || n == "align_of") && args.is_empty() && type_args.len() == 1 {
                    match type_size_or_align(&type_args[0], n == "align_of") {
                        Some(size) => {
                            let span = e.span;
                            *e = Expr { kind: ExprKind::IntLit(size), span, id: crate::ast::ExprId::UNSET, debug_only: false };
                            return;
                        }
                        None => {
