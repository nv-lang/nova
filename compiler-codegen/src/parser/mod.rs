...
impl Parser {
    ...
    fn parse_effect_methods(&mut self, is_protocol: bool) -> Result<Vec<EffectMethod>, Diagnostic> {
        ...
        while !matches!(self.peek().kind, TokenKind::RBrace) {
            ...
            // D459 (план 272, волна Ф-МОСТ): в теле ЭФФЕКТА операция может
            // начинаться словом `fn`, как всякое объявление функции языка.
            //
            // Это ровно пропуск токена: `fn` не несёт семантики, не меняет
            // ни узел, ни слот диспетчеризации, ни кодоген. Объявление
            // внутри эффекта было ЕДИНСТВЕННЫМ местом языка, где функция
            // объявлялась БЕЗ `fn`, и читатель (как и инструмент) обязан был
            // знать про это исключение. Слово его снимает.
            //
            // ТОЛЬКО В РЕЖИМЕ ЭФФЕКТА. У протокола форма своя (`@`, `mut @`,
            // `consume @`, `.`), и `fn` туда не вводится: заказ владельца —
            // «только добавить fn и ничего больше», а протокол не просили.
            //
            // ЗАМЕР ДО ПРАВКИ (записан в плане 272, 2026-08-16): оракул
            // отвечал `expected identifier, got `fn`` ровно здесь.
            if !is_protocol {
                let _ = self.eat(&TokenKind::KwFn);
            }
        ...
        while i < bytes.len() {
            ...
            if b == b'\\' && i + 1 < bytes.len() {
                ...
                let esc_len = match bytes[i + 1] {
                    // D467 §6 (план 277 Ф.4): набор escape в backtick — РОВНО
                    // три, и только то, что иначе не записать: сам backtick,
                    // сам слеш и буквальный `${`. `\n`/`\t` УБРАНЫ — backtick
                    // многострочный (перевод строки набирается клавишей), а
                    // раскрытие `\n`/`\t` ломало пути и regex: `` `C:\new` ``
                    // давало перевод строки и таб. Всё остальное после `\`
                    // проходит насквозь ДВУМЯ символами, отчего
                    // `` regex`\d{3}` `` работает без двойного экранирования.
                    b'`' => { cur.push('`'); 1 }
                    b'\\' => { cur.push('\\'); 1 }
                    b'$' => { cur.push('$'); 1 }
                    other => {
                        // ЭТОТ СИМВОЛ БЫВАЕТ МНОГОБАЙТНЫМ, и здесь было сразу
                        // ДВА дефекта одного корня (реестр 221.1 №853).
                        // `other as char` — один БАЙТ, прочитанный как
                        // Latin-1: `\я` попадал в ЗНАЧЕНИЕ литерала как `\Ñ`.
                        // А общий `i += 2` оставлял индекс ВНУТРИ символа, и
                        // следующий срез `raw[i..]` падал: «byte index is not
                        // a char boundary». Падал не только `nova build`, но и
                        // языковой сервер — редактор перезапускал его на
                        // каждой правке такого файла.
                        //
                        // `i` стоит на ASCII-слэше, значит `i + 1` — граница
                        // символа, и срез безопасен.
                        let ch = raw[i + 1..]
                            .chars()
                            .next()
