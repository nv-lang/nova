...
impl<'a> Lexer<'a> {
    ...
    fn lex_string(&mut self, start: usize) -> Result<Token, Diagnostic> {
        ...
        loop {
            ...
            match b {
                ...
                // D467 §2 (план 277 Ф.2, реестр 221.1 №794): СЫРОЙ перевод
                // строки внутри `"..."` — хард-ошибка, а не содержимое.
                //
                // ДО ЭТОГО ветка `_` ниже глотала его как любой другой байт, и
                // многострочный `"..."` работал ДЕ-ФАКТО, нигде не будучи
                // санкционированным: спека такой формы не знала, а лексер
                // `novac` (`lex.nv:435`) её СОЗНАТЕЛЬНО отвергал. Два
                // компилятора расходились в базовом правиле языка — тот же
                // класс, что закрыл `E_STR_CONCAT_PLUS` 2026-07-21.
                //
                // ПОЧЕМУ ХАРД, А НЕ ПРЕДУПРЕЖДЕНИЕ: забытая закрывающая кавычка
                // раньше уезжала в следующую строку и всплывала где угодно
                // ниже; теперь она называется на месте. Законные формы никуда
                // не делись и обе описаны в D467: продолжение строки `\` +
                // перенос (Ф.1) для длинного текста без переводов и backtick
                // (Ф.3) для многострочного.
                //
                // ЦЕНА ЗАМЕРЕНА, А НЕ ОЦЕНЕНА (2026-09-06, свой сканер по
                // правилу счёта Ф.2, доказанный обеими сторонами): носителей в
                // дереве НОЛЬ — `novac/src` 0 (миграция слита `6bf621f75`),
                // `std/src` 0, `examples` 0, `nova_tests` 0. Единственный —
                // негативная фикстура `neg/interp_unterminated_neg.nv:17`, и
                // она нарочно обрывает строку.
                b'\n' => {
                    return Err(Diagnostic::new(
                        "[E_STR_RAW_NEWLINE] a raw newline inside a \"...\" literal is not \
                         allowed: end the line with `\\` to continue the string, or use a \
                         backtick literal `` `...` `` for multi-line text (D467)"
                            .to_string(),
                        self.span(start, self.pos),
                    ));
                }
    ...
    fn lex_char(&mut self, start: usize) -> Result<Token, Diagnostic> {
        ...
        let cp: u32 = if b == b'\\' {
            ...
            // дефект был не в семантике, а в том, что автор писал не то, что
            // думал: `'<таб>'` и `'<пробел>'` при чтении неотличимы, а
            // `'<LF>'` ещё и разрывает строку. Такое не ловится ни тестом, ни
            // ревью — глазу не на что смотреть. На этом обжёгся лексер novac:
            // собранный бинарь зациклился на выделении памяти до смерти Boehm
            // (`novac/src/lex/lex.nv:235`, маркер `[LEGACY-#675]`).
            //
            // ПРОВЕРКА ПО БАЙТУ, А НЕ ПО ДЕКОДИРОВАННОМУ СИМВОЛУ, и это не
            // срезание угла: все управляющие из U+0000..U+001F и U+007F —
            // однобайтовые в UTF-8, а ни один байт продолжения (0x80..0xBF)
            // в этот диапазон не попадает. То есть условие точное, а не
            // приблизительное.
            let shown = match b {
                0x09 => "\\t",
                0x0A => "\\n",
