...
pub enum AssignOp {
    ...
    Mul,
    Div,
    /// `%=` — D46-амендмент §D (2026-08-16). Лоуэрится как `Div`-близнец:
    /// D427 даёт `checked_rem` для каждого члена `Ints` тем же guard'ом,
    /// что `checked_div` (`b==0` и signed MIN/-1 — x86 `idiv` фолтит на
    /// overflow частного независимо от истинного остатка).
    Mod,
