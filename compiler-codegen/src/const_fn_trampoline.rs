            ...
            if let ExprKind::Ident(n) = &base.kind {
                if (n == "size_of" || n == "align_of") && args.is_empty() && type_args.len() == 1 {
                    match type_size_or_align(&type_args[0], n == "align_of") {
                        Some(v) => {
                            *e = Expr { kind: ExprKind::IntLit(v), span, id: crate::ast::ExprId::UNSET, debug_only: false };
                            return;
                        }
                        None => {
    ...
    fn maybe_rewrite(&self, e: &mut Expr) -> bool {
        if let ExprKind::Ident(n) = &e.kind {
            ...
            if self.trampoline_set.contains(resolved) {
                ...
                *e = Expr { kind: ExprKind::Ident(new_name), span, id: crate::ast::ExprId::UNSET, debug_only: false };
                return true;
            }
            if let Some(mangled) = self.generic_instance_map.get(resolved) {
                let span = e.span;
                *e = Expr { kind: ExprKind::Ident(mangled.clone()), span, id: crate::ast::ExprId::UNSET, debug_only: false };
                return true;
            }
        }
        false
