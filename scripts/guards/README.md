...
| [`check-single-mco-resume.sh`](check-single-mco-resume.sh) | Единственный вызов `mco_resume()` в рантайме Vela — внутри `fibers.h::nova_resume_fiber` (реестр 221.1 №446/№447): закрывает класс «инвариант держится соглашением на N сайтах, не структурой». |
