...
def offenders(text):
    ...
    for i, line in enumerate(text.split("\n"), 1):
        # Строка-КОММЕНТАРИЙ не судится: в комментарии законны и история
        # класса, и образец запрещённой формы. То же правило у соседа по
        # предмету — `check-novac-no-grammar-excuse` судит ТОЛЬКО строковые
        # литералы, «в комментарии история класса законна».
        #
        # Пробел найден СОБСТВЕННОЙ обратной пробой: зонд был вставлен
        # комментарием, страж на него покраснел — то есть в прежнем виде он
        # запрещал бы РАССКАЗЫВАТЬ о дефекте рядом с местом, где тот жил.
...
def main():
    ...
    if not bad:
        # МИШЕНЬ, А НЕ НОЛЬ (№911). Ноль носителей и ноль нарушений печатаются
        # одинаково, поэтому зелёный ноль С ЧИСЛОМ на потерянной мишени
        # читается как успешный замер. Честны ровно два ответа: ненулевой код
        # либо `ok` с оговоркой. Здесь — отказ, как у образца
        # check-novac-prim-id-compare.
        if seen == 0:
            sys.stderr.write(
                "%s: FAIL — под судом ни одного файла .rs в %s: мишень"
                " потеряна, а не «данных в позиции формата 0» (№911)\n"
                % (NAME, src))
            return 1
