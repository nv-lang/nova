#!/bin/sh
# Самотест check-novac-no-alloc-in-lookup.py (П16: обязан доказать, что ловит).
#
...
GD="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$GD/../.." && pwd)"
G="$GD/check-novac-no-alloc-in-lookup.py"
...
# --- 12. настоящее дерево ------------------------------------------------
python "$G" "$ROOT" >/dev/null 2>&1 && ok "настоящее дерево novac/src — зелёное" || bad "настоящее дерево покраснело: $(python "$G" "$ROOT" 2>&1 | head -3)"
