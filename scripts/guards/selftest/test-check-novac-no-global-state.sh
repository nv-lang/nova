#!/bin/sh
# Самотест check-novac-no-global-state.py — оба направления (норма 254):
# ловит нарушение И не краснеет на законном.
export LC_ALL=C
G="$(cd "$(dirname "$0")/.." && pwd)/check-novac-no-global-state.py"
...
EOF
python "$G" "$ROOT" "$T/g2/src" >/dev/null 2>&1 && ok "локальный mut проходит" || bad "локальный mut покраснел"
python "$G" "$ROOT" "$T/g2/src" 2>/dev/null | grep -q 'ok:' && ok "печатает строку ok:" || bad "нет строки ok: (№645)"
...
fn run() {}
EOF
python "$G" "$ROOT" "$T/g3/src" >/dev/null 2>&1 && bad "top-level mut прошёл" || ok "top-level mut пойман"
...
EOF
python "$G" "$ROOT" "$T/g4/src" >/dev/null 2>&1 && bad "export mut прошёл" || ok "export mut пойман"
...
EOF
python "$G" "$ROOT" "$T/g5/src" >/dev/null 2>&1 && bad "static mut прошёл" || ok "static mut пойман"
...
interner
EOF
python "$G" "$ROOT" "$T/g6/src" >/dev/null 2>&1 && ok "имя из GLOBALS.allow проходит" || bad "имя из GLOBALS.allow покраснело"
...
interner
EOF
python "$G" "$ROOT" "$T/g7/src" >/dev/null 2>&1 && bad "чужое имя при непустом allow прошло" || ok "чужое имя при непустом allow поймано"
