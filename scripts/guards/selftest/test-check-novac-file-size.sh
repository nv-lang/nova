#!/bin/sh
# Самотест check-novac-file-size.sh — оба направления (норма 254):
# ловит нарушение И не краснеет на законном.
export LC_ALL=C
G="$(cd "$(dirname "$0")/.." && pwd)/check-novac-file-size.sh"
...
sh "$G" "$ROOT" "$T/src" >/dev/null 2>&1 && ok "короткий файл проходит" || bad "короткий файл покраснел"
...
sh "$G" "$ROOT" "$T/src" >/dev/null 2>&1 && ok "ровно 1000 строк проходит" || bad "1000 строк покраснели"

# 3. Нарушение: 1001 строка — красный.
awk 'BEGIN{for(i=0;i<1001;i++)print "line"}' > "$T/src/big.nv"
sh "$G" "$ROOT" "$T/src" >/dev/null 2>&1 && bad "1001 строка прошла" || ok "1001 строка поймана"
rm -f "$T/src/big.nv"

# 4. Законный: не-.nv файл любой длины — не судится.
awk 'BEGIN{for(i=0;i<2000;i++)print "line"}' > "$T/src/huge.c"
sh "$G" "$ROOT" "$T/src" >/dev/null 2>&1 && ok "не-.nv файл не судится" || bad "не-.nv файл покраснел"
...
mkdir -p "$T/empty"
sh "$G" "$ROOT" "$T/empty" 2>/dev/null | grep -q 'ok: судить нечего' && ok "пустая директория — судить нечего" || bad "пустая директория — не зелено или молчит"
...
sh "$G" "$ROOT" "$T/src" >/dev/null 2>&1 && bad "вложенный длинный файл прошёл" || ok "вложенный длинный файл пойман"
