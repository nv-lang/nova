...
# --- разделитель имени — не имя (ЗАМЕР 2026-08-20) -----------------------
# c_callable собирает имя StringBuilder-ом, и куски "__"/"__to_" попали в тот
# же греп, что имена: страж покраснел на верной правке. Оба случая ниже держат
# границу — назван поимённо ровно разделитель, а не всё, что с двух подчёркиваний.
F=$(mk joiner 'fn a() -> str => "novac_fn_"' 'fn b() -> str => "__"' 'fn c() -> str => "__to_"')
run "$F" && ok "названные разделители имени законны" || bad "разделитель покраснел: $(cat "$T/err")"

F=$(mk joinerlike 'fn a() -> str => "novac_fn_"' 'fn b() -> str => "__weird_name"')
if run "$F"; then
    bad "неназванный кусок с двух подчёркиваний прошёл — перечисление подменено образцом"
else
    grep -q "__weird_name" "$T/err" && ok "неназванный кусок пойман (образец бы пропустил)" || bad "красный, но не про кусок"
fi
