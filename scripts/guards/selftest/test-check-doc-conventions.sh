...
(
    ...
    if [ "$code" -ne 0 ]; then
        echo "SELFTEST FAIL: 2b — предупреждение не должно ронять гейт (код $code)" >&2
        exit 1
    fi
    grep -q "same-commit pairing" /tmp/dc_2b_out_$$ || { echo "SELFTEST FAIL: 2b не сообщил об однобокой правке пары" >&2; exit 1; }
    rm -f /tmp/dc_2b_out_$$
) || fails=$((fails + 1))
rm -rf "$GTMP"
