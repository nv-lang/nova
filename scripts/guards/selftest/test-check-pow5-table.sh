...
#   6. объявленная длина `[650]u64` при 651 строке — красно, названа именно она:
#      длина стала частью типа после фикса №998/№1003, и это второй, независимый
#      способ увидеть ту же порчу;
#   7. таблицы нет вовсе — красно и называет путь (мишень потеряна != ноль).
#
...
REL="std/src/runtime/float_parse/pow5_table.nv"
FAILED=0
CASES=0
...
# 6. Объявление расходится с числом строк: `[650]u64` при 651 строке.
cp "$TMP/table.good" "$TMP/$REL"
sed -i 's/^const POW5_HI \[651\]u64 = \[$/const POW5_HI [650]u64 = [/' "$TMP/$REL"
if cmp -s "$TMP/table.good" "$TMP/$REL"; then
    bad "фикстура случая 6 не изменила файл — объявление POW5_HI [651]u64 не найдено"
else
    out=$(bash "$G" "$TMP" 2>&1); rc=$?
    if [ "$rc" -eq 1 ] && echo "$out" | grep -q "declared length"; then
        ok "ловит объявленную длину, разошедшуюся с числом строк"
    else
        bad "не поймал расхождение объявленной длины (код $rc): $out"
    fi
fi

# 7. Таблицы нет вовсе — красно, с путём.
rm -f "$TMP/$REL"
out=$(bash "$G" "$TMP" 2>&1); rc=$?
