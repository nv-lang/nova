...
# ── ОКУПАЕМОСТЬ (пункт 7 владельца, 2026-09-23) ─────────────────────────
# 13. три последние свёрнутые охоты по трём разным клеткам без единой строки
#     реестра — красный, и красный называет развилку владельца;
L="$T/docs/dev/hunts/novac/LEDGER.md"
mk_root
printf '%s\n' 'СВЁРНУТО | 2026-09-01-lex-k1 | lex | К1 | находок 0 | —' \
    'СВЁРНУТО | 2026-09-02-parse-k2 | parse | К2 | находок 0 | —' \
    'СВЁРНУТО | 2026-09-03-check-k3 | check | К3 | находок 0 | —' > "$L"
if sh "$GUARD" "$T" >"$T.o13" 2>&1; then
    echo "FAIL: три пустые охоты по разным клеткам прошли — окупаемость не судится" >&2
    rc=1
elif ! grep -q 'НЕ ОКУПАЕТСЯ' "$T.o13" || ! grep -q 'решение владельца' "$T.o13"; then
    echo "FAIL: красный по окупаемости не назвал себя или развилку владельца:" >&2
    tail -2 "$T.o13" | sed 's/^/    /' >&2
    rc=1
fi
# 14. та же картина, но новейшая охота родила строку — зелёный;
mk_root
printf '%s\n' 'СВЁРНУТО | 2026-09-01-lex-k1 | lex | К1 | находок 0 | —' \
    'СВЁРНУТО | 2026-09-02-parse-k2 | parse | К2 | находок 0 | —' \
    'СВЁРНУТО | 2026-09-03-check-k3 | check | К3 | находок 1 | №1' > "$L"
if ! sh "$GUARD" "$T" >"$T.o14" 2>&1; then
    echo "FAIL: охота со строкой реестра среди трёх последних — ложный красный по окупаемости:" >&2
    tail -2 "$T.o14" | sed 's/^/    /' >&2
    rc=1
fi
# 15. три пустые охоты ОДНОЙ клетки говорят о клетке, не об охоте — не красный.
mk_root
printf '%s\n' 'СВЁРНУТО | 2026-09-01-lex-k1 | lex | К1 | находок 0 | —' \
    'СВЁРНУТО | 2026-09-02-lex-k1b | lex | К1 | находок 0 | —' \
    'СВЁРНУТО | 2026-09-03-lex-k1c | lex | К1 | находок 0 | —' > "$L"
...
[ "$rc" -eq 0 ] && echo "test-check-hunter-mark ok: одиннадцать подделок покраснели, цитата-в-прозе не ложнит, окупаемость судится в обе стороны, здоровый макет и живая половина зелёные"
exit "$rc"
