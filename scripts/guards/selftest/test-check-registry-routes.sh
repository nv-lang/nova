...
mk() {  # $1 = добавочные строки
    ...
    # Пустая база строк без поля статуса: в фикстурах их нет, все
    # строки написаны каноном. Файл нужен ПУСТЫМ, а не отсутствующим —
    # отсутствие базы страж считает своей поломкой и кричит, и это верно.
    printf 'nofield=0\n' > "$TMP/r/scripts/guards/registry-nofield.baseline"
}
...
echo "== ловит =="
# «ЗАКРЫТ ЧАСТИЧНО» обязана считаться ОТКРЫТОЙ: иначе остаток класса исчезает
# из всех трёх чисел разом. Поймано 2026-08-16 на №703 — пометка «частично»
# прятала непокрытые формы исчерпаемости от счётчика блокеров.
mk '| 7 | 🔴 К1 | **G.** КЛАСС: x. **ОГОВОРКА: фикс носителя приёмкой не считается** — y. **ЧИНИТСЯ:** план 9. **БЛОКИРУЕТ ТЕГ: ДА** **Статус:** ЗАКРЫТ ЧАСТИЧНО |'
out=$(bash "$TMP/r/scripts/guards/check-registry-routes.sh" "$TMP/r" 2>&1); rc=$?
if [ "$rc" -ne 0 ] && printf '%s' "$out" | grep -q 'БЛОКЕРОВ ТЕГА'; then ok "«ЗАКРЫТ ЧАСТИЧНО» считается ОТКРЫТОЙ, остаток класса не прячется"; else bad "«ЧАСТИЧНО» прочтено как закрытое (rc=$rc): $out"; fi
