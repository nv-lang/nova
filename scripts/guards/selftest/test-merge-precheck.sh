...
#   8. --report: красное своего дерева печатается дословно, с числами, rc=1;
#   9. --report на чистом дереве -> ok, rc=0.
set -u
export LC_ALL=C
...
echo "== 8. --report: красное своего дерева ДОСЛОВНО, с числами =="
reset
printf '%s\n' "$INH" > "$T/br/main.fails"; echo 1 > "$T/br/main.rc"
OUT=$(bash "$TOOL" --report "$T/br" 2>&1); RC=$?
[ "$RC" -eq 1 ] && ok "8: красное дерево — rc=1" || bad "8: rc=$RC: $OUT"
has "$OUT" '(было 3, стало 5)' "8: строка отказа напечатана как есть, числа не заменены"
has "$OUT" 'КРАСНОЕ НА ЭТОМ ДЕРЕВЕ (ярус loop обоих гейтов) — 1' "8: счёт назван"
hasnt "$OUT" "$T/base" "8: база не судилась — в чужое дерево не заходит"

echo "== 9. --report: чистое дерево — ok =="
reset
OUT=$(bash "$TOOL" --report "$T/br" 2>&1); RC=$?
[ "$RC" -eq 0 ] && ok "9: чистое дерево — rc=0" || bad "9: rc=$RC: $OUT"
has "$OUT" 'merge-precheck ok:' "9: строка ok напечатана"
