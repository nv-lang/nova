#!/bin/sh
# Самотест check-novac-atomics-door.py — оба направления (норма 254):
# ловит нарушение И не краснеет на законном.
export LC_ALL=C
G="$(cd "$(dirname "$0")/.." && pwd)/check-novac-atomics-door.py"
...
EOF
python "$G" "$ROOT" "$T/src" >/dev/null 2>&1 && ok "дверь с примитивами + чистый сосед проходят" || bad "законная дверь покраснела"
...
python "$G" "$ROOT" "$T/src" >/dev/null 2>&1 && bad "__atomic_ вне двери прошёл" || ok "__atomic_ вне двери пойман"
rm -f "$T/src/sched/raw.nv"

# 3. Нарушение: 'thread_local' вне двери — красный.
printf 'fn g() { emit("static thread_local int t;") }\n' > "$T/src/sched/tls.nv"
python "$G" "$ROOT" "$T/src" >/dev/null 2>&1 && bad "thread_local вне двери прошёл" || ok "thread_local вне двери пойман"
...
python "$G" "$ROOT" "$T/src" >/dev/null 2>&1 && bad "nova_atomic_ вне двери прошёл" || ok "nova_atomic_ вне двери пойман"
...
python "$G" "$ROOT" "$T/src" >/dev/null 2>&1 && bad "вложенное нарушение прошло" || ok "вложенное нарушение поймано"
