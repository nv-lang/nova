...
out=$(bash "$G" "$TMP" 2>&1); rc=$?
if [ "$rc" -eq 1 ] && echo "$out" | grep -q "spec/syntax.ru.md"; then ok "ловит устаревшую пометку"; else bad "не поймал устаревшую пометку (код $rc): $out"; fi
