#!/bin/sh
# Самотест режима `-c` у scripts/tools/gate-bg.sh.
#
# Доказывает мутацией обе половины правки 2026-09-07:
#   * МОЯ: при живом процессе яруса novac печатается «ИДЁТ ЯРУС NOVAC», а не
#     сохранённый вердикт от прошлого прогона (правило Г9: вердикт говорит ровно
#     то, что механизм установил);
#   * nova-55: сохранённый вердикт обязан называть свой ВОЗРАСТ и отставание
#     судимого дерева от HEAD — иначе он читается как свежий и про текущий HEAD.
#
# Проба идёт В ОБЕ СТОРОНЫ у каждой половины: свежий вердикт на текущем HEAD НЕ
# должен давать предупреждения об отставании, протухший на старом хэше — обязан.
# Без второй стороны первая ничего не доказывает.
#
# МАТРИЦА ПОКРЫТИЯ ПО ОСЯМ (заведена 2026-09-08, после того как девятый случай нашёл ЖИВОЙ ПРОГОН, а
# не этот файл). Случаи самотеста пишутся по историям — «такой отказ мы видели», — а отказы
# приходят по КОМБИНАЦИЯМ. Восемь случаев выглядели плотными и покрывали две клетки из
# четырёх по осям A и B; обожгла третья. Матрица ниже — единственное, что скажет следующему
# автору, где искать десятый случай.
#
# ОСИ, которые здесь варьируются:
#   A. хэш вердикта:      текущий HEAD | старый коммит | не найден в дереве
#   B. возраст файла:     свежий (< STALE_MIN) | старый
#   C. ярус novac:        живой процесс есть | нет
#
# КЛЕТКИ (A x B при C=нет; ось C проверяется отдельно двумя случаями):
#   текущий + свежий  -> ПОКРЫТА (случаи 1, 2а-свежий): «судил ТЕКУЩИЙ HEAD», метка СВЕЖИЙ
#   текущий + старый  -> ПОКРЫТА (случай 2б): метка ПРОТУХШИЙ, хотя дерево не двигалось.
#                        Эта клетка и обожгла живой прогон: совпадение хэша делает половину
#                        утверждения правдой и даёт основание не смотреть на минуты.
#   старый  + свежий  -> ПОКРЫТА (случаи 2, 2а-протухший): отставание названо числом
#   старый  + старый  -> НЕ НУЖНА ПО ПРИЧИНЕ: обе оси уже говорят «протухший», клетка не
#                        различает поведений — она сильнее каждой покрытой, а не иная
#   ненайден + любой  -> ПОКРЫТА одним случаем: ветка «хэш НЕ НАЙДЕН» печатается независимо
#                        от возраста, поэтому ось B здесь не варьируется по построению
#   ось C             -> ПОКРЫТА двумя случаями: с живым процессом строка про ярус есть, без
#                        процесса её нет (проба в обе стороны)
#   ось D             -> ПОКРЫТА двумя случаями (7 и 8), заполнена 2026-09-08 в тот же день,
#                        когда причина её пустоты исчезла. История ниже оставлена нарочно:
#                        клетка стояла пустой С ПРИЧИНОЙ, и в ней же был потерян блок за
#                        `exit 0` — пустая клетка есть адрес следующего дефекта. 2026-09-08
#                        `gate-bg -c` научился читать вердикт ЯРУСА NOVAC (`2f64811e2`,
#                        переиспользуя `_verdict_tag` этого же файла), то есть появилась ось
#                        «вердикт novac есть | нет». От оси C она НЕЗАВИСИМА: C про ЖИВОЙ
#                        ПРОЦЕСС, D про ОСТАВЛЕННЫЙ ФАЙЛ, и обычное состояние после прогона —
#                        файл есть, процесса нет. Случай не написан не из лени: фикс лежит в
#                        ЛОКАЛЬНОМ `main` интегратора, а `origin/main` от этой ветки отстаёт
#                        (`git rev-list --count HEAD..origin/main` = 0 при 32 коммитах в
#                        локальном main), и сливать чужую непушенную ветку окно не вправе.
#                        Сегодня такой случай проверял бы КОД, КОТОРОГО В ЭТОМ ДЕРЕВЕ НЕТ, и
#                        краснел бы по причине, к предмету не относящейся. Пишется первым
#                        делом после того, как `main` уедет в origin.
#                        ДВЕ КЛЕТКИ, которые он обязан назвать (проба в обе стороны): вердикт
#                        novac ЕСТЬ -> его возраст объявлен той же меткой, что у основного, и
#                        протухший говорит «протухший»; вердикта НЕТ -> строки про novac нет
#                        вовсе, а основная строка печатается как раньше.
#
# ЧТО ЭТО ЗНАЧИТ ДЛЯ ДОПИСЫВАЮЩЕГО: новая ось (например «файла вердикта нет вовсе» или
# «лог есть, а .done снят») удваивает матрицу — выпиши произведение и назови КАЖДУЮ клетку
# покрытой, ненужной с причиной, или незаполненной. «Не нужна» без причины неотличима от
# «не догадался».
export LC_ALL=C

GD="$(cd "$(dirname "$0")/.." && pwd)"
ROOT="$(cd "$GD/../.." && pwd)"
G="$ROOT/scripts/tools/gate-bg.sh"
T="${TMPDIR:-/tmp}/gate-bg-status-selftest.$$"
mkdir -p "$T"
trap 'rm -rf "$T"; kill "$FAKE_PID" 2>/dev/null' 0

fails=0
cases=0
ok()  { echo "  ok: $1"; cases=$((cases+1)); }
bad() { echo "  FAIL: $1" >&2; fails=$((fails+1)); }

HEAD_SHA=$(git -C "$ROOT" rev-parse HEAD 2>/dev/null)
OLD_SHA=$(git -C "$ROOT" rev-parse HEAD~5 2>/dev/null)

# Шов: подставляем свои файлы и своё имя процесса, живое дерево не трогаем.
run_c() {
    GATE_BG_DONE="$1" GATE_BG_LOG="$T/empty.log" GATE_BG_STAMP="$T/stamp" \
    GATE_BG_NOVAC_PAT="${2:-selftest-no-such-process-pattern}" \
    NOVA_NOVAC_VERDICT="${3:-$T/no-such-novac-verdict}" \
    bash "$G" -c 2>&1
}
: > "$T/empty.log"

# ── 1. свежий вердикт на ТЕКУЩЕМ HEAD — отставания нет ────────────────────
echo "RC=0 SEC=100 TIER=main:push HASH=$HEAD_SHA BRANCH=main" > "$T/fresh.done"
out=$(run_c "$T/fresh.done")
if echo "$out" | grep -q "судил ТЕКУЩИЙ HEAD"; then
    if echo "$out" | grep -q "ПОЗАДИ HEAD"; then
        bad "свежий вердикт назван отстающим"
    else
        ok "свежий вердикт на текущем HEAD — отставания не заявлено"
    fi
else
    bad "нет строки про текущий HEAD: [$(echo "$out" | tail -1)]"
fi

# ── 2. вердикт о СТАРОМ дереве — отставание названо числом ────────────────
echo "RC=0 SEC=100 TIER=main:push HASH=$OLD_SHA BRANCH=main" > "$T/stale.done"
out=$(run_c "$T/stale.done")
if echo "$out" | grep -qE "судил дерево на [0-9]+ коммит\(ов\) ПОЗАДИ HEAD"; then
    ok "протухший вердикт — отставание названо числом"
else
    bad "отставание не названо: [$(echo "$out" | tail -1)]"
fi

# ── 2а. СВЕЖЕСТЬ СТОИТ В ПЕРВОЙ строке, а не только в последней ──────
# Случаи 1 и 2 выше судят ПОДРОБНУЮ строку, и она печатается последней. Замер
# 2026-09-08: окно, которое эту строку и завело, четыре раза подряд читало вывод через
# `head -2` и её не видело — приняло вердикт девятичасовой давности за состояние машины.
# Свойство «сказано» выполнялось, «прочитано» — нет. Эти два случая режут вывод ровно так,
# как его читают, и требуют метки в заголовке.
first=$(run_c "$T/stale.done" | head -1)
if echo "$first" | grep -q "ПРОТУХШИЙ"; then
    ok "протухший вердикт назван протухшим в первой строке"
else
    bad "первая строка не называет протухания: [$first]"
fi

first=$(run_c "$T/fresh.done" | head -1)
if echo "$first" | grep -q "СВЕЖИЙ" && ! echo "$first" | grep -q "ПРОТУХШИЙ"; then
    ok "свежий вердикт назван свежим в первой строке"
else
    bad "первая строка судит свежий вердикт неверно: [$first]"
fi

# ── 2б. свежий хэш при СТАРОМ файле — случай, найденный ЖИВЫМ прогоном ────────
# Интегратор 2026-09-08: вердикт на ТЕКУЩЕМ HEAD и при этом 147 минут от роду. Он решил,
# что счётчик врёт, потому что «судил текущий HEAD» было правдой. Оси независимы: дерево
# не двигалось, а машина за два часа изменилась. Случаи 1 и 2а этой комбинации НЕ
# покрывали — обе стороны там со свежим файлом, и дыра нашлась не самотестом, а прогоном.
cp "$T/fresh.done" "$T/oldfile.done"
touch -d "@$(( $(date +%s) - 3 * 3600 ))" "$T/oldfile.done" 2>/dev/null \
    || touch -t "$(date -d '3 hours ago' +%Y%m%d%H%M 2>/dev/null)" "$T/oldfile.done" 2>/dev/null
first=$(run_c "$T/oldfile.done" | head -1)
if echo "$first" | grep -q "ПРОТУХШИЙ"; then
    ok "вердикт на текущем HEAD, но трёхчасовой — назван протухшим"
else
    bad "старый файл на текущем хэше сочтён свежим: [$first]"
fi

# ── 3. хэш, которого в дереве нет — сказано прямо ─────────────────────────
echo "RC=1 SEC=100 TIER=main:push HASH=deadbeefdeadbeefdeadbeefdeadbeefdeadbeef BRANCH=main" > "$T/unknown.done"
out=$(run_c "$T/unknown.done")
if echo "$out" | grep -q "НЕ НАЙДЕН"; then
    ok "неизвестный хэш назван неизвестным, а не выдан за совпадение"
else
    bad "молчит про неизвестный хэш: [$(echo "$out" | tail -1)]"
fi

# ── 4. ЖИВОЙ процесс яруса novac — вердикт не выдаётся за текущее состояние ─
# Случай кодирует ЗАМЕР: поднимается НАСТОЯЩИЙ процесс, чьё имя ловится тем же
# grep'ом, что и `gate-novac.sh`, а не подделывается вывод `ps`.
cat > "$T/selftest-fake-novac.sh" <<'FAKE'
sleep 12
FAKE
bash "$T/selftest-fake-novac.sh" &
FAKE_PID=$!
sleep 1
out=$(run_c "$T/fresh.done" "selftest-fake-novac")
if echo "$out" | grep -q "ИДЁТ ЯРУС NOVAC"; then
    ok "живой ярус novac виден и назван"
else
    bad "живой ярус novac не замечен: [$(echo "$out" | head -1)]"
fi

# ── 5. обратная сторона: процесса нет — строки про novac быть не должно ────
out=$(run_c "$T/fresh.done" "selftest-no-such-process-pattern")
if echo "$out" | grep -q "ИДЁТ ЯРУС NOVAC"; then
    bad "строка про novac печатается БЕЗ живого процесса"
else
    ok "без процесса строки про novac нет"
fi
kill "$FAKE_PID" 2>/dev/null

# ── 6. префиксы существующих строк не тронуты (их грепают планы и окна) ────
out=$(run_c "$T/fresh.done")
if echo "$out" | grep -qE "^ГЕЙТ ЗЕЛ"; then
    ok "префикс вердикта сохранён"
else
    bad "префикс вердикта изменён: [$(echo "$out" | head -1)]"
fi

# ── 7. ОСЬ D, клетка «вердикт novac ЕСТЬ»: строка печатается И несёт метку возраста.
# Заполнено 2026-09-08, как только фикс читателя приехал в `origin/main`. Клетка была
# названа незаполненной с причиной — и в тот же день именно в ней потерялась строка,
# уехавшая за `exit 0` при девяти зелёных случаях. Отсюда и форма проверки: мало убедиться,
# что строка ЕСТЬ, — она обязана нести возраст, иначе протухший вердикт novac промолчит о том,
# что он протухший.
echo "RC=1 SEC=2074 TIER=novac HASH=$OLD_SHA BRANCH=main" > "$T/novac.done"
touch -d "3 hours ago" "$T/novac.done" 2>/dev/null || touch -A -0300 "$T/novac.done" 2>/dev/null || :
out=$(run_c "$T/fresh.done" "" "$T/novac.done")
if echo "$out" | grep -q "ЯРУС NOVAC"; then
    if echo "$out" | grep -qE "ЯРУС NOVAC.*(ПРОТУХШИЙ|СВЕЖИЙ)"; then
        ok "вердикт novac назван и несёт метку возраста"
    else
        bad "строка про вердикт novac есть, но БЕЗ метки возраста: [$(echo "$out" | grep "ЯРУС NOVAC")]"
    fi
else
    bad "вердикт novac подставлен, а строки о нём нет (блок недостижим?): [$(echo "$out" | tail -2)]"
fi

# ── 8. ОСЬ D, обратная сторона: вердикта НЕТ — строки про вердикт novac быть не должно,
# а основная строка печатается как раньше. Это НЕ случай 5: тот про живой ПРОЦЕСС (ось C),
# этот про оставленный ФАЙЛ, и одно без другого бывает.
out=$(run_c "$T/fresh.done" "" "$T/no-such-novac-verdict")
if echo "$out" | grep -q "ЯРУС NOVAC ЗЕЛ\|ЯРУС NOVAC КРАСН"; then
    bad "строка про вердикт novac печатается БЕЗ файла вердикта"
else
    if echo "$out" | grep -qE "^ГЕЙТ (ЗЕЛ|КРАСН)"; then
        ok "без файла вердикта novac строки о нём нет, основная строка на месте"
    else
        bad "основная строка пропала вместе с novac: [$(echo "$out" | head -1)]"
    fi
fi

if [ "$fails" -eq 0 ]; then
    echo "test-gate-bg-status ok: $cases случаев (счётчик, не литерал)"
    exit 0
fi
echo "test-gate-bg-status FAIL: $fails" >&2
exit 1
