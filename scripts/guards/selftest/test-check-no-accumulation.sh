...
if [ "$rc6" -ne 0 ]; then
    ...
    # ФОРМА ПРОВЕРЯЕТСЯ ОТДЕЛЬНО ОТ СЛОВ, и это куплено прогоном
    # 34890208340 (2026-09-15). Страж воздерживался ЧЕСТНО и выходил
    # нулём, но без строки `ok:` — а гейт считает выход нулём без неё
    # недоказанным шагом (№645). Композиция двух ВЕРНЫХ правил давала
    # на CI вечный отказ с ВЫДУМАННОЙ причиной: «накопление выросло»
    # при нуле веток. Этот самотест тогда был ЗЕЛЁН: он спрашивал
    # только про слова.
    if printf %s "$out6" | grep -q "^check-no-accumulation ok:"; then
        ok "среда без веток: воздержание названо И в форме ok: (гейт примет)"
    else
        bad "среда без веток: воздержание названо словами, но НЕ строкой ok: — гейт сочтёт шаг недоказанным (№645)"
    fi
elif false; then
