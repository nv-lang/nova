...
# --- 8. ФОРМА, КОТОРОЙ ПИШЕТ ВЕСЬ КОРПУС ---------------------------------------
# ЗАЧЕМ ЭТОТ СЛУЧАЙ. До 2026-09-06 страж требовал `echo` В НАЧАЛЕ СТРОКИ, а живой
# самотест печатает итог иначе: `if [ "$FAILED" -eq 0 ]; then echo "... N/N ok"; fi`.
# Совпадение оказалось нулевым: страж докладывал 0 нарушителей при 22 живых.
# Причина промаха — не шаблон числа, а ПРОБА: фикстуры выше строятся с `echo` с
# начала строки, то есть проверяли форму, которой в корпусе нет ни разу. Этот
# случай держит корпусную форму, чтобы слепота не вернулась третий раз.
# Слово `echo` собирается из переменной НАРОЧНО. Напиши я его буквально —
# ЭТА строка сама попала бы под шаблон (перед `echo` стоит `then`), и самотест
# стал бы нарушителем собственного стража — ровно тот случай, от которого
# берёт случай 6. Поймано замером в день правки: страж показал 22 вместо 21.
E=echo
mk "$T/inline" "test-f.sh" "if [ 1 -eq 0 ]; then $E \"test-f ok: 8/8\"; fi"
if run "$T/inline" "$T/base0"; then
    bad "литерал в 'then echo' прошёл зелёным — форма всего корпуса не судится"
else
    grep -q "test-f.sh:" "$T/err" && ok "литерал в 'then echo' пойман (форма корпуса)" || bad "красный, но без адреса"
fi
