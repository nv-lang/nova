...
# 2. Ловит загрузку ЦП.
#
# №765 (2026-08-26): порог был 0 с расчётом «любая загрузка обязана сработать»,
# то есть случай судил ПО ПОГОДЕ — на тихой машине загрузка честно равна нулю, и
# `0 > 0` ложно. На раннере было хуже: источника загрузки там не существовало
# вовсе (`wmic` — только Windows), и случай не мог сработать в принципе. Порог
# −1 срабатывает на ЛЮБОМ измеренном значении, включая ноль, — проверяется
# МЕХАНИЗМ, а не занятость машины. Где источника нет, случай пропускает себя
# ВСЛУХ: молчаливый пропуск и красный одинаково бесполезны, но красный ещё и
# учит не читать отчёт.
if [ -r /proc/loadavg ] || command -v wmic >/dev/null 2>&1; then
    out=$(NOVA_MEASURE_CPU_MAX=-1 bash "$M" -n 1 -l selftest-cpu -- true 2>&1); rc=$?
    if [ "$rc" -eq 1 ] && echo "$out" | grep -q 'загрузка-ЦП'; then
        ok "ловит загрузку ЦП (код 1)"
    else
        bad "НЕ поймал загрузку ЦП при пороге -1 (код $rc): $out"
    fi
else
    echo "  ПРОПУСК: источника загрузки ЦП на этой платформе нет (ни wmic, ни /proc/loadavg) — проверять нечем"
fi
