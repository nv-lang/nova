...
# ── мишень потеряна: НЕ «нарушений 0» (№911) ─────────────────────────────
# Каталог ЕСТЬ, файлов .rs в нём нет. Раньше печаталось `ok: файлов 0, данных
# в позиции формата 0` — ноль носителей и ноль нарушений выглядят одинаково,
# и потерянная мишень читалась как успешный замер. Мета-страж
# `check-guard-empty-root` этого стража не поймал: на его пустом каркасе нет
# даже каталога, и ответ был честной оговоркой. Дыра тем не менее была.
