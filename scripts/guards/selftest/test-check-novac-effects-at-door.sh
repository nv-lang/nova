...
#
# Три случая тут не про «ловит», а про «не красит зря»: комментарий с
...
if [ "$fails" -eq 0 ]; then
    echo "test-check-novac-effects-at-door ok: все случаи — три формы ниже двери, те же формы у двери, ложные тревоги (комментарий, фикстура-строка, символьный литерал) и потеря мишени"
    exit 0
fi
exit 1
