...
# --- 6а. булево поле строки-ЭЛЕМЕНТА без пометки «[на элемент]» — красное -
# Живой случай 2026-08-16: ParamDef.is_recv — свойство, которое по инварианту
# может держать лишь ОДИН элемент, положили на каждый.
mk_sem "$T/s6a.nv" 'export type ParamDef value {
    type_id int /// тип
    is_recv bool /// получатель
}

export type FnTable {
    params []ParamDef /// параметры
}'
mk_plan "$T/p6a.md" "ParamDef type_id" "ParamDef is_recv"
if run "$T/s6a.nv" "$T/p6a.md"; then
    bad "булево на строке-элементе без пометки прошло (П23)"
else
    grep -q "на элемент" "$T/err" && ok "булево на элементе без пометки поймано" || bad "красный, но не про пометку"
fi

# --- 6б. то же С пометкой — зелёное (вопрос задан, ответ дан) ------------
{ echo "# План"; echo ""; echo "### 10.3в. Реестр полей строк"; echo ""
  echo "| строка | поле | зачем |"; echo "|---|---|---|"
  echo "| \`ParamDef\` | \`type_id\` | тип |"
  echo "| \`ParamDef\` | \`is_recv\` | **[на элемент]** различается у каждого |"
  echo ""; echo "### 10.4. Другое"; } > "$T/p6b.md"
run "$T/s6a.nv" "$T/p6b.md" && ok "пометка «[на элемент]» снимает вопрос" || bad "помеченное поле покраснело: $(cat "$T/err")"

# --- 6в. булево на строке, которая НЕ элемент списка, — не судится -------
mk_sem "$T/s6v.nv" 'export type Flags value {
    on bool /// просто флаг
}'
mk_plan "$T/p6v.md" "Flags on"
run "$T/s6v.nv" "$T/p6v.md" && ok "булево вне списка не судится" || bad "ложняк (C): $(cat "$T/err")"
