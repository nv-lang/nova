...
# ничего не значит. Поэтому здесь ЧЕТЫРЕ случая, и три из них красные: нет
...
if [ "$rc" -eq 1 ] && echo "$out" | grep -q "ozhidanie"; then
    ok "правила нет — красно, и сказано какого"
else
...
out=$(python "$G" "$TMP" 2>&1); rc=$?
if [ "$rc" -eq 1 ] && echo "$out" | grep -q "vtoroy poloviny"; then
    ok "заголовок без «чем занять» — красно"
else
...
if [ "$FAILED" -eq 0 ]; then
    echo "test-check-carina-no-idle-wait ok: 5 случаев, из них 3 красных"
else
