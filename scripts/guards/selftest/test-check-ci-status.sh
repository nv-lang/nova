#!/usr/bin/env bash
# Самотест check-ci-status.sh.
#
# Страж без самотеста у нас считается неработающим (урок LC_ALL=C: страж молча
# давал ноль хитов и выглядел зелёным). Здесь доказываются свойства — по
# парам «ловит / не лжёт»:
#   1. RED    — красный прогон на хеше: сообщает RED; в --strict роняет (exit 1).
#   2. OK     — все прогоны зелёные: сообщает OK и НЕ роняет даже в --strict.
#   3. STALE  — прогонов на хеше нет, коммит старый: сообщает STALE, в --strict
#               роняет; при свежем коммите (порог не пройден) — НЕ роняет.
#   4. SKIP   — `gh` недоступен: exit 0 и явная строка о причине (падение сети
#               не должно ронять локальный гейт).
#   5. ERROR (№585) — прогонов на хеше нет И сам хеш нельзя прочитать
#               (`git show` падает — объект недоступен локально): ОШИБКА,
#               ненулевой код возврата ДАЖЕ БЕЗ --strict, и текст не должен
#               маскироваться под «прогонов пока нет».
#
# Приём: подставной `gh` в начале PATH отдаёт заранее заданный JSON. Настоящая
# сеть в самотесте не участвует.
export LC_ALL=C
set -u

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")/../../.." && pwd)"
GUARD="$ROOT/scripts/guards/check-ci-status.sh"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT

FAILED=0
ok()  { echo "  ok: $*"; }
bad() { echo "  ПРОВАЛ: $*"; FAILED=1; }

# Хеш, про который спрашиваем — берём реальный HEAD, чтобы `git show -s` знал
# его дату (для проверки порога STALE).
SHA="$(git -C "$ROOT" rev-parse HEAD)"
SHORT="$(echo "$SHA" | cut -c1-9)"

make_gh() {  # $1 — JSON, который вернёт `gh run list`
    mkdir -p "$TMP/bin"
    cat > "$TMP/bin/gh" <<EOF
#!/usr/bin/env bash
case "\$1" in
  auth) exit 0 ;;
  run)  cat <<'JSON'
$1
JSON
        ;;
  *) exit 0 ;;
esac
EOF
    chmod +x "$TMP/bin/gh"
}

run_guard() {  # $@ — аргументы стража; печатает вывод, возвращает код
    PATH="$TMP/bin:$PATH" bash "$GUARD" "$@" 2>&1
}
run_guard_code() {
    PATH="$TMP/bin:$PATH" bash "$GUARD" "$@" >/dev/null 2>&1; echo $?
}

echo "самотест check-ci-status:"

# ── 1. RED: ловит и роняет в --strict ────────────────────────────────────────
make_gh "[{\"name\":\"nova-gate\",\"status\":\"completed\",\"conclusion\":\"failure\",\"headSha\":\"$SHA\",\"createdAt\":\"2026-08-07T00:00\"}]"
OUT="$(run_guard "$SHA")"
echo "$OUT" | grep -q "RED" && ok "RED — красный прогон распознан" || bad "RED не распознан: $OUT"
[ "$(run_guard_code --strict "$SHA")" = "1" ] \
    && ok "RED — --strict роняет (exit 1)" || bad "RED — --strict не уронил"

# ── 1б. ЧЕЙ КРАСНЫЙ: ночной прогон отличается от твоей отправки ──────────────
#     Реестр 221.1 №785, замер 2026-08-27: страж остановил отправку по красному
#     прогону — и был прав, — но не сказал, что прогон НОЧНОЙ (event=schedule).
#     Ночной идёт ярусом full и доходит до самотестов стражей, куда push не
#     доходит вовсе, то есть его краснота была СТАРШЕ отправки и не про неё.
#     Выяснение этого стоило целого захода. Теперь страж называет событие и
#     номер прогона, а на schedule добавляет прямое предупреждение.
make_gh "[{\"name\":\"nova-gate\",\"status\":\"completed\",\"conclusion\":\"failure\",\"headSha\":\"$SHA\",\"createdAt\":\"2026-08-07T00:00\",\"event\":\"schedule\",\"databaseId\":33074386783}]"
OUT_N="$(run_guard "$SHA")"
echo "$OUT_N" | grep -q "schedule" \
    && ok "RED — событие названо (schedule)" || bad "RED — событие не названо: $OUT_N"
echo "$OUT_N" | grep -q "33074386783" \
    && ok "RED — номер прогона назван" || bad "RED — номер прогона не назван"
echo "$OUT_N" | grep -q "НОЧНОЙ ПРОГОН" \
    && ok "RED — ночной прогон назван своим именем" || bad "RED — про ночной ни слова"

#     И обратная сторона: у обычной отправки предупреждения быть НЕ должно,
#     иначе оно превратится в шум и его перестанут читать.
make_gh "[{\"name\":\"nova-gate\",\"status\":\"completed\",\"conclusion\":\"failure\",\"headSha\":\"$SHA\",\"createdAt\":\"2026-08-07T00:00\",\"event\":\"push\",\"databaseId\":42}]"
OUT_P="$(run_guard "$SHA")"
echo "$OUT_P" | grep -q "push" \
    && ok "RED — событие push тоже названо" || bad "RED — событие push не названо"
echo "$OUT_P" | grep -q "НОЧНОЙ ПРОГОН" \
    && bad "RED — предупреждение о ночном на обычной отправке (ложняк)" \
    || ok "RED — на обычной отправке про ночной молчит"

# 1a. ОТМЕНА ПО ВЫТЕСНЕНИЮ — НЕ отказ.
#     У nova-gate задана группа параллелизма: при частых пушах GitHub
#     снимает СТОЯЩИЙ В ОЧЕРЕДИ прогон, когда приходит более новый.
#     2026-08-19 это случалось на каждом втором пуше подряд, и страж,
#     считавший любую отмену красной, ронял бы отправки по причине,
#     которой нет (ложный красный останавливает всех, довод 703).
make_gh "[{\"name\":\"nova-gate\",\"status\":\"completed\",\"conclusion\":\"cancelled\",\"headSha\":\"$SHA\",\"createdAt\":\"2026-08-19T01:00:00Z\"},{\"name\":\"nova-gate\",\"status\":\"completed\",\"conclusion\":\"success\",\"headSha\":\"deadbeef1\",\"createdAt\":\"2026-08-19T02:00:00Z\"}]"
OUT_S="$(run_guard "$SHA")"
echo "$OUT_S" | grep -q "RED" \
    && bad "superseded cancel засчитан красным: $OUT_S" \
    || ok "отмена по вытеснению отказом НЕ считается"

# 1b. ОТМЕНА БЕЗ более нового прогона — НЕ отказ и НЕ зелёное: вердикта
#     НЕТ. Прежде тут стояло «её сняли намеренно» — допущение, снятое
#     замером 2026-08-19 (второй носитель №735): три подряд отмены
#     crate-tests были сняты ИЗ ОЧЕРЕДИ, не начав ни одного шага.
#     Отправка по-прежнему останавливается — меняются только слова.
make_gh "[{\"name\":\"nova-gate\",\"status\":\"completed\",\"conclusion\":\"cancelled\",\"headSha\":\"$SHA\",\"createdAt\":\"2026-08-19T03:00:00Z\"}]"
OUT_C="$(run_guard "$SHA")"
echo "$OUT_C" | grep -q "NOVERDICT\|НЕТ ВЕРДИКТА" \
    && ok "одинокая отмена названа отсутствием вердикта, а не отказом" \
    || bad "одинокая отмена не распознана: $OUT_C"
echo "$OUT_C" | grep -q "ГЕЙТ КРАСНЫЙ" \
    && bad "одинокая отмена всё ещё зовётся красной: $OUT_C" \
    || ok "слово «красный» на снятом прогоне не звучит"
[ "$(run_guard_code --strict "$SHA")" -ne 0 ] \
    && ok "отсутствие вердикта не пускает отправку (--strict)" \
    || bad "отсутствие вердикта пропустило отправку под --strict"

# ── 2. OK: не лжёт (зелёное не должно ронять даже в --strict) ────────────────
make_gh "[{\"name\":\"nova-gate\",\"status\":\"completed\",\"conclusion\":\"success\",\"headSha\":\"$SHA\",\"createdAt\":\"2026-08-07T00:00\"}]"
OUT="$(run_guard "$SHA")"
echo "$OUT" | grep -q "OK" && ok "OK — зелёный прогон распознан" || bad "OK не распознан: $OUT"
[ "$(run_guard_code --strict "$SHA")" = "0" ] \
    && ok "OK — --strict НЕ роняет на зелёном" || bad "OK — --strict уронил на зелёном (ложняк!)"

# ── 3. STALE: прогонов на хеше нет ───────────────────────────────────────────
# Коммит HEAD старше порога 0 минут → STALE обязан сработать.
make_gh "[{\"name\":\"nova-gate\",\"status\":\"completed\",\"conclusion\":\"success\",\"headSha\":\"deadbeef00\",\"createdAt\":\"2026-08-07T00:00\"}]"
OUT="$(NOVA_CI_STALE_MIN=0 PATH="$TMP/bin:$PATH" bash "$GUARD" "$SHA" 2>&1)"
echo "$OUT" | grep -q "STALE" && ok "STALE — молчащий CI распознан" || bad "STALE не распознан: $OUT"
CODE="$(NOVA_CI_STALE_MIN=0 PATH="$TMP/bin:$PATH" bash "$GUARD" --strict "$SHA" >/dev/null 2>&1; echo $?)"
[ "$CODE" = "1" ] && ok "STALE — --strict роняет" || bad "STALE — --strict не уронил"
# И не лжёт: при огромном пороге тот же расклад ронять не должен.
CODE="$(NOVA_CI_STALE_MIN=999999 PATH="$TMP/bin:$PATH" bash "$GUARD" --strict "$SHA" >/dev/null 2>&1; echo $?)"
[ "$CODE" = "0" ] && ok "STALE — порог уважается (свежий коммит не роняет)" \
                  || bad "STALE — уронил при непройденном пороге (ложняк!)"

# ── 4. SKIP: gh недоступен → exit 0 и явная причина ──────────────────────────
rm -f "$TMP/bin/gh"
OUT="$(PATH="$TMP/bin:/usr/bin:/bin" bash "$GUARD" --strict "$SHA" 2>&1)"
CODE="$(PATH="$TMP/bin:/usr/bin:/bin" bash "$GUARD" --strict "$SHA" >/dev/null 2>&1; echo $?)"
echo "$OUT" | grep -q "SKIP" && [ "$CODE" = "0" ] \
    && ok "SKIP — без gh не роняет и объясняет причину" \
    || bad "SKIP — без gh повёл себя неверно (code=$CODE): $OUT"

# ── 5. ERROR (№585): прогонов нет, а хеш нельзя прочитать локально ──────────
# `git rev-parse` резолвит ref, не проверяя наличие объекта — фальшивый
# 40-символьный SHA даёт ровно ту же форму отказа, что и «объект ещё не
# распакован после гонки на общем .git»: `git show` не находит объект.
FAKE_SHA="0123456789abcdef0123456789abcdef01234567"
make_gh "[{\"name\":\"nova-gate\",\"status\":\"completed\",\"conclusion\":\"success\",\"headSha\":\"deadbeef00\",\"createdAt\":\"2026-08-07T00:00\"}]"
OUT="$(run_guard "$FAKE_SHA")"
CODE="$(run_guard_code "$FAKE_SHA")"
echo "$OUT" | grep -qi "ОШИБКА" && [ "$CODE" != "0" ] \
    && ok "ERROR — недоступный локально объект даёт явный отказ с ненулевым кодом" \
    || bad "ERROR — недоступный объект не распознан как отказ (code=$CODE): $OUT"
echo "$OUT" | grep -q "прогонов на .* пока нет" \
    && bad "ERROR — отказ замаскирован под «прогонов пока нет» (ложный зелёный, ровно баг №585) !!" \
    || ok "ERROR — не маскируется под легитимное «прогонов пока нет»"

if [ "$FAILED" -eq 0 ]; then
    echo "самотест check-ci-status: OK (RED/OK/STALE/SKIP — ловит и не лжёт)"
    exit 0
fi
echo "самотест check-ci-status: ПРОВАЛЕН"
exit 1
