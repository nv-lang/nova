#!/usr/bin/env bash
# Самотест check-novac-legacy-workarounds.py — обе стороны, на фикстурном корне.
...
G="$ROOT/scripts/guards/check-novac-legacy-workarounds.py"
...
OUT=$(NOVA_LEGACY_TBD_TIME="$FRESH" python "$G" "$FIX" 2>/dev/null); RC=$?
...
ERR=$(NOVA_LEGACY_TBD_TIME="$STALE" python "$G" "$FIX" 2>&1 >/dev/null); RC=$?
