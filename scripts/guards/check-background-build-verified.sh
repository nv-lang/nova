...
#
# САМОТЕСТ ИСКЛЮЧЁН ИЗ ПЕРИМЕТРА: его фикстуры ПО ПРИРОДЕ содержат строки
# вида `nohup cargo build …` — это и есть то, что он подкладывает. Детектор
# не должен ловить сам себя: иначе он либо краснеет на себе, либо (хуже)
# его «чинят» ослаблением правила. Тот же приём — в check-staged-secrets.sh.
#
