...
(канал чекера) и types/types.nv (интернер). Пока страж читал два, параллельные
векторы 1:1 в канале он не видел вовсе.
...
def main():
    ...
    if a[2]:
        ...
        s = root / "novac" / "src"
        sem_files = [s / "sem" / "sem.nv", s / "sem" / "channel.nv", s / "types" / "types.nv"]
