...
BADMARK=$(grep -nE "НАЙДЕНО ОХОТНИКОМ" "$REG" 2>/dev/null \
    | grep -vE "НАЙДЕНО ОХОТНИКОМ 20[0-9]{2}-[0-9]{2}-[0-9]{2} \((novac|oracle)\)" \
    | head -3)
if [ -n "$BADMARK" ]; then
    echo "check-hunter-mark: FAIL — строка реестра с меткой без даты (YYYY-MM-DD) или без трека «(novac)»/«(oracle)» после даты:" >&2
