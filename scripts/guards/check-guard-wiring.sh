...
# структуру main и правило ветки. Открывающая строка бывает и с префиксом `u`.
    ...
    FNR == 1 {
        ...
        prevfile = FILENAME; hc = 0; ref = 0; d = 0; inh = 1; dd = 0
    }
    FNR <= 20 {
        if ($0 ~ /^#/) { hc++ }
        else if ($0 ~ /^u?"""/) { d = !d; hc++ }
    ...
    # ГДЕ ИСКАТЬ АДРЕС — В ШАПКЕ, А НЕ ВО ВСЁМ ФАЙЛЕ (2026-09-23, интегратор).
    # Свойство, названное выше, — «шапка … и ссылка на план/реестр», а регулярка
    # ниже до этой правки судила ВЕСЬ файл: адрес в комментарии посреди кода
    # гасил долг. Так и случилось: `check-novac-row-fields` «получил ссылку»
    # строкой 86 («волна E4 (план 274.11)»), и страж посоветовал опустить базу.
    # Замер того же дня: у 14 стражей адрес стоит только в теле, после шапки.
    # Шапка — ВЕДУЩИЙ блок: шебанг, комментарии, пустые строки, докстринг.
    # Не «первые 20 строк»: длинная шапка законна, у этого самого стража адрес
    # стоит в строке 35, внутри шапки.
    inh {
        if (FNR == 1 && $0 ~ /^#!/) { }
        else if (dd) { if ($0 ~ /"""/) dd = 0 }
        else if ($0 ~ /^[ \t]*#/ || $0 ~ /^[ \t]*$/) { }
        else if ($0 ~ /^[ \t]*u?r?"""/) { _q = $0; if (gsub(/"""/, "", _q) < 2) dd = 1 }
        else { inh = 0 }
    }
    ...
    inh && /docs\/plans\/|план [0-9]|реестр 221\.1|реестр №[0-9]/ { ref = 1 }
