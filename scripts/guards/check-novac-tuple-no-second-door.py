...
СЛОВА: особой сущности «кортеж» в коде нет вообще — есть запись на стеке с
формой NamedTupleRecord/AnonymousTupleRecord, а её конструктор есть её
...
ЧТО СЧИТАЕТСЯ НАРУШЕНИЕМ:
  ...
  B. строка КОДА (не комментария), где после удаления разрешённых
     идентификаторов NamedTupleRecord/AnonymousTupleRecord остаётся `tuple`
     в любом регистре — идентификаторы, виды узлов и типов, строковые
     литералы, всё.

ЧТО РАЗРЕШЕНО:
  * комментарии (`//`, `///`) — прозой о кортежах объясняют само правило;
  * идентификаторы `NamedTupleRecord` и `AnonymousTupleRecord` точным
    написанием — варианты формы записи (архитектура §4б).
...
ALLOWED = ("NamedTupleRecord", "AnonymousTupleRecord")
...
def main():
    ...
    if words > base:
        ...
        print("  (NamedTupleRecord/AnonymousTupleRecord), его конструктор — его", file=sys.stderr)
