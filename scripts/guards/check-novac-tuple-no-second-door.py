...
печатает `1`, а novac молча считает `Pair` НЬЮТАЙПОМ над `int` -- оба имени
полей и второе поле отброшены -- и отказывает «constructing a newtype takes
exactly one argument». (Поправка к первой редакции этого текста: тексты
«unknown field» сняты с ФИГУРНОЙ формы `Pair { x: 1, y: 2 }`, не с
канонической вызов-формы; ошибку нашёл агент-трассировщик, сверка
подтвердила.) Дорога кортежа не заведена в путь записи начиная с parse.nv:
хвост объявления уходит в `@type_ref`, скобки читаются плоским прогоном
листьев без единого узла Field, и дальше поля просто неоткуда взять.
