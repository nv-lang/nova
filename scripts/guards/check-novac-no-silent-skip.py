...
# `@reject` и `@reject_*` — ТОЖЕ решения, и добавлены 2026-09-15 по красному гейту,
# который назвал молчанием строку `exprs.nv:425`, стоящую сразу за
# `@reject_first_leaf_of(...)`. Это была ошибка признака, а не кода: список знал
# ИМЕНА дверей, существовавшие на день его написания, и не знал свойства «дверь
# выносит решение». Проверено чтением, а не по имени: `check.nv:266` @reject ->
...
def main():
    ...
    for f in files:
        ...
        # ФУНКЦИЯ НЕ ПЕРЕСЕКАЕТ ГРАНИЦУ ФАЙЛА, значит и состояние не должно
        # (правка 2026-09-15). До неё `inwalk` жил на весь обход и сбрасывался только
        # строкой `fn `. Файл, кончившийся внутри прохода канала, оставлял флаг
        # поднятым, и СЛЕДУЮЩИЙ судился как продолжение чужой функции. Пока в
        # каждом файле была хоть одна `fn`, дефект спал; файл из одних констант
        # (`messages.nv`, 2026-09-15) разбудил его: слово `return` внутри ТЕКСТА сообщения
        # было принято за молчаливый выход из прохода.
        inwalk = False
        hist = []
