...
ЧТО СЧИТАЕТСЯ. Голые чтения: `self.types.get(...)`, `.contains_key(...)`,
обходы карты И передача её ЦЕЛИКОМ (`&self.types`) — последнее тот же
долг, только на шаг дальше: в свободной функции file_id нет по построению. НЕ считается `types_get_for_file(...)` — разрешение по file_id
...
BARE = (
    ...
    # КАРТА, УШЕДШАЯ ЦЕЛИКОМ, — тоже голое чтение, только на шаг дальше.
    # `is_fully_stack_value(&t, &self.types)` отдаёт свободной функции всю
    # last-write-wins карту, и та внутри читает её по ПРОСТОМУ имени, где
    # никакого file_id уже нет по построению. Добавлено 2026-08-19: до этого
    # форма `&self.types` не считалась вовсе, и восемь таких мест в
    # `types/mod.rs` были невидимы. Нашлось не рассуждением — обёртка
    # `TypeTable` заставила переписать один `for … in &self.types` в
    # `.iter()`, счётчик вырос на единицу, и стало видно, что он всё это
    # время не видел саму форму.
    re.compile(r"&(?:self|ctx|type_check_ctx)\.types\b"),
)
