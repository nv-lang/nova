...
План: docs/plans/221.1-bug-sweep.md, строка №1087 (реестр 221.1).
...
`run_guard` не существует. Дальше произошло ровно то, ради чего этот страж:
    ...
    scripts/gate.sh: line 1787: run_guard: command not found
...
НУЛЕВАЯ ТЕРПИМОСТЬ, И ОНА ИЗМЕРЕНА: 199 строк-предметов в двух гейтах (198 до
того, как прибавился шаг самого этого стража),
...
def main():
    ...
    if not bad:
        ...
        if subjects == 0:
            sys.stderr.write(
                "%s: FAIL — в гейт-скриптах (%d) ни одной строки, называющей"
                " файл стража: мишень потеряна, а не «неразрешённых имён 0»"
                " (№911)\n" % (NAME, len(files)))
            return 1
