...
ЧЕТЫРЕ ИСКЛЮЧЕНИЯ, которые страж пропускает сам, — потому что перенести их
нельзя или незачем:
  1. `import ...` — язык не переносит;
  2. образец арма: до `=>` только имена, `|` и пробелы;
  3. одна длинная строковая литера (60+ символов внутри кавычек);
  4. хвостовой `///`-док, когда КОД до него укладывается в предел.
...
def main():
    ...
    if bad:
        ...
        print("  вспомогательное имя или в if-цепочку. Исключения (импорт, образец", file=sys.stderr)
        print("  арма, одна длинная литера, хвостовой ///-док) страж пропускает сам.", file=sys.stderr)
        return 1
