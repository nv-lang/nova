...
# 2026-08-07 (№401b): считаем ВЫЗОВЫ, а не любые вхождения строки. Прежняя форма
# краснела от КОММЕНТАРИЯ про сам легаси-канал: слияние p401b добавило одну строку
# документации `/// \`infer_expr_c_type_legacy\` dispatch chain — …`, ни одного нового
# вызова, и храповик потребовал бампа. Метрика, ложно срабатывающая на объяснение
# собственного предмета, обесценивается — её начинают поднимать не глядя, а наша
# культура требует подробных комментариев-обоснований у каждого маркера.
# Фильтр: отбрасываем строки, которые после trim начинаются с `//` (в т.ч. `///`)
# или с `*` (продолжение блочного комментария).
m_infer=$(grep -n "infer_expr_c_type" "$EMIT"     | sed -e 's/^[0-9]*://'     | sed -e 's/^[[:space:]]*//'     | grep -vc -E '^(//|\*)')
fail=0
