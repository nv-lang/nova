...
Через час к ним добавился план 290 — тот самый, чьим пунктом страж заведён:
его вердикт цитирует фразу «до тега». Занесён в SUBJECT, причина там же.
...
SUBJECT = {
    ...
    # 290 попал сюда ЧЕРЕЗ ЧАС после заведения стража и собственным его
    # вердиктом: он ПИШЕТ о теге (ревизия планов, пункт 8), а не ждёт его.
    # Класс известен — текст о мере попадает в счёт этой меры; лекарство
    # тут только явный список: машина не отличит упоминание от зависимости.
    "290-integrator-shift-2026-09-16.md",
}
