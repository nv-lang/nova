#!/bin/sh
# scripts/guards/check-novac-selftest-proves-red.sh — самотест обязан
# ДОКАЗЫВАТЬ, что его страж ловит (конвенция П16, требование владельца
# 2026-08-16: «страж обязан доказать, что ловит то, что требуется»).
#
# ЗАЧЕМ ИМЕННО МУТАЦИЯ. Самотест, в котором есть только зелёная сторона
# («чистый вход не краснеет»), остаётся зелёным и над стражем, который
# вообще ничего не делает. Заявить «у меня есть красный случай» может любой
# файл; доказать — только опыт. Поэтому проверка одна: подменяем стража
# заглушкой (`#!/bin/sh` + `exit 0`) и гоняем его самотест. Самотест обязан
# упасть. Прошёл над заглушкой — значит он не судит стража, а рапортует.
#
# ПРОВЕРЯЕТ: для каждого scripts/guards/check-novac-*.sh И check-novac-*.py,
#   у которого есть scripts/guards/selftest/test-<имя>.sh ИЛИ test-<имя>.py, —
#   что самотест НАД ЗАГЛУШКОЙ возвращает ненулевой код. Печатает числом,
#   сколько доказали. ОБА РАСШИРЕНИЯ СТРАЖА ОБЯЗАТЕЛЬНЫ: замер 2026-08-19
#   показал 44 стража на python против 18 на shell, и все 44 стояли вне
#   доказательства — их самотесты считались «групповыми» и не краснели.
#   Класс №519: страж потерял мишень при переносе носителей и отрапортовал
#   зелёным. Заглушка пишется НА ЯЗЫКЕ СТРАЖА, иначе подмена не исполнится и
#   «доказательство» будет мусором.
#   ОБА РАСШИРЕНИЯ САМОТЕСТА — ТА ЖЕ ДЫРА, НАЙДЕНА ПОЗЖЕ (2026-09-25, CI
#   `novac-gate` красный три дня): этот файл искал только `test-<имя>.sh` и
#   гонял его через `sh`, поэтому страж с самотестом на python
#   (`check-novac-grammar-kinds` + `test-check-novac-grammar-kinds.py`) читался
#   как «без самотеста» — тот же класс №519, симметричный первому, просто
#   найден на другой стороне пары шестью неделями позже. Самотест теперь
#   ищется в обоих расширениях и гонится нужным интерпретатором.
# НЕ ПРОВЕРЯЕТ: качество зелёной стороны (ложняки — дело самого самотеста и
#   приёмки); стражей вне семьи novac (у них своя норма 254 и свой реестр);
#   групповые самотесты без одноимённого стража (test-check-novac-binary-guards
#   судит четвёрку сразу) — они считаются отдельным числом и не краснеют.
#
# БЕЗОПАСНОСТЬ. Подмена делается по одному файлу за раз, оригинал кладётся
# рядом (<страж>.proving-backup) и возвращается сразу же; trap возвращает
# всё при любом выходе, включая Ctrl-C. В конце страж СВЕРЯЕТ контрольные
# суммы всех тронутых файлов с исходными — расхождение само по себе красное.
#
# $1 — корень репозитория.
# $2 — override каталога стражей (шов самотеста; самотесты берутся из $2/selftest).
# env NOVAC_PROVE=0 — пропустить (дешёвая выборка; гейт гоняет всегда).
# env NOVAC_PROVE_DEADLINE — секунд на один самотест (по умолчанию 150).
# Проверялся: Windows (Git Bash), 2026-08-16.
export LC_ALL=C
# Корень приводится к АБСОЛЮТНОМУ пути: относительный `.` уводил поиск
# бинаря мимо цели, и страж писал «сломан раннер» о здоровом дереве
# (2026-08-18). Ложная краснота стоит дороже отсутствующей проверки:
# по ней идут искать поломку, которой нет, и в стража перестают верить.
# Если cd не удался — значение СОХРАНЯЕТСЯ как было: пустой ROOT судил бы
# корень файловой системы, а это хуже исходной болезни.
ROOT="${1:-$(dirname "$0")/../..}"
ROOT="$(cd "$ROOT" 2>/dev/null && pwd || printf '%s' "$ROOT")"
GUARDS="${2:-$ROOT/scripts/guards}"
SELF="$GUARDS/selftest"
NAME=check-novac-selftest-proves-red
DEADLINE="${NOVAC_PROVE_DEADLINE:-150}"

if [ ! -d "$GUARDS" ]; then
    echo "$NAME ok: судить нечего (нет $GUARDS)"
    exit 0
fi

# ЧУЖАЯ НЕВОССТАНОВЛЕННАЯ ПОДМЕНА — ОТКАЗ ДО ПЕРВОГО ШАГА (2026-09-04). Этот
# страж создаёт `<страж>.proving-backup` на время мутации и убирает его сразу;
# trap возвращает всё при любом выходе. Но процесс можно УБИТЬ так, что trap не
# отработает — и тогда в дереве остаётся ЗАГЛУШЕННЫЙ страж, который печатает
# зелёное, ничего не проверяя. Ровно это и случилось в тот день: агент делал
# мутационную пробу вручную и оставил файл изменённым; нашлось случайно, при
# чтении вывода другого стража. Инвариант «в дереве нет незавершённой подмены»
# принадлежит тому, кто подмены делает, — то есть этому файлу, и держится он
# теперь проверкой, а не надеждой на trap.
LEFTOVER=$(find "$GUARDS" -maxdepth 1 -name '*.proving-backup' 2>/dev/null | head -3)
if [ -n "$LEFTOVER" ]; then
    echo "$NAME: FAIL — в дереве осталась НЕВОССТАНОВЛЕННАЯ подмена стража:" >&2
    printf '%s\n' "$LEFTOVER" | sed 's/^/    /' >&2
    echo "    Это значит, что рядом лежит страж с ЗАГЛУШЕННЫМ телом: он печатает" >&2
    echo "    зелёное и не проверяет ничего. Верни оригинал по имени стража из" >&2
    echo "    git (источник истины, не .proving-backup) и удали остаток." >&2
    exit 1
fi

# Пропуск по NOVAC_PROVE=0 идёт ПОСЛЕ проверки остатков, а не до неё: выборка
# удешевляет ДОКАЗАТЕЛЬСТВО, но не право не заметить заглушенного стража в
# дереве. Первая редакция этой проверки стояла ниже, и при NOVAC_PROVE=0 остаток
# проходил молча — механизм, отключаемый ровно там, где он нужен.
if [ "${NOVAC_PROVE:-1}" = "0" ]; then
    echo "$NAME ok: пропущено по NOVAC_PROVE=0 (остатков подмен нет; гейт гоняет полностью)"
    exit 0
fi

# САМОПОДМЕНА БЕЗОПАСНА ТОЛЬКО ИЗ КОПИИ. sh читает файл ПО МЕРЕ исполнения,
# а не целиком заранее: заглушка на месте СОБСТВЕННОГО файла обрывает цикл
# тихим exit 0 без строки ok: — гейт получает «шаг ничего не доказал», и все
# стражи алфавитно ПОСЛЕ этого имени остаются недоказанными (живой случай
# 2026-08-16, два прогона подряд). Комментарий «процесс уже прочитан» был
# неверной посылкой. Поэтому первым делом перезапускаемся из копии во
# временном файле — тогда подмена своего файла на диске безопасна.
if [ -z "$NOVAC_PROVE_FROM_COPY" ]; then
    C="${TMPDIR:-/tmp}/novac-prove-copy.$$.sh"
    cp "$0" "$C" || { echo "$NAME: FAIL — не удалось скопировать себя для безопасной самоподмены" >&2; exit 1; }
    NOVAC_PROVE_FROM_COPY="$C" exec sh "$C" "$ROOT" ${2:+"$2"}
    echo "$NAME: FAIL — exec копии не состоялся" >&2; exit 1
fi

T="${TMPDIR:-/tmp}/novac-prove.$$"
mkdir -p "$T" || exit 1
restore() {
    [ -n "$NOVAC_PROVE_FROM_COPY" ] && rm -f "$NOVAC_PROVE_FROM_COPY"
    for b in "$GUARDS"/*.proving-backup; do
        [ -f "$b" ] || continue
        mv "$b" "${b%.proving-backup}"
    done
    rm -rf "$T"
}
trap restore 0 INT TERM

PROVED=0
BLIND=""
GROUPED=0
NOSELF=""

JUDGED=0
for g in "$GUARDS"/check-novac-*.sh "$GUARDS"/check-novac-*.py; do
    [ -f "$g" ] || continue
    JUDGED=$((JUDGED + 1))
    case "$g" in
        *.py) base=$(basename "$g" .py); stub='import sys
sys.exit(0)' ;;
        *)    base=$(basename "$g" .sh); stub='#!/bin/sh
exit 0' ;;
    esac
    st="$SELF/test-$base.sh"
    st_py="$SELF/test-$base.py"
    if [ -f "$st" ]; then
        st_run="sh"
    elif [ -f "$st_py" ]; then
        st="$st_py"
        st_run="python"
    else
        NOSELF="$NOSELF $base"
        continue
    fi
    # свой собственный файл судим тоже — БЕЗОПАСНО, потому что исполняется
    # копия из /tmp, а не этот файл (см. перезапуск выше; П16 п.2 — правило
    # без исключений, но исключение чуть не сделал sh своим ленивым чтением)
    cksum < "$g" > "$T/before.$base"
    cp "$g" "$g.proving-backup"
    printf '%s\n' "$stub" > "$g" 2>/dev/null
    # Запись заглушки может не пройти (на Windows файл иногда занят). Молча
    # пропустить нельзя: самотест тогда прогонится против НАСТОЯЩЕГО стража,
    # пройдёт, и мы зачтём доказательство, которого не было. Одна повторная
    # попытка, затем красный (поймано живым прогоном 2026-08-16).
    if [ "$(wc -l < "$g" | tr -d '[:space:]')" != "2" ]; then
        printf '%s\n' "$stub" > "$g" 2>/dev/null
    fi
    if [ "$(wc -l < "$g" | tr -d '[:space:]')" != "2" ]; then
        mv "$g.proving-backup" "$g"
        echo "$NAME: FAIL — не удалось подменить $base заглушкой (файл занят?): доказательство не получено" >&2
        exit 1
    fi
    ( NOVAC_CORPUS=0 NOVAC_SMOKE=0 NOVAC_EMISSION=0 NOVAC_COST=0 NOVAC_PROVE=0 timeout "$DEADLINE" "$st_run" "$st" ) > "$T/out" 2>&1
    rc=$?
    mv "$g.proving-backup" "$g"
    cksum < "$g" > "$T/after.$base"
    if ! cmp -s "$T/before.$base" "$T/after.$base"; then
        echo "$NAME: FAIL — $base не восстановлен после подмены (сверка контрольных сумм)" >&2
        exit 1
    fi
    if [ "$rc" -eq 124 ]; then
        echo "$NAME: FAIL — самотест $base не уложился в ${DEADLINE}с под заглушкой: доказательство не получено" >&2
        echo "  подними NOVAC_PROVE_DEADLINE либо разбей самотест на дешёвую и живую половины (П16 п.4)" >&2
        exit 1
    fi
    if [ "$rc" -eq 0 ]; then
        BLIND="$BLIND $base"
    else
        PROVED=$((PROVED + 1))
    fi
done

# групповые самотесты (без одноимённого стража) — считаем отдельно
for st in "$SELF"/test-check-novac-*.sh "$SELF"/test-check-novac-*.py; do
    [ -f "$st" ] || continue
    case "$st" in
        *.py) b=$(basename "$st" .py) ;;
        *)    b=$(basename "$st" .sh) ;;
    esac
    b=${b#test-}
    [ -f "$GUARDS/$b.sh" ] || [ -f "$GUARDS/$b.py" ] || GROUPED=$((GROUPED + 1))
done

if [ -n "$BLIND" ]; then
    echo "$NAME: FAIL — самотест проходит над ЗАГЛУШКОЙ, значит ничего не доказывает (П16):" >&2
    for b in $BLIND; do echo "  $b — подмени стража на 'exit 0' и убедись сам: самотест обязан упасть" >&2; done
    echo "  почини: впрысни нарушение в подложку (\$2-шов стража) и ассертируй ненулевой код + имя виновника в stderr" >&2
    exit 1
fi
if [ -n "$NOSELF" ]; then
    echo "$NAME: FAIL — страж без самотеста (П16 п.5: одной волной, не «потом»):" >&2
    for b in $NOSELF; do echo "  $b — нет $SELF/test-$b.sh" >&2; done
    exit 1
fi

# МИШЕНЬ УЕХАЛА, А НЕ «ВСЁ ДОКАЗАНО» (класс №911, страж check-guard-empty-root).
# Каталог стражей есть, а стражей в нём ноль: печатать здесь «доказали: 0;
# групповых: 0» значит выдавать пустоту за проверенное — ровно та болезнь,
# ради которой этот страж и живёт, только этажом выше.
if [ "$JUDGED" -eq 0 ] 2>/dev/null; then
    echo "$NAME ok: судить нечего (0 стражей check-novac-* в $GUARDS)"
    exit 0
fi
echo "$NAME ok: самотестов доказали красноту мутацией: $PROVED; групповых (без одноимённого стража): $GROUPED"
exit 0
