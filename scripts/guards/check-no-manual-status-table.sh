...
# `ok:` строчными и с двоеточием — форма, которую ищет обёртка guard() в гейте
# (реестр 221.1 №645, №647): «OK» без двоеточия она за доказательство не
# считает, и правильно делает — искать надо ровно ту форму, о которой уговор.
echo "check-no-manual-status-table ok: рукописных статус-таблиц нет ($hits подозрительных строк, порог $THRESHOLD)"
