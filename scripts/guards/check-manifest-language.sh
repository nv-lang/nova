...
# -- ПОЛ ПО ЧИСЛУ ПРОВЕРЕННОГО (правка 2026-09-08, класс реестра №1041) --
#
# ЧЕГО НЕ ХВАТАЛО: зелёный вердикт не зависел от `N` вовсе. Пустой `$FILES` —
# git отдал ноль, шаблон перестал совпадать, манифесты переехали или
# переименованы — давал `N=0`, цикл не исполнялся ни разу, и страж печатал
# «ok: манифесты по-английски». Проверка, ничего не проверив, выдавала вердикт
# о предмете (Г9). Тот же вид, что №1041 (TSAN-страж объявляет приёмку
# выполненной на пустом выводе) и №1040 (пустая выборка с кодом возврата 0).
#
# ЧТО ЗДЕСЬ УЖЕ БЫЛО СДЕЛАНО ПРАВИЛЬНО, и поэтому НЕ ТРОНУТО: строка 59
# печатает `проверено манифестов $N` ВСЕГДА, и самотест этот счёт проверяет
# (случай 5). То есть немота была ВИДНА в выводе — не хватало только того,
# чтобы она была ОТКАЗОМ. Я сам сперва прочёл файл двумя кусками и решил, что
# счёт не печатается вовсе; это неверно, и записано здесь, чтобы следующий не
# повторил: утверждение об ОТСУТСТВИИ строки требует грепа по всему файлу, а
# не выборочного чтения.
if [ "$N" -eq 0 ]; then
    echo "check-manifest-language: FAIL — проверено НОЛЬ манифестов" >&2
    echo "    Ноль проверенных — это не «нарушений нет», а «предмет не найден»." >&2
    echo "    Список берётся git ls-files по шаблонам *nova.toml/*nova.lock.toml," >&2
    echo "    а без git — обходом find. Манифесты переехали или переименованы —" >&2
    echo "    почини шаблон; их правда нет — скажи это словом, а не зелёной строкой." >&2
    exit 1
fi
