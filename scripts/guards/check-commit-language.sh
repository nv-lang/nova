...
# ИМЕННО --absolute-git-dir: обычный `--git-dir` печатает ОТНОСИТЕЛЬНЫЙ `.git`
# относительно каталога из `-C`, а не относительно cwd процесса. Первая версия
# на этом попалась: самотест звал стража с ROOT=временный-репозиторий, метка
# ушла в `.git` ГЛАВНОГО репо и несла SHA временного — чужой, несуществующий.
# Слепым страж не стал (невалидная метка = полный диапазон), но полный скан
# на нашей истории дольше предела шага — то есть порча метки роняла гейт.
...
# ОДИН вызов git на весь диапазон, а не четыре НА КАЖДЫЙ коммит (2026-08-12).
# Цикл `for sha in $(rev-list)` с четырьмя `git log -1` внутри — это тысячи
# процессов на нашей истории: полный скан шёл 6м40с и не влезал в предел шага
# даже честно (свежий клон, порченая метка). Правила НЕ изменились ни на букву:
#   * норма по ДАТЕ авторства (>= CUT_TS), не по достижимости;
#   * исключения из exempt-файла по SHA;
#   * строки-цитаты (`>`) и отступ-блоки (4 пробела) не проверяются;
#   * встроенные цитаты в обратных апострофах и одинарных кавычках вырезаются:
#     английское сообщение, описывающее русский литерал, обязано его
#     процитировать, иначе оно говорит не о том, о чём говорит.
# Разделители: \x1e между записями, \x1f между полями — в сообщениях коммитов
# этих байтов не бывает (а появись они, страж №620 поймает их раньше нас).
CAND=$(git -C "$ROOT" log --no-merges --format='%x1e%H%x1f%at%x1f%s%x1f%B' "$SCAN_FROM..HEAD" 2>/dev/null \
    | awk -v cut="$CUT_TS" -v exempt="${EXEMPT_FILE:-}" '
        BEGIN {
            # exempt читается ДО смены RS: `getline < файл` делит по RS, и с
            # RS="\x1e" весь файл склеивается в одну запись — в skip попадал
            # только ПЕРВЫЙ sha, остальные 78 исключений молча отваливались.
            # Поймано первым же прогоном: страж закраснел на исключённых.
            if (exempt != "") while ((getline l < exempt) > 0) { split(l, p, " "); skip[p[1]] = 1 }
            RS = "\x1e"; FS = "\x1f"
            q = sprintf("%c", 39)                 # одинарная кавычка
            qre = q "[^" q "]*" q
        }
        {
            if ($1 == "") next
            sha = $1; ats = $2 + 0; subj = $3
            if (ats < cut + 0) next
            if (sha in skip) next
            body = ""
...
HITS=$(printf '%s\n' "$CAND" | grep -Pa '(*UTF8)^[0-9a-f]{40}\x1f.*\x1f.*\p{Cyrillic}' || true)
if [ -n "$HITS" ]; then
    BAD=$(printf '%s\n' "$HITS" | awk -F'\x1f' '{ printf "\n  %s  %s", substr($1, 1, 9), $2 }')
fi
