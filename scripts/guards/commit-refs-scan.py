    ...
    for z in ZONES:
        ...
        if os.path.isfile(p):
            ...
            for base, dirs, fs in os.walk(p):
                # `docs/dev/agent-memory` ИСКЛЮЧЕНА 2026-08-21 — тем же
                # основанием, что и в `check-doc-truth` днём раньше. Это
                # ДОСЛОВНАЯ выгрузка рабочей памяти агента (перенесена
                # 2026-08-20), а не дока: её README прямо говорит, что она не
                # спека, не план и не норма. Две записи покраснили этого стража
                # на ровном месте — `f906bf1c0` (хеш параллельной сессии,
                # недостижимый после вычистки истории 2026-08-13) и адреса
                # зеркал в записи, которая ПРО зеркала и есть. Ни то, ни другое
                # не «ссылка в доке»: летопись цитирует прошлое, а не отсылает
                # к нему читателя. Править выгрузку нельзя — дословность её
                # единственный смысл; поднять базу нельзя тем более — это
                # узаконило бы рост, ради запрета которого храповик и заведён.
                # Значит верно сузить периметр, ровно как вчера.
                dirs[:] = [d for d in dirs
                           if d not in (".git", "target", "agent-memory")]
