...
`scripts/guards/selftest/selftest-honest-count.baseline` (ключ `literal=N`). Храповик ВНИЗ:
...
# ЯКОРЬ `^\s*` БЫЛ ВТОРОЙ СЛЕПОТОЙ ЭТОГО ЖЕ СТРАЖА (2026-09-06, интегратор,
# реестр 221.1 №989). Шапка выше рассказывает, как первая редакция знала одну
# форму вердикта и пропускала семь самотестов; исправлена была СЕРЕДИНА
# шаблона, а начало осталось прежним — и страж пропускал ВЕСЬ корпус. Живые
# самотесты не печатают итог с начала строки, они пишут
#     if [ "$FAILED" -eq 0 ]; then echo "селфтест X: 7/7 ok"; exit 0; fi
# Замер в день правки: страж докладывал 0 самотестов с рукописным числом, а
# их было 22 из 192. Его собственная проба строила фикстуры с `echo` в начале
# строки — то есть проверяла форму, которой в корпусе НЕТ НИ РАЗУ.
#
# ПОЧЕМУ НЕ ПРОСТО СНЯТЬ ЯКОРЬ. Он несущий: им держится случай 6 самотеста —
# `mk "$T/d" "test-x.sh" 'echo "test-x ok: 8/8"'` НЕ нарушение, там `echo`
# стоит аргументом внутри кавычек, а не печатает вердикт. Поэтому `echo`
# опознаётся КАК КОМАНДА: в начале строки либо после разделителя команд
# (`;`, `then`, `else`, `do`, `&&`, `||`, `{`). Внутри кавычек ему
# предшествует кавычка — и он по-прежнему не считается.
