...
if [ "$FAILED" -ne 0 ]; then
    echo "" >&2
    # Одинарные кавычки, а не двойные: в двойных оболочка ИСПОЛНЯЕТ то, что
    # стоит в обратных апострофах, и вместо подсказки печатает
    # `ЧИНИТСЯ: command not found`. Ровно так же ломался шаг gate.sh (2026-08-17).
