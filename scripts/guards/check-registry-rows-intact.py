...
ОБЕ СТОРОНЫ ПРОТИВОРЕЧИЯ «НОМЕР ↔ СПИСОК ДЫР» ДЕРЖАТСЯ ЗДЕСЬ, и сказано это
прямо, потому что половина меры хуже отсутствия меры — она выглядит мерой.
Список `gaps=` и множество номеров со строками обязаны быть СОГЛАСОВАНЫ, и
разойтись они могут в обе стороны:
  * номер БЕЗ строки и НЕ названный в `gaps=` — потерянная строка. Это случай 2
    («ДЫРА в нумерации»), ради которого страж и заводился;
  * номер СО строкой и ПРИ ЭТОМ названный в `gaps=` — протухшее объявление
    дыры. Это случай 2б, добавлен 2026-09-15; до него держалась только первая
    сторона, и вторая делала первую бессильной: пока номер числится законной
    дырой, потеря его строки проходит МОЛЧА — случай 2 видит отсутствие и
    сверяется со списком, где отсутствовать разрешено.
Ни одна из сторон не «главная»: первая ловит пропажу, вторая — то, что снимает
защиту с пропажи. Проверять только первую значит запирать дверь, оставив ключ
в замке.
...
def main():
    ...
    base_stale, err = parse_key(base_t, "stale_gaps", base_file)
    if err:
        return fail(err)
    ...
    # 2б. ПРОТУХШЕЕ ОБЪЯВЛЕНИЕ ДЫРЫ: номер числится законной дырой И имеет
    # строку. Два утверждения об одном номере, противоречащие друг другу, —
    # и машинно различимые, а до 2026-09-15 их не сверял никто.
    #
    # ЧЕМ ЭТО ОПАСНО (формулировка окна b6, найдено при заведении №1117): пока
    # номер числится ЗАКОННОЙ дырой, ПОТЕРЯ его строки проходит МОЛЧА — случай
    # 2 выше видит отсутствие и сверяется со списком, где отсутствовать
    # разрешено. Такая строка лежит ВНЕ защиты храповика, и заметить это можно
    # только чтением базы: прогон зелёный.
    #
    # ПОЧЕМУ РАНЬШЕ НЕ ЛОВИЛОСЬ: снятие номера из `gaps=` при появлении строки
    # держалось на внимании СЛИВШЕГО — то есть ровно на том, что мы везде
    # заменяем механизмом. Замер дня: три номера (1111, 1115, 1116) пролежали
    # так в committed `main`, пока правка, снимающая их, жила в рабочей копии
    # интегратора незакоммиченной.
    #
    # ХРАПОВИК, А НЕ НОЛЬ: страж судит то, что появилось после его заведения.
    # Красный на всей истории отключает первое же окно.
    stale_gaps = sorted(n for n in base_gaps if n in lines_of)
    if len(stale_gaps) > base_stale:
        head = ", ".join("№%d" % n for n in stale_gaps[:20])
        tail = " (и ещё %d)" % (len(stale_gaps) - 20) if len(stale_gaps) > 20 else ""
        problems.append("ПРОТУХШЕЕ ОБЪЯВЛЕНИЕ ДЫРЫ: %s%s — номер назван в `gaps=` базы %s "
                        "законной дырой, а строка с ним в реестре ЕСТЬ. Пока так, потеря "
                        "этой строки пройдёт МОЛЧА: проверка дыр увидит отсутствие и "
                        "сверится со списком, где отсутствовать разрешено. Сними номер из "
                        "`gaps=` ТЕМ ЖЕ ходом, которым пришла строка (их %d, база %d)."
                        % (head, tail, base_file, len(stale_gaps), base_stale))
    ...
    stale_tail = ""
    if len(stale_gaps) < base_stale:
        stale_tail = (" — база устарела, опусти stale_gaps до %d тем же коммитом"
                      % len(stale_gaps))
    print("%s ok: записей %d (база %d), наибольший номер %d (база %d), дублей 0, "
          "законных дыр %d, из них протухших (номер и в `gaps=`, и со строкой) %d "
          "(база %d)%s — ни одна строка не потеряна"
          % (NAME, now_rows, base_rows, now_max, base_max, len(base_gaps),
             len(stale_gaps), base_stale, stale_tail))
    return 0
