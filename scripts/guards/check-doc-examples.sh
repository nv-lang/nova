...
# `mut` ПОСЛЕ `let` — тоже снятая форма (канон: `mut x = ...`), а без
# этой группы класс её НЕ ЛОВИЛ: после `let ` шло `mut`, и регэксп ждал `=`
# сразу за ним, а находил имя. Замерено 2026-09-18: `let mut b = 1` в
# док-примере давал 0, `let b = 1` там же — 1. Дыра была в САМОМ классе,
# то есть и в markdown-периметре тоже, а не только в новом.
re_let='(^|[^A-Za-z0-9_])let[[:space:]]+(mut[[:space:]]+)?[A-Za-z_][A-Za-z0-9_]*[[:space:]]*=|(^|[^A-Za-z0-9_])(if|while)[[:space:]]+let([^A-Za-z0-9_]|$)'
