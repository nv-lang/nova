...
# ЛИБО волна плана снятия подмножества: (274.7 B1b), (274.7 V2). Вторая форма
# заведена 2026-09-05 по решению владельца (274 §1.3а): срок долга теперь — волна
# 274.7, и запись «(274.7 B1b)» называет его точнее этапа. Голая ссылка на план
# без буквы и номера волны сроком НЕ считается — иначе любая ссылка сошла бы.
RE_STAGE = re.compile(r"\(E[0-9][0-9a-zA-Z-]*\)|\(274\.7 [A-Z][0-9]+[a-z]?\)")
