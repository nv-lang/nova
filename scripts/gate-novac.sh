...
# ПРЕДЕЛ ДЛЯ ПАРАЛЛЕЛЬНЫХ СТРАЖЕЙ (заведён 2026-09-16, реестр №1138).
# До него `par_run` звал стражей голым `bash`, без всякого предела: машинерия
# `with-deadline`/`floor_for` работала только на последовательном пути `guard
# --deadline`. Значит единственным ограничителем оставался предел JOB'а на CI —
# 45 минут, — а он не отказ, а ОТМЕНА: проверки не досчитались, и в списке
# job'ов это выглядит тем же крестом. Замер: шаг `novac-heavy` шёл 37 минут и
# не кончился, сборка Карины и её модульные тесты не начинались вовсе.
#
# Число — на ЭТАЛОННОЙ машине, домножается калибровкой, как и всюду здесь.
# Разбор кодов в par_run уже умеет отличить снятие от отказа (124/137/143 —
# «ОБРЫВ, вердикта нет»), так что снятый страж теперь называет СЕБЯ, а не
# превращается в отмену всего job'а.
PAR_DEADLINE_DEFAULT=${NOVAC_PAR_DEADLINE:-600}
...
par_run() {
    ...
    while [ "$_i" -le "$PAR_N" ]; do
        ...
        # в сводку ЗАГОТОВЛЕННОЙ причиной стража («дифф-гейт красный:
        # расхождение поведения вне реестра»). Так оборванный ярус сообщал о
        # расхождении двух компиляторов, ни одного из которых не судили, и
        # звал чинить код, которого никто не мерил (замер 2026-09-15: ровно
        # это случилось с ярусом интегратора).
        #
        # Пустая строка здесь означает «вердикта нет» и отличима от любого
        # числа. Второй признак обрыва — код, каким отвечает убитый процесс:
        # 124 (предел `with-deadline.sh` перекладывает 137/143 в него), 137
        # (KILL) и 143 (TERM). Страж, вышедший так, не вынес вердикта — его
        # сняли.
        _rc=""
        [ -f "$PAR_DIR/$_i.rc" ] && read -r _rc < "$PAR_DIR/$_i.rc"
        case "$_rc" in 124|137|143) _rc="" ;; esac
        read -r _msg < "$PAR_DIR/$_i.msg"
        read -r _cmd < "$PAR_DIR/$_i.cmd"
        _base=${_cmd##*/}
        if [ -z "$_rc" ]; then
            # КРАСНЫЙ, но БЕЗ чужой причины (Г9): обрыв — не вердикт, и
            # называть предмет плохим механизм права не имеет. Красным он
            # обязан быть, иначе превращается в тихий пропуск (Г15).
            echo "ОБРЫВ: $_base снят, не дойдя до вердикта — кода возврата нет" >&2
            echo "  Это НЕ приговор предмету: о нём не узнали ничего (Г16)." >&2
            fail "$_base: ОБРЫВ, вердикта нет — предмет не судили (Г16, №1113)"
        elif [ "$_rc" -ne 0 ]; then
...
par_add "$ROOT/scripts/guards/check-novac-one-branch-head.py" "голову ветвления C печатает не только названная дверь (П21, волна M2b-2)"
