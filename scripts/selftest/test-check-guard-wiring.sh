...
# без отдельного шага в gate.sh: цикл `scripts/selftest/test-*.sh` подхватывает
# этот файл автоматически.
#
# Запуск: scripts/selftest/test-check-guard-wiring.sh
...
GUARD="$REPO_ROOT/scripts/check-guard-wiring.sh"
...
# Собрать игрушечную репу: scripts/ + selftest/ + gate.sh с циклом самотестов.
make_repo() { # каталог
    mkdir -p "$1/scripts/selftest"
    cat > "$1/scripts/gate.sh" <<'EOG'
#!/usr/bin/env bash
for st in "$ROOT"/scripts/selftest/test-*.sh; do bash "$st"; done
EOG
}
...
good_header "$r1/scripts/check-foo.sh" "check-foo.sh"
...
printf '#!/usr/bin/env bash\n# коротко\nexit 0\n' > "$r2/scripts/check-bar.sh"
touch "$r2/scripts/selftest/test-check-bar.sh"
...
{ printf '#!/usr/bin/env bash\n'; for i in $(seq 1 10); do printf '# строка шапки %s\n' "$i"; done; printf 'exit 0\n'; } > "$r3/scripts/check-baz.sh"
touch "$r3/scripts/selftest/test-check-baz.sh"
...
good_header "$r4/scripts/check-good.sh" "check-good.sh"
touch "$r4/scripts/selftest/test-check-good.sh"
