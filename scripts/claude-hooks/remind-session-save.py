...
CARD_ROLES = {
    ...
    "integrator": (os.path.join("docs", "dev", "prompts", "integrator-handoff.md"),
                   "/save", "ЗАПИСКА ИНТЕГРАТОРА"),
    # Помощник — только по ВИЗИТКЕ, и в `ROLES` (таблице веток) его нет
    # намеренно: он работает в ветке ПОД ЗАДАЧУ (`registry/…`, `guards/…`,
    # `docs/…`), и опознать роль по ветке нельзя даже в принципе.
    "assistant": (os.path.join("docs", "dev", "prompts", "assistant-handoff.md"),
                  "/save", "ЗАПИСКА ПОМОЩНИКА"),
}
...
def main():
    ...
    if card and card not in CARD_ROLES:
        # ВИЗИТКА НАЗЫВАЕТ РОЛЬ, КОТОРОЙ ЗДЕСЬ НЕТ — говорим ВСЛУХ (требование
        # интегратора 2026-09-20). Это НЕ то же самое, что незнакомая ВЕТКА:
        # незнакомая ветка — законный случай (пакетное окно, разовая волна), и
        # молчание там решено сознательно. А визитка заводится РУКОЙ, и опечатка
        # в имени роли молча выключает механизм для того, кто визитку и написал:
        # он видит файл на месте, а напоминание не приходит никогда.
        emit_context(
            u"ВИЗИТКА НАЗЫВАЕТ НЕИЗВЕСТНУЮ РОЛЬ `%s` — напоминание о записке "
            u"для неё не настроено.\n  Известные роли: %s.\n  Либо опечатка в "
            u"`scripts/tools/session-card.sh write <роль> …`, либо роль новая и "
            u"её надо\n  внести в `CARD_ROLES` этого хука и в `ROLE_NOTE` "
            u"`guard-stop-v2.py` — ОДНИМ слиянием."
            % (card, u", ".join(sorted(CARD_ROLES))))
        return 0
