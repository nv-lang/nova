...
#   СВЕЖЕСТЬЮ ФАЙЛА ПЕРЕДАЧИ этой сессии (`docs/.sessions/handoff-<id>.md`),
#   а НЕ свежестью ролевой записки — та свежа после любой работы; тем, что
...
ROLE_NOTE = {
    ...
    # Помощник — четвёртая роль (владелец 2026-09-19, записка заведена
    # 2026-09-20). До этой строки код `смена` был ему НЕДОКАЗУЕМ: записка
    # существовала, а механизм о ней не знал — то есть роль могла сдать смену
    # только кодом «очередь-пуста», и отказ выглядел как её ошибка.
    u"assistant": u"docs/dev/prompts/assistant-handoff.md",
}
...
def handoff_file(cwd, sid=None):
    u"""Файл передачи ЭТОЙ сессии — артефакт команды `/stop`.

    ПОЧЕМУ НЕ РОЛЕВАЯ ЗАПИСКА (правка 2026-09-20, найдено владельцем).
    Свежесть записки признак СОСЕДНИЙ: её обновляют по часовому напоминанию
    посреди обычной работы, и условие сходилось само — окно сказало «смена»,
    не сдав смены, и хук пропустил. Файл передачи обычной работой не
    появляется: его пишет только `/stop`. Это и есть предмет.
    """
    sid = (sid or os.environ.get(u"CLAUDE_CODE_SESSION_ID") or u"").strip()
    if not sid:
        return None
    return os.path.join(cwd or u".", u"docs", u".sessions",
                        u"handoff-%s.md" % sid)


def handoff_is_fresh(cwd, sid=None):
    p = handoff_file(cwd, sid)
    if not p:
        return False
    try:
...
def main():
    ...
    if code == u"очередь-пуста":
        ...
        if role not in ROLE_NOTE:
            # Роль, которой в таблице нет вовсе. С 2026-09-20 это уже НЕ
            # помощник (у него записка есть) — сюда попадает имя, выставленное
            # рукой через `NOVA_WINDOW_ROLE`, то есть чаще всего ОПЕЧАТКА.
            # Отказ называет и роль, и известные имена: молчаливый отказ здесь
            # читался бы как «код плох», тогда как плохо имя роли.
            ok, reason = False, (
                u"Код «смена» у роли «%s» не доказуем: ролевой записки у неё нет. "
                u"Известные роли: %s. Если имя роли написано рукой — это опечатка; "
                u"если роль новая, её записку надо внести в `ROLE_NOTE` этого хука "
                u"и в `CARD_ROLES` `remind-session-save.py` ОДНИМ слиянием. "
                u"Пока роль неизвестна, законный код — «СТОП: очередь-пуста»: "
                u"снимок проверит, что незакоммиченного не осталось."
                % (role, u", ".join(sorted(ROLE_NOTE)))
            )
        elif not handoff_is_fresh(cwd, data.get("session_id")):
            # Судим АРТЕФАКТ СДАЧИ, а не свежесть записки: записка свежа после
            # любой обычной работы, файл передачи — только после `/stop`.
            hp = handoff_file(cwd, data.get("session_id"))
            ok, reason = False, (
                u"Код «смена» без сданной смены: файла передачи этой сессии нет "
                u"или он старше %d минут (%s). Сдача смены — это ЗАПИСЬ, а не "
                u"слово: выполни `/stop` (он пишет передачу пятью строками), "
                u"потом заканчивай ход. Ролевая записка доказательством НЕ "
                u"считается с 2026-09-20: её обновляют по часовому напоминанию "
                u"посреди работы, и условие сходилось само."
                % (NOTE_FRESH_SEC // 60,
