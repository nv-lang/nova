...

{
  ...
  echo
  echo 'Источник правды — строка `**Статус:**` в самом файле плана. Этот файл'
  echo 'только собирает их в одну таблицу; редактировать руками бессмысленно —'
  echo 'следующий запуск генератора перезапишет его.'
  echo
