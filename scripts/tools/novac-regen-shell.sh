...
if [ "$MODE" = check ]; then
    ...
    # Оракул приписывает контракту std ЧУЖОЙ файл и номер, которого в этом файле
    # нет: в эмиссии probe (56 строк) стоит `"shell_probe.nv", 57`. Число к тому
    # же разное на разных платформах — здесь 46, на CI 45, при побайтно
    # одинаковых исходниках компилятора, std и самого probe. Значит снимок не
    # может быть свеж одновременно на обеих платформах, и страж краснел бы на CI
    # не по своей вине. Запись — docs/plans/274.12-novac-divergences.md, «ПОЗИЦИЯ
    # КОНТРАКТА std приписана чужому файлу»; починка идёт каналом 196.
    # Терпимость УЗКАЯ: гасится ровно числовой аргумент внутри
    # nova_contract_violation(...), всё остальное по-прежнему байт-в-байт. Как
    # только оракул починят, эти три строки снимаются — и разница снова станет
    # красной.
    sed 's/\(nova_contract_violation.*\.nv", \)[0-9][0-9]*/\1<POS>/g' "$TPL" > "$T/tpl.pos"
    sed 's/\(nova_contract_violation.*\.nv", \)[0-9][0-9]*/\1<POS>/g' "$T/shell.tpl.c" > "$T/new.pos"
    if cmp -s "$T/tpl.pos" "$T/new.pos"; then
        echo "novac-regen-shell --check ok: шаблон совпал с эмиссией оракула по probe ($LINES строк);"
        echo "  различие ТОЛЬКО в номерах строк контрактов — известный дефект оракула"
        echo "  (docs/plans/274.12-novac-divergences.md, замер 2026-08-23; канал 196)."
        exit 0
    fi
