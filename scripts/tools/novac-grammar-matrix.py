...
FORMS = [
    ...
    # БЕЗ импорта: с ним строка мерила отказ про `import`, а не замыкание
    # (2026-08-18, как только import начал отказываться по имени). Проба,
    # которую заслоняет чужой отказ, показывает зелёное там, где ничего не
    # проверено, — тот же класс, что и ложная причина в самой диагностике.
    # Vec.sort_by берёт замыкание и живёт в прелюдии.
    
    # ЗАСЛОНЕНО, и это записано честно: сегодня строку судит отказ про
    # fn-ТИП в сигнатуре (`f fn(int) -> int`), а не про само замыкание.
    # Отличие от прошлых двух редакций в том, что отказ теперь ИМЕНУЕТ
    # форму: строка меряет ближайшую нераскрытую форму, а не притворяется,
    # будто проверила замыкание. Развернётся сама, когда fn-типы дойдут.
    #
    # Ни массива, ни импорта: обе прошлые редакции этой строки мерили ЧУЖОЙ
    # отказ (сначала про `import`, потом про массив) и показывали работу там,
    # где ничего не проверено. Замыкание-«лайт» с нетипизированным параметром
    # передаётся в fn-тип — оракул принимает, и заслонить это нечем.
    ("closure_pipe", "B",
     'fn apply(f fn(int) -> int, v int) -> int => f(v)\n\n'
     'fn main() { println(apply(|x| x + 1, 2)) }\n'),
