...
# 3. ярус по путям
#
# БАЗА ДЛЯ ЯРУСА — НЕ ТА ЖЕ, ЧТО ДЛЯ СЧЁТА НЕПУШЕННЫХ (реестр 221.1 №1074,
# нашло окно 274 2026-09-13, чинит интегратор). `UP=origin/$BR` отвечает на
# вопрос «чего нет в моей отправленной копии» — и для пунктов 1 и 2 выше это
# ВЕРНО. Для яруса вопрос другой: «что из этого ЕЩЁ НЕ СУДИЛИ». Две величины
# расходятся ровно тогда, когда ветку влили в main и ушли вперёд: указатель
# `origin/$BR` остаётся на месте, а её содержимое давно в main.
#
# ЗАМЕР ОКНА 274 (их дерево, 2026-09-13): в `origin/p274-novac..HEAD` — 441
# файл и 121 исходник вне `novac/`, то есть `MAIN_TIER=push`; в
# `origin/main..HEAD`, то есть в их НОВОЙ работе, таких исходников НОЛЬ.
# Оба числа верны, просто от разных точек отсчёта. Цена ошибки процессная и
# несимметричная: ярус `push` — это мега-CU и полный `nova test`, а
# `AGENTS.md` (Gates) закрепляет их за интегратором. То есть инструмент,
# которым окну ВЕЛЕНО пушить, механически заводил окно в действие, ему
# запрещённое.
#
# ПОЧЕМУ ПРАВКА НЕ ОСЛАБЛЯЕТ ГЕЙТ, хотя СУЖАЕТ набор файлов. Судится не
# «меньше», а «своё»: содержимое, уже лежащее в `origin/main`, — не работа
# этого окна, и его повторное взвешивание есть суждение о ЧУЖОЙ работе,
# принятой другим прогоном. Именно поэтому правку делает интегратор, а не
# окно, которому она экономит двадцать минут: это тот случай, где выгода и
# полномочие обязаны лежать в разных руках.
#
# БАЗ ДВЕ, И БЕРУТСЯ ОБЕ. Судить надо то, чего нет НИ в отправленной копии
# ветки (`$UP` — судилось при её пуше), НИ в `origin/main` (судилось при
# слиянии). Односторонняя правка «считать только от main» была бы неверна
# зеркально: ветка, запушившая работу, которая в main ещё не влита, получила
# бы эту работу в ярус повторно.
TIER_BASES="$UP"
if git rev-parse --verify -q origin/main >/dev/null 2>&1; then
    TIER_BASES="$TIER_BASES origin/main"
fi
# `--not A B` = коммиты, достижимые из HEAD и НЕ достижимые ни из A, ни из B.
CHANGED=$(git log --format= --name-only HEAD --not $TIER_BASES | sort -u | grep -v '^$' || true)
