...
# абсолютному пути.
#
# ВИЗИТКА — ПОДСКАЗКА, А НЕ ВЛАСТЬ (решение владельца 2026-09-16). Роль
# подтверждает САМО окно контрольным вопросом-ответом («кто тут интегратор
# Карины?» -> «я интегратор Карины»), протокол — в `.claude/commands/peers.md`.
# Причина: визитку пишет кто угодно и она переживает смерть своего окна, а
# ответ даёт живая сессия и подтверждает разом и роль, и то, что она жива.
#
# ЗАЧЕМ ТОГДА ВИЗИТКА. Она экономит круг: вместо вопроса ВСЕМ соседям можно
# спросить одного — последнее известное имя роли. Разошлись визитка и ответ —
# верен ОТВЕТ, визитка переписывается.
#
