#!/usr/bin/env bash
# scripts/tools/repo-hygiene.sh — ежедневная уборка репозиториев семьи Nova
# (каталог, в котором `nova` лежит сиблингом остальным; выводится от скрипта).
#
...
# Корень ВЫВОДИТСЯ от расположения скрипта (2026-08-23): каталог, в котором
# репозиторий `nova` лежит сиблингом остальным. Литеральный путь к машине автора
# в публичном скрипте — тот самый класс №698, против которого заведён
# check-no-machine-paths. Переопределяется аргументом КОРЕНЬ.
...
# Решение владельца 2026-08-12: удаление ЗАМЕРШИХ НЕВЛИТЫХ веток делается
# только вручную и со списком перед глазами; крон ходит в безопасном режиме
# (влитые + gc). Задача планировщика уже переведена, но задача — это настройка,
# а настройку возвращают обратно одним щелчком. Поэтому запрет стоит ЗДЕСЬ:
# без человека за терминалом `--purge-stale` не работает вовсе.
if [ "$PURGE" -eq 1 ] && [ ! -t 0 ] && [ "${NOVA_HYGIENE_FORCE_PURGE:-0}" != "1" ]; then
    echo "repo-hygiene: --purge-stale без человека за терминалом ОТКЛОНЁН." >&2
    echo "  Удаление невлитой ветки уничтожает работу, которой нет больше нигде," >&2
    echo "  и делать это молча по расписанию нельзя (решение владельца 2026-08-12)." >&2
    echo "  Запусти вручную и посмотри список; либо, если это осознанный скрипт," >&2
    echo "  выставь NOVA_HYGIENE_FORCE_PURGE=1 — тогда ответственность на том," >&2
    echo "  кто это написал." >&2
    exit 2
fi
