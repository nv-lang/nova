...
# ЧТО ЭТА СХЕМА СТОИТ, И КАК ЭТО СНИМАЕТСЯ — вопрос владельца 2026-09-16
# («фаззер каждый раз будет делать новые испорченные файлы — это нормально?»).
#
# ТОЧНОСТЬ ПЕРВОЙ ВАЖНОСТИ: новых файлов он НЕ делает. Набор фиксирован и
# побайтово один и тот же; меняется лишь то, КАКУЮ ЧАСТЬ набора успели
# просудить за бюджет. Зерно тасует порядок, бюджет отрезает хвост.
#
# ЦЕНА, названная прямо: зелёный прогон больше НЕ значит «на этом коммите
# фаззер чист» — он значит «чист просуженный срез». Отсюда две ловушки:
#   * регрессия может проскочить несколько пушей, пока не попадёт в срез;
#   * краснота может выпасть на коммите, который её НЕ вызывал: сменился срез,
#     а не код. То есть по красноте нельзя обвинять последний диff.
#
# ПРОЦЕДУРА, КОТОРАЯ СНИМАЕТ ОБЕ (без неё схема негодна). Зерно берётся от
# хэша коммита, значит краснота ВОСПРОИЗВОДИМА и ПРОВЕРЯЕМА на причастность:
#
#   1) повторить на том же коммите:
#      NOVAC_FUZZ_SEED=<хэш> sh scripts/tools/novac-fuzz-mutations.sh
#   2) прогнать ТЕМ ЖЕ зерном на РОДИТЕЛЬСКОМ коммите:
#      git stash-free checkout родителя, затем NOVAC_FUZZ_SEED=<тот же хэш>
#      — зелено у родителя значит РЕГРЕССИЯ (виноват диff),
#      — красно у родителя значит дефект был и раньше, просто не попадался.
#
# Без шага (2) красный фаззер обвинял бы случайный коммит — это хуже, чем
# отсутствие проверки, потому что выглядит уликой.
#
# БЮДЖЕТ ВРЕМЕНИ И ЗЕРНО (решение владельца 2026-09-16: «на фаззер отведи 60 с,
# не больше, сколько он успеет попортить исходников, столько и берём»).
#
# ЗЕРНО ОБЯЗАТЕЛЬНО ВМЕСТЕ С БЮДЖЕТОМ, и это замер, а не осторожность: набор
# мутаций ДЕТЕРМИНИРОВАН — два прогона дают побайтово одинаковые файлы И
# одинаковый порядок (проверено `diff -rq`). С одним лишь бюджетом мы вечно
# судили бы первые случаи списка, а хвост не увидели бы НИКОГДА: это не
# выборка, а постоянная слепая зона. Зерно от хэша коммита даёт обе нужные
# вещи: на ОДНОМ коммите порядок воспроизводим (находка повторяется командой),
# на РАЗНЫХ — срез другой, и покрытие копится по пушам.
FUZZ_SEED="${NOVAC_FUZZ_SEED:-$(git -C "$ROOT" rev-parse --short HEAD 2>/dev/null || echo nogit)}"
FUZZ_BUDGET="${NOVAC_FUZZ_SECONDS:-60}"
# ПАЧКА ДРОБИТСЯ ПОД МАЛЫЙ БЮДЖЕТ. При 150 файлах пачка стоит ~25с, и в
# половину минутного бюджета влезает РОВНО ОДНА — замер: судимых 150 из 4570
# при стенке 77с. Мелкая пачка даёт ГРАНУЛЯРНОСТЬ вместо пропускной
# способности: запусков больше, зато бюджет расходуется ровнее и перебор
# меньше. При большом бюджете (ночной ярус) остаётся крупная — там важнее
# throughput, и ради него пачку и заводили (замер окна 274: 150 файлов —
# секунды против минуты по одному).
FUZZ_T0=$(date +%s)
[ "${FUZZ_BUDGET:-60}" -le 300 ] && CHUNK=40
# БЮДЖЕТ ДЕЛИТСЯ ПОПОЛАМ МЕЖДУ ДВУМЯ ФАЗАМИ, и это не аккуратность, а находка
# первого же прогона: при общем бюджете `check` съедал его целиком и вердикт
# честно печатал «эмиссией 0» — то есть вторая фаза не судилась НИ РАЗУ. А
# эмиссия — ровно та стадия, которую эта приёмка не осматривала годами (см.
# длинный комментарий выше). Голодающая вторая фаза — это снова «проверка не
# доходит до своего предмета», только теперь по нашей же вине.
fuzz_left() { echo $(( (FUZZ_BUDGET / 2) - ( $(date +%s) - FUZZ_T0 ) )); }
fuzz_left_emit() { echo $(( FUZZ_BUDGET - ( $(date +%s) - FUZZ_T0 ) )); }

GEN="$ROOT/scripts/tools/novac-fuzz-gen.py"
[ -f "$GEN" ] || { echo "novac-fuzz: нет генератора $GEN" >&2; exit 2; }
n=$(python "$GEN" "$CORPUS" "$T/cases" "$STEP" "$FUZZ_SEED" "$T/list") || {
...
# Список УЖЕ написан генератором и перемешан зерном — пересортировать его
...
judge_emit() {   # $1 = list file; 0 = zelyono, 1 = chto-to umerlo pri emissii
    ...
    while IFS= read -r case_nv; do
        [ -n "$case_nv" ] || continue
        # Здесь бюджет судится ПО КАЖДОМУ случаю, а не по пачке: `novac emit`
        # берёт один файл, дробить нечего, и перебор сводится к одному случаю.
        [ "$(fuzz_left_emit)" -le 0 ] && break
        _ei=$((_ei + 1))
...
red=""
judged=0
# ПЕРЕБОР БЮДЖЕТА ОГРАНИЧЕН ПО КОНСТРУКЦИИ, и вот чем именно: пачка — ОДИН
# неделимый вызов Карины, прервать её нельзя, поэтому бюджет проверяется
# МЕЖДУ пачками. Чтобы перебор не был случайным, следующая пачка не
# начинается, если остатка меньше, чем стоила ПРОШЛАЯ: это грубое, но честное
# предсказание — пачки одного размера и цена их близка. Замер до этой правки:
# бюджет 60с, стенка 113с.
_last=0
for c in "$T"/chunk.*; do
    _left=$(fuzz_left)
    [ "$_left" -le 0 ] && break
    [ "$_last" -gt 0 ] && [ "$_left" -lt "$_last" ] && break
    _t0=$(date +%s)
    judge "$c" || red="$red $c"
    _last=$(( $(date +%s) - _t0 ))
    judged=$(( judged + $(wc -l < "$c") ))
done
if [ -z "$red" ]; then
    ...
    for c in "$T"/chunk.*; do
        ...
        if [ "$(fuzz_left_emit)" -le 0 ]; then break; fi
        emit_judged=$(( emit_judged + $(wc -l < "$c") ))
        judge_emit "$c" || {
...
"
    ...
    rm -rf "$T"
    # ВЕРДИКТ НАЗЫВАЕТ ДОЛЮ, А НЕ ТОЛЬКО ЗЕЛЁНОЕ. «Судил 300 из 5500» и
    # «судил все» — разные утверждения, и путать их нельзя: зелёная строка без
    # доли читается как «проверено всё», хотя проверена минута работы.
    _wall=$(( $(date +%s) - FUZZ_T0 ))
    echo "novac-fuzz ok: судил $judged из $n мутаций (эмиссией $emit_judged), бюджет ${FUZZ_BUDGET}с, стенка ${_wall}с, зерно $FUZZ_SEED, шаг $STEP, $srcn файлов корпуса; паник 0"
    exit 0
fi
