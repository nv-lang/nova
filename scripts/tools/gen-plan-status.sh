...
# в docs/dev/conventions-governance.md; этот скрипт и есть тот генератор,
...
for f in "$PLANS_DIR"/*.md; do
  ...
  case "$base" in
    ...
    *-notes.md|*-progress.md|*-execution-plan.md|*-session*.md)
      ...
      ;;
    *-history.md)
      # АРХИВ дословных текстов, свёрнутых из плана при ревизии (пример —
      # 221-history.md). Собственного статуса у архива нет и быть не должно:
      # статус живёт в самом плане. Добавлено 2026-08-04 — страж
      # check-doc-conventions.sh (ветка p242) обязан нести ТОТ ЖЕ фильтр,
      # иначе счётчик plan_missing_status и STATUS.md разойдутся.
      continue
      ;;
  esac
