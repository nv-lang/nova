...
# СВЕЖЕСТЬ БИНАРЯ — ОТКАЗ, А НЕ ПРЕДУПРЕЖДЕНИЕ (заведено 2026-09-20).
# Скрипт мерит РАССТОЯНИЕ ДО САМОСБОРКИ, то есть до тега Карины, и делал это
# тем бинарём, какой лежит: проверялось только НАЛИЧИЕ. В гейте свежесть
# проверяет `gate-novac.sh` (бинарь обязан быть новее всех `.nv` и оракула),
# а при запуске руками — тем же скриптом, тем же именем — не проверял никто.
# Замер 2026-09-20: в главном дереве лежал `novac.exe` от 17-го, при исходниках
# от 19-го и 20-го; число, снятое им, относилось к позавчерашнему компилятору и
# ничем себя не выдавало. Родня — реестр №1029 (значение есть свойство пары
# «дерево + среда»): здесь среда — это то, что кто-то оставил в `target/`.
STALE_NV=$(find "$ROOT/novac/src" -name '*.nv' -newer "$NOVAC" 2>/dev/null | head -n 1)
if [ -n "$STALE_NV" ]; then
