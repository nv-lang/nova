...
if git rev-parse --verify -q origin/main >/dev/null; then
    ...
    echo "против origin/main: отстаю на $(printf '%s' "$LR" | awk '{print $1}'), впереди на $(printf '%s' "$LR" | awk '{print $2}')"
fi
...
if [ -z "$PLAN" ]; then
    ...
    if [ -z "$F" ]; then
        ...
        echo "статус-строки:"
        grep -n -a -E '^\*\*[^*]*(\xd0\xa1\xd1\x82\xd0\xb0\xd1\x82\xd1\x83\xd1\x81|Status)[^*]*\*\*' "$F" | head -3 | cut -c1-160 | sed 's/^/    /'
        echo "живые строки (СДЕЛАНО / живая строка):"
        grep -n -a -E '\xd0\xb6\xd0\xb8\xd0\xb2\xd0\xb0\xd1\x8f \xd1\x81\xd1\x82\xd1\x80\xd0\xbe\xd0\xba' "$F" | head -4 | cut -c1-160 | sed 's/^/    /'
        echo "последние закрытия (ЗАКРЫТ <дата>):"
        grep -n -a -E '\xd0\x97\xd0\x90\xd0\x9a\xd0\xa0\xd0\xab\xd0\xa2[^0-9]{0,4}20[0-9]{2}-[0-9]{2}-[0-9]{2}' "$F" | tail -4 | cut -c1-160 | sed 's/^/    /'
...
if [ -f "$REG" ]; then
    echo "строк без номера (TBD): $(grep -cE '^\| *(\xe2\x84\x96)?TBD *\|' "$REG" || true)"
