...
echo
echo "== ПОБЕГИ STOP-СТРАЖА (план 292 Ш.5) =="
# Побег — это случай, когда страж остановки НЕ СМОГ судить и отпустил окно:
# снимок очереди неполон, вход не разобран, третья блокировка подряд. Решение
# верное (запирать окно за чужую мёртвую сеть нельзя), но лог, которого никто
# не читает, равен отсутствию лога. Второй смысл появился 2026-09-19, когда
# страж подключили ко всем окнам: побеги показывают и то, ГДЕ ОН ОШИБАЕТСЯ, —
# без них о его неправоте узнать неоткуда.
if [ -f scripts/tools/stop-escapes.py ]; then
    python scripts/tools/stop-escapes.py . || true
else
    echo "    scripts/tools/stop-escapes.py нет — побеги не прочитаны"
fi
...
# ЗАБЫТЫЕ ФОНОВЫЕ ПРОЦЕССЫ — только перечень (поручение владельца 2026-09-23,
# пункт 5): в тот день вычищено пятнадцать забытых `tail -f`, и узнали о них
# случайно. Вход в сессию — то место, где их видно без поиска. Убивать здесь
# нечего и нечем: процесс бывает чужим живым сервером, снимает его то окно,
# которое узнаёт свою команду.
echo
echo "== ФОНОВЫЕ ПРОЦЕССЫ (перечень, не снимаются) =="
bash "$(dirname "${BASH_SOURCE[0]}")/stale-processes.sh"
