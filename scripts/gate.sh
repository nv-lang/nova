...
step loop "hunter-mark (мера охотника считается грепом, а не памятью — план 278 Ф.2)"
# Числитель — метки «НАЙДЕНО ОХОТНИКОМ <дата>» в реестре, знаменатель —
# строки «НАХОДКА |» в отчётах docs/dev/hunts/. Красное: метка без даты,
# отчёт-молчун (ни находок, ни «НИЧЕГО НЕ НАШЁЛ» — №770), метка без
# отчёта-источника. Ноль охот — законное 0/0.
guard "$ROOT/scripts/guards/check-hunter-mark.sh" "$ROOT" || fail "мера охотника (278 Ф.2) не считается: метка без даты, отчёт-молчун или метка без источника"

step loop "hunter-coverage (сетка охоты выведена из дерева; неохваченное — только вниз — план 278 Ф.3)"
# Ось модулей — таблица рёбер §3 novac-architecture.md; классы К1..К7; факт
# охоты — строка «КЛЕТКА | <модуль> | К<n>» в отчёте. Отказ на непонятой
# форме (урок №801), рукописной сетки в дереве нет, база засеяна сегодняшним
# числом (98) — храповик вниз.
guard python "$ROOT/scripts/guards/check-hunter-coverage.py" "$ROOT" || fail "карта охоты (278 Ф.3): отчёт с непонятой клеткой, модуль мимо таблицы рёбер, рукописная сетка или рост неохваченного над базой"
