...
**A. Char-литералы (`'c'`)** — 2 файла: complex.nv, json.nv. В spec нет
   решения о char/byte литералах. Парсеры строк используют их для
   проверки одного символа. Возможно, нужно D-решение про char/byte
   литералы или альтернатива через `s.char_at(i)` + сравнение строк.
