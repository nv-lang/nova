...
- **Оставшийся предел — высокая ОДНОВРЕМЕННАЯ параллельность соединений**
  (`[M-187-high-concurrency-connection-wedge]`, OPEN, эскалирован): ~20
  одновременных держит, 40 — backpressure с восстановлением, ~80 — сервер
  виснет насмерть (архитектурный M:N: нет work-stealing/преемпции для nested
  `supervised` под пиком). Для одного браузер-таба неактуально; воспроизводится
  только массовой параллельной атакой (`loadtest.ps1 -Concurrency 80`).
  Нагрузочный тест — `loadtest.ps1` (самодостаточный: сам собирает/поднимает/
  глушит сервер, 7 блоков).
