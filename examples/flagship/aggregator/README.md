...
- **СЕРЬЁЗНО: сервер часто (не «редко») виснет на 2-3-ем последовательном
  запросе к живому процессу** (`[M-187-supervised-nested-fiber-slot-race]`,
  честно задокументировано, НЕ обойдено полностью — только смягчено) —
  вероятностная гонка в fiber-scheduler'е (`nova_rt/fibers.h`) на ГЛУБОКО
  вложенных `supervised`-скоупах (accept-loop → per-connection scope →
  `aggregate`'s `parallel for` → `fetch_guarded`'s `supervised(deadline:)`),
  обнаружена в этой сессии. Симптом: запрос виснет НАВСЕГДА (curl с
  таймаутом 10с не дожидается ответа), в stderr сервера — `NOVA_RUNTIME_DUMP`
  (`pending_remote=1` — слот дочернего скоупа не помечен завершённым; это
  СОБСТВЕННЫЙ watchdog рантайма, не просто «долго» — он специально detect'ит
  зависшие слоты). Частота колебалась в течение сессии (иногда виснет на
  2-ом запросе, иногда доживает до 5-10), похоже коррелирует с общей
  загрузкой машины (параллельно шли другие `nova`-сборки в других worktree),
  но САМ watchdog срабатывает при ЛЮБОЙ загрузке — не спишешь на «просто
  медленно». Не деталь этого пакета — глубина такой вложенности нигде
  больше в кодовой базе не тестировалась. Митигация (само-контейнерный
  `supervised{spawn{...}}` на КАЖДОЕ соединение вместо голого вызова)
  снижает частоту относительно ещё более простой формы, не убирает гонку.
  Единственный ОДИНОЧНЫЙ запрос к свежезапущенному процессу — 100% надёжен
  (каждый успешный ответ в этом README — реальный вывод, ничего не
  подделано); при зависании — перезапустить бинарь. Это реальное,
  некосметическое ограничение текущего прогона, а не полировка.
